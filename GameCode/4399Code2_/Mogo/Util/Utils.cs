namespace Mogo.Util
{
    ...
    public static class Utils
    {
        ...
        public static void CompressDirectory(this Stream target, string sourcePath, int zipLevel, bool needPwd)
        {
            ...
            using (ZipOutputStream stream = new ZipOutputStream(target))
            {
                ...
                if (needPwd)
                {
                    stream.Password = GetResNumber().PackArray<byte>(',');
                }
                foreach (string str in list)
                {
                    ...
                    if (!str.EndsWith(@"\"))
                    {
                        ...
                        using (FileStream stream2 = System.IO.File.OpenRead(str))
                        {
                            int num2;
        ...
        public static void CompressDirectory(string sourcePath, string outputFilePath, int zipLevel = 0, bool needPwd = true)
        {
            new FileStream(outputFilePath, FileMode.OpenOrCreate).CompressDirectory(sourcePath, zipLevel, needPwd);
        }
        ...
        private static bool DecodeDic(LuaTable luaTable, System.Type type, out object result)
        {
            result = type.GetConstructor(System.Type.EmptyTypes).Invoke(null);
        ...
        private static bool DecodeDic(string inputString, System.Type type, ref int index, out object result)
        {
            ...
            try
            {
                while (index < inputString.Length)
                {
                    ...
                    bool flag;
                    object obj2;
                    Mogo.RPC.Utils.DecodeKey(inputString, ref index, out str, out flag);
        ...
        private static bool DecodeEntity(LuaTable luaTable, System.Type type, out object result)
        {
        ...
        private static bool DecodeEntity(string inputString, System.Type type, ref int index, out object result)
        {
            result = type.GetConstructor(System.Type.EmptyTypes).Invoke(null);
            if (!Mogo.RPC.Utils.WaitChar(inputString, '{', ref index))
            {
        ...
        private static bool DecodeList(LuaTable luaTable, System.Type type, out object result)
        {
            result = type.GetConstructor(System.Type.EmptyTypes).Invoke(null);
        ...
        private static bool DecodeValue(string inputString, System.Type type, ref int index, out object value)
        {
            int num2;
        ...
        public static void DecompressToDirectory(string targetPath, string zipFilePath)
        {
            ...
            {
                System.IO.File.OpenRead(zipFilePath).DecompressToDirectory(targetPath);
            }
            else
            {
        ...
        public static void DecompressToMogoFile(string zipFilePath)
        {
            if (System.IO.File.Exists(zipFilePath))
            {
                FileStream baseInputStream = System.IO.File.OpenRead(zipFilePath);
                ...
                {
                    ZipEntry entry;
                    stream2.Password = GetResNumber().PackArray<byte>(',');
                        ...
                        if (!entry.IsDirectory)
                        {
                            ...
                            if (!string.IsNullOrEmpty(name))
                            {
                                ...
                                if (name.EndsWith(".u"))
                                {
                                    ...
                                    using (FileStream stream3 = System.IO.File.Create(name))
                                    {
                                        while ((num = stream2.Read(buffer, 0, buffer.Length)) > 0)
                                        {
        ...
        {
            return new byte[] { 0x7b, 0x34, 0x35, 9, 12, 6, 0x17, 0x1a };
        }
        ...
        {
            return new byte[] { 0xe7, 20, 0xb9, 13, 20, 0x7f, 0x51, 0x4f };
        }
        ...
        public static object GetValue(string value, System.Type type)
        {
            if (type != null)
            {
                ...
                {
                    return GetValue(value, System.Enum.GetUnderlyingType(type));
                }
        ...
        {
            TextAsset assetToUnload = Resources.Load(fileName, typeof(TextAsset)) as TextAsset;
            byte[] bytes = assetToUnload.bytes;
        ...
        {
            UnityEngine.Object assetToUnload = Resources.Load(fileName);
            if (assetToUnload != null)
            {
                string str = assetToUnload.ToString();
                Resources.UnloadAsset(assetToUnload);
                return str;
            }
            return string.Empty;
        ...
        public static bool ParseLuaTable(LuaTable luaTable, System.Type type, out object result)
        {
            result = null;
        ...
        public static bool ParseLuaTable(string inputString, System.Type type, out object result)
        {
        ...
        public static bool UnpackFiles(string file, string dir)
        {
            try
            {
                ...
                ZipInputStream stream = new ZipInputStream(System.IO.File.OpenRead(file));
                while ((entry = stream.GetNextEntry()) != null)
                {
                    bool flag2;
                    ...
                    FileStream stream2 = System.IO.File.Create(dir + entry.Name);
                    int count = 0x800;
                    byte[] buffer = new byte[0x800];
                    goto Label_00D0;
                Label_009D:
