...
public class DownloadMgr
{
    ...
    {
        return this.tasks.Any<DownloadTask>(task => (url == task.Url));
    }
    ...
    public bool DownloadFile(string url, string localPath, string bakPath)
    {
        ...
        LoggerHelper.Debug("path: " + localPath, true, 0);
        ...
        try
        {
            this._webClient.DownloadFile(url, localPath);
            flag = true;
        }
    ...
    public void DownloadFileBreakPoint(string address, string fileName)
    {
        try
        {
            ...
            {
                LoggerHelper.Debug("需要下载的文件存在：" + fileName, true, 0);
    ...
    {
        LoggerHelper.Debug("DownloadFinishWithMd51111", true, 0);
    ...
    {
        return this.tasks.FirstOrDefault<DownloadTask>(task => (url == task.Url));
    }
    ...
    internal void Progress(string url, int progress)
    {
        Action action = delegate {
            if (this.ContainKey(url))
            {
                ...
                {
                    int num = this.tasks.Count<DownloadTask>(ta => ta.bFineshed);
