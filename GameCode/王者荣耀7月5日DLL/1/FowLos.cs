...
public class FowLos
{
	...
	private class ComparerVertex : IComparer<FowLos.SPolylineVertex>
	{
		...
		{
			if ((A.m_point - this.m_scanlineStart).get_sqrMagnitude() > (B.m_point - this.m_scanlineStart).get_sqrMagnitude())
			{
				return 1;
			}
			return -1;
	...
	public void ExploreCells(VInt3 location, int inSightRange, GameFowManager pFowMgr, COM_PLAYERCAMP camp, FowLos.EViewExploringMode ViewExploringMode, bool bDrawDebugLines)
	{
		...
		if (pFowMgr.WorldPosToGrid(vInt, out zero.x, out zero.y))
		{
	...
	public bool ExploreCellsFast(VInt3 location, int surfSightRange, GameFowManager pFowMgr, FieldObj inFieldObj, COM_PLAYERCAMP camp, bool bStaticExplore, bool bDistOnly)
	{
		...
		if (!pFowMgr.WorldPosToGrid(location, out vInt.x, out vInt.y))
		{
			return false;
		}
		...
		{
			VInt2 zero = VInt2.zero;
			if (inFieldObj.FindNearestGrid(vInt, location, FieldObj.EViewBlockType.Brick, 3, null, out zero))
			{
				vInt = zero;
			}
		}
		if (bStaticExplore)
		{
			FowLos.TraverseStaticSurCell(surfSightRange, vInt.x, vInt.y, inFieldObj.NumX, inFieldObj.NumY, camp, camp == pFowMgr.m_hostPlayerCamp);
		}
		else
		{
			FowLos.TraverseSurCell(surfSightRange, vInt.x, vInt.y, inFieldObj.NumX, inFieldObj.NumY, camp, camp == pFowMgr.m_hostPlayerCamp, bDistOnly);
		}
		return true;
	}
			...
			if (ViewExploringMode == FowLos.EViewExploringMode.EViewExploringMode_DistOnly)
			{
				for (int i = -surfSightRange - 1; i <= surfSightRange + 1; i++)
				{
					for (int j = -surfSightRange - 1; j <= surfSightRange + 1; j++)
					{
						...
						VInt2 inPos = newSurfPos + vInt;
						if (pFowMgr.IsInsideSurface(inPos.x, inPos.y))
						{
							if (vInt.get_sqrMagnitude() < surfSightRange * surfSightRange)
							{
				...
				if (ViewExploringMode == FowLos.EViewExploringMode.EViewExploringMode_RayCast)
				{
					...
					if (pFowMgr.m_pFieldObj.ViewBlockArrayImpl != null)
					{
						...
						while (enumerator.MoveNext())
						{
							...
							if ((int)viewBlockId == areaId)
							{
								if (sViewBlockAttr.BlockType == 1)
								{
									continue;
								}
								if (sViewBlockAttr.BlockType == 2)
								{
								}
							}
							if (this.ValidateViewBlock(current, cRaycastQuadrant.min.x, cRaycastQuadrant.max.x, cRaycastQuadrant.min.y, cRaycastQuadrant.max.y))
							{
								...
								{
									List<byte> arg_2A3_0 = cRaycastQuadrant.wallsHorizontal;
									KeyValuePair<byte, List<FowLos.SGridWall>> current2 = enumerator2.get_Current();
									BaseAlgorithm.AddUniqueItem<byte>(arg_2A3_0, current2.get_Key());
								}
								enumerator2 = current.m_wallsVertical.GetEnumerator();
								while (enumerator2.MoveNext())
								{
									List<byte> arg_2DF_0 = cRaycastQuadrant.wallsVertical;
									KeyValuePair<byte, List<FowLos.SGridWall>> current3 = enumerator2.get_Current();
									BaseAlgorithm.AddUniqueItem<byte>(arg_2DF_0, current3.get_Key());
								}
							}
						}
						this.RaycastCheck<TSetCellVisible>(pFowMgr, cRaycastQuadrant, newSurfPos, unitLoc, sightSqr, camp, bDrawDebugLines, ref setCellVisible);
					}
	...
	private void RaycastCheck<TSetCellVisible>(GameFowManager pFowMgr, FowLos.CRaycastQuadrant quadrant, VInt2 newSurfPos, VInt3 unitLoc, int sightSqr, COM_PLAYERCAMP camp, bool bDrawDebugLines, ref TSetCellVisible setCellVisible) where TSetCellVisible : ISetCellVisible
	{
		...
		for (int i = x; i <= x2; i++)
		{
			for (int j = y; j <= y2; j++)
			{
				...
				else if ((vInt - newSurfPos).get_sqrMagnitude() < sightSqr)
				{
					...
					int num = (!flag4) ? (newSurfPos.x + 1) : newSurfPos.x;
					int num2 = (!flag5) ? (newSurfPos.y + 1) : newSurfPos.y;
					int num3 = 0;
					while (!flag && (!flag2 || !flag3))
					{
						if (!flag2)
						{
							int num4 = num + ((!flag4) ? num3 : (-num3));
							if (flag4 && num4 < i + 1)
							{
								flag2 = true;
							}
							...
							if (!flag2 && quadrant.wallsVertical.Contains((byte)num4))
							{
								...
								while (enumerator.MoveNext() && !flag)
								{
									...
									if (current.m_wallsVertical.TryGetValue((byte)num4, ref list))
									{
										int count = list.get_Count();
										int num5 = 0;
										while (num5 < count && !flag)
										{
											FowLos.SGridWall sGridWall2 = list.get_Item(num5);
											if (bDrawDebugLines)
											{
						...
						if (!flag3 && !flag)
						{
							int num6 = num2 + ((!flag5) ? num3 : (-num3));
							if (flag5 && num6 < j + 1)
							{
								flag3 = true;
							}
							else if (!flag5 && num6 > j)
							{
								flag3 = true;
							}
							if (!flag3 && quadrant.wallsHorizontal.Contains((byte)num6))
							{
								...
								while (enumerator2.MoveNext() && !flag)
								{
									...
									if (current2.m_wallsHorizontal.TryGetValue((byte)num6, ref list2))
									{
										int count2 = list2.get_Count();
										int num7 = 0;
										while (num7 < count2 && !flag)
										{
											FowLos.SGridWall sGridWall3 = list2.get_Item(num7);
											if (bDrawDebugLines)
											{
