...
internal class NewbieWeakGuideControl : Singleton<NewbieWeakGuideControl>
{
	...
	public void Update()
	{
		...
		if (this.mToaddConfList != null && this.mToaddConfList.get_Count() > 0)
		{
	...
	{
		int count = this.mConfList.get_Count();
		if ((ulong)this.m_CurStep <= (ulong)((long)count))
		{
			NewbieGuideWeakConf conf = this.mConfList.get_Item((int)(this.m_CurStep - 1u));
			this.AddEffect(conf);
		}
		else
		{
	...
	private void AddEffectProcess()
	{
		...
		if (!this.mEffectCache.TryGetValue(this.curToAddEffectConf.dwID, ref gameObject))
		{
	...
	private bool HasEffect(uint weakGuideId)
	{
		...
		{
			if (this.mEffectCache.get_Item(weakGuideId) != null)
			{
				return true;
			}
	...
	{
		List<uint> list = new List<uint>(this.mEffectCache.get_Keys());
	...
	{
		GameObject gameObject = null;
		if (this.mEffectCache.TryGetValue(weakGuideId, ref gameObject))
		{
			this.mEffectCache.Remove(weakGuideId);
			if (gameObject != null)
			{
	...
	{
		NewbieGuideWeakConf newbieGuideWeakConf = this.mConfList.get_Item((int)(this.m_CurStep - 1u));
		this.RemoveEffect(weakGuideId);
	...
	private void ClickHandler(CUIEvent uiEvent)
	{
		...
		GameObject gameObject = null;
		this.mEffectCache.TryGetValue(uiEvent.m_eventParams.weakGuideId, ref gameObject);
		if (gameObject == null)
		{
			return;
		}
