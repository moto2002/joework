...
public class SCModuleControl : Singleton<SCModuleControl>
{
	...
	{
		return (this.m_pvpAndPvpOffMask & type) == null || CRoleInfo.GetCurrentUTCTime() < (int)this.m_pvpAndPvpOffSec;
	}

	[MessageHandler(5326)]
	public static void OnModuleSwitchNtf(CSPkg msg)
	{
		Singleton<SCModuleControl>.get_instance().OnModuleSwitchNtf(msg.stPkgData.get_stSwtichOffNtf().stOffNtfDetail);
	}
}
