...
public class CameraSystem : MonoSingleton<CameraSystem>
{
	...
	private void OnFocusSwitched(ref DefaultGameEventParam prm)
	{
		if (prm.src && ActorHelper.IsHostCtrlActor(ref prm.src) && !Singleton<WatchController>.GetInstance().IsWatching)
		{
			...
			if (!prm.src.get_handle().ActorControl.IsDeadState && !this.bFreeCamera)
			{
	...
	private void OnPlayerDead(ref GameDeadEventParam prm)
	{
		...
		{
			if (this.MobaCamera && prm.src.get_handle().ActorControl != null)
			{
				this.MobaCamera.SetAbsoluteLockLocation((Vector3)prm.src.get_handle().ActorControl.actorLocation);
			}
			...
			if (!prm.src.get_handle().TheStaticData.TheBaseAttribute.DeadControl)
			{
	...
	public void UpdatePanelCameraMovement(ref Vector2 InMovement)
	{
		...
		InMovement *= MonoSingleton<GlobalConfig>.get_instance().PanelCameraMoveSpeed;
		if (this.MobaCamera != null)
		{
