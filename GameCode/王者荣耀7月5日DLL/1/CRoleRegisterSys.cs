...
public class CRoleRegisterSys : Singleton<CRoleRegisterSys>
{
	...
	{
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.ROLE_CREATE, new CUIEventManager.OnUIEventHandler(this.OnRoleCreate));
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.ROLE_CREATE_RANDOM, new CUIEventManager.OnUIEventHandler(this.OnRandomName));
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.GAME_DIFF_SELECT, new CUIEventManager.OnUIEventHandler(this.OnGameDifSelect));
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.GAME_DIFF_CONFIRM, new CUIEventManager.OnUIEventHandler(this.OnGameDifConfirm));
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.ROLE_CREATE_TIMER_CHANGE, new CUIEventManager.OnUIEventHandler(this.OnRoleCreateTimerChange));
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.ROLE_HeroType_Select, new CUIEventManager.OnUIEventHandler(this.OnHeroTypeSelect));
		Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.ROLE_HeroType_Confirm, new CUIEventManager.OnUIEventHandler(this.OnHeroTypeConfirm));
		base.Init();
	}

	public override void UnInit()
	{
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.ROLE_CREATE, new CUIEventManager.OnUIEventHandler(this.OnRoleCreate));
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.ROLE_CREATE_RANDOM, new CUIEventManager.OnUIEventHandler(this.OnRandomName));
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.GAME_DIFF_SELECT, new CUIEventManager.OnUIEventHandler(this.OnGameDifSelect));
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.GAME_DIFF_CONFIRM, new CUIEventManager.OnUIEventHandler(this.OnGameDifConfirm));
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.ROLE_CREATE_TIMER_CHANGE, new CUIEventManager.OnUIEventHandler(this.OnRoleCreateTimerChange));
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.ROLE_HeroType_Select, new CUIEventManager.OnUIEventHandler(this.OnHeroTypeSelect));
		Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.ROLE_HeroType_Confirm, new CUIEventManager.OnUIEventHandler(this.OnHeroTypeConfirm));
		base.UnInit();
	}
	...
	{
		Singleton<CUIManager>.get_instance().CloseSendMsgAlert();
		if (msg.stPkgData.get_stAcntRegisterRes().bErrCode != 0)
		{
			Singleton<CRoleRegisterSys>.get_instance().ShowErrorCode((int)msg.stPkgData.get_stAcntRegisterRes().bErrCode);
		}
		Debug.Log("receive message , id = CSProtocolMacros.SCID_ACNT_REGISTER_RES , err code = " + msg.stPkgData.get_stAcntRegisterRes().bErrCode.ToString());
	}
	...
	public static void OnAcntSnsNameNtf(CSPkg msg)
	{
		...
		{
			CRoleRegisterView.RoleName = Utility.UTF8Convert(msg.stPkgData.get_stNtfAcntSnsName().szNickName);
		}
	}
}
