...
public class CTaskShow : MonoBehaviour
{
	...
	public void PlayParticle(bool bShowEffect)
	{
		...
		Singleton<CUIParticleSystem>.get_instance().AddParticle(parPath, 1.5f, base.gameObject, this.formScript);
		string eventName = (!bShowEffect) ? "UI_renwu_xiaoshi" : "UI_renwu_shuaxin";
		Singleton<CSoundManager>.get_instance().PostEvent(eventName, null);
	}
}
