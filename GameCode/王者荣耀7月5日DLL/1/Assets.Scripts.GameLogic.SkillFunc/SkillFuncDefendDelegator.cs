	...
	internal class SkillFuncDefendDelegator
	{
		...
		public static bool OnSkillFuncImmuneControlSkillEffect(ref SSkillFuncContext inContext)
		{
			if (inContext.inStage == ESkillFuncStage.Enter)
			{
				...
				if (inTargetObj)
				{
					inTargetObj.get_handle().ActorControl.AddNoAbilityFlag(ObjAbilityType.ObjAbility_ImmuneControl);
					if (inContext.GetSkillFuncParam(0, false) == 1)
					{
						int typeMask = 4;
						inTargetObj.get_handle().BuffHolderComp.ClearEffectTypeBuff(typeMask);
					}
				}
			}
			else if (inContext.inStage == ESkillFuncStage.Leave)
			{
				...
				if (inTargetObj2)
				{
					inTargetObj2.get_handle().ActorControl.RmvNoAbilityFlag(ObjAbilityType.ObjAbility_ImmuneControl);
				}
			}
			return true;
		}
