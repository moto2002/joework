...
namespace Assets.Scripts.UI
{
	public class CUIListScript : CUIComponent
	{
		...
		public void SetExternalElementPrefab(string externalElmtPrefPath)
		{
			...
			{
				gameObject = (GameObject)Singleton<CResourceManager>.GetInstance().GetResource(externalElmtPrefPath, typeof(GameObject), 4, false, false).m_content;
				if (gameObject != null)
				{
					cUIListElementScript = gameObject.GetComponent<CUIListElementScript>();
				}
		...
		public CUIListElementScript GetElemenet(int index)
		{
			...
			if (this.m_useOptimized)
			{
				...
				{
					if (this.m_elementScripts.get_Item(i).m_index == index)
					{
						return this.m_elementScripts.get_Item(i);
					}
				}
				return null;
			}
			return this.m_elementScripts.get_Item(index);
		}
		...
		public bool IsElementInScrollArea(int index)
		{
			...
			stRect stRect = (!this.m_useOptimized) ? this.m_elementScripts.get_Item(index).m_rect : this.m_elementsRect.get_Item(index);
		...
		public void MoveElementInScrollArea(int index, bool moveImmediately)
		{
			...
			else
			{
				...
				}, this.m_contentRectTransform.anchoredPosition, vector, this.m_fSpeed);
			}
		}
		...
		{
			int i = 0;
			while (i < this.m_elementScripts.get_Count())
			{
				if (!this.IsRectInScrollArea(ref this.m_elementScripts.get_Item(i).m_rect))
				{
					this.RecycleElement(this.m_elementScripts.get_Item(i), true);
				}
				else
				{
					i++;
			...
			for (int j = 0; j < this.m_elementAmount; j++)
			{
				...
				{
					bool flag = false;
					for (int k = 0; k < this.m_elementScripts.get_Count(); k++)
					{
						if (this.m_elementScripts.get_Item(k).m_index == j)
						{
							flag = true;
							break;
						}
		...
		protected stRect LayoutElement(int index, ref Vector2 contentSize, ref Vector2 offset)
		{
			...
			result.m_width = (int)((this.m_elementsSize != null && this.m_listType != enUIListType.Vertical && this.m_listType != enUIListType.VerticalGrid && this.m_listType != enUIListType.HorizontalGrid) ? this.m_elementsSize.get_Item(index).x : this.m_elementDefaultSize.x);
			result.m_height = (int)((this.m_elementsSize != null && this.m_listType != enUIListType.Horizontal && this.m_listType != enUIListType.VerticalGrid && this.m_listType != enUIListType.HorizontalGrid) ? this.m_elementsSize.get_Item(index).y : this.m_elementDefaultSize.y);
		...
		{
			while (this.m_elementScripts.get_Count() > 0)
			{
				CUIListElementScript cUIListElementScript = this.m_elementScripts.get_Item(0);
		...
		{
			CUIListElementScript cUIListElementScript = null;
			if (this.m_unUsedElementScripts.get_Count() > 0)
			{
				cUIListElementScript = this.m_unUsedElementScripts.get_Item(0);
		...
		{
			if (this.m_unUsedElementScripts != null && this.m_unUsedElementScripts.get_Count() > 0)
			{
				for (int i = 0; i < this.m_unUsedElementScripts.get_Count(); i++)
				{
					this.m_unUsedElementScripts.get_Item(i).Disable();
				}
			}
		}
