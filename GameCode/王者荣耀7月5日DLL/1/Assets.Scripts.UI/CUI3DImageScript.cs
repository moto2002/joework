...
namespace Assets.Scripts.UI
{
	...
	public class CUI3DImageScript : CUIComponent
	{
		...
		public override void Close()
		{
			base.Close();
			int i = 0;
			while (i < this.m_3DGameObjects.get_Count())
			{
				if (!this.m_3DGameObjects.get_Item(i).m_protogenic)
				{
					if (this.m_3DGameObjects.get_Item(i).m_gameObject != null)
					{
						if (this.m_3DGameObjects.get_Item(i).m_useGameObjectPool)
						{
							Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(this.m_3DGameObjects.get_Item(i).m_gameObject);
						}
						else
						{
							Object.Destroy(this.m_3DGameObjects.get_Item(i).m_gameObject);
						}
					}
					this.m_3DGameObjects.get_Item(i).m_path = null;
					this.m_3DGameObjects.get_Item(i).m_gameObject = null;
		...
		public void RemoveGameObject(string path)
		{
			...
			while (i < this.m_3DGameObjects.get_Count())
			{
				if (string.Equals(this.m_3DGameObjects.get_Item(i).m_path, path, 5))
				{
					if (this.m_3DGameObjects.get_Item(i).m_useGameObjectPool)
					{
						Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(this.m_3DGameObjects.get_Item(i).m_gameObject);
			...
			if (this.m_3DGameObjects.get_Count() <= 0)
			{
				this.m_renderCamera.enabled = false;
			}
		}
		...
		public void RemoveGameObject(GameObject removeObj)
		{
			...
			while (i < this.m_3DGameObjects.get_Count())
			{
				if (this.m_3DGameObjects.get_Item(i).m_gameObject == removeObj)
				{
					if (this.m_3DGameObjects.get_Item(i).m_useGameObjectPool)
					{
						Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(this.m_3DGameObjects.get_Item(i).m_gameObject);
		...
		public GameObject GetGameObject(string path)
		{
			for (int i = 0; i < this.m_3DGameObjects.get_Count(); i++)
			{
				if (this.m_3DGameObjects.get_Item(i).m_path.Equals(path))
				{
					return this.m_3DGameObjects.get_Item(i).m_gameObject;
				}
			}
			return null;
		}
