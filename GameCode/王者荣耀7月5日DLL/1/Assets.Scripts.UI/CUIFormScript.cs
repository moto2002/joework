...
namespace Assets.Scripts.UI
{
	...
	public class CUIFormScript : MonoBehaviour, IComparable
	{
		...
		{
			if (index < 0 || index >= this.m_relatedSceneCamera.get_Count() || this.m_relatedSceneCamera.get_Item(index) == null)
			{
				return;
			}
			for (int i = 0; i < this.m_relatedSceneCamera.get_Item(index).get_Count(); i++)
			{
				if (this.m_relatedSceneCamera.get_Item(index).get_Item(i) != null)
				{
					this.m_relatedSceneCamera.get_Item(index).get_Item(i).enabled = bEnable;
				}
			}
		}
		...
		public bool IsRelatedSceneExist(string sceneName)
		{
			for (int i = 0; i < this.m_relatedScenes.get_Count(); i++)
			{
				if (string.Equals(sceneName, this.m_relatedScenes.get_Item(i).name))
				{
					return true;
				}
			}
		...
		public void AddRelatedScene(GameObject scene, string sceneName)
		{
			...
			this.AddRelatedSceneCamera(this.m_relatedSceneCamera.get_Count() - 1, scene);
		}

		public void AddRelatedSceneCamera(int index, GameObject go)
		{
			if (index < 0 || index >= this.m_relatedSceneCamera.get_Count() || go == null)
			{
				return;
			}
			...
			if (component != null)
			{
				this.m_relatedSceneCamera.get_Item(index).Add(component);
			}
		...
		{
			for (int i = 0; i < this.m_uiComponents.get_Count(); i++)
			{
				this.m_uiComponents.get_Item(i).Close();
			}
		}
		...
		private void HideComponent()
		{
			for (int i = 0; i < this.m_uiComponents.get_Count(); i++)
			{
				this.m_uiComponents.get_Item(i).Hide();
			}
		}
		...
		private void AppearComponent()
		{
			for (int i = 0; i < this.m_uiComponents.get_Count(); i++)
			{
				this.m_uiComponents.get_Item(i).Appear();
			}
		}
		...
		private void SetComponentSortingOrder(int sortingOrder)
		{
			for (int i = 0; i < this.m_uiComponents.get_Count(); i++)
			{
				this.m_uiComponents.get_Item(i).SetSortingOrder(sortingOrder);
			}
		}
