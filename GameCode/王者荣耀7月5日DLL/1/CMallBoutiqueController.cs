...
public class CMallBoutiqueController : Singleton<CMallBoutiqueController>
{
	...
	public void RefreshNewArrivals()
	{
		...
		componetInChild.SetElementAmount(this.m_NewArrivalListView.get_Count());
	}
	...
	public void RefreshHotSale()
	{
		...
		componetInChild.SetElementAmount(this.m_HotSaleListView.get_Count());
	}

	private void OnNewArrivalEnable(CUIEvent uiEvent)
	{
		int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
		if (srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_NewArrivalListView.get_Count())
		{
			return;
		}
		...
		ResBoutiqueConf resBoutiqueConf = this.m_NewArrivalListView.get_Item(srcWidgetIndexInBelongedList);
	...
	{
		int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
		if (srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_HotSaleListView.get_Count())
		{
			return;
		}
		GameObject srcWidget = uiEvent.m_srcWidget;
		...
		ResBoutiqueConf resBoutiqueConf = this.m_HotSaleListView.get_Item(srcWidgetIndexInBelongedList);
	...
	public void OnFactoryProductClick(CUIEvent uiEvent)
	{
		...
		if (uiEvent.m_eventParams.tag < 0 || uiEvent.m_eventParams.tag >= products.get_Count())
		{
			Singleton<CUIManager>.GetInstance().OpenMessageBox("internal error: out of index range.", false);
			return;
		}
		CMallFactoryShopController.ShopProduct shopProduct = products.get_Item(uiEvent.m_eventParams.tag);
		if (!shopProduct.CanBuy())
		{
			return;
		}
		if (shopProduct.Type == 4 || shopProduct.Type == 7)
		{
