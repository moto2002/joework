...
namespace Assets.Scripts.GameLogic
{
	...
	public class ActorReviveCondition : PassiveCondition
	{
		...
		public override void Init(PoolObjHandle<ActorRoot> _source, PassiveEvent _event, ref ResDT_SkillPassiveCondition _config)
		{
			...
			this.sourceActor.get_handle().ActorControl.eventActorRevive += new ActorEventHandler(this.onActorRevive);
		}
		...
		{
			if (_prm.src != this.sourceActor || (this.localParams[0] == 1 && this.sourceActor.get_handle().ActorControl.IsEnableReviveContext()))
			{
				return;
			}
			this.bTrigger = true;
