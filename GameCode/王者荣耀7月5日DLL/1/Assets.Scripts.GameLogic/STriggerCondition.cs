...
namespace Assets.Scripts.GameLogic
{
	...
	public struct STriggerCondition
	{
		...
		public bool FilterMatch(EGlobalGameEvent inEventType, PoolObjHandle<ActorRoot> src, PoolObjHandle<ActorRoot> atker, ref SFilterMatchParam param, CTriggerMatch match, int inMatchIndex)
		{
			...
			if (this.GlobalVariable != 0 && Singleton<BattleLogic>.get_instance().m_globalTrigger != null && this.GlobalVariable != Singleton<BattleLogic>.get_instance().m_globalTrigger.CurGlobalVariable)
			{
				return false;
			}
		...
		private bool FilterMatchDamage(ref HurtEventResultInfo inHurtInfo)
		{
			...
			int actorHp = inHurtInfo.src.get_handle().ValueComponent.actorHp;
			...
			int num3 = inHurtInfo.src.get_handle().ValueComponent.actorHpTotal * num2 / 100;
