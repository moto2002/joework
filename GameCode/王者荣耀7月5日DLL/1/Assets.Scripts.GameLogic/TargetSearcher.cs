	...
	public class TargetSearcher : Singleton<TargetSearcher>
	{
				...
				if (i != InActor.TheActorMeta.ActorCamp)
				{
					...
					for (int j = 0; j < count; j++)
					{
						...
						if (actorType == ActorTypeDef.Actor_Type_Monster)
						{
							...
							{
								actorType = monsterWrapper.hostActor.get_handle().TheActorMeta.ActorType;
							}
						}
		...
		public ActorRoot GetLowestHpTarget(ActorRoot InActor, int srchR, TargetPriority priotity, uint filter, bool bEnemy = true, bool bWithMonsterNotInBattle = true)
		{
						...
						for (int j = 0; j < count; j++)
						{
							...
							if (((ulong)filter & (ulong)(1L << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && handle.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp) && InActor.CanAttack(handle))
							{
								if (TypeSearchCondition.Fit(handle, ActorTypeDef.Actor_Type_Hero))
								{
									...
									{
										targetPropertyLessEqualFilter.Searcher(handle, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
									}
								}
								...
								{
									targetPropertyLessEqualFilter2.Searcher(handle, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
								}
							}
						}
					}
			...
			else
			{
				...
				{
					ActorRoot handle2 = campActors2.get_Item(k).get_handle();
					if (((ulong)filter & (ulong)(1L << (int)(handle2.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && !handle2.ActorControl.IsDeadState && handle2.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
					{
						if (TypeSearchCondition.Fit(handle2, ActorTypeDef.Actor_Type_Hero))
						{
							...
							{
								targetPropertyLessEqualFilter.Searcher(handle2, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
							}
						}
						...
						{
							targetPropertyLessEqualFilter2.Searcher(handle2, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
						}
					}
				}
			}
				...
				if (i != InActor.TheActorMeta.ActorCamp)
				{
					...
					for (int j = 0; j < count; j++)
					{
						...
						if (((ulong)filter & (ulong)(1L << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && handle.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
						{
							...
							{
								ulong sqrMagnitudeLong2D2 = (ulong)(handle.location - InActor.location).get_sqrMagnitudeLong2D();
							...
							else
							{
								...
								if (monsterWrapper != null)
								{
									if (monsterWrapper.cfgInfo.bSoldierType == 3)
									{
										...
										{
											actorRoot = handle;
											num = sqrMagnitudeLong2D2;
										}
									}
									else if (monsterWrapper.cfgInfo.bSoldierType == 1)
									{
										ulong sqrMagnitudeLong2D3 = (ulong)(handle.location - InActor.location).get_sqrMagnitudeLong2D();
										if (sqrMagnitudeLong2D3 < num2 && InActor.CanAttack(handle))
										{
											actorRoot2 = handle;
											num2 = sqrMagnitudeLong2D3;
										}
									}
									else if (monsterWrapper.cfgInfo.bSoldierType == 2)
									{
										ulong sqrMagnitudeLong2D4 = (ulong)(handle.location - InActor.location).get_sqrMagnitudeLong2D();
										if (sqrMagnitudeLong2D4 < num3 && InActor.CanAttack(handle))
										{
											actorRoot3 = handle;
											num3 = sqrMagnitudeLong2D4;
										}
									}
									else if (monsterWrapper.cfgInfo.bMonsterType != 2)
									{
										ulong sqrMagnitudeLong2D5 = (ulong)(handle.location - InActor.location).get_sqrMagnitudeLong2D();
										if (sqrMagnitudeLong2D5 < num4 && InActor.CanAttack(handle))
										{
											actorRoot4 = handle;
											num4 = sqrMagnitudeLong2D5;
										}
									}
								}
							}
		...
		public int GetActorsInCircle(VInt3 center, int radius, PoolObjHandle<ActorRoot>[] outResults, AreaEventTrigger InTriggerRef)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					ulong sqrMagnitudeLong2D = (ulong)(poolObjHandle.get_handle().location - center).get_sqrMagnitudeLong2D();
		...
		private void FilterCoordActor(ref PoolObjHandle<ActorRoot> actorPtr)
		{
			...
			if ((this._coordActor && ((this._coordFilterAlly && handle.IsSelfCamp(this._coordActor.get_handle())) || ((this._coordFilterEnemy || (handle.ObjLinker.Invincible && handle.TheActorMeta.ActorType != ActorTypeDef.Actor_Type_EYE)) && handle.IsEnemyCamp(this._coordActor.get_handle())))) || (handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ && !handle.AttackOrderReady) || (this._coordCheckSight && !handle.HorizonMarker.IsVisibleFor(this._coordActor.get_handle().TheActorMeta.ActorCamp)))
			{
				return;
			}
		...
		public List<PoolObjHandle<ActorRoot>> GetActorsInPolygon(GeoPolygon collisionPolygon, AreaEventTrigger InTriggerRef)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (poolObjHandle)
				{
					if (!(null != InTriggerRef) || InTriggerRef.ActorFilter(ref poolObjHandle))
					{
						if (collisionPolygon.IsPointIn(poolObjHandle.get_handle().location))
						{
							if (list == null)
							{
			...
			for (int i = 0; i < 3; i++)
			{
				if (i != InActor.TheActorMeta.ActorCamp)
				{
					...
					for (int j = 0; j < count; j++)
					{
						...
						{
							long sqrMagnitudeLong2D = (handle.location - Pos).get_sqrMagnitudeLong2D();
							if (sqrMagnitudeLong2D < num)
							{
								return handle;
							}
		...
		public List<ActorRoot> GetOurCampActors(ActorRoot InActor, int srchR)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					long sqrMagnitudeLong2D = (handle.location - InActor.location).get_sqrMagnitudeLong2D();
					if (sqrMagnitudeLong2D < num)
					{
						if (list == null)
						{
		...
		public int GetEnemyCountInRange(ActorRoot InActor, int srchR)
		{
			...
			if (InActor.TheActorMeta.ActorCamp == 1)
			{
				campActors = Singleton<GameObjMgr>.GetInstance().GetCampActors(2);
			}
			else
			{
				if (InActor.TheActorMeta.ActorCamp != 2)
				{
					return 0;
				}
				campActors = Singleton<GameObjMgr>.GetInstance().GetCampActors(1);
			}
			int count = campActors.get_Count();
			for (int i = 0; i < count; i++)
			{
		...
		public void NotifySelfCampToAttack(PoolObjHandle<ActorRoot> InActor, int srchR, PoolObjHandle<ActorRoot> target)
		{
			long num = (long)srchR;
			num *= num;
			List<PoolObjHandle<ActorRoot>> campActors = Singleton<GameObjMgr>.GetInstance().GetCampActors(InActor.get_handle().TheActorMeta.ActorCamp);
			int count = campActors.get_Count();
			for (int i = 0; i < count; i++)
			{
				ActorRoot handle = campActors.get_Item(i).get_handle();
				if (handle.ObjID != InActor.get_handle().ObjID)
				{
					long sqrMagnitudeLong2D = (handle.location - InActor.get_handle().location).get_sqrMagnitudeLong2D();
					if (sqrMagnitudeLong2D < num)
					{
		...
		public void CommanderNotifyToAttack(COM_PLAYERCAMP camp, int srchR, PoolObjHandle<ActorRoot> target, bool ignoreSearchRange)
		{
			...
			{
				ActorRoot handle = campActors.get_Item(i).get_handle();
				if (handle.ObjID != target.get_handle().ObjID && handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
				{
					long sqrMagnitudeLong2D = (handle.location - target.get_handle().location).get_sqrMagnitudeLong2D();
					if (sqrMagnitudeLong2D < num)
					{
		...
		public PoolObjHandle<ActorRoot> GetCommanderTowerTarget(COM_PLAYERCAMP camp, int friendRange, int enemyRange)
		{
			...
			for (int i = 0; i < count; i++)
			{
				ActorRoot handle = towerActors.get_Item(i).get_handle();
			...
			for (int j = 0; j < count; j++)
			{
				ActorRoot handle2 = towerActors.get_Item(j).get_handle();
		...
		public bool HasCantAttackEnemyBuilding(ActorRoot InActor, int srchR)
		{
			...
			for (int i = 0; i < count; i++)
			{
				ActorRoot handle = organActors.get_Item(i).get_handle();
		...
		public bool HasSelfSoldierCountRoundEnemyBuilding(ActorRoot InActor, uint buildId)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ || handle.ObjID == buildId)
				{
					...
					{
						ActorRoot handle2 = soldierActors.get_Item(j).get_handle();
						if (handle2.IsSelfCamp(InActor))
						{
							long sqrMagnitudeLong2D = (handle2.location - handle.location).get_sqrMagnitudeLong2D();
							if (sqrMagnitudeLong2D < num)
							{
								result = true;
								break;
				...
				if (handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)
				{
					...
					if (sqrMagnitudeLong2D < num && !handle.ActorControl.IsDeadState && !InActor.IsSelfCamp(handle))
					{
						...
						for (int j = 0; j < count2; j++)
						{
							...
							if (handle2.IsSelfCamp(InActor))
							{
								long sqrMagnitudeLong2D2 = (handle2.location - handle.location).get_sqrMagnitudeLong2D();
