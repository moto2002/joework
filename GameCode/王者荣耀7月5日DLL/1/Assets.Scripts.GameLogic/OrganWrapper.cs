...
namespace Assets.Scripts.GameLogic
{
	public class OrganWrapper : ObjWrapper
	{
		...
		public override void OnUse()
		{
			...
			OrganWrapper._myselfHeightVec = new Vector3(0f, 3f, 0f);
		...
		protected override void OnDead()
		{
			...
			if (this.actor.TheStaticData.TheOrganOnlyInfo.DeadEnemySoldier > 0)
			{
				SoldierRegion soldierRegion = Singleton<BattleLogic>.GetInstance().mapLogic.GetSoldierRegion(this.actor.GiveMyEnemyCamp(), this.actor.TheStaticData.TheOrganOnlyInfo.AttackRouteID);
				if (soldierRegion != null)
				{
					soldierRegion.SwitchWave(this.actor.TheStaticData.TheOrganOnlyInfo.DeadEnemySoldier);
				}
			}
			if (FogOfWar.enable)
			{
		...
		public override void UpdateLogic(int nDelta)
		{
			...
			else
			{
				...
				{
					int nAddHp = this.cfgInfo.iOutBattleHPAdd * this.actor.ValueComponent.mActorValue[5].totalValue / 10000;
		...
		{
			if (base.myTarget && !base.myTarget.get_handle().ActorControl.IsDeadState && !this.actor.ActorControl.IsDeadState)
			{
				if (null == this._attackLinker)
				{
					GameObject gameObject = Singleton<CResourceManager>.GetInstance().GetResource("Prefab_Characters/Prefab_Organ/AroundEffect/AttackLinker", typeof(GameObject), 0, false, false).m_content as GameObject;
					if (gameObject != null)
					{
						...
						if (this._attackLinker != null)
						{
							...
							this._attackLinker.SetPosition(0, this.actor.myTransform.position + OrganWrapper._myselfHeightVec);
						}
					}
				}
				if (null != this._attackLinker)
				{
					float y = (float)base.myTarget.get_handle().CharInfo.iBulletHeight * 0.001f;
					this._attackLinker.SetPosition(1, base.myTarget.get_handle().myTransform.position + new Vector3(0f, y, 0f));
		...
		{
			if (_inActor && (this.actor.TheStaticData.TheOrganOnlyInfo.OrganType == 1 || this.actor.TheStaticData.TheOrganOnlyInfo.OrganType == 2 || this.actor.TheStaticData.TheOrganOnlyInfo.OrganType == 4) && !_inActor.get_handle().ActorControl.IsDeadState && !this.actor.ActorControl.IsDeadState)
			{
				if (FogOfWar.enable)
				{
					VInt3 worldLoc = new VInt3(_inActor.get_handle().location.x, _inActor.get_handle().location.z, 0);
					if (Singleton<GameFowManager>.get_instance().IsSurfaceCellVisible(worldLoc, this.actor.TheActorMeta.ActorCamp))
					{
						long sqrMagnitudeLong2D = (this.actor.location - _inActor.get_handle().location).get_sqrMagnitudeLong2D();
				...
				{
					long sqrMagnitudeLong2D2 = (this.actor.location - _inActor.get_handle().location).get_sqrMagnitudeLong2D();
