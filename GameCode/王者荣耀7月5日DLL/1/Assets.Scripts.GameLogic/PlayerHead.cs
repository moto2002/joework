...
namespace Assets.Scripts.GameLogic
{
	public class PlayerHead : MonoBehaviour
	{
		...
		public void Init(HeroHeadHud hudOwner, PoolObjHandle<ActorRoot> myHero)
		{
			...
			uint configId = (uint)myHero.get_handle().TheActorMeta.ConfigId;
			...
			this.OnHeroHpChange(myHero.get_handle().ValueComponent.actorHp, myHero.get_handle().ValueComponent.actorHpTotal);
		}
		...
		public void OnMyHeroDead()
		{
			...
			if (this.ReviveCdTxt != null)
			{
				this.ReviveCdTxt.text = string.Format("{0}", Mathf.RoundToInt((float)this.MyHero.get_handle().ActorControl.ReviveCooldown * 0.001f));
			...
			if (!Singleton<BattleLogic>.get_instance().GetCurLvelContext().IsGameTypeBurning() && !Singleton<BattleLogic>.get_instance().GetCurLvelContext().IsGameTypeArena())
			{
