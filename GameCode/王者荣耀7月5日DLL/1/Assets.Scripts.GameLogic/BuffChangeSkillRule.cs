		...
		public void ChangeSkillSlot(SkillSlotType _slotType, int _skillID, int _orgSkillID = 0)
		{
			...
			if (this.sourceActor.get_handle().SkillControl.TryGetSkillSlot(_slotType, out skillSlot))
			{
				...
				if (_slotType == SkillSlotType.SLOT_SKILL_7)
				{
					...
					{
						if ((long)this.sourceActor.get_handle().SkillControl.ornamentFirstSwitchCdEftTime >= (long)Singleton<FrameSynchr>.get_instance().LogicFrameTick)
						{
						...
						this.sourceActor.get_handle().SkillControl.ornamentFirstSwitchCdEftTime = 0;
					}
				}
				else
				{
				...
				this.sourceActor.get_handle().SkillControl.InitSkillSlot((int)_slotType, _skillID, num2);
				if (this.sourceActor.get_handle().SkillControl.TryGetSkillSlot(_slotType, out skillSlot))
				{
