	...
	public class HeroWrapper : ObjWrapper
	{
		...
		public override int CfgReviveCD
		{
			get
			{
				...
				return Singleton<BattleLogic>.get_instance().dynamicProperty.GetDynamicReviveTime(curLvelContext.m_dynamicPropertyConfig, curLvelContext.m_baseReviveTime);
			}
		}
		...
		public override void Fight()
		{
			...
			if (actorDataProvider != null && actorDataProvider.GetActorServerData(ref this.actor.TheActorMeta, ref actorServerData))
			{
				...
				if (this.m_skinId != 0u)
				{
					...
					{
						Singleton<CSoundManager>.get_instance().PostEvent(heroSkin.szSoundSwitchEvent, this.actor.gameObject);
					}
				}
			}
		...
		public override void CmdCommonLearnSkill(IFrameCommand cmd)
		{
			...
			if (!Singleton<BattleLogic>.get_instance().IsMatchLearnSkillRule(this.actorPtr, (SkillSlotType)frameCommand.cmdData.bSlotType))
			{
				return;
			}
			if (this.actor.SkillControl.m_iSkillPoint > 0)
			{
				...
				{
					SkillSlot skillSlot;
					actorPtr.get_handle().SkillControl.TryGetSkillSlot((SkillSlotType)frameCommand.cmdData.bSlotType, out skillSlot);
					if (skillSlot != null)
					{
