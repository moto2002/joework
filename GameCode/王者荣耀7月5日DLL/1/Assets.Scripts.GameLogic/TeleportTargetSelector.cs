...
namespace Assets.Scripts.GameLogic
{
	internal class TeleportTargetSelector : Singleton<TeleportTargetSelector>
	{
		...
		private void RefreshTeleportTargetEffect()
		{
			...
			if (this.m_NeedConfirm)
			{
				if (this.m_bConfirmed)
				{
					for (int j = 0; j < this.m_CanTeleportActorList.get_Count(); j++)
					{
						...
						if (!this.m_CachedTeleportGameObjects.ContainsKey(poolObjHandle2))
						{
			...
			else
			{
				for (int k = 0; k < this.m_CanTeleportActorList.get_Count(); k++)
				{
					...
					if (!this.m_CachedTeleportGameObjects.ContainsKey(poolObjHandle3))
					{
			...
			{
				this.ShowSkillButtonFlowEffect(this.m_CachedTeleportGameObjects.get_Count() > 0 && this.m_TargetSkillEnable);
			}
		}
		...
		public void ShowTeleportNoteEffect(PoolObjHandle<ActorRoot> actorRoot)
		{
			...
			{
				bool flag = false;
				GameObject pooledGameObjLOD = MonoSingleton<SceneMgr>.GetInstance().GetPooledGameObjLOD("Prefab_Skill_Effects/tongyong_effects/Indicator/select_b_01", false, SceneObjType.ActionRes, Vector3.zero, Quaternion.identity, out flag);
				if (pooledGameObjLOD != null)
				{
					pooledGameObjLOD.transform.SetParent(actorRoot.get_handle().myTransform);
