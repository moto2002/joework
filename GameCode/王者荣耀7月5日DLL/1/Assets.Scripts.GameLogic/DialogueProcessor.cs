...
namespace Assets.Scripts.GameLogic
{
	public class DialogueProcessor : MonoSingleton<DialogueProcessor>
	{
		...
		{
			DialogueProcessor.<NextPageNoTimeScale>c__Iterator10 <NextPageNoTimeScale>c__Iterator = new DialogueProcessor.<NextPageNoTimeScale>c__Iterator10();
			<NextPageNoTimeScale>c__Iterator.<>f__this = this;
			return <NextPageNoTimeScale>c__Iterator;
		}
		...
		private void OnDestroyPortrait(GameObject inPrefabObj)
		{
			...
			{
				base.StopCoroutine(this.Portrait3dAnimCoMap.get_Item(inPrefabObj));
		...
		private void ClearUiForms()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				KeyValuePair<int, CUIFormScript> current = enumerator.get_Current();
				arg_25_0.CloseForm(current.get_Value());
			}
		...
		{
			DialogueProcessor.<PlayAnimNoTimeScale>c__Iterator11 <PlayAnimNoTimeScale>c__Iterator = new DialogueProcessor.<PlayAnimNoTimeScale>c__Iterator11();
		...
		{
			DialogueProcessor.<StartFadingOut>c__Iterator12 <StartFadingOut>c__Iterator = new DialogueProcessor.<StartFadingOut>c__Iterator12();
		...
		{
			DialogueProcessor.<StartFadingIn>c__Iterator13 <StartFadingIn>c__Iterator = new DialogueProcessor.<StartFadingIn>c__Iterator13();
				...
				if (preNode.AnimName != node.AnimName || preNode.bAnimLoop != node.bAnimLoop || preNode.b3dPortrait != node.b3dPortrait || preNode.ActorType != node.ActorType || preNode.CharCfgId != node.CharCfgId)
				{
					...
					if (gameObject)
					{
						...
						if (component2)
						{
							...
							Coroutine coroutine = base.StartCoroutine(DialogueProcessor.PlayAnimNoTimeScale(component2, clipName, node.bAnimLoop, new Action(this.OnPortrait3dAnimComplete)));
							this.Portrait3dAnimCoMap.Add(gameObject, coroutine);
						}
					}
				}
			}
		...
		private void OnPortrait3dAnimComplete()
		{
			...
			if (3dPortraitName)
			{
				...
				if (component)
				{
					Coroutine coroutine = base.StartCoroutine(DialogueProcessor.PlayAnimNoTimeScale(component, "Idleshow", true, null));
					if (this.Portrait3dAnimCoMap.ContainsKey(3dPortraitName))
					{
						this.Portrait3dAnimCoMap.set_Item(3dPortraitName, coroutine);
					}
					else
					{
						this.Portrait3dAnimCoMap.Add(3dPortraitName, coroutine);
					}
				}
			}
		}
		...
		private bool PlayAgeActionInternal(GameObject inSrc, GameObject inAtker, int inGroupId)
		{
			...
			if (this.m_curAction)
			{
				this.m_curAction.get_handle().refParams.AddRefParam("DialogGroupIdRaw", inGroupId);
			}
		...
		{
			if (prm.src.get_handle().CharInfo && prm.src.get_handle().CharInfo.DyingDialogGroupId > 0)
			{
				GameObject inAtker = (!prm.orignalAtker) ? null : prm.orignalAtker.get_handle().gameObject;
				this.PlayDrama(prm.src.get_handle().CharInfo.DyingDialogGroupId, prm.src.get_handle().gameObject, inAtker, false);
			}
		}
		...
		private CUIFormScript QueryUiForm(int inDialogStyle)
		{
			...
			{
				return this.m_uiFormMap.get_Item(inDialogStyle);
			}
