...
namespace Assets.Scripts.GameLogic
{
	public class HeroKDA : KDAStat
	{
		...
		public void Initialize(PoolObjHandle<ActorRoot> actorRoot, int campPos)
		{
			...
			this.HeroId = this.actorHero.get_handle().TheActorMeta.ConfigId;
			...
			if (actorDataProvider.GetActorServerData(ref actorRoot.get_handle().TheActorMeta, ref actorServerData))
			{
				this.SkinId = actorServerData.SkinId;
			}
			this.commonSkillID = 0u;
			actorDataProvider.GetActorServerCommonSkillData(ref this.actorHero.get_handle().TheActorMeta, out this.commonSkillID);
			for (int i = 0; i < 5; i++)
			{
		...
		public void OnActorDead(ref GameDeadEventParam prm)
		{
			...
			else if (prm.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				if (flag)
				{
					...
					{
						this.m_numKill++;
						if (this.actorHero.get_handle().ValueComponent.actorHp * 100 / this.actorHero.get_handle().ValueComponent.actorHpTotal <= 10)
						{
			...
			{
				if (prm.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
				{
					if (prm.src.get_handle().ActorControl.GetActorSubType() == 2)
					{
						...
						if (monsterWrapper != null && monsterWrapper.cfgInfo != null)
						{
							if (monsterWrapper.cfgInfo.bSoldierType == 7)
							{
								...
								Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.BATTLE_DRAGON_KILL_CHANGED);
							}
					...
					else if (prm.src.get_handle().ActorControl.GetActorSubType() == 1)
					{
						this.m_numKillSoldier++;
					}
					if (prm.src.get_handle().TheActorMeta.ConfigId != prm.src.get_handle().TheActorMeta.EnCId)
					{
				...
				{
					if (prm.src.get_handle().TheStaticData.TheOrganOnlyInfo.OrganType == 1 || prm.src.get_handle().TheStaticData.TheOrganOnlyInfo.OrganType == 4)
					{
						this.m_numKillOrgan++;
					}
					else if (prm.src.get_handle().TheStaticData.TheOrganOnlyInfo.OrganType == 2)
					{
		...
		public void StatisticSkillInfo(HurtEventResultInfo prm)
		{
			...
			if (!this.actorHero || this.actorHero.get_handle().SkillControl == null || this.actorHero.get_handle().SkillControl.stSkillStat == null || this.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo == null)
			{
				return;
			}
			SKILLSTATISTICTINFO sKILLSTATISTICTINFO = this.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo[(int)prm.hurtInfo.atkSlot];
		...
		{
			ActorValueStatistic objValueStatistic = this.actorHero.get_handle().ValueComponent.ObjValueStatistic;
			if (objValueStatistic != null)
			{
				...
				objValueStatistic.iActorMinHp = Math.Max(objValueStatistic.iActorMinHp, prm.src.get_handle().ValueComponent.actorHp);
		...
		public void OnActorDamage(ref HurtEventResultInfo prm)
		{
			if (prm.src == this.actorHero)
			{
				...
				{
					if (prm.atker && prm.atker.get_handle().TheActorMeta.ActorType != ActorTypeDef.Actor_Type_Organ)
					{
						this.m_uiHealNum += 1u;
			...
			if (prm.atker && prm.atker == this.actorHero)
			{
				if (prm.hurtInfo.hurtType != HurtTypeDef.Therapic)
				{
					...
					{
						this.m_hurtToHero += prm.hurtTotal;
						this.m_uiHurtToHeroNum += 1u;
						if (prm.hurtInfo.hurtType == HurtTypeDef.PhysHurt)
						{
							this.m_iPhysHurtToHero += prm.hurtTotal;
						}
						else if (prm.hurtInfo.hurtType == HurtTypeDef.MagicHurt)
						{
							this.m_iMagicHurtToHero += prm.hurtTotal;
						}
						else if (prm.hurtInfo.hurtType == HurtTypeDef.RealHurt)
						{
							this.m_iRealHurtToHero += prm.hurtTotal;
						}
					}
					else if (prm.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)
					{
						this.m_iHurtToOrgan += prm.hurtTotal;
					}
					this.m_hurtToEnemy += prm.hurtTotal;
					this.m_uiHurtToEnemyNum += 1u;
					if (prm.atker.get_handle().SkillControl.CurUseSkill != null && prm.atker.get_handle().SkillControl.CurUseSkill.SlotType == SkillSlotType.SLOT_SKILL_0)
					{
						this.m_Skill0HurtToEnemy += prm.hurtTotal;
					}
					if (prm.hurtInfo.atkSlot == SkillSlotType.SLOT_SKILL_COUNT)
					{
						this.m_iEquipeHurtValue += (uint)prm.hurtTotal;
					}
				}
				else
				{
			...
			else if (prm.atker && prm.atker.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
			{
				MonsterWrapper monsterWrapper = prm.atker.get_handle().ActorControl as MonsterWrapper;
				if (monsterWrapper != null && monsterWrapper.hostActor && monsterWrapper.hostActor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && monsterWrapper.spawnSkillSlot < SkillSlotType.SLOT_SKILL_COUNT && monsterWrapper.hostActor.get_handle().SkillControl != null && monsterWrapper.hostActor.get_handle().SkillControl.stSkillStat != null && monsterWrapper.hostActor.get_handle().SkillControl.stSkillStat.SkillStatistictInfo != null)
				{
					monsterWrapper.hostActor.get_handle().SkillControl.stSkillStat.SkillStatistictInfo[(int)monsterWrapper.spawnSkillSlot].iHurtTotal += prm.hurtTotal;
				}
			}
		}
		...
		public void OnActorBeChosen(ref SkillChooseTargetEventParam prm)
		{
			...
			{
				if (prm.atker && prm.src.get_handle().IsEnemyCamp(prm.atker.get_handle()))
				{
		...
		{
			if (prm.atker && prm.atker == this.actorHero && prm.atker.get_handle().SkillControl != null && prm.atker.get_handle().SkillControl.stSkillStat != null && prm.atker.get_handle().SkillControl.stSkillStat.SkillStatistictInfo != null && prm.atker.get_handle().SkillControl.CurUseSkillSlot != null)
			{
				SKILLSTATISTICTINFO sKILLSTATISTICTINFO = prm.atker.get_handle().SkillControl.stSkillStat.SkillStatistictInfo[(int)prm.atker.get_handle().SkillControl.CurUseSkillSlot.SlotType];
		...
		{
			if (this.actorHero && this.actorHero.get_handle() != null && this.actorHero.get_handle().BuffHolderComp != null && this.actorHero.get_handle().BuffHolderComp.protectRule != null)
			{
				return (int)this.actorHero.get_handle().BuffHolderComp.protectRule.GetProtectTotalValue();
			}
			return 0;
		}
		...
		protected override uint GetShieldProtectedValueFormHero()
		{
			if (this.actorHero && this.actorHero.get_handle() != null && this.actorHero.get_handle().BuffHolderComp != null && this.actorHero.get_handle().BuffHolderComp.protectRule != null)
			{
				return this.actorHero.get_handle().BuffHolderComp.protectRule.ProtectValueFromHero;
			}
			return 0u;
		}
		...
		protected override uint GetBeShieldProtectedValueToHeroPhys()
		{
			if (this.actorHero && this.actorHero.get_handle() != null && this.actorHero.get_handle().BuffHolderComp != null && this.actorHero.get_handle().BuffHolderComp.protectRule != null)
			{
				return this.actorHero.get_handle().BuffHolderComp.protectRule.BeProtectedValueToHeroPhys;
			}
			return 0u;
		}
		...
		protected override uint GetBeShieldProtectedValueToHeroMagic()
		{
			if (this.actorHero && this.actorHero.get_handle() != null && this.actorHero.get_handle().BuffHolderComp != null && this.actorHero.get_handle().BuffHolderComp.protectRule != null)
			{
				return this.actorHero.get_handle().BuffHolderComp.protectRule.BeProtectedValueToHeroMagic;
			}
			return 0u;
		}
		...
		protected override uint GetBeShieldProtectedValueToHeroReal()
		{
			if (this.actorHero && this.actorHero.get_handle() != null && this.actorHero.get_handle().BuffHolderComp != null && this.actorHero.get_handle().BuffHolderComp.protectRule != null)
			{
				return this.actorHero.get_handle().BuffHolderComp.protectRule.BeProtectedValueToHeroReal;
			}
			return 0u;
		}
