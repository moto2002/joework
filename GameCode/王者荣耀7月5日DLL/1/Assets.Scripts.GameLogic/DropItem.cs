...
namespace Assets.Scripts.GameLogic
{
	public class DropItem
	{
		...
		{
			int num = MonoSingleton<GlobalConfig>.get_instance().PickupRange * MonoSingleton<GlobalConfig>.get_instance().PickupRange;
			...
			{
				VInt3 location2 = heroActors.get_Item(i).get_handle().location;
				if ((location2 - location).get_sqrMagnitude() <= (double)num && this.pickupEffect.CanPickup(heroActors.get_Item(i)))
				{
