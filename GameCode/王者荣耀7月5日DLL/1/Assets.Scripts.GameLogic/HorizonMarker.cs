	...
	public class HorizonMarker : HorizonMarkerBase
	{
		internal class CampMarker
		{
			...
			public bool Visible
			{
				get
				{
					return this.RuleVisible && (!Singleton<BattleLogic>.GetInstance().horizon.Enabled || (this.sightFrame > 0u && Singleton<FrameSynchr>.get_instance().CurFrameNum <= this.sightFrame + 8u) || this.HasShowMark(HorizonConfig.ShowMark.Skill));
				}
			}
		...
		{
			base.Init();
			Horizon.EnableMethod horizonEnableMethod = Singleton<BattleLogic>.get_instance().GetCurLvelContext().m_horizonEnableMethod;
		...
		public override void VisitSight(COM_PLAYERCAMP targetCamp)
		{
			if (this._campMarkers != null)
			{
				...
				{
					this._campMarkers[num].sightFrame = Singleton<FrameSynchr>.get_instance().CurFrameNum;
		...
		public override bool IsSightVisited(COM_PLAYERCAMP targetCamp)
		{
			...
			int num = BattleLogic.MapOtherCampIndex(this.actor.TheActorMeta.ActorCamp, targetCamp);
			return num < 0 || num >= this._campMarkers.Length || Singleton<FrameSynchr>.get_instance().CurFrameNum == this._campMarkers[num].sightFrame;
		}
