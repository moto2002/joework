...
namespace Assets.Scripts.GameLogic
{
	public class LockTargetAttackMode : BaseAttackMode
	{
		...
		public bool IsValidSkillTargetID(uint _targetID, uint _targetMask)
		{
			...
			return actor && ((ulong)_targetMask & (ulong)(1L << (int)(actor.get_handle().TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL;
		}
		...
		public bool IsValidLockTargetID(uint _targetID)
		{
			...
			long num = 10000L;
			num *= num;
			return (this.actor.ActorControl.actorLocation - actor.get_handle().location).get_sqrMagnitudeLong2D() <= num && flag;
		}
