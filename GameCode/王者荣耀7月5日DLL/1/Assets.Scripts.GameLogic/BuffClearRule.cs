	...
	public class BuffClearRule
	{
		...
		public void CheckBuffClear(RES_SKILLFUNC_CLEAR_RULE _rule)
		{
			...
			for (int i = 0; i < this.TempBuffList.get_Count(); i++)
			{
				...
				if (buffSkill.cfgData.bClearRule == _rule)
				{
					buffSkill.Stop();
				}
			}
		...
		public void CheckBuffNoClear(RES_SKILLFUNC_CLEAR_RULE _rule)
		{
			...
			{
				BuffSkill buffSkill = this.TempBuffList.get_Item(i);
				if (buffSkill.cfgData.bClearRule != _rule && buffSkill.cfgData.bEffectType != 3)
				{
					buffSkill.Stop();
				}
			}
		...
		{
			if (_buff.cfgData.bClearRule == _rule && _buff.cfgData.bEffectType == 3)
			{
