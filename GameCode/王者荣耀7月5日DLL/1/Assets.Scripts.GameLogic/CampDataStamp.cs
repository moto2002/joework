...
namespace Assets.Scripts.GameLogic
{
	internal class CampDataStamp
	{
		...
		{
			byte b = InActor.get_handle().TheActorMeta.ActorCamp;
		...
		{
			OutMaxPositive = this.CampGolds[InFrom].PositiveGolds[InTo];
			OutMaxNegative = this.CampGolds[InFrom].NegativeGolds[InTo];
		}

		public int CalcCampStat(COM_PLAYERCAMP InFrom, COM_PLAYERCAMP InTo)
		{
			return this.CampGolds[InFrom].Golds - this.CampGolds[InTo].Golds;
		}
