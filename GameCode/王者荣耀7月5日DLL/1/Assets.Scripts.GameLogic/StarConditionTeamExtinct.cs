...
namespace Assets.Scripts.GameLogic
{
	...
	internal class StarConditionTeamExtinct : StarCondition
	{
		...
		private bool ShouldCare(ActorRoot src)
		{
			...
			{
				return src.TheActorMeta.ActorCamp == Singleton<GamePlayerCenter>.get_instance().hostPlayerCamp;
			}
			return src.TheActorMeta.ActorCamp != Singleton<GamePlayerCenter>.get_instance().hostPlayerCamp;
		}
		...
		{
			if (prm.src && this.ShouldCare(prm.src.get_handle()))
			{
				this.bCanStat = true;
			}
		}
		...
		public override void OnActorDeath(ref GameDeadEventParam prm)
		{
			...
			if (prm.src && this.bCanStat && this.ShouldCare(prm.src.get_handle()))
			{
				bool flag = true;
				...
				while (enumerator.MoveNext())
				{
					...
					if (!current.get_handle().ActorControl.IsDeadState)
					{
						flag = false;
						break;
					}
		...
		{
			if (!Singleton<PVEReviveHeros>.get_instance().CheckAndPopReviveForm(new PVEReviveHeros.SetTriggerAllDeathResult(this.GetTriggerAllDeathResult), true))
			{
