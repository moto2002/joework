...
namespace Assets.Scripts.GameLogic
{
	public class HurtComponent : LogicComponent
	{
		...
		public int CommonDamagePart(ref HurtDataInfo hurt)
		{
			...
			int num = (hurtValue + hurt.adValue * hurt.attackInfo.iActorATT / 10000 + hurt.apValue * hurt.attackInfo.iActorINT / 10000 + hurt.hpValue * hurt.attackInfo.iActorMaxHp / 10000 + this.GetExtraHurtValue(ref hurt)) * this.GetBaseHurtValue(ref hurt) / this.GetBaseHurtRate(ref hurt);
			if (hurt.atkSlot == SkillSlotType.SLOT_SKILL_0 && hurt.atker)
			{
				num += hurt.atker.get_handle().ValueComponent.mActorValue[19].totalValue;
			}
			return num;
		}
		...
		private int ReduceDamagePart(ref HurtDataInfo hurt, HurtTypeDef hurtType)
		{
			...
			{
				int num = this.actor.ValueComponent.mActorValue[3].totalValue - hurt.attackInfo.iDEFStrike;
			...
			{
				int num3 = this.actor.ValueComponent.mActorValue[4].totalValue - hurt.attackInfo.iRESStrike;
		...
		private int CriticalDamagePart(ref HurtDataInfo hurt)
		{
			...
			if (flag)
			{
				...
				Singleton<GameEventSys>.get_instance().SendEvent<DefaultGameEventParam>(GameEventDef.Event_ActorCrit, ref defaultGameEventParam);
			}
			return result;
		}
		...
		private int Hemophagia(ref HurtDataInfo hurt, int hurtValue)
		{
			...
			if (hurt.atker)
			{
				...
				if (num > 0)
				{
					int num6 = hurt.atker.get_handle().ValueComponent.actorHp;
					hurt.atker.get_handle().ActorControl.ReviveHp(num);
					num6 = hurt.atker.get_handle().ValueComponent.actorHp - num6;
					HemophagiaEventResultInfo hemophagiaEventResultInfo = new HemophagiaEventResultInfo(hurt.atker, num6);
					Singleton<GameEventSys>.get_instance().SendEvent<HemophagiaEventResultInfo>(GameEventDef.Event_Hemophagia, ref hemophagiaEventResultInfo);
				}
				return num;
			}
			return num;
		...
		{
			if (hurt.atkSlot == SkillSlotType.SLOT_SKILL_0 && hurt.atker && hurt.atker.get_handle().ActorControl != null && hurt.atker.get_handle().ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_Blindness))
			{
			...
			return hurt.target && hurt.target.get_handle().ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_ImmuneNegative);
		}
		...
		public int TakeDamage(ref HurtDataInfo hurt)
		{
			...
			if (hurt.hurtType == HurtTypeDef.Therapic)
			{
				...
				if (hurt.atker && this.actor.TheActorMeta.ActorCamp == hurt.atker.get_handle().TheActorMeta.ActorCamp && this.actor.ValueComponent.actorHp > 0)
				{
			...
			else
			{
				...
				bool flag = hurt.atker && hurt.atker.get_handle().bOneKiller;
				int actorHp = this.actor.ValueComponent.actorHp;
				num3 = this.CommonDamagePart(ref hurt) * num2 / 10000;
				num3 = this.actor.BuffHolderComp.OnHurtBounceDamage(ref hurt, num3);
				num3 = num3 * (10000 - this.ReduceDamagePart(ref hurt, hurt.hurtType)) / 10000 * (10000 + num) / 10000 + hurt.attackInfo.iFinalHurt - this.actor.ValueComponent.mActorValue[14].totalValue;
				num3 = num3 * (10000 - this.actor.ValueComponent.mActorValue[30].totalValue) / 10000;
				...
				if (hurt.atker)
				{
					num3 = num3 * Singleton<BattleLogic>.GetInstance().clashAddition.CalcDamageAddition(hurt.atker.get_handle().TheStaticData.TheBaseAttribute.ClashMark, this.actor.TheStaticData.TheBaseAttribute.ClashMark) / 10000;
				}
				num3 = this.actor.BuffHolderComp.OnDamage(ref hurt, num3);
				if (hurt.atker && hurt.atker.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && hurt.atker.get_handle().TheStaticData.TheHeroOnlyInfo.AttackDistanceType == 2 && hurt.iLongRangeReduction > 0)
				{
					num3 = num3 * hurt.iLongRangeReduction / 10000;
				}
				int num5 = hurt.iDamageLimit;
				if (hurt.target && hurt.target.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
				{
				...
				if (hurt.target && hurt.target.get_handle().TheActorMeta.ActorType != ActorTypeDef.Actor_Type_Organ)
				{
			...
			Singleton<GameEventSys>.get_instance().SendEvent<HurtEventResultInfo>(GameEventDef.Event_ActorDamage, ref hurtEventResultInfo);
			return num4;
		}
		...
		public int TakeBouncesDamage(ref HurtDataInfo hurt)
		{
			...
			bool flag = hurt.atker && hurt.atker.get_handle().bOneKiller;
			int actorHp = this.actor.ValueComponent.actorHp;
			int num3 = this.CommonDamagePart(ref hurt) * (10000 - this.ReduceDamagePart(ref hurt, hurt.hurtType)) / 10000 * (10000 + num) / 10000 - this.actor.ValueComponent.mActorValue[14].totalValue;
			num3 = num3 * (10000 - this.actor.ValueComponent.mActorValue[30].totalValue) / 10000;
			if (num3 < 0)
			{
				num3 = 0;
