...
namespace Assets.Scripts.GameLogic
{
	public class AccelerateMotionControler : SpecialMotionControler
	{
		...
		public void InitMotionControler(int _time, int _distance, int _accelerateSpeed)
		{
			...
			this.lastLerpDistance = 0;
			this.accelerateSpeed = _accelerateSpeed;
			long num = (long)_distance * 2000L - (long)this.accelerateSpeed * ((long)_time * (long)_time);
			this.motionSpeed = (int)IntMath.Divide(num, (long)((long)_time << 1));
		}

		public override int GetMotionDistance(int _allTime)
		{
			long num = ((long)this.motionSpeed * (long)_allTime << 1) + (long)this.accelerateSpeed * ((long)_allTime * (long)_allTime);
			return (int)IntMath.Divide(num, 2000L);
		}
