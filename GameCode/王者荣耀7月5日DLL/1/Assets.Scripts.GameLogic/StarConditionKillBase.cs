...
namespace Assets.Scripts.GameLogic
{
	public abstract class StarConditionKillBase : StarCondition
	{
		...
		public override void OnActorDeath(ref GameDeadEventParam prm)
		{
			...
			if (prm.src && prm.orignalAtker && this.ShouldCare(prm.src.get_handle()) && prm.src.get_handle().TheActorMeta.ActorType == this.targetType && (this.targetID == 0 || prm.src.get_handle().TheActorMeta.ConfigId == this.targetID))
			{
				this.killCnt++;
				this.CheckResult();
				this.OnStatChanged();
