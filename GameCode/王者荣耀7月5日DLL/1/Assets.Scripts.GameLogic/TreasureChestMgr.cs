...
namespace Assets.Scripts.GameLogic
{
	public class TreasureChestMgr : Singleton<TreasureChestMgr>
	{
		...
		{
			base.Init();
			Singleton<GameEventSys>.get_instance().AddEventHandler<GameDeadEventParam>(GameEventDef.Event_ActorDead, new RefAction<GameDeadEventParam>(this.NotifyDropEvent));
		...
		public void Reset(SLevelContext InLevelContext, int InMaxCount)
		{
			...
			this.DropStrategy = (this.Factory.Create(InLevelContext.m_pveLevelType) as ITreasureChestStrategy);
		...
		private void OnDrop()
		{
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.DropTreasure);
		}
