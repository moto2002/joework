...
namespace Assets.Scripts.GameLogic
{
	public class CTreasureHud
	{
		...
		public void Init(GameObject obj)
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.DropTreasure, new Action(this.onGetTreasure));
		...
		{
			this.Show();
			this.m_DropNum = Singleton<TreasureChestMgr>.get_instance().droppedCount;
			if (this.Num != null)
			{
				Utility.GetComponetInChild<Text>(this.Num, "TxtNum").text = this.m_DropNum.ToString();
			}
			Singleton<CSoundManager>.get_instance().PlayBattleSound2D("UI_Prompt_get_box");
		}
	}
}
