...
namespace Assets.Scripts.GameLogic
{
	...
	{
		private Delegate[] ActionTable = new Delegate[52];
		...
		{
			if (this.postEventList.get_Count() > 0)
			{
				int i = 0;
				while (i < this.postEventList.get_Count())
				{
					uint curFrameNum = Singleton<FrameSynchr>.get_instance().CurFrameNum;
					IPostEventWrapper postEventWrapper = this.postEventList.get_Item(i);
