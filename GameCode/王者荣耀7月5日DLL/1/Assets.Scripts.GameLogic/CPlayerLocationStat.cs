...
namespace Assets.Scripts.GameLogic
{
	public class CPlayerLocationStat
	{
		...
		{
			if (this.bShouldCare && Singleton<FrameSynchr>.get_instance().CurFrameNum % 450u == 0u && Singleton<BattleLogic>.get_instance().isFighting)
			{
				this.OnStat(0);
			}
		}
			...
			try
			{
				...
				while (enumerator.MoveNext())
				{
					...
					if (current != null && current.Captain)
					{
						...
						if (!this.StatData.TryGetValue(current.PlayerId, ref list))
						{
							list = new List<VInt2>();
							this.StatData.Add(current.PlayerId, list);
						}
						VInt3 location = current.Captain.get_handle().location;
		...
		{
			List<VInt2> list = null;
			if (this.StatData.TryGetValue(playerID, ref list) && Index < list.get_Count())
			{
