...
namespace Assets.Scripts.GameLogic
{
	public class CShenFuStat
	{
		...
		{
			this.Clear();
			Singleton<EventRouter>.get_instance().AddEventHandler<COM_PLAYERCAMP, uint, uint>(EventID.BATTLE_SHENFU_EFFECT_CHANGED, new Action<COM_PLAYERCAMP, uint, uint>(this.OnShenFuEffect));
		}

		public void Clear()
		{
			this.m_recordList.Clear();
			Singleton<EventRouter>.get_instance().RemoveEventHandler<COM_PLAYERCAMP, uint, uint>(EventID.BATTLE_SHENFU_EFFECT_CHANGED, new Action<COM_PLAYERCAMP, uint, uint>(this.OnShenFuEffect));
		}

		private void OnShenFuEffect(COM_PLAYERCAMP playerCamp, uint playerId, uint shenFuId)
		{
			this.m_recordList.Add(new CShenFuStat.ShenFuRecord(playerCamp, playerId, shenFuId, (uint)Singleton<FrameSynchr>.get_instance().LogicFrameTick));
		}
