	...
	public class SkillControlIndicator
	{
		...
		public ActorRoot GetUseSkillTargetDefaultAttackMode()
		{
			if (!this.bUseAdvanceSelect)
			{
				...
				{
					this.targetActor = instance.SelectTarget(skill.cfgData.bSkillTargetRule, this.skillSlot);
				}
			}
			return this.targetActor;
		}
		...
		public void SetUseSkillTarget()
		{
			...
			if (skill.cfgData.bRangeAppointType == 1)
			{
				...
				{
					SkillSelectControl instance = Singleton<SkillSelectControl>.GetInstance();
					actorRoot = instance.SelectTarget(skill.cfgData.bSkillTargetRule, this.skillSlot);
				}
				...
				if (actorRoot != this.targetActor)
				{
					...
					if (this.skillSlot.Actor && actorRoot != null && actorRoot.ObjID != this.skillSlot.Actor.get_handle().ObjID)
					{
		...
		public void LateUpdate(int nDelta)
		{
			...
			if (this.effectPrefab != null)
			{
				if (this.bMoveFlag)
				{
					...
					if (this.deltaPosition.sqrMagnitude >= this.movePosition.sqrMagnitude)
					{
						...
						this.useSkillPosition = this.skillSlot.Actor.get_handle().myTransform.position + this.useOffsetPosition;
						this.effectPrefab.transform.position = this.useSkillPosition;
					}
					else
					{
						this.useSkillPosition = this.effectPrefab.transform.position + b;
						this.useSkillPosition += this.skillSlot.Actor.get_handle().myTransform.position - this.rootRosition;
						this.effectPrefab.transform.position = this.useSkillPosition;
				...
				VInt vInt = 0;
				if (PathfindingUtility.GetGroundY((VInt3)this.effectPrefab.transform.position, out vInt))
				{
					Vector3 position = this.effectPrefab.transform.position;
					position.y = (float)vInt + 0.3f;
			...
			{
				Vector3 position2 = this.effectPrefab.transform.position;
				if (!PathfindingUtility.IsValidTarget(this.skillSlot.Actor.get_handle(), (VInt3)position2))
				{
					this.PrefabSelecter = SkillControlIndicator.EEffectPrefabSelect.Block;
				}
				else if (Singleton<GameFowManager>.get_instance().QueryAttr(position2) == FieldObj.EViewBlockType.Grass)
				{
		...
		public void SetSkillUsePosition(ActorRoot target)
		{
			...
			if (skill.cfgData.bRangeAppointType == 2)
			{
				...
				vector = target.myTransform.position - this.skillSlot.Actor.get_handle().myTransform.position;
			...
			else if (skill.cfgData.bRangeAppointType == 3)
			{
				...
				{
					this.effectPrefab.transform.forward = forward;
					this.effectPrefab.transform.position = this.skillSlot.Actor.get_handle().myTransform.position;
					this.effectPrefab.transform.Translate(0f, 0.3f, 0f);
				}
			}
		}
		...
		public void SetSkillUseDefaultPosition()
		{
			...
			this.useOffsetPosition = Vector3.zero;
			this.useSkillPosition = this.skillSlot.Actor.get_handle().myTransform.position;
			this.useSkillDirection = this.skillSlot.Actor.get_handle().myTransform.forward;
			if (this.effectPrefab != null)
			{
				this.effectPrefab.transform.position = this.skillSlot.Actor.get_handle().myTransform.position;
				this.effectPrefab.transform.Translate(0f, 0.3f, 0f);
				this.effectPrefab.transform.forward = this.skillSlot.Actor.get_handle().myTransform.forward;
			}
		}
