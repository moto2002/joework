...
public class CHeroSelectChatView
{
	...
	public void Clear()
	{
		Singleton<CChatController>.get_instance().model.Clear_HeroSelected();
		...
		Singleton<CTimerManager>.get_instance().RemoveTimer(this.chat_cd_timer);
	...
	{
		CChatEntity lastUnread_Selected = Singleton<CChatController>.get_instance().model.GetLastUnread_Selected();
	...
	public void Start_Input_Timer()
	{
		...
		{
			Singleton<CTimerManager>.get_instance().ResetTimer(this.chat_cd_timer);
			Singleton<CTimerManager>.get_instance().ResumeTimer(this.chat_cd_timer);
		}
	}
	...
	private bool Show_Bubble(CChatEntity ent)
	{
		...
		int teamPlayerIndex = Singleton<CHeroSelectBaseSystem>.get_instance().GetTeamPlayerIndex(ent.ullUid, masterMemberInfo.camp);
	...
	public void Refresh_List(CHeroSelectChatView.Tab type)
	{
		...
		{
			this._refresh_list(this.show_listScript, Singleton<CChatController>.get_instance().model.GetCurGroupTemplateInfo());
		}
		else if (type == CHeroSelectChatView.Tab.All)
		{
			CChatChannel channel = Singleton<CChatController>.get_instance().model.channelMgr.GetChannel(EChatChannel.Select_Hero);
	...
	private void _refresh_list(CUIListScript listScript, List<CChatModel.HeroChatTemplateInfo> data_list)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				elemenet.transform.Find("Text").GetComponent<Text>().text = data_list.get_Item(i).GetTemplateString();
			}
		}
	}
	...
	private void _refresh_list(CUIListScript listScript, ListView<CChatEntity> data_list)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (elemenet != null && listScript.IsElementInScrollArea(i))
			{
				elemenet.transform.Find("Text").GetComponent<Text>().text = string.Format("{0}：{1}", data_list.get_Item(i).name, data_list.get_Item(i).text);
			}
		}
	}
	...
	private string _get_current_info(CHeroSelectChatView.Tab type, int index)
	{
		...
		if (type == CHeroSelectChatView.Tab.Template)
		{
			result = Singleton<CChatController>.get_instance().model.Get_HeroSelect_ChatTemplate(index).GetTemplateString();
		}
		else if (type == CHeroSelectChatView.Tab.All)
		{
			...
			{
				result = string.Format("{0}：{1}", list.get_Item(index).name, list.get_Item(index).text);
			}
		}
		return result;
	}
