...
public class LoaderHelper
{
	...
	{
		LoaderHelper.<GetGlobalPreload>c__Iterator24 <GetGlobalPreload>c__Iterator = new LoaderHelper.<GetGlobalPreload>c__Iterator24();
		<GetGlobalPreload>c__Iterator.result = result;
		<GetGlobalPreload>c__Iterator.<$>result = result;
		<GetGlobalPreload>c__Iterator.<>f__this = this;
		return <GetGlobalPreload>c__Iterator;
	}

	public void BuildStaticActor(ref List<ActorPreloadTab> list)
	{
		DebugHelper.Assert(MonoSingleton<GameLoader>.get_instance().staticActors != null, "Static actors cannot be null");
		for (int i = 0; i < MonoSingleton<GameLoader>.get_instance().staticActors.get_Count(); i++)
		{
			DebugHelper.Assert(MonoSingleton<GameLoader>.get_instance().staticActors.get_Item(i) != null, "actor config cannot be null at {0}", new object[]
			{
				i
			});
			if (MonoSingleton<GameLoader>.get_instance().staticActors.get_Item(i) != null)
			{
				ActorMeta actorMeta = default(ActorMeta);
				actorMeta.ActorType = MonoSingleton<GameLoader>.get_instance().staticActors.get_Item(i).ActorType;
				actorMeta.ConfigId = MonoSingleton<GameLoader>.get_instance().staticActors.get_Item(i).ConfigID;
				actorMeta.ActorCamp = MonoSingleton<GameLoader>.get_instance().staticActors.get_Item(i).CmpType;
	...
	{
		for (int i = 0; i < MonoSingleton<GameLoader>.get_instance().actorList.get_Count(); i++)
		{
			ActorMeta actorMeta = MonoSingleton<GameLoader>.get_instance().actorList.get_Item(i);
			this.AddPreloadActor(ref list, ref actorMeta, 0f, 0);
		}
	}
	...
	public void BuildSoldierRegions(ref List<ActorPreloadTab> list)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (soldierRegion != null && soldierRegion.Waves != null && soldierRegion.Waves.get_Count() > 0)
			{
				for (int j = 0; j < soldierRegion.Waves.get_Count(); j++)
				{
					Assets.Scripts.GameLogic.SoldierWave soldierWave = soldierRegion.Waves.get_Item(j);
		...
		if (array != null && array.Length > 0)
		{
			for (int i = 0; i < array.Length; i++)
			{
				...
				for (int j = 0; j < areaEventTrigger.TriggerActions.Length; j++)
				{
					...
					{
						Singleton<ShenFuSystem>.get_instance().PreLoadResource(triggerActionWrapper, ref loadInfo, this);
					}
				}
	...
	public void AddPreloadActor(ref List<ActorPreloadTab> list, ref ActorMeta actorMeta, float spawnCnt, int ownerSkinID = 0)
	{
		...
		CActorInfo actorInfo = CActorInfo.GetActorInfo(inStaticData.TheResInfo.ResPath, 0);
		if (actorInfo == null)
		{
			return;
		}
		...
		if (actorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
		{
			...
			if (dataByKey != null)
			{
				this.AnalyseHeroTalent(ref actorPreloadTab2, dataByKey);
			}
			this.AnalyseHeroBornAndReviveAge(ref actorPreloadTab2, actorMeta.ConfigId);
			ResHeroCfgInfo dataByKey2 = GameDataMgr.heroDatabin.GetDataByKey((uint)actorMeta.ConfigId);
			if (dataByKey2 != null)
			{
				this.AnalysePassiveSkill(ref actorPreloadTab2, dataByKey2.iPassiveID1);
				this.AnalysePassiveSkill(ref actorPreloadTab2, dataByKey2.iPassiveID2);
			}
		}
		else if (actorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
		{
			ActorStaticData actorStaticData = default(ActorStaticData);
			IGameActorDataProvider actorDataProvider3 = Singleton<ActorDataCenter>.get_instance().GetActorDataProvider(GameActorDataProviderType.StaticBattleDataProvider);
			...
			if (randomPassiveSkillRule > 0 && !this.randomSkillCheckerSet.ContainsKey(randomPassiveSkillRule))
			{
				...
				ResRandomSkillPassiveRule dataByKey3 = GameDataMgr.randomSkillPassiveDatabin.GetDataByKey((long)randomPassiveSkillRule);
				if (dataByKey3.astRandomSkillPassiveID1 != null && dataByKey3.astRandomSkillPassiveID1.Length > 0)
				{
					for (int k = 0; k < dataByKey3.astRandomSkillPassiveID1.Length; k++)
					{
						this.AnalysePassiveSkill(ref actorPreloadTab2, dataByKey3.astRandomSkillPassiveID1[k].iParam);
					}
				}
				if (dataByKey3.astRandomSkillPassiveID2 != null && dataByKey3.astRandomSkillPassiveID2.Length > 0)
				{
					for (int l = 0; l < dataByKey3.astRandomSkillPassiveID2.Length; l++)
					{
						this.AnalysePassiveSkill(ref actorPreloadTab2, dataByKey3.astRandomSkillPassiveID2[l].iParam);
					}
				}
			}
		}
			...
			if (dataByKey != null)
			{
				...
				if (dataByKey.astSkillFuncInfo != null && dataByKey.astSkillFuncInfo.Length > 0)
				{
					for (int i = 0; i < dataByKey.astSkillFuncInfo.Length; i++)
					{
						...
						if (resDT_SkillFunc.bSkillFuncType == 28 || resDT_SkillFunc.bSkillFuncType == 33 || resDT_SkillFunc.bSkillFuncType == 54 || resDT_SkillFunc.bSkillFuncType == 55 || resDT_SkillFunc.bSkillFuncType == 27 || resDT_SkillFunc.bSkillFuncType == 84)
						{
							if (resDT_SkillFunc.astSkillFuncParam != null && resDT_SkillFunc.astSkillFuncParam.Length != 0)
							{
								...
								{
									skillID = resDT_SkillFunc.astSkillFuncParam[1].iParam;
								}
	...
	{
		LoaderHelper.<ReduceSkillRefDatabin>c__Iterator25 <ReduceSkillRefDatabin>c__Iterator = new LoaderHelper.<ReduceSkillRefDatabin>c__Iterator25();
