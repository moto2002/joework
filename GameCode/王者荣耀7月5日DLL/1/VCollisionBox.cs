...
public class VCollisionBox : VCollisionShape
{
	...
	public override bool Intersects(VCollisionSphere s)
	{
		...
		if (this._tempDist.get_sqrMagnitudeLong() > this._tempRadius * this._tempRadius)
		{
			return false;
		}
		VInt3 vInt = s.WorldPos;
		long num = (long)s.WorldRadius;
		VInt3 vInt2 = this.ClosestPoint(vInt);
		return (vInt - vInt2).get_sqrMagnitudeLong() <= num * num;
	}
	...
	public VInt3 ClosestPoint(VInt3 targetPoint)
	{
		...
		VInt3 vInt2 = this.worldPos;
		int num = this.worldExtends.x * 1000;
		int num2 = this.worldExtends.y * 1000;
		int num3 = this.worldExtends.z * 1000;
		vInt2 += IntMath.Divide(this.axis[0], (long)Mathf.Clamp(VInt3.Dot(ref vInt, ref this.axis[0]), -num, num), 1000000L);
		vInt2 += IntMath.Divide(this.axis[1], (long)Mathf.Clamp(VInt3.Dot(ref vInt, ref this.axis[1]), -num2, num2), 1000000L);
		return vInt2 + IntMath.Divide(this.axis[2], (long)Mathf.Clamp(VInt3.Dot(ref vInt, ref this.axis[2]), -num3, num3), 1000000L);
	}
