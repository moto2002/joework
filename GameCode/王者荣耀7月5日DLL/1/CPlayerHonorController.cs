...
public class CPlayerHonorController : Singleton<CPlayerHonorController>
{
	...
	private void OnHonorChosen(CUIEvent uiEvent)
	{
		...
		CSPKG_USEHONOR_REQ stUseHonorReq = cSPkg.stPkgData.get_stUseHonorReq();
	...
	public static void ReceiveHonorChosenRsp(CSPkg msg)
	{
		...
		SCPKG_USEHONOR_RSP stUseHonorRsp = msg.stPkgData.get_stUseHonorRsp();
		if (stUseHonorRsp.iErrorCode != 0)
		{
			Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(1417, msg.stPkgData.get_stUseHonorRsp().iErrorCode), false, 1.5f, null, new object[0]);
			if (form != null)
			{
				COMDT_HONORINFO cOMDT_HONORINFO = new COMDT_HONORINFO();
				cOMDT_HONORINFO.iHonorID = msg.stPkgData.get_stUseHonorRsp().iHonorID;
		...
		{
			masterRoleInfo.selectedHonorID = msg.stPkgData.get_stUseHonorRsp().iHonorID;
			CPlayerProfile profile = Singleton<CPlayerInfoSystem>.GetInstance().GetProfile();
