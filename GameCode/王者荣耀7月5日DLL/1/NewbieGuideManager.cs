...
public class NewbieGuideManager : MonoSingleton<NewbieGuideManager>
{
	...
	public static void CompleteAllNewbieGuide()
	{
		...
		if (masterRoleInfo != null)
		{
			...
			masterRoleInfo.GameDifficult = 3;
		}
	...
	private void CheckWeakSkipCondition(NewbieGuideSkipConditionType type, params uint[] param)
	{
		...
		int count = newbieGuideWeakMianLineConfListBySkipType.get_Count();
		for (int i = 0; i < count; i++)
		{
			NewbieWeakGuideMainLineConf newbieWeakGuideMainLineConf = newbieGuideWeakMianLineConfListBySkipType.get_Item(i);
	...
	{
		if ((!this.IsMianLineComplete(conf.dwID) || conf.iSavePoint == -1) && !Singleton<WatchController>.get_instance().IsWatching)
		{
			bool flag = true;
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
			if (masterRoleInfo != null)
	...
	{
		SCPKG_NTF_NEWIEALLBITSYN stNewieAllBitSyn = msg.stPkgData.get_stNewieAllBitSyn();
		CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
		if (masterRoleInfo != null)
		{
	...
	private void CommitIsCompleteNewbieGuide(bool IsComplete)
	{
		...
		cSPkg.stPkgData.get_stAcntSetOldType().bYes = ((!IsComplete) ? 0 : 1);
