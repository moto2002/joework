...
namespace AGE
{
	public class CameraShakeDuration : DurationEvent
	{
		...
		public bool ShouldShake(Action _action)
		{
			...
			if (hostPlayer != null)
			{
				Player player = Singleton<GamePlayerCenter>.get_instance().GetPlayer(originator.get_handle().TheActorMeta.PlayerId);
				if (player != null)
				{
					if (this.filter_enemy && player.PlayerCamp != hostPlayer.PlayerCamp)
					{
						return this.CheckShakeDistance(hostPlayer.Captain.get_handle(), originator.get_handle());
					}
