...
namespace AGE
{
	...
	public class MoveBeamDuration : DurationCondition
	{
		...
		{
			Vector3 srcPosition = this.GetSrcPosition();
			Vector3 position = (Vector3)IntMath.Transform(this.bindDestOffet, this.targetActor.get_handle().forward, (VInt3)this.targetActor.get_handle().myTransform.position);
		...
		public override void Process(Action _action, Track _track, int _localTime)
		{
			...
			if (this.bInit || !this.bDone)
			{
				...
				VInt3 location = this.srcActor.get_handle().location;
				VInt3 location2 = this.targetActor.get_handle().location;
				int magnitude2D = (location2 - location).get_magnitude2D();
				if (magnitude2D > 0)
				{
					this.SetBeamLength(magnitude2D);
					this.RenderBeam();
				}
				if (this.srcActor.get_handle().ActorControl.IsDeadState || this.targetActor.get_handle().ActorControl.IsDeadState)
				{
					this.bDone = true;
					this.HideBeam();
					return;
