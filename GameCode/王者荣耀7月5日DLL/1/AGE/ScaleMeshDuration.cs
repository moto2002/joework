...
namespace AGE
{
	...
	public class ScaleMeshDuration : DurationEvent
	{
		...
		{
			if (this.actorObj && this.actorObj.get_handle().ActorMesh != null)
			{
				float num = (float)_scaleRate / 10000f;
				Vector3 localScale = new Vector3(num, num, num);
				this.originalScale = this.actorObj.get_handle().ActorMesh.transform.localScale;
				this.actorObj.get_handle().ActorMesh.transform.localScale = localScale;
			}
		}
		...
		private void RecorveMeshScale()
		{
			if (this.actorObj && this.actorObj.get_handle().ActorMesh != null)
			{
				this.actorObj.get_handle().ActorMesh.transform.localScale = this.originalScale;
			}
		}
		...
		private void SetCollisionScale(int _scaleRate)
		{
			if (this.actorObj)
			{
				...
				VCollisionShape shape = this.actorObj.get_handle().shape;
				if (shape != null)
				{
					int roundInt = vFactor.get_roundInt();
