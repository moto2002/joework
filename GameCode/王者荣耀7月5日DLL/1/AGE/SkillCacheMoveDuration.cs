...
namespace AGE
{
	...
	internal class SkillCacheMoveDuration : DurationCondition
	{
		...
		public override void Enter(Action _action, Track _track)
		{
			...
			this.targetActor.get_handle().ObjLinker.AddCustomMoveLerp(new CustomMoveLerpFunc(this.ActionMoveLerp));
			this.skillControl = this.targetActor.get_handle().SkillControl;
		...
		public override void Leave(Action _action, Track _track)
		{
			if (this.targetActor)
			{
				this.targetActor.get_handle().ObjLinker.RmvCustomMoveLerp(new CustomMoveLerpFunc(this.ActionMoveLerp));
			}
			if (!this.bInit || this.skillControl == null || this.skillControl.SkillUseCache == null)
			{
				return;
