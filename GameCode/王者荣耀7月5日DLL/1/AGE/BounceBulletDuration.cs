	...
	public class BounceBulletDuration : DurationCondition
	{
		...
		private void SpawnBuff(Action _action, int triggerCount)
		{
			if (this.tarActor && this.attackActor)
			{
				...
				bool flag = this.attackActor.get_handle().SkillControl.SpawnBuff(this.tarActor, refParamObject, this.TargetSkillCombine_1, false);
				flag |= this.attackActor.get_handle().SkillControl.SpawnBuff(this.tarActor, refParamObject, this.TargetSkillCombine_2, false);
				flag |= this.attackActor.get_handle().SkillControl.SpawnBuff(this.tarActor, refParamObject, this.TargetSkillCombine_3, false);
				if (flag)
				{
					this.tarActor.get_handle().ActorControl.BeAttackHit(this.attackActor, refParamObject.bExposing);
				}
			}
		}
		...
		private PoolObjHandle<ActorRoot> SearchTarget()
		{
			...
			for (int i = 0; i < gameActors.get_Count(); i++)
			{
				...
				ActorRoot handle = poolObjHandle3.get_handle();
				if (this.attackActor.get_handle().CanAttack(handle) && (num2 & 1 << (int)handle.TheActorMeta.ActorType) <= 0 && handle.ObjID != this.tarActor.get_handle().ObjID && handle.HorizonMarker.IsVisibleFor(this.attackActor.get_handle().TheActorMeta.ActorCamp))
				{
					if (!this.effectCountMap.TryGetValue(handle.ObjID, ref num) || num < this.maxTargetCount)
					{
						ulong sqrMagnitudeLong2D = (ulong)(handle.location - this.tarActor.get_handle().location).get_sqrMagnitudeLong2D();
		...
		{
			int num = (int)IntMath.Divide((long)(this.destPosition - this.moveActor.get_handle().location).get_magnitude2D() * 1000L, (long)this.velocity);
			if (num == 0)
			{
				return false;
			}
			...
			{
				this.gravityControler.InitMotionControler(num, vInt.i - this.moveActor.get_handle().location.y, this.gravity);
			}
			else
			{
