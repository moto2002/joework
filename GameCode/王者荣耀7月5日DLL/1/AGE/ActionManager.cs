...
namespace AGE
{
	public class ActionManager : MonoSingleton<ActionManager>
	{
		...
		public int ActionUpdatingCount
		{
			get
			{
				return this.actionUpdatingList.get_Count();
			}
		}
		...
		public static ActionManager Instance
		{
			get
			{
				return MonoSingleton<ActionManager>.get_instance();
			}
		}
		...
		{
			if (MonoSingleton<ActionManager>.get_instance() == null)
			{
				return;
			}
			this.StopAllActions();
		...
		{
			if (this.actionList.get_Count() == 0)
			{
				return;
			}
			...
			{
				Action action = listView.get_Item(i);
				action.Stop(true);
			}
		}
		...
		public void UpdateLogic(int nDelta)
		{
			...
			for (int i = 0; i < this.waitReleaseList.get_Count(); i++)
			{
				...
				{
					this.waitReleaseList.get_Item(i).get_handle().Stop(true);
				}
			}
			this.waitReleaseList.Clear();
			this.actionUpdatingList.Clear();
			int count = this.actionList.get_Count();
			for (int j = 0; j < count; j++)
			{
				this.actionUpdatingList.Add(this.actionList.get_Item(j));
			}
			count = this.actionUpdatingList.get_Count();
			for (int k = 0; k < count; k++)
			{
				if (!this.actionUpdatingList.get_Item(k).nextDestroy)
				{
					this.actionUpdatingList.get_Item(k).UpdateLogic(nDelta);
				}
			}
			for (int l = 0; l < this.waitReleaseList.get_Count(); l++)
			{
				...
				{
					this.waitReleaseList.get_Item(l).get_handle().Stop(true);
				}
			}
			this.waitReleaseList.Clear();
		}
		...
		private Action InternalPlayAction(string _actionName, bool _autoPlay, bool _stopConflictAction, GameObject[] _gameObjects)
		{
			...
			if (gameObject && _stopConflictAction)
			{
				...
				ListView<Action> listView = this.objectReferenceSet.get_Item(gameObject);
			...
			for (int j = 0; j < _gameObjects.Length; j++)
			{
				...
				if (!(gameObject3 == null))
				{
					...
					if (this.objectReferenceSet.TryGetValue(gameObject3, ref listView2))
					{
		...
		public void DestroyObject(GameObject _gameObject)
		{
			...
			if (this.objectReferenceSet.TryGetValue(_gameObject, ref listView) && listView != null)
			{
				for (int i = 0; i < listView.get_Count(); i++)
				{
					listView.get_Item(i).ClearGameObject(_gameObject);
				}
			}
		...
		public void RemoveAction(Action _action)
		{
			...
			int count = gameObjectList.get_Count();
			for (int i = 0; i < count; i++)
			{
				GameObject gameObject = gameObjectList.get_Item(i);
				if (!(gameObject == null))
				{
			...
			MonoSingleton<ActionManager>.get_instance().actionList.Remove(_action);
			_action.Release();
		}

		public void DeferReleaseAction(Action _action)
		{
			MonoSingleton<ActionManager>.get_instance().waitReleaseList.Add(new PoolObjHandle<Action>(_action));
		}
		...
		public Action LoadActionResource(string _actionName)
		{
			...
			if (this.actionResourceSet.TryGetValue(_actionName, ref action))
			{
				if (action != null)
				{
					return action;
						...
						if (securityElement6.get_Children() != null)
						{
							...
							for (int m = 0; m < securityElement6.get_Children().get_Count(); m++)
							{
								...
								else if (!(securityElement8.get_Tag() != "Event"))
								{
									...
									for (int n = 0; n < listView.get_Count(); n++)
									{
										this.SetEventField(action, baseEvent, listView.get_Item(n));
									}
		...
		private void LoadRefParamNode(Action result, SecurityElement paramNode)
		{
			...
			{
				int num = int.Parse(paramNode.Attribute("x"));
				int num2 = int.Parse(paramNode.Attribute("y"));
				int num3 = int.Parse(paramNode.Attribute("z"));
				VInt3 value8 = new VInt3(num, num2, num3);
				result.refParams.AddRefParam(name, value8);
			}
			else if (text == "quaternion")
			{
				float x2 = float.Parse(paramNode.Attribute("x"));
				float y2 = float.Parse(paramNode.Attribute("y"));
				float z2 = float.Parse(paramNode.Attribute("z"));
				float w = float.Parse(paramNode.Attribute("w"));
				Quaternion value9 = new Quaternion(x2, y2, z2, w);
			...
			else if (text == "eulerangle")
			{
				...
				Quaternion value10 = Quaternion.Euler(x3, y3, z3);
		...
		private void SetEventField(Action action, BaseEvent _trackEvent, SecurityElement _fieldNode)
		{
			...
			{
				int num5 = int.Parse(_fieldNode.Attribute("x"));
				int num6 = int.Parse(_fieldNode.Attribute("y"));
				int num7 = int.Parse(_fieldNode.Attribute("z"));
				VInt3 vInt = new VInt3(num5, num6, num7);
			...
			{
				float x2 = float.Parse(_fieldNode.Attribute("x"));
				float y2 = float.Parse(_fieldNode.Attribute("y"));
				float z2 = float.Parse(_fieldNode.Attribute("z"));
			...
			else if (text == "eulerangle")
			{
				...
				Quaternion quaternion2 = Quaternion.Euler(x3, y3, z3);
			...
			else if (text == "enum")
			{
				...
				if (flag)
				{
					int num8 = 0;
					action.refParams.AddRefData(name, field, _trackEvent);
					if (action.refParams.GetRefParam(name, ref num8))
					{
						field.SetValue(_trackEvent, num8);
					}
				}
			}
			else if (text == "array")
			{
				if (_fieldNode.get_Children() != null)
				{
					...
					if (text3 == "vector3i")
					{
						...
						int num9 = 0;
						IEnumerator enumerator = _fieldNode.get_Children().GetEnumerator();
						try
						{
							while (enumerator.MoveNext())
							{
								SecurityElement securityElement = (SecurityElement)enumerator.get_Current();
								int num10 = int.Parse(securityElement.Attribute("x"));
								int num11 = int.Parse(securityElement.Attribute("y"));
								int num12 = int.Parse(securityElement.Attribute("z"));
								VInt3 vInt2 = new VInt3(num10, num11, num12);
								array[num9++] = vInt2;
							}
						}
						finally
					...
					else if (text3 == "int")
					{
						...
						int num13 = 0;
						IEnumerator enumerator2 = _fieldNode.get_Children().GetEnumerator();
						try
						{
							while (enumerator2.MoveNext())
							{
					...
					else if (text3 == "templateobject" || text3 == "trackobject")
					{
						...
						int num15 = 0;
						IEnumerator enumerator3 = _fieldNode.get_Children().GetEnumerator();
						try
						{
					...
					else if (text3 == "uint")
					{
						...
						int num17 = 0;
						IEnumerator enumerator4 = _fieldNode.get_Children().GetEnumerator();
						try
						{
					...
					else if (text3 == "bool")
					{
						...
						int num19 = 0;
						IEnumerator enumerator5 = _fieldNode.get_Children().GetEnumerator();
						try
						{
					...
					else if (text3 == "float")
					{
						...
						int num20 = 0;
						IEnumerator enumerator6 = _fieldNode.get_Children().GetEnumerator();
						try
						{
					...
					else if (text3 == "string")
					{
						...
						int num22 = 0;
						IEnumerator enumerator7 = _fieldNode.get_Children().GetEnumerator();
						try
						{
		...
		public bool GetActionTemplateObjectsAndPredefRefParams(string actionName, ref List<TemplateObject> objs, ref List<string> refnames)
		{
			...
			{
				actionCommonData = this.actionCommonDataSet.get_Item(actionName);
			}
			else
			{
				CBinaryObject cBinaryObject = ActionManager.Instance.resLoader.LoadAge(actionName) as CBinaryObject;
				...
				if (securityElement2 != null)
				{
					IEnumerator enumerator = securityElement2.get_Children().GetEnumerator();
					try
					{
						while (enumerator.MoveNext())
						{
				...
				{
					IEnumerator enumerator2 = securityElement4.get_Children().GetEnumerator();
					try
					{
						while (enumerator2.MoveNext())
						{
			...
			if (actionCommonData != null)
			{
				objs.Clear();
				refnames.Clear();
				for (int i = 0; i < actionCommonData.templateObjects.get_Count(); i++)
				{
					objs.Add(actionCommonData.templateObjects.get_Item(i));
				}
				for (int j = 0; j < actionCommonData.predefRefParamNames.get_Count(); j++)
				{
					refnames.Add(actionCommonData.predefRefParamNames.get_Item(j));
				}
			}
			return true;
		}
		...
		public ActionSet FilterActionsByGameObject(GameObject obj, string nameInAction)
		{
			...
			for (int i = 0; i < this.actionList.get_Count(); i++)
			{
				Action action = this.actionList.get_Item(i);
				int index = 0;
