	...
	public class TeamMemberBuffTick : TickEvent
	{
		...
		private void AddActorRootList(COM_PLAYERCAMP inPlayerCamp, ref List<PoolObjHandle<ActorRoot>> actorRootList)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				List<PoolObjHandle<ActorRoot>> list = new List<PoolObjHandle<ActorRoot>>(allHeroes.get_Count());
				for (int i = 0; i < allHeroes.get_Count(); i++)
				{
					list.Add(allHeroes.get_Item(i));
				}
				...
				if (!this.bSkipDead)
				{
					ReadonlyContextExtension.AddRange<PoolObjHandle<ActorRoot>>(actorRootList, allHeroes);
				}
				else
				{
					...
					{
						PoolObjHandle<ActorRoot> poolObjHandle = allHeroes.get_Item(j);
						if (poolObjHandle)
						{
							if (!poolObjHandle.get_handle().ActorControl.IsDeadState)
							{
