...
namespace AGE
{
	...
	public class SetAttackDirDuration : DurationCondition
	{
		...
		private void Init(Action _action, Track _track)
		{
			...
			if (this.actor_.get_handle().MovementComponent == null)
			{
				return;
			}
			this.bRotate = true;
			this.actor_.get_handle().MovementComponent.SetRotate(vInt, true);
			...
			if (this.length <= 30)
			{
				this.actor_.get_handle().rotation = this.toRot;
				this.done_ = true;
				return;
			}
		...
		public override void Process(Action _action, Track _track, int _localTime)
		{
			...
			{
				this.actor_.get_handle().rotation = Quaternion.Slerp(this.fromRot, this.toRot, (float)_localTime / (float)this.rotTime_);
			}
