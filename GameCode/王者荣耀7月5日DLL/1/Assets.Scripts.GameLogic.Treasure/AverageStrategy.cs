...
namespace Assets.Scripts.GameLogic.Treasure
{
	internal abstract class AverageStrategy : BaseStrategy
	{
		...
		public override void NotifyDropEvent(PoolObjHandle<ActorRoot> actor)
		{
			...
			if (actor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster && this.hasRemain && (int)FrameRandom.Random(100u) <= MonoSingleton<GlobalConfig>.get_instance().NormalMonsterDropItemProbability)
			{
				this.PlayDrop();
			}
		}
