...
public class SGameGraphicRaycaster : GraphicRaycaster
{
	...
	private void UpdateTiles_Editor()
	{
		...
		for (int j = 0; j < this.m_allItems.get_Count(); j++)
		{
			SGameGraphicRaycaster.Item item = this.m_allItems.get_Item(j);
	...
	public void UpdateTiles()
	{
		...
		for (int i = 0; i < this.m_allItems.get_Count(); i++)
		{
			SGameGraphicRaycaster.Item item = this.m_allItems.get_Item(i);
	...
	private void InitializeAllItems()
	{
		...
		if (this.raycastMode == SGameGraphicRaycaster.RaycastMode.Sgame_tile)
		{
			...
			for (int j = 0; j < this.m_allItems.get_Count(); j++)
			{
				SGameGraphicRaycaster.Item item2 = this.m_allItems.get_Item(j);
	...
	public void RemoveItem(Image image)
	{
		...
		for (int i = 0; i < this.m_allItems.get_Count(); i++)
		{
			if (this.m_allItems.get_Item(i).m_image == image)
			{
				if (this.raycastMode == SGameGraphicRaycaster.RaycastMode.Sgame_tile)
				{
					this.RemoveFromTileList(this.m_allItems.get_Item(i));
				}
	...
	public void RefreshItem(Image image)
	{
		...
		for (int i = 0; i < this.m_allItems.get_Count(); i++)
		{
			if (this.m_allItems.get_Item(i).m_image == image)
			{
				item = this.m_allItems.get_Item(i);
				break;
			}
		}
		if (item != null)
	...
	private void Raycast2(PointerEventData eventData, List<RaycastResult> resultAppendList, bool useTiles)
	{
		...
		{
			listView.get_Item(i).Raycast(this.m_RaycastResults, position, this.eventCamera);
		}
			...
			if (flag)
			{
				...
				if (num < hitDistance)
				{
					RaycastResult raycastResult = new RaycastResult
					{
						...
						module = this,
						distance = num,
						index = (float)resultAppendList.get_Count(),
						depth = raycastResults.get_Item(i).depth,
