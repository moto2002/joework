...
public class UpdateShadowPlane : ActorComponent
{
	...
	private void EnablePlaneShadow()
	{
		if (this.shadowMesh == null)
		{
			...
			if (this.shadowMesh == null)
			{
				GameObject original = Singleton<CResourceManager>.GetInstance().GetResource("Prefab_Characters/PlaneShadow.prefab", typeof(GameObject), 0, false, false).m_content as GameObject;
	...
	{
		if (!GameSettings.IsHighQuality || this.mats == null || this.mats.get_Count() == 0)
		{
			return;
		}
		bool flag = false;
