...
public abstract class VCollisionShape
{
	...
	public void ConditionalUpdateShape()
	{
		if (this.dirty)
		{
			ActorRoot handle = this.owner.get_handle();
	...
	public static VCollisionShape InitActorCollision(ActorRoot actor)
	{
		if (actor.shape == null)
		{
			...
			if (!VCollisionShape.s_componentCache.TryGetValue(actor.gameObject.GetInstanceID(), ref sCollisionComponent))
			{
				sCollisionComponent = actor.gameObject.GetComponent<SCollisionComponent>();
				VCollisionShape.s_componentCache.set_Item(actor.gameObject.GetInstanceID(), sCollisionComponent);
			}
	...
	public static VCollisionShape InitActorCollision(ActorRoot actor, GameObject gameObj, string actionName)
	{
		...
		{
			SCollisionComponent sCollisionComponent = null;
			if (null != gameObj && !VCollisionShape.s_componentCache.TryGetValue(actor.gameObject.GetInstanceID(), ref sCollisionComponent))
			{
