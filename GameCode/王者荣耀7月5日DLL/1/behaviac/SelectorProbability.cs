	...
	public class SelectorProbability : BehaviorNode
	{
		private class SelectorProbabilityTask : CompositeTask
		{
			...
			protected override EBTStatus update(Agent pAgent, EBTStatus childStatus)
			{
				...
				{
					BehaviorTask behaviorTask = this.m_children.get_Item(this.m_activeChildIndex);
				...
				for (int i = 0; i < this.m_children.get_Count(); i++)
				{
					...
					{
						BehaviorTask behaviorTask2 = this.m_children.get_Item(i);
		...
		protected override void load(int version, string agentType, List<property_t> properties)
		{
			...
			using (List<property_t>.Enumerator enumerator = properties.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					{
						if (current.value.get_Chars(0) != '\0')
						{
