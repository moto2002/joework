...
namespace behaviac
{
	public abstract class BehaviorTask
	{
		...
		{
			if (this.m_attachments == null || this.m_attachments.get_Count() == 0)
			{
				return true;
			}
			bool flag = false;
			for (int i = 0; i < this.m_attachments.get_Count(); i++)
			{
				AttachmentTask attachmentTask = this.m_attachments.get_Item(i);
		...
		private void InstantiatePars(Agent pAgent)
		{
			...
			{
				for (int i = 0; i < node.m_pars.get_Count(); i++)
				{
					Property property = node.m_pars.get_Item(i);
