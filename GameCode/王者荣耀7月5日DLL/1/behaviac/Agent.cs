...
namespace behaviac
{
	public class Agent : MonoBehaviour
	{
		...
		public class CTagObjectDescriptor
		{
			...
			{
				using (ListView<CMemberBase>.Enumerator enumerator = this.ms_members.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						CMemberBase current = enumerator.get_Current();
			...
			public CMemberBase GetMember(string memberName)
			{
				...
				{
					for (int i = 0; i < this.ms_members.get_Count(); i++)
					{
						CMemberBase cMemberBase = this.ms_members.get_Item(i);
		...
		protected virtual void OnDestroy()
		{
			...
			if (this.m_behaviorTreeTasks != null)
			{
				for (int i = 0; i < this.m_behaviorTreeTasks.get_Count(); i++)
				{
					BehaviorTreeTask behaviorTreeTask = this.m_behaviorTreeTasks.get_Item(i);
		...
		public void SetName(string instanceName)
		{
			if (string.IsNullOrEmpty(instanceName))
			{
				...
				else
				{
					...
					string text2;
					string expr_7C = text2 = fullName;
					int num3 = dictionary.get_Item(text2);
					int num4;
					expr_78.set_Item(expr_7C, (num4 = num3) + 1);
					num2 = num4;
				}
		...
		{
			CStringID cStringID = new CStringID(className);
			if (Agent.Metas.ContainsKey(cStringID))
			{
				return Agent.Metas.get_Item(cStringID);
			}
			Agent.CTagObjectDescriptor cTagObjectDescriptor = new Agent.CTagObjectDescriptor();
			Agent.Metas.Add(cStringID, cTagObjectDescriptor);
		...
		{
			CStringID cStringID = new CStringID(typeName);
			return Agent.ms_metas.ContainsKey(cStringID);
		}
		...
		public CMemberBase FindMember(uint propertyId)
		{
			...
			for (int i = 0; i < descriptor.ms_members.get_Count(); i++)
			{
				CMemberBase cMemberBase = descriptor.ms_members.get_Item(i);
		...
		public static CMemberBase FindMemberBase(CStringID agentClassId, CStringID propertyId)
		{
			...
			{
				Agent.CTagObjectDescriptor cTagObjectDescriptor = Agent.Metas.get_Item(agentClassId);
				for (int i = 0; i < cTagObjectDescriptor.ms_members.get_Count(); i++)
				{
					CMemberBase cMemberBase = cTagObjectDescriptor.ms_members.get_Item(i);
		...
		public static CMethodBase FindMethodBase(CStringID agentClassId, CStringID propertyId)
		{
			...
			{
				Agent.CTagObjectDescriptor cTagObjectDescriptor = Agent.Metas.get_Item(agentClassId);
				for (int i = 0; i < cTagObjectDescriptor.ms_methods.get_Count(); i++)
				{
					CMethodBase cMethodBase = cTagObjectDescriptor.ms_methods.get_Item(i);
			...
			if (!string.IsNullOrEmpty(relativePath))
			{
				...
				else
				{
					...
					if (this.m_currentBT != null)
					{
						...
						{
							Agent.BehaviorTreeStackItem_t behaviorTreeStackItem_t = new Agent.BehaviorTreeStackItem_t(this.m_currentBT, triggerMode, bByEvent);
							this.BTStack.Add(behaviorTreeStackItem_t);
						}
					...
					for (int i = 0; i < this.BehaviorTreeTasks.get_Count(); i++)
					{
						BehaviorTreeTask behaviorTreeTask2 = this.BehaviorTreeTasks.get_Item(i);
					...
					bool flag = false;
					if (behaviorTreeTask != null && this.BTStack.get_Count() > 0)
					{
						for (int j = 0; j < this.BTStack.get_Count(); j++)
						{
							Agent.BehaviorTreeStackItem_t behaviorTreeStackItem_t2 = this.BTStack.get_Item(j);
							if (behaviorTreeStackItem_t2.bt.GetName() == relativePath)
							{
								flag = true;
								break;
							}
		...
		private EBTStatus btexec_()
		{
			...
			{
				this.m_currentBT.reset(this);
				if (this.BTStack.get_Count() <= 0)
				{
					break;
				}
				Agent.BehaviorTreeStackItem_t behaviorTreeStackItem_t = this.BTStack.get_Item(this.BTStack.get_Count() - 1);
				this.m_currentBT = behaviorTreeStackItem_t.bt;
				this.BTStack.RemoveAt(this.BTStack.get_Count() - 1);
		...
		public void btunload(string relativePath)
		{
			...
			for (int i = 0; i < this.BTStack.get_Count(); i++)
			{
				Agent.BehaviorTreeStackItem_t behaviorTreeStackItem_t = this.BTStack.get_Item(i);
			...
			for (int j = 0; j < this.BehaviorTreeTasks.get_Count(); j++)
			{
				BehaviorTreeTask behaviorTreeTask = this.BehaviorTreeTasks.get_Item(j);
		...
		private CNamedEvent findEvent(string eventName)
		{
			...
			if (cNamedEvent != null)
			{
				...
				this.EventInfos.set_Item(cStringID, cNamedEvent2);
				return cNamedEvent2;
			}
			return null;
		}
		...
		{
			CStringID cStringID = new CStringID(eventName);
			if (this.EventInfos.ContainsKey(cStringID))
			{
				CNamedEvent cNamedEvent = this.EventInfos.get_Item(cStringID);
		...
		public static Type GetTypeFromName(string typeName)
		{
			...
			if (Agent.Metas.ContainsKey(cStringID))
			{
				Agent.CTagObjectDescriptor cTagObjectDescriptor = Agent.Metas.get_Item(cStringID);
