...
public class AkAmbient : AkEvent
{
	...
	private void OnEnable()
	{
		...
		else if (this.multiPositionTypeLabel == MultiPositionTypeLabel.MultiPosition_Mode)
		{
			...
			if (AkAmbient.multiPosEventTree.TryGetValue(this.eventID, ref akMultiPosEvent))
			{
			...
			AkSoundEngine.SetMultiplePositions(akMultiPosEvent.list.get_Item(0).gameObject, akPositionArray2, (ushort)akPositionArray2.Count, MultiPositionType.MultiPositionType_MultiSources);
		}
	}
	...
	private void OnDisable()
	{
		...
		{
			AkMultiPosEvent akMultiPosEvent = AkAmbient.multiPosEventTree.get_Item(this.eventID);
			if (akMultiPosEvent.list.get_Count() == 1)
			{
			...
			AkSoundEngine.SetMultiplePositions(akMultiPosEvent.list.get_Item(0).gameObject, akPositionArray, (ushort)akPositionArray.Count, MultiPositionType.MultiPositionType_MultiSources);
		}
	}
	...
	public override void HandleEvent(GameObject in_gameObject)
	{
		...
		else
		{
			...
			this.soundEmitterObject = akMultiPosEvent.list.get_Item(0).gameObject;
			if (this.enableActionOnEvent)
			{
				AkSoundEngine.ExecuteActionOnEvent((uint)this.eventID, this.actionOnEventType, akMultiPosEvent.list.get_Item(0).gameObject, (int)this.transitionDuration * 1000, this.curveInterpolation);
			}
			else
			{
				AkSoundEngine.PostEvent((uint)this.eventID, akMultiPosEvent.list.get_Item(0).gameObject, 1u, new AkCallbackManager.EventCallback(akMultiPosEvent.FinishedPlaying), null, 0u, null, 0u);
			}
		}
	}
	...
	{
		AkPositionArray akPositionArray = new AkPositionArray((uint)eventPosList.list.get_Count());
		for (int i = 0; i < eventPosList.list.get_Count(); i++)
		{
			akPositionArray.Add(eventPosList.list.get_Item(i).transform.position, eventPosList.list.get_Item(i).transform.forward);
		}
