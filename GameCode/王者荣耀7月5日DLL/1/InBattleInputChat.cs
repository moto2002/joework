...
public class InBattleInputChat
{
	...
	public class InBatChatEntity
	{
		...
		{
			return this.m_maxCooldownTime > 0u;
		}
	...
	public void Add(InBattleInputChat.InBatChatEntity ent)
	{
		...
		{
			List<string> list = Singleton<CChatController>.get_instance().ColorParser.Parse(this.m_view.lineWidth - 10, ent.rawText, this.m_view.fontSize);
			if (list.get_Count() == 1)
			{
		...
		if (this.m_chatEntityList.get_Count() == InBattleInputChat.CHAT_LINE_COUNT)
		{
			this.m_chatEntityList.RemoveAt(0);
		}
	...
	public InBattleInputChat.InBatChatEntity ConstructEnt(ulong ullUid, string playerName, string content, byte camp)
	{
		...
		{
			text = playerByUid.Captain.get_handle().TheStaticData.TheResInfo.Name;
			validChatEntity.bAlice = playerByUid.Captain.get_handle().IsHostCamp();
		}
	...
	private InBattleInputChat.InBatChatEntity GetValidChatEntity()
	{
		...
		if (this.m_caches.get_Count() > 0)
		{
			result = this.m_caches.get_Item(0);
	...
	private void ReclyChatEntity(InBattleInputChat.InBatChatEntity ent)
	{
		...
		int num = -1;
		for (int i = 0; i < this.m_chatEntityList.get_Count(); i++)
		{
			InBattleInputChat.InBatChatEntity inBatChatEntity = this.m_chatEntityList.get_Item(i);
