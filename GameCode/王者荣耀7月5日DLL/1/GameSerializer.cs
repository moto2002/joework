...
public class GameSerializer
{
	...
	private static ICustomizedComponentSerializer GetComponentSerlizer(Type type)
	{
		...
		if (GameSerializer.componentSerializerTypeCache.TryGetValue(type, ref result))
		{
			return result;
		}
		return null;
	...
	private static ICustomizedObjectSerializer GetObjectSerlizer(Type type)
	{
		...
		if (GameSerializer.objectSerializerTypeCache.TryGetValue(type, ref result))
		{
			return result;
		}
		return null;
	...
	private static GameObject Load(BinaryDomDocument document)
	{
		...
		BinaryNode root = document.get_Root();
		if (root != null)
		{
	...
	{
		GameSerializer.<LoadAsync>c__Iterator6 <LoadAsync>c__Iterator = new GameSerializer.<LoadAsync>c__Iterator6();
		<LoadAsync>c__Iterator.lightmapAsset = lightmapAsset;
		<LoadAsync>c__Iterator.holder = holder;
		<LoadAsync>c__Iterator.<$>lightmapAsset = lightmapAsset;
		<LoadAsync>c__Iterator.<$>holder = holder;
		<LoadAsync>c__Iterator.<>f__this = this;
		return <LoadAsync>c__Iterator;
	}
	...
	{
		GameSerializer.<LoadAsync>c__Iterator7 <LoadAsync>c__Iterator = new GameSerializer.<LoadAsync>c__Iterator7();
		<LoadAsync>c__Iterator.data = data;
		<LoadAsync>c__Iterator.holder = holder;
		<LoadAsync>c__Iterator.<$>data = data;
		<LoadAsync>c__Iterator.<$>holder = holder;
		<LoadAsync>c__Iterator.<>f__this = this;
	...
	{
		GameSerializer.<LoadRecursionOnceAsync>c__Iterator8 <LoadRecursionOnceAsync>c__Iterator = new GameSerializer.<LoadRecursionOnceAsync>c__Iterator8();
		<LoadRecursionOnceAsync>c__Iterator.domNode = domNode;
		<LoadRecursionOnceAsync>c__Iterator.parentGo = parentGo;
		<LoadRecursionOnceAsync>c__Iterator.result = result;
		<LoadRecursionOnceAsync>c__Iterator.<$>domNode = domNode;
		<LoadRecursionOnceAsync>c__Iterator.<$>parentGo = parentGo;
		<LoadRecursionOnceAsync>c__Iterator.<$>result = result;
		return <LoadRecursionOnceAsync>c__Iterator;
	}

	private static void LoadRecursionTwice(GameObject parentGo, BinaryNode domNode, GameObject go)
	{
		if (domNode == domNode.get_OwnerDocument().get_Root())
		{
		...
		{
			BinaryNode parentNode = domNode.get_ParentNode();
	...
	{
		GameSerializer.<LoadRecursionTwiceAsync>c__Iterator9 <LoadRecursionTwiceAsync>c__Iterator = new GameSerializer.<LoadRecursionTwiceAsync>c__Iterator9();
		<LoadRecursionTwiceAsync>c__Iterator.domNode = domNode;
		<LoadRecursionTwiceAsync>c__Iterator.go = go;
		<LoadRecursionTwiceAsync>c__Iterator.<$>domNode = domNode;
		<LoadRecursionTwiceAsync>c__Iterator.<$>go = go;
		return <LoadRecursionTwiceAsync>c__Iterator;
	}
	...
	{
		GameSerializer.<LoadComponetsAsync>c__IteratorA <LoadComponetsAsync>c__IteratorA = new GameSerializer.<LoadComponetsAsync>c__IteratorA();
		<LoadComponetsAsync>c__IteratorA.domNode = domNode;
		<LoadComponetsAsync>c__IteratorA.go = go;
		<LoadComponetsAsync>c__IteratorA.<$>domNode = domNode;
		<LoadComponetsAsync>c__IteratorA.<$>go = go;
		return <LoadComponetsAsync>c__IteratorA;
	}
	...
	{
		GameSerializer.<GetObjectAsync>c__IteratorB <GetObjectAsync>c__IteratorB = new GameSerializer.<GetObjectAsync>c__IteratorB();
		<GetObjectAsync>c__IteratorB.currNode = currNode;
		<GetObjectAsync>c__IteratorB.holder = holder;
		<GetObjectAsync>c__IteratorB.<$>currNode = currNode;
		<GetObjectAsync>c__IteratorB.<$>holder = holder;
		return <GetObjectAsync>c__IteratorB;
	}
	...
	public static object GetResource(string pathName, Type type)
	{
		...
		{
			PrefabRefAsset prefabRefAsset = (PrefabRefAsset)Singleton<CResourceManager>.GetInstance().GetResource(text, typeof(PrefabRefAsset), 0, false, false).m_content;
		...
		{
			string text2 = pathName;
			string text3 = "Assets/Resources";
			int num = pathName.IndexOf(text3);
			if (num >= 0)
			{
				text2 = pathName.Substring(num + text3.get_Length() + 1);
			}
			result = Singleton<CResourceManager>.GetInstance().GetResource(text2, type, 0, false, false).m_content;
		}
		return result;
	}
	...
	private static void DeserializeObject(BinaryNode objInfoNode, GameObject go)
	{
		...
		if (binaryNode2 != null)
		{
			...
			if (component != null)
			{
				...
				{
					component.set_lightmapTilingOffset(UnityBasetypeSerializer.BytesToVector4(binaryAttribute4));
				}
			}
		}
	}
