...
public class Moba_Camera : MonoBehaviour
{
	...
	private void CalculateCameraMovement()
	{
		...
		{
			Vector3 position = this.settings.lockTarget.get_handle().myTransform.position;
			position.y += this.settings.targetHeight;
			bool bComposePlayerMovement = MonoSingleton<GlobalConfig>.get_instance().bComposePlayerMovement;
	...
	{
		DebugHelper.Assert(t && t.get_handle().CharInfo != null, "invalid parameter for SetTargetTransform", null);
		if (t && t.get_handle().CharInfo != null)
		{
			this.settings.lockTarget = t;
			this.settings.targetHeight = (float)t.get_handle().CharInfo.iBulletHeight * 0.001f;
		}
	}
