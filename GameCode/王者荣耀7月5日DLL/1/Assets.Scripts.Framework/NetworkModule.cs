	...
	public class NetworkModule : Singleton<NetworkModule>
	{
				...
				while (methods != null && num < methods.Length)
				{
					...
					if (methodInfo.get_IsStatic())
					{
						...
						for (int i = 0; i < customAttributes.Length; i++)
						{
							...
							{
								this.RegisterMsgHandler(messageHandlerAttribute.get_ID(), (NetMsgDelegate)Delegate.CreateDelegate(typeof(NetMsgDelegate), methodInfo));
								if (messageHandlerAttribute.get_AdditionalIdList() != null)
								{
									int num2 = messageHandlerAttribute.get_AdditionalIdList().Length;
									for (int j = 0; j < num2; j++)
									{
										this.RegisterMsgHandler(messageHandlerAttribute.get_AdditionalIdList()[j], (NetMsgDelegate)Delegate.CreateDelegate(typeof(NetMsgDelegate), methodInfo));
									}
								}
							}
						}
		...
		public static void InitRelayConnnecting(COMDT_TGWINFO inRelayTgw)
		{
			if (inRelayTgw.dwVipCnt > 0u)
			{
				...
				Singleton<ReconnectIpSelect>.get_instance().SetRelayTgw(inRelayTgw);
		...
		public void CloseGameServerConnect(bool switchLocal = true)
		{
			...
			MonoSingleton<Reconnection>.get_instance().ResetRelaySyncCache();
			Singleton<FrameWindow>.GetInstance().Reset();
		...
		private void HandleLobbyMsgRecv()
		{
			if (this.lobbySvr != null)
			{
				for (CSPkg cSPkg = this.lobbySvr.RecvPackage(); cSPkg != null; cSPkg = this.lobbySvr.RecvPackage())
				{
					...
					{
						uint num = (uint)(Time.realtimeSinceStartup * 1000f) - cSPkg.stPkgData.get_stGameSvrPing().dwTime;
		...
		private void UpdateLobbyConnection()
		{
			if (this.lobbySvr.CanSendPing() && Time.unscaledTime - this._preLobbyHeartTime > 5f)
			{
				...
				cSPkg.stPkgData.get_stGameSvrPing().dwTime = (uint)(Time.realtimeSinceStartup * 1000f);
		...
		{
			MonoSingleton<Reconnection>.get_instance().UpdateFrame();
			if (this.gameSvr.connected && Time.unscaledTime - this._preGameHeartTime > 3f)
			{
				...
				cSPkg.stPkgData.get_stRelaySvrPing().dwTime = (uint)(Time.realtimeSinceStartup * 1000f);
				cSPkg.stPkgData.get_stRelaySvrPing().dwSeqNo = (uint)Singleton<FrameSynchr>.GetInstance().m_SendHeartSeq;
				Singleton<FrameSynchr>.GetInstance().m_SendHeartSeq += 1L;
		...
		public bool SendLobbyMsg(ref CSPkg msg, bool isShowAlert = false)
		{
			if (this.isOnlineMode)
			{
				if (isShowAlert && !Singleton<BattleLogic>.get_instance().isRuning)
				{
