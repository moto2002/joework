...
namespace Assets.Scripts.Framework
{
	...
	public class FrameWindow : Singleton<FrameWindow>, IGameModule
	{
		...
		{
			if (FrameWindow.HandleFraqBootSingle(msg.stPkgData.get_stFrapBootSingle()))
			{
				MonoSingleton<Reconnection>.GetInstance().UpdateCachedLen(msg);
			}
			msg.Release();
		...
		private void RequestRepairFraqBootInfo()
		{
			...
			CSPKG_REQUESTFRAPBOOTSINGLE stReqFrapBootSingle = cSPkg.stPkgData.get_stReqFrapBootSingle();
			stReqFrapBootSingle.bNum = 0;
			for (uint num = 0u; num < 900u; num += 1u)
			{
				if ((int)stReqFrapBootSingle.bNum >= stReqFrapBootSingle.KFrapsNo.Length || (uint)stReqFrapBootSingle.bNum > this._maxFrqNo - this._begFrqNo + 2u)
				{
					break;
				}
				uint num2 = this._begFrqNo + num;
				if (this._receiveWindow[(int)((UIntPtr)this._FrqNoToWinIdx_(num2))] == null)
				{
					uint[] arg_A1_0 = stReqFrapBootSingle.KFrapsNo;
					CSPKG_REQUESTFRAPBOOTSINGLE expr_8D = stReqFrapBootSingle;
					byte bNum;
					expr_8D.bNum = (bNum = expr_8D.bNum) + 1;
					arg_A1_0[(int)bNum] = num2;
				}
			}
		...
		private void RequestTimeoutFraqBootInfo()
		{
			...
			cSPkg.stPkgData.get_stReqFrapBootTimeout().dwCurKFrapsNo = this._begFrqNo;
			Singleton<NetworkModule>.GetInstance().SendGameMsg(ref cSPkg, 0u);
		}
