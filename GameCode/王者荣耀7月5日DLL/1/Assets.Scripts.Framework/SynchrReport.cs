...
namespace Assets.Scripts.Framework
{
	...
	public class SynchrReport
	{
		...
		{
			SynchrReport._isSelfUnsync |= (pkg.stPkgData.get_stRelayHashChkRsp().dwIsSelfNE != 0u);
			SynchrReport._isDeskUnsync |= (pkg.stPkgData.get_stRelayHashChkRsp().dwIsDeskNE != 0u);
		...
		{
			SynchrReport.Upload((long)((ulong)pkg.stPkgData.get_stUploadCltlogReq().dwOffset));
		}

		private static void Upload(long offset)
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(5239u);
			CSPKG_UPLOADCLTLOG_NTF stUploadCltlogNtf = cSPkg.stPkgData.get_stUploadCltlogNtf();
			...
			if (SynchrReport._uploadList != null && SynchrReport._uploadIndex >= 0 && SynchrReport._uploadIndex < SynchrReport._uploadList.get_Count())
			{
				...
				if (offset < memoryStream.get_Length())
				{
					...
					if (memoryStream.get_Position() >= memoryStream.get_Length())
					{
						...
						stUploadCltlogNtf.bAllLogOver = ((!flag) ? 0 : 1);
					}
					else
					{
