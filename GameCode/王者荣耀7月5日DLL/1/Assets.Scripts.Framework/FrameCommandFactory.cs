...
namespace Assets.Scripts.Framework
{
	public class FrameCommandFactory
	{
		...
		public static FrameCommand<T> CreateFrameCommand<T>() where T : struct, ICommandImplement
		{
			...
			{
				isCSSync = false,
				cmdType = FrameCommandFactory.s_CommandTypeDef.get_Item(typeof(T)),
				cmdData = default(T)
			};
		}
		...
		public static FrameCommand<T> CreateCSSyncFrameCommand<T>() where T : struct, ICommandImplement
		{
			...
			{
				isCSSync = true,
				cmdType = FrameCommandFactory.s_CSSyncCommandTypeDef.get_Item(typeof(T)),
				cmdData = default(T)
			};
		}
		...
		public static FrameCommand<T> CreateSCSyncFrameCommand<T>() where T : struct, ICommandImplement
		{
			...
			{
				isCSSync = false,
				cmdType = FrameCommandFactory.s_SCSyncCommandTypeDef.get_Item(typeof(T)),
				cmdData = default(T)
			};
		}
		...
		public static FRAMECMD_ID_DEF GetCommandType(Type t)
		{
			...
			{
				return ((FrameCommandClassAttribute)customAttributes[0]).get_ID();
			}
			return 0;
		}
	}
