...
namespace Assets.Scripts.GameSystem
{
	public class CSoundChat
	{
		...
		public void Init(GameObject cooldownImage, GameObject tipObj)
		{
			...
			this.m_timer = Singleton<CTimerManager>.get_instance().AddTimer(MonoSingleton<VoiceSys>.GetInstance().TotalVoiceTime * 1000, -1, new CTimer.OnTimeUpHandler(this.OnTimerEnd));
			Singleton<CTimerManager>.get_instance().PauseTimer(this.m_timer);
			Singleton<CTimerManager>.get_instance().ResetTimer(this.m_timer);
		...
		public void OnBattle_Voice_Btn(CUIEvent uiEvent)
		{
			...
			if (CFakePvPHelper.bInFakeSelect)
			{
				if (!MonoSingleton<VoiceSys>.GetInstance().IsUseVoiceSysSetting)
				{
					...
					Singleton<CSoundManager>.get_instance().PlayBattleSound2D("UI_common_tishi");
					this.StartTimer();
		...
		private void StartTimer()
		{
			...
			Singleton<CTimerManager>.get_instance().ResumeTimer(this.m_timer);
		}
