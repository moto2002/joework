...
namespace Assets.Scripts.GameSystem
{
	...
	public class CArenaSystem : Singleton<CArenaSystem>
	{
		...
		private void Arena_OpenForm(CUIEvent uiEvent)
		{
			...
			{
				if (!Singleton<SCModuleControl>.get_instance().GetActiveModule(8))
				{
		...
		public void RefreshArenaForm()
		{
			...
			for (int j = 0; j < (int)this.m_fightHeroInfoList.stArenaInfo.stMatchFighter.bFigterNum; j++)
			{
				...
				if (component10)
				{
					MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component10, (int)fighterInfo.stVip.dwCurLevel, false);
				}
				if (component11)
				{
		...
		{
			Singleton<CHeroSelectBaseSystem>.get_instance().SetPveDataWithArena(0u, null, Singleton<CTextManager>.GetInstance().GetText("Arena_Arena_Defensive_Team"));
			Singleton<CHeroSelectBaseSystem>.get_instance().OpenForm(enSelectGameType.enArenaDefTeamConfig, 3, 0u, 0, 0);
		}
		...
		private void Arena_ConfirmResutlForm(CUIEvent uiEvent)
		{
			if (this.m_tarRank < this.m_serverInfo.dwTopRank || this.m_serverInfo.dwTopRank == 0u)
			{
				...
				{
					if (i < useableListFromReward.get_Count())
					{
						useableListFromReward.get_Item(i).SetMultiple(ref Singleton<SingleGameSettleMgr>.GetInstance().m_settleData.stDetail.stMultipleDetail, true);
						CUICommonSystem.SetItemCell(cUIFormScript, array[i].gameObject, useableListFromReward.get_Item(i), true, false, false, false);
					}
					else
					{
		...
		{
			uint shopInfoCfgId = CPurchaseSys.GetShopInfoCfgId(9, (int)(this.m_serverInfo.bBuyChallengeCnt + 1));
		...
		private void Arena_OpenPlayerInfoForm(CUIEvent uiEvent)
		{
			...
			if (component5)
			{
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component5, (int)fighterInfo.stVip.dwCurLevel, false);
			}
			if (component6)
			{
		...
		private void Arena_StartFight(CUIEvent uiEvent)
		{
			...
			Singleton<CHeroSelectBaseSystem>.get_instance().SetPveDataWithArena(dataByKey.dwBattleListID, cSDT_SINGLE_GAME_OF_ARENA, Singleton<CTextManager>.GetInstance().GetText("Arena_Arena"));
			Singleton<CHeroSelectBaseSystem>.get_instance().OpenForm(enSelectGameType.enArena, (byte)dataByKey.iHeroNum, 0u, 0, 0);
		}
		...
		private void Arena_RankInfoMenuClick(CUIEvent uiEvent)
		{
			...
			else if (selectedIndex == 1)
			{
				...
				for (int i = 0; i < component2.GetElementAmount(); i++)
				{
					...
					{
						if (j > rewardUseableList.get_Count() - 1)
						{
							array[j].CustomSetActive(false);
						}
						else
						{
							CUICommonSystem.SetItemCell(uiEvent.m_srcFormScript, array[j], rewardUseableList.get_Item(j), true, false, false, false);
							if (rewardUseableList.get_Item(j).m_stackCount == 0)
							{
								array[j].CustomSetActive(false);
							}
							else
				...
				{
					if (k > rewardUseableList2.get_Count() - 1)
					{
						array2[k].CustomSetActive(false);
					}
					else
					{
						CUICommonSystem.SetItemCell(uiEvent.m_srcFormScript, array2[k], rewardUseableList2.get_Item(k), true, false, false, false);
						if (rewardUseableList2.get_Item(k).m_stackCount == 0)
						{
							array2[k].CustomSetActive(false);
						}
						else
		...
		public ListView<CUseable> GetRewardUseableList(int rank)
		{
			...
			if (resArenaOneDayReward != null)
			{
				...
				listView.Add(CUseableManager.CreateUseable(2, resArenaOneDayReward.dwRewardPropId, (int)resArenaOneDayReward.dwRewardPropCnt));
			}
			return listView;
		}
		...
		private void Arena_RecordlementEnable(CUIEvent uiEvent)
		{
			...
			COMDT_ARENA_MEMBER_OF_ACNT fighterInfo = CArenaSystem.GetFighterInfo(this.m_recordList.get_Item(srcWidgetIndexInBelongedList).stTargetInfo);
			component.text = StringHelper.UTF8BytesToString(ref fighterInfo.szName);
			component2.text = Singleton<CTextManager>.GetInstance().GetText("Arena_Fighting_Power", new string[]
			{
				fighterInfo.dwPVPLevel.ToString()
			...
			component6.gameObject.CustomSetActive(false);
			component7.gameObject.CustomSetActive(false);
			if (this.m_recordList.get_Item(srcWidgetIndexInBelongedList).ullAtkerUid == playerUllUID)
			{
				component8.gameObject.CustomSetActive(true);
				if (this.m_recordList.get_Item(srcWidgetIndexInBelongedList).bResult == 1)
				{
					component3.text = Math.Abs((int)(this.m_recordList.get_Item(srcWidgetIndexInBelongedList).dwAtkerRank - this.m_recordList.get_Item(srcWidgetIndexInBelongedList).dwTargetRank)).ToString();
					component6.gameObject.CustomSetActive(true);
					component4.gameObject.CustomSetActive(true);
				}
				else
		...
		public static void SendSetDefTeamConfigMSG(List<uint> heroList)
		{
			...
			cSPkg.stPkgData.get_stSetBattleListOfArenaReq().stBattleList.wHeroCnt = (ushort)heroList.get_Count();
			cSPkg.stPkgData.get_stSetBattleListOfArenaReq().stBattleList.BattleHeroList = heroList.ToArray();
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void SendGetRankListMSG(bool bForceSend = true)
		{
			if (bForceSend || (long)CRoleInfo.GetElapseSecondsSinceLogin() - (long)((ulong)Singleton<CArenaSystem>.GetInstance().m_lastRankRequestTime) > 60L || Singleton<CArenaSystem>.GetInstance().m_rankInfoList == null)
			{
				...
				cSPkg.stPkgData.get_stGetTopFighterOfArenaReq().bTopNum = 100;
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
			}
		}
		...
		public static void SendBuyTimesMSG()
		{
			...
			cSPkg.stPkgData.get_stShopBuyReq().iBuyType = 9;
			cSPkg.stPkgData.get_stShopBuyReq().iBuySubType = (int)(Singleton<CArenaSystem>.GetInstance().m_serverInfo.bBuyChallengeCnt + 1);
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void SendResetCDMSG()
		{
			...
			cSPkg.stPkgData.get_stClrCdLimitReq().dwCdType = 1u;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static COMDT_ARENA_MEMBER_OF_ACNT GetFighterInfo(COMDT_ARENA_BLOCKDATA rankInfo)
		{
			...
			{
				cOMDT_ARENA_MEMBER_OF_ACNT = rankInfo.stMember.get_stAcnt();
			}
			else
			{
				cOMDT_ARENA_MEMBER_OF_ACNT.ullUid = (ulong)rankInfo.stMember.get_stNpc().dwObjId;
		...
		{
			SCPKG_JOINARENA_RSP stJoinArenaRsp = msg.stPkgData.get_stJoinArenaRsp();
		...
		{
			SCPKG_SETBATTLELISTOFARENA_RSP stSetBattleListOfArenaRsp = msg.stPkgData.get_stSetBattleListOfArenaRsp();
			if (stSetBattleListOfArenaRsp.bErrCode == 0)
			{
				Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_arenaDefHeroList.Clear();
				Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_arenaDefHeroList.AddRange(stSetBattleListOfArenaRsp.stResult.get_stSucc().stBattleList.BattleHeroList);
		...
		{
			SCPKG_CHGARENAFIGHTERRSP stChgArenaFighterRsp = msg.stPkgData.get_stChgArenaFighterRsp();
			Singleton<CArenaSystem>.GetInstance().m_fightHeroInfoList = stChgArenaFighterRsp;
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			if (Singleton<CArenaSystem>.get_instance().m_openArenaForm)
			{
				Singleton<CArenaSystem>.get_instance().m_openArenaForm = false;
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent("Arena_Fighter_Changed");
		}

		[MessageHandler(2910)]
		public static void ReciveRankListInfo(CSPkg msg)
		{
			SCPKG_GETTOPFIGHTEROFARENA_RSP stGetTopFighterOfArenaRsp = msg.stPkgData.get_stGetTopFighterOfArenaRsp();
			...
			{
				if (stGetTopFighterOfArenaRsp.stTopFighter.astFigterDetail[i].stFigterData.bMemberType == 1 && masterRoleInfo.playerUllUID == stGetTopFighterOfArenaRsp.stTopFighter.astFigterDetail[i].stFigterData.stMember.get_stAcnt().ullUid)
				{
					stGetTopFighterOfArenaRsp.stTopFighter.astFigterDetail[i].stFigterData.stMember.get_stAcnt().stBattleHero.astHero = new COMDT_ARENA_HERODETAIL[3];
					for (int j = 0; j < masterRoleInfo.m_arenaDefHeroList.get_Count(); j++)
					{
						...
						stGetTopFighterOfArenaRsp.stTopFighter.astFigterDetail[i].stFigterData.stMember.get_stAcnt().stBattleHero.astHero[j] = cOMDT_ARENA_HERODETAIL;
					}
					break;
				}
			}
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent("Rank_Arena_List");
		}

		[MessageHandler(1263)]
		public static void ReciveResetCD(CSPkg msg)
		{
			SCPKG_CLRCDLIMIT_RSP stClrCdLimitRsp = msg.stPkgData.get_stClrCdLimitRsp();
		...
		{
			SCPKG_GETARENAFIGHTHISTORY_RSP stGetArenaFightHistoryRsp = msg.stPkgData.get_stGetArenaFightHistoryRsp();
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent("Arena_Record_List");
		}
