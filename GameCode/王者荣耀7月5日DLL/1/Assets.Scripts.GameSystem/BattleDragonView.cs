	...
	public class BattleDragonView
	{
		...
		private void Draw()
		{
			...
			else
			{
				if (!this.m_isDragonOn)
				{
					...
					{
						return src.get_handle().TheActorMeta.ConfigId == Singleton<BattleLogic>.get_instance().DragonId && src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster;
					});
					if (actor)
					{
						actor.get_handle().HudControl.PlayMapEffect(MiniMapEffect.Dragon3V3SpawnEffect);
		...
		private void ShowText(COM_PLAYERCAMP killerCamp, int dragonNum)
		{
			...
			if (killerCamp == hostPlayer.PlayerCamp)
			{
				if (dragonNum == 1)
				{
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.GetDragonBuff1, (Vector3)hostPlayer.Captain.get_handle().location, new string[0]);
				}
				else if (dragonNum == 2)
				{
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.GetDragonBuff2, (Vector3)hostPlayer.Captain.get_handle().location, new string[0]);
				}
				else if (dragonNum == 3)
				{
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.GetDragonBuff3, (Vector3)hostPlayer.Captain.get_handle().location, new string[0]);
				}
			}
		}
	}
