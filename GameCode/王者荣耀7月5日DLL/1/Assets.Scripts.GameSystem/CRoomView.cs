	...
	internal class CRoomView
	{
		...
		public static void SetStartBtnStatus(GameObject root, RoomInfo data)
		{
			...
			if (isSelfRoomOwner)
			{
				...
				bool interactable = data[1].get_Count() > 0 && data[2].get_Count() > 0;
		...
		public static void UpdateBtnStatus(GameObject root, RoomInfo data)
		{
			...
			if (isSelfRoomOwner && masterRoleInfo != null)
			{
				...
				if (memberInfo != null)
				{
					COM_PLAYERCAMP cOM_PLAYERCAMP = (memberInfo.camp != 1) ? 1 : 2;
					if (data.GetFreePos(memberInfo.camp, bMaxAcntNum) >= 0)
					{
						if (memberInfo.camp == 1)
						{
		...
		private static void SetPlayerSlotData(GameObject item, MemberInfo memberInfo, MemberInfo masterMemberInfo, COM_PLAYERCAMP camp, int pos, bool bAvailable)
		{
			if (bAvailable)
			{
				...
				if (memberInfo == null)
				{
					...
					component.m_onClickEventParams.tag = camp;
				...
				else
				{
					...
					else if (memberInfo.RoomMemberType == 1u)
					{
						...
						component3.m_onClickEventParams.tag = camp;
						component3.m_onClickEventParams.tag2 = pos;
						component3.m_onClickEventParams.tag3 = 2;
						if (!CSysDynamicBlock.bFriendBlocked)
						{
							if (Singleton<CFriendContoller>.get_instance().model.GetGameOrSnsFriend(memberInfo.ullUid, (uint)memberInfo.iLogicWorldID) == null)
							{
								if (transform != null)
								{
		...
		public static void SetChgEnable(bool enable)
		{
			...
			for (int i = num; i <= num2; i++)
			{
				...
				for (int j = 0; j < 5; j++)
				{
					...
					bool flag2 = false;
					MemberInfo memberInfo = Singleton<CRoomSystem>.get_instance().roomInfo.GetMemberInfo(cOM_PLAYERCAMP, j);
					if (memberInfo != null)
					{
						flag2 = (memberInfo.RoomMemberType == 2u);
					}
					string path = string.Format("Panel_Main/{0}{1}/Occupied/BtnSwap", (cOM_PLAYERCAMP != 1) ? "RightPlayers/Right_Player" : "LeftPlayers/Left_Player", j + 1);
		...
		public static void SetChgEnable(bool enable, COM_PLAYERCAMP camp, int pos)
		{
			...
			string path = string.Format("Panel_Main/{0}{1}/Occupied/BtnSwap", (camp != 1) ? "RightPlayers/Right_Player" : "LeftPlayers/Left_Player", pos + 1);
			GameObject gameObject = Utility.FindChild(form.gameObject, path);
			if (gameObject == null)
			{
				return;
		...
		public static void SetSwapTimer(int totalSec, COM_PLAYERCAMP camp = 1, int pos = 0)
		{
			...
			for (int i = num; i <= num2; i++)
			{
				...
				for (int j = 0; j < 5; j++)
				{
					string path = string.Format("Panel_Main/{0}{1}/Occupied/TimerSwap", (cOM_PLAYERCAMP != 1) ? "RightPlayers/Right_Player" : "LeftPlayers/Left_Player", j + 1);
					GameObject gameObject = Utility.FindChild(form.gameObject, path);
		...
		public static void ShowSwapMsg(int totalSec, COM_PLAYERCAMP camp = 1, int pos = 0)
		{
			if (totalSec > 0)
			{
				...
				vector.y += (float)((camp != null) ? 28 : -20);
				...
				component.m_eventParams[0].tag2 = camp;
				component.m_eventParams[2].tag = pos;
				component.m_eventParams[2].tag2 = camp;
				component.ReStartTimer();
			}
			else
			{
		...
		public static void UpdateSwapBox(COM_PLAYERCAMP camp, int pos)
		{
			...
			componetInChild.text = Singleton<CTextManager>.get_instance().GetText("Room_Change_Pos_Tip_3", new string[]
			{
				Singleton<CTextManager>.get_instance().GetText("RoomCamp_" + camp),
				(pos + 1).ToString(),
