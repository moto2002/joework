...
namespace Assets.Scripts.GameSystem
{
	...
	public class CMallSystem : Singleton<CMallSystem>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.Mall_Set_Free_Draw_Timer, new Action(this.SetFreeDrawTimer));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.Mall_Sub_Module_Loaded, new Action(this.OnSubModuleLoaded));
			Singleton<EventRouter>.get_instance().AddEventHandler<CMallSystem.Tab>(EventID.Mall_Refresh_Tab_Red_Dot, new Action<CMallSystem.Tab>(this.RefreshTabRedDot));
		...
		public override void UnInit()
		{
			...
			instance.AddUIEventListener(enUIEventID.Mall_Sort_Type_Select, new CUIEventManager.OnUIEventHandler(this.OnSelectSortType));
			instance.AddUIEventListener(enUIEventID.Mall_Sort_Type_Change, new CUIEventManager.OnUIEventHandler(this.OnSortTypeChange));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.Mall_Set_Free_Draw_Timer, new Action(this.SetFreeDrawTimer));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.Mall_Sub_Module_Loaded, new Action(this.OnSubModuleLoaded));
			Singleton<EventRouter>.get_instance().RemoveEventHandler<CMallSystem.Tab>(EventID.Mall_Refresh_Tab_Red_Dot, new Action<CMallSystem.Tab>(this.RefreshTabRedDot));
			this.m_MallForm = null;
		}
		...
		public void OnMallOpenForm(CUIEvent uiEvent)
		{
			...
			GameObject gameObject2 = this.m_MallForm.gameObject.transform.FindChild("TopCommon/Button_Crystal").gameObject;
			Singleton<CUINewFlagSystem>.GetInstance().AddNewFlag(gameObject, enNewFlagKey.New_GiftCenterBtn_V1, enNewFlagPos.enTopRight, 0.8f, 0f, 0f, enNewFlagType.enNewFlag);
			Singleton<CUINewFlagSystem>.GetInstance().AddNewFlag(gameObject2, enNewFlagKey.New_CrystalBtn_V1, enNewFlagPos.enTopRight, 0.8f, -4f, 2f, enNewFlagType.enNewFlag);
				...
				for (int i = 0; i < cUIListScript.m_elementAmount; i++)
				{
					...
					if (elemenet != null)
					{
						...
						if (gameObject != null)
						{
							...
							if (this.m_AvailableTabList.get_Item(i) == CMallSystem.Tab.Symbol_Make && (CMallSystem.GetShopPromotion(10, 2) != null || CMallSystem.GetShopPromotion(10, 3) != null || CMallSystem.GetShopPromotion(11, 2) != null || CMallSystem.GetShopPromotion(11, 3) != null))
							{
							...
							if (this.m_AvailableTabList.get_Item(i) == CMallSystem.Tab.Roulette)
							{
								...
								if (Singleton<CMallRouletteController>.GetInstance().IsProbabilityDoubled(0))
								{
		...
		public static ResShopPromotion GetShopPromotion(RES_SHOPBUY_TYPE type, RES_SHOPDRAW_SUBTYPE subType)
		{
			...
			if (GameDataMgr.shopPromotionDict.ContainsKey(shopInfoCfgId))
			{
				...
				if (GameDataMgr.shopPromotionDict.TryGetValue(shopInfoCfgId, ref listView))
				{
					...
					if (listView != null)
					{
						...
						{
							ResShopPromotion resShopPromotion = listView.get_Item(i);
		...
		{
			ListView<CUseable> useableListFromReward = CUseableManager.GetUseableListFromReward(msg.stPkgData.get_stGiftUseGet().stRewardInfo);
			int count = useableListFromReward.get_Count();
			if (count == 0)
			{
				return;
			}
			...
			while (i < count)
			{
				switch (useableListFromReward.get_Item(i).m_type)
				{
				case 4:
					CUICommonSystem.ShowNewHeroOrSkin(useableListFromReward.get_Item(i).m_baseID, 0u, enUIEventID.None, true, 5, false, null, enFormPriority.Priority4, 0u, 0);
					break;
				case 5:
				case 6:
					goto IL_D0;
				case 7:
					CSkinInfo.ResolveHeroSkin(useableListFromReward.get_Item(i).m_baseID, out heroId, out skinId);
				...
				continue;
				IL_D0:
				if (useableListFromReward.get_Item(i).MapRewardType == 1 && (useableListFromReward.get_Item(i).ExtraFromType == 1 || useableListFromReward.get_Item(i).ExtraFromType == 2))
				{
					if (useableListFromReward.get_Item(i).ExtraFromType == 1)
					{
						heroId = (uint)useableListFromReward.get_Item(i).ExtraFromData;
						CUICommonSystem.ShowNewHeroOrSkin(heroId, 0u, enUIEventID.Activity_HeroShow_Back, true, 5, true, null, enFormPriority.Priority4, (uint)useableListFromReward.get_Item(i).m_stackCount, 0);
					}
					else if (useableListFromReward.get_Item(i).ExtraFromType == 2)
					{
						skinId = (uint)useableListFromReward.get_Item(i).ExtraFromData;
						CUICommonSystem.ShowNewHeroOrSkin(0u, skinId, enUIEventID.Activity_HeroShow_Back, true, 10, true, null, enFormPriority.Priority4, (uint)useableListFromReward.get_Item(i).m_stackCount, 0);
					}
				}
				goto IL_194;
			}
		...
		public static void ReceiveRouletteDataNTF(CSPkg msg)
		{
			...
			{
				cSDT_LUCKYDRAW_INFO = msg.stPkgData.get_stLuckyDrawDataNtf().stDiamond;
				CMallSystem.luckyDrawDic.Add(enPayType.Diamond, cSDT_LUCKYDRAW_INFO);
			}
			else
			{
				cSDT_LUCKYDRAW_INFO.dwCnt = msg.stPkgData.get_stLuckyDrawDataNtf().stDiamond.dwCnt;
				cSDT_LUCKYDRAW_INFO.dwDrawMask = msg.stPkgData.get_stLuckyDrawDataNtf().stDiamond.dwDrawMask;
				cSDT_LUCKYDRAW_INFO.dwReachMask = msg.stPkgData.get_stLuckyDrawDataNtf().stDiamond.dwReachMask;
				cSDT_LUCKYDRAW_INFO.dwLuckyPoint = msg.stPkgData.get_stLuckyDrawDataNtf().stDiamond.dwLuckyPoint;
			}
			...
			{
				cSDT_LUCKYDRAW_INFO2 = msg.stPkgData.get_stLuckyDrawDataNtf().stCoupons;
				CMallSystem.luckyDrawDic.Add(enPayType.DianQuan, cSDT_LUCKYDRAW_INFO2);
			}
			else
			{
				cSDT_LUCKYDRAW_INFO2.dwCnt = msg.stPkgData.get_stLuckyDrawDataNtf().stCoupons.dwCnt;
				cSDT_LUCKYDRAW_INFO2.dwDrawMask = msg.stPkgData.get_stLuckyDrawDataNtf().stCoupons.dwDrawMask;
				cSDT_LUCKYDRAW_INFO2.dwReachMask = msg.stPkgData.get_stLuckyDrawDataNtf().stCoupons.dwReachMask;
				cSDT_LUCKYDRAW_INFO2.dwLuckyPoint = msg.stPkgData.get_stLuckyDrawDataNtf().stCoupons.dwLuckyPoint;
			}
