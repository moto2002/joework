...
namespace Assets.Scripts.GameSystem
{
	public abstract class VoiceInteraction
	{
		...
		{
			Vector3 a = (Vector3)InSource.get_handle().location;
			Vector3 b = (Vector3)InRelevance.get_handle().location;
		...
		protected virtual bool TryTrigger(ref PoolObjHandle<ActorRoot> InSource, ref PoolObjHandle<ActorRoot> InRelevance, ref PoolObjHandle<ActorRoot> InSoundSource)
		{
			...
			{
				bool flag = false;
				if (this.InteractionCfg.bReceiveType == 1 && InSource.get_handle().IsEnemyCamp(hostPlayer.Captain.get_handle()))
				{
					flag = true;
				}
				...
				else if (this.InteractionCfg.bReceiveType == 0 && InSource.get_handle().IsSelfCamp(hostPlayer.Captain.get_handle()))
				{
					flag = true;
				}
				if (!flag)
		...
		public void DoTrigger()
		{
			...
			if (this.InteractionCfg != null)
			{
				Singleton<CSoundManager>.get_instance().PlayBattleSound(this.InteractionCfg.szVoiceEvent, this.SoundSourceActor, null);
			}
		}
		...
		{
			return InTestActor && (!this.isSpecialTrigger || this.IsSpecialTriggerID(InTestActor.get_handle().TheActorMeta.ConfigId));
		}
	}
}
