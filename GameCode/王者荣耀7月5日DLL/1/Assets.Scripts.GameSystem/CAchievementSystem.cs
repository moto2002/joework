...
namespace Assets.Scripts.GameSystem
{
	...
	public class CAchievementSystem : Singleton<CAchievementSystem>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler<SCPKG_GET_RANKING_ACNT_INFO_RSP>(EventID.ACHIEVE_GET_RANKING_ACCOUNT_INFO, new Action<SCPKG_GET_RANKING_ACNT_INFO_RSP>(this.HandleAchieveGetRankingAccountInfo));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.LOBBY_STATE_ENTER, new Action(this.HandleLobbyStateEnter));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.ACHIEVE_TROPHY_REWARD_INFO_STATE_CHANGE, new Action(this.OnTrophyStateChange));
			Singleton<EventRouter>.get_instance().AddEventHandler<RankingSystem.RankingType>("Ranking_List_Change", new Action<RankingSystem.RankingType>(this.RankingListChange));
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.Mall_Get_Product_OK, new Action(this.OnMallGetProductOk));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.ACHIEVE_RECEIVE_PVP_BAN_MSG, new Action(this.ProcessPvpBanMsg));
		}
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler<SCPKG_GET_RANKING_ACNT_INFO_RSP>(EventID.ACHIEVE_GET_RANKING_ACCOUNT_INFO, new Action<SCPKG_GET_RANKING_ACNT_INFO_RSP>(this.HandleAchieveGetRankingAccountInfo));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.LOBBY_STATE_ENTER, new Action(this.HandleLobbyStateEnter));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.ACHIEVE_TROPHY_REWARD_INFO_STATE_CHANGE, new Action(this.OnTrophyStateChange));
			Singleton<EventRouter>.get_instance().RemoveEventHandler<RankingSystem.RankingType>("Ranking_List_Change", new Action<RankingSystem.RankingType>(this.RankingListChange));
			Singleton<CUIEventManager>.GetInstance().AddUIEventListener(enUIEventID.NOBE_LevelUp_Form_Close, new CUIEventManager.OnUIEventHandler(this.OnNobeLevelUpFormClose));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.Mall_Get_Product_OK, new Action(this.OnMallGetProductOk));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.ACHIEVE_RECEIVE_PVP_BAN_MSG, new Action(this.ProcessPvpBanMsg));
		...
		public void ShowTrophyDetail(CAchieveInfo2 achieveInfo, uint achievementID)
		{
			...
			{
				cAchieveItem = achieveInfo.m_AchiveItemDic.get_Item(achievementID);
			}
		...
		private void OnBrowseAllRewards(CUIEvent uiEvent)
		{
			...
			if (component != null)
			{
				...
				component.SetElementAmount(this.m_TrophyRewardInfoWithRewardList.get_Count());
			}
		}
		...
		private void OnAchievementOpenOverviewForm(CUIEvent uiEvent)
		{
			...
			Singleton<CUINewFlagSystem>.get_instance().HideNewFlagForAchievementEntry();
			Singleton<CBattleGuideManager>.get_instance().OpenBannerDlgByBannerGuideId(18u, null, false);
		}
		...
		public void ProcessPvpBanMsg()
		{
			...
			if (CAchievementSystem.CachedPunishMsg != null && CAchievementSystem.CachedPunishMsg.get_Count() > 0)
			{
				for (int i = CAchievementSystem.CachedPunishMsg.get_Count() - 1; i >= 0; i--)
				{
					string strContent = string.Format(Singleton<CTextManager>.GetInstance().GetText("Achievement_10"), Utility.GetUtcToLocalTimeStringFormat((ulong)CAchievementSystem.CachedPunishMsg.get_Item(i).dwPunishEndTime, "yyyy年M月d日 H:m:s"));
		...
		{
			if (rsp.stAcntRankingDetail.get_stOfSucc().bNumberType != 8)
			{
				return;
			}
			...
			if (masterRoleInfo == null)
			{
				...
				CAchieveInfo2.AddWorldRank(0, 0uL, rsp.stAcntRankingDetail.get_stOfSucc().dwRankNo);
			}
			else
			{
				CAchieveInfo2.AddWorldRank(masterRoleInfo.logicWorldID, masterRoleInfo.playerUllUID, rsp.stAcntRankingDetail.get_stOfSucc().dwRankNo);
			}
		}
		...
		private void RankingListChange(RankingSystem.RankingType rankType)
		{
			if (rankType == RankingSystem.RankingType.Achievement)
			{
				...
				if (rankList != null)
				{
					...
					{
						CAchieveInfo2.AddWorldRank(rankList.astItemDetail[num].stExtraInfo.stDetailInfo.get_stAchievement().iLogicWorldId, rankList.astItemDetail[num].stExtraInfo.stDetailInfo.get_stAchievement().ullUid, (uint)(rankList.iStart + num));
						num++;
					}
				}
			}
		...
		public void SendGetTrophyRewardReq(uint id)
		{
			...
			cSPkg.stPkgData.get_stGetTrophyLvlRewardReq().dwTrophyLvl = id;
		...
		public void SendReqGetRankingAcountInfo()
		{
			...
			cSPkg.stPkgData.get_stGetRankingAcntInfoReq().bNumberType = 8;
		...
		public static void SendGetAchieveRewardReq(uint achieveId)
		{
			...
			cSPkg.stPkgData.set_stGetAchievementRewardReq(new CSPKG_GET_ACHIEVEMENT_REWARD_REQ());
			cSPkg.stPkgData.get_stGetAchievementRewardReq().dwAchievementID = achieveId;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void OnRecieveAchieveInfo(CSPkg msg)
		{
			...
			SCPKG_ACHIEVEMENT_INFO_NTF stGetAchievememtInfoNtf = msg.stPkgData.get_stGetAchievememtInfoNtf();
			CAchieveInfo2 masterAchieveInfo = CAchieveInfo2.GetMasterAchieveInfo();
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			SCPKG_GET_TROPHYLVL_REWARD_RSP stGetTrophyLvlmentRewardRsp = msg.stPkgData.get_stGetTrophyLvlmentRewardRsp();
			...
			CUseable[] trophyRewards = cTrophyRewardInfo.GetTrophyRewards();
			Singleton<CUIManager>.get_instance().OpenAwardTip(trophyRewards, null, false, enUIEventID.None, false, false, "Form_Award");
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			SCPKG_NTF_TROPHYLVLUP stNtfTrophyLvlUp = msg.stPkgData.get_stNtfTrophyLvlUp();
			CAchieveInfo2 masterAchieveInfo = CAchieveInfo2.GetMasterAchieveInfo();
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			SCPKG_ACHIEVEMENT_STATE_CHG_NTF stAchievementStateChgNtf = msg.stPkgData.get_stAchievementStateChgNtf();
			CAchieveInfo2 masterAchieveInfo = CAchieveInfo2.GetMasterAchieveInfo();
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			SCPKG_ACHIEVEMENT_DONE_DATA_CHG_NTF stAchievementDoneDataChgNtf = msg.stPkgData.get_stAchievementDoneDataChgNtf();
			CAchieveInfo2 masterAchieveInfo = CAchieveInfo2.GetMasterAchieveInfo();
		...
		{
			CAchievementSystem.AddPunishMsg(msg.stPkgData.get_stPvPBanNtf());
		}
	}
}
