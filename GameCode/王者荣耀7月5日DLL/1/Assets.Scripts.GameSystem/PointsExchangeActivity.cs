	...
	public class PointsExchangeActivity : Activity
	{
		...
		public override bool ReadyForDot
		{
			get
			{
				...
				{
					ActivityPhase activityPhase = base.PhaseList.get_Item(i);
		...
		public override void UpdateInfo(ref COMDT_WEAL_UNION actvInfo)
		{
			...
			byte bWealCnt = actvInfo.get_stPtExchange().bWealCnt;
			COMDT_WEAL_EXCHANGE_OBJ[] astWealList = actvInfo.get_stPtExchange().astWealList;
		...
		{
			uint num = 0u;
			return (!this._exchangeCount.TryGetValue(index, ref num)) ? 0u : num;
		}
