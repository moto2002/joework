...
namespace Assets.Scripts.GameSystem
{
	public class CGuildModel : Singleton<CGuildModel>
	{
		...
		public uint PlayerDailyActive
		{
			get
			{
				for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.uulUid == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().playerUllUID)
					{
						return this.CurrentGuildInfo.listMemInfo.get_Item(i).CurrActive;
					}
				}
				return 0u;
			}
			set
			{
				for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.uulUid == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().playerUllUID)
					{
						this.CurrentGuildInfo.listMemInfo.get_Item(i).CurrActive = value;
					}
				}
			}
		}
		...
		public uint PlayerWeekActive
		{
			get
			{
				for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.uulUid == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().playerUllUID)
					{
						return this.CurrentGuildInfo.listMemInfo.get_Item(i).WeekActive;
					}
				}
				return 0u;
			}
			set
			{
				for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.uulUid == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().playerUllUID)
					{
						this.CurrentGuildInfo.listMemInfo.get_Item(i).WeekActive = value;
					}
				}
			}
		}
		...
		{
			Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_baseGuildInfo.guildState = 7;
		...
		public bool IsInGuildStep()
		{
			...
			return Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_baseGuildInfo.guildState != null;
		}
		...
		{
			int count = guildInfoList.get_Count();
			for (int i = 0; i < count; i++)
			{
				this.AddGuildInfo(guildInfoList.get_Item(i));
			}
		}
		...
		{
			return this.m_GuildDic.get_Count();
		}

		public GuildInfo GetGuildInfoByIndex(int idx)
		{
			if (idx >= 0 && idx < this.m_GuildDic.get_Count())
			{
				int num = 0;
				...
				while (enumerator.MoveNext())
				{
					if (num == idx)
					{
						KeyValuePair<ulong, GuildInfo> current = enumerator.get_Current();
		...
		{
			int i = 0;
			int count = prepareGuildInfoList.get_Count();
			while (i < count)
			{
				this.AddPrepareGuildInfo(prepareGuildInfoList.get_Item(i));
				i++;
			}
		}
		...
		{
			return this.m_PrepareGuildDic.get_Count();
		}

		public PrepareGuildInfo GetPrepareGuildInfoByIndex(int idx)
		{
			if (idx >= 0 && idx < this.m_PrepareGuildDic.get_Count())
			{
				int num = 0;
				...
				while (enumerator.MoveNext())
				{
					if (num == idx)
					{
						KeyValuePair<ulong, PrepareGuildInfo> current = enumerator.get_Current();
						return current.get_Value();
					}
					num++;
				}
		...
		public List<stApplicantInfo> GetApplicants()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				arg_32_0.Add(current.get_Value());
			}
			return list;
		}
		...
		{
			return (this.m_ApplicantDic != null) ? this.m_ApplicantDic.get_Count() : 0;
		}
		...
		{
			return (!this.m_recommendTimeInfoDic.ContainsKey(uid)) ? -1 : this.m_recommendTimeInfoDic.get_Item(uid);
		}

		public int GetInviteTimeInfoByUid(ulong uid)
		{
			return (!this.m_inviteTimeInfoDic.ContainsKey(uid)) ? -1 : this.m_inviteTimeInfoDic.get_Item(uid);
		}

		public int GetRecommendInfoCount()
		{
			return (this.m_recommendInfoDic != null) ? this.m_recommendInfoDic.get_Count() : 0;
		}
		...
		{
			for (int i = 0; i < this.CurrentGuildInfo.listBuildingInfo.get_Count(); i++)
			{
				if (this.CurrentGuildInfo.listBuildingInfo.get_Item(i).type == buildingInfo.type)
				{
					this.CurrentGuildInfo.listBuildingInfo.get_Item(i).level = buildingInfo.level;
					return;
				}
			}
		...
		public GuildMemInfo GetGuildMemberInfoByUid(ulong uid)
		{
			for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
			{
				if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.uulUid == uid)
				{
					return this.CurrentGuildInfo.listMemInfo.get_Item(i);
				}
			}
			return null;
		}
		...
		public GuildMemInfo GetGuildMemberInfoByName(string name)
		{
			for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
			{
				if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.sName == name)
				{
					return this.CurrentGuildInfo.listMemInfo.get_Item(i);
				}
			}
		...
		{
			for (int i = 0; i < this.CurrentGuildInfo.listSelfRecommendInfo.get_Count(); i++)
			{
				if (this.CurrentGuildInfo.listSelfRecommendInfo.get_Item(i).uid == uid)
				{
					this.CurrentGuildInfo.listSelfRecommendInfo.get_Item(i).time = time;
					return;
				}
			}
			GuildSelfRecommendInfo guildSelfRecommendInfo = new GuildSelfRecommendInfo();
			guildSelfRecommendInfo.uid = uid;
			guildSelfRecommendInfo.time = time;
			this.CurrentGuildInfo.listSelfRecommendInfo.Add(guildSelfRecommendInfo);
		}
		...
		public void SetGuildMatchMemberInfo(SCPKG_GUILD_MATCH_MEMBER_CHG_NTF ntf)
		{
			...
			{
				for (int j = 0; j < this.CurrentGuildInfo.listMemInfo.get_Count(); j++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(j).stBriefInfo.uulUid == ntf.astChgInfo[i].ullUid)
					{
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.ullTeamLeaderUid = ntf.astChgInfo[i].ullTeamLeaderUid;
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.bContinueWin = ntf.astChgInfo[i].bContinueWin;
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.bIsReady = ntf.astChgInfo[i].bIsReady;
					}
				}
			}
		}
		...
		public void SetGuildMatchMemberInfo(SCPKG_CHG_GUILD_MATCH_LEADER_NTF ntf)
		{
			for (int i = 0; i < this.CurrentGuildInfo.listMemInfo.get_Count(); i++)
			{
				if (this.CurrentGuildInfo.listMemInfo.get_Item(i).stBriefInfo.uulUid == ntf.ullUid)
				{
					this.CurrentGuildInfo.listMemInfo.get_Item(i).GuildMatchInfo.bIsLeader = ntf.bIsLeader;
					this.CurrentGuildInfo.listMemInfo.get_Item(i).GuildMatchInfo.ullTeamLeaderUid = ntf.ullTeamLeaderUid;
					this.CurrentGuildInfo.listMemInfo.get_Item(i).GuildMatchInfo.bContinueWin = ntf.bContinueWin;
					return;
				}
			}
		}
		...
		public void SetGuildMatchMemberReadyState(SCPKG_SET_GUILD_MATCH_READY_NTF ntf)
		{
			for (int i = 0; i < (int)ntf.bCnt; i++)
			{
				for (int j = 0; j < this.CurrentGuildInfo.listMemInfo.get_Count(); j++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(j).stBriefInfo.uulUid == ntf.astInfo[i].ullUid)
					{
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.bIsReady = ntf.astInfo[i].bIsReady;
					}
				}
			}
		}
		...
		public void SetGuildMatchScore(SCPKG_GUILD_MATCH_SCORE_CHG_NTF ntf)
		{
			...
			for (int i = 0; i < ntf.astChgInfo.Length; i++)
			{
				for (int j = 0; j < this.CurrentGuildInfo.listMemInfo.get_Count(); j++)
				{
					if (this.CurrentGuildInfo.listMemInfo.get_Item(j).stBriefInfo.uulUid == ntf.astChgInfo[i].ullUid)
					{
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.dwScore = ntf.astChgInfo[i].dwScore;
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.dwWeekScore = ntf.astChgInfo[i].dwWeekScore;
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.bContinueWin = ntf.astChgInfo[i].bContinueWin;
						this.CurrentGuildInfo.listMemInfo.get_Item(j).GuildMatchInfo.bWeekMatchCnt = ntf.astChgInfo[i].bWeekMatchCnt;
					}
				}
			}
		}
