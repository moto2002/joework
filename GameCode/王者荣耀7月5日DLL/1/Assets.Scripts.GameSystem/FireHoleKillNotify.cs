...
namespace Assets.Scripts.GameSystem
{
	public class FireHoleKillNotify
	{
		...
		public FireHoleKillNotify()
		{
			...
			if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("FireHole_First_Notify_Num"), ref this.first_count))
			{
				DebugHelper.Assert(false, "--- 2爷 你配的 FireHole_First_Notify_Num 好像不是整数哦， check out");
			}
			if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("FireHole_Second_Notify_Num"), ref this.second_count))
			{
				DebugHelper.Assert(false, "--- 2爷 你配的 FireHole_Second_Notify_Num 好像不是整数哦， check out");
			}
			if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("FireHole_Third_Notify_Num"), ref this.third_count))
			{
		...
		{
			COM_PLAYERCAMP camp = (!bSelfCamp) ? this.getEnemyCamp() : Singleton<GamePlayerCenter>.get_instance().hostPlayerCamp;
		...
		{
			if (Singleton<GamePlayerCenter>.get_instance().hostPlayerCamp == 1)
			{
				return 2;
			}
			if (Singleton<GamePlayerCenter>.get_instance().hostPlayerCamp == 2)
			{
				return 1;
			}
