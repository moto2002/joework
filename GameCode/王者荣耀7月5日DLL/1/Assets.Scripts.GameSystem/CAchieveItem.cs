...
namespace Assets.Scripts.GameSystem
{
	public class CAchieveItem : IComparable
	{
		...
		public void SetDoneData(ref int[] achieveDoneArr)
		{
			...
			if (this.m_doneType >= 0 && this.m_doneType < achieveDoneArr.Length)
			{
				this.m_doneCnt = achieveDoneArr[this.m_doneType];
			}
		}
		...
		public uint GetAchievementAwardCnt(RES_REWARDS_TYPE rewardType)
		{
			...
			{
				if (this.m_cfgInfo.astReward[i].bRewardType == rewardType)
				{
		...
		{
			return this.m_state == 2 || 1 == this.m_state;
		}
