...
namespace Assets.Scripts.GameSystem
{
	public class CUIParticleSystem : Singleton<CUIParticleSystem>
	{
		...
		{
			for (int i = 0; i < this.m_particleList.get_Count(); i++)
			{
				if (this.m_particleList.get_Item(i).totlalTime >= 0f)
				{
					this.m_particleList.get_Item(i).currentTime += 0.1f;
					if (this.m_particleList.get_Item(i).currentTime >= this.m_particleList.get_Item(i).totlalTime)
					{
						if (this.m_particleContainerScript != null)
						{
							this.m_particleContainerScript.RemoveGameObject(this.m_particleList.get_Item(i).parObj);
						}
		...
		public void ClearAll(bool isClearCycle = false)
		{
			...
			{
				bool flag = true;
				if (this.m_particleList.get_Item(i).totlalTime < 0f && !isClearCycle)
				{
					flag = false;
				}
				if (flag)
