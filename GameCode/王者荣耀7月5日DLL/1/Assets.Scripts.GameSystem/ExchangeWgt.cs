...
namespace Assets.Scripts.GameSystem
{
	public class ExchangeWgt : ActivityWidget
	{
		...
		public ExchangeWgt(GameObject node, ActivityView view) : base(node, view)
		{
			...
			for (int i = 0; i < phaseList.get_Count(); i++)
			{
				...
				if (gameObject != null)
				{
					...
					this._elementList.Add(new ExchangeWgt.ExchangeElement(phaseList.get_Item(phaseList.get_Count() - i - 1) as ExchangePhase, gameObject, this, i));
				}
			}
		...
		private void OnExchangeHeroSkinConfirm(CUIEvent uiEvent)
		{
			...
			{
				ResDT_Item_Info stResItemInfo = this._elementList.get_Item(commonUInt32Param).phase.Config.stResItemInfo;
