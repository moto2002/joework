...
namespace Assets.Scripts.GameSystem
{
	internal class PvpAchievementForm
	{
		...
		private RES_SHOW_ACHIEVEMENT_TYPE m_curAchievemnt = 8;
		...
		private int GetKVDataCount(RES_STATISTIC_SETTLE_DATA_TYPE type)
		{
			...
			while ((long)num < (long)((ulong)masterRoleInfo.pvpDetail.stKVDetail.dwNum))
			{
				...
				if (cOMDT_STATISTIC_KEY_VALUE_INFO.dwKey == type)
				{
		...
		{
			int curAchievemnt = this.m_curAchievemnt;
		...
		private void SwitchAchievement(RES_SHOW_ACHIEVEMENT_TYPE achievement)
		{
			...
			Utility.FindChild(widget.gameObject, "Title/Text").GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("Share_PvpAchievement_Title" + achievement);
			...
			{
				CUIUtility.SetImageSprite(form.GetWidget(1).GetComponent<Image>(), PvpAchievementForm.s_imageSharePVPBadge + achievement + ".prefab", form, true, false, false, false);
			}
			...
			if (gameObject != null)
			{
				...
				if (achievementCount != 0)
				{
					...
					Utility.FindChild(gameObject, "Text").GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("Share_PvpAchievement_Desc_" + achievement);
				}
		...
		public void ShowDefeat()
		{
			...
			if (databinTable != null)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					if (resDefeatBarrageText.wAchievementType == this.m_curAchievemnt)
					{
		...
		private void OnSharePVPDefeatAddBarrage(CUIEvent uiEvent)
		{
			...
			component.SetElementAmount(this.barrageList.get_Count());
			component.MoveElementInScrollArea(0, true);
			for (int i = 0; i < this.barrageList.get_Count(); i++)
			{
		...
		{
			if (go == null || this.barrageList.get_Count() <= index)
			{
				return;
			}
			Utility.FindChild(go, "Text").GetComponent<Text>().text = this.barrageList.get_Item(index);
		}
		...
		private void OnSharePVPDefeatSelectBarrage(CUIEvent uiEvent)
		{
			...
			if (gameObject2 != null && selectedIndex < this.barrageList.get_Count())
			{
				gameObject2.GetComponent<Text>().text = this.barrageList.get_Item(selectedIndex);
			}
		}
