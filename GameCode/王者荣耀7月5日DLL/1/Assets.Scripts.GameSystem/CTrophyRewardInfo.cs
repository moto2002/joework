	...
	public class CTrophyRewardInfo
	{
		...
		public CUseable[] GetTrophyRewards()
		{
			...
			for (int i = 0; i < this.Cfg.astReqReward.Length; i++)
			{
				if (this.Cfg.astReqReward[i].dwRewardNum > 0u)
				{
					listView.Add(CUseableManager.CreateUsableByServerType(this.Cfg.astReqReward[i].bRewardType, (int)this.Cfg.astReqReward[i].dwRewardNum, this.Cfg.astReqReward[i].dwRewardID));
				}
			}
			CUseable[] array = new CUseable[listView.get_Count()];
			for (int j = 0; j < listView.get_Count(); j++)
			{
				array[j] = listView.get_Item(j);
			}
			return array;
		}
	}
