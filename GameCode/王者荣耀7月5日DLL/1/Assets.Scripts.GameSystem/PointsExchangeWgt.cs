	...
	public class PointsExchangeWgt : ActivityWidget
	{
		...
		public PointsExchangeWgt(GameObject node, ActivityView view) : base(node, view)
		{
			...
			for (int i = 0; i < phaseList.get_Count(); i++)
			{
				...
				if (gameObject != null)
				{
					...
					this._elementList.Add(new PointsExchangeWgt.PointsExchangeElement(phaseList.get_Item(i) as PointsExchangePhase, gameObject, this, i));
				}
			}
			node.GetComponent<RectTransform>().sizeDelta = new Vector2(node.GetComponent<RectTransform>().sizeDelta.x, height * (float)phaseList.get_Count() + (float)(phaseList.get_Count() - 1) * 5f);
		...
		private void OnStateChange(Activity acty)
		{
			...
			if (view != null)
			{
				...
				for (int i = 0; i < widgetList.get_Count(); i++)
				{
					IntrodWidget introdWidget = widgetList.get_Item(i) as IntrodWidget;
		...
		public override void Clear()
		{
			...
			{
				for (int i = 0; i < this._elementList.get_Count(); i++)
				{
					if (this._elementList.get_Item(i).uiItem)
					{
						Object.Destroy(this._elementList.get_Item(i).uiItem);
					}
				}
		...
		public override void Validate()
		{
			...
			for (int i = 0; i < this._elementList.get_Count(); i++)
			{
				this._elementList.get_Item(i).Validate();
			}
		}
		...
		private void OnExchangeHeroSkinConfirm(CUIEvent uiEvent)
		{
			...
			if (commonUInt32Param >= 0 && commonUInt32Param < this._elementList.get_Count())
			{
				uint dwResItemID = this._elementList.get_Item(commonUInt32Param).phase.Config.dwResItemID;
				CUseable cUseable = CUseableManager.CreateUseable(this._elementList.get_Item(commonUInt32Param).phase.Config.wResItemType, this._elementList.get_Item(commonUInt32Param).phase.Config.dwResItemID, (int)this._elementList.get_Item(commonUInt32Param).phase.Config.wResItemCnt);
				int maxExchangeCount = this._elementList.get_Item(commonUInt32Param).phase.GetMaxExchangeCount();
				if (maxExchangeCount > 1)
				{
					stUIEventParams par = default(stUIEventParams);
					par.commonUInt16Param1 = (ushort)commonUInt32Param;
					Singleton<CUIManager>.GetInstance().OpenExchangeCountSelectForm(cUseable, maxExchangeCount, enUIEventID.Activity_PtExchangeCountReady, par, this._elementList.get_Item(commonUInt32Param).phase.Config.dwPointCnt, Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().JiFen);
				}
				else
				{
					...
					this._elementList.get_Item(commonUInt32Param).phase.SetExchangeCountOnce(1);
		...
		private void OnClickExchange(CUIEvent uiEvent)
		{
			...
			if (commonUInt32Param >= 0 && commonUInt32Param < this._elementList.get_Count())
			{
				...
				{
					string text2 = (cUseable != null) ? cUseable.m_name : string.Empty;
					stUIEventParams par2 = default(stUIEventParams);
					par2.commonUInt16Param1 = (ushort)commonUInt32Param;
					this._elementList.get_Item(commonUInt32Param).phase.SetExchangeCountOnce(1);
		...
		private void OnClickExchangeConfirm(CUIEvent uiEvent)
		{
			...
			{
				this._elementList.get_Item(commonUInt16Param).phase.DrawReward();
			}
		}
		...
		private void OnClickExchangeCountSelectReady(CUIEvent uiEvent)
		{
			if (this._elementList != null)
			{
				...
				if (commonUInt16Param >= 0 && commonUInt16Param < this._elementList.get_Count())
				{
					uint dwResItemID = this._elementList.get_Item(commonUInt16Param).phase.Config.dwResItemID;
					CUseable cUseable = CUseableManager.CreateUseable(this._elementList.get_Item(commonUInt16Param).phase.Config.wResItemType, this._elementList.get_Item(commonUInt16Param).phase.Config.dwResItemID, 1);
