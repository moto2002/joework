...
namespace Assets.Scripts.GameSystem
{
	...
	public class CSurrenderSystem : Singleton<CSurrenderSystem>
	{
		...
		{
			if (this.IsWarmBattle() && !Singleton<LobbyLogic>.get_instance().inMultiGame)
			{
				...
				if (this.m_maxCnt == 1)
				{
					...
					BattleLogic.ForceKillCrystal(playerCamp);
				}
				else
				{
					this.m_haveRights = false;
					this.m_result = this.ConstrcutData(this.m_maxCnt);
					this.m_timerSeq = Singleton<CTimerManager>.get_instance().AddTimer(this.m_random.Next(500, 3000), 1, new CTimer.OnTimeUpHandler(this.OnTimerWarmBattle));
		...
		{
			return (this.m_lastSurrenderTime != 0u || (int)Singleton<FrameSynchr>.get_instance().LogicFrameTick < this.GetSurrenderStartTime() * 1000) && (this.m_lastSurrenderTime <= 0u || (long)CRoleInfo.GetCurrentUTCTime() < (long)((ulong)this.m_lastSurrenderTime + (ulong)((long)this.GetSurrenderCDTime())));
		}
		...
		public bool InSurrenderCD(out uint time)
		{
			...
			{
				if ((int)Singleton<FrameSynchr>.get_instance().LogicFrameTick >= this.GetSurrenderStartTime() * 1000)
				{
					return false;
				}
				time = (uint)(this.GetSurrenderStartTime() - (int)((uint)Singleton<FrameSynchr>.get_instance().LogicFrameTick / 1000u));
				return true;
			}
			else
			{
		...
		public void SendMsgSurrender(byte bSurrender)
		{
			...
			cSPkg.stPkgData.get_stSurrenderReq().bSurrender = bSurrender;
		...
		{
			Singleton<CSurrenderSystem>.get_instance().OnSurrenderRsp(msg.stPkgData.get_stSurrenderRsp());
		}

		[MessageHandler(4902)]
		public static void OnSurrenderNtf(CSPkg msg)
		{
			SCPKG_SURRENDER_NTF stSurrenderNtf = msg.stPkgData.get_stSurrenderNtf();
			Singleton<CSurrenderSystem>.get_instance().OnSurrenderNtf(stSurrenderNtf);
		}
