	...
	public class CLoginSystem : Singleton<CLoginSystem>
	{
		...
		private void LoginCancel()
		{
			...
			if (curPlatform != 1)
			{
				...
				if (curPlatform != 257)
				{
					Debug.LogWarning("user cancel on platform: " + Singleton<ApolloHelper>.get_instance().CurPlatform);
					return;
				}
			}
		...
		private void QuickLoginDone(ApolloWakeupInfo wakeupInfo)
		{
			...
			else if (accountInfo == null || accountInfo.get_OpenId() == wakeupInfo.OpenId)
			{
			...
			if (wakeupInfo.get_ExtensionInfo() != null)
			{
				int i = 0;
				int count = wakeupInfo.get_ExtensionInfo().get_Count();
				while (i < count)
				{
					ApolloKVPair apolloKVPair = wakeupInfo.get_ExtensionInfo().get_Item(i);
				...
				int j = 0;
				int count2 = wakeupInfo.get_ExtensionInfo().get_Count();
				while (j < count2)
				{
					ApolloKVPair apolloKVPair2 = wakeupInfo.get_ExtensionInfo().get_Item(j);
					if (apolloKVPair2 != null)
					{
						if (apolloKVPair2.Key == ApolloHelper.QQ_LAUNCH_FROM)
						{
							if (apolloKVPair2.Value == ApolloHelper.QQ_LAUNCH_FROM_GAMECENTER && !Singleton<LobbyLogic>.get_instance().isLogin)
							{
								Singleton<ApolloHelper>.GetInstance().IsQQGameCenter = true;
							}
						}
						else if (apolloKVPair2.Key == ApolloHelper.QQ_SHARE_GAMEDATA && (accountInfo == null || accountInfo.get_OpenId() == wakeupInfo.OpenId))
						{
			...
			if (accountInfo != null && !string.IsNullOrEmpty(accountInfo.get_OpenId()))
			{
				if (wakeupInfo.OpenId != accountInfo.get_OpenId())
				{
			...
			else if (wakeupInfo.state == 1 && Singleton<GameStateCtrl>.GetInstance().GetCurrentState() is LoginState)
			{
		...
		private void LoginSuccess(ApolloAccountInfo info)
		{
			...
			this.RegisterXGPush(info.get_OpenId());
		...
		private void OnLogin_GuestLogin(CUIEvent uiEvent)
		{
			...
			Singleton<ApolloHelper>.GetInstance().Login(5, 0uL, null);
		}

		private void OnLogin_QQLogin(CUIEvent uiEvent)
		{
			this.m_fLoginClickTime = Time.time;
			Singleton<ApolloHelper>.GetInstance().Login(2, 0uL, null);
		}
		...
		private void OnLogin_WtLogin(CUIEvent uiEvent)
		{
			...
			if (component3 != null && component3.text.get_Length() > 0)
			{
			...
			if (component != null && component2 != null && component.text != null && component.text.get_Length() > 0 && component2.text.get_Length() > 0)
			{
				PlayerPrefs.SetString("SGamePlayerUinCache", component.text);
				PlayerPrefs.SetString("SGamePlayerPwdCache", component2.text);
				Singleton<ApolloHelper>.GetInstance().Login(3, Convert.ToUInt64(component.text), component2.text);
			}
		}
		...
		private void OnLogin_WxLogin(CUIEvent uiEvent)
		{
			...
			Singleton<ApolloHelper>.GetInstance().Login(1, 0uL, null);
		}
		...
		private void OnLogin_QRWechatLogin(CUIEvent uiEvent)
		{
			...
			Singleton<ApolloHelper>.GetInstance().Login(257, 0uL, null);
		}
		...
		private void OnLogin_None(CUIEvent uiEvent)
		{
			...
			Singleton<ApolloHelper>.GetInstance().Login(0, 0uL, null);
		}
		...
		private void ConnectLimit(bool showTips = true, ApolloResult result = 0)
		{
			...
			int num2 = (int)now.Subtract(dateTime.AddSeconds((double)this.m_ReConnectFailedTime)).get_TotalSeconds();
			num -= (float)num2;
			if (num <= 0f)
			{
				flag2 = false;
			...
			if (flag2)
			{
				if (showTips)
				{
					text = string.Format("{0}，{1}", text, string.Format("请等待{0}再尝试开始游戏", (num <= 60f) ? string.Format("{0}秒", num) : string.Format("{0}分钟", (int)(num / 60f))));
				}
		...
		private void OnConfirmTransferData(CUIEvent uiEvent)
		{
			...
			cSPkg.stPkgData.get_stRspAcntTransVisitorSvrData().bAgree = 1;
		...
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(1027u);
			cSPkg.stPkgData.get_stRspAcntTransVisitorSvrData().bAgree = 0;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
		...
		private void OnChangeAccountYes(CUIEvent uiEvent)
		{
			...
			if (currentState is LoginState)
			{
				...
				if (Singleton<ApolloHelper>.GetInstance().m_LastTriedPlatform == 1 || Singleton<ApolloHelper>.GetInstance().m_LastTriedPlatform == 257)
				{
					Singleton<LobbyLogic>.GetInstance().GotoAccLoginPage();
					...
					Singleton<ApolloHelper>.GetInstance().m_LastTriedPlatform = 0;
					Singleton<ApolloHelper>.GetInstance().Login(lastTriedPlatform, 0uL, null);
				}
			}
			else
			{
		...
		public void SetTdirZoneGroupList(Transform containerTransform)
		{
			...
			for (int i = 0; i < svrUrlList.get_Count(); i++)
			{
				...
				component2.text = svrUrlList.get_Item(i).name;
			}
		...
		private void SetNickName(ApolloRelation aRelation)
		{
			...
			if (transform != null)
			{
				if (transform2 != null && aRelation.get_Result() == null)
				{
					transform.gameObject.CustomSetActive(true);
					Text component = transform2.GetComponent<Text>();
					using (List<ApolloPerson>.Enumerator enumerator = aRelation.get_Persons().GetEnumerator())
					{
						if (enumerator.MoveNext())
						{
							ApolloPerson current = enumerator.get_Current();
							component.text = current.get_NickName();
							this.m_nickName = current.get_NickName();
						}
					}
				}
				else
