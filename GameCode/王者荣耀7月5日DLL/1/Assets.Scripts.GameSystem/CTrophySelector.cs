...
namespace Assets.Scripts.GameSystem
{
	...
	public class CTrophySelector : Singleton<CTrophySelector>
	{
		...
		{
			base.UnInit();
			this.m_CurTrophies.RemoveRange(0, this.m_CurTrophies.get_Count());
		...
		private void OnChangeAchievement(CUIEvent uiEvent)
		{
			...
			for (int i = this.m_CurTrophies.get_Count() - 1; i >= 0; i--)
			{
				if (this.m_CurTrophies.get_Item(i) != null && Array.IndexOf<CAchieveItem2>(this.SelectedTrophies, this.m_CurTrophies.get_Item(i)) < 0)
				{
					listView.Add(this.m_CurTrophies.get_Item(i));
				}
			}
		...
		private void OnTrophyEnable(CUIEvent uiEvent)
		{
			...
			if (srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_CurTrophies.get_Count())
			{
				return;
			}
			...
			CAchieveItem2 cAchieveItem = this.m_CurTrophies.get_Item(srcWidgetIndexInBelongedList);
		...
		private void OnTrophySelectChange(CUIEvent uiEvent)
		{
			...
			if (uiEvent.m_srcFormScript == null || selectedIndex < 0 || selectedIndex >= this.m_CurTrophies.get_Count())
			{
				return;
			}
			CAchieveItem2 cAchieveItem = this.m_CurTrophies.get_Item(selectedIndex);
		...
		private void RefreshAchievementSelectForm(CUIFormScript form)
		{
			...
			component.SetElementAmount(this.m_CurTrophies.get_Count());
		}

		private void SendChgAchieveReq(uint id, byte idx)
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(4413u);
			cSPkg.stPkgData.get_stReqAchieveShow().dwAchieveID = id;
			cSPkg.stPkgData.get_stReqAchieveShow().bIndex = idx;
		...
		public static void ReceiveChgTrophyRsp(CSPkg msg)
		{
			...
			if (msg.stPkgData.get_stRspAchieveShow().iResult < 0)
			{
			...
			else
			{
				...
				Singleton<CUIManager>.GetInstance().CloseForm("UGUI/Form/System/Achieve/Form_Trophy_Select.prefab");
				Singleton<EventRouter>.GetInstance().BroadCastEvent<byte, CAchieveItem2>(EventID.ACHIEVE_SERY_SELECT_DONE, msg.stPkgData.get_stRspAchieveShow().bIndex, instance.NewTrophy);
			}
		}
	}
}
