...
namespace Assets.Scripts.GameSystem
{
	public class FightForm : IBattleForm
	{
		...
		public bool OpenForm()
		{
			...
			this.m_Vocetimer = Singleton<CTimerManager>.get_instance().AddTimer(this.m_VoiceTipsShowTime, -1, new CTimer.OnTimeUpHandler(this.OnVoiceTimeEnd));
			...
			this.m_VoiceMictime = Singleton<CTimerManager>.get_instance().AddTimer(this.m_VoiceTipsShowTime, -1, new CTimer.OnTimeUpHandler(this.OnVoiceMicTimeEnd));
			...
			GameObject obj = Utility.FindChild(this._formScript.gameObject, "PanelBtn/ToggleAutoBtn");
			obj.CustomSetActive(!curLvelContext.IsMobaMode());
			if (Singleton<BattleLogic>.GetInstance().GetCurLvelContext().m_pveLevelType == 4 || !Singleton<BattleLogic>.GetInstance().GetCurLvelContext().canAutoAI)
			{
			...
			GameObject obj2 = Utility.FindChild(this._formScript.gameObject, "PanelBtn/btnViewBattleInfo");
			obj2.CustomSetActive(curLvelContext.IsMobaMode());
			GameObject obj3 = Utility.FindChild(this._formScript.gameObject, "panelTopRight/SignalPanel");
			...
			Singleton<InBattleMsgMgr>.get_instance().InitView(Utility.FindChild(this._formScript.gameObject, "panelTopRight/SignalPanel/Button_Chat"), this._formScript);
			...
			if (curLvelContext.m_pveLevelType == 4)
			{
				if (this.m_OpenSpeakerObj)
				{
					this.m_OpenSpeakerObj.gameObject.CustomSetActive(false);
			...
			if (curLvelContext.m_pveLevelType == 5)
			{
			...
			GameObject obj4 = Utility.FindChild(this._formScript.gameObject, "ScoreBoard");
			GameObject obj5 = Utility.FindChild(this._formScript.gameObject, "ScoreBoardPvE");
			GameObject gameObject = Utility.FindChild(this._formScript.gameObject, "MapPanel/DragonInfo");
			if (curLvelContext.IsMobaMode())
			{
				this.scoreBoard = new ScoreBoard();
				this.scoreBoard.Init(obj4);
			...
			if (Singleton<BattleLogic>.get_instance().m_dragonSpawn != null)
			{
				...
				else
				{
					...
					this.m_dragonView.Init(gameObject, Singleton<BattleLogic>.get_instance().m_dragonSpawn);
				}
			}
			else
			{
			...
			Singleton<InBattleMsgMgr>.get_instance().RegInBattleEvent();
			if (CSysDynamicBlock.bUnfinishBlock)
			{
				GameObject obj6 = Utility.FindChild(this._formScript.gameObject, "panelTopRight/SignalPanel/Button_Chat");
		...
		private void InitWifiInfo()
		{
			...
			if (GameDataMgr.svr2CltCfgDict.TryGetValue(30u, ref resGlobalInfo))
			{
		...
		{
			if (captain && captain.get_handle().TheStaticData.TheBaseAttribute.DeadControl)
			{
		...
		public void BattleStart()
		{
			...
			this.ResetSkillButtonManager(Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().Captain, false, SkillSlotType.SLOT_SKILL_COUNT);
			if (this.m_skillButtonManager != null)
			{
				this.m_skillButtonManager.InitializeCampHeroInfo(this._formScript);
			}
			SLevelContext curLvelContext2 = Singleton<BattleLogic>.get_instance().GetCurLvelContext();
			...
			{
				bool flag = false;
				if (Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(22) && curLvelContext2.IsMobaModeWithOutGuide())
				{
		...
		public void Update()
		{
			if (this.m_isInBattle)
			{
				...
				if (Singleton<InBattleMsgMgr>.get_instance() != null)
				{
					Singleton<InBattleMsgMgr>.get_instance().Update();
				}
				if (Singleton<CBattleSystem>.get_instance().TheMinimapSys != null)
				{
					Singleton<CBattleSystem>.get_instance().TheMinimapSys.Update();
				}
		...
		public void ShowVoiceTips()
		{
			if (this.m_OpenSpeakerTipObj)
			{
				if (!MonoSingleton<VoiceSys>.GetInstance().IsUseVoiceSysSetting)
				{
					...
					this.m_VocetimerFirst = Singleton<CTimerManager>.get_instance().AddTimer(10000, 1, new CTimer.OnTimeUpHandler(this.OnVoiceTimeEndFirst));
				}
				else
				{
		...
		private void OnFormClosed(CUIEvent uiEvent)
		{
			...
			Singleton<InBattleMsgMgr>.get_instance().Clear();
			this.m_isInBattle = false;
			this.m_bOpenSpeak = false;
			this.m_bOpenMic = false;
			MonoSingleton<VoiceSys>.GetInstance().LeaveRoom();
			Singleton<CTimerManager>.get_instance().RemoveTimer(this.m_Vocetimer);
			Singleton<CTimerManager>.get_instance().RemoveTimer(this.m_VoiceMictime);
			Singleton<CTimerManager>.get_instance().RemoveTimer(this.m_VocetimerFirst);
			this.m_SkillSlotList.Clear();
			if (this.scoreBoard != null)
			{
		...
		{
			if (actor.get_handle() == null || this.m_skillButtonManager == null)
			{
				return;
			}
		...
		public void ResetHostPlayerSkillIndicatorSensitivity()
		{
			...
			if (!captain || captain.get_handle().SkillControl == null)
			{
				return;
			}
			float moveSpeed = 0f;
			...
			for (int i = 0; i < captain.get_handle().SkillControl.SkillSlotArray.Length; i++)
			{
				SkillSlot skillSlot = captain.get_handle().SkillControl.SkillSlotArray[i];
		...
		{
			MonoSingleton<CameraSystem>.get_instance().ToggleFreeCamera();
		}

		public void OnToggleAutoAI(CUIEvent uiEvent)
		{
			Player hostPlayer = Singleton<GamePlayerCenter>.get_instance().GetHostPlayer();
			if (hostPlayer != null && hostPlayer.Captain && hostPlayer.Captain.get_handle().ActorControl != null)
			{
				FrameCommand<SwitchActorAutoAICommand> frameCommand = FrameCommandFactory.CreateFrameCommand<SwitchActorAutoAICommand>();
				frameCommand.cmdData.IsAutoAI = ((!hostPlayer.Captain.get_handle().ActorControl.m_isAutoAI) ? 1 : 0);
				frameCommand.Send();
				Transform transform = uiEvent.m_srcWidget.gameObject.transform.Find("imgAuto");
				transform.gameObject.CustomSetActive(!hostPlayer.Captain.get_handle().ActorControl.m_isAutoAI);
		...
		{
			if (Singleton<GamePlayerCenter>.get_instance() != null && Singleton<GamePlayerCenter>.get_instance().GetHostPlayer() != null && Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().Captain && param.src == Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().Captain)
			{
				GameObject gameObject = (!(this._formScript != null)) ? null : Utility.FindChild(this._formScript.gameObject, "PanelBtn/ToggleAutoBtn");
				if (gameObject == null)
				{
					return;
				}
				...
				transform.gameObject.CustomSetActive(Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().Captain.get_handle().ActorControl.m_isAutoAI);
				MonoSingleton<DialogueProcessor>.GetInstance().bAutoNextPage = Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().Captain.get_handle().ActorControl.m_isAutoAI;
			}
		}
		...
		{
			Singleton<CUIManager>.GetInstance().CloseForm(CSettingsSys.SETTING_FORM);
			if (Singleton<BattleLogic>.GetInstance().GetCurLvelContext().IsGameTypeGuide() && !Singleton<CBattleGuideManager>.get_instance().bTrainingAdv)
			{
			...
			if (Singleton<CBattleGuideManager>.get_instance().bPauseGame)
			{
				Singleton<CBattleGuideManager>.get_instance().ResumeGame(this);
			}
			if (Singleton<LobbyLogic>.get_instance().inMultiGame)
			{
				Singleton<LobbyLogic>.get_instance().ReqMultiGameRunaway();
			}
			else
			{
				Singleton<BattleLogic>.get_instance().DoFightOver(false);
				Singleton<BattleLogic>.get_instance().SingleReqLoseGame();
			}
		}
		...
		private void OnPlayerProtectDisappear(ref ActorSkillEventParam _param)
		{
			if (_param.src)
			{
				Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.ShieldDisappear, (Vector3)_param.src.get_handle().location, new string[0]);
			}
		}
		...
		private void OnActorImmune(ref DefaultGameEventParam _param)
		{
			...
			{
				Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.Immunity, (Vector3)_param.src.get_handle().location, new string[0]);
			}
		...
		private void OnActorHurtAbsorb(ref DefaultGameEventParam _param)
		{
			if (_param.src && ActorHelper.IsHostActor(ref _param.src))
			{
				Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.Absorb, (Vector3)_param.src.get_handle().location, new string[0]);
			}
			else if (_param.atker && ActorHelper.IsHostActor(ref _param.atker))
			{
				Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.Absorb, (Vector3)_param.src.get_handle().location, new string[0]);
		...
		private void OnPlayerBlindess(ref ActorSkillEventParam _param)
		{
			if (_param.src)
			{
				Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.Blindess, (Vector3)_param.src.get_handle().location, new string[0]);
			}
		}
		...
		private void OnPlayerSkillCooldown(ref ActorSkillEventParam _param)
		{
			if (_param.src)
			{
				...
				{
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.InCooldown, (Vector3)_param.src.get_handle().location, new string[0]);
		...
		private void OnPlayerEnergyShortage(ref ActorSkillEventParam _param)
		{
			if (_param.src)
			{
				...
				{
					if (_param.src.get_handle().ValueComponent == null)
					{
						return;
					}
					int energyType = (int)_param.src.get_handle().ValueComponent.mActorValue.EnergyType;
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(EnergyCommon.GetShortageText(energyType), (Vector3)_param.src.get_handle().location, new string[0]);
		...
		private void OnPlayerSkillBeanShortage(ref ActorSkillEventParam _param)
		{
			if (_param.src)
			{
				...
				{
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.BeanShortage, (Vector3)_param.src.get_handle().location, new string[0]);
		...
		private void OnPlayerNoSkillTarget(ref ActorSkillEventParam _param)
		{
			if (_param.src)
			{
				...
				{
					Singleton<CBattleSystem>.GetInstance().CreateOtherFloatText(enOtherFloatTextContent.NoTarget, (Vector3)_param.src.get_handle().location, new string[0]);
		...
		{
			if (!Singleton<BattleLogic>.get_instance().isRuning || this.m_skillButtonManager == null)
			{
				return;
			}
		...
		{
			Singleton<CBattleGuideManager>.get_instance().PauseGame(this, true);
			if (this._formScript != null)
			{
		...
		public void ShowWinLosePanel(bool bWin)
		{
			...
			Singleton<CRecordUseSDK>.get_instance().DoFightOver();
		}
		...
		private void OnActorGoldCoinInBattleChanged(PoolObjHandle<ActorRoot> actor, int changeValue, bool isIncome, PoolObjHandle<ActorRoot> target)
		{
			...
			if (!actor || !ActorHelper.IsHostCtrlActor(ref actor))
			{
				return;
			}
			if (this._formScript != null)
			{
				...
				if (widget != null)
				{
					...
					{
						num = actor.get_handle().ValueComponent.GetGoldCoinInBattle();
					}
					if (component != null)
					{
		...
		{
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm(FightForm.s_battleDragonTipForm, false, true);
			...
			ResSkillCombineCfgInfo dataByKey = GameDataMgr.skillCombineDatabin.GetDataByKey((long)Singleton<BattleLogic>.get_instance().GetDragonBuffId(1));
			ResSkillCombineCfgInfo dataByKey2 = GameDataMgr.skillCombineDatabin.GetDataByKey((long)Singleton<BattleLogic>.get_instance().GetDragonBuffId(2));
			ResSkillCombineCfgInfo dataByKey3 = GameDataMgr.skillCombineDatabin.GetDataByKey((long)Singleton<BattleLogic>.get_instance().GetDragonBuffId(3));
			if (dataByKey != null)
			{
		...
		{
			Singleton<CUIManager>.get_instance().CloseForm(FightForm.s_battleDragonTipForm);
		}
		...
		private void OnCommon_BattleWifiCheckTimer(CUIEvent uiEvent)
		{
			...
			this.m_displayPing = ((!this.m_bShowRealPing) ? Singleton<FrameSynchr>.GetInstance().GameSvrPing : Singleton<FrameSynchr>.get_instance().RealSvrPing);
			...
			if (this.m_wifiIcon != null && this.m_wifiIconCheckTicks >= this.m_wifiIconCheckMaxTicks)
			{
				...
				{
					this.m_wifiIcon.SetSprite(CUIUtility.s_Sprite_System_Wifi_Dir + CLobbySystem.s_wifiStateName[(int)((UIntPtr)num2)], this._formScript, true, false, false, false);
				}
				else if (internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork)
				{
					this.m_wifiIcon.SetSprite(CUIUtility.s_Sprite_System_Wifi_Dir + CLobbySystem.s_netStateName[(int)((UIntPtr)num2)], this._formScript, true, false, false, false);
				}
						...
						if (gameObject != null)
						{
							...
							if (transform != null)
							{
								...
								if (transform2 != null)
								{
									...
									if (component != null)
									{
										float totalTime = (float)captain.get_handle().ActorControl.ReviveCooldown * 0.001f;
		...
		private void OnReviveTimerChange(CUIEvent uiEvent)
		{
			...
			if (this._formScript != null && curLvelContext != null)
			{
				...
				if (hostPlayer != null)
				{
					...
					if (captain)
					{
						float num = (float)captain.get_handle().ActorControl.ReviveCooldown * 0.001f;
		...
		{
			GameObject gameObject = Utility.FindChild(this._formScript.gameObject, "ArenaTimer63s");
			if (gameObject == null)
			{
				return;
			}
		...
		private void onUseSkill(ref ActorSkillEventParam prm)
		{
			...
			Player hostPlayer = Singleton<GamePlayerCenter>.get_instance().GetHostPlayer();
			int configId = hostPlayer.Captain.get_handle().TheActorMeta.ConfigId;
			SLOTINFO sLOTINFO = null;
			for (int i = 0; i < this.m_SkillSlotList.get_Count(); i++)
			{
				if (this.m_SkillSlotList.get_Item(i).id == configId)
				{
					sLOTINFO = this.m_SkillSlotList.get_Item(i);
					break;
				}
			}
			if (sLOTINFO == null)
		...
		private void OnBattleLearnSkillButtonClick(CUIEvent uiEvent)
		{
			...
			byte bSkillLvl = 0;
			if (captain.get_handle().SkillControl != null && captain.get_handle().SkillControl.SkillSlotArray[num] != null)
			{
				bSkillLvl = (byte)captain.get_handle().SkillControl.SkillSlotArray[num].GetSkillLevel();
			}
			this.SendLearnSkillCommand(captain, (SkillSlotType)num, bSkillLvl);
		...
		private void SendLearnSkillCommand(PoolObjHandle<ActorRoot> actor, SkillSlotType enmSkillSlotType, byte bSkillLvl)
		{
			...
			frameCommand.cmdData.dwHeroID = actor.get_handle().ObjID;
		...
		private void onHeroEnergyChange(PoolObjHandle<ActorRoot> actor, int curVal, int maxVal)
		{
			...
			if (actor && actor == captain)
			{
				for (int i = 1; i <= 3; i++)
				{
					SkillSlot skillSlot = actor.get_handle().SkillControl.SkillSlotArray[i];
		...
		private void OnHeroSoulLvlChange(PoolObjHandle<ActorRoot> hero, int level)
		{
			...
			if (hero.get_handle().ActorAgent != null && !hero.get_handle().ActorAgent.IsAutoAI() && captain == hero)
			{
		...
		public void ClearSkillLvlStates(int iSkillSlotType)
		{
			...
			if (button != null)
			{
				...
				if (skillLvlImg != null)
				{
					...
					num = listView.get_Count();
					for (int j = 0; j < num; j++)
					{
						GameObject obj = listView.get_Item(j);
						obj.CustomSetActive(false);
					}
					listView.Clear();
				}
		...
		{
			this.SendTrusteeshipResult(uiEvent.m_eventParams.commonUInt32Param1, 1);
		}

		public void OnCancelTrusteeship(CUIEvent uiEvent)
		{
			this.SendTrusteeshipResult(uiEvent.m_eventParams.commonUInt32Param1, 0);
		}
		...
		public void SendTrusteeshipResult(uint objID, ACCEPT_AIPLAYER_RSP trusteeshipRsp)
		{
			...
			cSPkg.stPkgData.get_stIsAcceptAiPlayerRsp().dwAiPlayerObjID = objID;
			cSPkg.stPkgData.get_stIsAcceptAiPlayerRsp().bResult = trusteeshipRsp;
		...
		private void OnBattleSkillDecShow(CUIEvent uiEvent)
		{
			...
			if (hostPlayer.Captain.get_handle().SkillControl.TryGetSkillSlot(type, out skillSlot))
			{
				...
				this.OnBattleHeroSkillTipOpen(skillSlot, position, (uint)hostPlayer.Captain.get_handle().TheActorMeta.ConfigId);
			}
		}
		...
		private void OnBattleHeroSkillTipOpen(SkillSlot skillSlot, Vector3 Pos, uint heroId)
		{
			...
			{
				component2.text = CUICommonSystem.GetSkillDesc(skillSlot.SkillObj.cfgData.szSkillDesc, actorValue, skillSlot.GetSkillLevel(), captain.get_handle().ValueComponent.actorSoulLevel, heroId);
			}
			Text component3 = this.skillTipDesc.transform.Find("SkillCDText").GetComponent<Text>();
			component3.text = Singleton<CTextManager>.get_instance().GetText("Skill_Cool_Down_Tips", new string[]
			{
				(skillSlot.GetSkillCDMax() / 1000).ToString()
			});
			Text component4 = component3.transform.Find("SkillEnergyCostText").GetComponent<Text>();
			component4.text = Singleton<CTextManager>.get_instance().GetText(EnergyCommon.GetEnergyShowText((uint)captain.get_handle().ValueComponent.mActorValue.EnergyType, EnergyShowType.CostValue), new string[]
			{
			...
			for (int i = 1; i <= 2; i++)
			{
				...
				{
					gameObject.CustomSetActive(true);
					gameObject.GetComponent<Image>().SetSprite(CSkillData.GetEffectSlotBg(skillEffectType[i - 1]), this._formScript, true, false, false, false);
					gameObject.transform.Find("Text").GetComponent<Text>().text = CSkillData.GetEffectDesc(skillEffectType[i - 1]);
				}
				else
				{
					gameObject.CustomSetActive(false);
				...
				if (!CFakePvPHelper.bInFakeSelect && MonoSingleton<VoiceSys>.GetInstance().IsBattleSupportVoice())
				{
					if (!MonoSingleton<VoiceSys>.GetInstance().GlobalVoiceSetting)
					{
						if (!bInit)
						{
							...
							this.m_OpenSpeakerTipObj.gameObject.CustomSetActive(true);
							Singleton<CTimerManager>.get_instance().ResumeTimer(this.m_Vocetimer);
						}
						return;
					}
		...
		private void OnBattleOpenMic(CUIEvent uiEvent)
		{
			...
			if (this.m_OpenMicTipObj)
			{
				if (!this.m_bOpenSpeak)
				{
					...
					Singleton<CTimerManager>.get_instance().ResumeTimer(this.m_VoiceMictime);
					return;
				}
				if (!CFakePvPHelper.bInFakeSelect && MonoSingleton<VoiceSys>.GetInstance().IsBattleSupportVoice())
				{
		...
		{
			ConnectorParam connectionParam = Singleton<NetworkModule>.get_instance().gameSvr.GetConnectionParam();
			Singleton<GameBuilder>.get_instance().EndGame();
			if (connectionParam != null)
			{
				Singleton<NetworkModule>.get_instance().InitGameServerConnect(connectionParam);
			}
		}
		...
		public void ShowHeroInfoPanel(PoolObjHandle<ActorRoot> hero)
		{
			...
			if (this.m_panelHeroInfo == null || this.m_panelHeroCanvas == null || !hero || hero.get_handle().ValueComponent == null)
			{
				return;
			}
		...
		{
			if (iCurVal <= 0 && Singleton<GamePlayerCenter>.GetInstance().HostPlayerId != 0u && hero == Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().Captain)
			{
		...
		public void UpdateHpInfo()
		{
			...
			int actorHp = this.m_selectedHero.get_handle().ValueComponent.actorHp;
			int totalValue = this.m_selectedHero.get_handle().ValueComponent.mActorValue[5].totalValue;
		...
		public void UpdateEpInfo()
		{
			...
			int actorEp = this.m_selectedHero.get_handle().ValueComponent.actorEp;
			int totalValue = this.m_selectedHero.get_handle().ValueComponent.mActorValue[32].totalValue;
		...
		public void UpdateAdValueInfo()
		{
			...
			int totalValue = this.m_selectedHero.get_handle().ValueComponent.mActorValue[1].totalValue;
		...
		public void UpdateApValueInfo()
		{
			...
			int totalValue = this.m_selectedHero.get_handle().ValueComponent.mActorValue[2].totalValue;
		...
		public void UpdatePhyDefValueInfo()
		{
			...
			int totalValue = this.m_selectedHero.get_handle().ValueComponent.mActorValue[3].totalValue;
		...
		public void UpdateMgcDefValueInfo()
		{
			...
			int totalValue = this.m_selectedHero.get_handle().ValueComponent.mActorValue[4].totalValue;
		...
		private void SetSelectedHeroInfo(PoolObjHandle<ActorRoot> hero)
		{
			...
			if (this.m_objHeroHead != null)
			{
				uint configId = (uint)hero.get_handle().TheActorMeta.ConfigId;
		...
		{
			if (Singleton<GameObjMgr>.get_instance() == null)
			{
				return;
			}
			PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.get_instance().GetActor(uilockTargetID);
			if (!actor)
			{
				return;
			}
		...
		private void OnLockTarget(ref LockTargetEventParam prm)
		{
			...
			{
				this.m_HideSelectedHero = Singleton<GameObjMgr>.get_instance().GetActor(prm.lockTargetID);
				return;
			}
		...
		private void OnSelectTarget(ref SelectTargetEventParam prm)
		{
			if (!GameSettings.EnableHeroInfo)
			{
				this.m_HideSelectedHero = Singleton<GameObjMgr>.get_instance().GetActor(prm.commonAttackTargetID);
				return;
			}
			if (Singleton<GamePlayerCenter>.GetInstance().HostPlayerId == 0u)
			{
