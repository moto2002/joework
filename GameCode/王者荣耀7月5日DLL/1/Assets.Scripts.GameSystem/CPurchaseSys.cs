...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CPurchaseSys : Singleton<CPurchaseSys>
	{
		...
		{
			switch (msg.stPkgData.get_stShopBuyRsp().iBuyType)
			{
			case 1:
			case 2:
			case 3:
			...
			case 10:
			case 11:
				Singleton<EventRouter>.get_instance().BroadCastEvent<CSPkg>("ShopBuyDraw", msg);
				break;
			case 4:
			case 5:
				Singleton<EventRouter>.get_instance().BroadCastEvent<CSPkg>("ShopBuyPurchase", msg);
				Singleton<CSoundManager>.get_instance().PostEvent("UI_Add_Physical", null);
				break;
			case 6:
				Singleton<EventRouter>.get_instance().BroadCastEvent<CSPkg>("ShopBuyLvlChallengeTime", msg);
				break;
			case 7:
				CSymbolWearController.OnSymbolBuySuccess(msg.stPkgData.get_stShopBuyRsp().iChgValue, msg.stPkgData.get_stShopBuyRsp().iBuySubType);
				break;
			case 9:
				Singleton<CArenaSystem>.GetInstance().ResetFightTimes(msg.stPkgData.get_stShopBuyRsp().iBuySubType, msg.stPkgData.get_stShopBuyRsp().iChgValue);
				break;
			case 12:
				CSymbolWearController.OnSymbolGridBuySuccess(msg.stPkgData.get_stShopBuyRsp().iChgValue);
				break;
			case 13:
				Singleton<CHeroSelectNormalSystem>.get_instance().OnHeroCountBought();
				break;
			}
		}
		...
		{
			Singleton<CHeroSelectBaseSystem>.get_instance().ResetRandHeroLeftCount((int)msg.stPkgData.get_stEntertainmentRandHeroCnt().dwRandHeroCnt);
		}
		...
		public static void BuyCoinRsp(CSPkg msg)
		{
			...
			Singleton<CSoundManager>.get_instance().PostEvent("UI_Add_Gold", null);
		}
		...
		{
			int iBuyType = msg.stPkgData.get_stShopBuyRsp().iBuyType;
			if (iBuyType != 4)
			{
				if (iBuyType == 5)
				{
