	...
	public class CAchieveInfo2
	{
		...
		public void OnServerAchieveInfo(ref COMDT_ACHIEVEMENT_INFO svrAchieveInfo)
		{
			...
			while ((long)num < (long)((ulong)svrAchieveInfo.dwAchievementNum))
			{
				...
				COM_ACHIEVEMENT_STATE bState = svrAchieveInfo.astAchievementData[num].bState;
				if (this.m_AchiveItemDic.ContainsKey(dwID))
				{
					this.m_AchiveItemDic.get_Item(dwID).DoneTime = dwDoneTime;
					this.m_AchiveItemDic.get_Item(dwID).State = bState;
					if (this.m_AchiveItemDic.get_Item(dwID).DoneType >= 0 && this.m_AchiveItemDic.get_Item(dwID).DoneType < array.Length)
					{
						this.m_AchiveItemDic.get_Item(dwID).DoneCnt = array[this.m_AchiveItemDic.get_Item(dwID).DoneType];
					}
				}
				num++;
			}
		...
		private void SetSelectedTrophies(ref CSDT_SHOWACHIEVE_DETAIL[] serverSelectedTrophies, bool setFinishRecursively = false)
		{
			for (int i = 0; i < this.SelectedTrophies.Length; i++)
			{
				if (this.m_AchiveItemDic.ContainsKey(serverSelectedTrophies[i].dwAchieveID))
				{
					...
					this.m_AchiveItemDic.TryGetValue(serverSelectedTrophies[i].dwAchieveID, ref cAchieveItem);
		...
		private void SetSelectedTrophies(ref uint[] serverSelectedTrophies)
		{
			for (int i = 0; i < this.SelectedTrophies.Length; i++)
			{
				...
				{
					CAchieveItem2 head;
					this.m_AchiveItemDic.TryGetValue(serverSelectedTrophies[i], ref head);
					if (head != null)
					{
						head = head.GetHead();
		...
		public uint GetTotalDonePoints()
		{
			...
			{
				if (this.m_Trophies.get_Item(i) != null)
				{
					num += this.m_Trophies.get_Item(i).GetTotalDonePoints();
				}
			}
			return num;
		}
		...
		public void GetTrophyProgress(ref uint cur, ref uint next)
		{
			...
			else
			{
				...
				{
					CTrophyRewardInfo cTrophyRewardInfo = this.TrophyRewardDic.get_Item(num);
		...
		public CTrophyRewardInfo GetFirstTrophyRewardInfoAwardNotGot()
		{
			...
			for (int i = 0; i < trophyRewardInfoWithRewards.get_Count(); i++)
			{
				CTrophyRewardInfo cTrophyRewardInfo = trophyRewardInfoWithRewards.get_Item(i);
		...
		public void TrophyLevelUp(uint oldLevel, uint newLevel)
		{
			for (uint num = oldLevel + 1u; num <= newLevel; num += 1u)
			{
				...
				{
					this.LastDoneTrophyRewardInfo = this.TrophyRewardDic.get_Item(num);
					this.LastDoneTrophyRewardInfo.State = TrophyState.Finished;
				}
			}
			uint num2 = newLevel + 1u;
			if (this.TrophyRewardDic.ContainsKey(num2))
			{
				this.TrophyRewardDic.get_Item(num2).State = TrophyState.OnGoing;
			}
		}
		...
		public void ChangeAchieveState(ref COMDT_ACHIEVEMENT_DATA data)
		{
			...
			{
				this.m_AchiveItemDic.get_Item(data.dwID).State = data.bState;
				this.m_AchiveItemDic.get_Item(data.dwID).DoneTime = data.dwDoneTime;
