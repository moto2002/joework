	...
	public class CBattleEquipSystem
	{
		...
		public CBattleEquipSystem()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ushort arg_112_1 = current.get_Key();
				KeyValuePair<ushort, CEquipInfo> current2 = enumerator.get_Current();
				arg_112_0.Add(arg_112_1, new CBattleEquipSystem.CEquipBuyPrice(current2.get_Value().m_resEquipInBattle.dwBuyPrice));
			}
		}
		...
		public void Update()
		{
			...
			{
				bool flag = this.m_hostCtrlHero.get_handle().EquipComponent.IsPermitedToBuyEquip(this.m_isInEquipLimitedLevel);
		...
		public bool IsEquipCanBought(ushort equipID, ref PoolObjHandle<ActorRoot> actor, ref CBattleEquipSystem.CEquipBuyPrice equipBuyPrice)
		{
			...
			if (dataByKey.bNeedPunish > 0 && !actor.get_handle().SkillControl.HasPunishSkill())
			{
				return false;
			}
			if (dataByKey.wGroup > 0 && actor.get_handle().EquipComponent.HasEquipInGroup(dataByKey.wGroup))
			{
				return false;
			}
			...
			if (requiredEquipIDs != null && requiredEquipIDs.Length > 0)
			{
				...
				{
					if (actor.get_handle().EquipComponent.HasEquip(requiredEquipIDs[i], 1u))
					{
						flag = true;
						break;
					}
			...
			return (ulong)actor.get_handle().ValueComponent.GetGoldCoinInBattle() >= (ulong)((long)equipBuyPrice.m_buyPrice) && actor.get_handle().EquipComponent.IsEquipCanAddedToGrid(equipID, ref equipBuyPrice);
		}
		...
		public bool IsHorizonEquipCanBought(ushort equipID, ref PoolObjHandle<ActorRoot> actor, ref int price)
		{
			...
			{
				SkillSlot skillSlot = actor.get_handle().SkillControl.SkillSlotArray[7];
				if (dataByKey.dwActiveSkillID > 0u && (skillSlot == null || (ulong)dataByKey.dwActiveSkillID != (ulong)((long)skillSlot.SkillObj.SkillID)) && actor.get_handle().ValueComponent.GetGoldCoinInBattle() >= (int)dataByKey.dwBuyPrice)
				{
					return true;
				}
			}
		...
		public void GetEquipBuyPrice(ushort equipID, ref PoolObjHandle<ActorRoot> actor, ref CBattleEquipSystem.CEquipBuyPrice equipBuyPrice)
		{
			...
			freeExistEquipInfoSet.Clone(actor.get_handle().EquipComponent.GetExistEquipInfoSet());
		...
		public ushort[] GetQuicklyBuyEquipList(ref PoolObjHandle<ActorRoot> actor)
		{
			...
			if (!actor || actor.get_handle().EquipComponent == null)
			{
				return this.m_tempQuicklyBuyEquipIDs;
			}
			ListView<CRecommendEquipInfo> recommendEquipInfos = actor.get_handle().EquipComponent.GetRecommendEquipInfos();
			Dictionary<ushort, uint> equipBoughtHistory = actor.get_handle().EquipComponent.GetEquipBoughtHistory();
			bool flag = false;
			...
			freeExistEquipInfoSet.Clone(actor.get_handle().EquipComponent.GetExistEquipInfoSet());
			int num2 = 0;
			for (int j = 0; j < recommendEquipInfos.get_Count(); j++)
			{
				if (!recommendEquipInfos.get_Item(j).m_hasBeenBought)
				{
					if (recommendEquipInfos.get_Item(j).m_resEquipInBattle.bUsage != 4 || !flag)
					{
						this.m_tempRelatedRecommondEquips.Clear();
						freeExistEquipInfoSet.ResetCalculateAmount();
						this.GetRelatedRecommondEquips(recommendEquipInfos.get_Item(j).m_equipID, true, ref actor, ref freeExistEquipInfoSet, ref this.m_tempRelatedRecommondEquips);
						for (int k = 0; k < num2; k++)
						{
							for (int l = 0; l < this.m_tempRelatedRecommondEquips.get_Count(); l++)
							{
								if (this.m_tempQuicklyBuyEquipIDs[k] == this.m_tempRelatedRecommondEquips.get_Item(l).m_equipID)
								{
						...
						if (this.m_tempRelatedRecommondEquips.get_Count() == 0)
						{
							break;
						}
						this.m_tempRelatedRecommondEquips.Sort();
						int num3 = Math.Min(this.m_tempRelatedRecommondEquips.get_Count(), 2 - num2);
						for (int m = 0; m < num3; m++)
						{
							this.m_tempQuicklyBuyEquipIDs[num2] = this.m_tempRelatedRecommondEquips.get_Item(m).m_equipID;
							num2++;
						}
						if ((long)num2 >= 2L)
						{
		...
		private void OnHeroEquipInBattleChanged(uint actorObjectID, stEquipInfo[] equips, bool bIsAdd, int iEquipSlotIndex)
		{
			...
			if (this.m_hostCtrlHero && actorObjectID == this.m_hostCtrlHero.get_handle().ObjID)
			{
				this.RefreshHostPlayerCachedEquipBuyPrice();
		...
		public void SendBuyEquipFrameCommand(ushort equipID, bool isQuicklyBuy)
		{
			...
			if (isQuicklyBuy)
			{
				...
				if (this.m_hostCtrlHero)
				{
					this.m_hostCtrlHero.get_handle().EquipComponent.m_iFastBuyEquipCount++;
				}
			}
			else if (this.m_hostCtrlHero)
			{
				this.m_hostCtrlHero.get_handle().EquipComponent.m_iBuyEquipCount++;
			}
		}
		...
		{
			if (!Singleton<WatchController>.GetInstance().IsWatching && !actor.get_handle().IsHostCamp())
			{
				return;
			}
			if (inOut > 0)
			{
				actor.get_handle().HudControl.AddInEquipShopHud();
			}
			else
			{
				actor.get_handle().HudControl.RemoveInEquipShopHud();
			}
		}
		...
		private void InitializeRecommendEquipList()
		{
			...
			if (this.m_hostCtrlHero)
			{
				ListView<CRecommendEquipInfo> recommendEquipInfos = this.m_hostCtrlHero.get_handle().EquipComponent.GetRecommendEquipInfos();
				if (recommendEquipInfos != null)
				{
					for (int i = 0; i < recommendEquipInfos.get_Count(); i++)
					{
						this.AddRecommendPreEquip(recommendEquipInfos.get_Item(i).m_equipID, true);
					}
				}
			}
		}
		...
		{
			if (!this.m_isEnabled || !actor || !actor.get_handle().EquipComponent.IsPermitedToBuyEquip(this.m_isInEquipLimitedLevel))
			{
				return;
			}
			CBattleEquipSystem.CEquipBuyPrice freeEquipBuyPrice = this.GetFreeEquipBuyPrice();
			...
			actor.get_handle().ValueComponent.ChangeGoldCoinInBattle(freeEquipBuyPrice.m_buyPrice * -1, false, false, default(Vector3), false, default(PoolObjHandle<ActorRoot>));
			for (int i = 0; i < freeEquipBuyPrice.m_swappedPreEquipInfoCount; i++)
			{
				if (freeEquipBuyPrice.m_swappedPreEquipInfos[i].m_equipID > 0 && freeEquipBuyPrice.m_swappedPreEquipInfos[i].m_swappedAmount > 0u)
				{
					...
					{
						actor.get_handle().EquipComponent.RemoveEquip(freeEquipBuyPrice.m_swappedPreEquipInfos[i].m_equipID);
			...
			if (requiredEquipIDs != null && requiredEquipIDs.Length > 0)
			{
				...
				{
					if (actor.get_handle().EquipComponent.HasEquip(requiredEquipIDs[j], 1u))
					{
						actor.get_handle().EquipComponent.RemoveEquip(requiredEquipIDs[j]);
						break;
					}
				}
			}
			actor.get_handle().EquipComponent.AddEquip(equipID);
			actor.get_handle().EquipComponent.UpdateEquipEffect();
			freeEquipBuyPrice.m_used = false;
		}

		private void SellEquip(int equipIndexInGrid, ref PoolObjHandle<ActorRoot> actor)
		{
			if (!this.m_isEnabled || equipIndexInGrid < 0 || equipIndexInGrid >= 6 || !actor || !actor.get_handle().EquipComponent.IsPermitedToBuyEquip(this.m_isInEquipLimitedLevel))
			{
				return;
			}
			stEquipInfo[] equips = actor.get_handle().EquipComponent.GetEquips();
			if (equips[equipIndexInGrid].m_equipID > 0 && equips[equipIndexInGrid].m_amount > 0u)
			{
				...
				if (dataByKey != null)
				{
					actor.get_handle().EquipComponent.RemoveEquip(equipIndexInGrid);
					actor.get_handle().EquipComponent.UpdateEquipEffect();
					actor.get_handle().ValueComponent.ChangeGoldCoinInBattle(dataByKey.dwSalePrice, false, false, default(Vector3), false, default(PoolObjHandle<ActorRoot>));
				}
			}
		}
		...
		private void BuyHorizonEquip(ushort equipID, ref PoolObjHandle<ActorRoot> actor)
		{
			...
			if (curLvelContext != null && curLvelContext.m_bEnableOrnamentSlot)
			{
				SkillComponent skillControl = actor.get_handle().SkillControl;
				if (actor.get_handle().BuffHolderComp != null && skillControl != null)
				{
					...
					if (skillSlot == null || skillSlot.SkillObj == null)
					{
						...
						if (actor.get_handle().SkillControl.TryGetSkillSlot(SkillSlotType.SLOT_SKILL_7, out skillSlot))
						{
					...
					{
						BuffChangeSkillRule changeSkillRule = actor.get_handle().BuffHolderComp.changeSkillRule;
					...
					actor.get_handle().EquipComponent.m_horizonEquipId = equipID;
				}
		...
		{
			for (int i = 0; i < this.m_existEquipInfoSetPool.get_Count(); i++)
			{
				if (!this.m_existEquipInfoSetPool.get_Item(i).m_used)
				{
					this.m_existEquipInfoSetPool.get_Item(i).Clear();
					this.m_existEquipInfoSetPool.get_Item(i).m_used = true;
					return this.m_existEquipInfoSetPool.get_Item(i);
				}
			}
		...
		{
			for (int i = 0; i < this.m_equipBuyPricePool.get_Count(); i++)
			{
				if (!this.m_equipBuyPricePool.get_Item(i).m_used)
				{
					this.m_equipBuyPricePool.get_Item(i).Clear();
					this.m_equipBuyPricePool.get_Item(i).m_used = true;
					return this.m_equipBuyPricePool.get_Item(i);
				}
			}
		...
		private void RefreshEquipItem(Transform equipItem, ushort equipID, bool onlyRefreshPriceAndOwned)
		{
			...
			if (resEquipInBattle != null)
			{
				...
				else
				{
					...
					flag2 = this.m_hostCtrlHero.get_handle().EquipComponent.HasEquip(equipInfo.m_equipID, 1u);
		...
		private void RefreshEquipSalePanel(CEquipInfo equipInfo, int positionInBag, Transform salePanel)
		{
			...
			CUICommonSystem.SetButtonEnableWithShader(component2, positionInBag != 6 && this.m_hostCtrlHeroPermitedToBuyEquip, true);
		...
		private Transform GetEquipItem(int level, int index)
		{
			...
			if (index >= 0 && index < this.m_equipItemList.get_Count())
			{
				return this.m_equipItemList.get_Item(index);
			}
			return null;
		}
		...
		private void InitBagEquipItemList()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				Transform transform2 = transform.Find(string.Format("equipItem{0}", i));
				this.m_bagEquipItemList.Add(transform2);
			}
		}

		private Transform GetBagEquipItem(int index)
		{
			if (index >= 0 && index < this.m_bagEquipItemList.get_Count())
			{
				return this.m_bagEquipItemList.get_Item(index);
			}
			return null;
		}
		...
		public void OnEquipItemSelect(CUIEvent uiEvent)
		{
			...
			this.SetSelectedEquipItem(CBattleEquipSystem.enSelectedEquipOrigin.EquipLibaray, uiEvent.m_eventParams.battleEquipPar.equipInfo, uiEvent.m_srcWidget.transform, -1);
		...
		public void OnEquipBagItemSelect(CUIEvent uiEvent)
		{
			...
			this.SetSelectedEquipItem(CBattleEquipSystem.enSelectedEquipOrigin.EquipBag, uiEvent.m_eventParams.battleEquipPar.equipInfo, uiEvent.m_srcWidget.transform, uiEvent.m_eventParams.battleEquipPar.pos);
		...
		public void OnBattleEquipSelectItemInEquipTree(CUIEvent uiEvent)
		{
			...
			this.SetSelectedEquipItem(CBattleEquipSystem.enSelectedEquipOrigin.EquipTree, this.GetEquipInfo(uiEvent.m_eventParams.commonUInt16Param1), uiEvent.m_srcWidget.transform, -1);
		...
		public void OnBattleEquipBackEquipListSelectedChanged(CUIEvent uiEvent)
		{
			...
			if (this.m_selectedEquipOrigin != CBattleEquipSystem.enSelectedEquipOrigin.None && this.m_selectedEquipItems[(int)this.m_selectedEquipOrigin].m_equipInfo != null)
			{
				...
				num = ((list != null) ? list.get_Count() : 0);
			}
			...
			if (cUIListScript != null)
			{
				...
				if (selectedIndex >= 0 && selectedIndex < num)
				{
					...
					this.SetSelectedEquipItem(CBattleEquipSystem.enSelectedEquipOrigin.EquipTree, this.GetEquipInfo(num2), equipItemTransform, -1);
					this.m_selectedEquipOrigin = CBattleEquipSystem.enSelectedEquipOrigin.EquipTree;
					this.RefreshEquipTreePanel(false);
					this.RefreshEquipFormRightPanel(false);
				}
		...
		private void SetSelectedEquipItem(CBattleEquipSystem.enSelectedEquipOrigin equipSelectedOrigin, CEquipInfo equipInfo, Transform equipItemTransform, int positionInBag)
		{
			...
			bool bIsActiveSKillEquip2 = this.IsActiveEquipButNotHorizon(equipInfo.m_equipID);
			if (equipItemTransform2 != equipItemTransform)
			{
		...
		private void OpenEquipTreePanel(CEquipInfo rootEquipInfo, bool selectRootItem)
		{
			...
			if (selectRootItem)
			{
				...
				this.SetSelectedEquipItem(CBattleEquipSystem.enSelectedEquipOrigin.EquipTree, rootEquipInfo, equipItemTransform, -1);
				this.m_selectedEquipOrigin = CBattleEquipSystem.enSelectedEquipOrigin.EquipTree;
			}
			this.RefreshEquipTreePanel(false);
			this.RefreshEquipFormRightPanel(false);
		...
		private void RefreshEquipBagPanel()
		{
			...
			{
				stEquipInfo[] equips = this.m_hostCtrlHero.get_handle().EquipComponent.GetEquips();
				for (int i = 0; i < 6; i++)
				{
		...
		private void RefreshHorizonEquipGrid(GameObject horizonEquipGrid)
		{
			...
			CEquipInfo equipInfo = this.GetEquipInfo(this.m_hostCtrlHero.get_handle().EquipComponent.m_horizonEquipId);
			if (equipInfo != null)
			{
				component.gameObject.CustomSetActive(true);
				component.SetSprite(equipInfo.m_equipIconPath, this.m_equipFormScript, true, false, false, false);
				...
				component2.m_onClickEventID = enUIEventID.BattleEquip_BagItem_Select;
				component2.m_onClickEventParams.battleEquipPar.equipInfo = equipInfo;
				component2.m_onClickEventParams.battleEquipPar.pos = 6;
			}
			else
			{
				component.gameObject.CustomSetActive(false);
		...
		private void RefreshGoldCoin()
		{
			...
			if (widget != null)
			{
				widget.GetComponent<Text>().text = this.m_hostCtrlHero.get_handle().ValueComponent.GetGoldCoinInBattle().ToString();
			}
		}
		...
		public void ExecChooseEquipSkillCmd(int iEquipSlotIndex, ref PoolObjHandle<ActorRoot> actor)
		{
			...
			if (actor && actor.get_handle().EquipComponent != null)
			{
				ENUM_EQUIP_ACTIVESKILL_STATUS equipActiveSkillSlotInfo = actor.get_handle().EquipComponent.GetEquipActiveSkillSlotInfo(iEquipSlotIndex);
				if (equipActiveSkillSlotInfo == ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_NOTSHOW)
				{
					SkillSlotType showEquipActiveSkillSlot = actor.get_handle().EquipComponent.GetShowEquipActiveSkillSlot();
					int equipSlotBySkillSlot = actor.get_handle().EquipComponent.GetEquipSlotBySkillSlot(showEquipActiveSkillSlot);
					if (equipSlotBySkillSlot >= 0 && equipSlotBySkillSlot < 6)
					{
						SkillSlot skillSlot;
						if (actor.get_handle().SkillControl != null && actor.get_handle().SkillControl.TryGetSkillSlot(showEquipActiveSkillSlot, out skillSlot))
						{
							int cd = skillSlot.CurSkillCD;
							actor.get_handle().EquipComponent.AddEquipActiveSkillCdInfo(equipSlotBySkillSlot, cd);
						}
						actor.get_handle().EquipComponent.SetEquipActiveSkillSlotInfo(equipSlotBySkillSlot, ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_NOTSHOW);
						if (actor == this.m_hostCtrlHero)
						{
							this.SetEquipSkillBeUsingBtnState(false, equipSlotBySkillSlot);
						}
					}
					if (actor.get_handle().EquipComponent.ChangeEquipSkillSlot(showEquipActiveSkillSlot, iEquipSlotIndex))
					{
						actor.get_handle().EquipComponent.SetEquipActiveSkillSlotInfo(iEquipSlotIndex, ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_ISSHOWING);
						if (actor == this.m_hostCtrlHero)
						{
							if (actor.get_handle().EquipComponent.GetEquipActiveSkillInitFlag(showEquipActiveSkillSlot) && Singleton<CBattleSystem>.GetInstance().FightForm != null)
							{
		...
		{
			if (this.m_bagEquipItemList.get_Item(iEquipSlotIndex))
			{
				Transform transform = this.m_bagEquipItemList.get_Item(iEquipSlotIndex).FindChild("SelectImg_InBattle");
				if (transform && transform.gameObject.activeSelf != bIsBtnShow)
				{
					transform.gameObject.CustomSetActive(bIsBtnShow);
				}
				Transform transform2 = this.m_bagEquipItemList.get_Item(iEquipSlotIndex).FindChild("BtnShowInBattle");
		...
		private void SetEquipSkillBeUsingBtnState(bool bIsBtnShow, int iEquipSlotIndex)
		{
			if (this.m_bagEquipItemList.get_Item(iEquipSlotIndex))
			{
				Transform transform = this.m_bagEquipItemList.get_Item(iEquipSlotIndex).FindChild("Beingused");
				if (transform && transform.gameObject.activeSelf != bIsBtnShow)
				{
					transform.gameObject.CustomSetActive(bIsBtnShow);
				}
		...
		{
			if (this.m_hostCtrlHero && actorObjectID == this.m_hostCtrlHero.get_handle().ObjID && this.m_hostCtrlHero.get_handle().EquipComponent != null)
			{
				ENUM_EQUIP_ACTIVESKILL_STATUS equipActiveSkillSlotInfo = this.m_hostCtrlHero.get_handle().EquipComponent.GetEquipActiveSkillSlotInfo(iEquipSlotIndex);
				SkillSlotType showingSkillSlotByEquipSlot = this.m_hostCtrlHero.get_handle().EquipComponent.GetShowingSkillSlotByEquipSlot(iEquipSlotIndex);
				...
				case ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_WHITHOUTACTIVESKILL:
					...
					if (!bIsAdd)
					{
						...
						for (int i = 0; i < 1; i++)
						{
							if (this.m_hostCtrlHero.get_handle().EquipComponent.GetEquipActiveSkillShowFlag(i + SkillSlotType.SLOT_SKILL_9))
							{
								int equipSlotBySkillSlot = this.m_hostCtrlHero.get_handle().EquipComponent.GetEquipSlotBySkillSlot(i + SkillSlotType.SLOT_SKILL_9);
				...
				case ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_ISSHOWING:
					if (this.m_hostCtrlHero.get_handle().EquipComponent.GetEquipActiveSkillInitFlag(showingSkillSlotByEquipSlot))
					{
				...
				case ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_WHITHOUTACTIVESKILL_REMOVECURSKILL:
					...
					for (int j = 0; j < 1; j++)
					{
						if (!this.m_hostCtrlHero.get_handle().EquipComponent.GetEquipActiveSkillShowFlag(j + SkillSlotType.SLOT_SKILL_9))
						{
		...
		private void UpdateEquipActiveSkillCd(int iEquipSlotIndex, bool bIsOpenForm = false)
		{
			...
			EquipComponent equipComponent = this.m_hostCtrlHero.get_handle().EquipComponent;
			...
			if (equips[iEquipSlotIndex].m_equipID > 0)
			{
				...
				if (equipActiveSkillSlotInfo == ENUM_EQUIP_ACTIVESKILL_STATUS.ENM_EQUIP_ACTIVESKILL_STATUS_ISSHOWING)
				{
					...
					SkillSlot skillSlot;
					if (this.m_hostCtrlHero.get_handle().SkillControl != null && this.m_hostCtrlHero.get_handle().SkillControl.TryGetSkillSlot(showingSkillSlotByEquipSlot, out skillSlot))
					{
