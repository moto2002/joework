...
namespace Assets.Scripts.GameSystem
{
	public class CChatSysData
	{
		...
		public void ClearEntryText()
		{
			...
			Singleton<CChatController>.get_instance().view.SetEntryChannelImage(EChatChannel.None);
			Singleton<CChatController>.get_instance().view.Clear_EntryForm_Node();
		...
		public void Add_NewContent_Entry_ColorFlag(string a, EChatChannel curChannel)
		{
			...
			int num = 0;
			List<string> list = Singleton<CChatController>.get_instance().ColorParser.Parse(lingWidth, this.entryEntity.text, Singleton<CChatController>.get_instance().view.ChatParser.viewFontSize, out num);
			if (list.get_Count() >= 1)
			{
				this.entryEntity.TextObjList.Add(new CTextImageNode(list.get_Item(0), true, (float)num, 0f, 0f, 0f));
			}
		}
