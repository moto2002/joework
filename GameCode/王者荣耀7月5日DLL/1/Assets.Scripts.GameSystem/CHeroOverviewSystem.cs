...
namespace Assets.Scripts.GameSystem
{
	...
	public class CHeroOverviewSystem : Singleton<CHeroOverviewSystem>
	{
		...
		{
			Singleton<CUIManager>.GetInstance().CloseForm(CHeroOverviewSystem.s_heroViewFormPath);
		...
		public void OnHeroView_ItemEnable(CUIEvent uiEvent)
		{
			...
			CHeroOverviewSystem.SetPveHeroItemData(uiEvent.m_srcFormScript, gameObject, this.m_heroList.get_Item(srcWidgetIndexInBelongedList));
		}
		...
		private void OnHeroInfo_Compose(CUIEvent uiEvent)
		{
			...
			cSPkg.stPkgData.get_stItemComp().wTargetType = 4;
			cSPkg.stPkgData.get_stItemComp().dwTargetID = uiEvent.m_eventParams.heroId;
		...
		public int GetHeroIndexByConfigId(uint inCfgId)
		{
			for (int i = 0; i < this.m_heroList.get_Count(); i++)
			{
				IHeroData heroData = this.m_heroList.get_Item(i);
		...
		{
			if (index >= 0 && index < this.m_heroList.get_Count())
			{
				return this.m_heroList.get_Item(index);
			}
			return null;
		}
