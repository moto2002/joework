...
namespace Assets.Scripts.GameSystem
{
	public class KillNotify
	{
		...
		{
			if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("KN_MaxCount"), ref KillNotify.max_count))
			{
				DebugHelper.Assert(false, "---killnotify 教练你配的 KN_MaxCount 好像不是整数哦， check out");
			}
			if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("KN_HideTime"), ref KillNotify.HideTime))
			{
				DebugHelper.Assert(false, "---killnotify 教练你配的 KN_HideTime 好像不是整数哦， check out");
			}
			if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("KN_Play_DeltaTime"), ref KillNotify.s_play_deltaTime))
			{
		...
		{
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().GetForm(KillNotify.s_killNotifyFormPath);
			if (cUIFormScript == null)
			{
				cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm(KillNotify.s_killNotifyFormPath, true, true);
			}
