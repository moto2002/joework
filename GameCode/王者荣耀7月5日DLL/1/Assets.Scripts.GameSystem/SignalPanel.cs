...
namespace Assets.Scripts.GameSystem
{
	public class SignalPanel
	{
		...
		public void Update()
		{
			...
			{
				int j = 0;
				while (j < this.m_signals.get_Count())
				{
					if (this.m_signals.get_Item(j).IsNeedDisposed())
					{
						this.m_signals.get_Item(j).Dispose();
						this.m_signals.RemoveAt(j);
					}
					else
					{
						this.m_signals.get_Item(j).Update(this.m_formScript, Time.deltaTime);
						j++;
					}
				}
			}
		...
		private void OnClickBattleScene(CUIEvent uievent)
		{
			if (!this.m_useSignalButton || this.m_selectedSignalID < 0)
			{
				...
				Singleton<InBattleMsgMgr>.get_instance().HideView();
				return;
			}
		...
		public void OnClickMiniMap(CUIEvent uiEvent)
		{
			...
			ActorRoot actorRoot = (hostPlayer != null) ? hostPlayer.Captain.get_handle() : null;
		...
		public void ExecCommand_SignalBtn_Position(uint senderPlayerID, byte signalID, ref VInt3 worldPos)
		{
			...
			Player player = Singleton<GamePlayerCenter>.get_instance().GetPlayer(senderPlayerID);
			int configId = player.Captain.get_handle().TheActorMeta.ConfigId;
		...
		public void ExecCommand_SignalMiniMap_Position(uint senderPlayerID, byte signalID, ref VInt3 worldPos, byte elementType)
		{
			...
			int configId = player.Captain.get_handle().TheActorMeta.ConfigId;
			this.ExecCommand(senderPlayerID, (uint)configId, (int)signalID, worldPos.x, worldPos.y, worldPos.z, (!flag) ? 0 : 1, elementType, 0u);
		}
		...
		public void ExecCommand_SignalMiniMap_Target(uint senderPlayerID, byte signalID, byte type, uint targetObjID)
		{
			...
			if (!actor || actor.get_handle().ObjID == 0u)
			{
				return;
			}
			if (!actor.get_handle().HorizonMarker.IsVisibleFor(hostPlayer.PlayerCamp))
			{
				return;
			}
			Player player = Singleton<GamePlayerCenter>.get_instance().GetPlayer(senderPlayerID);
			bool flag = hostPlayer.PlayerCamp == actor.get_handle().TheActorMeta.ActorCamp;
			int configId = actor.get_handle().TheActorMeta.ConfigId;
			int configId2 = player.Captain.get_handle().TheActorMeta.ConfigId;
			this.ExecCommand(actor, senderPlayerID, (uint)configId2, (int)signalID, (!flag) ? 0 : 1, type, 0u, (uint)configId);
		}
		...
		private PoolObjHandle<ActorRoot> GetActor(byte type, uint targetObjID)
		{
			...
			{
				list = Singleton<GameObjMgr>.get_instance().GameActors;
			}
			else if (type == 3)
			{
				list = Singleton<GameObjMgr>.get_instance().HeroActors;
			}
			else if (type == 1)
			{
				list = Singleton<GameObjMgr>.get_instance().TowerActors;
			}
			else if (type == 2)
			{
				list = Singleton<GameObjMgr>.get_instance().OrganActors;
			}
			for (int i = 0; i < list.get_Count(); i++)
			{
				PoolObjHandle<ActorRoot> poolObjHandle = list.get_Item(i);
				if (poolObjHandle && poolObjHandle.get_handle().ObjID == targetObjID)
				{
		...
		public void ExecCommand(uint senderPlayerID, uint heroID, int signalID, int worldPositionX, int worldPositionY, int worldPositionZ, byte bAlice = 0, byte elementType = 0, uint targetHeroID = 0u)
		{
			...
			bool bSmall = Singleton<CBattleSystem>.get_instance().TheMinimapSys.CurMapType() == MinimapSys.EMapType.Mini;
		...
		public void ExecCommand(PoolObjHandle<ActorRoot> followActor, uint senderPlayerID, uint heroID, int signalID, byte bAlice = 0, byte elementType = 0, uint targetObjID = 0u, uint targetHeroID = 0u)
		{
			...
			this.PlaySignalTipsSound(elementType, bAlice, targetHeroID);
			bool bUseCfgSound = elementType == 0;
			if (followActor && followActor.get_handle().Visible)
			{
		...
		private void RefreshSignalTipsList()
		{
			...
			if (this.m_signalTipsList != null)
			{
				this.m_signalTipsList.SetElementAmount(this.m_signalTipses.get_Count());
				this.m_signalTipsList.MoveElementInScrollArea(this.m_signalTipses.get_Count() - 1, false);
			}
		}
		...
		{
			if (this.m_signalTipses != null && this.m_signalTipses.get_Count() > 0)
			{
