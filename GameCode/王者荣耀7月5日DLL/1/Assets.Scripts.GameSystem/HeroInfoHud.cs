...
namespace Assets.Scripts.GameSystem
{
	public class HeroInfoHud
	{
		...
		public uint PickedHeroID
		{
			get
			{
				return (this._pickedHero == null) ? 0u : this._pickedHero.actorHero.get_handle().ObjID;
			}
		}
		...
		public void SetPickHero(HeroKDA hero)
		{
			...
			this._headImg.SetSprite(string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_BustCircle_Dir, CSkinInfo.GetHeroSkinPic((uint)this._pickedHero.actorHero.get_handle().TheActorMeta.ConfigId, 0u)), Singleton<CBattleSystem>.GetInstance().WatchFormScript, true, false, false, false);
			if (this._showBuffDesc != null)
			{
		...
		public void ValidateLevel()
		{
			...
			this._levelTxt.text = this._pickedHero.actorHero.get_handle().ValueComponent.actorSoulLevel.ToString();
		}
		...
		public void ValidateHp()
		{
			...
			int actorHp = this._pickedHero.actorHero.get_handle().ValueComponent.actorHp;
			int totalValue = this._pickedHero.actorHero.get_handle().ValueComponent.mActorValue[5].totalValue;
		...
		public void ValidateEp()
		{
			...
			int actorEp = this._pickedHero.actorHero.get_handle().ValueComponent.actorEp;
			int totalValue = this._pickedHero.actorHero.get_handle().ValueComponent.mActorValue[32].totalValue;
		...
		public void ValidateAd()
		{
			...
			int totalValue = this._pickedHero.actorHero.get_handle().ValueComponent.mActorValue[1].totalValue;
		...
		public void ValidateAp()
		{
			...
			int totalValue = this._pickedHero.actorHero.get_handle().ValueComponent.mActorValue[2].totalValue;
		...
		public void ValidatePhyDef()
		{
			...
			int totalValue = this._pickedHero.actorHero.get_handle().ValueComponent.mActorValue[3].totalValue;
		...
		public void ValidateMgcDef()
		{
			...
			int totalValue = this._pickedHero.actorHero.get_handle().ValueComponent.mActorValue[4].totalValue;
		...
		{
			stEquipInfo[] equips = this._pickedHero.actorHero.get_handle().EquipComponent.GetEquips();
			for (int i = 0; i < 6; i++)
			{
		...
		{
			if (this._pickedHero != null && this._pickedHero.actorHero && this._pickedHero.actorHero.get_handle().ActorControl.IsDeadState)
			{
				this._reviveTxt.text = string.Format("{0}", Mathf.FloorToInt((float)this._pickedHero.actorHero.get_handle().ActorControl.ReviveCooldown * 0.001f));
