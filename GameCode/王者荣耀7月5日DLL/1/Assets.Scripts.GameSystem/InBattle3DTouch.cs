...
namespace Assets.Scripts.GameSystem
{
	public class InBattle3DTouch
	{
		...
		{
			CUIFormScript form = Singleton<CUIManager>.get_instance().GetForm(CBattleEquipSystem.s_equipFormPath);
			if (form != null && (!form.IsHided() || form.GetComponent<Canvas>().enabled))
			{
				Singleton<CUIManager>.get_instance().CloseForm(form);
				return true;
			}
			CUIFormScript form2 = Singleton<CUIManager>.get_instance().GetForm(CSettingsSys.SETTING_FORM);
			if (form2 != null && !form2.IsHided())
			{
				Singleton<CUIManager>.get_instance().CloseForm(form2);
				return true;
			}
			CUIFormScript form3 = Singleton<CUIManager>.get_instance().GetForm(BattleStatView.s_battleStateViewUIForm);
			if (form3 != null && (!form3.IsHided() || form3.GetComponent<Canvas>().enabled) && Singleton<CBattleSystem>.get_instance().BattleStatView != null)
			{
				Singleton<CBattleSystem>.get_instance().BattleStatView.Hide();
				return true;
			}
			return false;
		}
		...
		public void Process_MiniMap()
		{
			...
			{
				Singleton<CUIEventManager>.get_instance().DispatchUIEvent(enUIEventID.BigMap_Open_BigMap);
			}
		}

		public void Process_InBattleShortCutMsg()
		{
			if (Singleton<InBattleMsgMgr>.get_instance().m_shortcutChat != null)
			{
				Singleton<InBattleMsgMgr>.get_instance().m_shortcutChat.Send_Config_Chat(0);
			}
		}
	}
}
