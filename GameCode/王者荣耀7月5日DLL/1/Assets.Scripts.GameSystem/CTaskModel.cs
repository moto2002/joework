	...
	public class CTaskModel
	{
		...
		public LevelRewardData curLevelRewardData
		{
			...
			set
			{
				...
				CTaskView taskView = Singleton<CTaskSys>.get_instance().m_taskView;
				if (taskView != null)
				{
				...
				{
					taskView.MoveElementInScrollArea(Singleton<CTaskSys>.get_instance().model.GetLevelIndex(this.m_curLevelRewardData.m_level));
				}
			}
		}
		...
		{
			DebugHelper.Assert(index < this.m_levelRewardDataMap.get_Count(), "CTaskModel GetLevelRewardData_ByIndex, index > count, check out...");
			if (index < this.m_levelRewardDataMap.get_Count())
			{
				return this.m_levelRewardDataMap.get_Item(index);
			}
			return null;
		}
