		...
		public void Create(ushort rootEquipID, Dictionary<ushort, CEquipInfo> equipInfoDictionary)
		{
			...
			if (equipInfoDictionary.TryGetValue(rootEquipID, ref cEquipInfo))
			{
				...
				for (int i = 0; i < cEquipInfo.m_resEquipInBattle.PreEquipID.Length; i++)
				{
					...
					if (num > 0 && equipInfoDictionary.TryGetValue(num, ref cEquipInfo2) && cEquipInfo2.m_resEquipInBattle.bInvalid == 0)
					{
						...
						for (int j = 0; j < cEquipInfo2.m_resEquipInBattle.PreEquipID.Length; j++)
						{
							...
							if (num2 > 0 && equipInfoDictionary.TryGetValue(num2, ref cEquipInfo3) && cEquipInfo3.m_resEquipInBattle.bInvalid == 0)
							{
