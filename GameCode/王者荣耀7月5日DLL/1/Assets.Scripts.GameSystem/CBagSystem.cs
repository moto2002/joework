...
namespace Assets.Scripts.GameSystem
{
	...
	public class CBagSystem : Singleton<CBagSystem>
	{
		...
		private CUseableContainer GetContainerBySelectType(CUseableContainer allContainer)
		{
			...
			if (this.m_selectUseableType == enItemMenuType.All)
			{
				...
				for (int i = 0; i < curUseableCount; i++)
				{
					...
					if (useableByIndex.m_type != 5 || useableByIndex.GetSalableCount() > 0)
					{
			...
			else if (this.m_selectUseableType == enItemMenuType.RecentGet)
			{
				...
				for (int j = 0; j < curUseableCount; j++)
				{
					...
					if ((dateTime.get_Date() - Utility.ToUtcTime2Local((long)useableByIndex.m_getTime).get_Date()).get_Days() <= 3)
					{
						if (useableByIndex.m_type != 5 || useableByIndex.GetSalableCount() > 0)
						{
		...
		{
			int result = -1;
			CUseable useableByBaseID = this.m_ContainerAll.GetUseableByBaseID(2, inItemId);
		...
		private void RefreshSelectItem(CUIFormScript formScript, GameObject root, int selectIndex)
		{
			...
			if (this.m_selectUseable.m_type == 5 && ((CSymbolItem)this.m_selectUseable).IsGuildSymbol())
			{
				component2.alpha = 0f;
			}
			else
		...
		private void OnBag_SaleForm_ConfirmSale(CUIEvent uiEvent)
		{
			...
			if (this.m_selectUseable.m_type == 5)
			{
		...
		private void OnSecurePwdConfirmSale(CUIEvent uiEvent)
		{
			...
			CBagSystem.SendItemSaleMsg(listView, uiEvent.m_eventParams.pwd);
		}

		private void OnBag_OpenUseForm(CUIEvent uiEvent)
		{
			if (this.m_selectUseable.m_type == 2)
			{
				CItem cItem = this.m_selectUseable as CItem;
				if (cItem.m_itemData.bType == 4 && cItem.m_itemData.EftParam[2] != 0f)
				{
			...
			else if (this.m_selectUseable.m_type == 5)
			{
				if (Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(8))
				{
		...
		private void OnTips_ItemSourceElementClick(CUIEvent uiEvent)
		{
			...
			if (itemGetInfoParams.isCanDo)
			{
				...
				{
					RES_SHOP_TYPE tag = uiEvent.m_eventParams.tag;
		...
		private CUseableContainer GetAutoSaleUseableContainer(ref uint salePrice)
		{
			...
			for (int i = 0; i < curUseableCount; i++)
			{
				...
				if (useableByIndex.m_type == 2 && useableByIndex.m_isSale > 0 && cUseableContainer.GetCurUseableCount() < 5)
				{
					CItem cItem = useableByIndex as CItem;
		...
		private void CheckOpenAutoSaleForm()
		{
			...
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm(CBagSystem.s_bagAutoSaleFormPath, false, true);
			if (cUIFormScript == null)
			{
				return;
			}
		...
		public static ListView<IHeroData> GetExpCardHeroList()
		{
			...
			{
				uint dwCfgID = allHeroList.get_Item(i).dwCfgID;
		...
		public static void SendItemSaleMsg(ListView<CSDT_ITEM_DELINFO> itemList, string pwd = "")
		{
			...
			cSDT_ITEM_DELLIST.wItemCnt = (ushort)itemList.get_Count();
			...
			StringHelper.StringToUTF8Bytes(pwd, ref cSPkg.stPkgData.get_stItemSale().szPswdInfo);
		...
		public static void SendItemUseMsgToServer(ulong itemID, uint heroID = 0u, int useCount = 0, string pwd = "")
		{
			...
			cSPkg.stPkgData.get_stPropUse().ullUniqueID = itemID;
			cSPkg.stPkgData.get_stPropUse().dwHeroID = heroID;
			cSPkg.stPkgData.get_stPropUse().wItemCnt = (ushort)useCount;
			StringHelper.StringToUTF8Bytes(pwd, ref cSPkg.stPkgData.get_stPropUse().szPswdInfo);
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		{
			COMDT_ITEM_ADDLIST stAddList = msg.stPkgData.get_stItemAdd().stAddList;
			...
			for (int i = 0; i < (int)stAddList.wItemCnt; i++)
			{
				...
				{
					iCount = cOMDT_ITEM_ADDINFO.stItemInfo.get_stPropInfo().iOverCnt;
				}
				else if (cOMDT_ITEM_ADDINFO.wItemType == 3)
				{
					iCount = cOMDT_ITEM_ADDINFO.stItemInfo.get_stEquipInfo().iOverCnt;
				}
				else if (cOMDT_ITEM_ADDINFO.wItemType == 5)
				{
					iCount = cOMDT_ITEM_ADDINFO.stItemInfo.get_stSymbolInfo().iOverCnt;
				}
				CUseable cUseable = useableContainer.Add(cOMDT_ITEM_ADDINFO.wItemType, cOMDT_ITEM_ADDINFO.ullUniqueID, cOMDT_ITEM_ADDINFO.dwItemID, iCount, useableContainer.GetMaxAddTime());
				if (cUseable != null)
				{
					if (cUseable != null && cUseable.m_stackCount >= cUseable.m_stackMax && (cUseable.m_type != 5 || !((CSymbolItem)cUseable).IsGuildSymbol()))
					{
			...
			switch (msg.stPkgData.get_stItemAdd().bAddReason)
			{
			}
			Singleton<CBagSystem>.GetInstance().RefreshBagForm();
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.BAG_ITEMS_UPDATE);
		}

		[MessageHandler(1103)]
		public static void ReciveItemDel(CSPkg msg)
		{
			CSDT_ITEM_DELLIST stDelList = msg.stPkgData.get_stItemDel().stDelList;
			CUseableContainer useableContainer = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().GetUseableContainer(enCONTAINER_TYPE.ITEM);
			...
			switch (msg.stPkgData.get_stItemDel().bDelReason)
			{
			case 3:
				...
				Singleton<CUIManager>.GetInstance().CloseForm(CBagSystem.s_bagSaleFormPath);
				Singleton<EventRouter>.get_instance().BroadCastEvent<CSDT_ITEM_DELLIST>(EventID.BAG_ITEM_SALED, stDelList);
		...
		public static void ReciveItemList(CSPkg msg)
		{
			...
			CUseableContainer useableContainer = masterRoleInfo.GetUseableContainer(enCONTAINER_TYPE.ITEM);
			int iCount = 0;
			for (int i = 0; i < (int)msg.stPkgData.get_stPkgDetail().stPkgInfo.wItemCnt; i++)
			{
				COMDT_ITEM_POSINFO cOMDT_ITEM_POSINFO = msg.stPkgData.get_stPkgDetail().stPkgInfo.astItemList[i];
				if (cOMDT_ITEM_POSINFO.wItemType == 2)
				{
					iCount = cOMDT_ITEM_POSINFO.stItemInfo.get_stPropInfo().iOverCnt;
				}
				else if (cOMDT_ITEM_POSINFO.wItemType == 3)
				{
					iCount = cOMDT_ITEM_POSINFO.stItemInfo.get_stEquipInfo().iOverCnt;
				}
				else if (cOMDT_ITEM_POSINFO.wItemType == 5)
				{
					iCount = cOMDT_ITEM_POSINFO.stItemInfo.get_stSymbolInfo().iOverCnt;
				}
				useableContainer.Add(cOMDT_ITEM_POSINFO.wItemType, cOMDT_ITEM_POSINFO.ullUniqueID, cOMDT_ITEM_POSINFO.dwItemID, iCount, cOMDT_ITEM_POSINFO.iAddUpdTime);
			}
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.BAG_ITEMS_UPDATE);
		}
		...
		{
			uint dwPropID = msg.stPkgData.get_stPropUseRsp().dwPropID;
			CItem cItem = CUseableManager.CreateUseable(2, dwPropID, 0) as CItem;
			if (cItem == null)
			{
				return;
			}
			RES_PROP_TYPE_TYPE bType = cItem.m_itemData.bType;
			if (bType == 11)
			{
