...
namespace Assets.Scripts.GameSystem
{
	public class UT
	{
		...
		{
			return Singleton<CTextManager>.get_instance().GetText(key);
		}
		...
		public static bool IsFreeze(uint lastTime)
		{
			...
			DateTime dateTime2 = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
			return DateTime.Compare(dateTime2, dateTime) > 0 && (dateTime2 - dateTime).get_Days() >= Singleton<CFriendContoller>.get_instance().model.freezeDayCount;
		}
		...
		public static void CheckGPS()
		{
			...
			if (enableShareLocation && !MonoSingleton<GPSSys>.get_instance().bGetGPSData)
			{
