...
namespace Assets.Scripts.GameSystem
{
	internal class CFakePvPHelper
	{
		...
		private static List<CFakePvPHelper.FakePlayerConfirm> SelectConfirmPlayer(int time)
		{
			...
			{
				if (!CFakePvPHelper.FakePlayerList.get_Item(i).bConfirmed)
				{
					if (CFakePvPHelper.FakePlayerList.get_Item(i).confirmWaitTime == time)
					{
						list.Add(CFakePvPHelper.FakePlayerList.get_Item(i));
					}
				}
			}
			return list;
		...
		private static void GotoHeroSelectPage()
		{
			...
			Singleton<CHeroSelectBaseSystem>.get_instance().OpenForm(enSelectGameType.enPVE_Computer, pvpMapCommonInfo.bHeroNum, roomInfo.roomAttrib.dwMapId, roomInfo.roomAttrib.bMapType, (int)pvpMapCommonInfo.bIsAllowHeroDup);
			Singleton<CTimerManager>.GetInstance().RemoveTimer(new CTimer.OnTimeUpHandler(CFakePvPHelper.OnConfirmTimout));
		}
		...
		private static CFakePvPHelper.FakeHeroSelect GetFakeHeroSelect(ulong playerUid)
		{
			...
			for (int i = 0; i < CFakePvPHelper.FakeHeroSelectList.get_Count(); i++)
			{
				if (CFakePvPHelper.FakeHeroSelectList.get_Item(i).FakePlayer.ullUid == playerUid)
				{
					result = CFakePvPHelper.FakeHeroSelectList.get_Item(i);
					break;
				}
			}
			return result;
		...
		public static void FakeLoadProcess(float progress)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current().Computer)
				{
					...
					GameObject gameObject = (enumerator.get_Current().PlayerCamp != 1) ? form.gameObject.transform.FindChild("DownPanel").FindChild(string.Format("Down_Player{0}", num)).gameObject : form.gameObject.transform.FindChild("UpPanel").FindChild(string.Format("Up_Player{0}", num)).gameObject;
					if (gameObject != null)
					{
