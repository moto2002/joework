...
namespace Assets.Scripts.GameSystem
{
	...
	public class CUnionBattleEntrySystem : Singleton<CUnionBattleEntrySystem>
	{
		...
		private const RES_BATTLE_MAP_TYPE m_ResMapType = 5;
		...
		private void Open_BattleEntry(CUIEvent uiEvt)
		{
			...
			{
				Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips2", true, 1.5f, null, new object[0]);
				return;
			}
			if (Singleton<CMatchingSystem>.get_instance().IsInMatching)
			{
		...
		private void Open_SecondBattleEntry(CUIEvent uiEvt)
		{
			...
			if (tag != 0)
			{
				if (tag == 1)
				{
					Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips3", true, 1.5f, null, new object[0]);
				}
			}
			else
			{
		...
		private void OnClickSecondUIBattleEntry(CUIEvent uiEvt)
		{
			...
			GameDataMgr.matchTimeInfoDict.TryGetValue(GameDataMgr.GetDoubleKey(5u, this.m_selectMapID), ref this.m_selectTimeInfo);
			...
			case 1:
				...
				{
					Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips15", true, 1.5f, null, new object[0]);
					return;
				}
		...
		private void OnClickStartMatch(CUIEvent uiEvt)
		{
			...
			stMatchOpenInfo matchOpenState = CUICommonSystem.GetMatchOpenState(5, this.m_selectMapID);
			...
			{
				int num = useableContainer.GetUseableStackCount(2, this.m_selectMapRes.dwConsumPayItemID) + useableContainer.GetUseableStackCount(2, this.m_selectMapRes.dwConsumFreeItemID);
				...
				else
				{
					...
					CUseable cUseable = CUseableManager.CreateUseable(2, this.m_selectMapRes.dwConsumPayItemID, num3);
					if (cUseable != null)
					{
						int payValue = (int)(cUseable.GetBuyPrice(this.m_selectMapRes.bCoinType) * (uint)num3);
			...
			{
				Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips4", true, 1.5f, null, new object[0]);
			}
			else if ((uint)this.m_selectStateInfo.bMatchCnt >= this.maxMatchRoundCnt)
			{
				Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips17", true, 1.5f, null, new object[0]);
			}
		}
		...
		private void OnBuyTiketClick(CUIEvent uiEvt)
		{
			...
			BuyPickDialog.Show(2, eventParams.tagUInt, eventParams.tag, 10000f, 99u, null, null, new BuyPickDialog.OnConfirmBuyCommonDelegate(this.OnBuyPickDialogConfirm), cUIEvent);
		}
		...
		private void OnBuyPickDialogConfirm(CUIEvent uiEvent, uint count)
		{
			...
			CUseable cUseable = CUseableManager.CreateUseable(2, tagUInt, num);
			if (cUseable != null)
			{
				int payValue = (int)(cUseable.GetBuyPrice(tag) * (uint)num);
		...
		private void initFirstFormWidget()
		{
			...
			{
				string text = Singleton<CTextManager>.get_instance().GetText("Union_Battle_Tips12", new string[]
				{
		...
		private void initSecondFormWidget()
		{
			...
			uint.TryParse(Singleton<CTextManager>.get_instance().GetText("MapID_Union_1"), ref array[0]);
			uint.TryParse(Singleton<CTextManager>.get_instance().GetText("MapID_Union_2"), ref array[1]);
			array2[0] = CUICommonSystem.IsMatchOpened(5, array[0]);
			array2[1] = CUICommonSystem.IsMatchOpened(5, array[1]);
			widget.GetComponent<CUIMiniEventScript>().m_onClickEventParams.tag = 0;
			widget2.GetComponent<CUIMiniEventScript>().m_onClickEventParams.tag = 1;
		...
		private void initConfirmFormWidget()
		{
			...
			CUseableContainer useableContainer = masterRoleInfo.GetUseableContainer(enCONTAINER_TYPE.ITEM);
			int useableStackCount = useableContainer.GetUseableStackCount(2, this.m_selectMapRes.dwConsumPayItemID);
			CUseable itemUseable = CUseableManager.CreateUseable(2, this.m_selectMapRes.dwConsumPayItemID, useableStackCount);
		...
		private void initThirdFormWidget()
		{
			...
			GameDataMgr.matchTimeInfoDict.TryGetValue(GameDataMgr.GetDoubleKey(5u, this.m_selectMapID), ref resRewardMatchTimeInfo);
		...
		private void refreshOneRewardElement(CUIFormScript form, GameObject element, int index)
		{
			...
			CUseable itemUseable = CUseableManager.CreateUsableByServerType(resRewardInfo.bRewardType, (int)resRewardInfo.dwRewardNum, resRewardInfo.dwRewardID);
			...
			for (int i = 0; i < dataByKey3.astRewardDetail.Length; i++)
			{
				...
				listView.Add(CUseableManager.CreateUsableByRandowReward(resDT_RandomRewardInfo.bItemType, (int)resDT_RandomRewardInfo.dwLowCnt, resDT_RandomRewardInfo.dwItemID));
			}
			component2.SetElementAmount(listView.get_Count());
			int count = listView.get_Count();
			for (int j = 0; j < count; j++)
			{
				CUIListElementScript elemenet = component2.GetElemenet(j);
				CUICommonSystem.SetItemCell(form, elemenet.gameObject, listView.get_Item(j), true, false, false, false);
		...
		{
			CUICommonSystem.JumpForm(10, 0, 0);
		}
		...
		private void ShowReward(COMDT_REWARDMATCH_RECORD matchState)
		{
			...
			for (int i = 0; i < dataByKey.astRewardItem.Length; i++)
			{
				...
				if (dataByKey.astRewardItem[i].bRewardType != 20 || (Singleton<HeadIconSys>.GetInstance().GetInfo(dataByKey.astRewardItem[i].dwRewardID) == null && !CSysDynamicBlock.bSocialBlocked))
				{
					...
					listView.Add(CUseableManager.CreateUsableByServerType(resRewardInfo.bRewardType, (int)resRewardInfo.dwRewardNum, resRewardInfo.dwRewardID));
				}
			}
		...
		private void ShowCountDownTime(GameObject Btn, bool bMapOpened)
		{
			...
			if (transform2 != null)
			{
				...
				else
				{
					int num = 0;
					uint num2 = 0u;
					CUICommonSystem.GetTimeUtilOpen(5, tagUInt, out num2, out num);
					if (num == 0 && num2 == 0u)
					{
						gameObject2.CustomSetActive(false);
						gameObject.CustomSetActive(true);
		...
		{
			return !Singleton<CFunctionUnlockSys>.GetInstance().FucIsUnlock(27);
		}
		...
		public static bool HasMatchInActiveTime()
		{
			...
			for (int i = 0; i < unionBattleMapCount; i++)
			{
				if (CUICommonSystem.GetMatchOpenState(5, CUnionBattleEntrySystem.GetUnionBattleMapInfoByIndex(i).dwMapId).matchState == enMatchOpenState.enMatchOpen_InActiveTime)
				{
					result = true;
					break;
				}
		...
		{
			CUnionBattleEntrySystem.stLastWinLoseCntChginfo[] winLoseChgInfo = Singleton<CUnionBattleEntrySystem>.get_instance().m_WinLoseChgInfo;
		...
		{
			CMatchingSystem.ReqStartSingleMatching(this.m_selectMapID, false, 5);
		}

		public static void SendAwartPoolReq(uint mapID)
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(5101u);
			cSPkg.stPkgData.get_stGetAwardPoolReq().dwMapId = mapID;
		...
		public static void SendBuyTicketRequest(uint mapID, uint itemNum)
		{
			...
			cSPkg.stPkgData.get_stBuyMatchTicketReq().dwMapId = mapID;
			cSPkg.stPkgData.get_stBuyMatchTicketReq().dwNum = itemNum;
		...
		public static void SendChgMatchStateReq(uint mapId, byte bState)
		{
			...
			cSPkg.stPkgData.get_stRewardMatchStateChgReq().dwMapId = mapId;
			cSPkg.stPkgData.get_stRewardMatchStateChgReq().bState = bState;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void ReciveAwardPoolInfo(CSPkg msg)
		{
			...
			Singleton<CUnionBattleEntrySystem>.get_instance().m_awardPoolInfo = msg.stPkgData.get_stGetAwardPoolRsp();
		}
		...
		{
			Singleton<CUIManager>.get_instance().CloseSendMsgAlert();
			Singleton<CUnionBattleEntrySystem>.get_instance().m_personInfo = msg.stPkgData.get_stMatchPointNtf();
		}
		...
		{
			Singleton<CUIManager>.get_instance().CloseSendMsgAlert();
			if (msg.stPkgData.get_stBuyMatchTicketRsp().iResult == 0)
			{
				Singleton<CUnionBattleEntrySystem>.get_instance().initConfirmFormWidget();
				Singleton<CUnionBattleEntrySystem>.get_instance().initTirdFormTicket();
				Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips14", true, 1.5f, null, new object[0]);
			}
			else if (msg.stPkgData.get_stBuyMatchTicketRsp().iResult == 1)
			{
				Singleton<CUIManager>.get_instance().OpenTips("Union_Battle_Tips13", true, 1.5f, null, new object[0]);
			}
		}
		...
		{
			Singleton<CUIManager>.get_instance().CloseSendMsgAlert();
			Singleton<CUnionBattleEntrySystem>.get_instance().m_baseInfo = msg.stPkgData.get_stGetMatchInfoRsp();
			Singleton<CUnionBattleEntrySystem>.get_instance().initFirstFormWidget();
		}
		...
		{
			Singleton<CUIManager>.get_instance().CloseSendMsgAlert();
			Singleton<CUnionBattleEntrySystem>.get_instance().m_stateInfo = msg.stPkgData.get_stGetRewardMatchInfoRsp().stRewardMatchInfo;
		}

		[MessageHandler(5111)]
		public static void UnionBattleStateChg(CSPkg msg)
		{
			COMDT_REWARDMATCH_RECORD stChgInfo = msg.stPkgData.get_stRewardMatchInfoChgNtf().stChgInfo;
			COMDT_REWARDMATCH_DATA stateInfo = Singleton<CUnionBattleEntrySystem>.get_instance().m_stateInfo;
			for (int i = 0; i < (int)stateInfo.bRecordCnt; i++)
			{
				if (stateInfo.astRecord[i].dwMapId == stChgInfo.dwMapId)
				{
					...
					{
						Singleton<CUnionBattleEntrySystem>.get_instance().StartARoundRewardMatch();
					}
					else if (stateInfo.astRecord[i].bState == 2 && stChgInfo.bState == 0)
					{
						Singleton<CUnionBattleEntrySystem>.get_instance().ShowReward(stChgInfo);
					}
