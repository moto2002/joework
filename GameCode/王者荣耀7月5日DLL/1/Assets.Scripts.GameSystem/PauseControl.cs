...
namespace Assets.Scripts.GameSystem
{
	...
	public class PauseControl
	{
		...
		public void RequestPause(bool isPause)
		{
			if (isPause)
			{
				if (this.CanPause)
				{
					...
					cSPkg.stPkgData.get_stPauseReq().bType = 1;
			...
			else
			{
				...
				cSPkg2.stPkgData.get_stPauseReq().bType = 2;
		...
		{
			Singleton<CBattleSystem>.GetInstance().pauseControl.ResponsePause(msg.stPkgData.get_stPauseRsp());
		}
		...
		public void ExecPauseCommand(byte pauseCommand, byte pauseByCamp, byte[] campPauseTimes)
		{
			...
			if (this._root)
			{
				...
				if (flag)
				{
					...
					COM_PLAYERCAMP cOM_PLAYERCAMP = (!Singleton<WatchController>.GetInstance().IsWatching) ? Singleton<GamePlayerCenter>.GetInstance().hostPlayerCamp : 0;
					this._pauseCampTxt.text = Singleton<CTextManager>.GetInstance().GetText(string.Format("PauseTips_{0}Look{1}", cOM_PLAYERCAMP, (int)pauseByCamp));
					this._passedSecond = 0;
					if (pauseByCamp == cOM_PLAYERCAMP)
					{
					...
					if (cOM_PLAYERCAMP != null)
					{
						...
						this._remainTimesTxt.text = (num - (int)campPauseTimes[cOM_PLAYERCAMP]).ToString();
					}
					else
					{
