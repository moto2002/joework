...
namespace Assets.Scripts.GameSystem
{
	...
	public class CPaySystem : Singleton<CPaySystem>
	{
		...
		private void OnDisplayChaoZhiGift(CUIEvent uiEvent)
		{
			...
			transform.gameObject.GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("GotoTehuiShopTitle");
		}
		...
		{
			CPaySystem.SendReqGetDianQuanReward(1);
		}

		private void OnGetRenewalReward(CUIEvent uiEvent)
		{
			CPaySystem.SendReqGetDianQuanReward(2);
		}
		...
		{
			if (Singleton<CUIManager>.get_instance().GetForm(CRoomSystem.PATH_ROOM) == null && Singleton<CUIManager>.get_instance().GetForm(CMatchingSystem.PATH_MATCHING_MULTI) == null)
			{
		...
		private uint GetDianQuanGiftCnt(int giftId)
		{
			uint result = 0u;
			giftId--;
			if (giftId >= 0 && giftId < this.dianQuanBuyInfoList.get_Count())
			{
				ResCouponsBuyInfo resCouponsBuyInfo = this.dianQuanBuyInfoList.get_Item(giftId);
		...
		public void RefreshBuyDianQuanPanel()
		{
			...
			component.SetElementAmount(this.dianQuanBuyInfoList.get_Count());
			for (int i = 0; i < this.dianQuanBuyInfoList.get_Count(); i++)
			{
				ResCouponsBuyInfo resCouponsBuyInfo = this.dianQuanBuyInfoList.get_Item(i);
		...
		private void RefreshRenewalPanel()
		{
			...
			CUseable itemUseable = CUseableManager.CreateUsableByRandowReward(dataByKey.astRewardDetail[0].bItemType, (int)dataByKey.astRewardDetail[0].dwLowCnt, dataByKey.astRewardDetail[0].dwItemID);
		...
		private void RefreshRewardList(CUIFormScript form, CUIListScript listScript, ref ResDT_RandomRewardInfo[] rewardInfoArr, int index = 0)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				CUseable itemUseable = CUseableManager.CreateUsableByRandowReward(rewardInfoArr[j + index].bItemType, (int)rewardInfoArr[j + index].dwLowCnt, rewardInfoArr[j + index].dwItemID);
		...
		{
			if (this.rewardItems != null && this.rewardItems.get_Count() > 0)
			{
		...
		public static void SendReqQueryAcntDianQuan(CS_COUPONS_PAYTYPE payType, bool isAlert = true)
		{
			...
			cSPkg.stPkgData.get_stAcntCouponsReq().bPayType = payType;
		...
		public static void SendReqGetDianQuanReward(CS_COUPONS_PAYTYPE payType)
		{
			...
			cSPkg.stPkgData.get_stCouponsRewardReq().bPayType = payType;
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			SCPKG_NTF_NEWIEBITSYN stNewieBitSyn = msg.stPkgData.get_stNewieBitSyn();
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
