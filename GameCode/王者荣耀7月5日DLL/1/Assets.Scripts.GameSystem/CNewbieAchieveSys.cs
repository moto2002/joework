...
namespace Assets.Scripts.GameSystem
{
	internal class CNewbieAchieveSys : Singleton<CNewbieAchieveSys>
	{
		...
		private void onActorDead(ref GameDeadEventParam param)
		{
			...
			{
				if (param.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
				{
					MonsterWrapper monsterWrapper = param.src.get_handle().AsMonster();
					DebugHelper.Assert(monsterWrapper != null && monsterWrapper.cfgInfo != null, "Can't find Monster config -- ID: {0}", new object[]
					{
						param.src.get_handle().TheActorMeta.ConfigId
					});
				...
				else if (param.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)
				{
					ResOrganCfgInfo dataCfgInfoByCurLevelDiff = OrganDataHelper.GetDataCfgInfoByCurLevelDiff(param.src.get_handle().TheActorMeta.ConfigId);
			...
			if (param.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && param.atker)
			{
				...
				else if (param.atker.get_handle().TheActorMeta.ActorType != ActorTypeDef.Actor_Type_Hero)
				{
					HeroWrapper heroWrapper = param.src.get_handle().ActorControl as HeroWrapper;
					if (heroWrapper != null)
					{
		...
		private void onMonsterGroupDead(ref GameDeadEventParam param)
		{
			...
			if (param.src)
			{
				...
				if (param.atker && ActorHelper.IsHostActor(ref param.atker))
				{
					if (Singleton<BattleLogic>.GetInstance().DragonId != 0 && param.src.get_handle().TheActorMeta.ConfigId == Singleton<BattleLogic>.GetInstance().DragonId)
					{
		...
		{
			if (MonoSingleton<Reconnection>.get_instance().isProcessingRelayRecover || Singleton<WatchController>.GetInstance().IsWatching)
			{
				return;
			}
