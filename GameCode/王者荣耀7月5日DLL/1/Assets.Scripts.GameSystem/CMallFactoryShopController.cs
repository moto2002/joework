	...
	public class CMallFactoryShopController : Singleton<CMallFactoryShopController>
	{
		public class ShopProduct
		{
				...
				get
				{
					...
					if (this._config.dwSpecSaleType == 2u)
					{
						...
						if (dataByKey.bType == 4)
						{
							...
							{
								this._propGiftUseType = 1;
							}
							else
							{
								this._propGiftUseType = 2;
							}
						}
						else
						{
			...
			public bool IsPropGiftUseImm
			{
				get
				{
					if (this.propGiftSet)
					{
						return this._propGiftUseType == 1;
					}
					return this.IsPropGift && this._propGiftUseType == 1;
				}
			}
			...
			public COM_ITEM_TYPE Type
			{
				get
				{
					return this._config.dwSpecSaleType;
				}
			}
			...
			private void InitRandomReawrd()
			{
				if (this._config.dwSpecSaleType == 2u)
				{
					...
					if (dataByKey.bType == 4)
					{
						...
						bool flag = dataByKey.EftParam[3] != 1f;
						if (num > 0 && flag)
						{
							...
							if (dataByKey2 != null)
							{
								for (int i = 0; i < dataByKey2.astRewardDetail.Length; i++)
								{
									...
									{
										ListView<CUseable> listView = CUseableManager.CreateUsableListByRandowReward(dataByKey2.astRewardDetail[i].bItemType, (int)dataByKey2.astRewardDetail[i].dwLowCnt, dataByKey2.astRewardDetail[i].dwItemID);
			...
			public string isHaveHeroSkin()
			{
				...
				{
					uint baseID = this._rewardList.get_Item(i).m_baseID;
					if (this._rewardList.get_Item(i).m_type == 4)
					{
						if (masterRoleInfo.IsHaveHero(baseID, false))
						{
							if (text != string.Empty)
							{
								text = text + "," + this._rewardList.get_Item(i).m_name;
							}
							else
							{
								text += this._rewardList.get_Item(i).m_name;
							}
						}
					}
					else if (this._rewardList.get_Item(i).m_type == 7)
					{
		...
		private class ShopProductWidget
		{
			...
			public void Validate()
			{
				...
				RES_LUCKYDRAW_ITEMTAG tag = this.data.Tag;
		...
		public override void UnInit()
		{
			...
			if (this._spListInTab != null)
			{
				...
				{
					KeyValuePair<uint, ListView<CMallFactoryShopController.ShopProduct>> current = enumerator.get_Current();
		...
		{
			bool result = false;
			if (this._filteredSpList != null && this._filteredSpList.get_Count() != 0)
			{
				for (int i = this._filteredSpList.get_Count() - 1; i >= 0; i--)
				{
					if (this.FilterProduct(this._filteredSpList.get_Item(i)) == 1)
					{
						result = true;
					}
				}
		...
		{
			bool result = false;
			if (this._spList != null && this._spList.get_Count() != 0)
			{
				for (int i = this._spList.get_Count() - 1; i >= 0; i--)
				{
					if (this.FilterProduct(this._spList.get_Item(i)) != 1)
					{
						result = true;
					}
				}
		...
		private int FilterProduct(CMallFactoryShopController.ShopProduct sp)
		{
			...
			case 2:
				...
				if (this._spDict.ContainsKey(sp.Key))
				{
					for (int i = this._spList.get_Count() - 1; i >= 0; i--)
					{
						if (this._spList.get_Item(i) != null && this._spList.get_Item(i).Key == sp.Key)
						{
				...
				{
					for (int j = this._spListInTab.get_Item(sp.Tab).get_Count() - 1; j >= 0; j--)
					{
						if (this._spListInTab.get_Item(sp.Tab).get_Item(j) != null && this._spListInTab.get_Item(sp.Tab).get_Item(j).Key == sp.Key)
						{
							this._spListInTab.get_Item(sp.Tab).RemoveAt(j);
							break;
						}
					}
					if (this._spListInTab.get_Item(sp.Tab).get_Count() == 0)
					{
			...
			case 1:
				if (this._spDict.ContainsKey(sp.Key))
				{
					for (int k = this._spList.get_Count() - 1; k >= 0; k--)
					{
						if (this._spList.get_Item(k) != null && this._spList.get_Item(k).Key == sp.Key)
						{
				...
				if (this._spListInTab.ContainsKey(sp.Tab))
				{
					for (int l = this._spListInTab.get_Item(sp.Tab).get_Count() - 1; l >= 0; l--)
					{
						if (this._spListInTab.get_Item(sp.Tab).get_Item(l) != null && this._spListInTab.get_Item(sp.Tab).get_Item(l).Key == sp.Key)
						{
							this._spListInTab.get_Item(sp.Tab).RemoveAt(l);
							break;
						}
					}
					if (this._spListInTab.get_Item(sp.Tab).get_Count() == 0)
			...
			case 4:
				if (!this._spDict.ContainsKey(sp.Key))
				{
					...
					this._spListInTab.get_Item(sp.Tab).Add(sp);
				}
				if (this._filteredSpDict.ContainsKey(sp.Key))
				{
					for (int m = this._filteredSpList.get_Count() - 1; m >= 0; m--)
					{
						if (this._filteredSpList.get_Item(m) != null && this._filteredSpList.get_Item(m).Key == sp.Key)
						{
		...
		public void InitLocalData()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, ResSpecSale> current = enumerator.get_Current();
				ResSpecSale value = current.get_Value();
				if (value != null)
				{
					CUseable cUseable = CUseableManager.CreateUseable(value.dwSpecSaleType, value.dwSpecSaleId, 0);
		...
		{
			this.InitLocalData();
			SCPKG_CMD_SPECIAL_SALEINFO stSPecialSaleDetail = msg.stPkgData.get_stSPecialSaleDetail();
			...
			else
			{
				for (int i = 0; i < (int)stSPecialSaleDetail.stSpecialSaleInfo.bSpecSaleCnt; i++)
				{
					...
					{
						this._spDict.get_Item(cOMDT_SPECSALE.dwId).BoughtCount = cOMDT_SPECSALE.dwNum;
					}
					if (this._filteredSpDict.ContainsKey(cOMDT_SPECSALE.dwId))
					{
						this._filteredSpDict.get_Item(cOMDT_SPECSALE.dwId).BoughtCount = cOMDT_SPECSALE.dwNum;
					}
				}
				this.ManageShelf();
			}
		...
		public CMallFactoryShopController.ShopProduct GetProduct(uint key)
		{
			...
			{
				return this._spDict.get_Item(key);
			}
			return null;
		}
		...
		public ListView<CMallFactoryShopController.ShopProduct> GetProductsInCurTab()
		{
			...
			return this._spListInTab.get_Item(this.CurTab);
		}
		...
		public int GetProductIndex(CMallFactoryShopController.ShopProduct product)
		{
			...
			{
				int count = this._spList.get_Count();
				for (int i = 0; i < count; i++)
				{
					if (this._spList.get_Item(i).Key == product.Key)
					{
						return i;
					}
				}
		...
		public int GetProductIndexInCurTab(CMallFactoryShopController.ShopProduct product)
		{
			if (this._spList != null)
			{
				...
				ListView<CMallFactoryShopController.ShopProduct> listView = this._spListInTab.get_Item(this.CurTab);
				int count = listView.get_Count();
				for (int i = 0; i < count; i++)
				{
					if (listView.get_Item(i).Key == product.Key)
					{
						return i;
					}
				}
		...
		public void Open(CUIFormScript form)
		{
			...
			{
				for (int i = 0; i < this._widgetList.get_Count(); i++)
				{
					this._widgetList.get_Item(i).Clear();
				}
			}
			...
			componetInChild.SetElementAmount(productsInCurTab.get_Count());
			for (int j = 0; j < productsInCurTab.get_Count(); j++)
			{
				CUIListElementScript elemenet = componetInChild.GetElemenet(j);
				if (elemenet != null && elemenet.gameObject != null)
				{
					CMallFactoryShopController.ShopProductWidget shopProductWidget = new CMallFactoryShopController.ShopProductWidget(elemenet.gameObject, productsInCurTab.get_Item(j));
			...
			if (targetID != 0)
			{
				...
				if (product != null)
				{
					...
					if (productIndexInCurTab >= 0 && productIndexInCurTab < productsInCurTab.get_Count())
					{
						componetInChild.MoveElementInScrollArea(productIndexInCurTab, true);
						if (productIndexInCurTab >= 0 && this._widgetList != null && productIndexInCurTab < this._widgetList.get_Count())
						{
							CMallFactoryShopController.ShopProductWidget shopProductWidget2 = this._widgetList.get_Item(productIndexInCurTab);
		...
		{
			for (int i = 0; i < this._spList.get_Count(); i++)
			{
				CMallFactoryShopController.ShopProduct shopProduct = this._spList.get_Item(i);
				if (shopProduct.IsOnSale == 1 && shopProduct.LimitCount > 0u && shopProduct.LimitCycle == 1u)
				{
					shopProduct.BoughtCount = 0u;
				}
			}
			for (int j = 0; j < this._filteredSpList.get_Count(); j++)
			{
				CMallFactoryShopController.ShopProduct shopProduct2 = this._filteredSpList.get_Item(j);
		...
		private void OnClickConfirmBuy(CUIEvent uiEvent)
		{
			...
			if (this._spDict.TryGetValue(tag, ref shopProduct))
			{
		...
		public void RequestBuy(CMallFactoryShopController.ShopProduct shopProduct, uint count)
		{
			...
			CSPKG_CMD_SPECSALEBUY stSpecSaleBuyReq = cSPkg.stPkgData.get_stSpecSaleBuyReq();
		...
		public void BuyShopProduct(CMallFactoryShopController.ShopProduct shopProduct, uint count, bool needConfirm, CUIEvent uiEvent = null)
		{
			...
			enPayType payType = CMallSystem.ResBuyTypeToPayType(shopProduct.CoinType);
			uint payValue;
		...
		{
			SCPKG_CMD_SPECSALEBUY stSpecSaleBuyRsp = msg.stPkgData.get_stSpecSaleBuyRsp();
			if (stSpecSaleBuyRsp.iErrCode == 0)
			{
				...
				{
					CMallFactoryShopController.ShopProduct shopProduct = this._spDict.get_Item(stSpecSaleBuyInfo.dwId);
					shopProduct.BoughtCount += stSpecSaleBuyInfo.dwNum;
					if (shopProduct.Type != 4 && (shopProduct.Type != 2 || !shopProduct.IsPropGift || !shopProduct.IsPropGiftUseImm))
					{
