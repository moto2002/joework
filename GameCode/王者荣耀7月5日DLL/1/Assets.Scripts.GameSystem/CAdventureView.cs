...
namespace Assets.Scripts.GameSystem
{
	internal class CAdventureView
	{
		...
		{
			return string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_Adventure_Dir, difficult.ToString());
		}
		...
		private static void SetReward(CUIFormScript formScript, GameObject itemCell, ResLevelCfgInfo resLevelInfo, int difficulty)
		{
			...
			CUseable cUseable = CUseableManager.CreateUseable(resDT_PveRewardShowInfo.bRewardType, resDT_PveRewardShowInfo.dwRewardID, 0);
			if (cUseable != null)
			{
		...
		private static void SetRewardItem(GameObject item, PVE_CHAPTER_COMPLETE_INFO chapterInfo, int stars, int chapterNo)
		{
			...
			if (stars == num)
			{
				if (chapterInfo.bIsGetBonus > 0)
				{
					...
					item.transform.FindChild("Unlock/StarText").GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("hasGot");
				}
				else
				{
