...
namespace Assets.Scripts.GameSystem
{
	...
	public class NobeSys : MonoSingleton<NobeSys>
	{
		...
		{
			MonoSingleton<NobeSys>.GetInstance().m_vipInfo = msg.stPkgData.get_stGameVipNtf();
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
			NobeSys._lastNobeLv = masterRoleInfo.GetNobeInfo().stGameVipClient.dwCurLevel;
			int dwCurLevel = (int)msg.stPkgData.get_stGameVipNtf().stGameVipClient.dwCurLevel;
			masterRoleInfo.SetNobeInfo(msg.stPkgData.get_stGameVipNtf());
			if (Singleton<BattleLogic>.get_instance().isRuning)
			{
				...
				MonoSingleton<NobeSys>.GetInstance().m_DelayNobeInfo.m_vipInfoBack = msg.stPkgData.get_stGameVipNtf();
			}
			else
			{
				if (MonoSingleton<NobeSys>.GetInstance().m_IsShowDelayNobeTips && msg.stPkgData.get_stGameVipNtf().dwNtfType > 0u)
				{
		...
		protected override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.Mall_Get_Product_OK, new Action(this.OnMall_Get_Product_OK));
		...
		private void UpdateBaseInfo()
		{
			...
			for (int i = 0; i < GameDataMgr.resNobeInfoDatabin.count; i++)
			{
				...
				if (dataByIndex != null && dataByIndex.bResType == 1)
				{
					...
					{
						ListView<ResNobeInfo> listView2 = dictionaryView.get_Item(dataByIndex.dwNobeLevel);
			...
			for (uint num4 = 1u; num4 <= 8u; num4 += 1u)
			{
				...
				for (uint num5 = num4; num5 >= 1u; num5 -= 1u)
				{
					...
					{
						for (int j = 0; j < dictionaryView.get_Item(num5).get_Count(); j++)
						{
							ResNobeInfo resNobeInfo = dictionaryView.get_Item(num5).get_Item(j);
							bool flag = false;
							for (int k = 0; k < listView3.get_Count(); k++)
							{
								ResNobeInfo resNobeInfo2 = listView3.get_Item(k);
			...
			for (uint num6 = 0u; num6 < 8u; num6 += 1u)
			{
				...
				if (!(elemenet == null))
				{
					...
					{
						ListView<ResNobeInfo> listView4 = dictionaryView2.get_Item(num6 + 1u);
						for (int l = 0; l < 4; l++)
						{
							...
							if (l < listView4.get_Count() && gameObject3 != null)
							{
								ResNobeInfo resNobeInfo3 = listView4.get_Item(l);
								if (gameObject3)
								{
			...
			for (int i = 1; i <= GameDataMgr.resNobeInfoDatabin.count; i++)
			{
				...
				if (dataByIndex != null && dataByIndex.bResType == 3 && (ulong)dataByIndex.dwNobeLevel == (ulong)((long)level))
				{
					...
					if (transform)
					{
						...
						transform5.gameObject.GetComponent<Text>().text = ((dataByIndex.dwJiaoBiaoNum <= 0u) ? null : dataByIndex.dwJiaoBiaoNum.ToString(CultureInfo.get_InvariantCulture()));
						...
						if (dataByKey != null)
						{
							...
							{
								CUseable cUseable = CUseableManager.CreateUsableByRandowReward(dataByKey2.astRewardDetail[num].bItemType, (int)dataByKey2.astRewardDetail[num].dwLowCnt, dataByKey2.astRewardDetail[num].dwItemID);
								if (cUseable != null)
								{
			...
			if (dataByKey != null)
			{
				...
				if (dataByKey2 != null)
				{
					for (int i = 0; i < dataByKey2.astRewardDetail.Length; i++)
					{
						...
						CUseable cUseable = CUseableManager.CreateUsableByRandowReward(dataByKey2.astRewardDetail[i].bItemType, (int)dataByKey2.astRewardDetail[i].dwLowCnt, dataByKey2.astRewardDetail[i].dwItemID);
						if (cUseable != null)
						{
							...
							if (!(transform == null))
							{
								if (transform)
								{
									...
									if (transform4 != null)
									{
										if (cUseable.m_type == 2 && CItem.IsHeroExperienceCard(cUseable.m_baseID))
										{
										...
										else if (cUseable.m_type == 2 && CItem.IsSkinExperienceCard(cUseable.m_baseID))
										{
											transform4.gameObject.CustomSetActive(true);
		...
		private void UpdateGrowInfo()
		{
			...
			for (int i = 1; i <= GameDataMgr.resVipDianQuan.count; i++)
			{
				...
				int num6 = (i - 1) / 5;
				if (!dictionaryView.ContainsKey(num6))
				{
					ListView<ResVIPCoupons> listView = new ListView<ResVIPCoupons>();
					listView.Add(dataByKey3);
					dictionaryView.Add(num6, listView);
				}
				else
				{
					ListView<ResVIPCoupons> listView2 = dictionaryView.get_Item(num6);
					listView2.Add(dataByKey3);
				}
			}
			for (int j = 0; j < num4; j++)
			{
				...
				{
					ListView<ResVIPCoupons> listView3 = dictionaryView.get_Item(j);
					for (int k = 0; k < 5; k++)
					{
						GameObject gameObject3 = null;
						...
						if (k < listView3.get_Count() && gameObject3 != null)
						{
							ResVIPCoupons resVIPCoupons = listView3.get_Item(k);
							gameObject3.CustomSetActive(true);
		...
		private void UpdateCHangeHeadFormData(int nHeadIDX)
		{
			...
			int count = changeHeadIconResList.get_Count();
			...
			{
				CUIListElementScript elemenet = component.GetElemenet(i);
				ResNobeInfo resNobeInfo = changeHeadIconResList.get_Item(i);
				...
				if ((ulong)resNobeInfo.dwNobeLevel <= (ulong)((long)dwCurLevel))
				{
					...
					if ((long)nHeadIDX == (long)((ulong)resNobeInfo.dwID))
					{
						gameObject3.CustomSetActive(true);
						string text3 = "已使用";
						gameObject3.GetComponent<Text>().text = text3;
				...
				else
				{
					...
					gameObject5.GetComponent<Text>().text = text4;
				}
			}
		}
		...
		public void SetHeadIconBk(Image image, int headIdx)
		{
			...
			if (headIdx > 0)
			{
				ResHeadImage resHeadImage = Singleton<HeadIconSys>.get_instance().GetResHeadImage(headIdx);
		...
		public void SetMyQQVipHead(Image image)
		{
			...
			if (Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().HasVip(16))
			{
				gameObject.CustomSetActive(true);
				CUIUtility.SetImageSprite(image, string.Format("{0}QQ_Svip_icon.prefab", "UGUI/Sprite/Common/"), null, true, false, false, false);
			}
			else if (Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().HasVip(1))
			{
				gameObject.CustomSetActive(true);
		...
		public void SetGameCenterVisible(GameObject obj, COM_PRIVILEGE_TYPE privilegeType, ApolloPlatform platform, bool isHideInIos = false, bool isIosBlock = false)
		{
			...
			bool gameCenterVisible = this.GetGameCenterVisible(privilegeType, platform, isHideInIos, isIosBlock);
			obj.CustomSetActive(gameCenterVisible);
		}
		...
		{
			bool result = false;
			if (Singleton<ApolloHelper>.GetInstance().CurPlatform == 1 && platform == 1)
			{
				if (privilegeType == 1)
				{
					result = true;
			...
			else if (Singleton<ApolloHelper>.GetInstance().CurPlatform == 2 && platform == 2)
			{
				if (privilegeType == 2)
				{
					result = true;
			...
			else if (Singleton<ApolloHelper>.GetInstance().CurPlatform == 5 && platform == 5)
			{
				result = true;
			}
		...
		protected void ShowNobeLevelUpForm(SCPKG_GAME_VIP_NTF vipInfo)
		{
			...
			{
				cUIFormScript.m_formWidgets[0].GetComponent<Text>().text = dwCurLevel.ToString(CultureInfo.get_InvariantCulture());
				cUIFormScript.m_formWidgets[1].GetComponent<Text>().text = NobeSys._lastNobeLv.ToString(CultureInfo.get_InvariantCulture());
				cUIFormScript.m_formWidgets[2].GetComponent<Text>().text = dwCurLevel.ToString(CultureInfo.get_InvariantCulture());
		...
		private void ShowNobeLoseTips(SCPKG_GAME_VIP_NTF vipInfo)
		{
			...
			CS_GAMEVIP_NTF_TYPE dwNtfType = vipInfo.dwNtfType;
			string text = string.Empty;
			if (dwNtfType == 2)
			{
