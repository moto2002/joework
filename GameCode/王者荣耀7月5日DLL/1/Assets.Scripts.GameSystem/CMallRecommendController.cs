...
namespace Assets.Scripts.GameSystem
{
	...
	public class CMallRecommendController : Singleton<CMallRecommendController>
	{
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.GetInstance().RemoveEventHandler(EventID.Mall_Recommend_Recommend_Data_Refresh, new Action(this.RefreshRecommendView));
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.BAG_ITEMS_UPDATE, new Action(this.OnItemAdd));
		}
		...
		public void Draw(CUIFormScript form)
		{
			...
			Singleton<EventRouter>.GetInstance().AddEventHandler(EventID.Mall_Recommend_Recommend_Data_Refresh, new Action(this.RefreshRecommendView));
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.BAG_ITEMS_UPDATE, new Action(this.OnItemAdd));
			this.InitElements();
		...
		private bool SetCurrentLotteryCtrl(uint drawID = 0u)
		{
			...
			if (drawID != 0u && GameDataMgr.recommendLotteryCtrlDict.TryGetValue(drawID, ref resRandDrawInfo) && resRandDrawInfo.bDrawType >= 1 && resRandDrawInfo.bDrawType < 3)
			{
			...
			{
				KeyValuePair<uint, ResRandDrawInfo> current = enumerator.get_Current();
		...
		private bool SetCurrentPoolData()
		{
			...
			for (byte b2 = 1; b2 < b; b2 += 1)
			{
				...
				while (enumerator.MoveNext())
				{
					KeyValuePair<long, ResRewardPoolInfo> current = enumerator.get_Current();
				...
				if (this.m_CurrentPoolList[(int)b2].get_Count() == 0)
				{
		...
		private bool SetCurrentRecommendData()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, ResSaleRecommend> current = enumerator.get_Current();
			...
			if (this.m_CurrentRecommendProductList.get_Count() == 0)
			{
		...
		private void RefreshPoolView()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				CUseable cUseable = CUseableManager.CreateUsableByRandowReward(listView.get_Item(i).stRewardInfo.wItemType, (int)listView.get_Item(i).stRewardInfo.wItemCnt, listView.get_Item(i).stRewardInfo.dwItemID);
		...
		private void RefreshRecommendView()
		{
			...
			for (int i = 0; i < this.m_CurrentRecommendProductList.get_Count(); i++)
			{
				ResSaleRecommend resSaleRecommend = this.m_CurrentRecommendProductList.get_Item(i);
				COM_ITEM_TYPE wSaleType = resSaleRecommend.wSaleType;
				if (wSaleType == 4)
				{
		...
		private void InitRecommendTab()
		{
			...
			{
				CMallRecommendController.Tab tab = list.get_Item((int)b2);
		...
		private void InitListTab()
		{
			...
			while ((int)b2 < array2.Length)
			{
				CMallRecommendController.ListTab listTab = list.get_Item((int)b2);
		...
		private void RefreshSkillPanel(ResHeroCfgInfo heroCfg)
		{
			...
			for (int i = 0; i < skillArr.Length - 1; i++)
			{
				...
				if (!(gameObject2 == null))
				{
					...
					eventParams.skillTipParam.skillEnergyCost = ((i != 0) ? Singleton<CTextManager>.get_instance().GetText(EnergyCommon.GetEnergyShowText(heroCfg.dwEnergyType, EnergyShowType.CostValue), new string[]
					{
		...
		private void RefreshHeroPricePnl(ResHeroCfgInfo heroCfg)
		{
			...
			if (gameObject4 != null)
			{
				...
				GameDataMgr.heroShopInfoDict.TryGetValue(heroCfg.dwCfgID, ref resHeroShop);
				if (resHeroShop != null)
				{
		...
		private void RefreshSkinPricePnl(ResHeroSkin skinCfg)
		{
			...
			if (gameObject4 != null)
			{
				...
				GameDataMgr.skinShopInfoDict.TryGetValue(skinCfg.dwID, ref resHeroSkinShop);
		...
		public string GetExchangeTypeIconPath(enExchangeType type)
		{
			if (type != enExchangeType.HeroExchangeCoupons)
			{
				if (type == enExchangeType.SkinExchangeCoupons)
				{
					...
					CUseable cUseable = CUseableManager.CreateUseable(2, dwConfValue, 0);
					if (cUseable != null)
					{
			...
			else
			{
				...
				uint dwConfValue = dataByKey.dwConfValue;
				CUseable cUseable2 = CUseableManager.CreateUseable(2, dwConfValue, 0);
				if (cUseable2 != null)
				{
		...
		public static uint GetCurrencyValueFromRoleInfo(CRoleInfo roleInfo, enExchangeType exchangeType)
		{
			if (exchangeType != enExchangeType.HeroExchangeCoupons)
			{
				...
				return (uint)useableContainer.GetUseableStackCount(2, dwConfValue);
			}
			else
			{
				...
				return (uint)useableContainer2.GetUseableStackCount(2, dwConfValue2);
			}
		}
		...
		private void OnNtyAddSkin(uint heroID, uint skinID, uint addReason)
		{
			if (addReason == 4u)
			{
				CUICommonSystem.ShowNewHeroOrSkin(heroID, skinID, enUIEventID.None, true, 10, false, null, enFormPriority.Priority1, 0u, 0);
			}
		...
		private void RefreshRecommendList(enExchangeType type)
		{
			...
			if (type != enExchangeType.HeroExchangeCoupons)
			{
				if (type == enExchangeType.SkinExchangeCoupons)
				{
					...
					component.SetElementAmount(this.m_CurrentRecommendSkins.get_Count());
				}
			}
			else
			{
				...
				component2.SetElementAmount(this.m_CurrentRecommendHeros.get_Count());
			}
		}
		...
		private void OnRecommendListHeroEnable(CUIEvent uiEvent)
		{
			...
			if (srcFormScript == null || srcWidget == null || srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_CurrentRecommendHeros.get_Count())
			{
				return;
			}
			ResHeroCfgInfo resHeroCfgInfo = this.m_CurrentRecommendHeros.get_Item(srcWidgetIndexInBelongedList);
			ResHeroShop resHeroShop = null;
			GameDataMgr.heroShopInfoDict.TryGetValue(resHeroCfgInfo.dwCfgID, ref resHeroShop);
			...
			if (!this.m_CurrentRecommendProductMap.TryGetValue(doubleKey, ref num))
			{
				Debug.LogError("recommend product not found");
				return;
			}
			ResSaleRecommend resSaleRecommend = new ResSaleRecommend();
			if (!GameDataMgr.recommendProductDict.TryGetValue(num, ref resSaleRecommend))
			{
				Debug.LogError("recommend product not found");
				return;
			}
			...
			CUICommonSystem.SetHeroItemImage(srcFormScript, transform2.gameObject, StringHelper.UTF8BytesToString(ref this.m_CurrentRecommendHeros.get_Item(srcWidgetIndexInBelongedList).szImagePath), enHeroHeadType.enBust, false, true);
		...
		private void OnRecommendListSkinEnable(CUIEvent uiEvent)
		{
			...
			GameObject srcWidget = uiEvent.m_srcWidget;
			int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
			if (srcFormScript == null || srcWidget == null || srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_CurrentRecommendSkins.get_Count())
			{
				return;
			}
			ResHeroSkin resHeroSkin = this.m_CurrentRecommendSkins.get_Item(srcWidgetIndexInBelongedList);
			ResHeroSkinShop resHeroSkinShop = null;
			GameDataMgr.skinShopInfoDict.TryGetValue(resHeroSkin.dwID, ref resHeroSkinShop);
		...
		{
			enPayType tag = (enPayType)uiEvent.m_eventParams.tag;
			CS_SALERECMD_BUYTYPE buyType = 1;
			switch (tag)
			{
			case enPayType.GoldCoin:
				buyType = 4;
		...
		private void OnRecommendExchangeConfirm(CUIEvent uiEvent)
		{
			enExchangeType tag = (enExchangeType)uiEvent.m_eventParams.tag;
			uint recommendProductID = 0u;
			CS_SALERECMD_BUYTYPE buyType = 3;
			enExchangeType enExchangeType = tag;
			if (enExchangeType != enExchangeType.HeroExchangeCoupons)
			{
				if (enExchangeType == enExchangeType.SkinExchangeCoupons)
		...
		private void OnLotteryBuyConfirm(CUIEvent uiEvent)
		{
			...
			cSPkg.stPkgData.set_stRandDrawReq(cSPKG_CMD_RANDDRAW_REQ);
		...
		private void HeroSelect_Skill_Down(CUIEvent uiEvent)
		{
			...
			for (int i = 1; i <= 2; i++)
			{
				...
				if (!(gameObject2 == null))
				{
					...
					{
						gameObject2.CustomSetActive(true);
						gameObject2.GetComponent<Image>().SetSprite(CSkillData.GetEffectSlotBg(skillEffect[i - 1]), uiEvent.m_srcFormScript, true, false, false, false);
						gameObject2.transform.Find("Text").GetComponent<Text>().text = CSkillData.GetEffectDesc(skillEffect[i - 1]);
					}
					else
					{
						gameObject2.CustomSetActive(false);
		...
		private void ReqBuy(uint recommendProductID, CS_SALERECMD_BUYTYPE buyType)
		{
			...
			cSPkg.stPkgData.get_stSaleRecmdBuyReq().bBuyType = buyType;
			cSPkg.stPkgData.get_stSaleRecmdBuyReq().dwRcmdID = recommendProductID;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void ReceiveRecommendBuyRsp(CSPkg msg)
		{
			...
			if (msg.stPkgData.get_stSaleRecmdBuyRsp().iResult != 0)
			{
				Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(1176, msg.stPkgData.get_stSaleRecmdBuyRsp().iResult), false, 1.5f, null, new object[0]);
				return;
			}
			uint dwRcmdID = msg.stPkgData.get_stSaleRecmdBuyRsp().dwRcmdID;
			ResSaleRecommend resSaleRecommend = new ResSaleRecommend();
			if (GameDataMgr.recommendProductDict.TryGetValue(dwRcmdID, ref resSaleRecommend))
			{
				COM_ITEM_TYPE wSaleType = resSaleRecommend.wSaleType;
				if (wSaleType == 4)
				{
					...
					if (dataByKey != null)
					{
						CUICommonSystem.ShowNewHeroOrSkin(dataByKey.dwCfgID, 0u, enUIEventID.None, true, 5, false, null, enFormPriority.Priority1, 0u, 0);
					}
					else
					{
		...
		public static void ReceiveRecommendSyncIDNTF(CSPkg msg)
		{
			...
			instance.RefreshPoolData(msg.stPkgData.get_stSyncRandDraw().dwDrawID);
		}
		...
		public static void ReceiveLotteryRsp(CSPkg msg)
		{
			...
			if (msg.stPkgData.get_stRandDrawRsp().iResult != 0)
			{
				Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(1178, msg.stPkgData.get_stRandDrawRsp().iResult), false, 1.5f, null, new object[0]);
				return;
			}
			CMallRecommendController instance = Singleton<CMallRecommendController>.GetInstance();
			ResRandDrawInfo resRandDrawInfo = new ResRandDrawInfo();
			if (!GameDataMgr.recommendLotteryCtrlDict.TryGetValue(msg.stPkgData.get_stRandDrawRsp().dwDrawID, ref resRandDrawInfo))
			{
				Singleton<CUIManager>.GetInstance().OpenTips("Mall_Recommend_Config_Err_Tips", true, 1f, null, new object[]
				{
					"ctr",
					msg.stPkgData.get_stRandDrawRsp().dwDrawID
				});
				return;
			}
			uint dwRewardPoolID = resRandDrawInfo.dwRewardPoolID;
			long doubleKey = GameDataMgr.GetDoubleKey(dwRewardPoolID, (uint)msg.stPkgData.get_stRandDrawRsp().bPoolIdx);
			ResRewardPoolInfo resRewardPoolInfo = new ResRewardPoolInfo();
			if (GameDataMgr.recommendRewardDict.TryGetValue(doubleKey, ref resRewardPoolInfo))
			{
				ListView<CUseable> listView = new ListView<CUseable>();
				CUseable cUseable = CUseableManager.CreateUsableByRandowReward(resRewardPoolInfo.stRewardInfo.wItemType, (int)resRewardPoolInfo.stRewardInfo.wItemCnt, resRewardPoolInfo.stRewardInfo.dwItemID);
				if (cUseable != null)
				{
					listView.Add(cUseable);
					CUseable[] array = new CUseable[listView.get_Count()];
					for (int i = 0; i < array.Length; i++)
					{
						array[i] = listView.get_Item(i);
					}
