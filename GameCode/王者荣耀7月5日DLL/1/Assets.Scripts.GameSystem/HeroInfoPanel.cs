...
namespace Assets.Scripts.GameSystem
{
	public class HeroInfoPanel
	{
		...
		public void Init(GameObject heroInfoPanelObj)
		{
			...
			{
				if (allPlayers.get_Item(i).PlayerCamp == hostPlayer.PlayerCamp && allPlayers.get_Item(i) != hostPlayer)
				{
					int listIndex = num2;
					CUIListElementScript elemenet2 = this.heroInfoList.GetElemenet(num2);
					if (elemenet2 != null && allPlayers.get_Item(i).Captain)
					{
						elemenet2.gameObject.CustomSetActive(true);
						this.InitHeroItemData(elemenet2, allPlayers.get_Item(i).Captain);
						this.AddHeroToDic(allPlayers.get_Item(i).Captain, listIndex);
						this.m_teamHeroList.Add(allPlayers.get_Item(i).Captain);
					}
					num2++;
				}
			}
		...
		{
			if (index >= 0 && index < this.m_reviveTextCacheList.get_Count())
			{
				return this.m_reviveTextCacheList.get_Item(index);
			}
			return null;
		}
		...
		{
			if (index >= 0 && index < this.m_hpImageCacheList.get_Count())
			{
				return this.m_hpImageCacheList.get_Item(index);
			}
			return null;
		}
		...
		private void AddHeroToDic(PoolObjHandle<ActorRoot> actor, int listIndex)
		{
			...
			if (!this.m_HeroToIndexDic.ContainsKey(actor.get_handle().ObjID))
			{
				this.m_HeroToIndexDic.Add(actor.get_handle().ObjID, listIndex);
			}
			else
			{
				this.m_HeroToIndexDic.set_Item(actor.get_handle().ObjID, listIndex);
			}
		}
		...
		public void Update()
		{
			...
			{
				PoolObjHandle<ActorRoot> poolObjHandle = this.m_teamHeroList.get_Item(i);
				CUIListElementScript elemenet = this.heroInfoList.GetElemenet(i);
				if (poolObjHandle && poolObjHandle.get_handle().SkillControl != null && !(null == elemenet))
				{
					SkillSlot skillSlot = null;
					if (poolObjHandle.get_handle().SkillControl.TryGetSkillSlot(SkillSlotType.SLOT_SKILL_3, out skillSlot) && skillSlot.IsUnLock() && skillSlot.IsCDReady)
					{
					...
					if (num != 0)
					{
						if ((num & 1) != 0)
						{
							...
							if (hpImage != null)
							{
								hpImage.fillAmount = poolObjHandle.get_handle().ValueComponent.GetHpRate().get_single();
							}
							num &= -2;
						}
						...
						{
							reviveText.text = SimpleNumericString.GetNumeric(Mathf.RoundToInt((float)poolObjHandle.get_handle().ActorControl.ReviveCooldown * 0.001f));
						}
			...
			for (int i = 0; i < count2; i++)
			{
				...
				if (elemenet != null && this.m_enemyHeroList.get_Item(i))
				{
					Text reviveText2 = this.GetReviveText(i + 5);
					if (reviveText2 != null)
					{
						reviveText2.text = SimpleNumericString.GetNumeric(Mathf.RoundToInt((float)this.m_enemyHeroList.get_Item(i).get_handle().ActorControl.ReviveCooldown * 0.001f));
					}
				}
			}
		}
		...
		private void RefreshHeroIconImage(CUIListElementScript elementScript, PoolObjHandle<ActorRoot> actor)
		{
			...
			if (widget != null)
			{
				ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey((uint)actor.get_handle().TheActorMeta.ConfigId);
				Image component = widget.GetComponent<Image>();
				if (component != null && dataByKey != null)
				{
					component.SetSprite(string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_BustCircle_Dir, dataByKey.szImagePath), Singleton<CBattleSystem>.get_instance().FightFormScript, true, false, false, false);
				}
			}
		}
		...
		{
			int num = 0;
			if (!actor || !this.m_HeroToIndexDic.TryGetValue(actor.get_handle().ObjID, ref num))
			{
				return;
			}
		...
		{
			if (eventParam.src && eventParam.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				bool flag = eventParam.src.get_handle().IsHostCamp();
				int num = 0;
				...
				if (this.m_HeroToIndexDic.TryGetValue(eventParam.src.get_handle().ObjID, ref num))
				{
		...
		private void OnActorRevive(ref DefaultGameEventParam eventParam)
		{
			if (eventParam.src && eventParam.src.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				if (flag)
				{
					if (this.m_HeroToIndexDic.TryGetValue(eventParam.src.get_handle().ObjID, ref listIndex))
					{
		...
		{
			if (actor && actor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && actor.get_handle().IsHostCamp())
			{
