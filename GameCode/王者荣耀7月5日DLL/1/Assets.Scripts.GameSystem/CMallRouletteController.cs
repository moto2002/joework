...
namespace Assets.Scripts.GameSystem
{
	...
	public class CMallRouletteController : Singleton<CMallRouletteController>
	{
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.Mall_Receive_Roulette_Data, new Action(this.RefreshExternRewards));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.Mall_Change_Tab, new Action(this.OnMallTabChange));
			Singleton<EventRouter>.get_instance().RemoveEventHandler<uint>("HeroAdd", new Action<uint>(this.OnNtyAddHero));
			Singleton<EventRouter>.get_instance().RemoveEventHandler<uint, uint, uint>("HeroSkinAdd", new Action<uint, uint, uint>(this.OnNtyAddSkin));
		}
		...
		public void Draw(CUIFormScript form)
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.Mall_Receive_Roulette_Data, new Action(this.RefreshExternRewards));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.Mall_Change_Tab, new Action(this.OnMallTabChange));
			Singleton<EventRouter>.get_instance().AddEventHandler<uint>("HeroAdd", new Action<uint>(this.OnNtyAddHero));
			Singleton<EventRouter>.get_instance().AddEventHandler<uint, uint, uint>("HeroSkinAdd", new Action<uint, uint, uint>(this.OnNtyAddSkin));
			this.InitElements();
			this.RefreshData(0u);
			this.InitTab();
		}
		...
		public void RefreshData(uint targetPoolId = 0u)
		{
			...
			{
				KeyValuePair<enPayType, ResLuckyDrawPrice> current = enumerator.get_Current();
				...
				if (value != null)
				{
					...
					{
						KeyValuePair<long, ResLuckyDrawRewardForClient> current2 = enumerator2.get_Current();
						...
						if (value2 != null && value2.dwRewardPoolID == num)
						{
							...
							if (this.m_RewardDic.TryGetValue((uint)value.bMoneyType, ref listView))
							{
								listView.Add(value2);
							}
						}
					...
					{
						KeyValuePair<uint, ResLuckyDrawExternReward> current3 = enumerator3.get_Current();
						...
						if (value3 != null && value3.bMoneyType == value.bMoneyType)
						{
							...
							if (this.m_ExternRewardDic.TryGetValue((uint)value.bMoneyType, ref listView2))
							{
								for (int i = 0; i < (int)value3.bExternCnt; i++)
								{
									ResDT_LuckyDrawExternReward resDT_LuckyDrawExternReward = value3.astReward[i];
									listView2.Add(resDT_LuckyDrawExternReward);
								}
			...
			if (componetInChild != null)
			{
				...
				for (int i = 0; i < componetInChild.m_elementAmount; i++)
				{
					...
					if (elemenet != null)
					{
						...
						RES_SHOPBUY_COINTYPE coinType = 0;
						bool flag = false;
						...
						if (tab != CMallRouletteController.Tab.DianQuan)
						{
							if (tab == CMallRouletteController.Tab.Diamond)
							{
								...
								coinType = 10;
								stPayInfo payInfo = CMallRouletteController.GetPayInfo(2, CMallRouletteController.Tab.Diamond);
								stPayInfo payInfo2 = CMallRouletteController.GetPayInfo(3, CMallRouletteController.Tab.Diamond);
						...
						else
						{
							...
							coinType = 2;
							stPayInfo payInfo3 = CMallRouletteController.GetPayInfo(2, CMallRouletteController.Tab.DianQuan);
							stPayInfo payInfo4 = CMallRouletteController.GetPayInfo(3, CMallRouletteController.Tab.DianQuan);
			...
			if (componetInChild != null)
			{
				...
				if (curTab != CMallRouletteController.Tab.DianQuan)
				{
					if (curTab == CMallRouletteController.Tab.Diamond)
					{
						...
						this.m_ExternRewardDic.TryGetValue(10u, ref listView);
					}
				}
				else
				{
					...
					this.m_ExternRewardDic.TryGetValue(2u, ref listView);
				}
			}
			if (listView == null || listView.get_Count() == 0)
			{
				...
				for (byte b = 0; b < 5; b += 1)
				{
					...
					if ((int)b < listView.get_Count())
					{
						if (gameObject2 != null)
						{
							...
							ResDT_LuckyDrawExternReward resDT_LuckyDrawExternReward = listView.get_Item((int)b);
		...
		public void RefreshRewards()
		{
			...
			if (curTab != CMallRouletteController.Tab.DianQuan)
			{
				if (curTab == CMallRouletteController.Tab.Diamond)
				{
					...
					GameDataMgr.mallRoulettePriceDict.TryGetValue(enPayType.Diamond, ref resLuckyDrawPrice);
					ListView<ResLuckyDrawRewardForClient> listView = null;
					this.m_RewardDic.TryGetValue(10u, ref listView);
					if (listView == null || listView.get_Count() == 0)
					{
						return;
					}
			...
			else
			{
				...
				GameDataMgr.mallRoulettePriceDict.TryGetValue(enPayType.DianQuan, ref resLuckyDrawPrice);
				ListView<ResLuckyDrawRewardForClient> listView2 = null;
				this.m_RewardDic.TryGetValue(2u, ref listView2);
				if (listView2 == null || listView2.get_Count() == 0)
				{
					return;
				}
		...
		private void SetRewardItemCells(ListView<ResLuckyDrawRewardForClient> rewardList)
		{
			...
			for (byte b3 = 0; b3 < 14; b3 += 1)
			{
				...
				if (!(gameObject == null))
				{
					ResLuckyDrawRewardForClient resLuckyDrawRewardForClient = rewardList.get_Item((int)b3);
					...
					CUseable cUseable = CUseableManager.CreateUsableByRandowReward(resLuckyDrawRewardForClient.dwItemType, (int)resLuckyDrawRewardForClient.dwItemCnt, resLuckyDrawRewardForClient.dwItemID);
					...
					if (gameObject2 != null)
					{
						if (cUseable.m_type == 4 || cUseable.m_type == 7 || (cUseable.m_type == 2 && CItem.IsHeroExperienceCard(cUseable.m_baseID)) || (cUseable.m_type == 2 && CItem.IsSkinExperienceCard(cUseable.m_baseID)))
						{
							gameObject2.CustomSetActive(true);
						}
						else
		...
		private void RefreshButtonView()
		{
			...
			stPayInfo payInfo = CMallRouletteController.GetPayInfo(2, CMallRouletteController.Tab.None);
			stPayInfo payInfo2 = CMallRouletteController.GetPayInfo(3, CMallRouletteController.Tab.None);
		...
		public static stPayInfo GetPayInfo(RES_SHOPDRAW_SUBTYPE drawType = 2, CMallRouletteController.Tab tab = CMallRouletteController.Tab.None)
		{
			...
			if (drawType != 2)
			{
				if (drawType == 3)
				{
					if (GameDataMgr.mallRoulettePriceDict.TryGetValue(result.m_payType, ref resLuckyDrawPrice))
					{
			...
			else if (GameDataMgr.mallRoulettePriceDict.TryGetValue(result.m_payType, ref resLuckyDrawPrice))
			{
		...
		{
			if (this.m_RewardDic == null || this.m_RewardDic.get_Count() == 0)
			{
				this.RefreshData(0u);
			}
			if (this.m_RewardDic == null)
			...
			while (enumerator.MoveNext())
			{
				if (coinType != null)
				{
					KeyValuePair<uint, ListView<ResLuckyDrawRewardForClient>> current = enumerator.get_Current();
					if (current.get_Key() != (uint)coinType)
					{
						continue;
					}
				}
				KeyValuePair<uint, ListView<ResLuckyDrawRewardForClient>> current2 = enumerator.get_Current();
				if (current2.get_Value() != null)
				{
					KeyValuePair<uint, ListView<ResLuckyDrawRewardForClient>> current3 = enumerator.get_Current();
					if (current3.get_Value().get_Count() > 0)
					{
						KeyValuePair<uint, ListView<ResLuckyDrawRewardForClient>> current4 = enumerator.get_Current();
						ListView<ResLuckyDrawRewardForClient> value = current4.get_Value();
						int count = value.get_Count();
						for (int i = 0; i < count; i++)
						{
							if (value.get_Item(i).bShowProbabilityDoubled > 0)
							{
								return true;
							}
						}
		...
		public ListView<CUseable> GetRewardUseables(SCPKG_LUCKYDRAW_RSP stLuckyDrawRsp)
		{
			...
			for (int i = 0; i < (int)stLuckyDrawRsp.bRewardCnt; i++)
			{
				...
				if (GameDataMgr.mallRouletteRewardDict.TryGetValue(doubleKey, ref resLuckyDrawRewardForClient))
				{
					CUseable cUseable = CUseableManager.CreateUsableByRandowReward(resLuckyDrawRewardForClient.dwItemType, (int)resLuckyDrawRewardForClient.dwItemCnt, resLuckyDrawRewardForClient.dwItemID);
					if (cUseable != null)
					{
						...
						case 4:
						{
							...
							if (heroData.bPlayerOwn)
							{
								...
								cUseable = CUseableManager.CreateUseable(2, dwConfValue, (int)((cHeroInfoData.m_info.shopCfgInfo == null) ? 1u : cHeroInfoData.m_info.shopCfgInfo.dwChgItemCnt));
						...
						case 7:
						{
							...
							if (masterRoleInfo != null && masterRoleInfo.IsHaveHeroSkin(heroId, skinId, false))
							{
								...
								cUseable = CUseableManager.CreateUseable(2, dwConfValue2, (int)((resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwChgItemCnt));
		...
		public void OpenAwardTip(RES_SHOPDRAW_SUBTYPE drawType, ListView<CUseable> items, string title = null, bool playSound = false, enUIEventID eventID = enUIEventID.None, bool displayAll = false)
		{
			...
			int b = 5;
			int num = Mathf.Min(items.get_Count(), b);
			this.m_CurRewardIdx = 0;
			CUIFormScript cUIFormScript;
			if (items.get_Count() < 5)
			{
			...
			if (drawType != 2)
			{
				if (drawType == 3)
				{
					stPayInfo = CMallRouletteController.GetPayInfo(3, CMallRouletteController.Tab.None);
			...
			{
				stPayInfo = CMallRouletteController.GetPayInfo(2, CMallRouletteController.Tab.None);
			...
			if (cUITimerScript == null)
			{
				for (int j = 0; j < num; j++)
				{
					...
					CUICommonSystem.SetItemCell(cUIFormScript, gameObject3, items.get_Item(j), true, displayAll, false, false);
					if (items.get_Item(j).m_itemSortNum > 0uL)
					{
					...
					if (playSound)
					{
						COM_REWARDS_TYPE mapRewardType = items.get_Item(j).MapRewardType;
						if (mapRewardType != null)
						{
							if (mapRewardType != 9)
							{
								if (mapRewardType == 16)
			...
			else
			{
				for (int k = 0; k < num; k++)
				{
					...
					CUICommonSystem.SetItemCell(cUIFormScript, gameObject4, items.get_Item(k), true, displayAll, false, false);
					if (items.get_Item(k).m_itemSortNum > 0uL)
					{
		...
		public void ShowHeroSkin(ListView<CUseable> items)
		{
			...
			while (i < count)
			{
				switch (items.get_Item(i).m_type)
				{
				case 2:
					if (items.get_Item(i).ExtraFromType == 1)
					{
						CUICommonSystem.ShowNewHeroOrSkin((uint)items.get_Item(i).ExtraFromData, 0u, enUIEventID.None, true, 5, true, null, enFormPriority.Priority4, (uint)items.get_Item(i).m_stackCount, 0);
					}
					else if (items.get_Item(i).ExtraFromType == 2)
					{
						CSkinInfo.ResolveHeroSkin((uint)items.get_Item(i).ExtraFromData, out heroId, out skinId);
						CUICommonSystem.ShowNewHeroOrSkin(heroId, skinId, enUIEventID.None, true, 10, true, null, enFormPriority.Priority4, (uint)items.get_Item(i).m_stackCount, 0);
					}
					break;
				case 4:
					CUICommonSystem.ShowNewHeroOrSkin(items.get_Item(i).m_baseID, 0u, enUIEventID.None, true, 5, false, null, enFormPriority.Priority4, 0u, 0);
					break;
				case 7:
					CSkinInfo.ResolveHeroSkin(items.get_Item(i).m_baseID, out heroId, out skinId);
					CUICommonSystem.ShowNewHeroOrSkin(heroId, skinId, enUIEventID.None, true, 10, false, null, enFormPriority.Priority4, 0u, 0);
					break;
				}
				IL_11E:
				i++;
		...
		private void OnRouletteBuyFive(CUIEvent uiEvent)
		{
			...
			stPayInfo payInfo = CMallRouletteController.GetPayInfo(3, CMallRouletteController.Tab.None);
			CUIEvent uIEvent = Singleton<CUIEventManager>.GetInstance().GetUIEvent();
		...
		private void OnSpinEnd(CUIEvent uiEvent)
		{
			...
			if (this.m_LuckyDrawRsp != null && this.m_LuckyDrawRsp.bRewardCnt != 0)
			{
				Singleton<CTimerManager>.GetInstance().AddTimer(600, 1, delegate(int sequence)
				{
					...
					{
						this.OpenAwardTip(2, this.m_RewardList, title, true, enUIEventID.None, false);
					}
					else
					{
						this.OpenAwardTip(3, this.m_RewardList, title, true, enUIEventID.None, false);
					}
		...
		private void OnOpenExternRewardTip(CUIEvent uiEvent)
		{
			...
			if (listView == null || listView.get_Count() == 0 || tag > listView.get_Count())
			{
				return;
			}
			ResDT_LuckyDrawExternReward resDT_LuckyDrawExternReward = listView.get_Item(tag);
			...
			for (int i = 0; i < dataByKey.astRewardDetail.Length; i++)
			{
				...
				ListView<CUseable> listView3 = CUseableManager.CreateUsableListByRandowReward(dataByKey.astRewardDetail[i].bItemType, (int)dataByKey.astRewardDetail[i].dwLowCnt, dataByKey.astRewardDetail[i].dwItemID);
				if (listView3 != null && listView3.get_Count() > 0)
				{
			...
			else
			{
				...
				if (b2 == 0)
				{
					...
					{
						tag = tag,
						tag2 = CMallSystem.PayTypeToResBuyCoinType(payInfo.m_payType)
					});
				}
				else
				{
			...
			int num = 5;
			int num2 = Mathf.Min(listView2.get_Count(), num);
			for (int j = 0; j < num2; j++)
			{
				...
				CUICommonSystem.SetItemCell(cUIFormScript, gameObject3, listView2.get_Item(j), true, false, false, false);
				gameObject3.CustomSetActive(true);
				gameObject3.transform.FindChild("ItemName").GetComponent<Text>().text = listView2.get_Item(j).m_name;
			}
		...
		private void OnDrawExternReward(CUIEvent uiEvent)
		{
			...
			cSPkg.stPkgData.get_stLuckyDrawExternReq().bMoneyType = (byte)uiEvent.m_eventParams.tag2;
			cSPkg.stPkgData.get_stLuckyDrawExternReq().bExternIndex = (byte)uiEvent.m_eventParams.tag;
		...
		private void OnTmpRewardEnable(CUIEvent uiEvent)
		{
			...
			if (this.m_RewardList == null || srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList > this.m_RewardList.get_Count())
			{
				return;
			}
			GameObject srcWidget = uiEvent.m_srcWidget;
			GameObject gameObject = srcWidget.transform.Find("itemCell").gameObject;
			CUseable itemUseable = this.m_RewardList.get_Item(srcWidgetIndexInBelongedList);
		...
		private int GetCryStalItemCount(uint tabID)
		{
			...
			if ((ulong)tabID < (ulong)((long)this.m_CrySatlDic.get_Count()))
			{
				ListView<ResRareExchange> listView = this.m_CrySatlDic.get_Item(tabID);
				if (listView.get_Count() > 0)
				{
					uint dwRareItemID = listView.get_Item(0).dwRareItemID;
					return useableContainer.GetUseableStackCount(2, dwRareItemID);
				}
			}
			return 0;
		}
		...
		private void InitCryStalData()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, ResRareExchange> current = enumerator.get_Current();
				...
				{
					ListView<ResRareExchange> listView2 = this.m_CrySatlDic.get_Item(dwTabID);
					listView2.Add(value);
				}
			}
			this.m_nTotalCrySalTab = this.m_CrySatlDic.get_Count();
			uint num = 0u;
			while ((ulong)num < (ulong)((long)this.m_nTotalCrySalTab))
			{
				ListView<ResRareExchange> listView3 = this.m_CrySatlDic.get_Item(num);
		...
		private void InitCryStalItemID()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (!dictionaryView.ContainsKey(dwTabID))
				{
					ListView<ResRareExchange> listView = new ListView<ResRareExchange>();
					listView.Add(value);
					dictionaryView.Add(dwTabID, listView);
				}
				else
				{
					ListView<ResRareExchange> listView2 = dictionaryView.get_Item(dwTabID);
					listView2.Add(value);
				}
			}
			if (this.m_CrystalItemID.Length < dictionaryView.get_Count())
			{
				this.m_CrystalItemID = new int[dictionaryView.get_Count()];
			}
			uint num = 0u;
			while ((ulong)num < (ulong)((long)dictionaryView.get_Count()))
			{
				ListView<ResRareExchange> listView3 = dictionaryView.get_Item(num);
				if (listView3.get_Count() > 0)
				{
					this.m_CrystalItemID[(int)((UIntPtr)num)] = (int)listView3.get_Item(0).dwRareItemID;
				}
				else
				{
		...
		private void ProcessCrystalDataTabID(ref ListView<ResRareExchange> resultList)
		{
			...
			for (int i = 0; i < resultList.get_Count(); i++)
			{
				ResRareExchange resRareExchange = resultList.get_Item(i);
			...
			{
				resultList.Add(listView.get_Item(j));
			}
			for (int k = 0; k < listView2.get_Count(); k++)
			{
				resultList.Add(listView2.get_Item(k));
			}
			for (int l = 0; l < listView3.get_Count(); l++)
			{
				resultList.Add(listView3.get_Item(l));
			}
		}
		...
		private void UpdateCryStalTab(uint tabID)
		{
			...
			if ((ulong)tabID < (ulong)((long)this.m_CrySatlDic.get_Count()))
			{
				component2.SetElementAmount(this.m_CrySatlDic.get_Item(tabID).get_Count() + 1);
			}
		}
		...
		{
			int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
			if ((ulong)this.m_nCurCryStalTabID >= (ulong)((long)this.m_CrySatlDic.get_Count()))
			{
				return;
			}
			if (srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_CrySatlDic.get_Item(this.m_nCurCryStalTabID).get_Count())
			{
				this.OnDefault(uiEvent);
				return;
			}
			ResRareExchange resRareExchange = this.m_CrySatlDic.get_Item(this.m_nCurCryStalTabID).get_Item(srcWidgetIndexInBelongedList);
			if (resRareExchange.wExchangeType == 4)
			{
		...
		private void OnCrystalExchange(CUIEvent uiEvent)
		{
			...
			RES_ITEM_TYPE tag = uiEvent.m_eventParams.tag;
			...
			if (commonUInt16Param < 0u || (ulong)commonUInt16Param >= (ulong)((long)this.m_CrySatlDic.get_Item(this.m_nCurCryStalTabID).get_Count()))
			{
				return;
			}
			ResRareExchange resRareExchange = this.m_CrySatlDic.get_Item(this.m_nCurCryStalTabID).get_Item((int)commonUInt16Param);
		...
		private void RequestBuyCrystal(uint rareID)
		{
			...
			cSPkg.stPkgData.get_stRareExchangeReq().dwID = rareID;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		{
			if (msg.stPkgData.get_stRareExchangeRsp().iResult == 0)
			{
				Singleton<CMallRouletteController>.GetInstance().RefreshCrystalPnl();
				uint num = 0u;
				while ((ulong)num < (ulong)((long)Singleton<CMallRouletteController>.GetInstance().m_CrySatlDic.get_Count()))
				{
					for (int i = 0; i < Singleton<CMallRouletteController>.GetInstance().m_CrySatlDic.get_Item(num).get_Count(); i++)
					{
						ResRareExchange resRareExchange = Singleton<CMallRouletteController>.GetInstance().m_CrySatlDic.get_Item(num).get_Item(i);
						if (resRareExchange != null && resRareExchange.dwID == msg.stPkgData.get_stRareExchangeRsp().dwID)
						{
							if (resRareExchange.wExchangeType == 4)
							{
								uint dwExchangeID = resRareExchange.dwExchangeID;
								CUICommonSystem.ShowNewHeroOrSkin(dwExchangeID, 0u, enUIEventID.None, true, 5, false, null, enFormPriority.Priority1, 0u, 0);
							}
							else if (resRareExchange.wExchangeType == 7)
							{
								uint dwExchangeID2 = resRareExchange.dwExchangeID;
								CUICommonSystem.ShowNewHeroOrSkin(0u, dwExchangeID2, enUIEventID.None, true, 10, false, null, enFormPriority.Priority1, 0u, 0);
							}
						}
					}
					num += 1u;
		...
		private void SendLotteryMsg(RES_SHOPDRAW_SUBTYPE drawType = 2)
		{
			...
			cSPKG_LUCKYDRAW_REQ.bMoneyType = CMallSystem.PayTypeToResBuyCoinType(payInfo.m_payType);
			cSPKG_LUCKYDRAW_REQ.bDrawType = drawType;
			cSPkg.stPkgData.set_stLuckyDrawReq(cSPKG_LUCKYDRAW_REQ);
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		...
		public static void ReceiveLotteryRes(CSPkg msg)
		{
			...
			if (msg.stPkgData.get_stLuckyDrawRsp().iResult != 0)
			{
				CUITimerScript componetInChild = Utility.GetComponetInChild<CUITimerScript>(mallForm.gameObject, "pnlBodyBg/pnlRoulette/timerSpin");
				if (componetInChild != null)
				{
				...
				Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(4802, msg.stPkgData.get_stLuckyDrawRsp().iResult), false, 1.5f, null, new object[0]);
			...
			uint num = 0u;
			if (Singleton<CMallRouletteController>.GetInstance().m_RewardPoolDic.TryGetValue((uint)msg.stPkgData.get_stLuckyDrawRsp().bMoneyType, ref num))
			{
				if (num != msg.stPkgData.get_stLuckyDrawRsp().dwRewardPoolID)
				{
					Singleton<CMallRouletteController>.GetInstance().RefreshData(msg.stPkgData.get_stLuckyDrawRsp().dwRewardPoolID);
				}
				else
				{
			...
			Singleton<CMallRouletteController>.GetInstance().SetRewardData(msg.stPkgData.get_stLuckyDrawRsp());
			int idx = (int)msg.stPkgData.get_stLuckyDrawRsp().szRewardIndex[0];
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			if (msg.stPkgData.get_stLuckyDrawExternRsp().iResult != 0)
			{
				Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(4804, msg.stPkgData.get_stLuckyDrawExternRsp().iResult), false, 1.5f, null, new object[0]);
				return;
			}
			ListView<CUseable> useableListFromReward = CUseableManager.GetUseableListFromReward(msg.stPkgData.get_stLuckyDrawExternRsp().stReward);
			if (useableListFromReward.get_Count() == 0)
			{
				return;
			}
			CUseable[] array = new CUseable[useableListFromReward.get_Count()];
			for (int i = 0; i < array.Length; i++)
			{
				array[i] = useableListFromReward.get_Item(i);
			}
