	...
	public class ExchangePhase : ActivityPhase
	{
		...
		public override bool ReadyForGet
		{
			get
			{
				...
				CUseableContainer useableContainer = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().GetUseableContainer(enCONTAINER_TYPE.ITEM);
				...
				if (resDT_Item_Info != null)
				{
					...
					int useableStackCount = useableContainer.GetUseableStackCount(wItemType, dwItemID);
				...
				if (resDT_Item_Info2 != null)
				{
					...
					int useableStackCount2 = useableContainer.GetUseableStackCount(wItemType2, dwItemID2);
