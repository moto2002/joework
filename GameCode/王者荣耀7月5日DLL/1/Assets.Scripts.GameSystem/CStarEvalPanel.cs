...
namespace Assets.Scripts.GameSystem
{
	public class CStarEvalPanel
	{
		...
		private void initEvent()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.StarSystemInitialized, new Action(this.reset));
		...
		private void DeinitEvent()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.StarSystemInitialized, new Action(this.reset));
		...
		public void reset()
		{
			...
			{
				this.conditionTexts[enumerator.get_Current().index] = transform.Find(string.Format("Condition{0}", enumerator.get_Current().index + 1)).GetComponent<Text>();
				this.conditionTexts[enumerator.get_Current().index].text = enumerator.get_Current().description;
				if (enumerator.get_Current().isSuccess)
				{
					this.conditionTexts[enumerator.get_Current().index].color = Color.green;
				}
				else
				{
					this.conditionTexts[enumerator.get_Current().index].color = Color.white;
				}
			}
