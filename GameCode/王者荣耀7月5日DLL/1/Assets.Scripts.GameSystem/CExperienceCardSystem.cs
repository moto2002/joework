...
namespace Assets.Scripts.GameSystem
{
	...
	public class CExperienceCardSystem : Singleton<CExperienceCardSystem>
	{
		...
		private void OnExperienceHeroAdd(uint heroId, int experienceDays)
		{
			...
			{
				CUICommonSystem.ShowNewHeroOrSkin(heroId, 0u, enUIEventID.None, true, 5, false, null, enFormPriority.Priority1, 0u, experienceDays);
			}
			else if (!Singleton<CHeroSelectBaseSystem>.get_instance().IsMultilMode() && Singleton<CHeroSelectBaseSystem>.get_instance().uiType == enUIType.enNormal)
			{
				IHeroData selectHeroData = CHeroDataFactory.CreateHeroData(heroId);
				Singleton<CHeroSelectNormalSystem>.get_instance().HeroSelect_SelectHero(selectHeroData);
			}
		}
		...
		{
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
			SCPKG_LIMITSKIN_DEL stLimitSkinDel = msg.stPkgData.get_stLimitSkinDel();
			int num = 0;
		...
		public static void OnReceiveUseExpCardNtf(CSPkg msg)
		{
			...
			switch (msg.stPkgData.get_stUseExpCardNtf().iResult)
			{
			case 2:
