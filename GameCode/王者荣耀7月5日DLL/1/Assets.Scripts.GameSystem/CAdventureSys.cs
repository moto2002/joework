...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CAdventureSys : Singleton<CAdventureSys>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler<CSPkg>("ShopBuyLvlChallengeTime", new Action<CSPkg>(this.OnBuyPlayTimeRsp));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.SINGLEGAME_ERR_FREEHERO, new Action(this.OnFreeHeroChanged));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.GLOBAL_REFRESH_TIME, new Action(CAdventureSys.ResetElitePlayNum));
		}
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler<CSPkg>("ShopBuyLvlChallengeTime", new Action<CSPkg>(this.OnBuyPlayTimeRsp));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.SINGLEGAME_ERR_FREEHERO, new Action(this.OnFreeHeroChanged));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.GLOBAL_REFRESH_TIME, new Action(CAdventureSys.ResetElitePlayNum));
		}
		...
		private void CheckUnlockTips()
		{
			...
			if (masterRoleInfo != null)
			{
				if (this.m_chapterStatus == null || this.m_difficultyStatus == null)
				{
					...
					{
						this.m_chapterStatus[i] = ((!Singleton<CFunctionUnlockSys>.get_instance().ChapterIsUnlock((uint)(i + 1))) ? 0 : 1);
					}
				...
				else
				{
					...
					{
						if (this.m_chapterStatus[l] == 0 && Singleton<CFunctionUnlockSys>.get_instance().ChapterIsUnlock((uint)(l + 1)))
						{
					...
					if (num >= 0)
					{
						...
						if (dataByIndex != null)
						{
							Singleton<CUIManager>.get_instance().OpenTips("Adventure_Unlock_Tips", true, 1f, null, new object[]
							{
								Utility.UTF8Convert(dataByIndex.szChapterName),
								Singleton<CTextManager>.get_instance().GetText(string.Format("Adventure_Level_{0}", num2 + 1))
							});
						}
						return;
					}
		...
		{
			if (!Singleton<SCModuleControl>.get_instance().GetActiveModule(2))
			{
			...
			if (masterRoleInfo != null)
			{
				Singleton<CBattleGuideManager>.get_instance().bTrainingAdv = false;
				if (!this.IsLevelVaild(this.currentChapter, this.currentLevelSeq, this.currentDifficulty))
				{
		...
		private void OnLevelDetail_OpenForm(CUIEvent uiEvent)
		{
			...
			if (enterAdvError == EnterAdvError.Locked)
			{
				...
				{
					Singleton<CUIManager>.get_instance().OpenTips("Level_Error_Tips_1", true, 1.5f, null, new object[0]);
				}
				else
				{
					Singleton<CUIManager>.get_instance().OpenTips("Level_Error_Tips_3", true, 1.5f, null, new object[0]);
				}
			}
			else if (enterAdvError == EnterAdvError.Other)
			{
				Singleton<CUIManager>.get_instance().OpenTips("Level_Error_Tips_2", true, 1.5f, null, new object[0]);
			}
		...
		private void OnOpenChooseHero(CUIEvent uiEvent)
		{
			...
			if (component.interactable)
			{
				...
				Singleton<CHeroSelectBaseSystem>.get_instance().SetPVEDataWithAdventure(dataByKey2.dwBattleListID, cSDT_SINGLE_GAME_OF_ADVENTURE, StringHelper.UTF8BytesToString(ref dataByKey2.szName));
				Singleton<CHeroSelectBaseSystem>.get_instance().OpenForm(enSelectGameType.enPVE_Adventure, (byte)dataByKey2.iHeroNum, 0u, 0, 0);
			}
		}
		...
		{
			if (Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(7))
			{
				Button component = uiEvent.m_srcWidget.GetComponent<Button>();
				if (component.interactable)
				{
		...
		private void ReqMopup(byte SweepCount)
		{
			...
			cSPkg.stPkgData.get_stSweepSingleGameReq().iLevelID = this.currentLevelId;
			cSPkg.stPkgData.get_stSweepSingleGameReq().bGameType = 0;
			cSPkg.stPkgData.get_stSweepSingleGameReq().dwSweepCnt = (uint)SweepCount;
		...
		private void ReqGetChapterReward()
		{
			...
			cSPkg.stPkgData.get_stGetChapterRewardReq().bChapterNo = (byte)this.currentChapter;
			cSPkg.stPkgData.get_stGetChapterRewardReq().bDifficultType = (byte)this.currentDifficulty;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		private void ReqBuyPlayTime(int BuyTime)
		{
			...
			cSPkg.stPkgData.get_stShopBuyReq().stExtraInfo.dwLevelID = (uint)this.currentLevelId;
			cSPkg.stPkgData.get_stShopBuyReq().iBuyType = 6;
			cSPkg.stPkgData.get_stShopBuyReq().iBuySubType = BuyTime;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void OnGetChapterReward(CSPkg msg)
		{
			...
			if (msg.stPkgData.get_stGetChapterRewardRsp().iErrorCode == 0)
			{
				PVE_ADV_COMPLETE_INFO pVE_ADV_COMPLETE_INFO = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().pveLevelDetail[(int)(msg.stPkgData.get_stGetChapterRewardRsp().bDifficultType - 1)];
				DebugHelper.Assert(pVE_ADV_COMPLETE_INFO != null, "PVE info is NULL!!! -- Difficulty", new object[]
				{
					msg.stPkgData.get_stGetChapterRewardRsp().bDifficultType
				});
				pVE_ADV_COMPLETE_INFO.ChapterDetailList[(int)(msg.stPkgData.get_stGetChapterRewardRsp().bChapterNo - 1)].bIsGetBonus = 1;
				ListView<CUseable> useableListFromReward = CUseableManager.GetUseableListFromReward(msg.stPkgData.get_stGetChapterRewardRsp().stRewardDetail);
				...
				if (form != null)
				{
					CAdventureView.InitChapterForm(form, (int)msg.stPkgData.get_stGetChapterRewardRsp().bChapterNo, Singleton<CAdventureSys>.get_instance().currentLevelSeq, (int)msg.stPkgData.get_stGetChapterRewardRsp().bDifficultType);
					CAdventureView.CloseChapterRewardPanel(form.gameObject);
				}
			}
			else
			{
				Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("PVE_Level_All_Chapter_Starts_Error") + Utility.ProtErrCodeToStr(1057, msg.stPkgData.get_stGetChapterRewardRsp().iErrorCode), false, 1.5f, null, new object[0]);
			}
		}
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			ResLevelCfgInfo dataByKey = GameDataMgr.levelDatabin.GetDataByKey((long)msg.stPkgData.get_stShopBuyRsp().stExtraInfo.dwLevelID);
			DebugHelper.Assert(dataByKey != null);
			...
			pVE_LEVEL_COMPLETE_INFO.PlayLimit = msg.stPkgData.get_stShopBuyRsp().iBuySubType;
		...
		private bool HasEnoughDiamondForTicket(int count, out int ReqDiamondNum)
		{
			...
			return Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().DianQuan >= (ulong)((long)ReqDiamondNum);
		}
			...
			if ((long)num <= (long)((ulong)CAdventureSys.CHALLENGE_BUYTIME_LIMIT))
			{
				...
				while (enumerator.MoveNext())
				{
					...
					if (resShopInfo.iType == 6 && resShopInfo.iSubType == num)
					{
						...
						{
							if (Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().DianQuan >= (ulong)resShopInfo.dwCoinPrice)
							{
								return 0;
							}
						...
						else
						{
							...
							if (Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().GoldCoin >= resShopInfo.dwCoinPrice)
							{
								return 0;
							}
		...
		private bool CheckFullItem(out string itemName)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				if (dataByKey.astRewardShowDetail[i].dwRewardID != 0u)
				{
					...
					{
						int useableStackCount = useableContainer.GetUseableStackCount(2, dataByKey.astRewardShowDetail[i].dwRewardID);
					...
					{
						int useableStackCount = useableContainer.GetUseableStackCount(3, dataByKey.astRewardShowDetail[i].dwRewardID);
		...
		private void OnOpenExplore(CUIEvent uiEvent)
		{
			...
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm(CAdventureSys.EXLPORE_FORM_PATH, true, true);
		...
		{
			Singleton<CUIManager>.get_instance().CloseForm(CAdventureSys.EXLPORE_FORM_PATH);
		}

		private void OnDragStart(CUIEvent uiEvent)
		{
			Singleton<CSoundManager>.get_instance().PostEvent("UI_Add_Button", null);
		}
		...
		{
			CUIFormScript form = Singleton<CUIManager>.get_instance().GetForm(CAdventureSys.EXLPORE_FORM_PATH);
			if (form != null)
			{
		...
		public static int GetLastChapter(int difficulty = 1)
		{
			...
			for (int i = 0; i < CAdventureSys.CHAPTER_NUM; i++)
			{
				if (!Singleton<CFunctionUnlockSys>.get_instance().ChapterIsUnlock((uint)(i + 1)))
				{
					break;
				}
				result = i + 1;
		...
		{
			int result = 1;
			if (!Singleton<CFunctionUnlockSys>.get_instance().ChapterIsUnlock((uint)chapterId))
			{
				return result;
			}
			if (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo() == null)
