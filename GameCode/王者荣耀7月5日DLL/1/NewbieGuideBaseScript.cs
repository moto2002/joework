		...
		while (enumerator.MoveNext())
		{
			...
			if (!(current == null))
			{
				...
				if (!(gameObject == null))
				{
					...
					if (bShowFinger)
					{
						GameObject gameObject2 = Singleton<CResourceManager>.GetInstance().GetResource("UGUI/Form/System/Dialog/WeakGuideHighlighter.prefab", typeof(GameObject), 4, false, false).m_content as GameObject;
						if (gameObject2 != null)
						{
	...
	protected void AddHighLightAreaClickAnyWhere(GameObject baseGo, CUIFormScript inOriginalForm)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (!(current == null))
			{
				...
				GameObject gameObject = Singleton<CResourceManager>.GetInstance().GetResource("UGUI/Form/System/Dialog/HighlightAreaMask.prefab", typeof(GameObject), 4, false, false).m_content as GameObject;
				if (gameObject != null)
				{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (!(gameObject == null) && !(gameObject2 == null))
				{
					...
					if (i < NewbieGuideBaseScript.ms_guideTextList.get_Count())
					{
						...
						if (specialTipConfig != null && specialTipConfig.bGuideTextPos > 0)
						{
							NewbieGuideBaseScript.ms_guideTextList.get_Item(i).CustomSetActive(gameObject.activeSelf);
