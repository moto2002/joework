...
public class SceneMgr : MonoSingleton<SceneMgr>
{
	...
	public SceneMgr()
	{
		...
		this.commonEffects = new object[]
		{
			...
			5,
			"Prefab_Skill_Effects/tongyong_effects/Indicator/select_b_01",
			10,
	...
	public void PrepareGameObjectLOD(string path, bool isParticle, enResourceType type, int count)
	{
		...
		Singleton<CGameObjectPool>.get_instance().PrepareGameObject(empty, type, count, true);
	}

	[DebuggerHidden]
	public IEnumerator PreloadCommonAssets()
	{
		SceneMgr.<PreloadCommonAssets>c__IteratorD <PreloadCommonAssets>c__IteratorD = new SceneMgr.<PreloadCommonAssets>c__IteratorD();
		<PreloadCommonAssets>c__IteratorD.<>f__this = this;
		return <PreloadCommonAssets>c__IteratorD;
	}

	[DebuggerHidden]
	public IEnumerator PreloadCommonEffects()
	{
		SceneMgr.<PreloadCommonEffects>c__IteratorE <PreloadCommonEffects>c__IteratorE = new SceneMgr.<PreloadCommonEffects>c__IteratorE();
		<PreloadCommonEffects>c__IteratorE.<>f__this = this;
		return <PreloadCommonEffects>c__IteratorE;
	}
	...
	{
		CResource resource = Singleton<CResourceManager>.GetInstance().GetResource("Config/ParticleLimit", typeof(TextAsset), 1, false, true);
		if (resource == null)
		{
			return;
		}
	...
	public GameObject Spawn(string name, SceneObjType sceneObjType, VInt3 position, VInt3 forward)
	{
		...
		if (text != null)
		{
			GameObject gameObject = Singleton<CResourceManager>.GetInstance().GetResource(text, typeof(GameObject), 0, true, false).m_content as GameObject;
			gameObject2 = Singleton<CGameObjectPool>.GetInstance().GetGameObject(text, 0);
	...
	private T LoadResource<T>(string path) where T : Object
	{
		...
		return Singleton<CResourceManager>.GetInstance().GetResource(path, typeof(T), 0, true, false).m_content as T;
	}
	...
	private GameObject _GetPooledGameObjLOD(string prefabName, bool isParticle, SceneObjType sceneObjType, Vector3 pos, Quaternion rot, bool useRotation, out bool isInit)
	{
		...
		this.GetPrefabLOD<GameObject>(prefabName, isParticle, out text);
		GameObject gameObject;
		if (useRotation)
		{
			gameObject = Singleton<CGameObjectPool>.GetInstance().GetGameObject(text, pos, rot, 0, ref isInit);
		}
		else
		{
			gameObject = Singleton<CGameObjectPool>.GetInstance().GetGameObject(text, pos, 0, ref isInit);
		}
		if (gameObject != null)
		{
	...
	{
		return new SceneMgr.<UnloadAssets_Coroutine>c__IteratorF();
	}
