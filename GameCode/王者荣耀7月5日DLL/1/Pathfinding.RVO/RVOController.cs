...
namespace Pathfinding.RVO
{
	...
	public class RVOController : MonoBehaviour, IPooledMonoBehaviour
	{
		...
		public VInt3 position
		{
			get
			{
				return this.rvoAgent.get_InterpolatedPosition();
			}
		}
		...
		public VInt3 velocity
		{
			get
			{
				return this.rvoAgent.get_Velocity();
			}
		}
		...
		public void EnsureActorAndSimulator()
		{
			...
			if (this.actor)
			{
				vInt = this.actor.get_handle().location;
			}
			else
			{
			...
			this.adjustedY = this.rvoAgent.get_Position().y;
		}
		...
		public void OnEnable()
		{
			...
			if (this.rvoAgent != null)
			{
				...
				this.rvoAgent.set_DesiredVelocity(VInt3.zero);
		...
		{
			this.rvoAgent.set_Radius(this.radius);
			this.rvoAgent.set_MaxSpeed(this.maxSpeed);
			this.rvoAgent.set_Height(this.height);
			this.rvoAgent.set_AgentTimeHorizon(this.agentTimeHorizon);
			this.rvoAgent.set_ObstacleTimeHorizon(this.obstacleTimeHorizon);
			this.rvoAgent.set_Locked(this.locked);
			this.rvoAgent.set_MaxNeighbours(this.maxNeighbours);
			this.rvoAgent.set_DebugDraw(this.debug);
			this.rvoAgent.set_NeighbourDist(this.neighbourDist);
			this.rvoAgent.set_Layer(this.layer);
			this.rvoAgent.set_CollidesWith(this.collidesWith);
		}
		...
		{
			this.actor.get_handle().location = pos;
		...
		public void DoUpdate(float dt)
		{
			...
			this.UpdateAgentProperties();
			VInt3 interpolatedPosition = this.rvoAgent.get_InterpolatedPosition();
			this.rvoAgent.SetYPosition(this.adjustedY);
			this.rvoAgent.set_DesiredVelocity(this.desiredVelocity);
			...
			if (this.checkNavNode)
			{
				...
				VInt groundY;
				VInt3 vInt2 = PathfindingUtility.Move(this.actor, delta, out groundY, out handle.hasReachedNavEdge, null);
				VInt3 vInt3 = handle.location + vInt2;
				handle.location = vInt3;
				handle.groundY = groundY;
				this.rvoAgent.Teleport(vInt3);
				this.adjustedY = vInt3.y;
			}
			else
			{
			...
			if (this.enableRotation && this.velocity != VInt3.zero)
			{
				...
				transform.rotation = Quaternion.Lerp(transform.rotation, Quaternion.LookRotation(forward), dt * this.rotationSpeed * Mathf.Min((float)this.velocity.get_magnitude(), 0.2f));
			}
		}
