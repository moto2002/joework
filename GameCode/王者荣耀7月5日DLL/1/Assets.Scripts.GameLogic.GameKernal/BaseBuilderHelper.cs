	...
	internal class BaseBuilderHelper
	{
		...
		public virtual void PreLoad()
		{
			...
			if (!MonoSingleton<GameFramework>.get_instance().EditorPreviewMode)
			{
				...
				{
					MonoSingleton<GameLoader>.get_instance().AddLevel(Singleton<GameContextEx>.GetInstance().GameContextCommonInfo.MapDesignFileName);
				}
				else
				{
					MonoSingleton<GameLoader>.get_instance().AddDesignSerializedLevel(Singleton<GameContextEx>.GetInstance().GameContextCommonInfo.MapDesignFileName);
					MonoSingleton<GameLoader>.get_instance().AddArtistSerializedLevel(Singleton<GameContextEx>.GetInstance().GameContextCommonInfo.MapArtistFileName);
				}
		...
		public virtual void OnGameLoadComplete()
		{
			...
			if (!Singleton<GameContextEx>.GetInstance().IsMultiPlayerGame())
			{
				...
				{
					flag = true;
					MonoSingleton<DialogueProcessor>.get_instance().PlayDrama(preDialogId, hostPlayer.Captain.get_handle().gameObject, hostPlayer.Captain.get_handle().gameObject, true);
				}
				if (!flag)
				{
