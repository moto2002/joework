...
namespace Assets.Scripts.GameLogic.GameKernal
{
	public class Player
	{
		...
		public void SetCaptain(uint configId)
		{
			...
			this.Captain = this._heroes.Find((PoolObjHandle<ActorRoot> hero) => (long)hero.get_handle().TheActorMeta.ConfigId == (long)((ulong)configId));
			if (this.PlayerId == Singleton<GamePlayerCenter>.get_instance().HostPlayerId)
			{
		...
		public bool IsAllHeroesDead()
		{
			...
			{
				if (!this._heroes.get_Item(i).get_handle().ActorControl.IsDeadState)
				{
					return false;
				}
			}
		...
		public bool IsMyTeamOutOfBattle()
		{
			for (int i = 0; i < this._heroes.get_Count(); i++)
			{
				if (this._heroes.get_Item(i).get_handle().ActorControl.IsInBattle)
				{
					return false;
				}
			}
		...
		private static int GetHeroCombatEft(PoolObjHandle<ActorRoot> actor)
		{
			...
			if (actorDataProvider.GetActorServerData(ref actor.get_handle().TheActorMeta, ref actorServerData))
			{
				...
				for (int i = 0; i < 30; i++)
				{
					if (actorDataProvider.GetActorServerRuneData(ref actor.get_handle().TheActorMeta, (ActorRunelSlot)i, ref actorServerRuneData))
					{
