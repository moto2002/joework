...
public class CChatView
{
	...
	public class CChatViewEntryNode
	{
		...
		public void Refresh_EntryForm(GameObject entryGameObject)
		{
			...
			else if (entryEntity.TextObjList.get_Count() > 0)
			{
				for (int i = 0; i < entryEntity.TextObjList.get_Count(); i++)
				{
					CTextImageNode cTextImageNode = entryEntity.TextObjList.get_Item(i);
					if (cTextImageNode.bText)
					{
						this.Create_Content(gameObject, cTextImageNode.content, true, cTextImageNode.posX, cTextImageNode.posY, cTextImageNode.width, entryEntity.type == EChaterType.Self, false, true);
					}
					else
					{
						this.Create_Content(gameObject, cTextImageNode.content, false, cTextImageNode.posX, cTextImageNode.posY, cTextImageNode.width, entryEntity.type == EChaterType.Self, false, true);
					}
				}
			}
			else
			{
				this.Create_Content(gameObject, Singleton<CTextManager>.get_instance().GetText("ChatEntry_Default_Text"), true, 0f, 0f, 200f, true, false, true);
				this.SetEntryChannelImage(EChatChannel.Default);
			}
			this.SetBubbleCount(Singleton<CChatController>.get_instance().model.channelMgr.GetAllFriendUnreadCount());
		}
	...
	public EChatChannel CurTab
	{
		...
		set
		{
			...
			case EChatChannel.Room:
				...
				break;
			case EChatChannel.Lobby:
				if (Singleton<CChatController>.get_instance().view != null && Singleton<CLoudSpeakerSys>.get_instance().CurLoudSpeaker != null)
				{
					Singleton<CChatController>.get_instance().view.ShowLoudSpeaker(true, Singleton<CLoudSpeakerSys>.get_instance().CurLoudSpeaker);
				}
				this.SetShow(this.LobbyScript_cg, true);
				this.Refresh_ChatEntity_List(true, EChatChannel.None);
				break;
	...
	public void Update()
	{
		...
		if (text != null && text.get_Length() > CChatView.ChatMaxLength)
		{
			...
			Singleton<CUIManager>.GetInstance().OpenMessageBox(string.Format(Singleton<CTextManager>.get_instance().GetText("chat_input_max"), CChatView.ChatMaxLength), false);
		}
	}
	...
	private void Init()
	{
		Singleton<CChatController>.get_instance().model.channelMgr.GetChannel(EChatChannel.Lobby).Init_Timer();
		...
		this.curChannels = Singleton<CChatController>.get_instance().model.channelMgr.CurActiveChannels;
	...
	public void BuildTabList(List<uint> list, int index = 0)
	{
		...
		for (int i = 0; i < this.listScript.m_elementAmount; i++)
		{
			...
			component2.text = Singleton<CTextManager>.get_instance().GetText(CChatUT.GetEChatChannel_Text((EChatChannel)list.get_Item(i)));
		}
	...
	public void ReBuildTabText()
	{
		...
		for (int i = 0; i < this.curChannels.get_Count(); i++)
		{
			...
			component.text = Singleton<CTextManager>.get_instance().GetText(CChatUT.GetEChatChannel_Text((EChatChannel)this.curChannels.get_Item(i)));
		}
	}
	...
	public void ShowDetailChatForm()
	{
		...
		this.BuildTabList(this.curChannels, 0);
		this.CurTab = Singleton<CChatController>.get_instance().model.sysData.LastChannel;
		int index = 0;
		for (int i = 0; i < this.curChannels.get_Count(); i++)
		{
	...
	public void Refresh_EntryForm()
	{
		if (this.m_viewEntryNode != null)
		{
			...
			if (form != null && !form.IsHided())
			{
				...
				this.m_viewEntryNode.Refresh_EntryForm(gameObject);
			}
			CUIFormScript form2 = Singleton<CUIManager>.get_instance().GetForm(CRoomSystem.PATH_ROOM);
			if (form2 != null && !form2.IsHided())
			{
				GameObject gameObject2 = form2.transform.Find("entry_node").gameObject;
				this.m_viewEntryNode.Refresh_EntryForm(gameObject2);
			}
			CUIFormScript form3 = Singleton<CUIManager>.get_instance().GetForm(CMatchingSystem.PATH_MATCHING_MULTI);
			if (form3 != null && !form3.IsHided())
			{
				GameObject gameObject3 = form3.transform.Find("entry_node").gameObject;
				this.m_viewEntryNode.Refresh_EntryForm(gameObject3);
			}
			CUIFormScript form4 = Singleton<CUIManager>.get_instance().GetForm(SettlementSystem.SettlementFormName);
			if (form4 != null && !form4.IsHided())
			{
				GameObject gameObject4 = form4.transform.Find("Panel/entry_node").gameObject;
				this.m_viewEntryNode.Refresh_EntryForm(gameObject4);
			}
			CUIFormScript form5 = Singleton<CUIManager>.get_instance().GetForm(CGuildMatchSystem.GuildMatchFormPath);
			if (form5 != null && !form5.IsHided())
			{
				GameObject gameObject5 = form5.transform.Find("entry_node").gameObject;
				this.m_viewEntryNode.Refresh_EntryForm(gameObject5);
			}
		}
	}
	...
	{
		this.info_node_obj.CustomSetActive(true);
		bool flag = Singleton<CFriendContoller>.get_instance().model.IsAnyFriendExist(true);
		if (!Singleton<CFriendContoller>.get_instance().model.IsAnyFriendExist(false))
		{
	...
	public void On_Friend_TabList_Selected(CUIEvent uiEvent)
	{
		...
		DebugHelper.Assert(uiEvent.m_srcWidgetIndexInBelongedList <= this.friendTablist.get_Count() - 1, "---Chat, On_Friend_TabList_Selected");
		if (uiEvent.m_srcWidgetIndexInBelongedList > this.friendTablist.get_Count() - 1)
		{
			return;
		}
		int selectedIndex = (uiEvent.m_srcWidgetScript as CUIListScript).GetSelectedIndex();
		COMDT_FRIEND_INFO cOMDT_FRIEND_INFO = this.friendTablist.get_Item(selectedIndex);
		...
		COfflineChatIndex cOfflineChatIndex = Singleton<CChatController>.get_instance().model.GetCOfflineChatIndex(cOMDT_FRIEND_INFO.stUin.ullUid, cOMDT_FRIEND_INFO.stUin.dwLogicWorldId);
		if (cOfflineChatIndex != null)
		{
			CChatNetUT.Send_Clear_Offline(cOfflineChatIndex.indexList);
			Singleton<CChatController>.get_instance().model.ClearCOfflineChatIndex(cOfflineChatIndex);
		}
	...
	{
		CChatModel model = Singleton<CChatController>.get_instance().model;
		...
		for (int i = 0; i < this.curChannels.get_Count(); i++)
		{
			...
			if (!(elemenet == null))
			{
				if (num2 > 0)
				{
					CUICommonSystem.AddRedDot(elemenet.gameObject, enRedDotPos.enTopRight, num2);
				}
				else
				{
	...
	private int GetUnReadCount(EChatChannel channelType)
	{
		...
		{
			result = Singleton<CChatController>.get_instance().model.channelMgr.GetFriendTotal_UnreadCount();
		}
		else
		{
			CChatChannel channel = Singleton<CChatController>.get_instance().model.channelMgr.GetChannel(channelType);
			if (channel != null)
			{
	...
	public void Refresh_ChatEntity_List(bool bForce = true, EChatChannel tab = EChatChannel.None)
	{
		...
		{
			CChatSysData sysData = Singleton<CChatController>.get_instance().model.sysData;
	...
	public void Refresh_ChatInputView()
	{
		...
		if (this.CurTab == EChatChannel.Lobby)
		{
			CChatChannel channel = Singleton<CChatController>.get_instance().model.channelMgr.GetChannel(EChatChannel.Lobby);
			CChatController.enCheckChatResult enCheckChatResult = Singleton<CChatController>.get_instance().CheckSend(EChatChannel.Lobby, string.Empty, false);
			if (enCheckChatResult == CChatController.enCheckChatResult.CdLimit)
			{
				this.inputField.placeholder.GetComponent<Text>().text = string.Format(Singleton<CTextManager>.get_instance().GetText("Chat_Common_Tips_2"), channel.Get_Left_CDTime());
			}
			else if (!CSysDynamicBlock.bChatPayBlock)
			{
				this.inputField.placeholder.GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("Chat_Common_Tips_1", new string[]
				{
			...
			{
				this.inputField.placeholder.GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("Chat_Common_Tips_5");
		...
		{
			this.inputField.placeholder.GetComponent<Text>().text = Singleton<CTextManager>.get_instance().GetText("Chat_Common_Tips_11");
			this.sendBtn.transform.GetChild(0).gameObject.CustomSetActive(false);
			this.sendBtn.transform.GetChild(1).gameObject.CustomSetActive(true);
		}
		else
	...
	public void On_List_ElementEnable(CUIEvent uievent)
	{
		...
		{
			cChatEntity = listView.get_Item(srcWidgetIndexInBelongedList);
		}
		if (cChatEntity != null && uievent.m_srcWidget != null)
		{
			this.Show_ChatItem(uievent.m_srcWidget, cChatEntity);
		}
		if (srcWidgetIndexInBelongedList == listView.get_Count() - 1 && this.bubbleNode != null)
		{
			this.bubbleNode.CustomSetActive(false);
		}
	}
	...
	public void ShowLoudSpeaker(bool bShow, COMDT_CHAT_MSG_HORN data = null)
	{
		...
		if (data != null)
		{
			...
			CChatUT.GetUser(EChaterType.LoudSpeaker, data.stFrom.ullUid, (uint)data.stFrom.iLogicWorldID, out text, out text2, out url, out cOMDT_GAME_VIP_CLIENT);
			componetInChild2.text = text;
			componetInChild3.text = Utility.UTF8Convert(data.szContent);
			componetInChild6.text = text2;
			UT.SetHttpImage(componetInChild, url);
			if (cOMDT_GAME_VIP_CLIENT != null)
			{
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(componetInChild4, (int)cOMDT_GAME_VIP_CLIENT.dwCurLevel, false);
	...
	private void _refresh_list(CUIListScript listScript, ListView<CChatEntity> data_list, bool bShow_Last, List<Vector2> sizeVec, CChatChannel channel)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				this.Show_ChatItem(elemenet.gameObject, data_list.get_Item(i));
				start_index = i;
			}
		}
		if (!this.bRefreshNew)
	...
	private void calc_size(ListView<CChatEntity> data_list, List<Vector2> sizeVec)
	{
		...
		sizeVec.Clear();
		for (int i = 0; i < data_list.get_Count(); i++)
		{
			CChatEntity cChatEntity = data_list.get_Item(i);
	...
	private void _refresh_friends_list(CUIListScript listScript, ListView<COMDT_FRIEND_INFO> data_list)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (elemenet != null && listScript.IsElementInScrollArea(i))
			{
				this.Show_FriendTabItem(elemenet.gameObject, data_list.get_Item(i));
			}
		}
	}
	...
	public void On_FriendsList_ElementEnable(CUIEvent uievent)
	{
		...
		if (srcWidgetIndexInBelongedList >= 0 && srcWidgetIndexInBelongedList < listView.get_Count())
		{
			cOMDT_FRIEND_INFO = listView.get_Item(srcWidgetIndexInBelongedList);
		}
	...
	public void Show_FriendTabItem(GameObject node, COMDT_FRIEND_INFO info)
	{
		...
		if (component2 != null)
		{
			bool flag = Singleton<CFriendContoller>.get_instance().model.IsFriendOfflineOnline(info.stUin.ullUid, info.stUin.dwLogicWorldId);
		...
		if (info.stGameVip != null)
		{
			...
			MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(gameObject2.GetComponent<Image>(), (int)info.stGameVip.dwCurLevel, false);
		...
		{
			component3.text = Singleton<CTextManager>.get_instance().GetText("Chat_Common_Tips_3");
		}
	...
	{
		if (ent.TextObjList == null || ent.TextObjList.get_Count() == 0)
		{
			return;
		}
		...
		{
			bool flag = Singleton<CFriendContoller>.get_instance().model.IsFriendOfflineOnline(ent.ullUid, ent.iLogicWorldID);
			UT.SetImage(component2, !flag);
		}
		if (ent.stGameVip != null)
		{
			...
			if (gameObject2 != null)
			{
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(gameObject2.GetComponent<Image>(), (int)ent.stGameVip.dwCurLevel, false);
			}
		...
		if (gameObject4 != null)
		{
			...
			{
				Object.Destroy(gameObject4.transform.GetChild(i).gameObject);
			}
		}
		bool flag2 = false;
		for (int j = 0; j < ent.TextObjList.get_Count(); j++)
		{
			CTextImageNode cTextImageNode = ent.TextObjList.get_Item(j);
			if (cTextImageNode.bText)
			{
				if (!flag2)
				{
				...
				if (ent.stGameVip != null)
				{
					this.Create_Content(gameObject4, cTextImageNode.content, true, cTextImageNode.posX, cTextImageNode.posY, cTextImageNode.width, ent.type == EChaterType.Self, ent.stGameVip.dwCurLevel > 0u, false);
				}
			}
			else if (ent.stGameVip != null)
			{
				this.Create_Content(gameObject4, cTextImageNode.content, false, cTextImageNode.posX, cTextImageNode.posY, cTextImageNode.width, ent.type == EChaterType.Self, ent.stGameVip.dwCurLevel > 0u, false);
			}
		}
