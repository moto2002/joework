...
internal class SetHeroCustomEquipCommand : CheatCommandNetworking
{
	...
	{
		CheatCmdRef.set_stSetHeroCustomEquip(new CSDT_CHEAT_SET_HERO_CUSTOM_EQUIP());
		CheatCmdRef.get_stSetHeroCustomEquip().dwHeroID = CheatCommandBase.SmartConvert<uint>(InArguments[0]);
		CheatCmdRef.get_stSetHeroCustomEquip().bForceRefresh = CheatCommandBase.SmartConvert<byte>(InArguments[7]);
		CheatCmdRef.get_stSetHeroCustomEquip().bGameResult = CheatCommandBase.SmartConvert<byte>(InArguments[8]);
		for (int i = 0; i < 6; i++)
		{
			CheatCmdRef.get_stSetHeroCustomEquip().EquipID[i] = CheatCommandBase.SmartConvert<uint>(InArguments[i + 1]);
		}
