...
public class CheatCommandMethod : CheatCommandBase
{
	...
	protected void CacheArgumentDescriptions()
	{
		...
		if (parameters != null && parameters.Length > 0)
		{
			...
			{
				this.ArgumentDescs[i] = new ArgumentDescriptionAttribute(ArgumentDescriptionAttribute.EDefaultValueTag.Tag, i, parameters[i].get_ParameterType(), parameters[i].get_Name(), (parameters[i].get_DefaultValue() == null) ? string.Empty : parameters[i].get_DefaultValue().ToString(), new object[0]);
			}
		}
	}
