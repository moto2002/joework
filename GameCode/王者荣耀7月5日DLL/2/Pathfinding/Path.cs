...
namespace Pathfinding
{
	public abstract class Path
	{
		...
		{
			Path.<WaitForPath>c__Iterator7 <WaitForPath>c__Iterator = new Path.<WaitForPath>c__Iterator7();
		...
		public uint CalculateHScore(GraphNode node)
		{
			...
			case Heuristic.Manhattan:
			{
				...
				uint num2 = (this.hTargetNode == null) ? 0u : AstarPath.active.euclideanEmbedding.GetHeuristic(node.NodeIndex, this.hTargetNode.NodeIndex);
			...
			case Heuristic.DiagonalManhattan:
			{
				...
				uint num = (uint)((float)(14 * num3 / 10 + (num4 - num3) + vInt.y) * this.heuristicScale);
				uint num2 = (this.hTargetNode == null) ? 0u : AstarPath.active.euclideanEmbedding.GetHeuristic(node.NodeIndex, this.hTargetNode.NodeIndex);
				return Math.Max(num, num2);
			}
		...
		public virtual string DebugString(PathLog logMode)
		{
			...
			if (!this.error)
			{
				...
				debugStringBuilder.Append((this.path != null) ? this.path.get_Count().ToString() : "Null");
