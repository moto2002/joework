...
public class AstarPath : MonoBehaviour
{
	...
	{
		AstarPath.<DelayedGraphUpdate>c__Iterator4 <DelayedGraphUpdate>c__Iterator = new AstarPath.<DelayedGraphUpdate>c__Iterator4();
	...
	{
		AstarPath.<UpdateGraphsInteral>c__Iterator5 <UpdateGraphsInteral>c__Iterator = new AstarPath.<UpdateGraphsInteral>c__Iterator5();
	...
	private void QueueGraphUpdatesInternal()
	{
		...
		while (this.graphUpdateQueue.get_Count() > 0)
		{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					{
						AstarPath.GUOSingle gUOSingle = default(AstarPath.GUOSingle);
						gUOSingle.order = AstarPath.GraphUpdateOrder.GraphUpdate;
						gUOSingle.obj = graphUpdateObject;
						gUOSingle.graph = updatableGraph;
		...
		if (flag)
		{
			AstarPath.GUOSingle gUOSingle2 = default(AstarPath.GUOSingle);
			gUOSingle2.order = AstarPath.GraphUpdateOrder.FloodFill;
	...
	public void ApplyLinks()
	{
		if (this.astarData.userConnections != null && this.astarData.userConnections.Length > 0)
		{
			...
			for (int i = 0; i < this.astarData.userConnections.Length; i++)
			{
				...
				GameObject gameObject2 = (!dictionary.ContainsKey((VInt3)userConnection.p1)) ? new GameObject("Link " + i) : dictionary.get_Item((VInt3)userConnection.p1);
				GameObject gameObject3 = (!dictionary.ContainsKey((VInt3)userConnection.p2)) ? new GameObject("Link " + i) : dictionary.get_Item((VInt3)userConnection.p2);
	...
	public void ReturnPaths(bool timeSlice)
	{
		...
		long num = (!timeSlice) ? 0L : (DateTime.get_UtcNow().get_Ticks() + 10000L);
		int num2 = 0;
	...
	{
		AstarPath.<CalculatePaths>c__Iterator6 <CalculatePaths>c__Iterator = new AstarPath.<CalculatePaths>c__Iterator6();
