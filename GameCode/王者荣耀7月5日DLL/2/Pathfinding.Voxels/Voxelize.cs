			...
			for (int num14 = 0; num14 < list.get_Count(); num14++)
			{
				...
				if (this.CalcAreaOfPolygon2D(voxelContour2.verts, voxelContour2.nverts) < 0)
				{
					...
					for (int num16 = 0; num16 < list.get_Count(); num16++)
					{
						if (num14 != num16)
						{
							if (list.get_Item(num16).nverts > 0 && list.get_Item(num16).reg == voxelContour2.reg && this.CalcAreaOfPolygon2D(list.get_Item(num16).verts, list.get_Item(num16).nverts) > 0)
							{
								num15 = num16;
								break;
							}
				...
				while (n < simplified.get_Count() / 4)
				{
					...
					if (flag4)
					{
						...
						if ((float)(num39 * num39 + num40 * num40) > num29 * num29)
						{
							...
							{
								int num41 = (num36 >= num33) ? (num36 - num33) : (num36 + num14 - num33);
								num37 = (num33 + num41 / 2) % num14;
							}
							else
							{
								int num42 = (num36 >= num33) ? (num36 - num33) : (num36 + num14 - num33);
							...
							for (int m = num6; m <= num8; m++)
							{
								...
								if (num11 >= 3)
								{
									...
									if (num11 >= 3)
									{
										...
										int num15 = AstarMath.Clamp((int)Math.Round((double)num12), 0, this.voxelArea.depth - 1);
										int num16 = AstarMath.Clamp((int)Math.Round((double)num13), 0, this.voxelArea.depth - 1);
										for (int num17 = num15; num17 <= num16; num17++)
										{
											int num18 = Utility.ClipPolygon(array3, num11, array4, 1f, (float)(-(float)num17) + 0.5f, 2);
											if (num18 >= 3)
											{
												...
												if (num18 >= 3)
												{
													...
													if (num23 >= 0)
													{
														...
														this.voxelArea.AddLinkedSpan(num17 * this.voxelArea.width + m, (uint)((num24 < 0) ? 0 : num24), (uint)num23, area2, this.voxelWalkableClimb);
													}
												}
											}
										}
					...
					if (linkedSpans[j + i].bottom != 4294967295u)
					{
						for (int num4 = j + i; num4 != -1; num4 = linkedSpans[num4].next)
						{
							if (linkedSpans[num4].area != 0)
							{
								...
								int num5 = (int)((linkedSpans[num4].next == -1) ? 65536u : linkedSpans[linkedSpans[num4].next].bottom);
								int num6 = 65536;
								...
								for (int k = 0; k < 4; k++)
								{
									...
									int num13 = (int)((linkedSpans[num11].bottom == 4294967295u) ? 65536u : linkedSpans[num11].bottom);
									...
									if (linkedSpans[num11].bottom != 4294967295u)
									{
										for (int num14 = num11; num14 != -1; num14 = linkedSpans[num14].next)
										{
											...
											num13 = (int)((linkedSpans[num14].next == -1) ? 65536u : linkedSpans[linkedSpans[num14].next].bottom);
											if ((long)(Math.Min(num5, num13) - Math.Max(top, num12)) > (long)((ulong)voxelWalkableHeight))
											{
