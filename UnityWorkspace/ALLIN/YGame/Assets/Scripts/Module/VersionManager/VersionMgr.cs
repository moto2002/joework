...
public class VersionMgr : Singleton<VersionMgr>
{
    ...
    public void InitStart(Action callback)
    {
        ...
        if (GameConst.DebugMode)//测试模式，不进行解包和更新
        {
    ...
    {
        bool isExists = Directory.Exists(MyUnityTools.DataPath) && File.Exists(MyUnityTools.DataPath + m_versionTxtName);
        return isExists;
    }
    ...
    private void AllDownloadTaskFinished(bool updateVersionTxt)
    {
        ...
        if (updateVersionTxt)
        {
            ...
            MyTools.CreateFile(this.m_localPath, m_versionTxtName, this.m_remoteVersionData);
