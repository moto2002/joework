	...
	public class BattleStatView
	{
		private class HeroItem
		{
			...
			public void updateHeroValue(Player curPlayer)
			{
				...
				{
					ValueDataInfo[] actorValue = curPlayer.Captain.get_handle().ValueComponent.mActorValue.GetActorValue();
		...
		{
			if (uievent.m_eventParams.commonUInt64Param1 == Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().PlayerUId)
			{
				return;
			}
			MonoSingleton<VoiceSys>.get_instance().SwitchForbidden(uievent.m_eventParams.commonUInt64Param1);
		...
		private void UpdateSortBtn()
		{
			if (this.sortByCoinBtn != null)
			{
				...
				if (componetInChild != null)
				{
					if (this.m_sortByCoin)
					{
						componetInChild.text = Singleton<CTextManager>.get_instance().GetText("Battle_Statistic_Sort_Coin");
					}
					else
					{
						componetInChild.text = Singleton<CTextManager>.get_instance().GetText("Battle_Statistic_Sort_Common");
					}
				}
			}
		}
