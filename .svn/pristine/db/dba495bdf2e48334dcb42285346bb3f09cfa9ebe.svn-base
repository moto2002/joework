...
namespace Assets.Scripts.GameSystem
{
	public class CBattleSystem : Singleton<CBattleSystem>
	{
		...
		public CUIFormScript LoadForm(CBattleSystem.FormType formType)
		{
			...
			{
				return Singleton<CUIManager>.GetInstance().OpenForm(WatchForm.s_watchUIForm, false, true);
			}
			return null;
		}
		...
		private void OnPlayerSpawnBuff(ref SpawnBuffEventParam _param)
		{
			if (_param.src)
			{
				...
				{
					this.CreateRestrictFloatText(_param.showType, (Vector3)_param.src.get_handle().location);
				}
				else if (_param.floatTextID > 0u)
				{
					this.CreateSpecifiedFloatText(_param.floatTextID, (Vector3)_param.src.get_handle().location);
				}
			}
		}
		...
		private void OnActorGoldCoinInBattleChanged(PoolObjHandle<ActorRoot> actor, int changeValue, bool isIncome, PoolObjHandle<ActorRoot> target)
		{
			...
			{
				this.m_battleEquipSystem.OnActorGoldChange(changeValue, actor.get_handle().ValueComponent.GetGoldCoinInBattle());
			}
		}
