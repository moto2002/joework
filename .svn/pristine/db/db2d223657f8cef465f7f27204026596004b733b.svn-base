	...
	public class KDAStat
	{
		...
		protected void recordDead(PoolObjHandle<ActorRoot> src, PoolObjHandle<ActorRoot> atker)
		{
			...
			if (src && this.m_arrDeadPosition != null && this.m_numDead <= 50)
			{
				...
				if (this.m_arrDeadPosition[this.m_numDead - 1] != null)
				{
					this.m_arrDeadPosition[this.m_numDead - 1].iXPos = src.get_handle().location.x;
					this.m_arrDeadPosition[this.m_numDead - 1].iZPos = src.get_handle().location.z;
				}
			}
		}
		...
		{
			DebugHelper.Assert(src && src.get_handle().ActorControl != null, "invalid source data.");
			if (src && src.get_handle().ActorControl != null)
			{
				List<KeyValuePair<uint, ulong>>.Enumerator enumerator = src.get_handle().ActorControl.hurtSelfActorList.GetEnumerator();
				while (enumerator.MoveNext())
				{
					...
					if (current.get_Key() != atker.get_handle().ObjID)
					{
						goto IL_8D;
					}
					continue;
					...
					if (killer)
					{
						...
						if (current2.get_Key() == killer.get_handle().ObjID)
						{
							continue;
						}
					}
					KeyValuePair<uint, ulong> current3 = enumerator.get_Current();
					if (current3.get_Key() == self.get_handle().ObjID)
					{
						this.m_numAssist++;
						return;
					}
			...
			if (killer && killer.get_handle().ActorControl != null)
			{
				List<KeyValuePair<uint, ulong>>.Enumerator enumerator2 = killer.get_handle().ActorControl.helpSelfActorList.GetEnumerator();
				while (enumerator2.MoveNext())
				{
					...
					if (current4.get_Key() != atker.get_handle().ObjID)
					{
						goto IL_15F;
					}
					continue;
					...
					if (current5.get_Key() != killer.get_handle().ObjID)
					{
						KeyValuePair<uint, ulong> current6 = enumerator2.get_Current();
						if (current6.get_Key() == self.get_handle().ObjID)
						{
							this.m_numAssist++;
							return;
						}
