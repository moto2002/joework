...
namespace Assets.Scripts.GameSystem
{
	public class CMallSymbolGiftController : Singleton<CMallSymbolGiftController>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler<CSPkg>("ShopBuyDraw", new Action<CSPkg>(CMallSymbolGiftController.ReceiveLotteryRes));
			Singleton<EventRouter>.get_instance().AddEventHandler<CSPkg>("ShopBuyDraw", new Action<CSPkg>(CMallSymbolGiftController.ReceiveLotteryRes));
		}
		...
		private void PlayLotteryAnimation(CUIFormScript form, string state, bool PlayAni = true, bool PlayFx = false, bool enableEndEvent = false)
		{
			if (state != null)
			{
				if (CMallSymbolGiftController.<>f__switch$map3 == null)
				{
					Dictionary<string, int> dictionary = new Dictionary<string, int>(1);
					dictionary.Add("Open", 0);
					CMallSymbolGiftController.<>f__switch$map3 = dictionary;
				}
				int num;
				if (CMallSymbolGiftController.<>f__switch$map3.TryGetValue(state, ref num))
				{
					if (num == 0)
					{
		...
		private void RefreshDesc(CUIFormScript form)
		{
			...
			{
				componetInChild.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Mall_Symbol_Gifts_Common_Buy_Dsc"), (int)this.m_CommonDrawSymbolCycle - CMallSymbolGiftController.GetTotalDrawedCnt(10) % (int)this.m_CommonDrawSymbolCycle);
			}
			if (componetInChild2 != null)
			{
				componetInChild2.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Mall_Symbol_Gifts_Senior_Buy_Dsc"), (int)this.m_SeniorDrawSymbolCycle - CMallSymbolGiftController.GetTotalDrawedCnt(11) % (int)this.m_SeniorDrawSymbolCycle);
			}
		}
		...
		private void SendLotteryMsg(RES_SHOPBUY_TYPE LotteryType, RES_SHOPDRAW_SUBTYPE LotterySubType)
		{
			...
			cSPKG_CMD_SHOPBUY.iBuyType = LotteryType;
			cSPKG_CMD_SHOPBUY.iBuySubType = LotterySubType;
			cSPkg.stPkgData.set_stShopBuyReq(cSPKG_CMD_SHOPBUY);
			...
			if (bRewardCnt != 0)
			{
				if (bRewardCnt != 1)
				{
					...
					while ((int)b < instance.m_ItemCellFlag.Length)
					{
						...
						if (CMallSymbolGiftController.IsAdvSymbol(instance.astRewardList[(int)(b - 1)]))
						{
							...
							instance.m_AdvSymbolIds[(int)b] = instance.astRewardList[(int)(b - 1)].stRewardInfo.get_stSymbol().dwSymbolID;
						}
						else
						{
				...
				else
				{
					...
					if (CMallSymbolGiftController.IsAdvSymbol(instance.astRewardList[0]))
					{
						...
						instance.m_AdvSymbolIds[0] = instance.astRewardList[0].stRewardInfo.get_stSymbol().dwSymbolID;
					}
		...
		private static void SetRewardItemCell(CUIFormScript formScript, byte itemCellIndex, COMDT_REWARD_INFO stRewardInfo)
		{
			...
			{
			case 1:
				itemUseable = CUseableManager.CreateUseable(2, stRewardInfo.stRewardInfo.get_stItem().dwItemID, (int)stRewardInfo.stRewardInfo.get_stItem().dwCnt);
				break;
			case 2:
				itemUseable = CUseableManager.CreateVirtualUseable(enVirtualItemType.enExp, (int)stRewardInfo.stRewardInfo.get_stExp().bReserve);
				break;
			case 4:
				itemUseable = CUseableManager.CreateUseable(3, stRewardInfo.stRewardInfo.get_stEquip().dwEquipID, (int)stRewardInfo.stRewardInfo.get_stEquip().dwCnt);
				break;
			case 5:
				itemUseable = CUseableManager.CreateUseable(4, stRewardInfo.stRewardInfo.get_stHero().dwHeroID, (int)stRewardInfo.stRewardInfo.get_stHero().dwCnt);
				break;
			case 6:
				itemUseable = CUseableManager.CreateUseable(5, stRewardInfo.stRewardInfo.get_stSymbol().dwSymbolID, 0);
				break;
			case 10:
				itemUseable = CUseableManager.CreateUseable(7, stRewardInfo.stRewardInfo.get_stSkin().dwSkinID, (int)stRewardInfo.stRewardInfo.get_stSkin().dwCnt);
				break;
			case 11:
			...
			case 16:
				...
				break;
			case 20:
				itemUseable = CUseableManager.CreateUseable(8, stRewardInfo.stRewardInfo.get_stHeadImage().dwHeadImgID, (int)stRewardInfo.stRewardInfo.get_stHeadImage().dwGetTime);
				break;
			}
		...
		public static bool IsAdvSymbol(COMDT_REWARD_INFO reward)
		{
			if (reward.bType == 6)
			{
				CSymbolItem cSymbolItem = CUseableManager.CreateUsableByServerType(6, 1, reward.stRewardInfo.get_stSymbol().dwSymbolID) as CSymbolItem;
				if (cSymbolItem == null)
				{
					return false;
				}
		...
		private bool HasFreeDraw(COM_SHOP_DRAW_TYPE drawType)
		{
			...
			return masterRoleInfo != null && drawType >= 0 && drawType <= masterRoleInfo.m_freeDrawInfo.Length && masterRoleInfo.m_freeDrawInfo[drawType].dwLeftFreeDrawCnt > 0;
		}
		...
		public static void ResetFreeDrawCD(COM_SHOP_DRAW_TYPE drawType)
		{
			...
			if (drawType < 0 || drawType > masterRoleInfo.m_freeDrawInfo.Length)
			{
				return;
			}
			masterRoleInfo.m_freeDrawInfo[drawType].dwLeftFreeDrawCnt = 0;
			int num = 0;
			if (drawType != 3)
			{
				if (drawType == 4)
			...
			masterRoleInfo.m_freeDrawInfo[drawType].dwLeftFreeDrawCD = CRoleInfo.GetCurrentUTCTime() + num;
		...
		private void OnLotteryBuySymbolConfirm(CUIEvent uiEvent)
		{
			...
			RES_SHOPBUY_TYPE tag = uiEvent.m_eventParams.tag;
			RES_SHOPDRAW_SUBTYPE tag2 = uiEvent.m_eventParams.tag2;
		...
		private void TryToPayForLotterySymbol(RES_SHOPBUY_TYPE lotteryType, RES_SHOPDRAW_SUBTYPE lotterySubType)
		{
			...
			uIEvent.m_eventParams.tag = lotteryType;
			uIEvent.m_eventParams.tag2 = lotterySubType;
		...
		private void OnLotteryShowRewardEnd(CUIEvent uiEvent)
		{
			...
			for (int i = 0; i < this.m_AdvSymbolIds.Length; i++)
			{
				...
				{
					CUseable useable = CUseableManager.CreateUseable(5, this.m_AdvSymbolIds[i], 1);
			...
			for (int j = 0; j < this.m_ItemCellFlag.Length; j++)
			{
				if (this.m_ItemCellFlag[j])
				{
					...
					if (j == 0)
					{
						cUseable = CUseableManager.CreateUseable(5, this.astRewardList[j].stRewardInfo.get_stSymbol().dwSymbolID, 1);
					}
					else
					{
						cUseable = CUseableManager.CreateUseable(5, this.astRewardList[j - 1].stRewardInfo.get_stSymbol().dwSymbolID, 1);
					}
					if (cUseable != null)
					{
		...
		public static void ReceiveLotteryRes(CSPkg msg)
		{
			...
			CMallSymbolGiftController instance = Singleton<CMallSymbolGiftController>.GetInstance();
			SCPKG_CMD_SHOPBUY stShopBuyRsp = msg.stPkgData.get_stShopBuyRsp();
			RES_SHOPBUY_TYPE iBuyType = stShopBuyRsp.iBuyType;
			RES_SHOPDRAW_SUBTYPE iBuySubType = stShopBuyRsp.iBuySubType;
			RES_SHOPBUY_TYPE rES_SHOPBUY_TYPE = iBuyType;
			if (rES_SHOPBUY_TYPE != 10)
			{
				if (rES_SHOPBUY_TYPE == 11)
				{
					if (iBuySubType == 1)
					{
						CMallSymbolGiftController.ResetFreeDrawCD(4);
					...
					if (iBuySubType >= 1 && iBuySubType < 5)
					{
						CMallSymbolGiftController.SymbolGiftSeniorDrawedCntInfo[iBuySubType]++;
						if (CMallSymbolGiftController.GetTotalDrawedCnt(11) >= 1000)
						{
							CMallSymbolGiftController.ResetDrawedCnt(11);
						}
					}
				}
			}
			else
			{
				if (iBuySubType == 1)
				{
					CMallSymbolGiftController.ResetFreeDrawCD(3);
					Singleton<EventRouter>.GetInstance().BroadCastEvent<CMallSystem.Tab>(EventID.Mall_Refresh_Tab_Red_Dot, CMallSystem.Tab.Symbol_Make);
					Singleton<CMallSymbolGiftController>.GetInstance().RefreshButtonView(form);
				}
				if (iBuySubType >= 1 && iBuySubType < 5)
				{
					CMallSymbolGiftController.SymbolGiftCommonDrawedCntInfo[iBuySubType]++;
					if (CMallSymbolGiftController.GetTotalDrawedCnt(10) >= 1000)
					{
						CMallSymbolGiftController.ResetDrawedCnt(10);
					}
				}
			}
		...
		private void RefreshButtonView(CUIFormScript form)
		{
			...
			if (gameObject != null)
			{
				stPayInfo drawPayInfo = this.GetDrawPayInfo(10, 2);
			...
			if (gameObject2 != null)
			{
				stPayInfo drawPayInfo2 = this.GetDrawPayInfo(10, 3);
			...
			if (flag)
			{
				...
				if (gameObject8 != null)
				{
					stPayInfo drawPayInfo3 = this.GetDrawPayInfo(11, 1);
			...
			else
			{
				...
				if (gameObject5 != null)
				{
					stPayInfo drawPayInfo4 = this.GetDrawPayInfo(11, 2);
			...
			if (gameObject6 != null)
			{
				stPayInfo drawPayInfo5 = this.GetDrawPayInfo(11, 3);
		...
		private stPayInfo GetDrawPayInfo(RES_SHOPBUY_TYPE buyType, RES_SHOPDRAW_SUBTYPE subType)
		{
			...
			ResShopInfo cfgShopInfo = CPurchaseSys.GetCfgShopInfo(buyType, subType);
