	...
	public class CSymbolMakeController : Singleton<CSymbolMakeController>
	{
		...
		private bool SetBreakSymbolList()
		{
			...
			for (int i = 0; i < num; i++)
			{
				if (this.m_breakSymbolList[i] != null)
				{
					for (int j = 0; j < this.m_breakSymbolList[i].get_Count(); j++)
					{
						if (this.m_breakSymbolList[i].get_Item(j).symbol != null && this.m_breakSymbolList[i].get_Item(j).bBreak)
						{
							CSDT_SYMBOLOPT_INFO cSDT_SYMBOLOPT_INFO = new CSDT_SYMBOLOPT_INFO();
							cSDT_SYMBOLOPT_INFO.dwSymbolID = this.m_breakSymbolList[i].get_Item(j).symbol.m_baseID;
							cSDT_SYMBOLOPT_INFO.iSymbolCnt = this.m_breakSymbolList[i].get_Item(j).symbol.m_stackCount - this.m_breakSymbolList[i].get_Item(j).symbol.GetMaxWearCnt();
							if (this.m_breakSymbolList[i].get_Item(j).symbol.m_SymbolData.bNeedPswd > 0)
							{
								result = true;
							}
		...
		private void SetSymbolMakeListData()
		{
			...
			int count = CSymbolMakeController.s_allSymbolCfgList.get_Count();
			for (int i = 0; i < count; i++)
			{
				if (CSymbolMakeController.s_allSymbolCfgList.get_Item(i) != null && (int)CSymbolMakeController.s_allSymbolCfgList.get_Item(i).wLevel == this.m_symbolFilterLevel && (this.m_symbolFilterType == enSymbolType.All || ((int)CSymbolMakeController.s_allSymbolCfgList.get_Item(i).wType & 1 << (int)this.m_symbolFilterType) != 0))
				{
					this.m_symbolMakeList.Add(CSymbolMakeController.s_allSymbolCfgList.get_Item(i));
				}
			}
		}
		...
		private void RefreshSymbolMakeList()
		{
			...
			componetInChild.SetElementAmount(this.m_symbolMakeList.get_Count());
		}
		...
		{
			if (uiEvent.m_srcWidgetIndexInBelongedList < 0 || uiEvent.m_srcWidgetIndexInBelongedList >= this.m_symbolMakeList.get_Count())
			{
				DebugHelper.Assert(false, "OnSymbolMakeListEnable index out of range");
				return;
			}
			CSymbolSystem.RefreshSymbolItem(this.m_symbolMakeList.get_Item(uiEvent.m_srcWidgetIndexInBelongedList), uiEvent.m_srcWidget, uiEvent.m_srcFormScript, this.Source);
		}
		...
		private void OnSymbolMakeListClick(CUIEvent uiEvent)
		{
			...
			if (source != enSymbolMakeSource.SymbolManage)
			{
				if (source == enSymbolMakeSource.Mall)
				{
					CSymbolBuyPickDialog.Show(this.m_curTransformSymbol, 9, 10000f, new CSymbolBuyPickDialog.OnConfirmBuyDelegate(this.BuySymbol), null, null);
				}
			}
			else
			{
		...
		public void RefreshSymbolTransformForm()
		{
			...
			{
				num = useableContainer.GetUseableStackCount(5, this.m_curTransformSymbol.dwID);
				CTextManager instance = Singleton<CTextManager>.GetInstance();
			...
			if (num <= 0)
			{
				CUICommonSystem.SetButtonEnable(gameObject3.GetComponent<Button>(), false, false, true);
			}
			else
			{
				CUICommonSystem.SetButtonEnable(gameObject3.GetComponent<Button>(), true, true, true);
			}
		}
		...
		private CSymbolItem GetSymbolByCfgID(uint cfgId)
		{
			...
			for (int i = 0; i < allSymbolList.get_Count(); i++)
			{
				CSymbolItem cSymbolItem = allSymbolList.get_Item(i);
			...
			for (int i = 0; i < this.m_breakSymbolList.Length; i++)
			{
				if (this.m_breakSymbolList[i] != null)
				{
					for (int j = 0; j < this.m_breakSymbolList[i].get_Count(); j++)
					{
						if (this.m_breakSymbolList[i].get_Item(j).symbol != null && this.m_breakSymbolList[i].get_Item(j).bBreak)
						{
							num += (this.m_breakSymbolList[i].get_Item(j).symbol.m_stackCount - this.m_breakSymbolList[i].get_Item(j).symbol.GetMaxWearCnt()) * (int)this.m_breakSymbolList[i].get_Item(j).symbol.m_SymbolData.dwBreakCoin;
						}
					}
				}
			}
		...
		private int GetBreakExcessSymbolCoinCnt(ushort breakLvlMask = 65535)
		{
			...
			for (int i = 0; i < allSymbolList.get_Count(); i++)
			{
				if (this.CheckSymbolBreak(allSymbolList.get_Item(i), breakLvlMask))
				{
					num += (allSymbolList.get_Item(i).m_stackCount - allSymbolList.get_Item(i).GetMaxWearCnt()) * (int)allSymbolList.get_Item(i).m_SymbolData.dwBreakCoin;
				}
			}
			return num;
		}
		...
		private void OnReceiveBreakItemList()
		{
			...
			{
				int j = 0;
				while (j < this.m_breakSymbolList[i].get_Count())
				{
					if (this.m_breakSymbolList[i].get_Item(j).bBreak)
					{
		...
		private void OnBreakListItemEnable(CUIEvent uiEvent)
		{
			...
			if (srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_breakSymbolList[this.m_breakDetailIndex].get_Count())
			{
				return;
			}
			CBreakSymbolItem cBreakSymbolItem = this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(srcWidgetIndexInBelongedList);
		...
		private void OnBreakListItemSelToggle(CUIEvent uiEvent)
		{
			...
			if (tag < 0 || tag >= this.m_breakSymbolList[this.m_breakDetailIndex].get_Count())
			{
				return;
			}
			CBreakSymbolItem cBreakSymbolItem = this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(tag);
		...
		private void OnOpenBreakDetailForm(CUIEvent uiEvent)
		{
			...
			if (transform != null)
			{
				...
				if (tag >= 0 && tag < this.m_breakSymbolList.Length && this.m_breakSymbolList[tag] != null)
				{
					...
					component.SetElementAmount(this.m_breakSymbolList[tag].get_Count());
				}
			}
		}
		...
		private void OnBreakDetailFormConfirm(CUIEvent uiEvent)
		{
			...
			{
				for (int i = 0; i < this.m_breakSymbolList[this.m_breakDetailIndex].get_Count(); i++)
				{
					if (this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreak != this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreakToggle)
					{
						this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreak = this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreakToggle;
					}
				}
			}
		...
		private void OnBreakDetailFormCancle(CUIEvent uiEvent)
		{
			if (this.m_breakDetailIndex >= 0 && this.m_breakDetailIndex < this.m_breakSymbolList.Length && this.m_breakSymbolList[this.m_breakDetailIndex] != null)
			{
				for (int i = 0; i < this.m_breakSymbolList[this.m_breakDetailIndex].get_Count(); i++)
				{
					if (this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreakToggle != this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreak)
					{
						this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreakToggle = this.m_breakSymbolList[this.m_breakDetailIndex].get_Item(i).bBreak;
					}
				}
			}
			Singleton<CUIManager>.GetInstance().CloseForm(CSymbolMakeController.s_symbolBreakDetailPath);
		...
		private void SetBreakSymbolListData(int index)
		{
			if (index >= 0 && index < this.m_breakSymbolList.Length)
			{
				...
				for (int i = 0; i < count; i++)
				{
					if (this.CheckSymbolBreak(allSymbolList.get_Item(i), breakLvlMask))
					{
						CBreakSymbolItem cBreakSymbolItem = new CBreakSymbolItem(allSymbolList.get_Item(i), true);
						this.m_breakSymbolList[index].Add(cBreakSymbolItem);
					}
				}
			}
		}
		...
		private bool IsAllSymbolBreak(int index)
		{
			if (index >= 0 && index < this.m_breakSymbolList.Length)
			{
				for (int i = 0; i < this.m_breakSymbolList[index].get_Count(); i++)
				{
					if (!this.m_breakSymbolList[index].get_Item(i).bBreak)
					{
						return false;
					}
				}
		...
		private void OnBreakExcessSymbolClickConfirm(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				if (this.m_svrBreakSymbolList.get_Count() > 0)
				{
					if (flag)
					{
		...
		private void OnSecurePwdBreakExcessSymbolConfirm(CUIEvent uiEvent)
		{
			...
			else if (this.m_svrBreakSymbolList.get_Count() > 0)
			{
		...
		public static void SendReqSymbolMake(uint heroId, uint symbolId, int cnt)
		{
			...
			cSPkg.stPkgData.get_stSymbolMake().dwBelongHeroID = heroId;
			cSPkg.stPkgData.get_stSymbolMake().stSymbolInfo.dwSymbolID = symbolId;
			cSPkg.stPkgData.get_stSymbolMake().stSymbolInfo.iSymbolCnt = cnt;
		...
		public static void SendReqSymbolBreak(uint heroId, uint symbolId, int cnt, string pwd = "")
		{
			...
			cSPkg.stPkgData.get_stSymbolBreak().dwBelongHeroID = heroId;
			cSPkg.stPkgData.get_stSymbolBreak().wSymbolCnt = 1;
			cSPkg.stPkgData.get_stSymbolBreak().bBreakType = 0;
			StringHelper.StringToUTF8Bytes(pwd, ref cSPkg.stPkgData.get_stSymbolBreak().szPswdInfo);
			CSDT_SYMBOLOPT_INFO cSDT_SYMBOLOPT_INFO = new CSDT_SYMBOLOPT_INFO();
			cSDT_SYMBOLOPT_INFO.dwSymbolID = symbolId;
			cSDT_SYMBOLOPT_INFO.iSymbolCnt = cnt;
			cSPkg.stPkgData.get_stSymbolBreak().astSymbolList[0] = cSDT_SYMBOLOPT_INFO;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(1156u);
			cSPkg.stPkgData.get_stSymbolBreak().wSymbolCnt = (ushort)breakSymbolList.get_Count();
			cSPkg.stPkgData.get_stSymbolBreak().bBreakType = 1;
			StringHelper.StringToUTF8Bytes(pwd, ref cSPkg.stPkgData.get_stSymbolBreak().szPswdInfo);
			int num = 0;
			while (num < breakSymbolList.get_Count() && num < 400)
			{
				cSPkg.stPkgData.get_stSymbolBreak().astSymbolList[num] = breakSymbolList.get_Item(num);
				num++;
			}
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void ReciveSymbolMakeRsp(CSPkg msg)
		{
			...
			SCPKG_CMD_SYMBOL_MAKE stSymbolMakeRsp = msg.stPkgData.get_stSymbolMakeRsp();
			if (stSymbolMakeRsp.iResult == 0)
			{
				CUseable cUseable = CUseableManager.CreateUseable(5, stSymbolMakeRsp.stSymbolInfo.dwSymbolID, stSymbolMakeRsp.stSymbolInfo.iSymbolCnt);
				if (cUseable != null)
				{
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			SCPKG_CMD_SYMBOL_BREAK stSymbolBreakRsp = msg.stPkgData.get_stSymbolBreakRsp();
			int num = 0;
