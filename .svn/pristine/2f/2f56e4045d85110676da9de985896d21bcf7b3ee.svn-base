...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CGuildListController : Singleton<CGuildListController>
	{
		...
		private void RequestPrepareGuildList(int page)
		{
			...
			cSPkg.stPkgData.get_stGetPrepareGuildListReq().bPageID = (byte)page;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
		...
		public void RequestApplyJoinGuild(ulong guildId, bool isRecruit)
		{
			...
			cSPkg.stPkgData.get_stApplyJoinGuildReq().ullGuildID = guildId;
			cSPkg.stPkgData.get_stApplyJoinGuildReq().bIsZhaomu = Convert.ToByte(isRecruit);
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
		...
		private void RequestCreatePrepareGuild(stPrepareGuildCreateInfo info)
		{
			...
			StringHelper.StringToUTF8Bytes(info.sName, ref cSPkg.stPkgData.get_stCreateGuildReq().szName);
			StringHelper.StringToUTF8Bytes(info.sBulletin, ref cSPkg.stPkgData.get_stCreateGuildReq().szNotice);
			cSPkg.stPkgData.get_stCreateGuildReq().bIsOnlyFriend = Convert.ToByte(info.isOnlyFriend);
			cSPkg.stPkgData.get_stCreateGuildReq().dwHeadID = info.dwHeadId;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
		...
		private void RequestPrepareGuildInfo()
		{
			...
			cSPkg.stPkgData.get_stGetPrepareGuildInfoReq().ullGuildID = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_baseGuildInfo.uulUid;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
