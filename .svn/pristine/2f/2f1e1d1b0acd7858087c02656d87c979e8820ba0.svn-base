...
namespace Assets.Scripts.GameLogic
{
	internal class LockModeKeySelector : Singleton<LockModeKeySelector>
	{
		...
		private void SortActorListByTag(PoolObjHandle<ActorRoot> InActor, ref List<ActorRoot> actorList, SelectEnemyType type)
		{
			...
			{
				array[0] = TargetProperty.GetPropertyHpRate(actorList.get_Item(0), 5);
			}
			else
			{
				array[0] = (ulong)(InActor.get_handle().location - actorList.get_Item(0).location).get_sqrMagnitudeLong2D();
			}
			for (int i = 1; i < count; i++)
			{
				...
				if (type == SelectEnemyType.SelectLowHp)
				{
					num = TargetProperty.GetPropertyHpRate(actorRoot, 5);
				}
				else
				{
					num = (ulong)(InActor.get_handle().location - actorList.get_Item(0).location).get_sqrMagnitudeLong2D();
				}
				array[i] = num;
				int j;
				for (j = i; j >= 1; j--)
		...
		private uint GetSelectTargetByTag(AttackTargetType targetType, SelectEnemyType selectType)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (handle.HorizonMarker.IsVisibleFor(captain.get_handle().TheActorMeta.ActorCamp))
				{
					if (captain.get_handle().CanAttack(handle))
					{
						if (targetType == AttackTargetType.ATTACK_TARGET_HERO)
						{
							if (TypeSearchCondition.Fit(handle, ActorTypeDef.Actor_Type_Hero) && DistanceSearchCondition.Fit(handle, captain, captain.get_handle().ActorControl.SearchRange))
							{
								list.Add(handle);
							}
						}
						...
						{
							if (DistanceSearchCondition.Fit(handle, captain, captain.get_handle().ActorControl.SearchRange))
							{
								list4.Add(handle);
							}
						}
						else if (TypeSearchCondition.Fit(handle, ActorTypeDef.Actor_Type_Monster) && DistanceSearchCondition.Fit(handle, captain, captain.get_handle().ActorControl.SearchRange))
						{
			...
			else
			{
				...
				{
					list5.Add(list2.get_Item(j));
				}
				for (int j = 0; j < list4.get_Count(); j++)
				{
					list5.Add(list4.get_Item(j));
				}
