...
namespace Assets.Scripts.GameSystem
{
	public class CGuildHelper
	{
		...
		public static void SetHallBuildingLevel(byte hallLevel)
		{
			...
			for (int i = 0; i < listBuildingInfo.get_Count(); i++)
			{
				if (listBuildingInfo.get_Item(i).type == 1)
				{
					listBuildingInfo.get_Item(i).level = hallLevel;
					return;
				}
			}
		}
		...
		public static void GetViceChairmanUidAndName(out List<ulong> uids, out List<string> names)
		{
			...
			for (int i = 0; i < listMemInfo.get_Count(); i++)
			{
				if (listMemInfo.get_Item(i).enPosition == 4)
				{
					uids.Add(listMemInfo.get_Item(i).stBriefInfo.uulUid);
					names.Add(listMemInfo.get_Item(i).stBriefInfo.sName);
				}
			}
		}
		...
		public static ulong GetChairmanUid()
		{
			...
			for (int i = 0; i < listMemInfo.get_Count(); i++)
			{
				if (listMemInfo.get_Item(i).enPosition == 3)
				{
					return listMemInfo.get_Item(i).stBriefInfo.uulUid;
				}
			}
			return 0uL;
		}
		...
		public static RankpointRankInfo GetPlayerGuildRankpointRankInfo(enGuildRankpointRankListType rankListType)
		{
			...
			{
				if (Singleton<CGuildModel>.GetInstance().CurrentGuildInfo.briefInfo.uulUid == listView.get_Item(i).guildId)
				{
		...
		{
			return rsp.stRankingListDetail.get_stOfSucc().iStart == 1;
		}
		...
		{
			ResGuildGradeConf gradeResByRankpointScore = CGuildHelper.GetGradeResByRankpointScore(rankpointScore);
			return (uint)((gradeResByRankpointScore == null) ? 0 : gradeResByRankpointScore.bIndex);
		}
		...
		{
			return Singleton<CGuildModel>.GetInstance().CurrentGuildInfo.listMemInfo.get_Count();
		}
		...
		public static bool IsSelfInGuildMemberList()
		{
			...
			if (listMemInfo != null)
			{
				for (int i = 0; i < listMemInfo.get_Count(); i++)
				{
					if (listMemInfo.get_Item(i).stBriefInfo.uulUid == playerUllUID)
					{
						return true;
					}
				}
		...
		{
			return CGuildHelper.GetGuildMatchLeftCntInCurRound(curMatchCnt) <= 0;
		}
