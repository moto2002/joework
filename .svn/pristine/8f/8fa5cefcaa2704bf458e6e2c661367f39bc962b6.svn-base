...
public class InBattleMsgInputView
{
	...
	public void Init(CUIFormScript battleFormScript)
	{
		...
		CUIFormScript form = Singleton<CUIManager>.get_instance().GetForm(InBattleShortcut.InBattleMsgView_FORM_PATH);
		...
		if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("InBat_IntimeMaxCount"), ref this.intimeMaxCount))
		{
			DebugHelper.Assert(false, "---InBatMsg jason 你配置的 InBat_IntimeMaxCount 好像不是整数哦， check out");
		}
		if (!int.TryParse(Singleton<CTextManager>.get_instance().GetText("InBat_IntimeMSecond"), ref this.intimeMSecond))
		{
			DebugHelper.Assert(false, "---InBatMsg jason 你配置的 InBat_IntimeMSecond 好像不是整数哦， check out");
		}
		if (!uint.TryParse(Singleton<CTextManager>.get_instance().GetText("InBat_IntimeCD"), ref this.intime_cd))
		{
			DebugHelper.Assert(false, "---InBatMsg jason 你配置的 InBat_IntimeCD 好像不是整数哦， check out");
		}
		this.m_inCDTxt = Singleton<CTextManager>.get_instance().GetText("InBatInput_InCD");
		this.m_inWramBattle = Singleton<CTextManager>.get_instance().GetText("InBatInput_InWarm");
	}
	...
	private void On_EndEdit(string content)
	{
		...
		InBattleInputChat inputChat2 = Singleton<InBattleMsgMgr>.get_instance().m_InputChat;
		if (inputChat2 == null)
		{
			return;
		}
		...
		if (curLvelContext != null && !Singleton<InBattleMsgMgr>.get_instance().ShouldBeThroughNet(curLvelContext))
		{
			InBattleInputChat inputChat3 = Singleton<InBattleMsgMgr>.get_instance().m_InputChat;
			if (inputChat3 != null)
			{
	...
	public void Refresh(ListView<InBattleInputChat.InBatChatEntity> chatEntityList)
	{
		...
		int i = 0;
		int num = 0;
		while (num < chatEntityList.get_Count() && i < InBattleInputChat.CHAT_LINE_COUNT)
		{
			InBattleInputChat.InBatChatEntity inBatChatEntity = chatEntityList.get_Item(num);
	...
	{
		Player playerByUid = Singleton<GamePlayerCenter>.get_instance().GetPlayerByUid(uid);
		return playerByUid != null && Singleton<GamePlayerCenter>.get_instance().IsAtSameCamp(Singleton<GamePlayerCenter>.get_instance().HostPlayerId, playerByUid.PlayerId);
	}
