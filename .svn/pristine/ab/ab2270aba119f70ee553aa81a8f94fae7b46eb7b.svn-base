	...
	public class CEnemyHeroAtkBtn
	{
		...
		private struct BTN_INFO
		{
			...
			public BTN_INFO(Transform _btnTransform, PoolObjHandle<ActorRoot> _actorPtr, CEnemyHeroAtkBtn.ENM_ENEMY_HERO_STATE _heroState, bool _bIsUseBlueHpImage, GameObject _objUI3DHp, Sprite3D _objHp)
			{
				...
				this.btnScreenPos = CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, _btnTransform.position);
			...
			if (iBtnIndex >= 0 && iBtnIndex < this.m_iCurEnemyPlayerCount && this.m_arrBtnInfo != null && this.m_arrBtnInfo[iBtnIndex].btnTransform != null)
			{
				if (this.m_arrBtnInfo[iBtnIndex].heroState != 0)
				{
					if (this.m_arrBtnInfo[iBtnIndex].btnTransform.gameObject.activeSelf)
					{
						...
						else if (this.m_hostActor && this.m_hostActor.get_handle().ActorAgent != null && this.m_hostActor.get_handle().ActorAgent.m_wrapper.myBehavior == ObjBehaviMode.Normal_Attack && this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget && this.m_arrBtnInfo[iBtnIndex].actorPtr && this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget.get_handle().ObjID == this.m_arrBtnInfo[iBtnIndex].actorPtr.get_handle().ObjID && Singleton<CBattleSystem>.GetInstance().FightForm != null)
						{
				...
				else if (!this.m_arrBtnInfo[iBtnIndex].btnTransform.gameObject.activeSelf)
				{
		...
		private void SetBtnHpFillAmount(int iBtnIndex)
		{
			...
			{
				float num = (float)this.m_arrBtnInfo[iBtnIndex].actorPtr.get_handle().ValueComponent.actorHp / (float)this.m_arrBtnInfo[iBtnIndex].actorPtr.get_handle().ValueComponent.actorHpTotal;
		...
		private void OnHeroDead(PoolObjHandle<ActorRoot> hero)
		{
			if (hero)
			{
				if (ActorHelper.IsHostEnemyActor(ref hero))
				{
					...
					{
						CEnemyHeroAtkBtn.BTN_INFO[] expr_3E_cp_0 = this.m_arrBtnInfo;
						int expr_3E_cp_1 = btnIndexByActor;
						expr_3E_cp_0[expr_3E_cp_1].heroState = (expr_3E_cp_0[expr_3E_cp_1].heroState | 4);
		...
		private void UpdateHeroAtkBtnWithDistance(int iBtnIndex)
		{
			...
			if (iBtnIndex >= 0 && iBtnIndex < this.m_iCurEnemyPlayerCount)
			{
				...
				int magnitude2D = (this.m_hostActor.get_handle().location - this.m_arrBtnInfo[iBtnIndex].actorPtr.get_handle().location).get_magnitude2D();
		...
		private void UpdateHeroAtkBtnDistance()
		{
			if (GameSettings.ShowEnemyHeroHeadBtnMode && this.m_hostActor)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					if (hero.get_handle().TheActorMeta.ActorCamp != this.m_hostActor.get_handle().TheActorMeta.ActorCamp)
					{
			...
			if (uiEvent.m_srcWidget != null)
			{
				...
				if (num >= 0 && num < this.m_iCurEnemyPlayerCount)
				{
					if (this.m_arrBtnInfo[num].actorPtr)
					{
						...
						uint objID = this.m_arrBtnInfo[num].actorPtr.get_handle().ObjID;
		...
		private int GetDrawAttackLinkerBtnIndex()
		{
			...
			if (this.m_hostActor && this.m_hostActor.get_handle().ActorAgent != null && this.m_hostActor.get_handle().ActorAgent.m_wrapper != null && num < 0)
			{
				ObjBehaviMode myBehavior = this.m_hostActor.get_handle().ActorAgent.m_wrapper.myBehavior;
				if (myBehavior >= ObjBehaviMode.UseSkill_1 && myBehavior <= ObjBehaviMode.UseSkill_3)
				{
					...
					SkillSlot skillSlot;
					if (this.m_hostActor.get_handle().SkillControl != null && this.m_hostActor.get_handle().SkillControl.TryGetSkillSlot(type, out skillSlot))
					{
						...
						{
							num = this.GetBtnIndexByActor(this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget);
						}
					}
				}
				else if (myBehavior == ObjBehaviMode.Normal_Attack && this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget && this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && this.m_hostActor.get_handle().IsEnemyCamp(this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget))
				{
					num = this.GetBtnIndexByActor(this.m_hostActor.get_handle().ActorAgent.m_wrapper.myTarget);
				}
			}
		...
		public void OnSkillBtnUp(SkillSlotType skillSlotType, enSkillJoystickMode mode = enSkillJoystickMode.General)
		{
			...
			{
				num = this.m_arrBtnInfo[this.m_iCurTargetEnemyBtnIndex].actorPtr.get_handle().ObjID;
			...
			if (hostPlayer != null && hostPlayer.Captain)
			{
				if (num != 0u)
				{
					hostPlayer.Captain.get_handle().SkillControl.RequestUseSkillSlot(skillSlotType, enSkillJoystickMode.EnemyHeroSelect, num);
					SkillSlot skillSlot;
				...
				{
					hostPlayer.Captain.get_handle().SkillControl.RequestUseSkillSlot(skillSlotType, mode, num);
				}
			}
		}
		...
		{
			bool flag = false;
			if (!this.m_hostActor || this.m_hostActor.get_handle().ActorAgent == null || this.m_hostActor.get_handle().ActorAgent.m_wrapper == null)
			{
				return;
			}
			BaseAttackMode currentAttackMode = this.m_hostActor.get_handle().ActorAgent.m_wrapper.GetCurrentAttackMode();
			uint num = 0u;
			...
			{
				Player hostPlayer = Singleton<GamePlayerCenter>.GetInstance().GetHostPlayer();
				if (hostPlayer != null && hostPlayer.Captain && hostPlayer.Captain.get_handle().SkillControl != null)
				{
					hostPlayer.Captain.get_handle().SkillControl.TryGetSkillSlot(skillSlotType, out skillSlot);
				}
			}
			if (bIsInCancelArea)
			{
		...
		public uint GetLockedEnemyHeroObjId()
		{
			...
			if (this.m_iCurTargetEnemyBtnIndex >= 0 && this.m_iCurTargetEnemyBtnIndex < this.m_iCurEnemyPlayerCount && this.m_arrBtnInfo[this.m_iCurTargetEnemyBtnIndex].actorPtr)
			{
				result = this.m_arrBtnInfo[this.m_iCurTargetEnemyBtnIndex].actorPtr.get_handle().ObjID;
			}
			return result;
		}
		...
		public void ShowEnemyHeroHeadBtn()
		{
			for (int i = 0; i < this.m_iCurEnemyPlayerCount; i++)
			{
				if (this.m_arrBtnInfo[i].actorPtr && this.m_arrBtnInfo[i].actorPtr.get_handle().Visible && this.m_arrBtnInfo[i].actorPtr.get_handle().ValueComponent != null && this.m_arrBtnInfo[i].actorPtr.get_handle().ValueComponent.actorHp > 0)
				{
					this.UpdateHeroAtkBtnWithDistance(i);
				}
			}
		...
		private void UpdateAndDrawAttackLinker3D()
		{
			...
			if ((num < 0 || num > this.m_iCurEnemyPlayerCount) && this.m_iLastDrawLinkerBtnIndex >= 0 && this.m_iLastDrawLinkerBtnIndex < this.m_iCurEnemyPlayerCount)
			{
				...
				{
					this.m_fDrawLinkerMissBtnIndexFrameTime = Singleton<FrameSynchr>.get_instance().LogicFrameTick;
				}
				else if (Singleton<FrameSynchr>.get_instance().LogicFrameTick - this.m_fDrawLinkerMissBtnIndexFrameTime >= 500f)
				{
			...
			if (num >= 0 && num < this.m_iCurEnemyPlayerCount)
			{
				...
				if (actorPtr && !actorPtr.get_handle().ActorControl.IsDeadState && this.m_hostActor && !this.m_hostActor.get_handle().ActorControl.IsDeadState)
				{
					...
					{
						float num2 = (float)actorPtr.get_handle().CharInfo.iBulletHeight * 0.001f;
						Vector3 position = actorPtr.get_handle().myTransform.position;
						position.y += num2;
						Vector3 vector = Camera.main.WorldToScreenPoint(position);
						Vector3 position2 = this.m_arrBtnInfo[num].btnTransform.position;
						Vector3 vector2 = CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, position2);
