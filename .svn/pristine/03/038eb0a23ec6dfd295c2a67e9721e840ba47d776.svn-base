...
namespace Assets.Scripts.GameLogic
{
	...
	public class PassiveRemoveBuffCondition : PassiveCondition
	{
		...
		public override void Init(PoolObjHandle<ActorRoot> _source, PassiveEvent _event, ref ResDT_SkillPassiveCondition _config)
		{
			...
			if (_source && _source.get_handle().BuffHolderComp != null && _source.get_handle().BuffHolderComp.SpawnedBuffList != null)
			{
				for (int i = 0; i < _source.get_handle().BuffHolderComp.SpawnedBuffList.get_Count(); i++)
				{
					BuffSkill buffSkill = _source.get_handle().BuffHolderComp.SpawnedBuffList.get_Item(i);
		...
		private void OnPlayerBuffChange(ref BuffChangeEventParam prm)
		{
			...
			if (prm.stBuffSkill.get_handle().cfgData != null && prm.stBuffSkill.get_handle().cfgData.iCfgID == this.localParams[0])
			{
				if (prm.bIsAdd)
				{
					this.bTrigger = false;
