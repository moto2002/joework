...
public class XMLParser
{
	...
	{
		//暂时从Resource下读取
		byte[] bytes = Utils.LoadByteResource (fileName);
		return bytes;
	}
	...
	public static Dictionary<int, Dictionary<string, string>> LoadIntMap(SecurityElement xml, string source)
	{
		...
		foreach (SecurityElement element in xml.Children)
		{
			...
			{
			     LogHelper.Warning(string.Concat(new object[] { "empty row in row NO.", num, " of ", source }), true);
			}
			else
			{
				...
				{
					LogHelper.Warning(string.Format("Key {0} already exist, in {1}.", key, source), true);
				}
				else
				{
					...
					for (int i = 1; i < element.Children.Count; i++)
					{
						...
						{
                            LogHelper.Warning(string.Format("Key {0} already exist, index {1} of {2}.", element2.Tag, i, element2.ToString()), true);
						}
					}
				}
			}
	...
	public static bool LoadIntMap(string fileName, bool isForceOutterRecoure, out Dictionary<int, Dictionary<string, string>> map)
	{
		try
		{
			...
			if (element == null)
			{
                LogHelper.Error("File not exist: " + fileName, true);
				map = null;
				return false;
			}
		...
		{
            LogHelper.Error("Load Int Map Error: " + fileName + "  " + exception.Message, true);
			map = null;
			return false;
		}
	}
	...
	public static Dictionary<string, Dictionary<string, string>> LoadMap(SecurityElement xml)
	{
		...
		foreach (SecurityElement element in xml.Children)
		{
			...
			if (dictionary.ContainsKey(key))
			{
                LogHelper.Warning(string.Format("Key {0} already exist, in {1}.", key, xml.ToString()), true);
			}
			else
			{
				Dictionary<string, string> dictionary2 = new Dictionary<string, string>();
	...
	public static string LoadText(string fileName)
	{
		try
		{
			string s = Utils.LoadFileResource(fileName);
			return s;
		}
		catch (Exception exception)
		{
