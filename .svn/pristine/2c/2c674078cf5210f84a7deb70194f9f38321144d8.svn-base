...
public class NewbieGuideDataManager : Singleton<NewbieGuideDataManager>
{
	...
	public NewbieGuideDataManager()
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (!this.mScriptCacheDic.TryGetValue((uint)newbieGuideScriptConf.wMainLineID, ref listView))
			{
		...
		for (int j = 0; j < num; j++)
		{
			...
			if (!this.mWeakScriptCacheDic.TryGetValue(newbieGuideWeakConf.dwWeakLineID, ref listView2))
			{
	...
	public ListView<NewbieGuideScriptConf> GetScriptList(uint mainLineID)
	{
		...
		if (this.mScriptCacheDic.TryGetValue(mainLineID, ref result))
		{
			return result;
		}
		return null;
	...
	public ListView<NewbieGuideWeakConf> GetWeakScriptList(uint mainLineID)
	{
		...
		if (this.mWeakScriptCacheDic.TryGetValue(mainLineID, ref result))
		{
			return result;
		}
		return null;
