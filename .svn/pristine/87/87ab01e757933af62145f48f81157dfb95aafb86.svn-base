	...
	public class AreaEventTrigger : FuncRegion, ITrigger
	{
		...
		private void Awake()
		{
			...
			if (this.TargetActorCamps != null)
			{
				...
				{
					this._targetActorCampMask |= 1 << this.TargetActorCamps[j];
				}
			}
		...
		{
			Singleton<GameEventSys>.get_instance().RmvEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDestroy, new RefAction<DefaultGameEventParam>(this.onActorDestroy));
		...
		private void UpdateLogicSimple(int delta)
		{
			...
			if (this.m_updateTimer <= 0)
			{
				...
				while (num < collidingActors && num < this.Capacity)
				{
					...
					if (!this.IgnoreDeathActor || !poolObjHandle.get_handle().ActorControl.IsDeadState)
					{
		...
		private void UpdateLogicEnterLeave(int delta)
		{
				...
				else if (this.TriggerTimes <= 0 || this.m_triggeredCount < this.TriggerTimes)
				{
					...
					if (this.bTriggerByTeam)
					{
						...
						else
						{
							...
							{
								if (!this._inActorsCache.ContainsKey(allHeroes.get_Item(i).get_handle().ObjID))
								{
									flag2 = false;
									break;
								}
					...
					if (flag)
					{
						...
						this._inActors.Add(actor.get_handle().ObjID, new AreaEventTrigger.STriggerContext(actor, this._testToken, inRefParams, this.UpdateInterval));
						if (this.UpdateInterval > 0)
						{
							this.DoActorUpdate(ref actor);
						}
			...
			while (enumerator.MoveNext())
			{
				...
				if (current.get_Value().token != this._testToken)
				{
					...
					arg_2BC_0[expr_2A7] = current2.get_Key();
				}
			}
			for (int j = 0; j < num2; j++)
			{
				uint num3 = this._actorTormvCache[j];
				PoolObjHandle<ActorRoot> actor2 = this._inActors.get_Item(num3).actor;
			...
			if (this._inActorsCache != null)
			{
				...
				while (enumerator2.MoveNext())
				{
					...
					if (current3.get_Value() != this._testToken)
					{
						...
						arg_371_0[expr_35C] = current4.get_Key();
					}
				}
		...
		public void DoSelfDeactivating()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				int expr_21 = num++;
				KeyValuePair<uint, AreaEventTrigger.STriggerContext> current = enumerator.get_Current();
				arg_35_0[expr_21] = current.get_Key();
			}
			for (int i = 0; i < num; i++)
			{
				uint num2 = this._actorTormvCache[i];
				PoolObjHandle<ActorRoot> actor = this._inActors.get_Item(num2).actor;
				this.DoActorLeave(ref actor);
				this._inActors.Remove(num2);
			}
			this.DeactivateSelf();
			this.ActivateNext();
		}
		...
		public override void UpdateLogic(int delta)
		{
			...
			if (this.CoolMode)
			{
				...
				else
				{
					...
					{
						Singleton<TriggerEventSys>.get_instance().SendEvent(TriggerEventDef.ActorInside, this, delta);
					}
				}
			}
			else
		...
		public bool ActorFilter(ref PoolObjHandle<ActorRoot> act)
		{
			...
			ActorRoot handle = act.get_handle();
			...
			return (this._targetActorTypeMask == 0 || (this._targetActorTypeMask & 1 << (int)handle.TheActorMeta.ActorType) != 0) && (this._targetActorCampMask == 0 || (this._targetActorCampMask & 1 << handle.TheActorMeta.ActorCamp) != 0) && (this._targetActorSubTypeMask == 0 || (this._targetActorSubTypeMask & 1 << (int)handle.ActorControl.GetActorSubType()) != 0);
		}
		...
		protected virtual DictionaryView<TriggerActionBase, RefParamOperator> DoActorEnter(ref PoolObjHandle<ActorRoot> inActor)
		{
			...
			Singleton<TriggerEventSys>.get_instance().SendEvent(TriggerEventDef.ActorEnter, this, inActor.get_handle());
		...
		protected virtual void DoActorLeave(ref PoolObjHandle<ActorRoot> inActor)
		{
			...
			Singleton<TriggerEventSys>.get_instance().SendEvent(TriggerEventDef.ActorLeave, this, inActor.get_handle());
		...
		public override void Startup()
		{
			...
			Singleton<GameEventSys>.get_instance().RmvEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDestroy, new RefAction<DefaultGameEventParam>(this.onActorDestroy));
			Singleton<GameEventSys>.get_instance().AddEventHandler<DefaultGameEventParam>(GameEventDef.Event_ActorDestroy, new RefAction<DefaultGameEventParam>(this.onActorDestroy));
		...
		public void onActorDestroy(ref DefaultGameEventParam prm)
		{
			if (prm.src)
			{
				uint objID = prm.src.get_handle().ObjID;
		...
		public List<PoolObjHandle<ActorRoot>> GetActors(Func<PoolObjHandle<ActorRoot>, bool> predicate)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (actor && (predicate == null || predicate.Invoke(actor)))
				{
					...
					KeyValuePair<uint, AreaEventTrigger.STriggerContext> current2 = enumerator.get_Current();
					arg_67_0.Add(current2.get_Value().actor);
				}
			}
			return list;
		}
