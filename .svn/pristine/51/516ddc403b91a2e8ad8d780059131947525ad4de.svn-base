...
namespace Assets.Scripts.GameSystem
{
	public class CBattleShowBuffDesc
	{
		...
		{
			if (!stBuffSkill || stBuffSkill.get_handle().cfgData == null || stBuffSkill.get_handle().cfgData.bIsShowBuff == 0)
			{
				return;
			}
			if (bIsAdd)
			{
				if (this.m_CurShowBuffCount == 0)
				{
					...
					this.m_ArrShowBuffSkill[0].iBufCD = stBuffSkill.get_handle().cfgData.iDuration;
				...
				while (i < this.m_CurShowBuffCount)
				{
					...
					if (this.m_ArrShowBuffSkill[i].stBuffSkill && this.m_ArrShowBuffSkill[i].stBuffSkill.get_handle().cfgData.bShowBuffPriority > stBuffSkill.get_handle().cfgData.bShowBuffPriority)
					{
					...
					if (this.m_ArrShowBuffSkill[i].stBuffSkill && this.m_ArrShowBuffSkill[i].stBuffSkill.get_handle().cfgData.bShowBuffPriority == stBuffSkill.get_handle().cfgData.bShowBuffPriority)
					{
						if (this.m_ArrShowBuffSkill[i].stBuffSkill.get_handle().cfgData.iCfgID == stBuffSkill.get_handle().cfgData.iCfgID)
						{
							this.m_ArrShowBuffSkill[i].stBuffSkill = stBuffSkill;
							this.m_ArrShowBuffSkill[i].iBufCD = stBuffSkill.get_handle().cfgData.iDuration;
				...
				else if (!flag && num >= 0 && num < this.m_CurShowBuffCount)
				{
					...
					this.m_ArrShowBuffSkill[num].iBufCD = stBuffSkill.get_handle().cfgData.iDuration;
			...
			else
			{
				...
				{
					if (this.m_ArrShowBuffSkill[k].stBuffSkill && this.m_ArrShowBuffSkill[k].stBuffSkill.get_handle().cfgData.iCfgID == stBuffSkill.get_handle().cfgData.iCfgID)
					{
						...
						{
							if (this.m_selectBuff && this.m_selectBuff.get_handle().cfgData != null && this.m_ArrShowBuffSkill[k].stBuffSkill && this.m_ArrShowBuffSkill[k].stBuffSkill.get_handle().cfgData != null && this.m_selectBuff.get_handle().cfgData.iCfgID == this.m_ArrShowBuffSkill[k].stBuffSkill.get_handle().cfgData.iCfgID)
							{
								this.m_BuffDescNodeObj.CustomSetActive(false);
							}
							this.m_CurShowBuffCount--;
		...
		private void OnBuffSkillBtnDown(CUIEvent uiEvent)
		{
			if (uiEvent.m_srcWidget != null)
			{
				...
				if (this.m_ArrShowBuffSkill != null && num < this.m_ArrShowBuffSkill.Length)
				{
					...
					{
						uint iCfgID = (uint)this.m_selectBuff.get_handle().cfgData.iCfgID;
		...
		{
			if (!actor || actor.get_handle().BuffHolderComp == null)
			{
				return;
			}
			...
			this.m_CurShowBuffCount = 0;
			List<BuffSkill> spawnedBuffList = actor.get_handle().BuffHolderComp.SpawnedBuffList;
		...
		public void UpdateBuffCD(int delta)
		{
			for (int i = 0; i < this.m_CurShowBuffCount; i++)
			{
				...
				{
					if (this.m_ArrShowBuffSkill[i].stBuffSkill.get_handle().cfgData != null)
					{
						int iDuration = this.m_ArrShowBuffSkill[i].stBuffSkill.get_handle().cfgData.iDuration;
						if (iDuration != -1)
						{
