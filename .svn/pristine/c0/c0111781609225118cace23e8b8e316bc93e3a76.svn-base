...
namespace Assets.Scripts.GameSystem
{
	...
	public class CHeroInfoSystem2 : Singleton<CHeroInfoSystem2>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler<string>("HeroUnlockPvP", new Action<string>(this.OnHeroUnlockPvP));
			...
			Singleton<EventRouter>.get_instance().AddEventHandler<uint, uint, uint>("HeroSkinAdd", new Action<uint, uint, uint>(this.OnAddHeroSkin));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.GLOBAL_REFRESH_TIME, new Action(CHeroInfoSystem2.ResetDirectBuyLimit));
		}
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler<string>("HeroUnlockPvP", new Action<string>(this.OnHeroUnlockPvP));
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler<uint, uint, uint>("HeroSkinAdd", new Action<uint, uint, uint>(this.OnAddHeroSkin));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.GLOBAL_REFRESH_TIME, new Action(CHeroInfoSystem2.ResetDirectBuyLimit));
		}
		...
		public void OnHeroInfo_OpenForm(CUIEvent uiEvent)
		{
			...
			{
				if (GameDataMgr.GetGlobeValue(294) == 0u)
				{
		...
		{
			Singleton<RankingSystem>.get_instance().OpenRankingForm(RankingSystem.RankingType.God, this.m_selectHeroData.cfgID);
		}
		...
		private void OnHeroInfo_MaterialDirectBuyConfirm(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				cSPkg.stPkgData.get_stDirectBuyItemReq().bItemType = uiEvent.m_eventParams.iconUseable.m_type;
				cSPkg.stPkgData.get_stDirectBuyItemReq().dwItemID = uiEvent.m_eventParams.iconUseable.m_baseID;
				cSPkg.stPkgData.get_stDirectBuyItemReq().dwItemCnt = (uint)uiEvent.m_eventParams.iconUseable.m_stackCount;
		...
		public void Refresh2DImage(uint heroID, int skinId)
		{
			...
			CBinaryObject cBinaryObject = Singleton<CResourceManager>.GetInstance().GetResource("UGUI/Sprite/Dynamic/BustHeroLarge/" + text + ".bytes", typeof(TextAsset), 6, false, false).m_content as CBinaryObject;
		...
		private void OnShow2DImage(CUIEvent uiEvent)
		{
			...
			CBinaryObject x = Singleton<CResourceManager>.GetInstance().GetResource("UGUI/Sprite/Dynamic/BustHeroLarge/" + text + ".bytes", typeof(TextAsset), 6, false, false).m_content as CBinaryObject;
			if (x == null)
			{
		...
		private void OnHeroSkin_ItemEnable(CUIEvent uiEvent)
		{
			if (this.m_heroInfoForm != null)
			{
				...
				CUIListScript component = widget.transform.Find("List_Skin").GetComponent<CUIListScript>();
				int selectedIndex = component.GetSelectedIndex();
				this.SetSkinListItem(uiEvent.m_srcWidget, availableSkinByHeroId.get_Item(srcWidgetIndexInBelongedList), srcWidgetIndexInBelongedList == selectedIndex);
			}
		}
		...
		protected void SetSkinListItem(GameObject listItem, ResHeroSkin skinInfo, bool bSelect)
		{
			...
			CUICommonSystem.RefreshExperienceSkinLeftTime(gameObject4, gameObject5, skinInfo.dwHeroID, skinInfo.dwSkinID, Singleton<CTextManager>.GetInstance().GetText("ExpCard_ExpTime"));
		...
		public static string GetSkinCannotBuyStr(ResHeroSkin skinInfo)
		{
			...
			GameDataMgr.skinShopInfoDict.TryGetValue(skinInfo.dwID, ref resHeroSkinShop);
		...
		public static void ReqWearHeroSkin(uint heroId, uint skinId, bool isSendGameSvr = false)
		{
			...
			cSPkg.stPkgData.get_stWearHeroSkinReq().dwHeroID = heroId;
			cSPkg.stPkgData.get_stWearHeroSkinReq().dwSkinID = skinId;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		{
			if (addReason == 1u && !Singleton<CHeroSelectBaseSystem>.get_instance().m_isInHeroSelectState)
			{
		...
		public static void RefreshAtkPropPanel(GameObject root, ref ValueDataInfo[] info, int level, uint heroId)
		{
			...
			MonoSingleton<GlobalConfig>.get_instance().bOnExternalSpeedPicker = true;
			component2.text = CHeroInfoSystem2.GetFormStr((float)(info[15].basePropertyValue / 10), (float)(info[15].extraPropertyValue / 10));
			MonoSingleton<GlobalConfig>.get_instance().bOnExternalSpeedPicker = false;
		...
		public static void ReceiveMaterialDirectBuy(CSPkg msg)
		{
			...
			{
				uint dwConfValue = GameDataMgr.globalInfoDatabin.GetDataByKey(86u).dwConfValue;
				masterRoleInfo.MaterialDirectBuyLimit = (byte)(dwConfValue - msg.stPkgData.get_stDirectBuyItemRsp().dwDirectBuyItemCnt);
			}
		}
		...
		public static void OnWearHeroSkinRsp(CSPkg msg)
		{
			...
			SCPKG_WEARHEROSKIN_RSP stWearHeroSkinRsp = msg.stPkgData.get_stWearHeroSkinRsp();
			...
			{
				CS_WEARHEROSKIN_ERRCODE iResult = stWearHeroSkinRsp.iResult;
				CTextManager instance = Singleton<CTextManager>.GetInstance();
				CS_WEARHEROSKIN_ERRCODE cS_WEARHEROSKIN_ERRCODE = iResult;
				if (cS_WEARHEROSKIN_ERRCODE != 1)
				{
					if (cS_WEARHEROSKIN_ERRCODE == 2)
					{
