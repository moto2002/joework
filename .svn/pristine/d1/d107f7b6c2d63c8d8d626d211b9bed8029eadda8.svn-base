...
namespace Assets.Scripts.GameSystem
{
	...
	public class WatchController : Singleton<WatchController>
	{
		...
		public bool StartObserve(COMDT_TGWINFO inTGW)
		{
			...
			if (GameDataMgr.svr2CltCfgDict.TryGetValue(215u, ref resGlobalInfo))
			{
		...
		private void RequestVideoPiece(bool isNewPiece)
		{
			...
			if (isNewPiece)
			{
				cSPkg.stPkgData.get_stGetVideoFrapsReq().dwOffset = this._totalRecvSize;
				cSPkg.stPkgData.get_stGetVideoFrapsReq().bNew = 1;
			}
			else
			{
				cSPkg.stPkgData.get_stGetVideoFrapsReq().bNew = 0;
			}
		...
		{
			Singleton<WatchController>.GetInstance().HandleVideoPiece(msg.stPkgData.get_stGetVideoFrapsRsp());
		}

		[MessageHandler(5231)]
		public static void OnQuitObserveRsp(CSPkg msg)
		{
			Singleton<WatchController>.GetInstance().HandleQuitResponse(msg.stPkgData.get_stQuitObserveRsp());
		}
	}
}
