...
namespace TMPro
{
	[AddComponentMenu("Mesh/TextMesh Pro"), ExecuteInEditMode, RequireComponent(typeof(MeshRenderer)), RequireComponent(typeof(TextContainer)), RequireComponent(typeof(MeshFilter))]
	public class TextMeshPro : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			if (this.m_mesh == null)
			{
				...
				this.m_mesh.hideFlags = (HideFlags.HideInHierarchy | HideFlags.DontSaveInEditor | HideFlags.NotEditable);
		...
		private void StringToCharArray(string text, ref int[] chars)
		{
			...
			{
				int num = (text.get_Length() <= 1024) ? Mathf.NextPowerOfTwo(text.get_Length() + 1) : (text.get_Length() + 256);
				chars = new int[num];
			}
			int num2 = 0;
			int i = 0;
		...
		private void GenerateFastScaleAndColor()
		{
			if (this.m_renderMode == TextRenderFlags.Render)
			{
				...
				{
					Color[] array = new Color[this.m_vertColors.Length];
					for (int i = 0; i < this.m_vertColors.Length; i++)
					{
						array[i] = this.m_fontColor;
					}
					this.m_mesh.colors = array;
					this.haveColorChanged = false;
				}
				if (this.haveFastScaleChanged)
				{
					Vector3[] array2 = new Vector3[this.m_vertices.Length];
					this.m_vertices.CopyTo(array2, 0);
					for (int j = 0; j < array2.Length; j++)
					{
						array2[j].Set(array2[j].x * this.m_fastScale.x, array2[j].y * this.m_fastScale.y, array2[j].z * this.m_fastScale.z);
					}
					this.m_mesh.vertices = array2;
					this.haveFastScaleChanged = false;
				}
			}
		}
			...
			else
			{
				...
				for (int j = 0; j < this.m_characterCount; j++)
				{
					...
					else if (((char.IsPunctuation(character2) || char.IsWhiteSpace(character2) || j == this.m_characterCount - 1) && flag6) || j == 0)
					{
						...
						tMP_WordInfo.lastCharacterIndex = num47;
						tMP_WordInfo.characterCount = num47 - num35 + 1;
				...
				this.m_textInfo.wordCount = (int)((num32 == 0 || this.m_characterCount <= 0) ? 1 : ((short)num32));
				...
				if (this.m_renderMode == TextRenderFlags.Render)
				{
					...
					else
					{
						...
						{
							array[k].Set(array[k].x * this.m_fastScale.x, array[k].y * this.m_fastScale.y, array[k].z * this.m_fastScale.z);
						}
