namespace VRTK
{
    ...
    {
        public uint controllerIndex;//所使用控制器的索引
        public float buttonPressure;//0到1的浮点数，表示按钮按下的所施加的压力值,不同于其他按钮的是，其他按钮按压只记录一个为1或者0按钮压力（要么就是点击要么就没有），但扳机是一个可以记录不同按钮压力的模拟量(知道扳机键被按下程度)。
        public Vector2 touchpadAxis;//一个表示触摸板触摸位置的Vector2类型变量,(如用触摸板来移动角色)
        public float touchpadAngle;//一个表示触摸板触摸位置的转动位置的浮点数，0是顶部，180是底部，其他角度随之对应
    }
    ...
    public class VRTK_ControllerEvents : MonoBehaviour
    {
        ...
        public event ControllerInteractionEventHandler AliasPointerOn;//打开关闭镭射棒(激光指针)
        ...
        public event ControllerInteractionEventHandler AliasGrabOn;//抓取游戏对象
        public event ControllerInteractionEventHandler AliasGrabOff;

        public event ControllerInteractionEventHandler AliasUseOn;//使用对象
        public event ControllerInteractionEventHandler AliasUseOff;

        public event ControllerInteractionEventHandler AliasMenuOn;//打开一个游戏内菜单
