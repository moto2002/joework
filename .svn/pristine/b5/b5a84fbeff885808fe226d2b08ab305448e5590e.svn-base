	...
	public class GameTask
	{
		...
		public int TimeRemain
		{
			get
			{
				if (this._timer > 0)
				{
					int timerCurrent = Singleton<CTimerManager>.get_instance().GetTimerCurrent(this._timer);
					if (timerCurrent > -1)
					{
		...
		public bool Achieving
		{
			get
			{
				return GameTask._compareFuncs[this.CompareOperator - 1](this.Current, this.Target);
			}
		}
		...
		{
			this.OnDestroy();
			Singleton<CTimerManager>.get_instance().RemoveTimerSafely(ref this._timer);
			Singleton<CTimerManager>.get_instance().RemoveTimerSafely(ref this._readyTimer);
		}
		...
		public void Start()
		{
			...
			{
				Singleton<CTimerManager>.get_instance().RemoveTimerSafely(ref this._readyTimer);
				this._readyTimer = Singleton<CTimerManager>.get_instance().AddTimer(this.TimeReady, 1, new CTimer.OnTimeUpHandler(this.OnReadyOver), true);
		...
		private void DoStart()
		{
			...
			if (this.TimeLimit > 0)
			{
				this._timer = Singleton<CTimerManager>.get_instance().AddTimer(this.TimeLimit, 1, new CTimer.OnTimeUpHandler(this.OnTimeOut), true);
			}
		...
		public void Close()
		{
			...
			{
				for (int i = 0; i < this._ownerGroups.get_Count(); i++)
				{
					this._ownerGroups.get_Item(i)._OnChildClosed(this);
				}
			}
		}
