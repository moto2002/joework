    ...
    public class MogoFileSystem
    {
        ...
        private bool CheckBackUpIndex()
        {
            ...
            if (File.Exists(path))
            {
                ...
                {
                    int count = ((int) length) - PackageInfo.GetPackageSize();
        ...
        private void GetIndexInfo(long fileSize, bool needBackUpIndex)
        {
            lock (m_locker)
            {
                ...
                if ((this.m_packageInfo != null) && (this.m_packageInfo.IndexOffset != 0))
                {
                    int count = (int) ((fileSize - PackageInfo.GetPackageSize()) - this.m_packageInfo.IndexOffset);
                    byte[] buffer = new byte[count];
                    this.m_fileStream.Position = this.m_packageInfo.IndexOffset;
        ...
        private PackageInfo GetPackageInfo(FileStream fileStream, long fileSize)
        {
            ...
            fileStream.Position = fileSize - PackageInfo.GetPackageSize();
            byte[] buffer = new byte[PackageInfo.GetPackageSize()];
            fileStream.Read(buffer, 0, PackageInfo.GetPackageSize());
            int index = 0;
