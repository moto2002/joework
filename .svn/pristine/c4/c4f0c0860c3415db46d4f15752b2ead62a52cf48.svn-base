...
namespace Assets.Scripts.GameLogic
{
	public class BurnExpeditionModel
	{
		...
		public void RandomRobotIcon()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				int num = this.robotIconInts[Random.Range(0, this.robotIconInts.Length)];
				this.levelRandomRobotIcon.Add(num);
			}
		}

		public int GetRandomRobotIcon(int index)
		{
			return this.levelRandomRobotIcon.get_Item(index);
		}
		...
		{
			Dictionary<uint, int> dictionary = this._hero_map.get_Item((uint)this.curDifficultyType);
		...
		public int Get_HeroMaxHP(uint heroCfgID)
		{
			...
			if (masterRoleInfo.GetHeroInfo(heroCfgID, out cHeroInfo, true))
			{
				if (cHeroInfo != null)
				{
					result = cHeroInfo.mActorValue[5].totalValue;
				}
			}
		...
		public void SetLevelDetail(BurnExpeditionModel.EDifficultyType type, COMDT_BURNING_LEVEL_DETAIL detail)
		{
			...
			{
				this._mapDetails.set_Item((uint)type, detail);
			}
		}

		public void UnLockLevel(int levelIndex)
		{
			this.Set_Level_State(this.curDifficultyType, levelIndex, 1);
		}

		public void FinishLevel(int levelIndex)
		{
			this.Set_Level_State(this.curDifficultyType, levelIndex, 2);
		}

		public void UnLockBox(int levelIndex)
		{
			this.Set_Box_State(this.curDifficultyType, levelIndex, 1);
		}

		public void FinishBox(int levelIndex)
		{
			this.Set_Box_State(this.curDifficultyType, levelIndex, 2);
		}
		...
		public string Get_Buff_Description(int buffid)
		{
			...
			{
				return this.buff_ids.get_Item(buffid);
			}
		...
		public void SetProgress(COMDT_BURNING_LEVEL_PROGRESS data)
		{
			...
			for (int i = 0; i < (int)data.bDiffNum; i++)
			{
				...
				Dictionary<uint, int> dictionary = this._hero_map.get_Item((uint)cOMDT_BURNING_LEVEL_DETAIL.bDifficultType);
				if (dictionary != null)
				{
		...
		public void Record_HeroData()
		{
			...
			{
				Dictionary<uint, int> dictionary = this._hero_map.get_Item((uint)i);
		...
		{
			return this._getLevelDetail(this.curDifficultyType).astLevelDetail[levelNo].bLevelStatus;
		}

		public COM_LEVEL_STATUS Get_ChestRewardStatus(int levelNo)
		{
			return this._getLevelDetail(this.curDifficultyType).astLevelDetail[levelNo].bRewardStatus;
		}
		...
		public COMDT_PLAYERINFO Get_Current_Enemy_PlayerInfo()
		{
			...
			{
				stEnemyDetail = curLevel_ENEMY_TEAM_INFO.stDetail.get_stRealMan().stEnemyDetail;
			}
			else
			{
				stEnemyDetail = curLevel_ENEMY_TEAM_INFO.stDetail.get_stRobot().stEnemyDetail;
			}
		...
		public string GetPlayerHearUrl()
		{
			...
			if (curLevel_ENEMY_TEAM_INFO.bType == 1)
			{
				return UT.Bytes2String(curLevel_ENEMY_TEAM_INFO.stDetail.get_stRealMan().szHeadUrl);
			}
			return string.Empty;
		}
		...
		private COMDT_BURNING_LEVEL_DETAIL _getLevelDetail(BurnExpeditionModel.EDifficultyType type)
		{
			if (this._mapDetails.ContainsKey((uint)type))
			{
				return this._mapDetails.get_Item((uint)type);
			}
			return null;
		}
		...
		{
			this._getLevelDetail(this.curDifficultyType).astLevelDetail[levelIndex].bLevelStatus = state;
		}

		private void Set_Box_State(BurnExpeditionModel.EDifficultyType type, int boxIndex, COM_LEVEL_STATUS state)
		{
			this._getLevelDetail(this.curDifficultyType).astLevelDetail[boxIndex].bRewardStatus = state;
		}
	}
}
