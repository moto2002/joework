...
namespace Assets.Scripts.GameLogic
{
	...
	public class PassiveDistanceEvent : PassiveEvent
	{
		...
		private int curTriggerCount
		{
			...
			set
			{
				if (value >= 0 && value <= this.maxTriggerCount)
				{
					...
					if (this.sourceActor && this.sourceActor.get_handle().ValueComponent != null && this.sourceActor.get_handle().ValueComponent.IsEnergyType(EnergyType.SpeedResource))
					{
						this.sourceActor.get_handle().ValueComponent.actorEp = this._curTriggerCount;
					}
				}
			}
		}
		...
		{
			return this.sourceActor.get_handle().BuffHolderComp != null && this.sourceActor.get_handle().BuffHolderComp.IsExistSkillFuncType(7);
		}

		private bool IsLastMove()
		{
			return this.sourceActor.get_handle().MovementComponent.isExcuteMoving && !this.sourceActor.get_handle().ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_Move) && (!this.IsReduceSpeedState() || this.sourceActor.get_handle().ValueComponent.actorMoveSpeed >= this.minMoveSpeed);
		}
		...
		private void UpdateLastMove(int _delta)
		{
			...
			if (!this.bDashingState)
			{
				...
				{
					this.triggerDistance = this.localParams[0];
					int actorMoveSpeed = this.sourceActor.get_handle().ValueComponent.actorMoveSpeed;
		...
		private void AbortDashingState()
		{
			if (this.bDashingState)
			{
				...
				{
					this.passiveSkill.CurAction.get_handle().Stop(false);
				}
		...
		private void RemoveSkillEffectGroup(int _groupID)
		{
			if (this.sourceActor)
			{
				this.sourceActor.get_handle().BuffHolderComp.RemoveSkillEffectGroup(_groupID);
			}
		}
		...
		private void RemoveSkillEffect(int _skillCombineID)
		{
			if (this.sourceActor)
			{
				this.sourceActor.get_handle().BuffHolderComp.RemoveBuff(_skillCombineID);
			}
		}
