...
namespace com.tencent.pandora
{
	public static class ToLuaExport
	{
		...
		{
			ToLuaExport.<Generate>c__AnonStorey89 <Generate>c__AnonStorey = new ToLuaExport.<Generate>c__AnonStorey89();
			...
			<Generate>c__AnonStorey.ps = ToLuaExport.type.GetProperties();
			int i;
			for (i = 0; i < <Generate>c__AnonStorey.ps.Length; i++)
			{
				int num = list.FindIndex((MethodInfo m) => m.get_Name() == "get_" + <Generate>c__AnonStorey.ps[i].get_Name());
				if (num >= 0 && list.get_Item(num).get_Name() != "get_Item")
				{
					list.RemoveAt(num);
				}
				num = list.FindIndex((MethodInfo m) => m.get_Name() == "set_" + <Generate>c__AnonStorey.ps[i].get_Name());
			...
			for (int i = 0; i < ToLuaExport.methods.Length; i++)
			{
				...
				else
				{
					...
					else
					{
						...
						int num = (!methodInfo.get_IsStatic()) ? 2 : 1;
						...
						int num3 = (methodInfo.get_ReturnType() != typeof(void)) ? 1 : 0;
		...
		{
			ToLuaExport.<GenConstruct>c__AnonStorey8C <GenConstruct>c__AnonStorey8C = new ToLuaExport.<GenConstruct>c__AnonStorey8C();
			...
			<GenConstruct>c__AnonStorey8C.list = new List<ConstructorInfo>();
			for (int k = 0; k < constructors.Length; k++)
			{
				if (!ToLuaExport.HasDecimal(constructors[k].GetParameters()))
				{
					if (!ToLuaExport.IsObsolete(constructors[k]))
					{
						...
						int num = <GenConstruct>c__AnonStorey8C.list.FindIndex((ConstructorInfo p) => ToLuaExport.CompareMethod(p, r) >= 0);
						if (num >= 0)
						{
							if (ToLuaExport.CompareMethod(<GenConstruct>c__AnonStorey8C.list.get_Item(num), r) == 2)
							{
								<GenConstruct>c__AnonStorey8C.list.RemoveAt(num);
								<GenConstruct>c__AnonStorey8C.list.Add(r);
							}
						}
						else
						{
			...
			if (<GenConstruct>c__AnonStorey8C.list.get_Count() == 0)
			{
				if (!ToLuaExport.type.get_IsValueType())
				{
					ToLuaExport.NoConsturct();
			...
			<GenConstruct>c__AnonStorey8C.list.Sort(new Comparison<ConstructorInfo>(ToLuaExport.Compare));
			ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
			ToLuaExport.sb.AppendFormat("\tstatic int _Create{0}(IntPtr L)\r\n", ToLuaExport.wrapClassName);
			ToLuaExport.sb.AppendLine("\t{");
			ToLuaExport.sb.AppendLine("\t\tint count = LuaDLL.lua_gettop(L);");
			...
			int i;
			for (i = 0; i < <GenConstruct>c__AnonStorey8C.list.get_Count(); i++)
			{
				int num2 = <GenConstruct>c__AnonStorey8C.list.FindIndex((ConstructorInfo p) => p != <GenConstruct>c__AnonStorey8C.list.get_Item(i) && p.GetParameters().Length == <GenConstruct>c__AnonStorey8C.list.get_Item(i).GetParameters().Length);
				if (num2 >= 0 || (ToLuaExport.HasOptionalParam(<GenConstruct>c__AnonStorey8C.list.get_Item(i).GetParameters()) && <GenConstruct>c__AnonStorey8C.list.get_Item(i).GetParameters().Length > 1))
				{
					list.Add(<GenConstruct>c__AnonStorey8C.list.get_Item(i));
				}
			}
			MethodBase methodBase = <GenConstruct>c__AnonStorey8C.list.get_Item(0);
			bool flag = <GenConstruct>c__AnonStorey8C.list.get_Item(0).GetParameters().Length == 0;
			...
			else
			{
				...
				if (<GenConstruct>c__AnonStorey8C.list.get_Count() == 1 || methodBase.GetParameters().Length != <GenConstruct>c__AnonStorey8C.list.get_Item(1).GetParameters().Length)
				{
			...
			ToLuaExport.sb.AppendLine("\t\t{");
			int num3 = ToLuaExport.ProcessParams(methodBase, 3, true, <GenConstruct>c__AnonStorey8C.list.get_Count() > 1, false);
			ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num3);
			ToLuaExport.sb.AppendLine("\t\t}");
			for (int j = 1; j < <GenConstruct>c__AnonStorey8C.list.get_Count(); j++)
			{
				flag = (<GenConstruct>c__AnonStorey8C.list.get_Item(j).GetParameters().Length == 0 || flag);
				methodBase = <GenConstruct>c__AnonStorey8C.list.get_Item(j);
				ParameterInfo[] parameters3 = methodBase.GetParameters();
				if (!ToLuaExport.HasOptionalParam(methodBase.GetParameters()))
				{
					if (list.Contains(<GenConstruct>c__AnonStorey8C.list.get_Item(j)))
					{
		...
		{
			int num = (!lhs.get_IsStatic()) ? 1 : 0;
			int num2 = (!rhs.get_IsStatic()) ? 1 : 0;
		...
		private static int ProcessParams(MethodBase md, int tab, bool beConstruct, bool beLuaString, bool beCheckTypes = false)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				string text2 = "arg" + j;
				int num2 = (!md.get_IsStatic() && !beConstruct) ? 2 : 1;
			...
			string text5 = (!md.get_IsStatic()) ? "obj" : ToLuaExport.className;
		...
		private static bool CompareParmsCount(MethodBase l, MethodBase r)
		{
			...
			int num = (!l.get_IsStatic()) ? 1 : 0;
			int num2 = (!r.get_IsStatic()) ? 1 : 0;
		...
		private static int CompareMethod(MethodBase l, MethodBase r)
		{
			...
			for (int k = 0; k < list.get_Count(); k++)
			{
				...
				{
					num = ((ToLuaExport.typeSize.get_Item(list.get_Item(k)) < ToLuaExport.typeSize.get_Item(list2.get_Item(k))) ? 2 : 1);
				}
		...
		{
			ToLuaExport.<GenOverrideFunc>c__AnonStorey8F <GenOverrideFunc>c__AnonStorey8F = new ToLuaExport.<GenOverrideFunc>c__AnonStorey8F();
			<GenOverrideFunc>c__AnonStorey8F.list = new List<MethodInfo>();
			for (int k = 0; k < ToLuaExport.methods.Length; k++)
			{
				...
				{
					ToLuaExport.Push(<GenOverrideFunc>c__AnonStorey8F.list, ToLuaExport.methods[k]);
				}
			}
			if (<GenOverrideFunc>c__AnonStorey8F.list.get_Count() == 1)
			{
				return <GenOverrideFunc>c__AnonStorey8F.list.get_Item(0);
			}
			<GenOverrideFunc>c__AnonStorey8F.list.Sort(new Comparison<MethodInfo>(ToLuaExport.Compare));
			ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
			...
			List<MethodInfo> list = new List<MethodInfo>();
			int i;
			for (i = 0; i < <GenOverrideFunc>c__AnonStorey8F.list.get_Count(); i++)
			{
				int num = <GenOverrideFunc>c__AnonStorey8F.list.FindIndex((MethodInfo p) => ToLuaExport.CompareParmsCount(p, <GenOverrideFunc>c__AnonStorey8F.list.get_Item(i)));
				if (num >= 0 || (ToLuaExport.HasOptionalParam(<GenOverrideFunc>c__AnonStorey8F.list.get_Item(i).GetParameters()) && <GenOverrideFunc>c__AnonStorey8F.list.get_Item(i).GetParameters().Length > 1))
				{
					list.Add(<GenOverrideFunc>c__AnonStorey8F.list.get_Item(i));
				}
			}
			ToLuaExport.sb.AppendLine();
			MethodInfo methodInfo = <GenOverrideFunc>c__AnonStorey8F.list.get_Item(0);
			int num2 = (methodInfo.get_ReturnType() != typeof(void)) ? 1 : 0;
			int num3 = (!methodInfo.get_IsStatic()) ? 1 : 0;
			int num4 = num3 + 1;
			int num5 = methodInfo.GetParameters().Length + num3;
			int num6 = <GenOverrideFunc>c__AnonStorey8F.list.get_Item(1).GetParameters().Length + ((!<GenOverrideFunc>c__AnonStorey8F.list.get_Item(1).get_IsStatic()) ? 1 : 0);
			bool flag = true;
			...
			ToLuaExport.sb.AppendLine("\t\t{");
			int num7 = ToLuaExport.ProcessParams(methodInfo, 3, false, <GenOverrideFunc>c__AnonStorey8F.list.get_Count() > 1 && flag, beCheckTypes);
			ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num2 + num7);
			ToLuaExport.sb.AppendLine("\t\t}");
			for (int j = 1; j < <GenOverrideFunc>c__AnonStorey8F.list.get_Count(); j++)
			{
				flag = true;
				beCheckTypes = true;
				methodInfo = <GenOverrideFunc>c__AnonStorey8F.list.get_Item(j);
				num3 = ((!methodInfo.get_IsStatic()) ? 1 : 0);
				num4 = num3 + 1;
				num2 = ((methodInfo.get_ReturnType() != typeof(void)) ? 1 : 0);
				if (!ToLuaExport.HasOptionalParam(methodInfo.GetParameters()))
				{
					ParameterInfo[] parameters3 = methodInfo.GetParameters();
					if (list.Contains(<GenOverrideFunc>c__AnonStorey8F.list.get_Item(j)))
					{
		...
		private static void GenNewIndexFunc()
		{
			for (int i = 0; i < ToLuaExport.fields.Length; i++)
			{
				if (!ToLuaExport.fields[i].get_IsLiteral() && !ToLuaExport.fields[i].get_IsInitOnly() && !ToLuaExport.fields[i].get_IsPrivate())
				{
					...
					ToLuaExport.sb.AppendLine("\t{");
					string o = (!ToLuaExport.fields[i].get_IsStatic()) ? "obj" : ToLuaExport.className;
					if (!ToLuaExport.fields[i].get_IsStatic())
					{
						ToLuaExport.sb.AppendFormat("\t\tobject o = LuaScriptMgr.GetLuaObject(L, 1);\r\n", new object[0]);
						if (!ToLuaExport.type.get_IsValueType())
		...
		private static void ProcessExtends(List<MethodInfo> list)
		{
			...
			if (ToLuaExport.extendType != null)
			{
				ToLuaExport.<ProcessExtends>c__AnonStorey91 <ProcessExtends>c__AnonStorey = new ToLuaExport.<ProcessExtends>c__AnonStorey91();
				<ProcessExtends>c__AnonStorey.list2 = new List<MethodInfo>();
				<ProcessExtends>c__AnonStorey.list2.AddRange(ToLuaExport.extendType.GetMethods(4 | ToLuaExport.binding | 2));
				int i;
				for (i = <ProcessExtends>c__AnonStorey.list2.get_Count() - 1; i >= 0; i--)
				{
					if ((!<ProcessExtends>c__AnonStorey.list2.get_Item(i).get_Name().Contains("op_") && !<ProcessExtends>c__AnonStorey.list2.get_Item(i).get_Name().Contains("add_") && !<ProcessExtends>c__AnonStorey.list2.get_Item(i).get_Name().Contains("remove_")) || ToLuaExport.IsNeedOp(<ProcessExtends>c__AnonStorey.list2.get_Item(i).get_Name()))
					{
						list.RemoveAll((MethodInfo md) => md.get_Name() == <ProcessExtends>c__AnonStorey.list2.get_Item(i).get_Name());
						if (!ToLuaExport.IsObsolete(<ProcessExtends>c__AnonStorey.list2.get_Item(i)))
						{
							list.Add(<ProcessExtends>c__AnonStorey.list2.get_Item(i));
						}
					}
				}
			}
