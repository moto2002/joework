...
public class AreaEventTriggerTextBubbleStatic : AreaEventTrigger
{
	...
	protected override DictionaryView<TriggerActionBase, RefParamOperator> DoActorEnter(ref PoolObjHandle<ActorRoot> inActor)
	{
		...
		if (!this.actorRoot || this.actorRoot.get_handle().isRecycled)
		{
			return null;
		}
		if (this.total_time > 0f)
		{
			...
			Singleton<CTimerManager>.get_instance().PauseTimer(this.timer);
		}
		...
		else if (this.actorRoot && !this.actorRoot.get_handle().ActorControl.IsDeadState && this.actorRoot.get_handle().HudControl != null)
		{
			this.actorRoot.get_handle().HudControl.SetTextHud(Utility.GetBubbleText((uint)this.DialogueGroupId), this.offset_x, this.offset_y, false);
		}
	...
	{
		if (this.actorRoot && this.actorRoot.get_handle().HudControl != null)
		{
			if (this.total_time > 0f)
			{
				Singleton<CTimerManager>.get_instance().ResumeTimer(this.timer);
			}
			this.actorRoot.get_handle().HudControl.SetTextHud(Utility.GetBubbleText((uint)this.DialogueGroupId), this.offset_x, this.offset_y, false);
		}
	}
	...
	private void SetHudText(string txt)
	{
		if (this.actorRoot)
		{
			this.actorRoot.get_handle().HudControl.SetTextHud(txt, this.offset_x, this.offset_y, false);
		}
	}
