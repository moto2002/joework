...
namespace Assets.Scripts.GameSystem
{
	public class HuoyueData
	{
		...
		public ResHuoYueDuReward GetRewardCfg(ushort id)
		{
			...
			GameDataMgr.huoyueduDict.TryGetValue(id, ref result);
			return result;
		}
		...
		public CUseable GetUsable(ushort id)
		{
			...
			if (cUseable == null)
			{
				...
				{
					cUseable = CUseableManager.CreateUseable(2, rewardCfg.dwRewardID, 0);
				}
				else
				{
					cUseable = CUseableManager.CreateUseable(2, resDT_HuoYueDuReward_PeriodInfo.dwRewardID, (int)resDT_HuoYueDuReward_PeriodInfo.dwRewardNum);
				}
			}
			return cUseable;
		}
		...
		private uint Calc_Day_Max_Num()
		{
			...
			for (int i = 0; i < this.day_huoyue_list.get_Count(); i++)
			{
				...
				GameDataMgr.huoyueduDict.TryGetValue(this.day_huoyue_list.get_Item(i), ref resHuoYueDuReward);
		...
		public void Set(RES_HUOYUEDU_TYPE type, uint num, int length, ushort[] ary)
		{
			...
			List<ushort> list = (type != 1) ? this.have_get_list_week : this.have_get_list_day;
			list.Clear();
		...
		{
			List<ushort> list = (type != 1) ? this.have_get_list_week : this.have_get_list_day;
			uint num = (type != 1) ? this.week_curNum : this.day_curNum;
