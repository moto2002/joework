...
namespace Assets.Scripts.GameLogic
{
	public class SkillSlot
	{
		...
		public bool IsEnergyEnough
		{
			get
			{
				return this.Actor && this.Actor.get_handle() != null && this.Actor.get_handle().ValueComponent != null && (this.Actor.get_handle().ValueComponent.IsEnergyType(EnergyType.NoneResource) || this.Actor.get_handle().ValueComponent.actorEp >= this.NextSkillEnergyCostTotal());
			}
		}
		...
		public bool EnableButtonFlag
		{
			get
			{
				return this.Actor && this.Actor.get_handle() != null && this.Actor.get_handle().ValueComponent != null && (this.CurSkillCD <= 0 && this.IsEnergyEnough && this.skillLevel > 0 && this.skillBean.IsBeanEnough()) && (!this.Actor.get_handle().ActorControl.IsDeadState || this.Actor.get_handle().TheStaticData.TheBaseAttribute.DeadControl);
			}
		}
		...
		public void AddSkillUseCount()
		{
			...
			this.Actor.get_handle().BuffHolderComp.logicEffect.InitSkillSlotExtraHurt(this);
		}
		...
		public int GetSkillCDMax()
		{
			...
			{
				if (this.Actor && this.Actor.get_handle().SkillControl.bZeroCd)
				{
					num = 0;
				}
				return num;
			...
			{
				int num3 = 0;
				ValueDataInfo valueDataInfo = this.Actor.get_handle().ValueComponent.mActorValue[20];
			...
			{
				ValueDataInfo valueDataInfo = this.Actor.get_handle().ValueComponent.mActorValue[18];
				DebugHelper.Assert(valueDataInfo != null, "Failed get value data skill 0");
				num = num * 10000 / (10000 + valueDataInfo.totalValue);
				num = ((num >= 0) ? num : 0);
			}
			if (this.Actor && this.Actor.get_handle().SkillControl.bZeroCd && this.SlotType != SkillSlotType.SLOT_SKILL_0)
			{
				num = 0;
			}
			return num;
		...
		public void StartSkillCD()
		{
			...
			{
				this.Actor.get_handle().SkillControl.ornamentFirstSwitchCdEftTime = 0;
			}
		...
		{
			if (this.Actor && this.Actor.get_handle().SkillControl != null && this.Actor.get_handle().SkillControl.bZeroCd)
			{
				return 0;
			}
		...
		{
			if (!this.Actor || this.Actor.get_handle().ValueComponent == null || this.Actor.get_handle().ValueComponent.IsEnergyType(EnergyType.NoneResource))
			{
				return;
			}
			this.Actor.get_handle().ValueComponent.actorEp -= this.CurSkillEnergyCostTotal();
		}
		...
		private bool IsSkillUnlock(int slotType)
		{
			...
			{
				SkillSlot skillSlot = this.Actor.get_handle().SkillControl.GetSkillSlot((SkillSlotType)slotType);
				if (skillSlot != null)
				{
		...
		{
			return this.IsCDReady && this.IsUnLock() && !this.bLimitUse && (!this.Actor.get_handle().ActorControl.IsDeadState || this.Actor.get_handle().TheStaticData.TheBaseAttribute.DeadControl) && this.IsEnergyEnough;
		}

		public bool CanEnableSkillSlotByEnergy()
		{
			return this.IsCDReady && this.IsUnLock() && !this.bLimitUse && this.skillBean.IsBeanEnough() && !this.Actor.get_handle().ActorControl.IsDeadState;
		}

		public bool IsUseSkillJoystick()
		{
			return this.SkillObj != null && this.SkillObj.cfgData != null && (this.IsEnableSkillSlot() || this.Actor.get_handle().ActorControl.IsUseAdvanceCommonAttack()) && (this.SlotType != SkillSlotType.SLOT_SKILL_0 || this.Actor.get_handle().ActorControl.IsUseAdvanceCommonAttack());
		}
		...
		private void ReadyUseSkillDefaultAttackMode(Skill readySkillObj)
		{
			...
			{
				actorRoot = Singleton<TargetSearcher>.GetInstance().GetLowestHpTarget(this.Actor.get_handle(), readySkillObj.cfgData.iMaxAttackDistance, TargetPriority.TargetPriority_Hero, readySkillObj.cfgData.dwSkillTargetFilter, true, true);
			}
			if (actorRoot != null)
			{
		...
		public void ReadyUseSkill()
		{
			...
			{
				this.ReadyUseSkillDefaultAttackMode(skill);
			}
			else if (playerOperateMode == OperateMode.LockMode)
			{
				this.ReadyUseSkillLockAttackMode(skill);
			}
		}
		...
		{
			uint num = 0u;
			BaseAttackMode currentAttackMode = this.Actor.get_handle().ActorControl.GetCurrentAttackMode();
			FrameCommand<UseObjectiveSkillCommand> frameCommand = FrameCommandFactory.CreateCSSyncFrameCommand<UseObjectiveSkillCommand>();
		...
		private bool SendRequestUseSkillDir(Skill readySkillObj)
		{
			...
			{
				vInt = this.Actor.get_handle().forward;
			}
			short degree = (short)((double)(IntMath.atan2(-vInt.z, vInt.x).get_single() * 180f) / 3.1416);
		...
		private bool SendRequestUseSkill()
		{
			...
			if (skill.AppointType == 3)
			{
				this.SendRequestUseSkillDir(skill);
			}
			else if (skill.AppointType == 2)
			{
		...
		{
			Skill skill = (this.NextSkillObj == null) ? this.SkillObj : this.NextSkillObj;
			return skill.cfgData.dwSkillTargetFilter == 0u || ((ulong)skill.cfgData.dwSkillTargetFilter & (ulong)(1L << (int)(_target.get_handle().TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL;
		}
		...
		public bool IsValidSkillTarget(ref PoolObjHandle<ActorRoot> _target)
		{
			...
			{
				if (this.Actor.get_handle().IsEnemyCamp(_target) && this.IsValidSkillTargetFilter(ref _target))
				{
					return true;
				}
			}
			...
			{
				if (this.Actor.get_handle().IsSelfCamp(_target) && this.IsValidSkillTargetFilter(ref _target))
				{
					return skill.cfgData.bWheelType != 1 || !(this.Actor == _target);
				}
			}
			else if (skill.cfgData.bSkillTargetRule == 5 && this.Actor.get_handle().IsEnemyCamp(_target) && this.IsValidSkillTargetFilter(ref _target) && this.NextSkillTargetIDs.IndexOf(_target.get_handle().ObjID) >= 0)
			{
				return true;
			}
			return false;
		...
		public bool IsSkillUseValid(ref SkillUseParam _param)
		{
			...
			{
				if (_param.SlotType != SkillSlotType.SLOT_SKILL_0 && this.Actor && this.Actor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && _param.AppointType != skill.AppointType)
				{
					return false;
				}
				if (skill.AppointType == 1 || skill.AppointType == null)
				{
					...
					num += ((_param.TargetActor.get_handle().shape == null) ? 0L : ((long)_param.TargetActor.get_handle().shape.AvgCollisionRadius));
					long num2 = num * num;
					long sqrMagnitudeLong2D = (this.Actor.get_handle().location - _param.TargetActor.get_handle().location).get_sqrMagnitudeLong2D();
				...
				else if (skill.AppointType == 2)
				{
					long num = skill.cfgData.iMaxAttackDistance * 12000L / 10000L;
					long num2 = num * num;
					long sqrMagnitudeLong2D = (this.Actor.get_handle().location - _param.UseVector).get_sqrMagnitudeLong2D();
					if (sqrMagnitudeLong2D <= num2)
					{
						return true;
					}
