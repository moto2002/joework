...
namespace Assets.Scripts.GameSystem
{
	...
	public class CShopSystem : Singleton<CShopSystem>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler("MasterAttributesChanged", new Action(this.MasterAttrChanged));
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler("MasterAttributesChanged", new Action(this.MasterAttrChanged));
		...
		{
			ResShopType dataByKey = GameDataMgr.shopTypeDatabin.GetDataByKey((uint)shopType);
			if (dataByKey == null)
			{
				return false;
			}
			if (this.m_Shops.ContainsKey(shopType) && this.m_ShopItems.ContainsKey(shopType) && this.m_ShopItems.get_Item(shopType).Length == 0)
			{
				return true;
			}
			stShopInfo stShopInfo = new stShopInfo();
			bool flag = this.m_Shops.TryGetValue(shopType, ref stShopInfo);
			if (stShopInfo == null)
			{
				return false;
			}
			...
			for (int i = 0; i < refreshTime.Length; i++)
			{
				...
				dateTime4 = dateTime5.AddSeconds((double)(num * 3600) + (double)(num2 * 60) - 28800.0);
				while (DateTime.Compare(dateTime4, dateTime2) <= 0)
				{
					if (DateTime.Compare(dateTime3, dateTime4) < 0 && DateTime.Compare(dateTime4, dateTime2) < 0)
					{
						return true;
					}
		...
		public void RefreshShopItemsPanel()
		{
			...
			if (widget != null)
			{
				...
				{
					byte b = 0;
					if (this.m_ShopItems.ContainsKey(this.m_CurShopType))
					{
						b = Convert.ToByte(Math.Ceiling((double)this.m_ShopItems.get_Item(this.m_CurShopType).Length / 6.0));
					}
					if (b <= 1)
					{
		...
		private void SetPageItems(CUIListElementScript listElementScript, byte page)
		{
			...
			while (i < num)
			{
				...
				if (i >= this.m_ShopItems.get_Item(this.m_CurShopType).Length)
				{
					if (transform != null)
					{
				...
				else if (transform != null)
				{
					...
					this.SetItemInfo(gameObject2, ref this.m_ShopItems.get_Item(this.m_CurShopType)[i], i);
				}
				b += 1;
				i++;
			}
		...
		{
			return (this.m_CurShopType == 6 && (long)slotOffset >= (long)((ulong)CGuildHelper.GetGuildItemShopOpenSlotCount())) || (this.m_CurShopType == 7 && (long)slotOffset >= (long)((ulong)CGuildHelper.GetGuildHeadImageShopOpenSlotCount()));
		}
		...
		private void ResetManualRefreshCountIfNeed()
		{
			...
			if (this.m_Shops.TryGetValue(this.m_CurShopType, ref stShopInfo))
			{
		...
		private void RefreshBuyPnl()
		{
			...
			stShopItemInfo stShopItemInfo = Singleton<CShopSystem>.GetInstance().m_ShopItems.get_Item(this.m_CurShopType)[this.m_currentSelectItemIdx];
			...
			int num = 0;
			if (stShopItemInfo.enItemType == 4)
			{
			...
			else if (stShopItemInfo.enItemType == 7)
			{
				uint heroId = 0u;
				uint skinId = 0u;
			...
			if (stShopItemInfo.enItemType == 2)
			{
			...
			else if (stShopItemInfo.enItemType == 3)
			{
			...
			else if (stShopItemInfo.enItemType == 5)
			{
			}
		...
		private DateTime GetNextAutoRefreshTime(RES_SHOP_TYPE shopType)
		{
			...
			int[] refreshTime = GameDataMgr.shopTypeDatabin.GetDataByKey((uint)shopType).RefreshTime;
			for (int i = 0; i < refreshTime.Length; i++)
			{
		...
		{
			return GameDataMgr.shopRefreshCostDatabin.FindIf((ResShopRefreshCost x) => x.wShopType == shopType && refreshCnt == (int)x.wRefreshFreq);
		}
			...
			if (this.m_saleItemsCnt > 0)
			{
				...
				while ((ushort)b < this.m_saleItemsCnt)
				{
					...
					case 2:
					{
						...
						stShopItemInfo.enItemType = 2;
						stShopItemInfo.enCostType = 4;
		...
		private void SetSaleItemInfo(CUIListElementScript listElementScript, ref stShopItemInfo info)
		{
			...
			{
			case 2:
				cUseable = CUseableManager.CreateUseable(2, info.dwItemId, (int)info.wItemCnt);
				if (cUseable != null)
				{
				...
				break;
			case 3:
				cUseable = CUseableManager.CreateUseable(3, info.dwItemId, (int)info.wItemCnt);
				if (cUseable != null)
				{
				...
				break;
			case 5:
				cUseable = CUseableManager.CreateUseable(5, info.dwItemId, (int)info.wItemCnt);
				if (cUseable != null)
				{
					num = GameDataMgr.itemDatabin.GetDataByKey(info.dwItemId).dwCoinSale;
				}
		...
		private int GetManualRefreshedCnt()
		{
			...
			if (this.m_Shops != null && this.m_Shops.TryGetValue(this.m_CurShopType, ref shopInfo))
			{
		...
		private static int GetManualRefreshMaxCnt(RES_SHOP_TYPE shopType)
		{
			...
			{
				if (GameDataMgr.shopRefreshCostDatabin.GetDataByIndex(i).wShopType == shopType && (int)GameDataMgr.shopRefreshCostDatabin.GetDataByIndex(i).wRefreshFreq > num)
				{
		...
		{
			if (Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(12))
			{
				if (this.m_IsShopFormOpen)
				{
					return;
		...
		private void OnShop_Tab_Change(CUIEvent uiEvent)
		{
			...
			int curTab;
			if (!int.TryParse(component2.text, ref curTab))
			{
			...
			case 0:
				...
				break;
			case 1:
				if (!Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(14))
				{
					ResSpecialFucUnlock dataByKey2 = GameDataMgr.specialFunUnlockDatabin.GetDataByKey(14u);
					Singleton<CUIManager>.get_instance().OpenTips(Utility.UTF8Convert(dataByKey2.szLockedTip), false, 1.5f, null, new object[0]);
					flag = false;
				}
				break;
			case 2:
				if (!Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(4))
				{
					...
					Singleton<CUIManager>.get_instance().OpenTips(Utility.UTF8Convert(dataByKey3.szLockedTip), false, 1.5f, null, new object[0]);
					flag = false;
				}
				break;
			case 3:
				if (!Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(9))
				{
					...
					Singleton<CUIManager>.get_instance().OpenTips(Utility.UTF8Convert(dataByKey4.szLockedTip), false, 1.5f, null, new object[0]);
					flag = false;
				}
				break;
			case 4:
				if (!Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(17))
				{
					ResSpecialFucUnlock dataByKey5 = GameDataMgr.specialFunUnlockDatabin.GetDataByKey(17u);
					Singleton<CUIManager>.get_instance().OpenTips(Utility.UTF8Convert(dataByKey5.szLockedTip), false, 1.5f, null, new object[0]);
					flag = false;
				}
				break;
			}
		...
		private void OnShop_SubTab_Change(CUIEvent uiEvent)
		{
			...
			if (subTab != CShopSystem.SubTab.Guild_Item_Shop)
			{
				...
				{
					this.m_CurShopType = 7;
					widget.CustomSetActive(false);
				}
			}
			else
			{
				this.m_CurShopType = 6;
				widget.CustomSetActive(true);
			}
			this.CurTab = CShopSystem.Tab.Guild_Shop;
			this.InitShop();
		...
		private void OnShop_SelectItem(CUIEvent uiEvent)
		{
			...
			if (this.m_currentSelectItemIdx < 0 || this.m_currentSelectItemIdx >= Singleton<CShopSystem>.GetInstance().m_ShopItems.get_Item(this.m_CurShopType).Length)
			{
				return;
			}
			stShopItemInfo stShopItemInfo = Singleton<CShopSystem>.GetInstance().m_ShopItems.get_Item(this.m_CurShopType)[this.m_currentSelectItemIdx];
		...
		private void OnShop_ConfirmManualRefresh(CUIEvent uiEvent)
		{
			...
			this.m_Shops.set_Item(this.m_CurShopType, stShopInfo);
			...
			cSPKG_CMD_MANUALREFRESH.wShopType = this.m_CurShopType;
			cSPkg.stPkgData.set_stManualRefresh(cSPKG_CMD_MANUALREFRESH);
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public void OnShop_SaleTipConfirm(CUIEvent uiEvent)
		{
			...
			if (listView.get_Count() > 0)
			{
				...
				cSDT_ITEM_DELLIST.wItemCnt = (ushort)listView.get_Count();
				cSDT_ITEM_DELLIST.astItemList = LinqS.ToArray<CSDT_ITEM_DELINFO>(listView);
				cSPkg.stPkgData.get_stItemSale().stSaleList = cSDT_ITEM_DELLIST;
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
			}
		}
		...
		private void OnRequestAutoRefreshShopItems()
		{
			...
			cSPKG_CMD_AUTOREFRESH.wShopType = this.m_CurShopType;
			cSPkg.stPkgData.set_stAutoRefresh(cSPKG_CMD_AUTOREFRESH);
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		...
		{
			SCPKG_NTF_SHOPTIMEOUT stShopTimeOut = msg.stPkgData.get_stShopTimeOut();
			string text = Singleton<CTextManager>.GetInstance().GetText("Shop_Err_Shop_Time_Out");
			string text2 = string.Empty;
			RES_SHOP_TYPE wShopType = stShopTimeOut.wShopType;
			if (wShopType != 1)
			{
				if (wShopType != 2)
				{
		...
		public static void ReceiveItemBuy(CSPkg msg)
		{
			...
			SCPKG_CMD_ITEMBUY stItemBuyRsp = msg.stPkgData.get_stItemBuyRsp();
			int num = 0;
			stShopInfo stShopInfo = new stShopInfo();
			if (instance.m_Shops.TryGetValue(instance.m_CurShopType, ref stShopInfo))
			{
			...
			instance.m_ShopItems.get_Item(instance.m_CurShopType)[(int)stItemBuyRsp.bItemIdx].isSoldOut = true;
			Singleton<EventRouter>.GetInstance().BroadCastEvent<RES_SHOP_TYPE>(EventID.Shop_Receive_Shop_Items, instance.m_CurShopType);
			stShopItemInfo stShopItemInfo = instance.m_ShopItems.get_Item(instance.m_CurShopType)[(int)stItemBuyRsp.bItemIdx];
			switch (stShopItemInfo.enItemType)
			{
			case 4:
				CUICommonSystem.ShowNewHeroOrSkin(stShopItemInfo.dwItemId, 0u, enUIEventID.None, true, 5, true, null, enFormPriority.Priority2, 0u, 0);
				break;
			case 7:
			{
				uint heroId = 0u;
				uint skinId = 0u;
				CSkinInfo.ResolveHeroSkin(stShopItemInfo.dwItemId, out heroId, out skinId);
				CUICommonSystem.ShowNewHeroOrSkin(heroId, skinId, enUIEventID.None, true, 10, true, null, enFormPriority.Priority2, 0u, 0);
				break;
			}
			}
		}
		...
		{
			COMDT_SHOP_DETAIL stShopInfo = msg.stPkgData.get_stShopDetail().stShopInfo;
			COMDT_SHOP_ITEMLIST stItemList = stShopInfo.stItemList;
			COMDT_SHOP_ITEMINFO[] astShopItem = stItemList.astShopItem;
			RES_SHOP_TYPE wShopType = stShopInfo.wShopType;
			CShopSystem instance = Singleton<CShopSystem>.GetInstance();
			...
			ResShopType dataByKey = GameDataMgr.shopTypeDatabin.GetDataByKey((uint)wShopType);
			if (dataByKey != null)
			{
			...
			if (!instance.m_Shops.ContainsKey(wShopType))
			{
				instance.m_Shops.Add(wShopType, stShopInfo2);
			}
			else
			{
				instance.m_Shops.set_Item(wShopType, stShopInfo2);
			}
			if (instance.m_ShopItems.ContainsKey(wShopType))
			{
				instance.m_ShopItems.set_Item(wShopType, new stShopItemInfo[(int)stItemList.bItemCnt]);
			}
			else
			{
				instance.m_ShopItems.Add(wShopType, new stShopItemInfo[(int)stItemList.bItemCnt]);
			}
			for (byte b = 0; b < stItemList.bItemCnt; b += 1)
			{
				...
				stShopItemInfo.enItemType = cOMDT_SHOP_ITEMINFO.wItemType;
				stShopItemInfo.enCostType = cOMDT_SHOP_ITEMINFO.bCostType;
				...
				instance.m_ShopItems.get_Item(wShopType)[(int)b] = stShopItemInfo;
			}
