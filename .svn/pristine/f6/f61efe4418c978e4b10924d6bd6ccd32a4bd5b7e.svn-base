...
namespace Assets.Scripts.GameSystem
{
	public class ScoreBoard
	{
		...
		public void Update()
		{
			...
			{
				if (Singleton<WinLoseByStarSys>.get_instance().CurLevelTimeDuration <= 0u)
				{
					int num2 = num / 60;
				...
				{
					uint num4 = Singleton<WinLoseByStarSys>.get_instance().CurLevelTimeDuration / 1000u;
					uint num5 = 0u;
					if (num4 > (uint)num)
					{
						num5 = num4 - (uint)num;
		...
		private void UpdateMVPHonor()
		{
			...
			{
				Player player = Singleton<GamePlayerCenter>.get_instance().GetPlayer(lastBestPlayer);
				if (player != null)
				{
			...
			Player player2 = Singleton<GamePlayerCenter>.get_instance().GetPlayer(bestPlayer);
			if (player2 != null)
			{
