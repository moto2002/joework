...
public class AkGameObj : MonoBehaviour
{
	...
	private void OnTriggerExit(Collider other)
	{
		if (this.isEnvironmentAware)
		{
			...
			if (component2 != null)
			{
				for (int j = 0; j < this.m_envData.activePortals.get_Count(); j++)
				{
					for (int k = 0; k < 2; k++)
					{
						if (component2 == this.m_envData.activePortals.get_Item(j).environments[k])
						{
							this.m_envData.auxSendValues = null;
							this.UpdateAuxSend();
							return;
	...
	private void UpdateAuxSend()
	{
		...
		{
			this.m_envData.auxSendValues = new AkAuxSendArray((uint)((this.m_envData.activeEnvironments.get_Count() >= AkEnvironment.MAX_NB_ENVIRONMENTS) ? AkEnvironment.MAX_NB_ENVIRONMENTS : this.m_envData.activeEnvironments.get_Count()));
		}
		else
		{
			this.m_envData.auxSendValues.Reset();
		}
		for (int i = 0; i < this.m_envData.activePortals.get_Count(); i++)
		{
			for (int j = 0; j < 2; j++)
			{
				AkEnvironment akEnvironment = this.m_envData.activePortals.get_Item(i).environments[j];
				...
				{
					this.m_envData.auxSendValues.Add(akEnvironment.GetAuxBusID(), this.m_envData.activePortals.get_Item(i).GetAuxSendValueForPosition(base.transform.position, j));
				}
			}
		}
		if ((ulong)this.m_envData.auxSendValues.m_Count < (ulong)((long)AkEnvironment.MAX_NB_ENVIRONMENTS) && (ulong)this.m_envData.auxSendValues.m_Count < (ulong)((long)this.m_envData.activeEnvironments.get_Count()))
		{
			...
			int num = Math.Min(AkEnvironment.MAX_NB_ENVIRONMENTS - (int)this.m_envData.auxSendValues.m_Count, this.m_envData.activeEnvironments.get_Count() - (int)this.m_envData.auxSendValues.m_Count);
			for (int k = 0; k < num; k++)
			{
				if (!this.m_envData.auxSendValues.Contains(listView.get_Item(k).GetAuxBusID()))
				{
					if (!listView.get_Item(k).isDefault || k == 0)
					{
						this.m_envData.auxSendValues.Add(listView.get_Item(k).GetAuxBusID(), listView.get_Item(k).GetAuxSendValueForPosition(base.transform.position));
						if (listView.get_Item(k).excludeOthers)
						{
							break;
						}
					}
