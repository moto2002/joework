...
namespace Assets.Scripts.GameSystem
{
	...
	public class MySteryShop : Singleton<MySteryShop>
	{
		...
		public int GetTimeToClose()
		{
			...
			{
				return (timeSpan.get_TotalSeconds() <= 2147483647.0) ? ((int)timeSpan.get_TotalSeconds()) : 2147483647;
			}
			return 0;
		}
		...
		private void UpdateItem(CUIEvent uiEvent, COMDT_AKALISHOP_GOODS productInfo)
		{
			...
			COM_ITEM_TYPE wItemType = productInfo.wItemType;
			...
			case 7:
			{
				...
				GameDataMgr.skinShopInfoDict.TryGetValue(heroSkin.dwSkinID, ref resHeroSkinShop);
		...
		{
			Singleton<MySteryShop>.GetInstance().m_AkaliShopDetail = msg.stPkgData.get_stAkaliShopInfo().stAkaliShopData;
			Singleton<MySteryShop>.GetInstance().m_AkaliShopInfoSvr = msg.stPkgData.get_stAkaliShopInfo().stAkaliShopBuy;
			Singleton<MySteryShop>.GetInstance().m_bRequestDiscount = msg.stPkgData.get_stAkaliShopInfo().stAkaliShopBuy.bAlreadyGet;
			Singleton<MySteryShop>.GetInstance().m_bRequestBuyCount = msg.stPkgData.get_stAkaliShopInfo().stAkaliShopBuy.bBuyCnt;
		}
		...
		public static void ReceiveDiscount(CSPkg msg)
		{
			...
			if (msg.stPkgData.get_stAkaliShopFlagRsp().iResult == 0)
			{
		...
		private void RequestBuyItem(byte buyIdx)
		{
			...
			cSPkg.stPkgData.get_stAkaliShopBuyReq().bBuyIdx = buyIdx;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		{
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			if (msg.stPkgData.get_stAkaliShopBuyRsp().iResult == 0)
			{
				MySteryShop expr_24 = Singleton<MySteryShop>.GetInstance();
				expr_24.m_bRequestBuyCount += 1;
				Singleton<MySteryShop>.GetInstance().UpdateUI();
				byte bBuyIdx = msg.stPkgData.get_stAkaliShopBuyRsp().bBuyIdx;
				if (bBuyIdx < Singleton<MySteryShop>.GetInstance().m_AkaliShopDetail.bGoodsCnt)
				{
					...
					{
						CUICommonSystem.ShowNewHeroOrSkin(dwItemID, 0u, enUIEventID.None, true, 5, false, null, enFormPriority.Priority1, 0u, 0);
					}
			...
			{
				Singleton<CUIManager>.GetInstance().OpenTips(Utility.ProtErrCodeToStr(1408, msg.stPkgData.get_stAkaliShopBuyRsp().iResult), false, 1.5f, null, new object[0]);
			}
		}
	}
}
