	...
	public class CPlayerSoulLevelStat
	{
		...
		private void OnSoulLvlChange(PoolObjHandle<ActorRoot> act, int curSoulLevel)
		{
			...
			if (act)
			{
				...
				uint playerId = act.get_handle().TheActorMeta.PlayerId;
				bool flag = false;
				...
				if (curSoulLevel <= ValueProperty.GetMaxSoulLvl() && curSoulLevel > 0)
				{
					soulLevelDetail.changeTime[curSoulLevel - 1] = (uint)Singleton<FrameSynchr>.get_instance().LogicFrameTick;
				}
			}
		}
		...
		public uint GetPlayerLevelChangedTime(uint playerId, uint soulLevel)
		{
			for (int i = 0; i < this.playerSoulLevelDetail.get_Count(); i++)
			{
				...
				{
					return this.playerSoulLevelDetail.get_Item(i).changeTime[(int)((UIntPtr)(soulLevel - 1u))];
				}
			}
			return 0u;
		}
