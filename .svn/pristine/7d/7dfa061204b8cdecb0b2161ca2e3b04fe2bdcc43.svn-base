...
namespace Assets.Scripts.GameSystem
{
	public class CBattleFloatDigitManager
	{
		...
		private void updateFloatDigitInLastFrame()
		{
			...
			for (int i = 0; i < this.m_floatDigitInfoList.get_Count(); i++)
			{
				...
				if (floatDigitInfo.m_attacker && floatDigitInfo.m_target)
				{
					...
					{
						Vector3 position = floatDigitInfo.m_target.get_handle().myTransform.position;
						this.CreateBattleFloatDigit(floatDigitInfo.m_value, floatDigitInfo.m_digitType, ref position);
					}
					else
					{
						Vector3 position2 = floatDigitInfo.m_target.get_handle().myTransform.position;
						Vector3 position3 = floatDigitInfo.m_attacker.get_handle().myTransform.position;
		...
		{
			string text = Singleton<CTextManager>.GetInstance().GetText(CBattleFloatDigitManager.s_restrictTextKeys[restrictType]);
		...
		public void CreateSpecifiedFloatText(uint floatTextID, ref Vector3 worldPosition)
		{
			...
			if (dataByKey != null)
			{
				string animatorState = (dataByKey.szAnimation.get_Length() <= 0) ? CBattleFloatDigitManager.s_restrictTextAnimatorState : dataByKey.szAnimation;
		...
		private void CreateBattleFloatText(string content, ref Vector3 worldPosition, string animatorState, uint fontSize = 0u)
		{
			...
			GameObject gameObject = Singleton<CGameObjectPool>.GetInstance().GetGameObject("Text/FloatText/FloatText.prefab", 0);
			...
			if (texMeshPro != null)
			{
				texMeshPro.set_text(content);
				texMeshPro.set_fontSize((fontSize <= 0u) ? 6u : fontSize);
			}
			Singleton<CGameObjectPool>.GetInstance().RecycleGameObjectDelay(gameObject, 2000, null);
		}
