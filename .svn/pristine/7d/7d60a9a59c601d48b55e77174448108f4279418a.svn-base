	...
	public class BurnExpeditionView
	{
		...
		private void _show_LevelNode(int index, string name, string icon, COM_LEVEL_STATUS state)
		{
			...
			if (gameObject != null)
			{
				...
				gameObject.GetComponent<CUIEventScript>().enabled = (state == 1);
			}
			this._show_Node_ByState(gameObject, Singleton<BurnExpeditionController>.get_instance().model.lastUnlockLevelIndex == index, state);
		}
