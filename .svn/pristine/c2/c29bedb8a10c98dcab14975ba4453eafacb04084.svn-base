...
namespace AGE
{
	...
	public class SetNextSkillTargetDuration : DurationCondition
	{
		...
		public override void Enter(Action _action, Track _track)
		{
			...
			if (skillSlot != null && this.nextSkillTargetID > 0 && skillSlot.NextSkillTargetIDs.IndexOf(this.nextSkillObj.get_handle().ObjID) < 0)
			{
				skillSlot.NextSkillTargetIDs.Add(this.nextSkillObj.get_handle().ObjID);
			}
		}
		...
		public override void Process(Action _action, Track _track, int _localTime)
		{
			...
			if (this.nextSkillObj.get_handle().ActorControl.IsDeadState)
			{
		...
		private void RemoveTarget(Action _action, PoolObjHandle<ActorRoot> nextObj)
		{
			...
			if (skillSlot != null)
			{
				int num = skillSlot.NextSkillTargetIDs.IndexOf(nextObj.get_handle().ObjID);
				if (num >= 0)
				{
