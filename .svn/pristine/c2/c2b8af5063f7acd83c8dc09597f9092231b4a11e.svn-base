...
namespace Assets.Scripts.GameSystem
{
	public class CFriendHeartData
	{
		...
		public bool BCanSendHeart(COMDT_ACNT_UNIQ uniq)
		{
			...
			return heartDataIndex == -1 || this._sendHeartList.get_Item(heartDataIndex).bCanSend;
		}
		...
		private CFriendHeartData.CDHeartData GetFriendData(COMDT_ACNT_UNIQ uniq)
		{
			...
			return this._sendHeartList.get_Item(heartDataIndex);
		}
		...
		private int GetHeartDataIndex(COMDT_ACNT_UNIQ uniq)
		{
			...
			for (int i = 0; i < this._sendHeartList.get_Count(); i++)
			{
				CFriendHeartData.CDHeartData cDHeartData = this._sendHeartList.get_Item(i);
