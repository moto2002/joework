	...
	public class ObjWrapper : LogicComponent
	{
		...
		public void SetInBattle()
		{
			...
			if (curLvelContext != null && !curLvelContext.IsMobaMode() && ownerPlayer != null)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					{
						allHeroes.get_Item(i).get_handle().ActorControl.SetSelfInBattle();
					}
				}
			}
			else
		...
		public void SetSelfInBattle()
		{
			...
			if (!isInBattle)
			{
				...
				Singleton<GameEventSys>.get_instance().SendEvent<DefaultGameEventParam>(GameEventDef.Event_ActorEnterCombat, ref defaultGameEventParam);
		...
		public void SetSelfExitBattle()
		{
			...
			DefaultGameEventParam defaultGameEventParam = new DefaultGameEventParam(base.GetActor(), base.GetActor());
			Singleton<GameEventSys>.get_instance().SendEvent<DefaultGameEventParam>(GameEventDef.Event_ActorExitCombat, ref defaultGameEventParam);
		...
		public void SetInAttack(PoolObjHandle<ActorRoot> inAttackee, bool bExposeAttacker)
		{
			...
			if (FogOfWar.enable)
			{
				if (inAttackee)
				{
					this.actor.HorizonMarker.ExposeAndShowAsAttacker(inAttackee.get_handle().TheActorMeta.ActorCamp, bExposeAttacker);
				}
			}
			else if (bExposeAttacker)
			{
				this.actor.HorizonMarker.SetExposeMark(true, (!inAttackee) ? 3 : inAttackee.get_handle().TheActorMeta.ActorCamp, false);
			}
		}
		...
		public void SetAutoAI(bool autoAI)
		{
			if (this.m_isAutoAI != autoAI)
			{
				...
				Singleton<GameEventSys>.get_instance().SendEvent<DefaultGameEventParam>(GameEventDef.Event_AutoAISwitch, ref defaultGameEventParam);
			}
			if (!autoAI)
			{
				this.m_offline = false;
		...
		protected virtual void OnDead()
		{
			...
			GameDeadEventParam gameDeadEventParam = new GameDeadEventParam(this.actorPtr, this.myLastAtker, poolObjHandle, logicAtker, this.m_reviveContext.bEnable, null);
			if (this.eventActorDead != null)
			{
				this.eventActorDead(ref gameDeadEventParam);
			}
			Singleton<GameEventSys>.get_instance().SendEvent<GameDeadEventParam>(GameEventDef.Event_ActorDead, ref gameDeadEventParam);
			Singleton<GameEventSys>.get_instance().SendEvent<GameDeadEventParam>(GameEventDef.Event_PostActorDead, ref gameDeadEventParam);
		}
		...
		protected virtual void OnRevive()
		{
			...
			Singleton<GameEventSys>.get_instance().SendEvent<DefaultGameEventParam>(GameEventDef.Event_ActorRevive, ref defaultGameEventParam);
		...
		public override void Fight()
		{
			...
			this.actor.ValueComponent.mActorValue.SetChangeEvent(5, new ValueChangeDelegate(this.OnHpChange));
			this.actor.ValueComponent.mActorValue.SetChangeEvent(15, new ValueChangeDelegate(this.OnMoveSpdChange));
		...
		public virtual void SelectTarget(PoolObjHandle<ActorRoot> tar)
		{
			...
			{
				if (this.myTarget.get_handle().ObjID != this.myTarget.get_handle().ObjID)
				{
				}
			}
		...
		{
			if (this.actorPtr && this.actorPtr.get_handle().SkillControl != null)
			{
				this.actorPtr.get_handle().SkillControl.SetDisableSkillSlot(_type, true);
			}
		}
		...
		public virtual void RmvDisableSkillFlag(SkillSlotType _type, bool bForce = false)
		{
			if (this.actorPtr && this.actorPtr.get_handle().SkillControl != null)
			{
				this.actorPtr.get_handle().SkillControl.SetDisableSkillSlot(_type, false);
			}
		}
		...
		public bool SetSkill(SkillSlotType InSlot, bool bSpecial)
		{
			...
			if (!bSpecial)
			{
				...
				if (skill.cfgData.bRangeAppointType == 2)
				{
					...
					this.curSkillUseInfo.Init(InSlot, this.myTarget.get_handle().location);
				}
				else if (skill.cfgData.bRangeAppointType == 1)
				{
					this.curSkillUseInfo = default(SkillUseParam);
					this.curSkillUseInfo.Init(InSlot, this.myTarget.get_handle().ObjID);
				}
				else if (skill.cfgData.bRangeAppointType == 3)
				{
					this.curSkillUseInfo = default(SkillUseParam);
					this.curSkillUseInfo.Init(InSlot, this.myTarget.get_handle().location - this.actor.location, false, this.myTarget.get_handle().ObjID);
				}
				else
				{
					this.curSkillUseInfo = default(SkillUseParam);
		...
		public void BTExMoveCmd()
		{
			...
			else if (this.curMoveCommand.cmdType == 131)
			{
				...
				VInt3 direction = VInt3.right;
				direction = direction.RotateY((int)frameCommand2.cmdData.Degree);
		...
		public void MoveToTarget()
		{
			if (this.myTarget)
			{
				this.RealMovePosition(this.myTarget.get_handle().location, 0u);
			}
		}
		...
		public VInt3 GetRouteCurWaypointPos()
		{
			...
			Vector3 vec = this.actor.location.get_vec3();
			...
			if (nextPoint != null)
			{
				...
				Vector3 position2 = nextPoint.transform.position;
				Vector3 vector = vec - position2;
				vector.y = 0f;
				Vector3 vector2 = position - position2;
				vector2.y = 0f;
				Vector3 vector3 = vec - position;
				vector3.y = 0f;
				if (vector.sqrMagnitude < vector2.sqrMagnitude || vector.sqrMagnitude < vector3.sqrMagnitude || vector3.sqrMagnitude < 1f)
				{
			...
			long sqrMagnitudeLong2D = (base.actorLocation - this.m_curWaypointTargetPosition).get_sqrMagnitudeLong2D();
			if (sqrMagnitudeLong2D > (long)MonoSingleton<GlobalConfig>.get_instance().WaypointIgnoreDist)
			{
		...
		public VInt3 GetRouteCurWaypointPosPre()
		{
			...
			{
				Vector3 position = this.m_curWaypointTarget.transform.position;
				Vector3 position2 = prePoint.transform.position;
				Vector3 vector = vec - position2;
				vector.y = 0f;
				Vector3 vector2 = position - position2;
				vector2.y = 0f;
		...
		public virtual void CmdCommonAttackMode(IFrameCommand cmd, sbyte Start, uint uiRealObjID)
		{
			...
			else
			{
				...
				if (player != null && player.Captain.get_handle().ActorAgent != null && player.Captain.get_handle().ActorAgent.m_wrapper != null)
				{
					BaseAttackMode currentAttackMode = player.Captain.get_handle().ActorAgent.m_wrapper.GetCurrentAttackMode();
					if (currentAttackMode != null)
					{
		...
		public bool IsTargetObjInSearchDistance()
		{
			...
			if (!this.myTarget || this.myTarget.get_handle().shape == null || this.myTarget.get_handle().ActorAgent == null || this.myTarget.get_handle().ActorControl.IsDeadState)
			{
				return false;
			}
			long num = (long)nextSkill.GetMaxSearchDistance(0);
			num += (long)this.myTarget.get_handle().shape.AvgCollisionRadius;
			num *= num;
			return (base.actorLocation - this.myTarget.get_handle().location).get_sqrMagnitudeLong2D() <= num;
		}
		...
		{
			if (hurt.atker && hurt.target && hurt.atker.get_handle().TheActorMeta.ActorCamp != hurt.target.get_handle().TheActorMeta.ActorCamp && hurt.hurtType != HurtTypeDef.Therapic)
			{
				this.myLastAtker = hurt.atker;
				PoolObjHandle<ActorRoot> orignalActor = this.myLastAtker.get_handle().ActorControl.GetOrignalActor();
				if (orignalActor && orignalActor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
				{
			...
			if (hurt.atker && hurt.atker.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ && hurt.hurtType != HurtTypeDef.Therapic && (this.actor.ValueComponent.actorHp * 100 < this.actor.ValueComponent.actorHpTotal * 95 || (this.myLastAtker && !this.myLastAtker.get_handle().AttackOrderReady)) && this.actor.ActorAgent != null)
			{
		...
		public void NotifyAssistActor(ref PoolObjHandle<ActorRoot> assist)
		{
			...
			assist.get_handle().ActorControl.OnAssist(ref this.actorPtr);
			assist.get_handle().BuffHolderComp.OnAssistEffect(ref this.actorPtr);
		}
		...
		protected virtual void OnHpChange()
		{
			if (this.actor.HudControl != null)
			{
				this.actor.HudControl.UpdateBloodBar(Math.Min(this.actor.ValueComponent.actorHp, this.actor.ValueComponent.mActorValue[5].totalValue), this.actor.ValueComponent.mActorValue[5].totalValue);
			}
		...
		public void AddHurtSelfActor(PoolObjHandle<ActorRoot> actor)
		{
			...
			{
				if (this.hurtSelfActorList.get_Item(i).get_Key() == actor.get_handle().ObjID)
				{
					this.hurtSelfActorList.set_Item(i, new KeyValuePair<uint, ulong>(actor.get_handle().ObjID, logicFrameTick));
					break;
				}
			}
			if (i >= this.hurtSelfActorList.get_Count())
			{
				this.hurtSelfActorList.Add(new KeyValuePair<uint, ulong>(actor.get_handle().ObjID, logicFrameTick));
			}
		}
		...
		public void AddHelpSelfActor(PoolObjHandle<ActorRoot> actor)
		{
			...
			{
				if (this.helpSelfActorList.get_Item(i).get_Key() == actor.get_handle().ObjID)
				{
					this.helpSelfActorList.set_Item(i, new KeyValuePair<uint, ulong>(actor.get_handle().ObjID, logicFrameTick));
					break;
				}
			}
			if (i >= this.helpSelfActorList.get_Count())
			{
				this.helpSelfActorList.Add(new KeyValuePair<uint, ulong>(actor.get_handle().ObjID, logicFrameTick));
			}
		}
