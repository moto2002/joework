...
public class AkAudioListener : MonoBehaviour
{
	...
	private void UpdateCache()
	{
		...
		if (Singleton<BattleLogic>.get_instance().isRuning)
		{
			...
			{
				if (Singleton<WatchController>.GetInstance().IsWatching || hostPlayer.Captain.get_handle().ActorControl.IsDeadState)
				{
					if (Camera.main != null)
					{
						...
						{
							this.m_PositionCache = ray.GetPoint(distance) + ((!hostPlayer.Captain.get_handle().ActorControl.IsDeadState) ? Vector3.zero : this.m_DeadOffset);
						}
					}
				}
				else
				{
					this.m_PositionCache = (Vector3)hostPlayer.Captain.get_handle().location;
				}
			}
		}
	}
