...
namespace AGE
{
	public class Action : PooledClassObject
	{
		...
		public override void OnUse()
		{
			...
			DebugHelper.Assert(this.actorHandles.get_Count() == 0 && this.gameObjects.get_Count() == 0, "age gameObjects is not null");
		}
		...
		public void Stop(bool bForce)
		{
			if (bForce)
			{
				...
				if (this.tracks != null)
				{
					for (int j = 0; j < this.tracks.get_Count(); j++)
					{
						Track track2 = this.tracks.get_Item(j);
						track2.Release();
					}
					this.tracks.Clear();
				}
		...
		private void Update()
		{
			if (ActionManager.Instance.frameMode || !this.enabled || this.playSpeed.get_IsZero())
			{
				return;
			}
			int num = ActionUtility.SecToMs(Time.deltaTime * this.playSpeed.get_single());
			num += this.time;
		...
		public void LoadAction(Action _actionResource, params GameObject[] _gameObjects)
		{
			...
			int count = _actionResource.tracks.get_Count();
			for (int i = 0; i < count; i++)
			{
				Track track = _actionResource.tracks.get_Item(i);
		...
		public Track AddTrack(Type _eventType)
		{
			...
			track.trackIndex = this.tracks.get_Count();
		...
		{
			_track.action = this;
			_track.trackIndex = this.tracks.get_Count();
		...
		{
			if (_index < 0 || _index >= this.tracks.get_Count())
			{
				return null;
			}
			return this.tracks.get_Item(_index);
		}
		...
		{
			if (_index < 0 || _index >= this.gameObjects.get_Count())
			{
				return null;
			}
			return this.gameObjects.get_Item(_index);
		}

		public void ClearGameObject(GameObject _gameObject)
		{
			for (int i = 0; i < this.gameObjects.get_Count(); i++)
			{
				if (this.gameObjects.get_Item(i) == _gameObject)
				{
		...
		public void SetGameObject(int _index, GameObject go)
		{
			...
			this.gameObjects.set_Item(_index, go);
		}

		public void ExpandGameObject(int maxIdx)
		{
			while (maxIdx >= this.gameObjects.get_Count())
			{
		...
		public void ResetLength(int inLengthMs, bool bPlaySpeed)
		{
			...
			VFactor vFactor = new VFactor((long)inLengthMs, (long)this.length);
			if (bPlaySpeed)
			{
				this.SetPlaySpeed(vFactor.get_Inverse());
				return;
			}
			this.length *= vFactor;
			int count = this.tracks.get_Count();
		...
		public void GetAssociatedResources(Dictionary<object, AssetRefType> results, int markID = 0)
		{
			...
			for (int i = 0; i < this.tracks.get_Count(); i++)
			{
				Track track = this.tracks.get_Item(i);
				if (track != null)
				{
		...
		public void AddTempObject(Action.PlaySpeedAffectedType type, GameObject obj)
		{
			...
			if (!this.tempObjsAffectedByPlaySpeed.TryGetValue((uint)type, ref listView))
			{
			...
			{
				GameObject x = listView.get_Item(i);
				if (x == obj)
				{
					return;
				}
			...
			listView.Add(obj);
			float single = this.playSpeed.get_single();
		...
		private void UpdateTempObjectSpeed()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, ListView<GameObject>> current = enumerator.get_Current();
				Action.PlaySpeedAffectedType key = (Action.PlaySpeedAffectedType)current.get_Key();
				KeyValuePair<uint, ListView<GameObject>> current2 = enumerator.get_Current();
				ListView<GameObject> value = current2.get_Value();
				int count = value.get_Count();
				for (int i = 0; i < count; i++)
				{
					GameObject gameObject = value.get_Item(i);
