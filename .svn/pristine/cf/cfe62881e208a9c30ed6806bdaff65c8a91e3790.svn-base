...
namespace Assets.Scripts.GameLogic
{
	public class BufferLogicEffect
	{
		...
		{
			ResDT_SkillFunc resDT_SkillFunc = null;
			if (inBuff && inBuff.get_handle().FindSkillFunc(72, out resDT_SkillFunc))
			{
		...
		private int DamageExtraEffect(ref HurtDataInfo _hurt, int _hurtVale, int _hurtRate, int _typeMask, int _typeSubMask)
		{
			...
			if (_hurt.atker.get_handle().SkillControl.TryGetSkillSlot(_hurt.atkSlot, out skillSlot))
			{
				BufferLogicEffect logicEffect = _hurt.atker.get_handle().BuffHolderComp.logicEffect;
		...
		{
			SkillSlot skillSlot = null;
			if (!_hurt.atker || _hurt.atker.get_handle().ActorControl.IsDeadState)
			{
				return _hurtValue;
			}
			BufferLogicEffect logicEffect = _hurt.atker.get_handle().BuffHolderComp.logicEffect;
			...
			for (int i = 0; i < list.get_Count(); i++)
			{
				...
				if (_hurt.atkSlot == (SkillSlotType)skillFuncParam && _hurt.atker.get_handle().SkillControl.TryGetSkillSlot(_hurt.atkSlot, out skillSlot))
				{
