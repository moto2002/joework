...
namespace Assets.Scripts.GameLogic
{
	...
	internal class ClearanceAppointTime : StarCondition
	{
		...
		public override void Initialize(ResDT_ConditionInfo InConditionInfo)
		{
			...
			Singleton<GameEventSys>.get_instance().AddEventHandler<DefaultGameEventParam>(GameEventDef.Event_FightStart, new RefAction<DefaultGameEventParam>(this.AddTimer));
		}

		public void AddTimer(ref DefaultGameEventParam prm)
		{
			Singleton<CTimerManager>.get_instance().RemoveTimer(new CTimer.OnTimeUpHandler(this.OnAppointTimeFinished));
			Singleton<CTimerManager>.get_instance().AddTimer(this.timeMSeconds + 1, -1, new CTimer.OnTimeUpHandler(this.OnAppointTimeFinished));
		}
