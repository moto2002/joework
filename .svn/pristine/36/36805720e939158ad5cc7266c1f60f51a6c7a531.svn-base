...
namespace Assets.Scripts.GameSystem
{
	public class CHeroAnimaSystem : Singleton<CHeroAnimaSystem>
	{
		...
		{
			for (int i = 0; i < this.m_animatSoundList.get_Count(); i++)
			{
				if (this.m_animatSoundList.get_Item(i).AnimaName == aniName && !string.IsNullOrEmpty(this.m_animatSoundList.get_Item(i).SoundName))
				{
					Singleton<CSoundManager>.GetInstance().PostEvent(this.m_animatSoundList.get_Item(i).SoundName, this.m_3DModel);
				}
			}
		}
		...
		public void OnHeroInfo_ClickModel(CUIEvent uiEvent)
		{
			...
			if (!component.IsPlaying(component.clip.name) && Singleton<FrameSynchr>.get_instance().LogicFrameTick - this.m_tickAnimat < 2000uL)
			{
				return;
			}
			if (component && this.m_clikAnimaList != null && this.m_clikAnimaList.get_Count() > 0)
			{
				...
				this.m_tickAnimat = Singleton<FrameSynchr>.get_instance().LogicFrameTick;
			}
		}
