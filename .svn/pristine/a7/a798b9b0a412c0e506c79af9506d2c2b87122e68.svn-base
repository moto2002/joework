...
namespace Assets.Scripts.GameSystem
{
	public class RewardWidget : ActivityWidget
	{
		...
		public RewardWidget(GameObject node, ActivityView view) : base(node, view)
		{
			...
			ListView<ActivityPhase> listView = new ListView<ActivityPhase>(view.activity.PhaseList.get_Count());
			...
			for (int i = 0; i < listView.get_Count(); i++)
			{
				...
				if (i > 0)
				{
					...
					gameObject.transform.localPosition = this._rewardList.get_Item(i - 1).root.transform.localPosition + new Vector3(0f, -(height + 5f), 0f);
				...
				RewardWidget.RewardListItem rewardListItem = new RewardWidget.RewardListItem(gameObject, this, listView.get_Item(i));
				this._rewardList.Add(rewardListItem);
			}
			if (this._rewardList.get_Count() == 0)
			{
				this._elementTmpl.SetActive(false);
			}
			node.GetComponent<RectTransform>().sizeDelta = new Vector2(node.GetComponent<RectTransform>().sizeDelta.x, (this._rewardList.get_Count() <= 0) ? 0f : (height * (float)this._rewardList.get_Count() + (float)(this._rewardList.get_Count() - 1) * 5f));
		...
		public override void Clear()
		{
			...
			{
				this._rewardList.get_Item(i).Clear();
				if (i > 0)
				{
					Object.Destroy(this._rewardList.get_Item(i).root);
				}
			}
