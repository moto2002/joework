...
namespace Pathfinding
{
	...
	public class RecastGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder
	{
		...
		public void SnapForceBoundsToScene()
		{
			...
			bounds = list.get_Item(0).bounds;
			for (int i = 1; i < list.get_Count(); i++)
			{
				bounds.Encapsulate(list.get_Item(i).bounds);
			}
		...
		public void GetRecastMeshObjs(Bounds bounds, List<ExtraMesh> buffer)
		{
			...
			for (int i = 0; i < list.get_Count(); i++)
			{
				...
				if (meshFilter != null && renderer != null)
				{
					...
					extraMesh.matrix = renderer.localToWorldMatrix;
			...
			if ((tagMask != null && tagMask.get_Count() > 0) || layerMask != 0)
			{
				...
				for (int j = 0; j < listView.Count; j++)
				{
					...
					else if (component2.bounds.Intersects(bounds))
					{
						...
						extraMesh.matrix = component2.localToWorldMatrix;
		...
		private void CollectTreeMeshes(Terrain terrain, List<ExtraMesh> extraMeshes)
		{
			...
			for (int i = 0; i < terrainData.treeInstances.Length; i++)
			{
				...
				TreePrototype treePrototype = terrainData.treePrototypes[treeInstance.get_prototypeIndex()];
				if (treePrototype.prefab.GetComponent<Collider>() == null)
				{
					Bounds b = new Bounds(terrain.transform.position + Vector3.Scale(treeInstance.get_position(), terrainData.size), new Vector3(treeInstance.get_widthScale(), treeInstance.get_heightScale(), treeInstance.get_widthScale()));
					Matrix4x4 matrix = Matrix4x4.TRS(terrain.transform.position + Vector3.Scale(treeInstance.get_position(), terrainData.size), Quaternion.identity, new Vector3(treeInstance.get_widthScale(), treeInstance.get_heightScale(), treeInstance.get_widthScale()) * 0.5f);
					ExtraMesh extraMesh = new ExtraMesh(this.BoxColliderVerts, this.BoxColliderTris, b, matrix);
					extraMeshes.Add(extraMesh);
				}
				else
				{
					Vector3 pos = terrain.transform.position + Vector3.Scale(treeInstance.get_position(), terrainData.size);
					Vector3 s = new Vector3(treeInstance.get_widthScale(), treeInstance.get_heightScale(), treeInstance.get_widthScale());
					ExtraMesh extraMesh2 = this.RasterizeCollider(treePrototype.prefab.GetComponent<Collider>(), Matrix4x4.TRS(pos, Quaternion.identity, s));
					if (extraMesh2.vertices != null)
					{
