...
public class PreloadActionAssets
{
	...
	private void AddActionsFromActors(DictionaryView<string, Action> actions, List<PoolObjHandle<ActorRoot>> actors)
	{
		...
		{
			ActorRoot handle = actors.get_Item(i).get_handle();
	...
	private void AddReferencedAssets(DictionaryView<string, Action> actions, Dictionary<object, AssetRefType> actionAssets)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			case AssetRefType.Action:
			{
				...
				string text = current2.get_Key() as string;
				if (!this.actionDict.ContainsKey(text) && !actions.ContainsKey(text))
				{
					actions.Add(text, null);
				}
				break;
			...
			case AssetRefType.Particle:
			{
				...
				string text2 = current5.get_Key() as string;
				if (!this.prefabDict.ContainsKey(text2))
				{
					this.prefabDict.Add(text2, value == AssetRefType.Particle);
				}
				break;
			}
	...
	private void AddActionsFromSoldierRegions(DictionaryView<string, Action> actions)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (soldierRegion != null && soldierRegion.Waves != null && soldierRegion.Waves.get_Count() > 0)
			{
				...
				{
					SoldierWave soldierWave = soldierRegion.Waves.get_Item(j);
	...
	{
		PreloadActionAssets.<Exec>c__Iterator26 <Exec>c__Iterator = new PreloadActionAssets.<Exec>c__Iterator26();
