...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CUILoadingSystem : Singleton<CUILoadingSystem>
	{
		...
		{
			base.Init();
			Singleton<GameEventSys>.get_instance().AddEventHandler<DefaultGameEventParam>(GameEventDef.Event_FightPrepare, new RefAction<DefaultGameEventParam>(this.onGamePrepareFight));
			Singleton<GameEventSys>.get_instance().AddEventHandler<DefaultGameEventParam>(GameEventDef.Event_FightStart, new RefAction<DefaultGameEventParam>(this.onGameStartFight));
			Singleton<GameEventSys>.get_instance().AddEventHandler(GameEventDef.Event_MultiRecoverFin, new Action(this.onGameRecoverFin));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.ADVANCE_STOP_LOADING, new Action(this.HideLoading));
			Singleton<GameEventSys>.get_instance().AddEventHandler<PreDialogStartedEventParam>(GameEventDef.Event_PreDialogStarted, new RefAction<PreDialogStartedEventParam>(this.OnPreDialogStarted));
		}
		...
		{
			GameObject memberItem = CUILoadingSystem.GetMemberItem(msg.stPkgData.get_stMultGameLoadProcessRsp().dwObjId);
			if (memberItem != null)
			{
				...
				if (gameObject)
				{
					...
					component.text = string.Format("{0}%", msg.stPkgData.get_stMultGameLoadProcessRsp().wProcess);
				}
			}
		}
		...
		private static void ShowMultiLoading()
		{
			...
			for (int i = 1; i <= 2; i++)
			{
				List<Player> allCampPlayers = Singleton<GamePlayerCenter>.GetInstance().GetAllCampPlayers(i);
				...
				else
				{
					...
					while (enumerator.MoveNext())
					{
						...
						if (current != null)
						{
							...
							if (component3)
							{
								MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component3, (int)current.VipLv, true);
							}
							...
							if (gameObject5 != null && current.IntimacyData != null)
							{
								...
								if (current.IntimacyData.state == 1)
								{
									gameObject6.CustomSetActive(true);
									gameObject7.CustomSetActive(false);
								}
								else if (current.IntimacyData.state == 2)
								{
							...
							if (gameObject8 != null)
							{
								if (current.IntimacyData != null && current.IntimacyData.state == 1)
								{
									gameObject8.transform.Find("btnFriend").gameObject.CustomSetActive(true);
									gameObject8.transform.Find("btnLover").gameObject.CustomSetActive(false);
								}
								else if (current.IntimacyData != null && current.IntimacyData.state == 2)
								{
									gameObject8.transform.Find("btnFriend").gameObject.CustomSetActive(false);
									gameObject8.transform.Find("btnLover").gameObject.CustomSetActive(true);
								}
								else
								{
							...
							bool flag2 = (Singleton<WatchController>.GetInstance().IsWatching && current.PlayerUId == Singleton<WatchController>.GetInstance().TargetUID) || (!Singleton<WatchController>.GetInstance().IsWatching && current.PlayerId == Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().PlayerId);
							if (flag2)
							{
							...
							if (gameObject10)
							{
								...
								{
									component6.text = ((!MonoSingleton<Reconnection>.get_instance().isProcessingRelayRecover && !Singleton<WatchController>.GetInstance().IsWatching) ? "1%" : "100%");
								}
							}
			...
			else
			{
				...
				widget2.CustomSetActive(MonoSingleton<Reconnection>.get_instance().isProcessingRelayRecover);
			}
		}
		...
		private static GameObject GetMemberItem(uint ObjId)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current().PlayerId == ObjId)
				{
					...
					return (enumerator.get_Current().PlayerCamp != 1) ? form.gameObject.transform.FindChild("DownPanel").FindChild(string.Format("Down_Player{0}", num)).gameObject : form.gameObject.transform.FindChild("UpPanel").FindChild(string.Format("Up_Player{0}", num)).gameObject;
				}
			}
			return null;
		}
