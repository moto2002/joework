...
namespace Assets.Scripts.GameSystem
{
	public class CFunctionUnlockSys : Singleton<CFunctionUnlockSys>
	{
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler("FucUnlockConditionChanged", new Action(this.UnlockConditionChanged));
			base.Init();
		}
		...
		public override void UnInit()
		{
			...
			Singleton<EventRouter>.get_instance().RemoveEventHandler("FucUnlockConditionChanged", new Action(this.UnlockConditionChanged));
			base.UnInit();
		}
			...
			if (dataByKey != null && masterRoleInfo != null)
			{
				for (int i = 0; i < dataByKey.UnlockArray.Length; i++)
				{
					...
					if (num > 0u)
					{
						...
						if (dataByKey2 != null && dataByKey2.wUnlockType == conditionType)
						{
		...
		public void UnlockConditionChanged()
		{
			...
			for (int i = num; i < num2; i++)
			{
				...
				RES_SPECIALFUNCUNLOCK_TYPE rES_SPECIALFUNCUNLOCK_TYPE = i;
				if (dataByKey != null && !this.TipsHasShow(rES_SPECIALFUNCUNLOCK_TYPE) && dataByKey.bIsShowUnlockTip == 1 && this.FucIsUnlock(rES_SPECIALFUNCUNLOCK_TYPE))
				{
					...
					{
						Singleton<CLobbySystem>.get_instance().Play_UnLock_Animation(rES_SPECIALFUNCUNLOCK_TYPE);
		...
		{
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm(CFunctionUnlockSys.FUC_UNLOCK_FORM_PATH, false, false);
		...
		{
			Singleton<CUIManager>.get_instance().CloseForm(CFunctionUnlockSys.FUC_UNLOCK_FORM_PATH);
		...
		public void ReqUnlockTipsMask()
		{
			...
			cSPkg.stPkgData.get_stFuncUnlockReq().ullUnlockFlag = this.m_tipUnlockMask;
