...
namespace Assets.Scripts.GameLogic
{
	...
	public class BurnExpeditionNetCore
	{
		...
		public static void Send_GET_BURNING_REWARD_REQ(byte levelNo, int levelID)
		{
			...
			cSPkg.stPkgData.get_stGetBurningRewardReq().bLevelNo = levelNo;
			cSPkg.stPkgData.get_stGetBurningRewardReq().iLevelID = levelID;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void Send_RESET_BURNING_PROGRESS_REQ(byte value)
		{
			...
			cSPkg.stPkgData.get_stResetBurningProgressReq().bDifficultType = value;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		public static void Clear_ResetBurning_Limit()
		{
			...
			cSPkg.stPkgData.get_stCheatCmd().iCheatCmd = 35;
			cSPkg.stPkgData.get_stCheatCmd().stCheatCmdDetail = new CSDT_CHEATCMD_DETAIL();
			cSPkg.stPkgData.get_stCheatCmd().stCheatCmdDetail.set_stClrBurningLimit(new CSDT_CHEAT_CLR_BURNING_LIMIT());
		...
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(1012u);
			cSPkg.stPkgData.get_stCheatCmd().iCheatCmd = 39;
			cSPkg.stPkgData.get_stCheatCmd().stCheatCmdDetail = new CSDT_CHEATCMD_DETAIL();
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
	}
