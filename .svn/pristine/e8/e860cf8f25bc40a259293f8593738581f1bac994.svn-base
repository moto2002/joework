...
namespace Assets.Scripts.GameSystem
{
	...
	public class PVEReviveHeros : Singleton<PVEReviveHeros>
	{
		...
		private void OnConfirmRevive(CUIEvent uiEvent)
		{
			...
			cSPkg.stPkgData.get_stPveReviveReq().bReviveNo = (byte)(3 - PVEReviveHeros.m_iRemainReviveNum + 1);
			Singleton<BattleLogic>.get_instance().m_bIsPayStat = true;
			if (!Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true))
			{
				string text = Singleton<CTextManager>.get_instance().GetText("Common_Network_Err");
		...
		{
			Singleton<BattleLogic>.get_instance().m_bIsPayStat = false;
			Singleton<CUIManager>.GetInstance().CloseSendMsgAlert();
			if (msg.stPkgData.get_stPveReviveRsp().iErrCode == 0 && ((int)msg.stPkgData.get_stPveReviveRsp().bDifficultType == Singleton<PVEReviveHeros>.get_instance().CurDifficulType || msg.stPkgData.get_stPveReviveRsp().iLevelID == Singleton<PVEReviveHeros>.get_instance().CurLevelId || (int)msg.stPkgData.get_stPveReviveRsp().bReviveNo == Singleton<PVEReviveHeros>.get_instance().CurReviveNum))
			{
				PVEReviveHeros.ReviveAllHeros();
				return;
			}
			string text = Singleton<CTextManager>.get_instance().GetText("PVE_Revive_Data_Error");
			Singleton<CUIManager>.GetInstance().OpenTips(text, false, 1.5f, null, new object[0]);
			Singleton<PVEReviveHeros>.get_instance().ReviveHerosFailed();
		}

		public static void ReviveAllHeros()
		{
			int inSkillCombineId = (int)Singleton<PVEReviveHeros>.get_instance().m_WishBuffId[(int)Singleton<PVEReviveHeros>.get_instance().m_iSelectedBuffItem];
			Player hostPlayer = Singleton<GamePlayerCenter>.get_instance().GetHostPlayer();
			if (hostPlayer == null || !hostPlayer.Captain)
			{
				return;
			}
			hostPlayer.Captain.get_handle().ActorControl.SetReviveContext(0, 10000, false, false, false);
			hostPlayer.Captain.get_handle().ActorControl.Revive(false);
			SkillUseParam skillUseParam = default(SkillUseParam);
			skillUseParam.SetOriginator(hostPlayer.Captain);
			hostPlayer.Captain.get_handle().SkillControl.SpawnBuff(hostPlayer.Captain, ref skillUseParam, inSkillCombineId, true);
			ReadonlyContext<PoolObjHandle<ActorRoot>> allHeroes = hostPlayer.GetAllHeroes();
			int count = allHeroes.get_Count();
			for (int i = 0; i < count; i++)
			{
				if (allHeroes.get_Item(i) && allHeroes.get_Item(i) != hostPlayer.Captain && allHeroes.get_Item(i))
				{
					allHeroes.get_Item(i).get_handle().ActorControl.SetReviveContext(0, 10000, false, false, false);
					allHeroes.get_Item(i).get_handle().ActorControl.Revive(false);
					skillUseParam.SetOriginator(allHeroes.get_Item(i));
					allHeroes.get_Item(i).get_handle().SkillControl.SpawnBuff(allHeroes.get_Item(i), ref skillUseParam, inSkillCombineId, true);
				}
			}
			PVEReviveHeros.m_iRemainReviveNum--;
			Singleton<PVEReviveHeros>.get_instance().ReviveHerosSucess();
		}
		...
		{
			Singleton<FrameSynchr>.get_instance().SetSynchrRunning(!bPause);
		}
