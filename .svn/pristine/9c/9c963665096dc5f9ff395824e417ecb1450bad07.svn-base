...
namespace Assets.Scripts.GameLogic
{
	public class LobbyLogic : Singleton<LobbyLogic>, IUpdateLogic
	{
		...
		public void OpenLobby()
		{
			...
			Singleton<CLoudSpeakerSys>.get_instance().StartReqTimer();
		...
		private void OnUiFormUpdate(ListView<CUIFormScript> inForms)
		{
			...
			CUIFormScript form12 = Singleton<CUIManager>.get_instance().GetForm(CTaskSys.TASK_FORM_PATH);
			CUIFormScript form13 = Singleton<CUIManager>.get_instance().GetForm(CAdventureSys.EXLPORE_FORM_PATH);
			CUIFormScript form14 = Singleton<CUIManager>.get_instance().GetForm(CSymbolSystem.s_symbolFormPath);
			...
			{
				if (!listView.Contains(enumerator.get_Current()))
				{
					flag = false;
					break;
				}
			...
			if (this.m_bShouldNewbieEnterHall)
			{
				...
				{
					Singleton<CBattleGuideManager>.get_instance().OpenOldPlayerFirstForm();
				}
			}
			if (form9 != null)
			{
			...
			{
				Singleton<EventRouter>.get_instance().BroadCastEvent("FucUnlockConditionChanged");
			}
			if (form10 != null)
			{
				listView.Add(form10);
		...
		private static bool TestTopFormCustom(CUIFormScript inTesterForm, ListView<CUIFormScript> inExcludeForms)
		{
			...
			bool result = false;
			ListView<CUIFormScript> listView = new ListView<CUIFormScript>();
			ListView<CUIFormScript> forms = Singleton<CUIManager>.get_instance().GetForms();
			if (forms != null)
			{
			...
			{
				if (inExcludeForms != null && inExcludeForms.get_Count() > 0)
				{
					ListView<CUIFormScript>.Enumerator enumerator = inExcludeForms.GetEnumerator();
					while (enumerator.MoveNext())
					{
						listView.Remove(enumerator.get_Current());
					}
				}
				if (listView.Contains(inTesterForm))
				{
					CUIFormScript cUIFormScript = null;
					int count = listView.get_Count();
					for (int i = 0; i < count; i++)
					{
						if (!(listView.get_Item(i) == null))
						{
							if (cUIFormScript == null)
							{
								cUIFormScript = listView.get_Item(i);
							}
							else if (listView.get_Item(i).GetSortingOrder() > cUIFormScript.GetSortingOrder())
							{
								cUIFormScript = listView.get_Item(i);
							}
						}
		...
		public void StartSettleTimers()
		{
			...
			this._settleMsgTimer = Singleton<CTimerManager>.GetInstance().AddTimer(20000, 1, new CTimer.OnTimeUpHandler(this.onSettleMsgTimer));
			this.StopGameEndTimer();
			this._gameEndTimer = Singleton<CTimerManager>.GetInstance().AddTimer(15000, 1, new CTimer.OnTimeUpHandler(this.onGameEndTimer));
		...
		{
			this.StopSettlePanelTimer();
			this._settlePanelTimer = Singleton<CTimerManager>.GetInstance().AddTimer(10000, 1, new CTimer.OnTimeUpHandler(this.onSettlePanelTimer));
		}
		...
		public void LoginGame()
		{
			if (!this.isLogin)
			{
				...
				cSPkg.stPkgData.get_stGameLoginReq().iCltAppVersion = (int)versionNumber;
				uint versionNumber2 = CVersion.GetVersionNumber(CVersion.GetUsedResourceVersion());
				cSPkg.stPkgData.get_stGameLoginReq().iCltResVersion = (int)versionNumber2;
				string buildNumber = CVersion.GetBuildNumber();
				Utility.StringToByteArray(buildNumber, ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szCltBuildNumber);
				string revisonNumber = CVersion.GetRevisonNumber();
				Utility.StringToByteArray(revisonNumber, ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szCltSvnVersion);
				Utility.StringToByteArray(SystemInfo.deviceName + " " + SystemInfo.deviceModel, ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szSystemHardware);
				Utility.StringToByteArray(SystemInfo.operatingSystem, ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szSystemSoftware);
				...
				if (internetReachability != NetworkReachability.ReachableViaCarrierDataNetwork)
				{
					...
					{
						Utility.StringToByteArray("Wi-Fi/LAN", ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szNetwork);
					}
				}
				else
				{
					Utility.StringToByteArray("2G/3G", ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szNetwork);
				}
				Utility.StringToByteArray(GameFramework.AppVersion, ref cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.szClientVersion);
				...
				cSPkg.stPkgData.get_stGameLoginReq().ullVisitorUid = ullVisitorUid;
				cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.iLoginChannel = MonoSingleton<IDIPSys>.GetInstance().m_ChannelID;
				cSPkg.stPkgData.get_stGameLoginReq().stClientInfo.iMemorySize = SystemInfo.systemMemorySize;
				cSPkg.stPkgData.get_stGameLoginReq().iLogicWorldID = MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.logicWorldID;
				cSPkg.stPkgData.get_stGameLoginReq().dwTaccZoneID = (uint)MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.nodeID;
				cSPkg.stPkgData.get_stGameLoginReq().bPrivilege = Singleton<ApolloHelper>.GetInstance().GetCurrentLoginPrivilege();
				byte[] bytes = Encoding.get_ASCII().GetBytes(SystemInfo.deviceUniqueIdentifier);
				if (bytes.Length > 0)
				{
					Buffer.BlockCopy(bytes, 0, cSPkg.stPkgData.get_stGameLoginReq().szCltIMEI, 0, (bytes.Length <= 64) ? bytes.Length : 64);
				}
				if (MonoSingleton<ShareSys>.GetInstance().m_RecruitSource != null)
				{
					cSPkg.stPkgData.get_stGameLoginReq().stRecruitSource = MonoSingleton<ShareSys>.GetInstance().m_RecruitSource;
					MonoSingleton<ShareSys>.GetInstance().m_RecruitSource = null;
				}
				else
				{
					cSPkg.stPkgData.get_stGameLoginReq().stRecruitSource.iPlatId = 0;
					cSPkg.stPkgData.get_stGameLoginReq().stRecruitSource.stUin.dwLogicWorldId = 0u;
					cSPkg.stPkgData.get_stGameLoginReq().stRecruitSource.stUin.ullUid = 0uL;
				}
		...
		public static void ReqStartGuideLevel11(bool bReEntry, uint heroType = 0u)
		{
			if (heroType == 0u)
			{
				heroType = GameDataMgr.GetGlobeValue(284);
			}
			uint heroID = 0u;
			...
			if (CBattleGuideManager.GetGuide1v1HeroIDAndLevelID(heroType, out heroID, out inGuideLevelId))
			{
				...
				Singleton<CBattleGuideManager>.get_instance().bReEntry = bReEntry;
			}
		}
		...
		public static void ReqStartGuideLevel33(bool bReEntry)
		{
			...
			CLobbySystem.AutoPopAllow = false;
			Singleton<CBattleGuideManager>.get_instance().bReEntry = bReEntry;
			Singleton<CBattleGuideManager>.get_instance().bTrainingAdv = true;
		}
		...
		public static void ReqStartGuideLevelSelHero(bool bReEntry, int levelId)
		{
			...
			Singleton<CBattleGuideManager>.get_instance().bTrainingAdv = true;
			Singleton<CBattleGuideManager>.get_instance().bReEntry = bReEntry;
			Singleton<CAdventureSys>.get_instance().currentLevelId = levelId;
			...
			Singleton<CHeroSelectBaseSystem>.get_instance().SetPVEDataWithAdventure(dataByKey.dwBattleListID, cSDT_SINGLE_GAME_OF_ADVENTURE, StringHelper.UTF8BytesToString(ref dataByKey.szName));
			Singleton<CHeroSelectBaseSystem>.get_instance().OpenForm(enSelectGameType.enGuide, (byte)dataByKey.iHeroNum, 0u, 0, 0);
		}
		...
		public void ReqStartGuideLevel(int inGuideLevelId, uint heroID)
		{
			...
			cSPkg.stPkgData.get_stStartSingleGameReq().stBattleParam.bGameType = 2;
			cSPkg.stPkgData.get_stStartSingleGameReq().stBattleParam.stGameDetail.construct(2L);
			cSPkg.stPkgData.get_stStartSingleGameReq().stBattleParam.stGameDetail.get_stGameOfGuide().iLevelID = inGuideLevelId;
			cSPkg.stPkgData.get_stStartSingleGameReq().stBattleList.stBattleList.wHeroCnt = 1;
			cSPkg.stPkgData.get_stStartSingleGameReq().stBattleList.stBattleList.BattleHeroList[0] = heroID;
			...
			{
				Singleton<CSoundManager>.get_instance().LoadBank("Newguide_Voice", CSoundManager.BankType.Battle);
			}
			else if ((long)inGuideLevelId == (long)((ulong)CBattleGuideManager.GuideLevelID3v3))
			{
				Singleton<CSoundManager>.get_instance().LoadBank("Newguide_3V3_Voice", CSoundManager.BankType.Battle);
			}
			else if (CBattleGuideManager.Is5v5GuideLevel(inGuideLevelId))
			{
				Singleton<CSoundManager>.get_instance().LoadBank("Newguide_5V5_Voice", CSoundManager.BankType.Battle);
			}
			else if ((long)inGuideLevelId == (long)((ulong)CBattleGuideManager.GuideLevelIDCasting))
			{
				Singleton<CSoundManager>.get_instance().LoadBank("Newguide_Voice_Rotate", CSoundManager.BankType.Battle);
			}
			else if ((long)inGuideLevelId == (long)((ulong)CBattleGuideManager.GuideLevelIDJungle))
			{
				Singleton<CSoundManager>.get_instance().LoadBank("Newguide_Voice_WildMonster", CSoundManager.BankType.Battle);
			}
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		...
		public void ReqSingleGameFinish(bool ClickGameOver = false, bool bGmWin = false)
		{
			if (this.isLogin)
			{
				...
				{
					b = ((!Singleton<WinLose>.get_instance().LastSingleGameWin) ? 2 : 1);
				}
				else if (Singleton<StarSystem>.get_instance().isFailure)
				{
					b = 2;
				}
				else
				...
				cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.bGameType = curLvelContext.GetGameType();
				cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.iLevelID = curLvelContext.m_mapID;
				cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.dwUsedTime = (uint)Singleton<FrameSynchr>.GetInstance().LogicFrameTick / 1000u;
				cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.bGameResult = b;
				cSPkg.stPkgData.get_stFinSingleGameReq().stCommonData = this.CreateReportData(Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().PlayerId, bGmWin, b == 1);
				if (curLvelContext.IsGameTypeAdventure())
				{
					cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.stGameDetail.set_stAdventure(new COMDT_SINGLE_GAME_PARAM_ADVENTURE());
					cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.stGameDetail.get_stAdventure().bChapterNo = (byte)curLvelContext.m_chapterNo;
					cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.stGameDetail.get_stAdventure().bLevelNo = curLvelContext.m_levelNo;
					cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam.stGameDetail.get_stAdventure().bDifficultType = (byte)curLvelContext.m_levelDifficulty;
				}
				else if (curLvelContext.IsGameTypeBurning())
				{
					BurnExpeditionUT.Build_Burn_BattleParam(cSPkg.stPkgData.get_stFinSingleGameReq().stBattleParam, ClickGameOver);
				}
				if (!bGmWin)
				{
					if (ClickGameOver)
					{
						cSPkg.stPkgData.get_stFinSingleGameReq().bPressExit = 1;
						battleStat.iBattleResult = 2;
					}
					else
					{
						cSPkg.stPkgData.get_stFinSingleGameReq().bPressExit = 0;
					}
				}
			...
			if (this.isLogin && battleStat != null)
			{
				...
				for (int i = 0; i < allPlayers.get_Count(); i++)
				{
					if (!allPlayers.get_Item(i).Computer)
					{
						...
						{
							cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].stBattleParam.bGameResult = (byte)num;
						}
						else
						{
							cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].stBattleParam.bGameResult = (byte)num2;
						}
						if (allPlayers.get_Item(i) == Singleton<GamePlayerCenter>.get_instance().GetHostPlayer())
						{
							cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].stBattleParam.wNetworkMode = (ushort)this.CheckWifi();
						}
						else
						{
							cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].stBattleParam.wNetworkMode = 0;
						}
						bool flag = cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].stBattleParam.bGameResult == 1;
						cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].dwAcntObjID = allPlayers.get_Item(i).PlayerId;
						cSPkg.stPkgData.get_stMultGameOverReq().astAcntInfo[num3].stCommonData = this.CreateReportData(allPlayers.get_Item(i).PlayerId, flag, flag);
						num3++;
						if (num3 > 4)
						{
							cSPkg.stPkgData.get_stMultGameOverReq().bAcntNum = (byte)num3;
		...
		public void ReqStartMultiGame()
		{
			if (this.isLogin && this.inMultiRoom)
			{
				if (!Singleton<BattleLogic>.get_instance().isWaitMultiStart)
				{
		...
		public void ReqMultiGameRunaway()
		{
			...
			cSPkg.stPkgData.set_stRunAwayReq(new CSPKG_MULTGAME_RUNAWAY_REQ());
		...
		public void ReqSelectHero(uint id)
		{
			if (this.isLogin)
			{
				...
				cSPkg.stPkgData.get_stAchieveHeroReq().dwHeroId = id;
		...
		private void VerifyReportData(ref COMDT_SETTLE_COMMON_DATA CommonData)
		{
			...
			for (int i = 0; i < curLvelContext.m_starDetail.Length; i++)
			{
				...
				if (dataByKey != null)
				{
					...
					IStarEvaluation evaluationAt = Singleton<StarSystem>.get_instance().GetEvaluationAt(i);
		...
		{
			Singleton<StarSystem>.get_instance().GetEnumerator();
		...
		{
			if (HeroInfo.actorHero.get_handle() == null || HeroInfo.actorHero.get_handle().ValueComponent == null)
			{
				return;
			}
			ActorValueStatistic objValueStatistic = HeroInfo.actorHero.get_handle().ValueComponent.ObjValueStatistic;
			if (objValueStatistic != null)
			{
				...
				stHeroDetailInfo.iTotalSoulExp = HeroInfo.actorHero.get_handle().ValueComponent.actorSoulExp + HeroInfo.actorHero.get_handle().ValueComponent.actorSoulMaxExp;
			}
			ulong logicFrameTick = Singleton<FrameSynchr>.get_instance().LogicFrameTick;
			if (HeroInfo.actorHero.get_handle().MovementComponent != null)
			{
				PlayerMovement playerMovement = HeroInfo.actorHero.get_handle().MovementComponent as PlayerMovement;
		...
		private void CreateBattleStatisticInfoData(HeroKDA HeroInfo, ref COMDT_HERO_BATTLE_STATISTIC_INFO stBattleStatisticInfo)
		{
			...
			float num = Singleton<FrameSynchr>.get_instance().LogicFrameTick / 60000f;
			if (num > 0f && HeroInfo.actorHero && HeroInfo.actorHero.get_handle().ValueComponent != null)
			{
				stBattleStatisticInfo.iGPM = (int)((float)HeroInfo.actorHero.get_handle().ValueComponent.GetGoldCoinIncomeInBattle() / num);
				stBattleStatisticInfo.iXPM = (int)((float)(HeroInfo.actorHero.get_handle().ValueComponent.actorSoulExp + HeroInfo.actorHero.get_handle().ValueComponent.actorSoulMaxExp) / num);
			}
			stBattleStatisticInfo.iHurtPM = (int)((float)HeroInfo.hurtToHero / num);
			if (Singleton<BattleStatistic>.GetInstance().m_playerKDAStat.m_CampKdaStat != null)
			{
				uint teamTotalDamage = Singleton<BattleStatistic>.GetInstance().m_playerKDAStat.m_CampKdaStat.GetTeamTotalDamage(HeroInfo.actorHero.get_handle().TheActorMeta.ActorCamp);
				if (teamTotalDamage != 0u)
				{
					stBattleStatisticInfo.bTotalHurtPercent = (byte)((long)(100 * HeroInfo.hurtToEnemy) / (long)((ulong)teamTotalDamage));
				}
				uint teamTotalToHeroDamage = Singleton<BattleStatistic>.GetInstance().m_playerKDAStat.m_CampKdaStat.GetTeamTotalToHeroDamage(HeroInfo.actorHero.get_handle().TheActorMeta.ActorCamp);
				if (teamTotalToHeroDamage != 0u)
				{
					stBattleStatisticInfo.bTotalHurtHeroPercent = (byte)((long)(100 * HeroInfo.hurtToHero) / (long)((ulong)teamTotalToHeroDamage));
				}
				uint teamTotalTakenDamage = Singleton<BattleStatistic>.GetInstance().m_playerKDAStat.m_CampKdaStat.GetTeamTotalTakenDamage(HeroInfo.actorHero.get_handle().TheActorMeta.ActorCamp);
				if (teamTotalTakenDamage != 0u)
				{
					stBattleStatisticInfo.bTotalBeHurtPercent = (byte)((long)(100 * HeroInfo.hurtTakenByEnemy) / (long)((ulong)teamTotalTakenDamage));
				}
			}
			stBattleStatisticInfo.dwLevelThreeTime = Singleton<BattleStatistic>.get_instance().m_playerSoulLevelStat.GetPlayerLevelChangedTime(HeroInfo.actorHero.get_handle().TheActorMeta.PlayerId, 3u);
			stBattleStatisticInfo.dwLevelFourTime = Singleton<BattleStatistic>.get_instance().m_playerSoulLevelStat.GetPlayerLevelChangedTime(HeroInfo.actorHero.get_handle().TheActorMeta.PlayerId, 4u);
			if (HeroInfo.actorHero && HeroInfo.actorHero.get_handle().SkillControl != null && HeroInfo.actorHero.get_handle().SkillControl.stSkillStat != null)
			{
				stBattleStatisticInfo.bEyeNum = ((HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.m_uiSpawnEyeTimes >= 15u) ? 15 : ((byte)HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.m_uiSpawnEyeTimes));
				for (int i = 0; i < (int)stBattleStatisticInfo.bEyeNum; i++)
				{
					stBattleStatisticInfo.astEyePos[i].iXPos = HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.stEyePostion[i].x;
					stBattleStatisticInfo.astEyePos[i].iZPos = HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.stEyePostion[i].z;
				}
				stBattleStatisticInfo.bFakeWardNum = (byte)HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.m_uiRealSpawnEyeTimes;
				if (HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo != null)
				{
					if (HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo[7].uiUsedTimes > (uint)stBattleStatisticInfo.bFakeWardNum)
					{
						stBattleStatisticInfo.bVisionWardNum = (byte)(HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo[7].uiUsedTimes - (uint)stBattleStatisticInfo.bFakeWardNum);
					}
					stBattleStatisticInfo.dwActiveEquipUseNum = HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo[8].uiUsedTimes;
				}
				stBattleStatisticInfo.dwSwitchWardNum = HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.m_uiEyeSwitchTimes;
			}
			...
			if (HeroInfo.actorHero && HeroInfo.actorHero.get_handle().SkillControl != null && HeroInfo.actorHero.get_handle().SkillControl.stSkillStat != null)
			{
				stBattleStatisticInfo.iHeroCtrlSkillNum = HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.GetStunSkillNum();
				stBattleStatisticInfo.iHeroBeCtrledTime = (int)HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.BeStunTime;
				stBattleStatisticInfo.iHeroCtrlOtherTime = (int)HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.StunTime;
			}
			...
			CBattleDeadStat battleDeadStat = Singleton<BattleStatistic>.get_instance().m_battleDeadStat;
			if (battleDeadStat != null)
			{
				stBattleStatisticInfo.dwAverageFightTime = (uint)battleDeadStat.GetActorAverageFightTime(HeroInfo.actorHero.get_handle().TheActorMeta.ActorCamp, ActorTypeDef.Actor_Type_Hero, HeroInfo.actorHero.get_handle().TheActorMeta.ConfigId);
			}
		}
		...
		{
			if (HeroInfo.actorHero && HeroInfo.actorHero.get_handle().SkillControl != null && HeroInfo.actorHero.get_handle().SkillControl.stSkillStat != null && HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo != null)
			{
				SKILLSTATISTICTINFO[] skillStatistictInfo = HeroInfo.actorHero.get_handle().SkillControl.stSkillStat.SkillStatistictInfo;
				SkillSlot[] skillSlotArray = HeroInfo.actorHero.get_handle().SkillControl.SkillSlotArray;
				for (int i = 0; i < 5; i++)
				{
		...
		private void CreateHeroData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current().actorHero && enumerator.get_Current().actorHero.get_handle().ValueComponent != null)
				{
					...
					uint dwBloodTTH = BurnExpeditionUT.Get_BloodTH(enumerator.get_Current().actorHero.get_handle());
					CommonData.stHeroData.astHeroList[(int)b].dwHeroConfID = (uint)enumerator.get_Current().actorHero.get_handle().TheActorMeta.ConfigId;
					CommonData.stHeroData.astHeroList[(int)b].dwBloodTTH = dwBloodTTH;
					CommonData.stHeroData.astHeroList[(int)b].astTalentDetail = enumerator.get_Current().TalentArr;
					CommonData.stHeroData.astHeroList[(int)b].dwGhostLevel = (uint)enumerator.get_Current().actorHero.get_handle().ValueComponent.actorSoulLevel;
					this.CreateHeroBaseInfoData(enumerator.get_Current(), ref CommonData.stHeroData.astHeroList[(int)b].stHeroDetailInfo);
					this.CreateBattleStatisticInfoData(enumerator.get_Current(), ref CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo);
					this.CreateSkillDetailInfoData(enumerator.get_Current(), ref CommonData.stHeroData.astHeroList[(int)b].astSkillStatisticInfo, ref CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo);
					PoolObjHandle<ActorRoot> captain = Singleton<GamePlayerCenter>.GetInstance().GetPlayer(playerId).Captain;
					if (captain.get_handle().EquipComponent != null)
					{
						Dictionary<ushort, uint> equipBoughtHistory = captain.get_handle().EquipComponent.GetEquipBoughtHistory();
						...
						{
							COMDT_SETTLE_INBATTLE_EQUIP_INFO arg_269_0 = CommonData.stHeroData.astHeroList[(int)b].astInBattleEquipInfo[num];
							KeyValuePair<ushort, uint> current = enumerator2.get_Current();
							arg_269_0.dwEquipID = (uint)current.get_Key();
							COMDT_SETTLE_INBATTLE_EQUIP_INFO arg_294_0 = CommonData.stHeroData.astHeroList[(int)b].astInBattleEquipInfo[num];
							KeyValuePair<ushort, uint> current2 = enumerator2.get_Current();
							arg_294_0.dwFirstBuyTime = current2.get_Value();
							if (++num >= (int)b2)
							{
								break;
							}
					...
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.dwSoliderKillCoin = (uint)enumerator.get_Current().CoinFromKillSolider;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.dwMonsterKillCoin = (uint)enumerator.get_Current().CoinFromKillMonster;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.dwTowerKillCoin = (uint)enumerator.get_Current().CoinFromKillTower;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.dwDragonKillCoin = (uint)enumerator.get_Current().CoinFromKillDragon;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.dwHeroKillCoin = (uint)enumerator.get_Current().CoinFromKillHero;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.dwSystemAddCoin = (uint)enumerator.get_Current().CoinFromSystem;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.bDeathNum = (byte)enumerator.get_Current().numDead;
					int num4 = 0;
					while (num4 < enumerator.get_Current().numDead && num4 < 50)
					{
						CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.astDeathPos[num4].iXPos = enumerator.get_Current().m_arrDeadPosition[num4].iXPos;
						CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.astDeathPos[num4].iZPos = enumerator.get_Current().m_arrDeadPosition[num4].iZPos;
						num4++;
					}
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.iHeroEndLive = ((enumerator.get_Current().actorHero.get_handle().ActorControl.myBehavior != ObjBehaviMode.State_Dead) ? 1 : 0);
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.iHeroInitHp = enumerator.get_Current().actorHero.get_handle().ValueComponent.actorHpTotal;
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.iHeroEndHp = enumerator.get_Current().actorHero.get_handle().ValueComponent.actorHp;
					b += 1;
				}
			}
		...
		private void CreateDestroyReportData(ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			COMDT_STATISTIC_BASE_STRUCT[] astDetail = cOMDT_STATISTIC_STRUCT_PILE.astDetail;
			if (destroyStat.get_Count() != 0)
			{
				cOMDT_STATISTIC_STRUCT_PILE.bReportType = 1;
				DictionaryView<uint, Dictionary<int, DestroyStat>>.Enumerator enumerator = destroyStat.GetEnumerator();
				while (enumerator.MoveNext())
				{
					KeyValuePair<uint, Dictionary<int, DestroyStat>> current = enumerator.get_Current();
					...
					while (enumerator2.MoveNext())
					{
						...
						int arg_AC_1 = 0;
						KeyValuePair<uint, Dictionary<int, DestroyStat>> current2 = enumerator.get_Current();
						arg_AC_0[arg_AC_1] = current2.get_Key();
						int[] arg_CC_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_CC_1 = 1;
						KeyValuePair<int, DestroyStat> current3 = enumerator2.get_Current();
						arg_CC_0[arg_CC_1] = current3.get_Key();
						...
						arg_115_0[arg_115_1] = current4.get_Value().CampEnemyNum;
						num++;
						int arg_134_0 = num2;
						KeyValuePair<int, DestroyStat> current5 = enumerator2.get_Current();
						num2 = arg_134_0 + current5.get_Value().CampEnemyNum;
						astDetail[num].stKeyInfo.bKeyNum = 3;
						int[] arg_164_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_164_1 = 0;
						KeyValuePair<uint, Dictionary<int, DestroyStat>> current6 = enumerator.get_Current();
						arg_164_0[arg_164_1] = current6.get_Key();
						int[] arg_184_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_184_1 = 1;
						KeyValuePair<int, DestroyStat> current7 = enumerator2.get_Current();
						arg_184_0[arg_184_1] = current7.get_Key();
						...
						arg_1CD_0[arg_1CD_1] = current8.get_Value().CampSelfNum;
						num++;
						int arg_1EC_0 = num3;
						KeyValuePair<int, DestroyStat> current9 = enumerator2.get_Current();
						num3 = arg_1EC_0 + current9.get_Value().CampSelfNum;
					}
					astDetail[num].stKeyInfo.bKeyNum = 3;
					int[] arg_228_0 = astDetail[num].stKeyInfo.KeyDetail;
					int arg_228_1 = 0;
					KeyValuePair<uint, Dictionary<int, DestroyStat>> current10 = enumerator.get_Current();
					arg_228_0[arg_228_1] = current10.get_Key();
					...
					int arg_29D_1 = 0;
					KeyValuePair<uint, Dictionary<int, DestroyStat>> current11 = enumerator.get_Current();
					arg_29D_0[arg_29D_1] = current11.get_Key();
					astDetail[num].stKeyInfo.KeyDetail[1] = 0;
					astDetail[num].stKeyInfo.KeyDetail[2] = 0;
					astDetail[num].stValueInfo.bValueNum = 1;
		...
		private void CreateSettleReportData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin, bool bWin)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (enumerator.get_Current().bHurtTakenMost)
				{
					flag3 = true;
				}
				if (enumerator.get_Current().bGetCoinMost)
				{
					flag2 = true;
				}
				if (enumerator.get_Current().bHurtToHeroMost)
				{
					flag = true;
				}
				if (enumerator.get_Current().bAsssistMost)
				{
					flag5 = true;
				}
				if (enumerator.get_Current().bKillMost)
				{
					flag4 = true;
				}
				if (enumerator.get_Current().bKillOrganMost)
				{
					flag6 = true;
				}
			}
			...
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetDeadTime(1, ActorTypeDef.Actor_Type_Organ, 0);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 7;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetDeadTime(1, ActorTypeDef.Actor_Type_Organ, 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 8;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetDeadTime(2, ActorTypeDef.Actor_Type_Organ, 0);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 9;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetDeadTime(2, ActorTypeDef.Actor_Type_Organ, 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 10;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetKillDragonNum(1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 11;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetKillDragonNum(2);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			...
			if (Singleton<CBattleSystem>.GetInstance().FightForm != null)
			{
				...
				astDetail[num8].stValueInfo.bValueNum = 1;
				astDetail[num8].stValueInfo.ValueDetail[0] = (int)Singleton<CBattleSystem>.GetInstance().m_MaxBattleFPS;
				num8++;
				astDetail[num8].stKeyInfo.bKeyNum = 1;
				...
				astDetail[num8].stValueInfo.ValueDetail[0] = (int)(Singleton<CBattleSystem>.GetInstance().m_AveBattleFPS / Singleton<CBattleSystem>.GetInstance().m_BattleFPSCount);
				num8++;
				astDetail[num8].stKeyInfo.bKeyNum = 1;
				astDetail[num8].stKeyInfo.KeyDetail[0] = 21;
				astDetail[num8].stValueInfo.bValueNum = 1;
				astDetail[num8].stValueInfo.ValueDetail[0] = (int)Singleton<CBattleSystem>.GetInstance().m_MinBattleFPS;
				num8++;
			}
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 22;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = (int)Singleton<FrameSynchr>.get_instance().m_MaxPing;
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 23;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = (int)Singleton<FrameSynchr>.get_instance().m_AvePing;
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 24;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = (int)Singleton<FrameSynchr>.get_instance().m_MinPing;
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 25;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetBaronDeadCount(1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 26;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = battleDeadStat.GetBaronDeadCount(2);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			...
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = Singleton<BattleStatistic>.get_instance().GetCampScore(playerKDA.PlayerCamp);
			num8++;
			CommonData.stStatisticData.astDetail[(int)CommonData.stStatisticData.bNum].wNum = (ushort)num8;
			COMDT_STATISTIC_DATA expr_D2E = CommonData.stStatisticData;
			expr_D2E.bNum += 1;
		}
		...
		private void CreateAttrReportData(ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			while (enumerator.MoveNext())
			{
				ListView<IStarCondition>.Enumerator enumerator2 = enumerator.get_Current().GetEnumerator();
				while (enumerator2.MoveNext())
				{
					if (enumerator2.get_Current().type == 2)
					{
						int[] keys = enumerator2.get_Current().keys;
						int[] values = enumerator2.get_Current().values;
						...
						for (int j = 0; j < values.Length; j++)
						{
							...
							{
								astDetail[num].stValueInfo.ValueDetail[j] = this.FakeValue(enumerator2.get_Current().configInfo.ComparetorDetail[j], enumerator2.get_Current().configInfo.ValueDetail[j]);
							}
						}
						num++;
					}
		...
		private void CreateGeneralData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			List<CShenFuStat.ShenFuRecord> shenFuRecord = Singleton<BattleStatistic>.get_instance().m_shenFuStat.GetShenFuRecord(playerId);
			...
			if (num2 > 0)
			{
				...
				for (int l = 0; l < num3; l++)
				{
					...
					int num5 = 60 + l * 30;
					stGeneralData.astGame30SecDetail[l].dwGhostLevel = (uint)Singleton<BattleStatistic>.get_instance().m_playerSoulLevelStat.GetPlayerSoulLevelAtTime(playerId, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwKillCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetHeroDeadAtTime(playerId, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwKillDragonCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetKillDragonNumAtTime(playerId, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwKillBigDragonCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetKillSpecialMonsterNumAtTime(playerId, num5 * 1000, 8);
					stGeneralData.astGame30SecDetail[l].dwDestroyTowerCnt = (uint)(Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDeadNumAtTime(BattleLogic.GetOppositeCmp(playerKDA.PlayerCamp), ActorTypeDef.Actor_Type_Organ, 1, num5 * 1000) + Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDeadNumAtTime(BattleLogic.GetOppositeCmp(playerKDA.PlayerCamp), ActorTypeDef.Actor_Type_Organ, 4, num5 * 1000));
					stGeneralData.astGame30SecDetail[l].dwDestroyOuterTowerCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDeadNumAtTime(BattleLogic.GetOppositeCmp(playerKDA.PlayerCamp), ActorTypeDef.Actor_Type_Organ, 1, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwDestroyBaseTowerCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDeadNumAtTime(BattleLogic.GetOppositeCmp(playerKDA.PlayerCamp), ActorTypeDef.Actor_Type_Organ, 4, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwBigDragonBuffCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleBuffStat.GetDataByIndex(playerKDA.PlayerCamp, l);
					stGeneralData.astGame30SecDetail[l].dwKillMonsterCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetMonsterDeadAtTime(playerId, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwKillSoldierCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetSoldierDeadAtTime(playerId, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwCoinCnt = playerKDA.GetPlayerCoinAtTimeWithType(l + 1, KDAStat.GET_COIN_CHANNEL_TYPE.GET_COIN_CHANNEL_TYPE_ALL);
					stGeneralData.astGame30SecDetail[l].dwSoliderKillCoin = playerKDA.GetPlayerCoinAtTimeWithType(l + 1, KDAStat.GET_COIN_CHANNEL_TYPE.GET_COIN_CHANNEL_TYPE_KILLSOLIDER);
					stGeneralData.astGame30SecDetail[l].dwMonsterKillCoin = playerKDA.GetPlayerCoinAtTimeWithType(l + 1, KDAStat.GET_COIN_CHANNEL_TYPE.GET_COIN_CHANNEL_TYPE_KILLMONSTER);
					stGeneralData.astGame30SecDetail[l].dwRedBuffCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetKillRedBaNumAtTime(playerId, num5 * 1000);
					stGeneralData.astGame30SecDetail[l].dwBlueBuffCnt = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetKillBlueBaNumAtTime(playerId, num5 * 1000);
					VInt2 timeLocation = Singleton<BattleStatistic>.get_instance().m_locStat.GetTimeLocation(playerId, l);
					stGeneralData.astGame30SecDetail[l].iXPos = timeLocation.x;
					stGeneralData.astGame30SecDetail[l].iZPos = timeLocation.y;
				}
			}
			...
			PoolObjHandle<ActorRoot> captain = Singleton<GamePlayerCenter>.GetInstance().GetPlayer(playerId).Captain;
			if (captain && captain.get_handle().EquipComponent != null)
			{
				CExistEquipInfoSet existEquipInfoSet = captain.get_handle().EquipComponent.GetExistEquipInfoSet();
				int num6 = (existEquipInfoSet.m_existEquipInfoCount >= 6) ? 6 : existEquipInfoSet.m_existEquipInfoCount;
				stGeneralData.bEquipNum = (byte)num6;
				for (int m = 0; m < num6; m++)
				{
					stGeneralData.astEquipDetail[m].bCnt = (byte)existEquipInfoSet.m_existEquipInfos[m].m_amount;
					stGeneralData.astEquipDetail[m].dwEquipID = (uint)existEquipInfoSet.m_existEquipInfos[m].m_equipID;
				}
			}
			if (captain && captain.get_handle().ValueComponent != null)
			{
				stGeneralData.dwTotalInGameCoin = (uint)captain.get_handle().ValueComponent.GetGoldCoinIncomeInBattle();
				stGeneralData.dwMaxInGameCoin = (uint)captain.get_handle().ValueComponent.GetMaxGoldCoinIncomeInBattle();
			}
			SLevelContext curLvelContext = Singleton<BattleLogic>.GetInstance().GetCurLvelContext();
			if (curLvelContext.IsMobaModeWithOutGuide())
			{
				...
				num7 *= captain.get_handle().TheActorMeta.ActorCamp - 1;
			...
			uint num8 = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetTotalNum(playerKDA.PlayerCamp, ActorTypeDef.Actor_Type_Monster, 2, 8);
			if (num8 > 10u)
			{
				num8 = 10u;
			}
			...
			{
				stGeneralData.BigDragonBattleToDeadTimeDtail[(int)((UIntPtr)num9)] = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetRecordAtIndex(playerKDA.PlayerCamp, ActorTypeDef.Actor_Type_Monster, 2, 8, (int)num9).fightTime;
			}
			if (curLvelContext != null && curLvelContext.IsMobaModeWithOutGuide())
			{
				if (curLvelContext.m_pvpPlayerNum == 10)
				{
					uint num10 = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetTotalNum(playerKDA.PlayerCamp, ActorTypeDef.Actor_Type_Monster, 2, 9);
					if (num10 > 10u)
					{
						num10 = 10u;
					}
					...
					{
						stGeneralData.DragonBattleToDeadTimeDtail[(int)((UIntPtr)num11)] = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetRecordAtIndex(playerKDA.PlayerCamp, ActorTypeDef.Actor_Type_Monster, 2, 9, (int)num11).fightTime;
					}
				}
				else
				{
					uint num12 = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetTotalNum(playerKDA.PlayerCamp, ActorTypeDef.Actor_Type_Monster, 2, 7);
					if (num12 > 10u)
					{
						num12 = 10u;
					}
					...
					{
						stGeneralData.DragonBattleToDeadTimeDtail[(int)((UIntPtr)num13)] = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetRecordAtIndex(playerKDA.PlayerCamp, ActorTypeDef.Actor_Type_Monster, 2, 7, (int)num13).fightTime;
					}
				}
				if (curLvelContext.m_pvpPlayerNum == 10)
				{
					...
					{
						stGeneralData.MonsterDeadDetail[(int)((UIntPtr)num15)] = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDeadNum(0, ActorTypeDef.Actor_Type_Monster, 2, array[(int)((UIntPtr)num15)]);
					}
				}
				else
				{
					...
					{
						stGeneralData.MonsterDeadDetail[(int)((UIntPtr)num17)] = (uint)Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDeadNum(0, ActorTypeDef.Actor_Type_Monster, 2, array2[(int)((UIntPtr)num17)]);
					}
				}
			}
			stGeneralData.iTimeUse = (int)(Singleton<FrameSynchr>.get_instance().LogicFrameTick * 0.001f);
			...
			DictionaryView<uint, DictionaryView<uint, NONHERO_STATISTIC_INFO>> nonHeroInfo = Singleton<BattleStatistic>.get_instance().m_NonHeroInfo;
			DictionaryView<uint, NONHERO_STATISTIC_INFO> dictionaryView;
			NONHERO_STATISTIC_INFO nONHERO_STATISTIC_INFO;
			if (nonHeroInfo.TryGetValue(2u, ref dictionaryView))
			{
				if (dictionaryView.TryGetValue(playerCamp, ref nONHERO_STATISTIC_INFO))
				{
			...
			if (nonHeroInfo.TryGetValue(2u, ref dictionaryView))
			{
				COM_PLAYERCAMP cOM_PLAYERCAMP = (playerCamp != 1) ? 1 : 2;
				if (dictionaryView.TryGetValue(cOM_PLAYERCAMP, ref nONHERO_STATISTIC_INFO))
				{
				...
				else
				{
					...
					List<PoolObjHandle<ActorRoot>> organActors2 = Singleton<GameObjMgr>.get_instance().OrganActors;
			...
			if (Singleton<BattleStatistic>.get_instance().m_stSoulStatisticInfo != null)
			{
				GET_SOUL_EXP_STATISTIC_INFO stSoulStatisticInfo = Singleton<BattleStatistic>.get_instance().m_stSoulStatisticInfo;
			...
			if (nonHeroInfo.TryGetValue(1u, ref dictionaryView))
			{
				COM_PLAYERCAMP cOM_PLAYERCAMP2 = (playerCamp != 1) ? 1 : 2;
				if (dictionaryView.TryGetValue(cOM_PLAYERCAMP2, ref nONHERO_STATISTIC_INFO))
				{
			...
			List<PoolObjHandle<ActorRoot>> list3 = ActorHelper.FilterActors(Singleton<GameObjMgr>.get_instance().HeroActors, new ActorFilterDelegate(Singleton<BattleLogic>.get_instance().FilterEnemyActor));
			stGeneralData.iBossCount = list3.get_Count();
			for (int num19 = 0; num19 < list3.get_Count(); num19++)
			{
				stGeneralData.iBossHPMax = Math.Max(stGeneralData.iBossHPMax, list3.get_Item(num19).get_handle().ValueComponent.ObjValueStatistic.iActorMaxHp);
				if (list3.get_Item(num19).get_handle().ValueComponent.ObjValueStatistic.iActorMinHp != -1)
				{
					if (num19 == 0)
					{
						stGeneralData.iBossHPMin = list3.get_Item(num19).get_handle().ValueComponent.ObjValueStatistic.iActorMinHp;
					}
					else
					{
						stGeneralData.iBossHPMin = Math.Min(stGeneralData.iBossHPMin, list3.get_Item(num19).get_handle().ValueComponent.ObjValueStatistic.iActorMinHp);
					}
				}
				for (int num20 = 0; num20 < 10; num20++)
				{
					if (list3.get_Item(num19) && list3.get_Item(num19).get_handle().SkillControl != null && list3.get_Item(num19).get_handle().SkillControl.stSkillStat != null && list3.get_Item(num19).get_handle().SkillControl.stSkillStat.SkillStatistictInfo != null)
					{
						SKILLSTATISTICTINFO[] skillStatistictInfo = list3.get_Item(num19).get_handle().SkillControl.stSkillStat.SkillStatistictInfo;
			...
			stGeneralData.iCommunicationCount2 = (int)Singleton<NetworkModule>.get_instance().RecvGameMsgCount;
			stGeneralData.iCommunicationCount1 = (int)Singleton<NetworkModule>.get_instance().RecvGameMsgCount;
			stGeneralData.bSelfCampKillTowerCnt = Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDestroyTowerCount(playerKDA.PlayerCamp, 1);
			stGeneralData.bSelfCampKillBaseCnt = Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetDestroyTowerCount(playerKDA.PlayerCamp, 2);
			this.FillVDStat(ref stGeneralData, playerId);
			List<PoolObjHandle<ActorRoot>> organActors3 = Singleton<GameObjMgr>.get_instance().OrganActors;
			for (int num21 = 0; num21 < organActors3.get_Count(); num21++)
			{
				PoolObjHandle<ActorRoot> poolObjHandle3 = organActors3.get_Item(num21);
				if (poolObjHandle3.get_handle().TheActorMeta.ActorCamp == captain.get_handle().TheActorMeta.ActorCamp && poolObjHandle3.get_handle().TheStaticData.TheOrganOnlyInfo.OrganType == 2 && poolObjHandle3.get_handle().ValueComponent != null)
				{
					stGeneralData.dwSelfCampBaseBlood = (uint)poolObjHandle3.get_handle().ValueComponent.actorHp;
				}
			}
			if (captain && captain.get_handle().EquipComponent != null)
			{
				stGeneralData.bQuickBuyItemCnt = (byte)captain.get_handle().EquipComponent.m_iFastBuyEquipCount;
				stGeneralData.bPanelBuyItemCnt = (byte)captain.get_handle().EquipComponent.m_iBuyEquipCount;
			}
			int num22 = 0;
			...
			stGeneralData.iEnemyKIllHeroGap = Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetEnemyKillHeroMaxGap(playerKDA.PlayerCamp);
		}

		private void FillVDStat(ref COMDT_SETTLE_GAME_GENERAL_INFO stGeneralData, uint playerID)
		{
			if (Singleton<BattleStatistic>.get_instance().m_vdStat != null)
			{
				Player player = Singleton<GamePlayerCenter>.get_instance().GetPlayer(playerID);
				if (player != null)
				{
					COM_PLAYERCAMP inTo = 0;
					if (player.PlayerCamp == 1)
					{
						inTo = 2;
					}
					else if (player.PlayerCamp == 2)
					{
						inTo = 1;
					}
					stGeneralData.bAdvantageNum = (((byte)Singleton<BattleStatistic>.get_instance().m_vdStat.count >= 16) ? 16 : ((byte)Singleton<BattleStatistic>.get_instance().m_vdStat.count));
					for (int i = 0; i < (int)stGeneralData.bAdvantageNum; i++)
					{
						Singleton<BattleStatistic>.get_instance().m_vdStat.GetMaxCampStat(i, player.PlayerCamp, inTo, out stGeneralData.astAdvantageDetail[i].iMaxAdvantageValue, out stGeneralData.astAdvantageDetail[i].iMaxDisadvantageValue);
					}
					stGeneralData.iCurrentDisparity = Singleton<BattleStatistic>.get_instance().m_vdStat.CalcCampStat(player.PlayerCamp, inTo);
					stGeneralData.bSelfCampHaveWinningFlag = ((!Singleton<BattleStatistic>.get_instance().bSelfCampHaveWinningFlag) ? 0 : 1);
				}
			}
		}
