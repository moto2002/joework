	...
	public class MiniMapTrack_3DUI
	{
		public class InnerData
		{
			...
			public void _updateRotation(PoolObjHandle<ActorRoot> actorRoot)
			{
				...
				if (this.forward != actorRoot.get_handle().forward)
				{
					float num = Mathf.Atan2((float)actorRoot.get_handle().forward.z, (float)actorRoot.get_handle().forward.x) * 57.29578f - 90f;
					...
					this.forward = actorRoot.get_handle().forward;
				}
			}
		}
		...
		public void Clear()
		{
			for (int i = 0; i < this.m_innerDatas.get_Count(); i++)
			{
				MiniMapTrack_3DUI.InnerData innerData = this.m_innerDatas.get_Item(i);
				if (innerData != null)
				{
					innerData.Recyle();
				}
		...
		public MiniMapTrack_3DUI.InnerData Prepare_Imp(PoolObjHandle<ActorRoot> actorHandle, string iconPath)
		{
			...
			innerData.SetData(actorHandle.get_handle().ObjID, iconPath, actorHandle.get_handle().IsHostCamp());
			return innerData;
		}
		...
		public void SetTrackPosition_Imp(PoolObjHandle<ActorRoot> actorHandle, string iconPath)
		{
			...
			MiniMapTrack_3DUI.InnerData innerData = this.GetObjIDUsedInnerData(actorHandle.get_handle().ObjID);
			if (innerData == null)
			{
		...
		public static void SetTrackPosition(PoolObjHandle<ActorRoot> actorHandle, string iconPath)
		{
			...
			else if (actorHandle.get_handle().IsHostCamp() && theMinimapSys.MMiniMapTrack_3Dui != null)
			{
				theMinimapSys.MMiniMapTrack_3Dui.SetTrackPosition_Imp(actorHandle, iconPath);
			}
		}
