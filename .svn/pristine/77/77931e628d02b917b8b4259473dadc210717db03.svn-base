...
namespace Assets.Scripts.GameSystem
{
	internal class CBuyCoinPanel : CBuyPanelBase
	{
		...
		private uint calcGainCoin(uint coinBase)
		{
			...
			return coinBase + Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().Level * dwConfValue;
		}
		...
		private void sendBuyCoinMsg(int BuyCoinType)
		{
			...
			cSPkg.stPkgData.set_stCoinBuyReq(new CSPKG_CMD_COINBUY());
			cSPkg.stPkgData.get_stCoinBuyReq().bBuyType = (byte)BuyCoinType;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}

		public void BuyCoinRsp(CSPkg msg)
		{
			this.CurBuyTime = (int)(msg.stPkgData.get_stCoinBuyRsp().wBuyStartFreq + (ushort)msg.stPkgData.get_stCoinBuyRsp().stBuyList.bCoinGetCnt - 1);
