...
namespace Assets.Scripts.GameLogic
{
	public class DistanceSearchCondition
	{
		public static bool Fit(ActorRoot _actor, ActorRoot _mainActor, int _dis)
		{
			...
			ulong sqrMagnitudeLong2D = (ulong)(_actor.location - _mainActor.location).get_sqrMagnitudeLong2D();
		...
		{
			ulong num = (ulong)((long)_dis * (long)_dis);
			ulong sqrMagnitudeLong2D = (ulong)(_curPos - _mainActor.location).get_sqrMagnitudeLong2D();
			return num >= sqrMagnitudeLong2D;
		}
	}
}
