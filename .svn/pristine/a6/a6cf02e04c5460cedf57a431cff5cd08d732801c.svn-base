...
namespace Assets.Scripts.GameSystem
{
	public class CTaskSys : Singleton<CTaskSys>
	{
		...
		public void InitReport()
		{
			...
			{
				Singleton<CTimerManager>.get_instance().RemoveTimer(this.sendReportTimer);
			}
			...
			this.sendReportTimer = Singleton<CTimerManager>.get_instance().AddTimer((int)(dataByKey.dwConfValue * 1000u), 0, new CTimer.OnTimeUpHandler(this.OnReportTimerEnd));
			Singleton<CTimerManager>.get_instance().ResumeTimer(this.sendReportTimer);
		}
		...
		public void SendReport()
		{
			...
			cSPkg.stPkgData.get_stShareTLogReq().dwSecretaryNum = 7u;
			for (int i = 0; i < this.reportInts.Length; i++)
			{
				cSPkg.stPkgData.get_stShareTLogReq().SecretaryDetail[i] = this.reportInts[i];
			}
		...
		{
			uint mentorTaskID = Singleton<CTaskSys>.get_instance().mentorTaskID;
			CTask task = Singleton<CTaskSys>.get_instance().model.GetTask(mentorTaskID);
		...
		public override void Init()
		{
			...
			Singleton<EventRouter>.get_instance().AddEventHandler("TASK_HUOYUEDU_Change", new Action(this.On_TASK_HUOYUEDU_Change));
		...
		{
			SCPKG_CMD_GAMELOGINRSP stGameLoginRsp = pkg.stPkgData.get_stGameLoginRsp();
			int num = 0;
		...
		{
			SCPKG_TASKUPD_NTF stTaskUdpNtf = pkg.stPkgData.get_stTaskUdpNtf();
			int num = 0;
			while ((long)num < (long)((ulong)stTaskUdpNtf.dwUpdTaskCnt))
			{
				...
				if (sCDT_UPDTASKONE != null)
				{
					...
					{
						num2 = sCDT_UPDTASKONE.stUpdTaskInfo.get_stUdpPrerequisite().dwTaskID;
					}
					if (this.model.task_Data.task_map.ContainsKey(num2))
					{
						SCDT_UDPPREREQUISITE stUdpPrerequisite = sCDT_UPDTASKONE.stUpdTaskInfo.get_stUdpPrerequisite();
		...
		{
			SCDT_USUTASKLIST stUsualTaskList = pkg.stPkgData.get_stUsualTaskRes().stUpdateDetail.get_stUsualTaskList();
		...
		{
			SCPKG_NEWTASKGET_NTF stNewTaskGet = pkg.stPkgData.get_stNewTaskGet();
			int num = 0;
		...
		{
			if (pkg.stPkgData.get_stSubmitTaskRes().bSubmitResult == 0)
			{
				this._lastGetAwardTaskId = pkg.stPkgData.get_stSubmitTaskRes().dwTaskID;
				this._show_task_award(this._lastGetAwardTaskId);
				CTask task = this.model.GetTask(this._lastGetAwardTaskId);
				if (task != null)
				{
					task.SetState((CTask.State)pkg.stPkgData.get_stSubmitTaskRes().bTaskState);
		...
		private void _show_task_award(uint taskid)
		{
			...
			if (task != null)
			{
				...
				for (int j = 0; j < resAward.astRewardInfo.Length; j++)
				{
					...
					{
						array[j] = CUseableManager.CreateUsableByServerType(resTaskRewardDetail2.dwRewardType, resTaskRewardDetail2.iCnt, resTaskRewardDetail2.dwRewardID);
					}
				}
		...
		{
			SCPKG_GETHUOYUEDUREWARD_RSP stHuoYueDuRewardRsp = pkg.stPkgData.get_stHuoYueDuRewardRsp();
			this.model.huoyue_data.Get_Reward(stHuoYueDuRewardRsp.bRewardType, stHuoYueDuRewardRsp.wHuoYueDuId);
		...
		{
			COMDT_HUOYUEDU_DATA stHuoYueDuInfo = pkg.stPkgData.get_stNtfHuoYueDuInfo().stHuoYueDuInfo;
			this.model.huoyue_data.Set(2, stHuoYueDuInfo.dwWeekHuoYue, stHuoYueDuInfo.iWeekRewardCnt, stHuoYueDuInfo.WeekReward);
			this.model.huoyue_data.Set(1, stHuoYueDuInfo.dwDayHuoYue, stHuoYueDuInfo.iDayRewardCnt, stHuoYueDuInfo.DayReward);
			if (this.m_taskView != null)
			{
				this.m_taskView.Refresh_Huoyue();
			}
		...
		{
			int iErrorCode = pkg.stPkgData.get_stHuoYueDuRewardErr().iErrorCode;
		...
		private void OnTaskAwardGetConfirm(CUIEvent uiEvent)
		{
			if (this._lastGetAwardTaskId != 0u)
			{
				...
				bool flag = this._lastGetAwardTaskId == Singleton<CTaskSys>.get_instance().monthCardTaskID;
				bool flag2 = this._lastGetAwardTaskId == Singleton<CTaskSys>.get_instance().weekCardTaskID;
				if (flag)
				{
		...
		public void OnCloseTaskForm(CUIEvent uiEvent)
		{
			...
			Singleton<CMiShuSystem>.get_instance().OnCloseTalk(null);
			Singleton<EventRouter>.get_instance().BroadCastEvent("TaskUpdated");
		}
		...
		{
			if (Singleton<CTaskSys>.get_instance().model.share_task_id > 0u)
			{
				CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(1505u);
				cSPkg.stPkgData.get_stClientReportTaskDone().dwTaskID = Singleton<CTaskSys>.get_instance().model.share_task_id;
				cSPkg.stPkgData.get_stClientReportTaskDone().bEventType = 0;
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
			}
		}
		...
		private void _check(CUIEvent uiEvent, bool bDay)
		{
			...
			RES_HUOYUEDU_TYPE type = (!bDay) ? 2 : 1;
			...
			{
				Singleton<CUICommonSystem>.get_instance().OpenUseableTips(this.model.huoyue_data.GetUsable(rewardCfg.wID), uiEvent.m_pointerEventData.pressPosition.x, uiEvent.m_pointerEventData.pressPosition.y, enUseableTipsPos.enTop);
			}
		}
