...
public class Canvas3DImpl : Singleton<Canvas3DImpl>
{
	private class AutoAtlasInfo
	{
		...
		public void Register(Sprite3D sprite)
		{
			...
			if (this.textures.TryGetValue(nativeTextureID, ref uVDetail))
			{
	...
	public void registerAutoAtlas(Sprite3D sprite)
	{
		...
		if (!this.m_atlas.TryGetValue(sprite.autoAtlasTag, ref autoAtlasInfo))
		{
	...
	private void _DoRebuildAtlas()
	{
		...
		while (enumerator.MoveNext())
		{
			KeyValuePair<string, Canvas3DImpl.AutoAtlasInfo> current = enumerator.get_Current();
