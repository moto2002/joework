...
namespace Assets.Scripts.GameSystem
{
	public abstract class Activity
	{
		...
		public virtual RES_WEAL_TIME_TYPE TimeType
		{
			get
			{
				return this._config.dwTimeType;
			}
		}
		...
		public virtual RES_WEAL_COLORBAR_TYPE FlagType
		{
			get
			{
				...
				RES_WEAL_COLORBAR_TYPE rES_WEAL_COLORBAR_TYPE = this._config.bColorBar;
				if (rES_WEAL_COLORBAR_TYPE == 3 && this.StartTime > 0L)
				{
		...
		public virtual int Current
		{
			get
			{
				...
				{
					if (!this.PhaseList.get_Item(i).Marked)
					{
						return i;
					}
				}
				return this.PhaseList.get_Count();
			}
		}
		...
		public virtual bool ReadyForGet
		{
			get
			{
				...
				for (int i = 0; i < this.PhaseList.get_Count(); i++)
				{
					if (this.PhaseList.get_Item(i).ReadyForGet)
					{
						return true;
					}
				}
		...
		public virtual ActivityPhase CurPhase
		{
			get
			{
				for (int i = 0; i < this.PhaseList.get_Count(); i++)
				{
					if (!this.PhaseList.get_Item(i).Marked)
					{
						return this.PhaseList.get_Item(i);
					}
				}
				if (this.PhaseList.get_Count() > 0)
				{
					return this.PhaseList.get_Item(this.PhaseList.get_Count() - 1);
				}
				return null;
			}
		}
		...
		public string UID
		{
			get
			{
				...
				{
					"sgame_",
					(accountInfo == null) ? "0" : accountInfo.get_OpenId(),
					"_activity_",
					this.Key
				});
			}
		...
		{
			for (int i = 0; i < this._phaseList.get_Count(); i++)
			{
				this._phaseList.get_Item(i).Clear();
			}
		...
		public virtual void SetPhaseMarks(ulong mask)
		{
			for (int i = 0; i < this.PhaseList.get_Count(); i++)
			{
				this.PhaseList.get_Item(i).Marked = ((mask & 1uL << i) > 0uL);
			}
			this.NotifyMaskStateChanged();
		}
		...
		{
			if ((ulong)phaseId < (ulong)((long)this.PhaseList.get_Count()))
			{
				this.PhaseList.get_Item((int)phaseId).Marked = true;
				this.NotifyMaskStateChanged();
			}
		}
