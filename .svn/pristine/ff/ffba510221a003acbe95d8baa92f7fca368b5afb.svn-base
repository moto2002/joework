...
public class Seeker : MonoBehaviour, IPooledMonoBehaviour
{
			...
			if (!(monoModifier != null) || monoModifier.enabled)
			{
				...
				case Seeker.ModifierPass.PostProcess:
				{
					...
					else
					{
						...
						{
							"Error converting ",
							(j <= 0) ? "original" : pathModifier.GetType().get_Name(),
							"'s output to ",
	...
	{
		Seeker.<DelayPathStart>c__Iterator0 <DelayPathStart>c__Iterator = new Seeker.<DelayPathStart>c__Iterator0();
		<DelayPathStart>c__Iterator.p = p;
		<DelayPathStart>c__Iterator.<$>p = p;
		<DelayPathStart>c__Iterator.<>f__this = this;
		return <DelayPathStart>c__Iterator;
	}
