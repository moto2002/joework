...
namespace Assets.Scripts.GameSystem
{
	public class CTrainingHelper : Singleton<CTrainingHelper>
	{
		...
		private void OnTrainingHelperInit(CUIEvent uiEvent)
		{
			if (this.m_form == null)
			{
				this.m_form = Singleton<CBattleSystem>.get_instance().FightFormScript;
		...
		{
			List<PoolObjHandle<ActorRoot>>.Enumerator enumerator = Singleton<GameObjMgr>.get_instance().HeroActors.GetEnumerator();
			while (enumerator.MoveNext())
			{
		...
		{
			List<PoolObjHandle<ActorRoot>>.Enumerator enumerator = Singleton<GameObjMgr>.get_instance().OrganActors.GetEnumerator();
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current())
				{
		...
		private static void SpawnDynamicActor(ref CTrainingHelper.ActorSpawnInfo inSpawnInfo)
		{
			...
			PoolObjHandle<ActorRoot> poolObjHandle = Singleton<GameObjMgr>.get_instance().SpawnActorEx(null, ref actorMeta, bornPos, bornDir, false, true);
			if (poolObjHandle)
			{
		...
		private void DoCheatAction(CTrainingHelper.ECheatAct inAct, CTrainingHelper.EActTarget inTar, int inParam)
		{
			...
			case CTrainingHelper.ECheatAct.ToggleInvincible:
			{
				...
				if (hostPlayer2 != null && hostPlayer2.Captain && hostPlayer2.Captain.get_handle().ActorControl is HeroWrapper)
				{
					HeroWrapper heroWrapper = (HeroWrapper)hostPlayer2.Captain.get_handle().ActorControl;
			...
			{
				Singleton<BattleLogic>.get_instance().mapLogic.ResetSoldierRegion();
				Singleton<BattleLogic>.get_instance().dynamicProperty.ResetTimer();
				Singleton<GameObjMgr>.GetInstance().KillSoldiers();
				CTrainingHelper.OrganVisiter(1, inParam, new Action<ActorRoot, int>(CTrainingHelper.ReviveTower));
				CTrainingHelper.OrganVisiter(2, inParam, new Action<ActorRoot, int>(CTrainingHelper.ReviveTower));
				AttackOrder attackOrder = Singleton<BattleLogic>.get_instance().attackOrder;
				if (attackOrder != null)
				{
			...
			case CTrainingHelper.ECheatAct.AddGold:
			{
				...
				if (hostPlayer3 != null && hostPlayer3.Captain && hostPlayer3.Captain.get_handle().ValueComponent != null)
				{
					hostPlayer3.Captain.get_handle().ValueComponent.ChangeGoldCoinInBattle(1000, true, true, default(Vector3), false, default(PoolObjHandle<ActorRoot>));
				}
				break;
			}
