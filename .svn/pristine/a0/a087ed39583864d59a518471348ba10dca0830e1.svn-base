	...
	public class ActorConfig : MonoBehaviour, IPooledMonoBehaviour
	{
		...
		public int GroundSpeed
		{
			...
			set
			{
				...
				this.maxFrameMove = (float)((long)this.groundSpeed * (long)((ulong)Singleton<FrameSynchr>.get_instance().FrameDelta) / 1000L) * 0.001f;
			}
		}
		...
		private Vector3 NormalMoveLerp(uint nDeltaTick)
		{
			...
			if (this.ActorObj.hasReachedNavEdge || this.ActorObj.hasCollidedWithAgents)
			{
				...
				this.FrameBlockIndex = Singleton<FrameSynchr>.get_instance().CurFrameNum;
			}
			else
			{
				vector2.y = vector3.y;
				...
				float num2 = this.maxFrameMove * Singleton<FrameSynchr>.get_instance().PreActFrames;
				...
				else if (Singleton<FrameSynchr>.get_instance().CurFrameNum == this.FrameBlockIndex)
				{
					result = position;
				}
				else
		...
		{
			this.bNeedLerp = (Singleton<FrameSynchr>.get_instance().bActive && this.ActorObj.TheActorMeta.ActorType != ActorTypeDef.Actor_Type_Organ);
		...
		private void Update()
		{
			if (this.ActorObj != null && this.ActorObj.Visible && Singleton<BattleLogic>.get_instance().isFighting && Singleton<FrameSynchr>.GetInstance().isRunning)
			{
				try
				{
					bool flag = Singleton<FrameSynchr>.get_instance().FrameSpeed == 1 && this.bNeedLerp && (this.ActorObj.InCamera || this.ActorObj.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Bullet);
					...
					else if (flag && this.ActorControl != null && this.ActorControl.CanMove && this.ActorMovement != null && (this.ActorMovement.isMoving || this.ActorMovement.nLerpStep > 0))
					{
						...
						VInt vInt;
						if (!this.ActorMovement.isLerpFlying && PathfindingUtility.GetGroundY((VInt3)vector, out vInt))
						{
							vector.y = (float)vInt;
						}
					...
					else if (this.oldLocation != this.ActorObj.location)
					{
						...
						float magnitude;
						if (this.groundSpeed <= 0 || !flag || (magnitude = vector3.magnitude) > this.maxFrameMove * Singleton<FrameSynchr>.get_instance().PreActFrames)
						{
							this.myTransform.position = (Vector3)this.ActorObj.location;
							if (this.CustomMoveLerp != null)
							{
		...
		private void Awake()
		{
			...
			if (this.isStatic)
			{
				MonoSingleton<GameLoader>.get_instance().AddStaticActor(this);
			}
		}
