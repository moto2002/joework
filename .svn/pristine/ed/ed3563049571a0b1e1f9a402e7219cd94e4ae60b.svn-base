...
namespace Assets.Scripts.GameLogic.Treasure
{
	internal abstract class OrganRandomStrategy : BaseStrategy
	{
		...
		public override void NotifyDropEvent(PoolObjHandle<ActorRoot> actor)
		{
			...
			{
				ResOrganCfgInfo dataCfgInfoByCurLevelDiff = OrganDataHelper.GetDataCfgInfoByCurLevelDiff(actor.get_handle().TheActorMeta.ConfigId);
				...
				else if (this.hasRemain && (int)FrameRandom.Random(100u) <= MonoSingleton<GlobalConfig>.get_instance().OrganDropItemProbability)
				{
					this.PlayDrop();
				}
			}
