	...
	public static void PostCallbacks()
	{
		...
		while (true)
		{
			...
			if (commonCB.eType == AkCallbackType.AK_Monitoring)
			{
				...
				akMonitoringMsg.errorCode = (ErrorCode)Marshal.ReadInt32(intPtr);
			...
			{
				if (commonCB.eType != AkCallbackType.AK_Bank)
				{
								...
								if (byType != 176)
								{
									if (byType != 192)
									{
										if (byType != 208)
										{
											if (byType != 224)
											{
												if (byType != 240 && byType != 247 && byType != 255)
												{
													goto IL_5EC;
												}
												goto IL_5EC;
		...
		Block_17:
		string message = string.Format("WwiseUnity: PostCallbacks aborted due to error: Undefined callback type found. Callback object possibly corrupted.", new object[0]);
