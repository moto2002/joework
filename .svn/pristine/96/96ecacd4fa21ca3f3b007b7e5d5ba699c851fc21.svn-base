...
namespace AGE
{
	...
	public class HitTriggerTick : TickEvent
	{
		...
		private List<PoolObjHandle<ActorRoot>> FilterTargetByTriggerRegion(Action _action, PoolObjHandle<ActorRoot> _attackActor, BaseSkill _skill)
		{
			...
			if (targetActor && !targetActor.get_handle().ActorControl.IsDeadState)
			{
			...
			if (this.shape != null)
			{
				Singleton<TargetSearcher>.get_instance().BeginCollidedActorList(_attackActor, this.shape, false, true, null, this.bCheckSight);
				List<PoolObjHandle<ActorRoot>> collidedActors = Singleton<TargetSearcher>.get_instance().GetCollidedActors();
				...
				Singleton<TargetSearcher>.get_instance().EndCollidedActorList();
			}
			return HitTriggerTick.targetActors;
		}
	}
