	...
	public class CChatController : Singleton<CChatController>
	{
		...
		private void On_Chat_Text_Send(CUIEvent uiEvent)
		{
			if (this.view != null && this.view.bShow)
			{
				...
				{
					Singleton<CUIManager>.get_instance().OpenTips("Chat_Common_Tips_6", true, 1.5f, null, new object[0]);
				}
				else if (enCheckChatResult == CChatController.enCheckChatResult.BanLimit)
				{
					DateTime banTime = MonoSingleton<IDIPSys>.GetInstance().GetBanTime(8);
				...
				else if (enCheckChatResult == CChatController.enCheckChatResult.CdLimit)
				{
					...
					Singleton<CUIManager>.get_instance().OpenTips("chat_input_cd", true, 1.5f, null, new object[0]);
					this.view.ClearInputText();
				}
				else if (enCheckChatResult == CChatController.enCheckChatResult.FreeCntLimit)
				{
					uiEvent.m_eventParams.tagStr = inputText;
					CMallSystem.TryToPay(enPayPurpose.Chat, string.Empty, enPayType.DiamondAndDianQuan, (uint)Singleton<CChatController>.get_instance().model.sysData.chatCostNum, enUIEventID.Chat_Cost_Send, ref uiEvent.m_eventParams, enUIEventID.None, false, true, false);
				}
			}
		}
		...
		public CChatController.enCheckChatResult CheckSend(EChatChannel channel, string content = "", bool bContentCheck = false)
		{
			...
			if (channel == EChatChannel.Lobby && !Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(23))
			{
		...
		public bool On_InputFiled_EndEdit(string content)
		{
			...
			if (this.view.CurTab == EChatChannel.Settle)
			{
				...
				if (curLvelContext != null && curLvelContext.m_isWarmBattle)
				{
					...
					Singleton<EventRouter>.get_instance().BroadCastEvent("Chat_Settle_Change");
		...
		{
			if (Singleton<CUIManager>.get_instance().GetForm(CFriendContoller.FriendFormPath) != null)
			{
				this.view.HideDetailChatForm();
			}
			else
		...
		{
			SCPKG_OFFLINE_CHAT_NTF stOfflineChatNtf = msg.stPkgData.get_stOfflineChatNtf();
			for (int i = 0; i < (int)stOfflineChatNtf.bChatCnt; i++)
			{
				...
				if (cSDT_OFFLINE_CHAT_INFO != null)
				{
					...
					COMDT_FRIEND_INFO gameOrSnsFriend = Singleton<CFriendContoller>.get_instance().model.GetGameOrSnsFriend(stFrom.ullUid, (uint)stFrom.iLogicWorldID);
					if (gameOrSnsFriend != null)
					{
						...
						if (cChatChannel == null)
						{
							...
							{
								cChatChannel.list.Add(CChatUT.Build_4_OfflineInfo(Singleton<CTextManager>.get_instance().GetText("FriendChat_Offline_Info")));
							}
						...
						if (cChatEntity != null)
						{
							...
							Singleton<CChatController>.get_instance().view.ChatParser.Parse(cChatEntity.text, CChatParser.start_x, cChatEntity);
		...
		{
			string playerName = Singleton<BattleStatistic>.GetInstance().GetPlayerName(msg.stPkgData.get_stLeaveSettleUiNtf().ullUid, msg.stPkgData.get_stLeaveSettleUiNtf().dwLogicWorldID);
		...
		{
			SCPKG_CMD_CHAT_NTF stChatNtf = msg.stPkgData.get_stChatNtf();
			...
			if (num == 0)
			{
				...
				Singleton<CTimerManager>.get_instance().ResetTimerTotalTime(this.chatTimer, this.cur_chatTimer_totalTime * 1000);
				return;
			}
			this.cur_chatTimer_totalTime = CChatController.init_chatTime;
			Singleton<CTimerManager>.get_instance().ResetTimerTotalTime(this.chatTimer, this.cur_chatTimer_totalTime * 1000);
			...
			for (int i = num - 1; i >= 0; i--)
			{
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom = cOMDT_CHAT_MSG.stContent.get_stPrivate().stFrom;
					if (stFrom.ullUid == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().playerUllUID)
					{
						string text = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stPrivate().szContent);
					...
					{
						if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom.ullUid, (uint)stFrom.iLogicWorldID))
						{
							return;
						}
						string rawText = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stPrivate().szContent);
						CChatEntity chatEnt = CChatUT.Build_4_Friend(cOMDT_CHAT_MSG.stContent.get_stPrivate());
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom2 = cOMDT_CHAT_MSG.stContent.get_stLogicWord().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom2.ullUid, (uint)stFrom2.iLogicWorldID))
					{
						return;
					}
					...
					this.model.Add_Palyer_Info(cOMDT_CHAT_MSG.stContent.get_stLogicWord().stFrom);
					CChatEntity cChatEntity = CChatUT.Build_4_Lobby(cOMDT_CHAT_MSG.stContent.get_stLogicWord());
					if (flag8)
					{
						cChatEntity.type = EChaterType.Self;
					}
					this.model.channelMgr.Add_ChatEntity(cChatEntity, EChatChannel.Lobby, 0uL, 0u);
					string rawText2 = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stLogicWord().szContent);
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom3 = cOMDT_CHAT_MSG.stContent.get_stGuild().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom3.ullUid, (uint)stFrom3.iLogicWorldID))
					{
						return;
					}
					...
					this.model.Add_Palyer_Info(cOMDT_CHAT_MSG.stContent.get_stGuild().stFrom);
					CChatEntity cChatEntity2 = CChatUT.Build_4_Guild(cOMDT_CHAT_MSG.stContent.get_stGuild());
					if (flag9)
					{
						cChatEntity2.type = EChaterType.Self;
					}
					this.model.channelMgr.Add_ChatEntity(cChatEntity2, EChatChannel.Guild, 0uL, 0u);
					string rawText3 = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stGuild().szContent);
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom4 = cOMDT_CHAT_MSG.stContent.get_stGuildTeam().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom4.ullUid, (uint)stFrom4.iLogicWorldID))
					{
						return;
					}
					...
					CChatEntity cChatEntity3 = CChatUT.Build_4_GuildMatchTeam(cOMDT_CHAT_MSG.stContent.get_stGuildTeam());
					if (flag10)
					{
						cChatEntity3.type = EChaterType.Self;
					}
					this.model.channelMgr.Add_ChatEntity(cChatEntity3, EChatChannel.GuildMatchTeam, 0uL, 0u);
					string rawText4 = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stGuildTeam().szContent);
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom5 = cOMDT_CHAT_MSG.stContent.get_stRoom().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom5.ullUid, (uint)stFrom5.iLogicWorldID))
					{
						return;
					}
					...
					CChatEntity cChatEntity4 = CChatUT.Build_4_Room(cOMDT_CHAT_MSG.stContent.get_stRoom());
					if (flag11)
					{
					...
					string rawText5 = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stRoom().szContent);
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom6 = cOMDT_CHAT_MSG.stContent.get_stBattle().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom6.ullUid, (uint)stFrom6.iLogicWorldID))
					{
						return;
					}
					...
					this.model.Add_Palyer_Info(cOMDT_CHAT_MSG.stContent.get_stBattle().stFrom);
					CChatEntity chatEnt2 = CChatUT.Build_4_SelectHero(cOMDT_CHAT_MSG.stContent.get_stBattle());
				...
				{
					COMDT_CHAT_PLAYER_INFO stFrom7 = cOMDT_CHAT_MSG.stContent.get_stTeam().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom7.ullUid, (uint)stFrom7.iLogicWorldID))
					{
						return;
					}
					...
					this.model.Add_Palyer_Info(cOMDT_CHAT_MSG.stContent.get_stTeam().stFrom);
					CChatEntity cChatEntity5 = CChatUT.Build_4_Team(cOMDT_CHAT_MSG.stContent.get_stTeam());
					if (flag13)
					{
						cChatEntity5.type = EChaterType.Self;
					}
					this.model.channelMgr.Add_ChatEntity(cChatEntity5, EChatChannel.Team, 0uL, 0u);
					string rawText6 = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stTeam().szContent);
					...
					flag6 = true;
					Singleton<EventRouter>.get_instance().BroadCastEvent("Chat_TeamChat_Change");
				}
				else if (CChatUT.Convert_ChatMsgType_Channel(cOMDT_CHAT_MSG.bType) == EChatChannel.Settle)
				{
					COMDT_CHAT_PLAYER_INFO stFrom8 = cOMDT_CHAT_MSG.stContent.get_stSettle().stFrom;
					if (Singleton<CFriendContoller>.get_instance().model.IsBlack(stFrom8.ullUid, (uint)stFrom8.iLogicWorldID))
					{
						return;
					}
					...
					this.model.Add_Palyer_Info(cOMDT_CHAT_MSG.stContent.get_stSettle().stFrom);
					CChatEntity cChatEntity6 = CChatUT.Build_4_Settle(cOMDT_CHAT_MSG.stContent.get_stSettle());
					if (flag14)
					{
						cChatEntity6.type = EChaterType.Self;
					}
					this.model.channelMgr.Add_ChatEntity(cChatEntity6, EChatChannel.Settle, 0uL, 0u);
					string rawText7 = UT.Bytes2String(cOMDT_CHAT_MSG.stContent.get_stSettle().szContent);
				...
				{
					Singleton<InBattleMsgMgr>.get_instance().Handle_InBattleMsg_Ntf(cOMDT_CHAT_MSG.stContent.get_stInBattle());
				}
			}
			if (flag2)
			{
