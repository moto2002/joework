...
namespace AGE
{
	...
	public class FilterTargetType : TickCondition
	{
		...
		private void FilterActorCamp(PoolObjHandle<ActorRoot> actorObj)
		{
			...
			if (this.bFilterSameCamp && !hostPlayer.Captain.get_handle().IsEnemyCamp(actorObj.get_handle()))
			{
				this.bCheckFilter = false;
				return;
			}
			if (this.bFilterDiffCamp && hostPlayer.Captain.get_handle().IsEnemyCamp(actorObj.get_handle()))
			{
				this.bCheckFilter = false;
				return;
			}
		...
		public override void Process(Action _action, Track _track)
		{
			...
			{
				this.bCheckFilter = actorHandle.get_handle().ActorControl.IsEnableReviveContext();
			}
