	...
	public class ObjAgent : BTBaseAgent, IPooledMonoBehaviour, IActorComponent
	{
		...
		public virtual void Born(ActorRoot actor)
		{
			...
			if (curLvelContext != null && curLvelContext.IsMobaMode() && curLvelContext.m_warmHeroAiDiffInfo != null && ownerPlayer != null && ownerPlayer.Computer)
			{
				...
				{
					if (this.m_wrapper.actor.TheActorMeta.ActorCamp == 1)
					{
		...
		public uint GetNewTargetByPriorityForSiege(uint objID, int srchR)
		{
			...
			if (actor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ)
			{
				return objID;
			}
		...
		{
			return this.m_wrapper.actor.location.get_vec3();
		}

		[MethodMetaInfo("获取自己当前的朝向", "")]
		public Vector3 GetMyForward()
		{
			return this.m_wrapper.actor.forward.get_vec3();
		}
		...
		public COM_PLAYERCAMP GetEnemyCamp()
		{
			...
			if (this.m_wrapper.actor.TheActorMeta.ActorCamp == 2)
			{
				return 1;
			}
			return 1;
		...
		{
			ResAiParamConf dataByKey = GameDataMgr.aiParamConfDataBin.GetDataByKey((uint)type);
			if (dataByKey == null)
			{
				return 0;
			}
		...
		{
			return this.m_wrapper.actor.ValueComponent.mActorValue[21].totalValue;
		}
		...
		public bool IsOutOfPursuitRange(Vector3 aimPos)
		{
			...
			if (monsterWrapper != null)
			{
				...
				{
					VInt3 vInt2 = this.m_wrapper.actorLocation - pursuit.PursuitOrigin;
					long sqrMagnitudeLong2D = vInt2.get_sqrMagnitudeLong2D();
					vInt2.Normalize();
					int num = pursuit.PursuitAngle / 2;
					if (sqrMagnitudeLong2D >= (long)pursuit.PursuitRadius * (long)pursuit.PursuitRadius || (num < 180 && VInt3.DotXZLong(ref vInt2, ref pursuit.PursuitDir) <= (long)pursuit.CosHalfAngle))
					{
						return true;
					}
				}
				else if (monsterWrapper.cfgInfo != null)
				{
					...
					return (this.m_wrapper.actorLocation - vInt).get_sqrMagnitudeLong2D() >= (long)(iPursuitR * iPursuitR);
				}
			}
			return false;
		}
		...
		public bool IsContinueCommonAttack()
		{
			...
			if (!flag)
			{
				...
				if (curUseSkill != null && curUseSkill.cfgData != null && ((curUseSkill.AppointType == 1 && curUseSkill.cfgData.bSkillTargetRule != 2) || curUseSkill.AppointType == 2))
				{
		...
		public uint GetMyTargetID()
		{
			...
			{
				result = this.m_wrapper.myTarget.get_handle().ObjID;
			}
			return result;
		}
		...
		public uint GetHeroWhoAttackSelf()
		{
			...
			return this.m_wrapper.LastHeroAtker.get_handle().ObjID;
		}
		...
		public EBTStatus IsAttackingBuildingAndHeroOrSoldierAttackMe()
		{
			...
			if (this.m_wrapper.myTarget.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ && this.m_wrapper.myLastAtker.get_handle().TheActorMeta.ActorType != ActorTypeDef.Actor_Type_Organ)
			{
				return EBTStatus.BT_SUCCESS;
			}
			return EBTStatus.BT_FAILURE;
		...
		{
			SLevelContext curLvelContext = Singleton<BattleLogic>.GetInstance().GetCurLvelContext();
			if (curLvelContext != null && curLvelContext.IsMobaModeWithOutGuide() && curLvelContext.m_heroAiType == 2)
			{
				return EBTStatus.BT_SUCCESS;
			}
			return EBTStatus.BT_FAILURE;
		...
		public void ShowRandomSignal()
		{
			...
			uint configId = (uint)ownerPlayer.Captain.get_handle().TheActorMeta.ConfigId;
			...
			ActorRoot handle = towerActors.get_Item(num).get_handle();
			...
			using (Dictionary<int, ShenFuObjects>.Enumerator enumerator = Singleton<ShenFuSystem>.GetInstance()._shenFuTriggerPool.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					if (current.get_Value().ShenFu.activeSelf)
					{
						...
						VInt3 vInt = new VInt3(position);
						long num = (long)range;
						if ((this.m_wrapper.actorLocation - vInt).get_sqrMagnitudeLong2D() <= num * num)
						{
							return position;
						}
					}
		...
		{
			Player ownerPlayer = ActorHelper.GetOwnerPlayer(ref this.m_wrapper.actorPtr);
			return ownerPlayer.Captain.get_handle().ObjID;
		}
		...
		public uint GetHostPlayrCaptain()
		{
			...
			return hostPlayer.Captain.get_handle().ObjID;
		}
		...
		public uint GetLeader()
		{
			...
			if (actor)
			{
				...
				{
					ActorRoot nearestSelfCampHero = Singleton<TargetSearcher>.get_instance().GetNearestSelfCampHero(this.m_wrapper.actor, 16000);
		...
		public uint GetMaster()
		{
			...
			{
				return monsterWrapper.hostActor.get_handle().ObjID;
			}
			return 0u;
		}
		...
		public EBTStatus IsAlive(uint objID)
		{
			...
			if (actor)
			{
				return (!actor.get_handle().ActorControl.IsDeadState) ? EBTStatus.BT_SUCCESS : EBTStatus.BT_FAILURE;
			}
			return EBTStatus.BT_FAILURE;
		}
		...
		public uint GetTerrorMeActor()
		{
			...
			{
				return this.m_wrapper.m_terrorMeActor.get_handle().ObjID;
			}
			return 0u;
		}
		...
		public uint GetTeamMemberTarget()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ActorRoot handle = current.get_handle();
				if (handle != null && handle.ActorControl != null && handle.ActorControl.myTarget && handle.ActorControl.myTarget.get_handle().ActorControl != null && !handle.ActorControl.myTarget.get_handle().ActorControl.IsDeadState && !this.m_wrapper.actor.IsSelfCamp(handle.ActorControl.myTarget.get_handle()))
				{
					return handle.ActorControl.myTarget.get_handle().ObjID;
				}
			}
			return 0u;
		}
		...
		{
			PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.GetInstance().GetActor(objID);
			if (actor && actor.get_handle().ActorControl != null && actor.get_handle().ActorControl.myTarget && actor.get_handle().ActorControl.myTarget.get_handle().ActorControl != null && !actor.get_handle().ActorControl.myTarget.get_handle().ActorControl.IsDeadState)
			{
				return actor.get_handle().ActorControl.myTarget.get_handle().ObjID;
			}
			return 0u;
		}
		...
		public uint GetNearestMember()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				{
					ulong sqrMagnitudeLong2D = (ulong)(handle.location - this.m_wrapper.actor.location).get_sqrMagnitudeLong2D();
		...
		public uint GetNearestMemberNotCaptain()
		{
			...
			while (enumerator.MoveNext())
			{
				PoolObjHandle<ActorRoot> current = enumerator.get_Current();
				ActorRoot handle = current.get_handle();
				if (this.m_wrapper.actor.ObjID != handle.ObjID && ownerPlayer.Captain.get_handle().ObjID != handle.ObjID)
				{
					ulong sqrMagnitudeLong2D = (ulong)(handle.location - this.m_wrapper.actor.location).get_sqrMagnitudeLong2D();
					if (sqrMagnitudeLong2D < num)
					{
		...
		public bool IsActorInBattle(uint objID)
		{
			...
			ObjAgent actorAgent = actor.get_handle().ActorAgent;
		...
		public ActorTypeDef GetActorType(uint objID)
		{
			...
			return actor.get_handle().TheActorMeta.ActorType;
		}
		...
		public int GetActorHPPercent(uint objID)
		{
			...
			if (actor)
			{
				return actor.get_handle().ValueComponent.actorHp * 10000 / actor.get_handle().ValueComponent.actorHpTotal;
			}
			return 0;
		}
		...
		public EBTStatus IsBase(uint objID)
		{
			...
			if (actor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ && actor.get_handle().TheStaticData.TheOrganOnlyInfo.OrganType == 2)
			{
				return EBTStatus.BT_SUCCESS;
			}
			return EBTStatus.BT_FAILURE;
		...
		public EBTStatus IsDragonAlive()
		{
			...
			{
				PoolObjHandle<ActorRoot> poolObjHandle = Singleton<BattleLogic>.GetInstance().m_dragonSpawn.GetSpawnedList().get_Item(0);
				if (poolObjHandle)
				{
		...
		{
			if (Singleton<BattleStatistic>.get_instance().m_battleDeadStat != null)
			{
				return Singleton<BattleStatistic>.get_instance().m_battleDeadStat.GetBaronDeadCount();
			}
			return 0;
		}
		...
		public EBTStatus PlayAgeAction(string actionName)
		{
			...
			else
			{
				...
				this.m_currentAction.get_handle().refParams.AddRefParam("SkillContext", skillUseContext);
				if (!this.m_currentAction)
				{
					return EBTStatus.BT_FAILURE;
				}
		...
		public virtual void RealMoveInMoveAttack(uint objID, int range)
		{
			...
			if (actor.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				int num2 = actor.get_handle().ValueComponent.actorHp * 10000 / this.m_wrapper.actor.ValueComponent.actorHpTotal;
			...
			if (flag)
			{
				this.m_wrapper.RealMovePosition(actor.get_handle().location, 0u);
			}
			else
			{
				VInt3 vInt = this.m_wrapper.actor.location - actor.get_handle().location;
				long num3 = (long)(range - vInt.get_magnitude2D());
				vInt.y = 0;
				vInt = vInt.NormalizeTo(1000);
				VInt3 vInt2 = (!(vInt == VInt3.zero)) ? vInt : VInt3.forward;
				if (actor.get_handle().ActorControl.GetActorSubType() != 2 && Math.Abs(num3) >= 1000L)
				{
				...
				else
				{
					...
					VInt3 vInt3 = VInt3.Cross(VInt3.up, vInt2);
					int num5 = (num4 - 5000) / 5000;
					vInt3 *= num5;
					if (actor.get_handle().ActorControl.GetActorSubType() == 2)
					{
						vInt3 -= vInt2;
						vInt3.NormalizeTo(1000);
					}
					int num6 = (int)(2000 + FrameRandom.Random(500u));
					VInt3 dest2 = this.m_wrapper.actor.location + vInt3 * num6 / 1000f;
		...
		{
			VInt3 vInt = new VInt3(dest);
			VInt3 direction = this.m_wrapper.actor.location - vInt;
			this.m_wrapper.RealMoveDirection(direction, 0u);
		}
		...
		public virtual Vector3 GetRandomFarPoint(uint objID)
		{
			...
			ActorRoot handle = actor.get_handle();
			VInt3 location = handle.location;
			VInt3 vInt = (this.m_wrapper.actor.location - location).NormalizeTo(1000);
			for (int i = 0; i < 20; i++)
			{
				VInt3 target = this.m_wrapper.actor.location + vInt * (int)(FrameRandom.Random(5u) + 6);
		...
		public Vector3 GetCmdDest()
		{
			...
			{
				return ((FrameCommand<MoveToPosCommand>)this.m_wrapper.curMoveCommand).cmdData.destPosition.get_vec3();
			}
			if (this.m_wrapper.curMoveCommand.cmdType == 4)
			{
				return ((FrameCommand<AttackPositionCommand>)this.m_wrapper.curMoveCommand).cmdData.WorldPos.get_vec3();
			}
			return Vector3.zero;
		}
		...
		public bool IsDistanceToPosMoreThanRange(Vector3 aimPos, int range)
		{
			VInt3 vInt = new VInt3(aimPos);
			long num = (long)range;
			return (this.m_wrapper.actorLocation - vInt).get_sqrMagnitudeLong2D() > num * num;
		}
		...
		public bool IsDistanceToActorMoreThanRange(uint objID, int range)
		{
			...
			return (this.m_wrapper.actorLocation - handle.location).get_sqrMagnitudeLong2D() > num * num;
		}
		...
		{
			return this.m_wrapper.GetRouteCurWaypointPos().get_vec3();
		}

		[MethodMetaInfo("获取路径点中的当前点的位置,用于沿路径点返回", "前提是已设定好路径")]
		public Vector3 GetRouteCurWaypointPosPre()
		{
			return this.m_wrapper.GetRouteCurWaypointPosPre().get_vec3();
		}
		...
		public bool SelectRoute()
		{
			...
			if (Singleton<BattleLogic>.GetInstance().mapLogic == null)
			{
				...
				{
					Singleton<GameStateCtrl>.get_instance().currentStateName
				});
				return false;
			}
			...
			if (waypointsList == null || waypointsList.get_Count() == 0)
			{
				return false;
			}
			int num = (int)FrameRandom.Random(10000u);
			num %= waypointsList.get_Count();
			if (waypointsList.get_Item(num) == null)
			{
				DebugHelper.Assert(false, "routeList[index]为空");
				return false;
			}
			this.m_wrapper.m_curWaypointsHolder = waypointsList.get_Item(num);
		...
		public bool SelectRouteBySelfIndex()
		{
			...
			for (int i = 0; i < waypointsList.get_Count(); i++)
			{
				if (waypointsList.get_Item(i) == null)
				{
					return false;
				}
				if (waypointsList.get_Item(i).m_index == heroTeamPosIndex)
				{
					this.m_wrapper.m_curWaypointsHolder = waypointsList.get_Item(i);
					this.m_wrapper.m_curWaypointTarget = this.m_wrapper.m_curWaypointsHolder.startPoint;
					this.m_wrapper.m_curWaypointTargetPosition = new VInt3(this.m_wrapper.m_curWaypointTarget.transform.position);
					return true;
				}
			}
			if (waypointsList.get_Count() > heroTeamPosIndex)
			{
				this.m_wrapper.m_curWaypointsHolder = waypointsList.get_Item(heroTeamPosIndex);
				this.m_wrapper.m_curWaypointTarget = this.m_wrapper.m_curWaypointsHolder.startPoint;
				this.m_wrapper.m_curWaypointTargetPosition = new VInt3(this.m_wrapper.m_curWaypointTarget.transform.position);
				return true;
			}
			if (waypointsList.get_Count() > 0)
			{
				this.m_wrapper.m_curWaypointsHolder = waypointsList.get_Item(0);
				this.m_wrapper.m_curWaypointTarget = this.m_wrapper.m_curWaypointsHolder.startPoint;
				this.m_wrapper.m_curWaypointTargetPosition = new VInt3(this.m_wrapper.m_curWaypointTarget.transform.position);
				return true;
			}
		...
		public EBTStatus SelectRouteBySelfCampIndex()
		{
			...
			for (int i = 0; i < waypointsList.get_Count(); i++)
			{
				if (!(waypointsList.get_Item(i) == null))
				{
					if (waypointsList.get_Item(i).m_index == num)
					{
						this.m_wrapper.m_curWaypointsHolder = waypointsList.get_Item(i);
						this.m_wrapper.m_curWaypointTarget = this.m_wrapper.m_curWaypointsHolder.startPoint;
						this.m_wrapper.m_curWaypointTargetPosition = new VInt3(this.m_wrapper.m_curWaypointTarget.transform.position);
		...
		public EBTStatus SelectNearestRoute()
		{
			...
			for (int i = 0; i < waypointsList.get_Count(); i++)
			{
				VInt3 vInt = new VInt3(waypointsList.get_Item(i).startPoint.transform.position);
				long sqrMagnitudeLong2D = (this.m_wrapper.actorLocation - vInt).get_sqrMagnitudeLong2D();
				if (sqrMagnitudeLong2D < num)
				{
					waypointsHolder = waypointsList.get_Item(i);
					num = sqrMagnitudeLong2D;
				}
			}
		...
		public Vector3 GetRandomPointAroundGivenPoint(Vector3 aimPos, int range)
		{
			...
			if (PathfindingUtility.IsValidTarget(this.m_wrapper.actor, new VInt3(vector)))
			{
				return vector;
			}
			return aimPos;
		...
		{
			Vector3 vector = aimPos - this.m_wrapper.actor.location.get_vec3();
			Vector3 aimPos2 = this.m_wrapper.actor.location.get_vec3() + vector.normalized * ((float)distance * 0.001f);
		...
		{
			Vector3 vector = aimPos - this.m_wrapper.actor.location.get_vec3();
			Vector3 result = this.m_wrapper.actor.location.get_vec3() + vector.normalized * ((float)distance * 0.001f);
			...
			Vector3 vector2 = new Vector3(result.x + (float)num2 * 0.001f, result.y, result.z + (float)num3 * 0.001f);
			if (PathfindingUtility.IsValidTarget(this.m_wrapper.actor, new VInt3(vector2)))
			{
				return vector2;
			}
			return result;
		...
		public SkillTargetRule GetSkillTargetRule(SkillSlotType InSlot)
		{
			...
			{
				return skill.cfgData.bSkillTargetRule;
			}
			return 1;
		}
		...
		public RES_SUMMONERSKILL_TYPE GetSummonerSkillType(SkillSlotType InSlot)
		{
			...
			if (skill != null && skill.cfgData != null)
			{
				return skill.cfgData.bSkillType;
			}
			return 2;
		}
		...
		public EBTStatus CheckSkillFilter(SkillSlotType InSlot, uint objID)
		{
			...
			PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.GetInstance().GetActor(objID);
			if (((ulong)dwSkillTargetFilter & (ulong)(1L << (int)(actor.get_handle().TheActorMeta.ActorType & (ActorTypeDef)31))) > 0uL)
			{
				return EBTStatus.BT_FAILURE;
			}
			return EBTStatus.BT_SUCCESS;
		...
		public bool CheckUseSkillPosition()
		{
			...
			if (skill != null)
			{
				...
				case 1:
				{
					if (!curSkillUseInfo.TargetActor || curSkillUseInfo.TargetActor.get_handle().ActorControl.IsDeadState)
					{
						return false;
					}
					long num = (long)(skill.cfgData.iMaxAttackDistance * skill.cfgData.iMaxAttackDistance);
					long num2 = (this.m_wrapper.actorLocation - curSkillUseInfo.TargetActor.get_handle().location).get_sqrMagnitudeLong2D() - num;
					return (double)num2 <= (double)num * 0.09;
				}
				case 2:
				{
					long num = (long)(skill.cfgData.iMaxAttackDistance * skill.cfgData.iMaxAttackDistance);
					long num2 = (this.m_wrapper.actorLocation - curSkillUseInfo.UseVector).get_sqrMagnitudeLong2D() - num;
					return (double)num2 <= (double)num * 0.09;
				}
				case 3:
					return true;
		...
		public virtual bool IsSkillMoveToTarget()
		{
			...
			if (nextSkill != null)
			{
				...
				case 1:
				{
					...
					num += (long)curSkillUseInfo.TargetActor.get_handle().shape.AvgCollisionRadius;
					num *= num;
					return (this.m_wrapper.actorLocation - curSkillUseInfo.TargetActor.get_handle().location).get_sqrMagnitudeLong2D() > num;
				}
				case 2:
				{
					long num2 = nextSkill.cfgData.iMaxAttackDistance * 11000L / 10000L;
					long num = num2 * num2;
					long num3 = (this.m_wrapper.actorLocation - curSkillUseInfo.UseVector).get_sqrMagnitudeLong2D() - num;
					return num3 > 0L;
				}
				case 3:
					return false;
		...
		public virtual EBTStatus MoveToSkillTarget()
		{
			...
			if (nextSkill != null)
			{
				...
				case 1:
				{
					...
					num += (long)curSkillUseInfo.TargetActor.get_handle().shape.AvgCollisionRadius;
					num *= num;
					if ((this.m_wrapper.actorLocation - curSkillUseInfo.TargetActor.get_handle().location).get_sqrMagnitudeLong2D() > num)
					{
						this.m_wrapper.RealMovePosition(curSkillUseInfo.TargetActor.get_handle().location, 0u);
						return EBTStatus.BT_RUNNING;
					}
					this.m_closeToTargetFrame = 0;
					return EBTStatus.BT_SUCCESS;
				...
				case 2:
				{
					...
					if ((this.m_wrapper.actorLocation - curSkillUseInfo.UseVector).get_sqrMagnitudeLong2D() > num)
					{
		...
		public virtual bool IsAttackMoveToTarget()
		{
			...
			if (nextSkill != null)
			{
				...
				if (appointType == 1)
				{
					if (!curSkillUseInfo.TargetActor || curSkillUseInfo.TargetActor.get_handle().ActorAgent == null || curSkillUseInfo.TargetActor.get_handle().ActorControl.IsDeadState)
					{
						return false;
					}
					DebugHelper.Assert(nextSkill.cfgData != null, "skillObj.cfgData != null");
					DebugHelper.Assert(curSkillUseInfo.TargetActor.get_handle().shape != null, "skillContext.TargetActor.handle.shape!=null");
					if (nextSkill.cfgData == null || curSkillUseInfo.TargetActor.get_handle().shape == null)
					{
						return false;
					}
					long num = nextSkill.cfgData.iMaxAttackDistance;
		...
		public virtual EBTStatus MoveToCommonAttackTargetWithRange(int range)
		{
			...
			if (nextSkill != null)
			{
				...
				if (appointType == 1)
				{
					...
					if (!curSkillUseInfo.TargetActor || curSkillUseInfo.TargetActor.get_handle().ActorControl.IsDeadState || !curSkillUseInfo.TargetActor.get_handle().HorizonMarker.IsVisibleFor(this.m_wrapper.actor.TheActorMeta.ActorCamp))
					{
						this.m_wrapper.TerminateMove();
					...
					{
						if (heroWrapper.CurOpMode == OperateMode.LockMode || ownerPlayer.useLastHitMode == LastHitMode.LastHit)
						{
					...
					if ((currentAttackMode == null || currentAttackMode.GetEnemyHeroAttackTargetID() <= 0u) && (this.m_wrapper.actorLocation - curSkillUseInfo.TargetActor.get_handle().location).get_sqrMagnitudeLong2D() > num)
					{
						this.m_wrapper.TerminateMove();
						this.m_wrapper.ClearTarget();
						this.m_wrapper.actor.SkillControl.SkillUseCache.SetMoveToAttackTarget(false);
					...
					num2 += (long)curSkillUseInfo.TargetActor.get_handle().shape.AvgCollisionRadius;
					num2 *= num2;
					if ((this.m_wrapper.actorLocation - curSkillUseInfo.TargetActor.get_handle().location).get_sqrMagnitudeLong2D() > num2)
					{
						this.m_wrapper.RealMovePosition(curSkillUseInfo.TargetActor.get_handle().location, 0u);
		...
		public uint NormalAttackSearchEnemy(int srchR)
		{
			...
			if (ownerPlayer != null && this.m_wrapper.myTarget && ownerPlayer.bCommonAttackLockMode)
			{
				...
				{
					return this.m_wrapper.myTarget.get_handle().ObjID;
				}
				this.m_wrapper.ClearTarget();
			}
		...
		public bool IsTargetCanBeAttacked(uint objID)
		{
			...
			bool flag = this.m_wrapper.CanAttack(actor) && actor.get_handle().HorizonMarker.IsVisibleFor(this.m_wrapper.actor.TheActorMeta.ActorCamp);
			if (!flag)
			{
				this.m_wrapper.ClearTarget();
			}
		...
		public EBTStatus IsVisible(uint objID)
		{
			...
			if (actor.get_handle().HorizonMarker.IsVisibleFor(this.m_wrapper.actor.TheActorMeta.ActorCamp))
			{
				return EBTStatus.BT_SUCCESS;
			}
			return EBTStatus.BT_FAILURE;
		...
		{
			return this.m_wrapper.IsNeedToHelpOther && this.m_wrapper.m_needToHelpTarget && this.m_wrapper.m_needToHelpTarget.get_handle().ObjID != 0u;
		}
		...
		public uint GetNeedHelpTarget()
		{
			...
			{
				return this.m_wrapper.m_needToHelpTarget.get_handle().ObjID;
			}
			return 0u;
		}
		...
		{
			return Singleton<FrameSynchr>.get_instance().LogicFrameTick;
		}
		...
		public void NotifyTeamSelectRoute(int index, int range, uint centerObjId)
		{
			...
			for (int i = 0; i < waypointsList.get_Count(); i++)
			{
				if (!(waypointsList.get_Item(i) == null))
				{
					if (waypointsList.get_Item(i).m_index == index)
					{
						long num = (long)range;
						num *= num;
						PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.get_instance().GetActor(centerObjId);
						for (int j = 0; j < heroActors.get_Count(); j++)
						{
							PoolObjHandle<ActorRoot> poolObjHandle = heroActors.get_Item(j);
							if (poolObjHandle.get_handle().TheActorMeta.ActorCamp == this.m_wrapper.actor.TheActorMeta.ActorCamp)
							{
								long sqrMagnitudeLong2D = (poolObjHandle.get_handle().location - actor.get_handle().location).get_sqrMagnitudeLong2D();
								if (sqrMagnitudeLong2D < num)
								{
									poolObjHandle.get_handle().ActorControl.m_curWaypointsHolder = waypointsList.get_Item(i);
									poolObjHandle.get_handle().ActorControl.m_curWaypointTarget = poolObjHandle.get_handle().ActorControl.m_curWaypointsHolder.startPoint;
									poolObjHandle.get_handle().ActorControl.m_curWaypointTargetPosition = new VInt3(poolObjHandle.get_handle().ActorControl.m_curWaypointTarget.transform.position);
								}
							}
						}
						break;
		...
		public void NotifyTeamSelectRandomRoute(int range, uint centerObjId)
		{
			...
			for (int i = 0; i < heroActors.get_Count(); i++)
			{
				...
				if (poolObjHandle.get_handle().TheActorMeta.ActorCamp == this.m_wrapper.actor.TheActorMeta.ActorCamp)
				{
					...
					if (sqrMagnitudeLong2D < num)
					{
						poolObjHandle.get_handle().ActorAgent.SelectRoute();
					}
				}
			}
		}
		...
		{
			int count = Singleton<GameObjMgr>.get_instance().GameActors.get_Count();
			for (int i = 0; i < count; i++)
			{
				PoolObjHandle<ActorRoot> poolObjHandle = Singleton<GameObjMgr>.get_instance().GameActors.get_Item(i);
				if (poolObjHandle && !poolObjHandle.get_handle().ActorControl.IsDeadState)
				{
					...
					{
						return poolObjHandle.get_handle().ObjID;
					}
				}
			}
			return 0u;
		...
		{
			PoolObjHandle<ActorRoot> commanderHeroTarget = Singleton<TargetSearcher>.get_instance().GetCommanderHeroTarget(this.m_wrapper.actor.TheActorMeta.ActorCamp, range);
			if (commanderHeroTarget)
			{
				return commanderHeroTarget.get_handle().ObjID;
			}
			return 0u;
		}
		...
		{
			return Singleton<TargetSearcher>.get_instance().GetAliveHeroNum(this.m_wrapper.actor.TheActorMeta.ActorCamp);
		}

		[MethodMetaInfo("获取敌方活着的英雄数量", "获取敌方活着的英雄数量")]
		public int GetEnemyCampAliveHerpNum()
		{
			COM_PLAYERCAMP camp = 0;
			if (this.m_wrapper.actor.TheActorMeta.ActorCamp == 1)
			{
				camp = 2;
			}
			else if (this.m_wrapper.actor.TheActorMeta.ActorCamp == 2)
			{
				camp = 1;
			}
			return Singleton<TargetSearcher>.get_instance().GetAliveHeroNum(camp);
		}
		...
		public int GetHeroNumInRange(uint objID, int range, COM_PLAYERCAMP camp)
		{
			...
			if (actor)
			{
				return Singleton<TargetSearcher>.get_instance().GetHeroNumInRange(actor.get_handle(), range, camp);
			}
			return 0;
		}
		...
		{
			PoolObjHandle<ActorRoot> commanderTowerTarget = Singleton<TargetSearcher>.get_instance().GetCommanderTowerTarget(this.m_wrapper.actor.TheActorMeta.ActorCamp, friendrange, enemyRange);
			if (commanderTowerTarget)
			{
				return commanderTowerTarget.get_handle().ObjID;
			}
			return 0u;
		}
		...
		public int GerPathIndexBuTowerId(uint TowerID)
		{
			...
			for (int i = 0; i < towerActors.get_Count(); i++)
			{
				...
				if (poolObjHandle.get_handle().ObjID == TowerID)
				{
					return poolObjHandle.get_handle().ObjLinker.PathIndex;
				}
			}
			return 0;
		}
		...
		public override void UpdateLogic(int delta)
		{
			...
			this.m_frame = Singleton<FrameSynchr>.get_instance().CurFrameNum;
			if (this.m_wrapper != null)
			{
				...
				else
				{
					...
					else if ((this.m_frame + this.m_wrapper.actor.ObjID) % (uint)(6 * num) == 0u)
					{
						base.UpdateLogic(delta);
					}
				}
		...
		{
			if (Singleton<FrameSynchr>.get_instance().bActive)
			{
				this.m_dengerCoolTick = 30;
			}
			else
