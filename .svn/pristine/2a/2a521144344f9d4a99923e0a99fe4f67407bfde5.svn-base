...
namespace Assets.Scripts.GameSystem
{
	...
	public class SettlementSystem : Singleton<SettlementSystem>
	{
		...
		private COM_PLAYERCAMP _myCamp = 1;
		...
		private void OnClickBattleAgain(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				if (Singleton<CMatchingSystem>.get_instance().cacheMathingInfo == null || Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.uiEventId == enUIEventID.None)
				{
					return;
				}
				if (Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.uiEventId == enUIEventID.Room_CreateRoom)
				{
					CRoomSystem.ReqCreateRoom(Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.mapId, Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.mapType, false);
				}
				else
				{
					CUIEvent cUIEvent2 = new CUIEvent();
					cUIEvent2.m_eventID = Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.uiEventId;
					cUIEvent2.m_eventParams.tagUInt = Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.mapId;
					cUIEvent2.m_eventParams.tag = Singleton<CMatchingSystem>.get_instance().cacheMathingInfo.AILevel;
		...
		{
			Singleton<CUIManager>.get_instance().OpenForm(this._achievementsTips, false, true);
		}
		...
		private void OnClickDianZanLaHei(CUIEvent uiEvent)
		{
			...
			{
				Singleton<CUIManager>.get_instance().OpenTips(string.Format(Singleton<CTextManager>.GetInstance().GetText("ZanTeam"), text), false, 1.5f, null, new object[0]);
			}
			else
			{
				Singleton<CUIManager>.get_instance().OpenTips(string.Format(Singleton<CTextManager>.GetInstance().GetText("ZanEnemyTeam"), text), false, 1.5f, null, new object[0]);
			}
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(1333u);
			cSPkg.stPkgData.get_stLikeReq().stAcntUin.ullUid = commonUInt64Param;
			cSPkg.stPkgData.get_stLikeReq().stAcntUin.dwLogicWorldId = (uint)num;
		...
		{
			Singleton<CChatController>.get_instance().model.channelMgr.Clear(EChatChannel.Settle, 0uL, 0u);
			...
			Singleton<CRecordUseSDK>.get_instance().CallGameJoyGenerateWithNothing();
		}
		...
		private void OnOpenPlayerDetailInfo(CUIEvent uiEvent)
		{
			...
			if (Utility.IsValidPlayer(commonUInt64Param, tag) && !Utility.IsSelf(commonUInt64Param, tag))
			{
				Singleton<CPlayerInfoSystem>.GetInstance().ShowPlayerDetailInfo(commonUInt64Param, tag, CPlayerInfoSystem.DetailPlayerInfoSource.DefaultOthers, true);
			}
		}
		...
		private void SetSpecialItem()
		{
			...
			if (rewards.bNum > 0)
			{
				...
				{
					array[num] = useableListFromReward.get_Item(i);
					num++;
				}
			}
			if (specialItemInfo.bOutputCnt > 0)
			{
				...
				if (pvpSpecialItemDict != null)
				{
					for (int j = 0; j < (int)specialItemInfo.bOutputCnt; j++)
					{
						...
						if (pvpSpecialItemDict.TryGetValue(specialItemInfo.astOutputInfo[j].dwPVPSpecItemID, ref resPVPSpecItem))
						{
							array[num] = CUseableManager.CreateUsableByServerType(resPVPSpecItem.wItemType, (int)specialItemInfo.astOutputInfo[j].dwPVPSpecItemCnt, resPVPSpecItem.dwItemID);
						}
						num++;
					}
				}
		...
		private void SetExpProfit()
		{
			...
			if (!CSysDynamicBlock.bSocialBlocked)
			{
				...
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component7, (int)masterRoleInfo.GetNobeInfo().stGameVipClient.dwCurLevel, false);
				MonoSingleton<NobeSys>.GetInstance().SetHeadIconBk(component8, (int)masterRoleInfo.GetNobeInfo().stGameVipClient.dwHeadIconId);
			}
			else
			{
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component7, 0, false);
			}
			...
			SettlementSystem.StrHelper[0] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_1", new string[]
			{
				text
			});
			if (array != null)
			{
				...
				while ((long)num4 < (long)((ulong)multipleInfo))
				{
					...
					if (num6 >= 0)
					{
						string[] expr_670_cp_0 = SettlementSystem.StrHelper2;
						int expr_670_cp_1 = num4 + 1;
						expr_670_cp_0[expr_670_cp_1] += "+";
					}
					string[] expr_68B_cp_0 = SettlementSystem.StrHelper2;
					int expr_68B_cp_1 = num4 + 1;
					expr_68B_cp_0[expr_68B_cp_1] += num6;
					switch (array[num4].iType)
					{
					case 1:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_6", new string[]
						{
							text2
						});
						break;
					case 2:
						...
						{
							SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_9", new string[]
							{
								text2
							});
						}
						...
						{
							SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_3", new string[]
							{
								text2
							});
						}
						break;
					case 3:
						SettlementSystem.StrHelper[num4 + 1] = string.Format(Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_4"), text2, masterRoleInfo.GetExpWinCount(), Math.Ceiling((double)((float)masterRoleInfo.GetExpExpireHours() / 24f)));
						break;
					case 4:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_2", new string[]
						{
						...
						break;
					case 5:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_13", new string[]
						{
							text2
						});
						break;
					case 6:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_15", new string[]
						{
							text2
						});
						break;
					case 7:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Daily_Quest_FirstVictoryName", new string[]
						{
							text2
						});
						break;
					case 8:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_16", new string[]
						{
							text2
						});
						break;
					case 9:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_14", new string[]
						{
							text2
						});
						break;
					case 10:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_21", new string[]
						{
							text2
						});
						break;
					case 11:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_17", new string[]
						{
							text2
						});
						break;
					case 12:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_18", new string[]
						{
							text2
						});
						break;
					case 13:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_20", new string[]
						{
							text2
						});
						break;
					case 14:
						SettlementSystem.StrHelper[num4 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_19", new string[]
						{
							text2
						});
						break;
		...
		private void SetGoldCoinProfit()
		{
			...
			text += multiple.ToString();
			SettlementSystem.StrHelper[0] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_7", new string[]
			{
				text
			});
			if (array != null)
			{
				...
				while ((long)num2 < (long)((ulong)multipleInfo))
				{
					...
					{
					case 1:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_6", new string[]
						{
							text2
						});
						break;
					case 2:
					{
						if (masterRoleInfo.HasVip(16))
						{
							SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_9", new string[]
							{
								text2
							});
						}
						...
						{
							SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_3", new string[]
							{
								text2
							});
						}
					...
					case 3:
						SettlementSystem.StrHelper[num2 + 1] = string.Format(Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_10"), text2, masterRoleInfo.GetCoinWinCount(), Math.Ceiling((double)((float)masterRoleInfo.GetCoinExpireHours() / 24f)));
						break;
					case 4:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_2", new string[]
						{
							masterRoleInfo.dailyPvpCnt.ToString(),
							text2
						});
					...
					case 5:
					{
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_13", new string[]
						{
							text2
						});
						Transform transform3 = gameObject.transform.FindChild("goldTotalGroup/goldGroup/WXIcon");
						MonoSingleton<NobeSys>.GetInstance().SetGameCenterVisible(transform3.gameObject, masterRoleInfo.m_privilegeType, 1, false, CSysDynamicBlock.bLobbyEntryBlocked);
						transform3.FindChild("Text").GetComponent<Text>().text = string.Format("{0}", text2);
						break;
					}
					case 6:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Guild_Settlement_Guild_Coin_Plus_Tip2", new string[]
						{
							text2
						});
						break;
					case 7:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Daily_Quest_FirstVictoryName", new string[]
						{
							text2
						});
						break;
					case 8:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_16", new string[]
						{
							text2
						});
						break;
					case 9:
					{
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_14", new string[]
						{
							text2
						});
						...
						if (!flag)
						{
							MonoSingleton<NobeSys>.GetInstance().SetGameCenterVisible(transform4.gameObject, masterRoleInfo.m_privilegeType, 2, false, CSysDynamicBlock.bLobbyEntryBlocked);
					...
					case 10:
					{
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_21", new string[]
						{
							text2
						});
						...
						if (!flag && !flag2)
						{
							MonoSingleton<NobeSys>.GetInstance().SetGameCenterVisible(transform5.gameObject, masterRoleInfo.m_privilegeType, 5, false, CSysDynamicBlock.bLobbyEntryBlocked);
					...
					case 11:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_17", new string[]
						{
							text2
						});
						break;
					case 12:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_18", new string[]
						{
							text2
						});
						break;
					case 13:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_20", new string[]
						{
							text2
						});
						break;
					case 14:
						SettlementSystem.StrHelper[num2 + 1] = Singleton<CTextManager>.get_instance().GetText("Pvp_settle_Common_Tips_19", new string[]
						{
							text2
						});
						break;
		...
		private void SetMapInfo()
		{
			...
			string text = Singleton<CTextManager>.get_instance().GetText("Battle_Settle_Game_Type_Single");
			string text2 = string.Empty;
			...
			if (curLvelContext.IsMobaMode())
			{
				...
				{
					text = Singleton<CTextManager>.get_instance().GetText(string.Format("Battle_Settle_Game_Type{0}", curLvelContext.m_pvpPlayerNum / 2));
				}
			}
			component.text = text;
		...
		private void SetVictoryTipsBtnInfo(CUIFormScript form)
		{
			...
			if (form != null)
			{
				...
				if (CBattleGuideManager.EnableHeroVictoryTips() && !CSysDynamicBlock.bLobbyEntryBlocked)
				{
					...
					if (!this._win && !MonoSingleton<NewbieGuideManager>.get_instance().isNewbieGuiding)
					{
						int num = PlayerPrefs.GetInt(SettlementSystem.PlayerWinTimesStr, 0);
						num++;
						uint globeValue = GameDataMgr.GetGlobeValue(210);
						if ((long)num >= (long)((ulong)globeValue))
						{
						...
						{
							PlayerPrefs.SetInt(SettlementSystem.PlayerWinTimesStr, num);
		...
		private void SetGuildInfo()
		{
			...
			if (num > 0u)
			{
				...
				component.text = num.ToString(CultureInfo.get_InvariantCulture());
			}
			else
			{
		...
		private void SetLadderInfo()
		{
			...
			if (transform != null)
			{
				...
				component.text = CLadderView.GetRankName(rankInfo.bNowGrade, Singleton<CRoleInfoManager>.GetInstance().GetSelfRankClass());
			}
			...
			if ((int)rankInfo.bNowGrade == GameDataMgr.rankGradeDatabin.count)
			{
			...
			if (!neutralShow)
			{
				...
				if (curLvelContext.IsRedBagMode())
				{
					...
					if (hostKDA != null)
					{
						...
						uint mvpPlayer = Singleton<BattleStatistic>.get_instance().GetMvpPlayer(hostKDA.PlayerCamp, this._win);
		...
		private void SetSettlementButton()
		{
			...
			gameObject.transform.FindChild("BtnSaveReplay").gameObject.CustomSetActive(Singleton<GameReplayModule>.GetInstance().HasRecord);
		...
		private void SetPlayerSettlement()
		{
			...
			int count = Singleton<GamePlayerCenter>.GetInstance().GetAllCampPlayers(1).get_Count();
			int count2 = Singleton<GamePlayerCenter>.GetInstance().GetAllCampPlayers(2).get_Count();
			...
			gameObject.transform.FindChild("LeftTotalKill").gameObject.GetComponent<Text>().text = this._camp1TotalKill.ToString(CultureInfo.get_InvariantCulture());
			gameObject.transform.FindChild("RightTotalKill").gameObject.GetComponent<Text>().text = this._camp2TotalKill.ToString(CultureInfo.get_InvariantCulture());
			...
			{
				KeyValuePair<uint, PlayerKDA> current2 = enumerator2.get_Current();
		...
		private void SetCreditSettlement()
		{
			...
			{
				componetInChild.text = Singleton<CTextManager>.get_instance().GetText("Credit_Change_Tips_1", new string[]
				{
			...
			{
				componetInChild.text = Singleton<CTextManager>.get_instance().GetText("Credit_Change_Tips_2", new string[]
				{
					masterRoleInfo.creditScore.ToString(),
					acntInfo.iSettleCreditValue.ToString()
				});
		...
		private void UpdatePlayerKda(PlayerKDA kda)
		{
			...
			bool flag = kda.PlayerCamp == Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().PlayerCamp;
			bool flag2 = (this._win && flag) || (!this._win && !flag);
			float num = 0f;
			if (!Singleton<BattleStatistic>.get_instance().GetServerMvpScore(kda.PlayerId, out num))
			{
			...
			if (this.playerNum > 2)
			{
				if (kda.PlayerId == Singleton<BattleStatistic>.get_instance().GetMvpPlayer(kda.PlayerCamp, flag2))
				{
					if (flag2)
					{
			...
			if (kda.PlayerId == Singleton<GamePlayerCenter>.GetInstance().HostPlayerId || Singleton<CFriendContoller>.get_instance().model.IsGameFriend(kda.PlayerUid, (uint)kda.WorldId) || (kda.IsComputer && !curLvelContext.m_isWarmBattle))
			{
			...
			if (kda.PlayerId != Singleton<GamePlayerCenter>.GetInstance().HostPlayerId && (!kda.IsComputer || curLvelContext.m_isWarmBattle) && this.playerNum >= 6 && !curLvelContext.IsGameTypePvpRoom())
			{
				...
				component.DianZanLaHei.GetComponent<CUIEventScript>().m_onClickEventParams.commonUInt32Param1 = kda.PlayerCamp;
				component.DianZanLaHei.GetComponent<CUIEventScript>().m_onClickEventParams.commonUInt16Param1 = Singleton<GamePlayerCenter>.GetInstance().hostPlayerCamp;
			}
			else
			{
			...
			while (enumerator.MoveNext())
			{
				...
				if (current != null)
				{
					...
					component.Kill.GetComponent<Text>().text = current.numKill.ToString(CultureInfo.get_InvariantCulture());
					component.Death.GetComponent<Text>().text = current.numDead.ToString(CultureInfo.get_InvariantCulture());
					component.Assist.GetComponent<Text>().text = current.numAssist.ToString(CultureInfo.get_InvariantCulture());
					component.Coin.GetComponent<Text>().text = current.TotalCoin.ToString(CultureInfo.get_InvariantCulture());
					uint num6;
					uint num7;
					uint num8;
					if (kda.PlayerCamp == 1)
					...
					component.Damage.transform.FindChild("TotalDamageBg/TotalDamage").gameObject.GetComponent<Text>().text = current.hurtToEnemy.ToString(CultureInfo.get_InvariantCulture());
					...
					component.Damage.transform.FindChild("TotalTakenDamageBg/TotalTakenDamage").gameObject.GetComponent<Text>().text = current.hurtTakenByEnemy.ToString(CultureInfo.get_InvariantCulture());
					...
					component.Damage.transform.FindChild("TotalDamageHeroBg/TotalDamageHero").gameObject.GetComponent<Text>().text = current.hurtToHero.ToString(CultureInfo.get_InvariantCulture());
		...
		public int GetMvpScoreRankInCamp()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, PlayerKDA> current = enumerator.get_Current();
				PlayerKDA value = current.get_Value();
				if (value != null && value.PlayerId == Singleton<GamePlayerCenter>.get_instance().HostPlayerId)
				{
					int serverRawMvpScore = Singleton<BattleStatistic>.get_instance().GetServerRawMvpScore(value.PlayerId);
			...
			while (enumerator.MoveNext())
			{
				...
				if (value2 != null)
				{
					int serverRawMvpScore2 = Singleton<BattleStatistic>.get_instance().GetServerRawMvpScore(value2.PlayerId);
					if (value2.PlayerCamp == Singleton<GamePlayerCenter>.get_instance().hostPlayerCamp && value2.PlayerId != Singleton<GamePlayerCenter>.get_instance().HostPlayerId && serverRawMvpScore2 > num)
					{
						num2++;
					}
				}
		...
		private int GetHostPlayerHeroId(DictionaryView<uint, PlayerKDA>.Enumerator playerKdaEmr)
		{
			...
			{
				KeyValuePair<uint, PlayerKDA> current = playerKdaEmr.get_Current();
				ulong arg_2D_0 = current.get_Value().PlayerUid;
				KeyValuePair<uint, PlayerKDA> current2 = playerKdaEmr.get_Current();
				if (Utility.IsSelf(arg_2D_0, current2.get_Value().WorldId))
				{
					KeyValuePair<uint, PlayerKDA> current3 = playerKdaEmr.get_Current();
					ListView<HeroKDA>.Enumerator enumerator = current3.get_Value().GetEnumerator();
					if (enumerator.MoveNext() && enumerator.get_Current() != null)
					{
						return enumerator.get_Current().HeroId;
					}
				}
			}
			return -1;
		...
		{
			bool result = false;
			if (Singleton<CUIManager>.get_instance().GetForm(FightForm.s_battleUIForm) != null || Singleton<CUIManager>.get_instance().GetForm(WinLose.m_FormPath) != null || Singleton<CUIManager>.get_instance().GetForm(SettlementSystem.SettlementFormName) != null || Singleton<CUIManager>.get_instance().GetForm(this._profitFormName) != null || Singleton<CUIManager>.get_instance().GetForm(SingleGameSettleMgr.PATH_BURNING_WINLOSE) != null || Singleton<CUIManager>.get_instance().GetForm(SingleGameSettleMgr.PATH_BURNING_SETTLE) != null || Singleton<CUIManager>.get_instance().GetForm(PVESettleSys.PATH_STAR) != null || Singleton<CUIManager>.get_instance().GetForm(PVESettleSys.PATH_EXP) != null || Singleton<CUIManager>.get_instance().GetForm(PVESettleSys.PATH_ITEM) != null || Singleton<CUIManager>.get_instance().GetForm(PVESettleSys.PATH_LOSE) != null || Singleton<CUIManager>.get_instance().GetForm(PVESettleSys.PATH_LEVELUP) != null)
			{
				result = true;
			}
			return result;
		...
		private void ImpSettlementTimerEnd()
		{
			...
			{
				Singleton<CChatController>.get_instance().model.channelMgr.Clear(EChatChannel.Settle, 0uL, 0u);
				Singleton<CChatController>.get_instance().model.channelMgr.SetChatTab(CChatChannelMgr.EChatTab.Settle);
		...
		protected void ImpShowReport(CUIEvent uiEvent)
		{
			...
			gameObject.transform.FindChild("ReportToggle/ReportName").gameObject.GetComponent<Text>().text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Report_PlayerName"), text);
		}
		...
		protected void ImpDoReport(CUIEvent uiEvent)
		{
			...
			GameObject gameObject = this._settleFormScript.m_formWidgets[3];
			gameObject.CustomSetActive(false);
			Singleton<CUIManager>.get_instance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("Report_Report"), false, 1.5f, null, new object[0]);
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(4305u);
			cSPkg.stPkgData.get_stUserComplaintReq().dwComplaintReason = 1u;
			cSPkg.stPkgData.get_stUserComplaintReq().ullComplaintUserUid = this._reportUid;
			cSPkg.stPkgData.get_stUserComplaintReq().iComplaintLogicWorldID = this._reportWordId;
			CPlayerKDAStat playerKDAStat = Singleton<BattleLogic>.GetInstance().battleStat.m_playerKDAStat;
			DictionaryView<uint, PlayerKDA>.Enumerator enumerator = playerKDAStat.GetEnumerator();
			int num = 0;
			while (enumerator.MoveNext())
			{
				...
				if (value != null && value.PlayerUid == this._reportUid && value.WorldId == this._reportWordId)
				{
					...
					{
						Utility.StringToByteArray(value.PlayerOpenId, ref cSPkg.stPkgData.get_stUserComplaintReq().szComplaintUserOpenId);
					}
					byte[] array = Utility.BytesConvert(value.PlayerName);
					byte[] szComplaintPlayerName = cSPkg.stPkgData.get_stUserComplaintReq().szComplaintPlayerName;
					...
					cSPkg.stPkgData.get_stUserComplaintReq().iComplaintPlayerCamp = ((value.PlayerCamp != this._myCamp) ? 2 : 1);
				}
				num++;
			}
			...
			{
				cSPkg.stPkgData.get_stUserComplaintReq().dwComplaintReason = 2u;
			}
			else if (gameObject2.transform.FindChild("ReportXiaoJi").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.get_stUserComplaintReq().dwComplaintReason = 3u;
			}
			else if (gameObject2.transform.FindChild("ReportMaRen").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.get_stUserComplaintReq().dwComplaintReason = 4u;
			}
			else if (gameObject2.transform.FindChild("ReportYanYuan").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.get_stUserComplaintReq().dwComplaintReason = 5u;
			}
			else if (gameObject2.transform.FindChild("ReportGua").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.get_stUserComplaintReq().dwComplaintReason = 6u;
			}
			string str = CUIUtility.RemoveEmoji(gameObject2.transform.FindChild("InputField").gameObject.GetComponent<InputField>().text);
			Utility.StringToByteArray(str, ref cSPkg.stPkgData.get_stUserComplaintReq().szComplaintRemark);
			cSPkg.stPkgData.get_stUserComplaintReq().dwClientStartTime = this._startTimeInt;
			cSPkg.stPkgData.get_stUserComplaintReq().iBattlePlayerNumber = num;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
			this._reportUid = 0uL;
			this._reportWordId = 0;
		...
		public void ShowLadderSettleForm(bool win)
		{
			...
			if (rankInfo != null)
			{
				this.SetLadderDisplayOldAndNewGrade((uint)rankInfo.bOldGrade, rankInfo.dwOldScore, (uint)rankInfo.bNowGrade, rankInfo.dwNowScore);
		...
		private void ShowBraveScoreDetailPanel()
		{
			...
			COMDT_RANK_SETTLE_INFO rankInfo = Singleton<BattleStatistic>.get_instance().rankInfo;
			if (rankInfo == null)
			{
			...
			if (num > 0u)
			{
				if (this.IsBraveScoreDeduction())
				{
					...
					component.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Clear_Score_Tip");
					component2.text = "0";
				}
				else
				{
					...
					GameObject widget3 = this._ladderForm.GetWidget(18);
					GameObject widget4 = this._ladderForm.GetWidget(19);
					GameObject widget5 = this._ladderForm.GetWidget(20);
					GameObject widget6 = this._ladderForm.GetWidget(21);
					if (dwGameScore > 0u)
					{
					...
					if (dwConWinScore > 0u)
					{
						...
						{
							widget4.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Liansheng", new string[]
							{
					...
					if (dwMVPScore > 0u)
					{
						...
						widget5.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Xianfeng", new string[]
						{
		...
		private void ShowLadderResultPanel(bool isSettle)
		{
			...
			if (isSettle)
			{
				...
				GameObject widget2 = this._ladderForm.GetWidget(15);
				CLadderView.ShowRankDetail(widget2, (byte)this._oldGrade, selfRankClass, this._oldScore, true, false, false, false, true);
				GameObject widget3 = this._ladderForm.GetWidget(16);
				CLadderView.ShowRankDetail(widget3, (byte)this._newGrade, selfRankClass, this._newScore, true, false, false, false, true);
		...
		private bool IsBraveScoreDeduction()
		{
			...
			return this._oldGrade == this._newGrade && this._oldScore == this._newScore && rankInfo.dwOldAddStarScore != 0u && rankInfo.dwNowAddStarScore == 0u;
		}
		...
		private string GetLadderResultDesc()
		{
			...
			else if (this._newGrade < this._oldGrade)
			{
		...
		private void OnAnimatiorBravePanelShowInEnd(CUIEvent uiEvent)
		{
			...
			if (this.IsBraveScoreExchange() && dwNowAddStarScore > 0u)
			{
			...
			{
				float processCircleFillAmount2 = CLadderView.GetProcessCircleFillAmount((int)dwNowAddStarScore, (int)selfBraveScoreMax);
				...
				component.StartFill(processCircleFillAmount2, (!flag) ? CUIProgressUpdaterScript.enFillDirection.Clockwise : CUIProgressUpdaterScript.enFillDirection.CounterClockwise, -1f);
			}
		...
		private void OnAnimatiorBraveProgressFillFull(CUIEvent uiEvent)
		{
			if (this.IsBraveScoreExchange())
			{
				...
				if (dwNowAddStarScore > 0u)
				{
					...
					float processCircleFillAmount2 = CLadderView.GetProcessCircleFillAmount(0, (int)selfBraveScoreMax);
					component2.m_fillEndEventID = enUIEventID.SettlementSys_Ladder_OnAnimatiorBraveProgressFillEnd;
					component2.StartFill(processCircleFillAmount, CUIProgressUpdaterScript.enFillDirection.Clockwise, processCircleFillAmount2);
				}
				else
				{
					Text component3 = this._ladderForm.GetWidget(13).GetComponent<Text>();
		...
		public void DianXing()
		{
			...
			else if (!this._doWangZheSpecial && this._oldGrade == this._newGrade && (ulong)this._newGrade == (ulong)((long)GameDataMgr.rankGradeDatabin.count) && this._oldScore == this._newScore && this._newScore == 0u)
			{
		...
		public uint NeedChangeGrade()
		{
			...
			if (this._isUp && this.NeedDianXing())
			{
				...
				{
					result = GameDataMgr.rankGradeDatabin.GetDataByKey(this._curGrade + 1u).dwGradeUpNeedScore;
				}
			}
		...
		private void OnShowPVPSettleData(CUIEvent uiEvnet)
		{
			...
			if (curLvelContext != null)
			{
				num = curLvelContext.GetGameType() + 1;
			...
			Singleton<EventRouter>.get_instance().AddEventHandler<Transform>(EventID.SHARE_TIMELINE_SUCC, new Action<Transform>(this.OnShareTimeLineSucc));
		...
		{
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm("UGUI/Form/System/ShareUI/Form_SharePVPLadder.prefab", false, true);
		...
		private void OnShareUpdateGradShareClose(CUIEvent uiEvent)
		{
			...
			{
				Singleton<CUIManager>.get_instance().CloseForm(this.m_UpdateGradeForm);
			}
