...
namespace Assets.Scripts.GameLogic
{
	...
	internal class StarConditionDeathStat : StarCondition
	{
		...
		{
			if (prm.src && prm.src.get_handle().TheActorMeta.ActorType == this.targetType && (this.targetID == 0 || this.targetID == prm.src.get_handle().TheActorMeta.ConfigId) && this.ShouldCare(prm.src.get_handle()))
			{
				this.DealthCount++;
