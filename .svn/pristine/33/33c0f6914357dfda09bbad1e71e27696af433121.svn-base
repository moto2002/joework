...
namespace Assets.Scripts.GameLogic
{
	public class TeleportTargetSearcher : Singleton<TeleportTargetSearcher>
	{
		...
		{
			if (!this.curActorPtr.get_handle().CanTeleport(_actorPtr) || this.curActorPtr == _actorPtr)
			{
				return;
			}
			...
			else if (TypeSearchCondition.Fit(_actorPtr, ActorTypeDef.Actor_Type_EYE, false) && DistanceSearchCondition.Fit(this.searchPosition, _actorPtr, this.searchRadius))
			{
