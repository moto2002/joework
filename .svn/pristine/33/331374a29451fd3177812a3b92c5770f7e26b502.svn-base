...
namespace Pathfinding.Serialization
{
	public class AstarSerializer
	{
		...
		{
			AstarSerializer.<SerializeExtraInfoBytes>c__AnonStorey20 <SerializeExtraInfoBytes>c__AnonStorey = new AstarSerializer.<SerializeExtraInfoBytes>c__AnonStorey20();
			if (!this.settings.nodes)
			{
				return null;
			}
			<SerializeExtraInfoBytes>c__AnonStorey.totCount = 0;
			for (int i = 0; i < this.graphs.Length; i++)
			{
				if (this.graphs[i] != null)
				{
					...
					{
						<SerializeExtraInfoBytes>c__AnonStorey.totCount = Math.Max(node.NodeIndex, <SerializeExtraInfoBytes>c__AnonStorey.totCount);
						if (node.NodeIndex == -1)
						{
			...
			MemoryStream memoryStream = new MemoryStream();
			<SerializeExtraInfoBytes>c__AnonStorey.wr = new BinaryWriter(memoryStream);
			<SerializeExtraInfoBytes>c__AnonStorey.wr.Write(<SerializeExtraInfoBytes>c__AnonStorey.totCount);
			int c = 0;
			for (int j = 0; j < this.graphs.Length; j++)
			{
				if (this.graphs[j] != null)
				{
					this.graphs[j].GetNodes(delegate(GraphNode node)
					{
						...
						<SerializeExtraInfoBytes>c__AnonStorey.wr.Write(node.NodeIndex);
						return true;
					});
				}
			}
			if (c != <SerializeExtraInfoBytes>c__AnonStorey.totCount)
			{
			...
			for (int k = 0; k < this.graphs.Length; k++)
			{
				...
				{
					GraphSerializationContext ctx = new GraphSerializationContext(<SerializeExtraInfoBytes>c__AnonStorey.wr);
					this.graphs[k].SerializeExtraInfo(ctx);
					ctx = new GraphSerializationContext(<SerializeExtraInfoBytes>c__AnonStorey.wr);
			...
			<SerializeExtraInfoBytes>c__AnonStorey.wr.Close();
			return memoryStream.ToArray();
		}
