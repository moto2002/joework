...
namespace Assets.Scripts.GameSystem
{
	public class MultiGainWgt : ActivityWidget
	{
		...
		public MultiGainWgt(GameObject node, ActivityView view) : base(node, view)
		{
			...
			for (int i = 0; i < phaseList.get_Count(); i++)
			{
				...
				if (i > 0)
				{
					...
					gameObject.transform.localPosition = this._elementList.get_Item(i - 1).root.transform.localPosition + new Vector3(0f, -(height + 5f), 0f);
				...
				MultiGainWgt.MultiGainListItem multiGainListItem = new MultiGainWgt.MultiGainListItem(gameObject, (MultiGainPhase)phaseList.get_Item(i));
				this._elementList.Add(multiGainListItem);
			}
			if (this._elementList.get_Count() == 0)
			{
				this._elementTmpl.SetActive(false);
			}
			node.GetComponent<RectTransform>().sizeDelta = new Vector2(node.GetComponent<RectTransform>().sizeDelta.x, (this._elementList.get_Count() <= 0) ? 0f : (height * (float)this._elementList.get_Count() + (float)(this._elementList.get_Count() - 1) * 5f));
		...
		public override void Clear()
		{
			...
			{
				this._elementList.get_Item(i).Clear();
				if (i > 0)
				{
					Object.Destroy(this._elementList.get_Item(i).root);
				}
			}
		...
		private void OnClickGoto(CUIEvent uiEvent)
		{
			for (int i = 0; i < this._elementList.get_Count(); i++)
			{
				MultiGainWgt.MultiGainListItem multiGainListItem = this._elementList.get_Item(i);
