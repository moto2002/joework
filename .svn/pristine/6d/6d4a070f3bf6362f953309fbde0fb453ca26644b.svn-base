...
public class AreaEventTriggerTextBubbleDynamic : AreaEventTrigger
{
	...
	private void SetHudText(string txt)
	{
		...
		{
			PoolObjHandle<ActorRoot> poolObjHandle = spawnedList.get_Item(0);
			if (poolObjHandle && poolObjHandle.get_handle().HudControl != null)
			{
				poolObjHandle.get_handle().HudControl.SetTextHud(txt, this.offset_x, this.offset_y, false);
			}
		}
	}
