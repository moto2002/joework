...
namespace Assets.Scripts.GameSystem
{
	public class DragonIcon
	{
			...
			public void SetData(Vector3 worldpos, int type, uint id, bool b5v5 = false, bool bUseCache = true, bool bRefreshCache = false, bool bRefreshMM = true)
			{
				...
				if (this.mmDragonNode_3dui != null && bRefreshMM)
				{
					if (this.mmDragonNode_ugui != null)
					{
						...
						rectTransform.anchoredPosition = new Vector2(worldpos.x * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Small.x, worldpos.z * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Small.y);
					}
				...
				if (this.bmDragonNode_3dui != null)
				{
					if (this.bmDragonNode_ugui != null)
					{
						...
						rectTransform2.anchoredPosition = new Vector2(worldpos.x * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Big.x, worldpos.z * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Big.y);
					}
					if (bUseCache)
					{
				...
				if (null != this.spawnGroup)
				{
					if (null != this.cdTxtInMini)
					{
						if (this.spawnGroup.IsCountingDown())
						{
							...
							this.cdTxtInMini.text = (this.spawnGroup.GetSpawnTimer() / 1000).ToString();
						}
						else
						{
					...
					if (null != this.cdTxtInBig)
					{
						if (this.spawnGroup.IsCountingDown())
						{
							...
							this.cdTxtInBig.text = (this.spawnGroup.GetSpawnTimer() / 1000).ToString();
						}
						else
						{
		...
		public void Init(GameObject mmNode_ugui, GameObject bmNode_ugui, GameObject mmNode_3dui, GameObject bmNode_3dui, bool b5V5)
		{
			...
			if (spawnGroups != null)
			{
				for (int m = 0; m < spawnGroups.get_Count(); m++)
				{
					SpawnGroup spawnGroup = spawnGroups.get_Item(m);
		...
		private void OnCDTimer(int seq)
		{
			...
			for (int i = 0; i < this.node_ary.get_Count(); i++)
			{
				this.node_ary.get_Item(i).ValidateCD();
			}
		}
