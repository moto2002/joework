...
namespace Pathfinding.RVO
{
	...
	public class RVOSimulator : MonoBehaviour
	{
		...
		public ThreadCount workerThreads = 2;
		...
		private void Awake()
		{
			...
			if (this.simulator == null)
			{
				...
				this.simulator = new Simulator(num, this.doubleBuffering);
				this.simulator.set_Interpolation(this.interpolation);
			}
		}
		...
		public void UpdateLogic(int nDelta)
		{
			...
			simulator.set_Interpolation(false);
			simulator.stepScale = this.stepScale;
			simulator.qualityCutoff = this.QualityCutoff;
			simulator.algorithm = this.samplingAlgorithm;
			simulator.set_Oversampling(this.oversampling);
			simulator.set_WallThickness(this.WallThickness);
