...
namespace Assets.Scripts.GameSystem
{
	public class BattleTaskView
	{
		...
		{
			this._root = obj;
			Singleton<BattleLogic>.get_instance().battleTaskSys.OnTaskReady += new GameTaskSys.TaskEventDelegate(this.onBattleTaskStart);
			Singleton<BattleLogic>.get_instance().battleTaskSys.OnTaskStart += new GameTaskSys.TaskEventDelegate(this.onBattleTaskStart);
			Singleton<BattleLogic>.get_instance().battleTaskSys.OnTaskGoing += new GameTaskSys.TaskEventDelegate(this.onBattleTaskGoing);
			Singleton<BattleLogic>.get_instance().battleTaskSys.OnTaskClose += new GameTaskSys.TaskEventDelegate(this.onBattleTaskClose);
		}
		...
		private void onBattleTaskStart(GameTask gt)
		{
			...
			if (this._timer == 0)
			{
				this._timer = Singleton<CTimerManager>.get_instance().AddTimer(1000, -1, new CTimer.OnTimeUpHandler(this.onTimerUpdate));
			}
		}
