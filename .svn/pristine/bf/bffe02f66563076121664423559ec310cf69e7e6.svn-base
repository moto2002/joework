...
namespace Assets.Scripts.GameSystem
{
	public class MinimapView
	{
		...
		private void UpdateUIMap(RectTransform mapPointerRectTransform, Vector3 actorPosition, float mapWidth, float mapHeight)
		{
			...
			{
				float x = actorPosition.x * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Small.x;
				float y = actorPosition.z * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Small.y;
		...
		{
			Singleton<GameEventSys>.get_instance().AddEventHandler<SCommonSpawnEventParam>(GameEventDef.Event_SpawnGroupStartCount, new RefAction<SCommonSpawnEventParam>(this.OnSpawnGroupStartCount));
			Singleton<GameEventSys>.get_instance().AddEventHandler<STailsmanEventParam>(GameEventDef.Event_TailsmanSpawn, new RefAction<STailsmanEventParam>(this.OnTailsmanSpawn));
		...
		public void Clear()
		{
			Singleton<GameEventSys>.get_instance().RmvEventHandler<SCommonSpawnEventParam>(GameEventDef.Event_SpawnGroupStartCount, new RefAction<SCommonSpawnEventParam>(this.OnSpawnGroupStartCount));
			Singleton<GameEventSys>.get_instance().RmvEventHandler<STailsmanEventParam>(GameEventDef.Event_TailsmanSpawn, new RefAction<STailsmanEventParam>(this.OnTailsmanSpawn));
		...
		{
			this.AddCharmIcon(param.tailsman.get_handle());
		}

		private void OnTailsmanUsed(ref STailsmanEventParam param)
		{
			this.RemoveCharmIcon(param.tailsman.get_handle());
		}
		...
		private void Draw()
		{
			...
			{
				MinimapView.SpawnGroupCounter current = enumerator.get_Current();
		...
		{
			if (this.m_spawnGroupCounter.get_Count() > 0)
			{
				int count = this.m_spawnGroupCounter.get_Count();
				for (int i = count - 1; i >= 0; i--)
				{
					MinimapView.SpawnGroupCounter spawnGroupCounter = this.m_spawnGroupCounter.get_Item(i);
