...
namespace Assets.Scripts.Framework
{
	public class LobbyConnector : BaseConnector
	{
		...
		private uint onTryReconnect(uint nCount, uint nMax)
		{
			...
			{
				this.lobbySendQueue.Add(listView.get_Item(k));
			}
			Singleton<NetworkModule>.GetInstance().m_lobbyReconnetCount++;
		...
		private bool SendPackage(CSPkg msg)
		{
			...
			int num = 0;
			if (msg.pack(ref this.szSendBuffer, LobbyConnector.nBuffSize, ref num, 0u) == null)
			{
				byte[] array = new byte[num];
				Array.Copy(this.szSendBuffer, array, num);
				return this.connector.WriteData(array, -1) == 0;
			}
			return false;
		}
