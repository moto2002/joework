...
namespace Assets.Scripts.GameLogic
{
	internal class CBattleGuideManager : Singleton<CBattleGuideManager>
	{
		...
		private static readonly uint GuideLevel1v1Tank = GameDataMgr.GetGlobeValue(116);

		private static readonly uint GuideLevel1v1AD = GameDataMgr.GetGlobeValue(263);

		private static readonly uint GuideLevel1v1AP = GameDataMgr.GetGlobeValue(264);

		private static readonly uint GuideLevel5v5Tank = GameDataMgr.GetGlobeValue(118);

		private static readonly uint GuideLevel5v5AD = GameDataMgr.GetGlobeValue(265);

		private static readonly uint GuideLevel5v5AP = GameDataMgr.GetGlobeValue(266);
		...
		public bool bPauseGame
		{
			...
			private set
			{
				if (value)
				{
					Singleton<GameInput>.get_instance().StopInput();
				}
		...
		private void UpdateGamePause(bool bEffectTimeScale)
		{
			...
			Singleton<FrameSynchr>.get_instance().SetSynchrRunning(!this.bPauseGame);
			if (bEffectTimeScale)
			{
				DebugHelper.Assert(!Singleton<FrameSynchr>.get_instance().bActive, "frame synchr active forbid set timeScale");
		...
		public void PauseGame(object sender, bool bEffectTimeScale = true)
		{
			...
			if (this.m_pauseGameStack.ContainsKey(sender))
			{
				...
				int num = pauseGameStack.get_Item(sender);
				expr_1E.set_Item(sender, num + 1);
			}
			else
			{
		...
		private void onOldPlayerFirstFormClose(CUIEvent uiEvent)
		{
			if (uiEvent != null)
			{
				Singleton<CUIManager>.get_instance().CloseForm(uiEvent.m_srcFormScript);
			}
		}
		...
		private void onCloseSkillGesture(CUIEvent uiEvent)
		{
			...
			if (hostPlayer != null)
			{
				...
				if (captain && captain.get_handle().EffectControl != null)
				{
					captain.get_handle().EffectControl.EndSkillGestureEffect();
				}
			}
		...
		private void onCloseSettle(CUIEvent uiEvent)
		{
			...
			{
				Singleton<CUIManager>.get_instance().OpenForm("UGUI/Form/System/Newbie/Form_Intro_3V3.prefab", false, true);
			}
			else
			{
				this.CloseSettle();
		...
		public void OpenFormShared(CBattleGuideManager.EBattleGuideFormType inFormType, int delayTime = 0, bool bPauseGame = true)
		{
			...
			Singleton<CUILoadingSystem>.get_instance().HideLoading();
			...
			if (Singleton<GameStateCtrl>.get_instance().isBattleState)
			{
				if (inFormType == CBattleGuideManager.EBattleGuideFormType.SkillGesture2)
				{
					...
					if (list.get_Count() > 0)
					{
						GameObject gameObject = list.get_Item(0);
						this.showSkillButton(gameObject.transform.FindChild("Present").gameObject, false);
						Singleton<CTimerManager>.get_instance().AddTimer(500, 1, new CTimer.OnTimeUpHandler(this.OnSkillGestEffTimer), false);
					}
				}
		...
		private void OnSkillGestEffTimer(int inTimeSeq)
		{
			...
			if (this.m_lastFormType == CBattleGuideManager.EBattleGuideFormType.SkillGesture2)
			{
				...
				if (captain && captain.get_handle().EffectControl != null)
				{
					captain.get_handle().EffectControl.StartSkillGestureEffect();
				}
			}
			else if (this.m_lastFormType == CBattleGuideManager.EBattleGuideFormType.SkillGesture2Cancel)
			{
				...
				if (captain && captain.get_handle().EffectControl != null)
				{
					captain.get_handle().EffectControl.StartSkillGestureEffectCancel();
				}
			}
			else if (this.m_lastFormType == CBattleGuideManager.EBattleGuideFormType.SkillGesture3)
			{
				...
				if (captain && captain.get_handle().EffectControl != null)
				{
					captain.get_handle().EffectControl.StartSkillGestureEffect3();
				}
			}
		...
		{
			Singleton<CTimerManager>.get_instance().RemoveTimer(new CTimer.OnTimeUpHandler(this.ShowInteractable));
		...
		private void OpenNewbieSettle()
		{
			...
			else if (CBattleGuideManager.IsCastingGuideLevel(curLvelContext.m_mapID))
			{
				...
				if (cUIFormScript != null)
				{
					...
					if (transform != null)
					{
						CUseable itemUseable = CUseableManager.CreateCoinUseable(4, (int)GameDataMgr.GetGlobeValue(297));
		...
		private void OpenTrainLevelSettle()
		{
			...
			if (masterRoleInfo != null)
			{
				...
				{
					if (useableListFromReward.get_Item(i).MapRewardType != 11 && useableListFromReward.get_Item(i).MapRewardType != 14)
					{
						listView.Add(useableListFromReward.get_Item(i));
					}
				}
		...
		private void CloseSettle()
		{
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent("CheckNewbieIntro");
		}
		...
		private void SetExpProfit(CUIFormScript profitForm)
		{
			...
			if (!CSysDynamicBlock.bSocialBlocked)
			{
				...
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component6, (int)masterRoleInfo.GetNobeInfo().stGameVipClient.dwCurLevel, false);
		...
		private void SetMapInfo(CUIFormScript profitForm)
		{
			...
			string text = Singleton<CTextManager>.get_instance().GetText("Battle_Settle_Game_Type_Train");
		...
		public void OpenOldPlayerFirstForm()
		{
			...
			if (Singleton<CUIManager>.get_instance().GetForm("UGUI/Form/System/Newbie/Form_OldTip.prefab") == null)
			{
				Singleton<CUIManager>.get_instance().OpenForm("UGUI/Form/System/Newbie/Form_OldTip.prefab", false, true);
		...
		public static bool EnableHeroVictoryTips()
		{
			...
			GameDataMgr.svr2CltCfgDict.TryGetValue(9u, ref resGlobalInfo);
		...
		public static bool GetGuide1v1HeroIDAndLevelID(uint heroType, out uint heroId, out int levelId)
		{
			...
			{
				heroId = GameDataMgr.GetGlobeValue(114);
				levelId = (int)GameDataMgr.GetGlobeValue(116);
				return true;
			}
			if (heroType == GameDataMgr.GetGlobeValue(285))
			{
				heroId = GameDataMgr.GetGlobeValue(259);
				levelId = (int)GameDataMgr.GetGlobeValue(263);
				return true;
			}
			if (heroType == GameDataMgr.GetGlobeValue(286))
			{
				heroId = GameDataMgr.GetGlobeValue(260);
				levelId = (int)GameDataMgr.GetGlobeValue(264);
				return true;
			}
			heroId = 0u;
			levelId = 0;
		...
		public static bool GetGuide5v5HeroIDAndLevelID(uint heroType, out uint heroId, out int levelId)
		{
			if (heroType == GameDataMgr.GetGlobeValue(284))
			{
				heroId = GameDataMgr.GetGlobeValue(145);
				levelId = (int)GameDataMgr.GetGlobeValue(118);
				return true;
			}
			if (heroType == GameDataMgr.GetGlobeValue(285))
			{
				heroId = GameDataMgr.GetGlobeValue(261);
				levelId = (int)GameDataMgr.GetGlobeValue(265);
				return true;
			}
			if (heroType == GameDataMgr.GetGlobeValue(286))
			{
				heroId = GameDataMgr.GetGlobeValue(262);
				levelId = (int)GameDataMgr.GetGlobeValue(266);
				return true;
			}
			heroId = 0u;
			levelId = 0;
