	...
	public class LobbySvrMgr : Singleton<LobbySvrMgr>
	{
		...
		public uint OnTryReconnect(uint curConnectTime, uint maxcount)
		{
			if (this.canReconnect)
			{
				if (this.isLogin)
				{
					string connectUrl = Singleton<ReconnectIpSelect>.get_instance().GetConnectUrl(ConnectorType.Lobby, curConnectTime);
					...
					Singleton<NetworkModule>.get_instance().lobbySvr.initParam.SetVip(ApolloConfig.loginOnlyIpOrUrl);
					if (!Singleton<LobbyLogic>.get_instance().inMultiGame && (Singleton<BattleLogic>.get_instance().isGameOver || Singleton<BattleLogic>.get_instance().m_bIsPayStat))
					{
						if (curConnectTime > maxcount)
						{
							if (curConnectTime == maxcount + 1u)
							{
								Singleton<LobbyLogic>.get_instance().OnSendSingleGameFinishFail();
							}
						}
						else
						{
		...
		public void ConnectServerWithTdirDefault(int index, ChooseSvrPolicy policy)
		{
			...
			{
				ApolloConfig.loginUrl = string.Format("tcp://{0}:{1}", MonoSingleton<CTongCaiSys>.get_instance().TongcaiUrl, MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.addrs.get_Item(index).port);
				ApolloConfig.loginOnlyIpOrUrl = MonoSingleton<CTongCaiSys>.get_instance().TongcaiUrl;
			}
			else
			{
		...
		private void ConnectServerWithTdirCandidate(int index)
		{
			...
			if (MonoSingleton<CTongCaiSys>.get_instance().IsCanUseTongCai())
			{
				flag = true;
				ApolloConfig.ISPType = MonoSingleton<TdirMgr>.GetInstance().GetISP();
				ApolloConfig.loginOnlyIpOrUrl = MonoSingleton<CTongCaiSys>.get_instance().TongcaiIps[ApolloConfig.ISPType];
			...
			{
				object obj = 0;
				if (MonoSingleton<TdirMgr>.GetInstance().ParseNodeAppAttr(MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.attr, TdirNodeAttrType.ISPChoose, ref obj))
				{
					...
					if (dictionary != null)
					{
						using (Dictionary<string, int>.Enumerator enumerator = dictionary.GetEnumerator())
						{
							while (enumerator.MoveNext())
							{
								...
								if (current.get_Value() == MonoSingleton<TdirMgr>.GetInstance().GetISP())
								{
