...
namespace Assets.Scripts.GameSystem
{
	public class GiftCenter
	{
		...
		{
			if (Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().PvpLevel >= 5u)
			{
				Singleton<CUINewFlagSystem>.GetInstance().DelNewFlag(uiEvent.m_srcWidget, enNewFlagKey.New_GiftCenterBtn_V1, true);
			}
		...
		private void OnItemEnable(CUIEvent uiEvent)
		{
			...
			if (this._curMenuPage == GiftCenter.MenuPage.Skin && num < this._skinList.get_Count())
			{
				ResHeroSkin skinInfo = this._skinList.get_Item(num);
			...
			else if (num < this._heroList.get_Count())
			{
				ResHeroCfgInfo heroInfo = this._heroList.get_Item(num);
		...
		private void SetItemTag(CMallItemWidget itemWidget, ResHeroPromotion heroPromotion, ResSkinPromotion skinPromotion, CUIFormScript form)
		{
			string text = null;
			string text2 = null;
			RES_LUCKYDRAW_ITEMTAG rES_LUCKYDRAW_ITEMTAG = 0;
			if (heroPromotion != null)
			{
				rES_LUCKYDRAW_ITEMTAG = heroPromotion.bTag;
			}
			else if (skinPromotion != null)
			{
				rES_LUCKYDRAW_ITEMTAG = skinPromotion.bTag;
			}
		...
		private void CleanSortSkinList()
		{
			...
			if (this._skinList.get_Count() > this._skinTotalNum)
			{
				this._skinList.RemoveRange(this._skinTotalNum + 1, this._skinList.get_Count() - this._skinTotalNum);
			}
		...
		private void RealIndexGetReady(bool forceUpdate = false)
		{
			...
			if (this._curMenuPage == GiftCenter.MenuPage.Skin)
			{
				...
				{
					ResHeroSkin resHeroSkin = this._skinList.get_Item(i);
			...
			else
			{
				...
				{
					ResHeroCfgInfo resHeroCfgInfo = this._heroList.get_Item(j);
