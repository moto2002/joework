...
namespace Assets.Scripts.GameLogic
{
	public class BuffProtectRule
	{
		...
		{
			DefaultGameEventParam defaultGameEventParam = new DefaultGameEventParam(this.buffHolder.actorPtr, atker);
			Singleton<GameEventSys>.get_instance().SendEvent<DefaultGameEventParam>(GameEventDef.Event_ActorImmune, ref defaultGameEventParam);
		}
		...
		{
			if (inBuff.get_handle().CustomParams[0] > 0 || inBuff.get_handle().CustomParams[1] > 0 || inBuff.get_handle().CustomParams[2] > 0 || inBuff.get_handle().CustomParams[3] > 0)
			{
				this.SpawnSkillEffect(inBuff.get_handle().CustomParams[5], inBuff.get_handle().SlotType, inBuff.get_handle().skillContext.bExposing);
			}
		}

		public void RemoveBuff(ref PoolObjHandle<BuffSkill> inBuff)
		{
			if (inBuff.get_handle().cfgData.bEffectSubType == 1)
			{
				this.SendProtectEvent(0, -inBuff.get_handle().CustomParams[0]);
				this.PhysicsProtectList.Remove(inBuff);
				this.RemoveProtectSpawnSkillEffect(ref inBuff);
			}
			else if (inBuff.get_handle().cfgData.bEffectSubType == 2)
			{
				this.SendProtectEvent(1, -inBuff.get_handle().CustomParams[1]);
				this.MagicProtectList.Remove(inBuff);
				this.RemoveProtectSpawnSkillEffect(ref inBuff);
			}
			else if (inBuff.get_handle().cfgData.bEffectSubType == 3)
			{
				this.SendProtectEvent(2, -inBuff.get_handle().CustomParams[2]);
				this.AllProtectBuffList.Remove(inBuff);
				this.RemoveProtectSpawnSkillEffect(ref inBuff);
			}
			else if (inBuff.get_handle().cfgData.bEffectSubType == 11)
			{
				this.SendProtectEvent(3, -inBuff.get_handle().CustomParams[3]);
				this.AllIncludeRealHurtProtectBuffList.Remove(inBuff);
				this.RemoveProtectSpawnSkillEffect(ref inBuff);
			}
			else if (inBuff.get_handle().cfgData.bEffectSubType == 4 || inBuff.get_handle().cfgData.bEffectSubType == 5 || inBuff.get_handle().cfgData.bEffectSubType == 6)
			{
		...
		private int ResistProtectImpl(int _hurtValue, List<PoolObjHandle<BuffSkill>> _inList, int _index)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				BuffSkill handle = array[i].get_handle();
		...
		public int ResistDamage(ref HurtDataInfo _hurt, int _hurtValue)
		{
			...
			{
				int num3 = this.buffHolder.actor.ValueComponent.mActorValue[5].totalValue;
