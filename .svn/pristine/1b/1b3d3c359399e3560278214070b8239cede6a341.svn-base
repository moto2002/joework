...
namespace Assets.Scripts.GameLogic
{
	public class CTailsman : PooledClassObject, IUpdateLogic
	{
			...
			if (this.CharmId > 0)
			{
				...
				if (dataByKey != null)
				{
					...
					if (bufConsumer.Use())
					{
						...
						Singleton<GameEventSys>.get_instance().SendEvent<STailsmanEventParam>(GameEventDef.Event_TailsmanUsed, ref sTailsmanEventParam);
					}
				}
			}
			this.DoClearing();
		...
		public void DoClearing()
		{
			...
			Singleton<ShenFuSystem>.get_instance().RemoveCharm(inCharm);
			base.Release();
		}
		...
		public void UpdateLogic(int inDelta)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (poolObjHandle2 && !poolObjHandle2.get_handle().ActorControl.IsDeadState)
				{
					...
					ulong sqrMagnitudeLong2D = (ulong)(poolObjHandle2.get_handle().location - this.InitLoc).get_sqrMagnitudeLong2D();
