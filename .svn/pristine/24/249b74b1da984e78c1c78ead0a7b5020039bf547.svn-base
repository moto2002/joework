...
namespace Assets.Scripts.GameSystem
{
	public class CSymbolInfo
	{
		...
		{
			int num = CSymbolInfo.s_symbolPvpLvlList.get_Count();
			bool flag = false;
			for (int i = num - 1; i >= 0; i--)
			{
				if ((int)CSymbolInfo.s_symbolPvpLvlList.get_Item(i).wSymbolMaxLvl == symbolLvl && num > (int)CSymbolInfo.s_symbolPvpLvlList.get_Item(i).wPvpLevel)
				{
					num = (int)CSymbolInfo.s_symbolPvpLvlList.get_Item(i).wPvpLevel;
					flag = true;
				}
			}
			if (!flag)
		...
		public int GetNextFreePageLevel()
		{
			...
			for (int i = 0; i < CSymbolInfo.s_symbolPvpLvlList.get_Count(); i++)
			{
				if ((uint)CSymbolInfo.s_symbolPvpLvlList.get_Item(i).wPvpLevel > masterRoleInfo.PvpLevel && CSymbolInfo.s_symbolPvpLvlList.get_Item(i).bPresentSymbolPage > 0)
				{
					return (int)CSymbolInfo.s_symbolPvpLvlList.get_Item(i).wPvpLevel;
				}
			}
			return 0;
		}
