...
namespace Assets.Scripts.GameLogic
{
	public sealed class JoystickMode : GameInputMode
	{
		...
		{
			Singleton<CUIEventManager>.get_instance().DispatchUIEvent(enUIEventID.Battle_CloseBigMap);
			MonoSingleton<CameraSystem>.get_instance().enableCameraMovement = true;
		}

		private void OnCameraAxisReleased(CUIEvent uiEvent)
		{
			MonoSingleton<CameraSystem>.get_instance().enableCameraMovement = false;
		}
		...
		private void OnPanelCameraDraging(CUIEvent uiEvent)
		{
			...
			MonoSingleton<CameraSystem>.get_instance().UpdatePanelCameraMovement(ref vector);
			Vector3 position = MonoSingleton<CameraSystem>.get_instance().MobaCamera.requirements.pivot.position;
			...
			theMinimapSys.MMiniMapCameraFrame_3Dui.SetPos(position.x * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Small.x, position.z * Singleton<CBattleSystem>.get_instance().world_UI_Factor_Small.y);
		}
		...
		{
			MonoSingleton<CameraSystem>.get_instance().UpdateCameraMovement(ref axis);
		}
	}
}
