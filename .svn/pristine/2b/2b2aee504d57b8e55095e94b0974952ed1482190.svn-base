...
namespace Assets.Scripts.GameSystem
{
	public class CRoleInfoContainer : CContainer
	{
		...
		public int GetContainerSize()
		{
			...
			{
				return this.m_roleInfoList.get_Count();
			}
			return 0;
		}
		...
		{
			if (this.m_roleInfoList == null || this.m_roleInfoList.get_Count() <= 0 || index >= this.m_roleInfoList.get_Count())
			{
				return null;
			}
			return this.m_roleInfoList.get_Item(index);
		}
		...
		public CRoleInfo FindRoleInfoByID(ulong uuID)
		{
			if (this.m_roleInfoList != null)
			{
				for (int i = 0; i < this.m_roleInfoList.get_Count(); i++)
				{
					if (this.m_roleInfoList.get_Item(i) != null && this.m_roleInfoList.get_Item(i).playerUllUID == uuID)
					{
						return this.m_roleInfoList.get_Item(i);
					}
				}
			}
			return null;
		...
		public void RemoveRoleInfoByUUID(ulong uuid)
		{
			...
			for (int i = 0; i < this.m_roleInfoList.get_Count(); i++)
			{
				cRoleInfo = this.m_roleInfoList.get_Item(i);
