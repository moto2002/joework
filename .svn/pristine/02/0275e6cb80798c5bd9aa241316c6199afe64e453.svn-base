...
namespace Assets.Scripts.GameSystem
{
	public class InBattleMsgMgr : Singleton<InBattleMsgMgr>
	{
		...
		{
			if (tabIndex < 0 || tabIndex >= Singleton<InBattleMsgMgr>.get_instance().title_list.get_Count())
			{
				return null;
			}
			ListView<TabElement> listView = null;
			string text = Singleton<InBattleMsgMgr>.get_instance().title_list.get_Item(tabIndex);
			Singleton<InBattleMsgMgr>.get_instance().tabElements.TryGetValue(text, ref listView);
			if (listView == null)
			{
				return null;
			}
			if (list_index < 0 || list_index >= listView.get_Count())
			{
				return null;
			}
			return listView.get_Item(list_index);
		}
		...
		public void ClearMenuItem(int index)
		{
			...
			{
				this.menuEntList.set_Item(index, null);
			}
		}
		...
		public bool IsEnableShortcut()
		{
			...
			return curLvelContext != null && (curLvelContext.m_isWarmBattle || Singleton<LobbyLogic>.get_instance().inMultiGame);
		}

		public bool IsEnableInputChat()
		{
			return Singleton<CChatController>.get_instance().model.bEnableInBattleInputChat && this.IsEnableShortcut() && GameSettings.InBattleInputChatEnable == 1;
		}
		...
		public void On_InBatMenu_OpenForm(CUIEvent uievent)
		{
			...
			Singleton<CUINewFlagSystem>.get_instance().SetNewFlagForMessageBtnEntry(false);
