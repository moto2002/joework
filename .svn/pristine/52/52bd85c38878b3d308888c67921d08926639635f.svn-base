...
namespace Assets.Scripts.GameSystem
{
	...
	public class HeadIconSys : Singleton<HeadIconSys>
	{
		...
		public uint UnReadFlagNum
		{
			...
			{
				uint num = 0u;
				for (int i = 0; i < this.m_headImgInfo.get_Count(); i++)
				{
					if (this.m_headImgInfo.get_Item(i).bNtfFlag == 1)
					{
						num += 1u;
					}
				}
		...
		{
			base.Init();
			Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.HeadIcon_Change_Form_Open, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Form_Open));
			Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.HeadIcon_Change_Form_Close, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Form_Close));
			Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.HeadIcon_Change_Tab_Click, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Tab_Click));
			Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.HeadIcon_Change_Icon_Click, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Icon_Click));
			Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.HeadIcon_Change_Confirm, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Confirm));
			Singleton<CUIEventManager>.get_instance().AddUIEventListener(enUIEventID.HeadIcon_Change_Item_Enable, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Item_Enable));
			Singleton<EventRouter>.get_instance().AddEventHandler(EventID.NOBE_STATE_CHANGE, new Action(this.OnHeadInfoRefresh));
		}

		public override void UnInit()
		{
			base.UnInit();
			Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.HeadIcon_Change_Form_Open, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Form_Open));
			Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.HeadIcon_Change_Form_Close, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Form_Close));
			Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.HeadIcon_Change_Tab_Click, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Tab_Click));
			Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.HeadIcon_Change_Icon_Click, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Icon_Click));
			Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.HeadIcon_Change_Confirm, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Confirm));
			Singleton<CUIEventManager>.get_instance().RemoveUIEventListener(enUIEventID.HeadIcon_Change_Item_Enable, new CUIEventManager.OnUIEventHandler(this.On_HeadIcon_Change_Item_Enable));
			Singleton<EventRouter>.get_instance().RemoveEventHandler(EventID.NOBE_STATE_CHANGE, new Action(this.OnHeadInfoRefresh));
		}
		...
		{
			ListView<ResHeadImage> curHeadImgList = this.GetCurHeadImgList(HeadIconSys.enHeadImgTab.All);
			for (int i = 0; i < curHeadImgList.get_Count(); i++)
			{
				ResHeadImage resHeadImage = curHeadImgList.get_Item(i);
		...
		{
			CUIFormScript cUIFormScript = Singleton<CUIManager>.get_instance().OpenForm(HeadIconSys.s_headImgChgForm, false, true);
			if (cUIFormScript != null)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					componetInChild.GetElemenet(i).GetComponentInChildren<Text>().text = Singleton<CTextManager>.get_instance().GetText(string.Format("HeadImg_Tab_Txt_{0}", i + 1));
				}
		...
		{
			CUIFormScript form = Singleton<CUIManager>.get_instance().GetForm(HeadIconSys.s_headImgChgForm);
			if (form != null)
			{
		...
		private ResHeadImage GetSelectedHeadImg()
		{
			...
			if (form != null)
			{
				...
				if (selectedIndex2 > -1 && selectedIndex2 < curHeadImgList.get_Count())
				{
					return curHeadImgList.get_Item(selectedIndex2);
				}
			}
			return null;
		}
		...
		public HeadIconSys.HeadImgInfo GetInfo(uint headImgId)
		{
			for (int i = 0; i < this.m_headImgInfo.get_Count(); i++)
			{
				if (this.m_headImgInfo.get_Item(i).dwID == headImgId)
				{
					return this.m_headImgInfo.get_Item(i);
				}
			}
			return null;
		}
		...
		public ResHeadImage GetResInfo(uint headImgId)
		{
			...
			for (int i = 0; i < curHeadImgList.get_Count(); i++)
			{
				if (curHeadImgList.get_Item(i).dwID == headImgId)
				{
					return curHeadImgList.get_Item(i);
				}
			}
			return null;
		}
		...
		private void OnShowMainPanel(HeadIconSys.enHeadImgTab curTab)
		{
			...
			if (form != null)
			{
				...
				int count = curHeadImgList.get_Count();
				...
				{
					if (this.IsHeadIconInUse(curHeadImgList.get_Item(i).dwID))
					{
						index = i;
						break;
					}
		...
		private void OnUpdateElement(GameObject element, int index, HeadIconSys.enHeadImgTab curTab)
		{
			...
			ListView<ResHeadImage> curHeadImgList = this.GetCurHeadImgList(curTab);
			if (index < 0 || index >= curHeadImgList.get_Count())
			{
				return;
			}
			ResHeadImage resHeadImage = curHeadImgList.get_Item(index);
			...
			MonoSingleton<NobeSys>.get_instance().SetHeadIconBk(componetInChild, (int)resHeadImage.dwID);
			if (info != null)
			{
		...
		private void OnShowDetailPanel(int index)
		{
			...
			if (form != null)
			{
				...
				if (index < curHeadImgList.get_Count())
				{
					...
					MonoSingleton<NobeSys>.get_instance().SetHeadIconBk(componetInChild3, (int)resHeadImage.dwID);
					MonoSingleton<NobeSys>.get_instance().SetHeadIconBkEffect(componetInChild3, (int)resHeadImage.dwID, form, 1f);
					if (resHeadImage.dwValidSecond == 0u)
					{
						componetInChild2.text = Singleton<CTextManager>.get_instance().GetText("HeadImg_Tips_1");
					}
					else if (info != null)
					{
						DateTime dateTime = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime() + (long)((ulong)resHeadImage.dwValidSecond));
						componetInChild2.text = Singleton<CTextManager>.get_instance().GetText("HeadImg_Tips_2", new string[]
						{
					...
					{
						componetInChild2.text = Singleton<CTextManager>.get_instance().GetText("HeadImg_Tips_3", new string[]
						{
		...
		public void ClearHeadImgFlag(uint headImg)
		{
			for (int i = 0; i < this.m_headImgInfo.get_Count(); i++)
			{
				if (this.m_headImgInfo.get_Item(i).dwID == headImg)
				{
					this.m_headImgInfo.get_Item(i).bNtfFlag = 0;
				}
			}
		}
		...
		public void AddHeadImgInfo(uint dwHeadImgID, uint dwGetTime)
		{
			...
			for (int i = 0; i < this.m_headImgInfo.get_Count(); i++)
			{
				if (this.m_headImgInfo.get_Item(i).dwID == dwHeadImgID)
				{
		...
		private static void OnHeadIconChangeReq(uint headImgId)
		{
			...
			cSPkg.stPkgData.get_stHeadImgChgReq().dwHeadImgID = headImgId;
		...
		{
			if (msg.stPkgData.get_stHeadImgChgRsp().iResult == 0)
			{
				Singleton<HeadIconSys>.get_instance().SetMasterHeadImg(msg.stPkgData.get_stHeadImgChgRsp().dwHeadImgID);
		...
		private static void OnHeadIconFlagClearReq(uint headImgId)
		{
			...
			cSPkg.stPkgData.get_stHeadImgFlagClrReq().dwHeadImgID = headImgId;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
		...
		{
			Singleton<HeadIconSys>.get_instance().ClearHeadImgFlag(msg.stPkgData.get_stHeadImgFlagClrRsp().dwHeadImgID);
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.NOBE_STATE_CHANGE);
		...
		{
			Singleton<HeadIconSys>.get_instance().SetMasterHeadImg(msg.stPkgData.get_stHeadImgChgNtf().dwHeadImgID);
			Singleton<HeadIconSys>.get_instance().RefreshUseButton(false);
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.NOBE_STATE_CHANGE);
		}
		...
		{
			Singleton<HeadIconSys>.get_instance().OnHeadIconSyncList(msg.stPkgData.get_stHeadImgListSync().stHeadImgList.wHeadImgCnt, msg.stPkgData.get_stHeadImgListSync().stHeadImgList.astHeadImgInfo);
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.HEAD_IMAGE_FLAG_CHANGE);
		}
		...
		{
			Singleton<HeadIconSys>.get_instance().AddHeadImgInfo(msg.stPkgData.get_stHeadImgAddNtf().dwHeadImgID, msg.stPkgData.get_stHeadImgAddNtf().dwGetTime);
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.HEAD_IMAGE_FLAG_CHANGE);
		}
		...
		{
			Singleton<HeadIconSys>.get_instance().DelHeadImgInfo(msg.stPkgData.get_stHeadImgDelNtf().dwHeadImgID);
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.HEAD_IMAGE_FLAG_CHANGE);
		}
	}
}
