...
public class CTaskView
{
	...
	public class CTaskUT
	{
		public static void ShowTaskAward(CUIFormScript formScript, CTask task, GameObject awardContainer, int awardItemcount = 2)
		{
			...
			for (int i = 0; i < awardItemcount; i++)
			{
				...
				{
					RES_REWARDS_TYPE dwRewardType = resTaskRewardDetail.dwRewardType;
	...
	public int TabIndex
	{
		...
		set
		{
			...
			Singleton<CTaskSys>.get_instance().Increse(this.m_tabIndex);
			...
			case 2:
				...
				if (this.tasklistScript_iwantgold != null)
				{
					...
					Singleton<CMiShuSystem>.get_instance().InitList(this.m_tabIndex, this.tasklistScript_iwantgold);
			...
			default:
				...
				if (this.tasklistScript_mishu != null)
				{
					...
					Singleton<CMiShuSystem>.get_instance().InitList(this.m_tabIndex, this.tasklistScript_mishu);
				}
				break;
			}
		}
	...
	public void refresh_Day_HuoYue()
	{
		...
		{
			ushort num2 = model.huoyue_data.day_huoyue_list.get_Item(i);
			ResHuoYueDuReward resHuoYueDuReward = null;
			GameDataMgr.huoyueduDict.TryGetValue(num2, ref resHuoYueDuReward);
	...
	private void _init_day_huoyue()
	{
		...
		{
			ushort num = model.huoyue_data.day_huoyue_list.get_Item(i);
			ResHuoYueDuReward resHuoYueDuReward = null;
			GameDataMgr.huoyueduDict.TryGetValue(num, ref resHuoYueDuReward);
			if (resHuoYueDuReward != null)
			{
				float num2 = resHuoYueDuReward.dwHuoYueDu / max_dayhuoyue_num * x;
				if (num2 > x)
				{
					num2 = x;
				}
				...
				if (!(element2 == null))
				{
					(element2.transform as RectTransform).anchoredPosition3D = new Vector3(num2, 0f, 0f);
	...
	public void Bind_Week_Node(GameObject node, ushort week_id)
	{
		...
		ResDT_HuoYueDuReward_PeriodInfo resDT_HuoYueDuReward_PeriodInfo = Singleton<CTaskSys>.get_instance().model.huoyue_data.IsInTime(rewardCfg);
		...
		bool flag = huoyue_data.BAlready_Reward(2, week_id);
	...
	private void _show_day_box(GameObject box, ushort id)
	{
		...
		ResHuoYueDuReward resHuoYueDuReward = null;
		GameDataMgr.huoyueduDict.TryGetValue(id, ref resHuoYueDuReward);
		if (resHuoYueDuReward == null)
		{
			return;
		}
		bool flag = Singleton<CTaskSys>.get_instance().model.huoyue_data.BAlready_Reward(1, id);
		component.text = resHuoYueDuReward.dwHuoYueDu.ToString();
		component2.gameObject.CustomSetActive(flag);
		if (component3 != null)
		{
			ResDT_HuoYueDuReward_PeriodInfo resDT_HuoYueDuReward_PeriodInfo = Singleton<CTaskSys>.get_instance().model.huoyue_data.IsInTime(resHuoYueDuReward);
			if (resDT_HuoYueDuReward_PeriodInfo != null)
			{
		...
		bool flag2 = !flag && Singleton<CTaskSys>.get_instance().model.huoyue_data.day_curNum >= resHuoYueDuReward.dwHuoYueDu;
	...
	public void Refresh_List(int tIndex)
	{
		...
		ListView<CTask> listView = Singleton<CTaskSys>.get_instance().model.task_Data.GetListView(tIndex);
	...
	public void On_List_ElementEnable(CUIEvent uievent)
	{
		...
		CTask cTask = this._get_current_info(this.m_tabIndex, srcWidgetIndexInBelongedList);
	...
	private void _calc_red_dot(enTaskTab type)
	{
		...
		int task_Count = Singleton<CTaskSys>.get_instance().model.task_Data.GetTask_Count(type, CTask.State.Have_Done);
		if (task_Count > 0)
		{
	...
	private void Refresh_LevelTab()
	{
		...
		this._refresh_Level_list(this.m_levelRewardList, Singleton<CTaskSys>.get_instance().model.m_levelRewardDataMap.get_Count());
		this.ShowLevelRightSide(Singleton<CTaskSys>.get_instance().model.curLevelRewardData);
	}
	...
	private void ShowLevelTask(LevelRewardData levelRewardData)
	{
		...
		if (flag || (!flag2 && !levelRewardData.IsHasCltCalcCompeletedTask()) || flag3)
		{
			...
			if (component != null)
			{
				...
				if (flag)
				{
					text = Singleton<CTextManager>.get_instance().GetText("Task_LevelRewardInfo_None");
				}
				if (!flag2 && !levelRewardData.IsHasCltCalcCompeletedTask())
				{
					text = Singleton<CTextManager>.get_instance().GetText("Task_LevelRewardInfo_NoGetTask");
				}
				if (flag3)
				{
					if (levelRewardData.m_level >= CTaskView.LevelValue)
					{
						text = Singleton<CTextManager>.get_instance().GetText("Task_LevelRewardInfo_Up21");
					}
					else
					{
						text = Singleton<CTextManager>.get_instance().GetText("Task_LevelRewardInfo_AllLevelTask");
					}
				}
				component.text = text;
			}
		...
		else
		{
			...
			if (component2 != null)
			{
				component2.text = Singleton<CTextManager>.get_instance().GetText("Task_LevelRewardInfo_Normal", new string[]
				{
			...
			for (int i = 0; i < levelRewardData.task_ids.Length; i++)
			{
				...
				if (resTask != null)
				{
					...
					{
						flag6 = Singleton<CTaskSys>.get_instance().model.IsInCltCalcCompletedTasks(resTask.dwTaskID);
	...
	private void ShowUnLock(LevelRewardData data, GameObject node)
	{
		...
		{
			this.m_unlockInfoTxt.text = string.Format(Singleton<CTextManager>.get_instance().GetText("Task_Unlock_Text"), data.m_level);
		}
		int num = 0;
		for (int i = 0; i < LevelRewardData.UNLOCK_MAX_COUNT; i++)
		{
			...
			if (resDT_LevelReward_UnlockInfo != null && !string.IsNullOrEmpty(resDT_LevelReward_UnlockInfo.szUnlockID))
			{
				...
				if (transform != null)
				{
					...
					this._SetUnlockButton(component3, resDT_LevelReward_UnlockInfo.bGotoID, (ulong)Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().PvpLevel >= (ulong)((long)data.m_level));
					num++;
				}
			}
		}
	...
	private void _SetUnlockButton(Button btn, RES_GAME_ENTRANCE_TYPE entryType, bool bEnable)
	{
		...
		if (Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(type) && bEnable)
		{
	...
	private void ShowLevelReward(LevelRewardData data, GameObject node)
	{
		...
		{
			this.m_levelRewardText.text = string.Format(Singleton<CTextManager>.get_instance().GetText("Task_Award_Text"), data.m_level);
		}
		...
		if (Singleton<CTaskSys>.get_instance().model.IsGetLevelReward(data.m_level))
		{
			if (gameObject2 != null)
			{
		...
		else
		{
			...
			if ((ulong)Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().PvpLevel >= (ulong)((long)data.m_level))
			{
				if (gameObject != null)
				{
					gameObject.CustomSetActive(true);
	...
	{
		if (this.m_levelRewardList != null && Singleton<CTaskSys>.get_instance().model.m_levelRewardDataMap != null)
		{
			this._refresh_Level_list(this.m_levelRewardList, Singleton<CTaskSys>.get_instance().model.m_levelRewardDataMap.get_Count());
		}
	}
	...
	private void _refresh_Level_list(CUIListScript listScript, int count)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (elemenet != null && listScript.IsElementInScrollArea(i))
			{
				this._ShowLevelNode(elemenet.gameObject, levelRewardDataMap.get_Item(i));
			}
		}
	}
	...
	public void On_LevelRewardList_ElementEnable(CUIEvent uievent)
	{
		ListView<LevelRewardData> levelRewardDataMap = Singleton<CTaskSys>.get_instance().model.m_levelRewardDataMap;
		int srcWidgetIndexInBelongedList = uievent.m_srcWidgetIndexInBelongedList;
		if (srcWidgetIndexInBelongedList < levelRewardDataMap.get_Count())
		{
			LevelRewardData levelRewardData = levelRewardDataMap.get_Item(srcWidgetIndexInBelongedList);
			if (levelRewardData != null)
			{
	...
	private void _ShowLevelNode(GameObject node, LevelRewardData data)
	{
		...
		int levelRewardData_Index = Singleton<CTaskSys>.get_instance().model.GetLevelRewardData_Index(data);
		bool flag = levelRewardData_Index == Singleton<CTaskSys>.get_instance().model.m_levelRewardDataMap.get_Count() - 1;
		GameObject gameObject = null;
		GameObject gameObject2 = null;
		...
		if (Singleton<CTaskSys>.get_instance().model.curLevelRewardData != null)
		{
			bool bActive2 = Singleton<CTaskSys>.get_instance().model.curLevelRewardData.IsEqual(data);
		...
		if (Singleton<CTaskSys>.get_instance().model.IsLevelNode_RedDot(data))
		{
