...
namespace Assets.Scripts.GameSystem
{
	internal class CLadderView
	{
		...
		public static void SetGameInfoRecentPanel(CUIFormScript form, COMDT_RANKDETAIL rankDetail, List<COMDT_RANK_CURSEASON_FIGHT_RECORD> dataList)
		{
			...
			if (rankDetail != null && dataList != null && dataList.get_Count() > 0 && rankDetail.dwSeasonIdx == dataList.get_Item(0).dwSeasonId)
			{
				...
				if (dataByKey != null)
				{
					...
					GameObject gameObject = form.GetWidget(5).gameObject;
					gameObject.CustomSetActive(cOMDT_RANK_CURSEASON_FIGHT_RECORD.bTeamerNum > 1 && cOMDT_RANK_CURSEASON_FIGHT_RECORD.bTeamerNum < 5);
					gameObject = form.GetWidget(6).gameObject;
					if (gameObject != null)
					{
						gameObject.CustomSetActive(cOMDT_RANK_CURSEASON_FIGHT_RECORD.bTeamerNum == 5);
					}
					GameObject gameObject2 = form.GetWidget(4).gameObject;
					gameObject2.CustomSetActive(Convert.ToBoolean(cOMDT_RANK_CURSEASON_FIGHT_RECORD.bIsBanPick));
				}
				widget.CustomSetActive(true);
				widget2.CustomSetActive(false);
			}
		...
		public static void InitLadderHistory(CUIFormScript form, List<COMDT_RANK_PASTSEASON_FIGHT_RECORD> dataList)
		{
			...
			else
			{
				...
				for (int i = 0; i < dataList.get_Count(); i++)
				{
					...
					int num = (list.get_Count() <= 4) ? list.get_Count() : 4;
					for (int j = 0; j < num; j++)
					{
		...
		public static void InitLadderRecent(CUIFormScript form, List<COMDT_RANK_CURSEASON_FIGHT_RECORD> dataList)
		{
			...
			{
				int num = (dataList.get_Count() >= 10) ? 10 : dataList.get_Count();
				...
				for (int i = 0; i < num; i++)
				{
					...
					if (dataByKey != null)
					{
						...
						Utility.FindChild(component2.gameObject, "Friend").CustomSetActive(cOMDT_RANK_CURSEASON_FIGHT_RECORD.bTeamerNum > 1 && cOMDT_RANK_CURSEASON_FIGHT_RECORD.bTeamerNum < 5);
		...
		public static bool IsSuperMaster(byte rankGrade, uint rankClass)
		{
			...
			return dataByKey != null && (int)rankGrade == CLadderSystem.MAX_RANK_LEVEL && (rankClass == 0u || rankClass > dataByKey.dwConfValue);
		}

		public static bool IsSuperKing(byte rankGrade, uint rankClass)
		{
			ResGlobalInfo dataByKey = GameDataMgr.globalInfoDatabin.GetDataByKey(205u);
			return dataByKey != null && (int)rankGrade == CLadderSystem.MAX_RANK_LEVEL && 0u < rankClass && rankClass <= dataByKey.dwConfValue;
		}
		...
		public static string GetRankIconPath(byte rankGrade, uint rankClass)
		{
			...
			if (CLadderView.IsSuperMaster(rankGrade, rankClass))
			{
				return CLadderView.RANK_ICON_PATH + StringHelper.UTF8BytesToString(ref dataByKey.szGradePicturePathSuperMaster);
			}
			return CLadderView.RANK_ICON_PATH + StringHelper.UTF8BytesToString(ref dataByKey.szGradePicturePath);
		}
		...
		public static string GetRankSmallIconPath(byte rankGrade, uint rankClass)
		{
			...
			if (CLadderView.IsSuperMaster(rankGrade, rankClass))
			{
				return CLadderView.RANK_SMALL_ICON_PATH + StringHelper.UTF8BytesToString(ref dataByKey.szGradePicturePathSuperMaster);
			}
			return CLadderView.RANK_SMALL_ICON_PATH + StringHelper.UTF8BytesToString(ref dataByKey.szGradePicturePath);
		}
		...
		public static string GetSubRankIconPath(byte rankGrade, uint rankClass)
		{
			...
			if (CLadderView.IsSuperMaster(rankGrade, rankClass))
			{
				return CLadderView.RANK_ICON_PATH + StringHelper.UTF8BytesToString(ref dataByKey.szGradeSmallPicPathSuperMaster);
			}
			return CLadderView.RANK_ICON_PATH + StringHelper.UTF8BytesToString(ref dataByKey.szGradeSmallPicPath);
		}
		...
		private static string GetTopUseHeroNames(ref COMDT_RANK_PASTSEASON_FIGHT_RECORD data, out List<COMDT_RANK_COMMON_USED_HERO> heroList)
		{
			...
			int num2 = (heroList.get_Count() <= 4) ? heroList.get_Count() : 4;
