...
namespace Assets.Scripts.GameSystem
{
	...
	public class CRoleInfo
	{
				...
				if (this.m_showGameRedDot != value)
				{
					...
					if (value)
					{
						if (PlayerPrefs.HasKey("GAMER_REDDOT"))
						{
							...
							if (@string.Contains(accountInfo.get_OpenId()))
							{
								...
								if (array != null)
								{
									...
									while (i < num)
									{
										if (array[i] == accountInfo.get_OpenId() && long.TryParse(array[i + 1], ref secondsFromUtcStart))
										{
					...
					else
					{
						...
						if (PlayerPrefs.HasKey("GAMER_REDDOT"))
						{
							...
							if (text.Contains(accountInfo.get_OpenId()))
							{
								...
								if (array2 != null)
								{
									...
									while (j < num3)
									{
										if (array2[j] == accountInfo.get_OpenId())
										{
							...
							{
								text = string.Format("{0}_{1}_{2}", text, accountInfo.get_OpenId(), num2);
							}
						}
						else
						{
							text = string.Format("{0}_{1}", accountInfo.get_OpenId(), num2);
						}
		...
		public uint BattlePower
		{
			get
			{
				ListView<CHeroInfo> listView = new ListView<CHeroInfo>(this.heroDic.get_Count());
				...
				while (enumerator.MoveNext())
				{
					...
					arg_33_0.Add(current.get_Value());
				}
				ushort num = (ushort)listView.get_Count();
				uint num2 = 0u;
				for (ushort num3 = 0; num3 < num; num3 += 1)
				{
					CHeroInfo cHeroInfo = listView.get_Item((int)num3);
		...
		public uint Exp
		{
			...
			set
			{
				...
				while (this.m_exp >= this.m_needExp)
				{
					...
					this.SetLevel(this.UInt32ChgAdjust(this.Level, 1), 0);
				}
			}
		}
		...
		public uint GoldCoin
		{
			...
			set
			{
				...
				Singleton<EventRouter>.get_instance().BroadCastEvent("MasterCurrencyChanged");
			}
		}
		...
		public ulong DianQuan
		{
			...
			set
			{
				...
				Singleton<EventRouter>.get_instance().BroadCastEvent("MasterAttributesChanged");
				Singleton<EventRouter>.get_instance().BroadCastEvent("MasterCurrencyChanged");
			}
		}
		...
		public byte MaterialDirectBuyLimit
		{
			...
			set
			{
				this.m_MaterialDirectBuyLimit = ((value >= 0) ? value : 0);
			}
		}
		...
		public COM_ACNT_NEWBIE_TYPE GameDifficult
		{
			get
			{
				return this.m_gameDifficult;
			}
			set
			{
				this.m_gameDifficult = value;
			}
		}
			...
			if (this.bNewbieAchieveChanged || resetOldPlayerGuided)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					for (int j = 0; j < 64; j++)
					{
						...
						{
							cSPkg.stPkgData.get_stUpdateClientBitsNtf().stClientBits.BitsDetail[i] += ((!this.NewbieAchieveBits.get_Item(64 * i + j)) ? 0uL : (1uL << j));
						}
					}
				}
		...
		public void SyncClientBitsToSvr()
		{
			if (this._clientBitsChanged)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					for (int j = 0; j < 64; j++)
					{
						cSPkg.stPkgData.get_stUpdNewClientBits().stClientBits.BitsDetail[i] += ((!this._clientBits.get_Item(64 * i + j)) ? 0uL : (1uL << j));
					}
				}
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		...
		private void SetInBattleNewbieBit(int iLevelId, uint ageId, bool bIsLastAge)
		{
			...
			if (!this.inBattleNewbieBits.TryGetValue(iLevelId, ref stInBattleLevelBits))
			{
				...
				stInBattleLevelBits.bReportFinished = ((!bIsLastAge) ? 0 : 1);
		...
		public void ReqSetInBattleNewbieBit(uint id, bool bIsLastAge, int time = 0)
		{
			...
			if (curLvelContext != null && curLvelContext.IsGameTypeGuide())
			{
				...
				cSPkg.stPkgData.get_stDyeInBattleNewbieBitReq().iLevelID = curLvelContext.m_mapID;
				cSPkg.stPkgData.get_stDyeInBattleNewbieBitReq().dwFinishedAgeID = id;
				cSPkg.stPkgData.get_stDyeInBattleNewbieBitReq().dwFinishTime = (uint)time;
				cSPkg.stPkgData.get_stDyeInBattleNewbieBitReq().bIsLastAge = ((!bIsLastAge) ? 0 : 1);
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		...
		public void SetPvELevelInfo(ref COMDT_ACNT_LEVEL_COMPLETE_DETAIL detail, uint ServerTime)
		{
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.PVE_LEVEL_DETAIL_CHANGED);
		}
		...
		public void InitHero(COMDT_HEROINFO heroInfo)
		{
			...
			{
				CHeroInfo cHeroInfo = new CHeroInfo();
				this.heroDic.Add(heroInfo.stCommonInfo.dwHeroID, cHeroInfo);
			}
			this.heroDic.get_Item(heroInfo.stCommonInfo.dwHeroID).Init(this.playerUllUID, heroInfo);
		}
		...
		public CHeroInfo GetHeroInfo(uint id, bool isIncludeValidExperienceHero = false)
		{
			if (isIncludeValidExperienceHero)
			{
				if (this.IsOwnHero(id) || this.IsValidExperienceHero(id))
				{
					return this.heroDic.get_Item(id);
				}
			}
			else if (this.IsOwnHero(id))
			{
		...
		public int GetHaveHeroCountWithoutBanHeroID(bool isIncludeValidExperienceHero, byte mapType, uint mapID)
		{
			...
			while (enumerator.MoveNext())
			{
				if (isIncludeValidExperienceHero)
				{
					...
					KeyValuePair<uint, CHeroInfo> current3 = enumerator.get_Current();
					if (!arg_B3_0.Contains(current3.get_Key()))
					{
						num++;
					}
				}
				else
				{
					...
					if (this.IsOwnHero(current4.get_Key()))
					{
						...
						if (!arg_F2_0.Contains(current5.get_Key()))
						{
							num++;
						}
					}
		...
		public bool CheckHeroBuyable(uint inHeroId, RES_SHOPBUY_COINTYPE coinType)
		{
			...
			GameDataMgr.heroShopInfoDict.TryGetValue(inHeroId, ref resHeroShop);
		...
		public void SetSkillPoint(uint pt, bool bRefreshTime = false, bool bDispathEvent = false)
		{
			...
			if (bDispathEvent)
			{
				Singleton<EventRouter>.get_instance().BroadCastEvent<uint>("SkillPointChange", this.m_skillPoint);
			}
		}
		...
		public void SetPvpLevel(uint value)
		{
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent("Chat_PlayerLevel_Set");
		}
		...
		public void SetAttributes(SCPKG_CMD_GAMELOGINRSP rsp)
		{
			...
			this.m_privilegeType = rsp.bPrivilege;
			this.m_gameDifficult = rsp.bAcntNewbieType;
			this.m_gender = rsp.bGender;
			this.getFriendCoinCnt = (int)rsp.bGetCoinNums;
			this.m_AcntOldType = rsp.bAcntOldType;
			...
			MonoSingleton<GPSSys>.get_instance().Clear();
			UT.CheckGPS();
			...
			CTimer timer = Singleton<CTimerManager>.get_instance().GetTimer(this.m_newDayTimer);
			if (timer == null)
			{
				this.m_newDayTimer = Singleton<CTimerManager>.get_instance().AddTimer((int)num, 1, new CTimer.OnTimeUpHandler(this.OnNewDayNtf));
			}
			else
			{
			...
			Singleton<CTaskSys>.get_instance().SetCardExpireTime(8, rsp.stMonthWeekCardInfo.dwMonthExpireTimeStamp);
			Singleton<CTaskSys>.get_instance().SetCardExpireTime(9, rsp.stMonthWeekCardInfo.dwWeekExpireTimeStamp);
			Singleton<CTaskSys>.get_instance().InitReport();
		...
		{
			uint num = Utility.GetNewDayDeltaSec(CRoleInfo.GetCurrentUTCTime()) * 1000u;
			CTimer timer = Singleton<CTimerManager>.get_instance().GetTimer(timerSequence);
			timer.ResetTotalTime((int)num);
			timer.Reset();
			timer.Resume();
			Singleton<EventRouter>.get_instance().BroadCastEvent(EventID.NEWDAY_NTF);
		}
		...
		public void SetGlobalRefreshTimer(uint serverTime, bool force = false)
		{
			...
			int num2 = (num - Convert.ToInt32(dateTime.get_TimeOfDay().get_TotalSeconds()) <= 0) ? (num - Convert.ToInt32(dateTime.get_TimeOfDay().get_TotalSeconds()) + 86400) : (num - Convert.ToInt32(dateTime.get_TimeOfDay().get_TotalSeconds()));
		...
		{
			if (drawType < this.m_freeDrawInfo.Length)
			{
				return this.m_freeDrawInfo[drawType];
			}
		...
		public void OnUpdate(ref SCDT_NTF_ACNT_INFO_UPD ntfAcntInfoUpd)
		{
			...
			{
			case 1:
				this.SetLevel(this.UInt32ChgAdjust(this.Level, ntfAcntInfoUpd.iUpdValChg), 0);
				break;
			case 2:
			...
			case 17:
				...
				Singleton<EventRouter>.get_instance().BroadCastEvent("MasterSymbolCoinChanged");
				break;
			case 18:
			{
				HuoyueData huoyue_data = Singleton<CTaskSys>.get_instance().model.huoyue_data;
				huoyue_data.day_curNum = (huoyue_data.week_curNum = this.UInt32ChgAdjust(huoyue_data.day_curNum, ntfAcntInfoUpd.iUpdValChg));
				Singleton<EventRouter>.get_instance().BroadCastEvent("TASK_HUOYUEDU_Change");
				break;
			}
			case 19:
			...
			case 21:
				...
				Singleton<EventRouter>.get_instance().BroadCastEvent("MasterJifenChanged");
				break;
			}
		}
		...
		public void OnLevelUp(SCPKG_NTF_ACNT_LEVELUP ntfAcntLevelUp)
		{
			...
			this.SetLevel(ntfAcntLevelUp.dwNewLevel, ntfAcntLevelUp.bFromType);
		...
		public void OnPvpLevelUp(SCPKG_NTF_ACNT_PVPLEVELUP ntfAcntPvpLevelUp)
		{
			...
			Singleton<EventRouter>.get_instance().BroadCastEvent("MasterPvpLevelChanged");
			Singleton<EventRouter>.get_instance().BroadCastEvent("MasterAttributesChanged");
		...
		public static void GetHeroPreLevleAndExp(uint HeroId, uint inDltExp, out int preLevel, out uint preExp)
		{
			...
			preLevel = 0;
			CHeroInfo cHeroInfo;
			if (!Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().heroDic.TryGetValue(HeroId, ref cHeroInfo))
			{
				return;
			}
		...
		{
			this.m_baseGuildInfo.guildState = rsp.stGuildBaseInfo.bGuildState;
		...
		{
			for (int i = 0; i < this.freeHeroSymbolList.get_Count(); i++)
			{
				if (this.freeHeroSymbolList.get_Item(i).dwHeroID == heroId)
				{
					return this.freeHeroSymbolList.get_Item(i);
				}
			}
			return null;
		}
		...
		public byte GetFreeHeroSymbolId(uint heroId)
		{
			for (int i = 0; i < this.freeHeroSymbolList.get_Count(); i++)
			{
				if (this.freeHeroSymbolList.get_Item(i).dwHeroID == heroId)
				{
					return this.freeHeroSymbolList.get_Item(i).bSymbolPageWear;
				}
			}
			return 0;
		}
		...
		public uint GetFreeHeroWearSkinId(uint heroId)
		{
			for (int i = 0; i < this.freeHeroSymbolList.get_Count(); i++)
			{
				if (this.freeHeroSymbolList.get_Item(i).dwHeroID == heroId)
				{
					return (uint)this.freeHeroSymbolList.get_Item(i).wSkinID;
				}
			}
			return 0u;
		}
		...
		public void SetFreeHeroWearSkinId(uint heroId, uint skinId)
		{
			if (this.IsFreeHero(heroId))
			{
				for (int i = 0; i < this.freeHeroSymbolList.get_Count(); i++)
				{
					if (this.freeHeroSymbolList.get_Item(i).dwHeroID == heroId)
					{
						this.freeHeroSymbolList.get_Item(i).wSkinID = (ushort)skinId;
						return;
					}
				}
		...
		public void SetFreeHeroSymbolId(uint heroId, byte symbolId)
		{
			for (int i = 0; i < this.freeHeroSymbolList.get_Count(); i++)
			{
				if (this.freeHeroSymbolList.get_Item(i).dwHeroID == heroId)
				{
					this.freeHeroSymbolList.get_Item(i).bSymbolPageWear = symbolId;
					return;
				}
			}
			COMDT_FREEHERO_INFO cOMDT_FREEHERO_INFO = new COMDT_FREEHERO_INFO();
		...
		public uint GetHeroWearSkinId(uint heroId)
		{
			if (this.heroDic.ContainsKey(heroId))
			{
				return this.heroDic.get_Item(heroId).m_skinInfo.GetWearSkinId();
			}
			if (this.IsFreeHero(heroId))
			{
		...
		public void OnAddHeroSkin(uint heroId, uint skinId)
		{
			...
			if (this.heroSkinDic.ContainsKey(heroId))
			{
				...
				ulong num2 = dictionary.get_Item(heroId);
				expr_1F.set_Item(heroId, num2 | num);
			}
			else
			{
		...
		public void OnWearHeroSkin(uint heroId, uint skinId)
		{
			if (this.heroDic.ContainsKey(heroId))
			{
				this.heroDic.get_Item(heroId).OnHeroSkinWear(skinId);
			}
			if (this.IsFreeHero(heroId))
			{
		...
		public ResPVPRatio GetCurDailyAdd()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, ResPVPRatio> current = enumerator.get_Current();
				uint key = current.get_Key();
				KeyValuePair<uint, ResPVPRatio> current2 = enumerator.get_Current();
		...
		{
			CHeroInfo cHeroInfo;
			return this.heroDic.TryGetValue(heroId, ref cHeroInfo) && !cHeroInfo.IsExperienceHero();
		}

		public bool IsExperienceHero(uint heroId)
		{
			CHeroInfo cHeroInfo;
			return this.heroDic.TryGetValue(heroId, ref cHeroInfo);
		}

		public bool IsValidExperienceHero(uint heroId)
		{
			CHeroInfo cHeroInfo;
			return this.heroDic.TryGetValue(heroId, ref cHeroInfo) && cHeroInfo.IsValidExperienceHero();
		}

		public int GetExperienceHeroLeftTime(uint heroId)
		{
			CHeroInfo cHeroInfo;
			if (this.heroDic.TryGetValue(heroId, ref cHeroInfo) && cHeroInfo.IsExperienceHero())
			{
		...
		{
			CHeroInfo cHeroInfo;
			if (this.heroDic.TryGetValue(heroId, ref cHeroInfo))
			{
		...
		public static void OnVipInfoRsp(CSPkg msg)
		{
			...
			if (masterRoleInfo != null)
			{
				masterRoleInfo.SetVipFlags(msg.stPkgData.get_stQQVIPInfoRsp().dwVIPFlag);
			}
		}
		...
		public static void OnDailyCheckDataNtf(CSPkg msg)
		{
			...
			if (masterRoleInfo != null)
			{
				masterRoleInfo.dailyPvpCnt = msg.stPkgData.get_stDailyCheckDataNtf().dwDailyPvpCnt;
			}
		}
		...
		public static void OnCoinLimitNtf(CSPkg msg)
		{
			...
			if (masterRoleInfo != null)
			{
				COMDT_PROFIT_LIMIT stProfitLimit = msg.stPkgData.get_stProfitLimitNtf().stProfitLimit;
				...
				while ((long)num < (long)((ulong)stProfitLimit.dwProfitNum))
				{
					...
					if (dwProfitType != 1u)
					{
						if (dwProfitType == 2u)
						{
							masterRoleInfo.m_goldWeekCur = stProfitLimit.astProfitUnion[num].stProfitDetail.get_stCoinLimit().dwCurCoin;
							int num2 = (int)GameDataMgr.globalInfoDatabin.GetDataByKey(90u).dwConfValue;
							num2 += stProfitLimit.astProfitUnion[num].stProfitDetail.get_stCoinLimit().iExtraCoin;
							if (num2 < 0)
							{
								num2 = 0;
							}
							masterRoleInfo.m_goldWeekLimit = (uint)num2;
							masterRoleInfo.m_goldWeekMask = stProfitLimit.astProfitUnion[num].stProfitDetail.get_stCoinLimit().dwCoinMask;
						}
					}
					else
					{
						masterRoleInfo.m_expWeekCur = stProfitLimit.astProfitUnion[num].stProfitDetail.get_stExpLimit().dwCurExp;
		...
		public static void ReciveCreditScoreInfo(CSPkg msg)
		{
			...
			if (masterRoleInfo != null)
			{
				masterRoleInfo.creditScore = msg.stPkgData.get_stNtfAcntCreditValue().dwCreditValue;
				masterRoleInfo.sumDelCreditValue = msg.stPkgData.get_stNtfAcntCreditValue().iSumDelCreditValue * -1;
				masterRoleInfo.mostDelCreditType = msg.stPkgData.get_stNtfAcntCreditValue().dwMostDelCreditType;
			}
		}
		...
		public static void OnNextFirstWinTimeChange(CSPkg msg)
		{
			...
			if (masterRoleInfo != null)
			{
				CSPKG_NEXTFIRSTWINSEC_NTF stNextFirstWinSecNtf = msg.stPkgData.get_stNextFirstWinSecNtf();
		...
		public void SetCoinGetLimitDailyCnt()
		{
			...
			if (GameDataMgr.svr2CltCfgDict.TryGetValue(1u, ref resGlobalInfo))
			{
				this.m_coinGetInfoDaily[0].LimitCntDaily = resGlobalInfo.dwConfValue;
			}
			if (GameDataMgr.svr2CltCfgDict.TryGetValue(2u, ref resGlobalInfo))
			{
				this.m_coinGetInfoDaily[1].LimitCntDaily = resGlobalInfo.dwConfValue;
			}
			uint num = 0u;
			if (GameDataMgr.svr2CltCfgDict.TryGetValue(3u, ref resGlobalInfo))
			{
				num = resGlobalInfo.dwConfValue;
			}
			if (GameDataMgr.svr2CltCfgDict.TryGetValue(4u, ref resGlobalInfo))
			{
		...
		public void CleanUpBattleRecord()
		{
			...
			Singleton<CPlayerPvpHistoryController>.get_instance().ClearHostData();
		}
