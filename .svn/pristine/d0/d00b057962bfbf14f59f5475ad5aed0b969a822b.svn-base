...
namespace Assets.Scripts.GameSystem
{
	...
	public class Day14CheckSystem : Singleton<Day14CheckSystem>
	{
		...
		public void OnLoginOpen(CUIEvent uiEvent)
		{
			...
			{
				bool flag = false;
				ListView<Activity> activityList = Singleton<ActivitySys>.GetInstance().GetActivityList((Activity actv) => actv.Entrance == 2);
		...
		private void InitUI()
		{
			...
			if (transform != null)
			{
				...
				if (component != null)
				{
					component.SetElementAmount(phaseList.get_Count());
				}
				if (this.m_SelectIDx < phaseList.get_Count())
				{
		...
		private void GetCheckParams()
		{
			if (this._curActivity != null && GameDataMgr.resWealParamDict.ContainsKey(this._curActivity.EntranceParam))
			{
				...
				bool flag = GameDataMgr.resWealParamDict.TryGetValue(this._curActivity.EntranceParam, ref resWealParam);
				if (flag)
				{
		...
		private void OnCheckItemEnable(CUIEvent uiEvent)
		{
			...
			ListView<ActivityPhase> phaseList = this._curActivity.PhaseList;
			bool flag = false;
			if (srcWidgetIndexInBelongedList >= 0 && srcWidgetIndexInBelongedList < phaseList.get_Count())
			{
				CheckInPhase checkInPhase = phaseList.get_Item(srcWidgetIndexInBelongedList) as CheckInPhase;
		...
		private void OnLeftUIItemEnable(CUIEvent uiEvent)
		{
			...
			num2--;
			if (num2 >= 0 && num2 < phaseList.get_Count())
			{
				CheckInPhase checkInPhase = phaseList.get_Item(num2) as CheckInPhase;
