...
namespace Assets.Scripts.UI
{
	public class CUIManager : Singleton<CUIManager>
	{
		...
		public void Update()
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				this.m_forms.get_Item(i).CustomUpdate();
				if (this.m_forms.get_Item(i).IsNeedClose())
				{
					if (this.m_forms.get_Item(i).TurnToClosed(false))
					{
						this.RecycleForm(i);
				...
				else if (this.m_forms.get_Item(i).IsClosed() && !this.m_forms.get_Item(i).IsInFadeOut())
				{
					this.RecycleForm(i);
					this.m_needSortForms = true;
					continue;
		...
		public void LateUpdate()
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				this.m_forms.get_Item(i).CustomLateUpdate();
			}
		...
		public void CloseForm(string formPath)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (string.Equals(this.m_forms.get_Item(i).m_formPath, formPath))
				{
					this.m_forms.get_Item(i).Close();
				}
			}
		}
		...
		public void CloseForm(CUIFormScript formScript)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (this.m_forms.get_Item(i) == formScript)
				{
					this.m_forms.get_Item(i).Close();
				}
			}
		...
		public void CloseForm(int formSequence)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (this.m_forms.get_Item(i).GetSequence() == formSequence)
				{
					this.m_forms.get_Item(i).Close();
				}
			}
		...
		public void CloseAllForm(string[] exceptFormNames = null, bool closeImmediately = true, bool clearFormPool = true)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				...
				if (exceptFormNames != null)
				{
					...
					{
						if (string.Equals(this.m_forms.get_Item(i).m_formPath, exceptFormNames[j]))
						{
							flag = false;
							break;
						}
			...
			{
				int k = 0;
				while (k < this.m_forms.get_Count())
				{
					if (this.m_forms.get_Item(k).IsNeedClose() || this.m_forms.get_Item(k).IsClosed())
					{
						if (this.m_forms.get_Item(k).IsNeedClose())
						{
							this.m_forms.get_Item(k).TurnToClosed(true);
						}
						this.RecycleForm(k);
					}
					else
		...
		{
			this.RemoveFromExistFormSequenceList(this.m_forms.get_Item(formIndex).GetSequence());
			this.RecycleForm(this.m_forms.get_Item(formIndex));
		...
		public bool HasForm()
		{
			return this.m_forms.get_Count() > 0;
		}
		...
		public CUIFormScript GetForm(string formPath)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (this.m_forms.get_Item(i).m_formPath.Equals(formPath) && !this.m_forms.get_Item(i).IsNeedClose() && !this.m_forms.get_Item(i).IsClosed())
				{
					return this.m_forms.get_Item(i);
				}
			}
			return null;
		}
		...
		public CUIFormScript GetForm(int formSequence)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (this.m_forms.get_Item(i).GetSequence() == formSequence && !this.m_forms.get_Item(i).IsNeedClose() && !this.m_forms.get_Item(i).IsClosed())
				{
					return this.m_forms.get_Item(i);
				}
			}
		...
		public void CloseGroupForm(int group)
		{
			...
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (this.m_forms.get_Item(i).m_group == group)
				{
					this.m_forms.get_Item(i).Close();
				}
			}
		...
		{
			for (int i = 0; i < this.m_pooledForms.get_Count(); i++)
			{
				Object.Destroy(this.m_pooledForms.get_Item(i).gameObject);
			}
		...
		public CUIFormScript GetTopForm()
		{
			...
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (!(this.m_forms.get_Item(i) == null))
				{
					if (cUIFormScript == null)
					{
						cUIFormScript = this.m_forms.get_Item(i);
					}
					else if (this.m_forms.get_Item(i).GetSortingOrder() > cUIFormScript.GetSortingOrder())
					{
						cUIFormScript = this.m_forms.get_Item(i);
					}
				}
		...
		private void ProcessFormList(bool sort, bool handleInputAndHide)
		{
			if (sort)
			{
				...
				for (int i = 0; i < this.m_forms.get_Count(); i++)
				{
					int formOpenOrder = this.GetFormOpenOrder(this.m_forms.get_Item(i).GetSequence());
					this.m_forms.get_Item(i).SetDisplayOrder(formOpenOrder);
				}
			}
		...
		private GameObject CreateForm(string formPrefabPath, bool useFormPool)
		{
			...
			if (useFormPool)
			{
				for (int i = 0; i < this.m_pooledForms.get_Count(); i++)
				{
					if (string.Equals(formPrefabPath, this.m_pooledForms.get_Item(i).m_formPath, 5))
					{
						this.m_pooledForms.get_Item(i).Appear(enFormHideFlag.HideByCustom, true);
						gameObject = this.m_pooledForms.get_Item(i).gameObject;
			...
			if (gameObject == null)
			{
				GameObject gameObject2 = (GameObject)Singleton<CResourceManager>.GetInstance().GetResource(formPrefabPath, typeof(GameObject), 3, false, false).m_content;
				if (gameObject2 == null)
				{
					return null;
				}
		...
		private void RecycleForm(CUIFormScript formScript)
		{
			...
			else
			{
				try
				{
					...
					Object.Destroy(formScript.gameObject);
				}
				catch (Exception ex)
				{
		...
		private CUIFormScript GetUnClosedForm(string formPath)
		{
			for (int i = 0; i < this.m_forms.get_Count(); i++)
			{
				if (this.m_forms.get_Item(i).m_formPath.Equals(formPath) && !this.m_forms.get_Item(i).IsClosed())
				{
					return this.m_forms.get_Item(i);
				}
			}
		...
		{
			bool flag = true;
			for (int i = this.m_forms.get_Count() - 1; i >= 0; i--)
			{
				if (!this.m_forms.get_Item(i).m_disableInput && !this.m_forms.get_Item(i).IsHided())
				{
					GraphicRaycaster graphicRaycaster = this.m_forms.get_Item(i).GetGraphicRaycaster();
					if (graphicRaycaster != null)
					{
						graphicRaycaster.enabled = flag;
					}
					if (this.m_forms.get_Item(i).m_isModal && flag)
					{
						flag = false;
					}
				}
		...
		private void UpdateFormHided()
		{
			...
			for (int i = this.m_forms.get_Count() - 1; i >= 0; i--)
			{
				if (flag)
				{
					this.m_forms.get_Item(i).Hide(enFormHideFlag.HideByOtherForm, false);
				}
				else
				{
					this.m_forms.get_Item(i).Appear(enFormHideFlag.HideByOtherForm, false);
				}
				if (!flag && !this.m_forms.get_Item(i).IsHided() && this.m_forms.get_Item(i).m_hideUnderForms)
				{
					flag = true;
				}
			}
		...
		public void OpenTips(string strContent, bool bReadDatabin = false, float timeDuration = 1.5f, GameObject referenceGameObject = null, params object[] replaceArr)
		{
			...
			if (cUIFormScript != null && referenceGameObject != null)
			{
				...
				if (component2 != null && rectTransform != null)
				{
					...
					float num = Math.Abs(CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, array[2]).x - CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, array[0]).x);
					float num2 = Math.Abs(CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, array[2]).y - CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, array[0]).y);
					Vector2 screenPoint = new Vector2(CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, array[0]).x + num / 2f, CUIUtility.WorldToScreenPoint(Singleton<CUIManager>.get_instance().FormCamera, array[0]).y + num2 / 2f);
			...
			Singleton<CSoundManager>.get_instance().PostEvent("UI_Click", null);
		}
		...
		public void OpenAwardTip(CUseable[] items, string title = null, bool playSound = false, enUIEventID eventID = enUIEventID.None, bool displayAll = false, bool forceNotGoToBag = false, string formPath = "Form_Award")
		{
			...
			if (!forceNotGoToBag && num == 1 && items[0].m_type == 2)
			{
				...
				if (cItem.m_itemData.bType == 4 || cItem.m_itemData.bType == 1 || cItem.m_itemData.bType == 11)
				{
					...
					CUseable useableByBaseID = useableContainer.GetUseableByBaseID(2, cItem.m_baseID);
		...
		public bool OpenSearchBox(string title, string recommendItemPath, string resultItemPath, uint recommendHandleCMD, uint searchHandleCMD, string evtCallbackSearch, string evtCallbackRecommend, string evtCallbackRecommendSingleEnable = null)
		{
			...
			GameObject gameObject = (GameObject)Singleton<CResourceManager>.GetInstance().GetResource(recommendItemPath, typeof(GameObject), 4, false, false).m_content;
