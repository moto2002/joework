...
namespace AGE
{
	...
	public class SpawnMonsterTick : TickEvent
	{
		...
		private void SpawnMonster(Action _action)
		{
			...
			string text = StringHelper.UTF8BytesToString(ref dataCfgInfoByCurLevelDiff.szCharacterInfo) + ".asset";
			...
			if (exists)
			{
				...
				VInt3 location = this.tarActor.get_handle().location;
				VInt3 forward = this.tarActor.get_handle().forward;
