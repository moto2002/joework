...
namespace Assets.Scripts.GameSystem
{
	public class CMallHeroController
	{
		...
		private void OnHeroItemEnable(CUIEvent uiEvent)
		{
			...
			if (uiEvent.m_srcWidget != null)
			{
				CMallItem item = new CMallItem(this.m_heroList.get_Item(srcWidgetIndexInBelongedList).dwCfgID, CMallItem.IconType.Normal);
		...
		private void RefreshHeroListObject(CUIFormScript form)
		{
			...
			componetInChild.SetElementAmount(this.m_heroList.get_Count());
		}
		...
		private void ResetHeroList()
		{
			...
			for (int i = 0; i < allHeroListAtShop.get_Count(); i++)
			{
				CMallItem cMallItem = new CMallItem(allHeroListAtShop.get_Item(i).dwCfgID, CMallItem.IconType.Normal);
				if (this.m_heroJobType == enHeroJobType.All || allHeroListAtShop.get_Item(i).bMainJob == (byte)this.m_heroJobType || allHeroListAtShop.get_Item(i).bMinorJob == (byte)this.m_heroJobType)
				{
					if (this.m_notOwnFlag)
					{
						if (!cMallItem.Owned(false))
						{
							this.m_heroList.Add(allHeroListAtShop.get_Item(i));
						}
					}
					else
					{
		...
		public int GetHeroIndexByConfigId(uint heroID = 0u)
		{
			...
			{
				if (this.m_heroList.get_Item(i) != null && this.m_heroList.get_Item(i).dwCfgID == heroID)
				{
					return i;
				}
			}
		...
		public int GetHeroListCount()
		{
			...
			return this.m_heroList.get_Count();
		}
	}
}
