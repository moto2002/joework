...
namespace AGE
{
	public class CurvlData
	{
		...
		{
			this.InitData();
			int count = this.transNodeObjs.get_Count();
			int i;
			for (i = 0; i < this.nodeLst.get_Count(); i++)
			{
				...
				{
					gameObject = this.transNodeObjs.get_Item(i);
					if (gameObject == null)
					{
						gameObject = (ActionManager.InstantiateObject(this.transObjPrefb) as GameObject);
						gameObject.name = name;
						...
						this.transNodeObjs.set_Item(i, gameObject);
					}
				}
			...
			{
				this.transNodeObjs.get_Item(k).SetActive(false);
			}
		}

		public void ReverseNodes()
		{
			if (this.nodeLst != null && this.nodeLst.get_Count() > 1)
			{
				this.nodeLst.Reverse();
				for (int i = this.transNodeObjs.get_Count() - 1; i >= 0; i--)
				{
					if (!this.transNodeObjs.get_Item(i).activeInHierarchy)
					{
						ActionManager.DestroyGameObject(this.transNodeObjs.get_Item(i));
		...
		public void ReplaceNodeObjectPrefab(GameObject obj)
		{
			...
			for (int i = 0; i < this.transNodeObjs.get_Count(); i++)
			{
				ActionManager.DestroyGameObject(this.transNodeObjs.get_Item(i));
			}
			this.transNodeObjs.Clear();
		...
		{
			if (this.transNodeObjs == null || index < 0 || index >= this.transNodeObjs.get_Count())
			{
				return null;
			}
			return this.transNodeObjs.get_Item(index);
		}
		...
		{
			if (this.nodeLst == null || index < 0 || index >= this.nodeLst.get_Count())
			{
				return null;
			}
			return this.nodeLst.get_Item(index);
		}
		...
		public TransNode InsertNode(int index, bool needRefreshGfxData)
		{
			...
			TransNode transNode = new TransNode();
			if (index >= this.nodeLst.get_Count())
			{
				if (this.nodeLst.get_Count() > 0)
				{
					TransNode transNode2 = this.nodeLst.get_Item(this.nodeLst.get_Count() - 1);
			...
			{
				TransNode transNode3 = this.nodeLst.get_Item(index);
		...
		private void CreateCurvlPoints(bool useUnifyInterp)
		{
			...
			int count = this.nodeLst.get_Count();
			...
			for (int i = 0; i < count; i++)
			{
				if (this.closeCurvl || i != 0)
				{
					...
					Vector3 pos = this.nodeLst.get_Item(num).pos;
					Vector3 pos2 = this.nodeLst.get_Item(i).pos;
					Vector3 pos3 = this.nodeLst.get_Item(num2).pos;
					Vector3 pos4 = this.nodeLst.get_Item(num3).pos;
					if ((useUnifyInterp && !this.isCubic) || (!useUnifyInterp && !this.nodeLst.get_Item(i).isCubic))
					{
		...
		private void UpdateNodeShowState(bool show)
		{
			...
			for (int i = 0; i < this.transNodeObjs.get_Count(); i++)
			{
				if (i < count)
				{
					this.transNodeObjs.get_Item(i).SetActive(show);
				}
				else
				{
					this.transNodeObjs.get_Item(i).SetActive(false);
				}
			}
		}
