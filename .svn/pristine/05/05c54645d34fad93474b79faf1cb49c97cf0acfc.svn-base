...
namespace Assets.Scripts.GameSystem
{
	...
	public class RankingSystem : Singleton<RankingSystem>
	{
		...
		private COMDT_RANKING_LIST_ITEM_EXTRA_MASTER_HERO GetSelfHeroMasterInfo(uint heroId)
		{
			...
			{
				if (localRankingInfo.ListInfo.astItemDetail[num].stExtraInfo.stDetailInfo.get_stMasterHero().stAcntInfo.ullUid == masterRoleInfo.playerUllUID)
				{
					return localRankingInfo.ListInfo.astItemDetail[num].stExtraInfo.stDetailInfo.get_stMasterHero();
				}
				num++;
			}
			return null;
		...
		{
			Singleton<CLobbySystem>.get_instance().ShowHideRankingBtn(false);
		...
		protected void RetrieveSortedFriendRankList()
		{
			...
			this._sortedFriendRankList = Singleton<CFriendContoller>.get_instance().model.GetSortedRankingFriendList(RankingSystem.ConvertRankingServerType(this._curRankingType));
		}
		...
		{
			this.Clear();
			Singleton<CLobbySystem>.get_instance().ShowHideRankingBtn(true);
		}
		...
		private void UpdateGodTitle()
		{
			...
			if (this._curRankingType == RankingSystem.RankingType.God)
			{
				...
				{
					if (this.m_heroList.get_Item(i).dwCfgID == this.m_heroMasterId)
					{
						RankingView.UpdateRankGodTitle(this.m_heroList.get_Item(i));
						break;
					}
				}
			}
		...
		protected void OnChangeRankToLadder(CUIEvent uiEvent)
		{
			...
			this.m_uiTlog[cOM_APOLLO_TRANK_SCORE_TYPE].dwType = cOM_APOLLO_TRANK_SCORE_TYPE;
			this.m_uiTlog[cOM_APOLLO_TRANK_SCORE_TYPE].dwCnt += 1u;
		}
		...
		{
			if (Singleton<CFunctionUnlockSys>.get_instance().FucIsUnlock(9))
			{
				this.TryToChangeRankType(RankingSystem.RankingType.Arena);
				RankingView.HideAllRankMenu();
				Singleton<CUINewFlagSystem>.GetInstance().SetNewFlagForArenaRankBtn(false);
		...
		protected void OnChangeRankToGod(CUIEvent uiEvent)
		{
			...
			RankingView.HideAllRankMenu();
			Singleton<CUINewFlagSystem>.GetInstance().SetNewFlagForGodRankBtn(false);
		...
		private static ListView<ResHeroCfgInfo> GetHeroList(enHeroJobType jobType, bool bOwn = false)
		{
			...
			if (masterRoleInfo != null)
			{
				...
				for (int i = 0; i < allHeroList.get_Count(); i++)
				{
					...
					{
						listView.Add(allHeroList.get_Item(i));
					}
				}
			}
			return listView;
		...
		private void OnGodDetailTabClick(CUIEvent uiEvent)
		{
			...
			{
				RankingView.OnRankGodDetailTab(selectedIndex, localRankingInfo.ListInfo.astItemDetail[this.m_curRankGodViewIndex].stExtraInfo.stDetailInfo.get_stMasterHero(), this.m_heroMasterId);
			}
		}
		...
		private void OnGodHeroChgItemEnable(CUIEvent uiEvent)
		{
			...
			{
				RankingView.OnHeroItemEnable(uiEvent, this.m_heroList.get_Item(srcWidgetIndexInBelongedList));
			}
		}
		...
		public int GetMyFriendRankNo()
		{
			...
			ListView<COMDT_FRIEND_INFO> sortedRankingFriendList = Singleton<CFriendContoller>.get_instance().model.GetSortedRankingFriendList(RankingSystem.ConvertRankingServerType(rankingType));
			uint num = (uint)(sortedRankingFriendList.get_Count() + 1);
			uint num2 = 0u;
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo();
			int num3 = 0;
			while ((long)num3 < (long)((ulong)num))
			{
				uint num4 = 0u;
				result = num3;
				if (num3 < sortedRankingFriendList.get_Count())
				{
					num4 = sortedRankingFriendList.get_Item(num3).RankVal[RankingSystem.ConvertRankingServerType(rankingType)];
					num2 = sortedRankingFriendList.get_Item(num3).dwPvpLvl;
				}
				if (num3 < sortedRankingFriendList.get_Count() && dwScore >= num4 && (rankingType != RankingSystem.RankingType.Ladder || dwScore != num4 || masterRoleInfo.PvpLevel >= num2))
				{
					break;
				}
				num3++;
		...
		protected void UpdateAllElementInView()
		{
			...
			else
			{
				...
				num = (uint)(this._sortedFriendRankList.get_Count() + 1);
				...
				while ((long)num2 < (long)((ulong)num))
				{
					...
					uint num3 = 0u;
					uint num4 = 0u;
					if (num2 < this._sortedFriendRankList.get_Count())
					{
						num3 = this._sortedFriendRankList.get_Item(num2).RankVal[RankingSystem.ConvertRankingServerType(this._curRankingType)];
						num4 = this._sortedFriendRankList.get_Item(num2).dwPvpLvl;
					}
					if (num2 < this._sortedFriendRankList.get_Count() && dwScore >= num3 && (this._curRankingType != RankingSystem.RankingType.Ladder || dwScore != num3 || masterRoleInfo.PvpLevel >= num4))
					{
						break;
					}
					num2++;
			...
			{
				this._form.m_formWidgets[15].CustomSetActive(false);
				this._form.m_formWidgets[22].CustomSetActive(num == 0u);
			}
			else
			{
		...
		private void UpdateOneElement(GameObject objElement, int viewIndex)
		{
			...
			if (this._curSubViewType == RankingSystem.RankingSubView.Friend)
			{
				if (this.IsMyFriendRankIndex(viewIndex))
				{
					...
					if (selfInfo != null && Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo() != null)
					{
						num = selfInfo.dwScore;
						text = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().Name;
						num2 = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().PvpLevel;
						serverUrl = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().HeadUrl;
						privilegeType = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().m_privilegeType;
						num3 = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().playerUllUID;
						num4 = (uint)MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.logicWorldID;
						level = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().GetNobeInfo().stGameVipClient.dwCurLevel;
						headIdx = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().GetNobeInfo().stGameVipClient.dwHeadIconId;
						num5 = 913913u;
						RankingView.SetGameObjChildText(this._myselfInfo, "NameGroup/PlayerName", Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().Name);
						RankingView.SetGameObjChildText(this._myselfInfo, "PlayerLv", string.Format("Lv.{0}", Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().PvpLevel.ToString(CultureInfo.get_InvariantCulture())));
					}
				}
				else
				{
					int num6 = this.ConvertFriendRankIndex(viewIndex);
					if (this._sortedFriendRankList != null && num6 < this._sortedFriendRankList.get_Count() && num6 >= 0)
					{
						COMDT_FRIEND_INFO cOMDT_FRIEND_INFO = this._sortedFriendRankList.get_Item(num6);
						if (cOMDT_FRIEND_INFO != null)
						{
							num = cOMDT_FRIEND_INFO.RankVal[RankingSystem.ConvertRankingServerType(this._curRankingType)];
							...
							privilegeType = cOMDT_FRIEND_INFO.bPrivilege;
							CFriendModel.FriendInGame friendInGaming = Singleton<CFriendContoller>.get_instance().model.GetFriendInGaming(num3, num4);
			...
			else
			{
				...
				if (cSDT_RANKING_LIST_ITEM_INFO != null)
				{
					...
					if (rankItemDetailInfo != null)
					{
						...
						privilegeType = rankItemDetailInfo.bPrivilege;
			...
			{
				int num10 = CLadderSystem.ConvertEloToRank(num);
				int curXingByEloAndRankLv = CLadderSystem.GetCurXingByEloAndRankLv(num, (uint)num10);
				if (num2 >= CLadderSystem.REQ_PLAYER_LEVEL && ((!this.IsMyFriendRankIndex(viewIndex)) ? Singleton<CLadderSystem>.GetInstance().IsHaveFightRecord(false, num10, curXingByEloAndRankLv) : Singleton<CLadderSystem>.GetInstance().IsHaveFightRecord(true, -1, -1)))
				{
					CLadderView.ShowRankDetail(component.LadderGo, (byte)num10, this.GetRankClass(num3), 1u, false, true, false, true, true);
			...
			COMDT_FRIEND_INFO info = Singleton<CFriendContoller>.get_instance().model.GetInfo(CFriendModel.FriendType.GameFriend, num3, num4);
			COMDT_FRIEND_INFO info2 = Singleton<CFriendContoller>.get_instance().model.GetInfo(CFriendModel.FriendType.SNS, num3, num4);
			bool flag = info != null;
			...
			{
				uint num11 = (uint)Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().playerUllUID;
				component.AddFriend.CustomSetActive(!flag && !flag2 && (ulong)num11 != num3);
			}
			CUIEventScript component2 = component.SendCoin.GetComponent<CUIEventScript>();
			if (flag2)
			{
				bool bEnable = Singleton<CFriendContoller>.get_instance().model.HeartData.BCanSendHeart(cOMDT_ACNT_UNIQ);
				...
				if (component.Online != null)
				{
					...
					{
						componetInChild.text = ((info2.bIsOnline == 0) ? Singleton<CTextManager>.GetInstance().GetText("Common_Offline") : string.Format("<color=#00ff00>{0}</color>", Singleton<CTextManager>.get_instance().GetText("Common_Online")));
					}
				}
			}
			else if (flag)
			{
				bool bEnable2 = Singleton<CFriendContoller>.get_instance().model.HeartData.BCanSendHeart(cOMDT_ACNT_UNIQ);
				...
				if (component.Online != null)
				{
					...
					{
						componetInChild2.text = ((info.bIsOnline == 0) ? Singleton<CTextManager>.GetInstance().GetText("Common_Offline") : string.Format("<color=#00ff00>{0}</color>", Singleton<CTextManager>.get_instance().GetText("Common_Online")));
					}
				}
			}
			else
		...
		protected void UpdateSelfInfo()
		{
			...
			case RankingSystem.RankingType.Ladder:
				...
				{
					int num3 = CLadderSystem.ConvertEloToRank(selfInfo.dwScore);
					CLadderView.ShowRankDetail(component.LadderGo, (byte)num3, Singleton<CRoleInfoManager>.GetInstance().GetSelfRankClass(), 1u, false, true, false, true, true);
					int curXingByEloAndRankLv = CLadderSystem.GetCurXingByEloAndRankLv(selfInfo.dwScore, (uint)num3);
					component.LadderXing.GetComponent<Text>().text = string.Format("x{0}", curXingByEloAndRankLv);
				}
				else
				{
			...
			case RankingSystem.RankingType.God:
			{
				...
				{
					num4 = selfHeroMasterInfo.dwWinCnt * 100u / selfHeroMasterInfo.dwGameCnt;
					num5 = selfHeroMasterInfo.dwWinCnt;
				}
				else
				{
					...
					{
						num4 = heroInfo.m_masterHeroWinCnt * 100u / heroInfo.m_masterHeroFightCnt;
						num5 = heroInfo.m_masterHeroWinCnt;
					}
				}
		...
		protected void OnHoldDetail(CUIEvent uiEvent)
		{
			...
			if (this._curSubViewType == RankingSystem.RankingSubView.Friend && this._curRankingType != RankingSystem.RankingType.God)
			{
				...
				COMDT_FRIEND_INFO cOMDT_FRIEND_INFO = this._sortedFriendRankList.get_Item(num3);
			...
			else
			{
				...
				if (rankItemDetailInfo != null)
				{
					...
					if (num == Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().playerUllUID && num2 == MonoSingleton<TdirMgr>.GetInstance().SelectedTdir.logicWorldID)
					{
						return;
					}
				}
		...
		protected void OnAddFriend(CUIEvent uiEvent)
		{
			...
			if (this._curSubViewType == RankingSystem.RankingSubView.Friend)
			{
				if ((long)this._curRankingListIndex != (long)((ulong)this._myLastFriendRank))
				{
					int num = this.ConvertFriendRankIndex(this._curRankingListIndex);
					COMDT_FRIEND_INFO cOMDT_FRIEND_INFO = this._sortedFriendRankList.get_Item(num);
					ullUid = cOMDT_FRIEND_INFO.stUin.ullUid;
					dwLogicWorldId = (int)cOMDT_FRIEND_INFO.stUin.dwLogicWorldId;
				}
				else
				{
					ullUid = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo().playerUllUID;
			...
			Singleton<CFriendContoller>.get_instance().Open_Friend_Verify(ullUid, (uint)dwLogicWorldId, false, 0, -1, true);
		}
		...
		private void OnArenaAddFriend(CUIEvent uiEvent)
		{
			...
			Singleton<CFriendContoller>.get_instance().Open_Friend_Verify(commonUInt64Param, (uint)tag, false, 0, -1, true);
		}
		...
		protected static RankingSystem.RankingType ConvertRankingLocalType(COM_APOLLO_TRANK_SCORE_TYPE rankType)
		{
			...
			{
			case 1:
				result = RankingSystem.RankingType.Power;
				return result;
			case 2:
				result = RankingSystem.RankingType.PvpRank;
				return result;
			case 3:
			case 4:
				IL_40:
				...
				result = RankingSystem.RankingType.God;
				return result;
			case 5:
				result = RankingSystem.RankingType.HeroNum;
				return result;
			case 6:
				result = RankingSystem.RankingType.SkinNum;
				return result;
			case 7:
				result = RankingSystem.RankingType.Ladder;
				return result;
			case 8:
				result = RankingSystem.RankingType.Achievement;
				return result;
			case 9:
				result = RankingSystem.RankingType.WinCount;
				return result;
			case 10:
				result = RankingSystem.RankingType.ConWinCount;
				return result;
			case 11:
				result = RankingSystem.RankingType.ConsumeQuan;
				return result;
			case 12:
				result = RankingSystem.RankingType.GameVip;
				return result;
			case 13:
				result = RankingSystem.RankingType.MentorPoint;
				return result;
			}
			goto IL_40;
		}
		...
		protected void ReqRankingList(COM_APOLLO_TRANK_SCORE_TYPE rankType, int subType = 0)
		{
			...
			cSPkg.stPkgData.get_stGetRankingListReq().iSubType = subType;
		...
		protected void ReqRankingDetail(int listIndex, bool isSelf = false, RankingSystem.RankingType rankType = RankingSystem.RankingType.None)
		{
			...
			if (isSelf)
			{
				cSPkg.stPkgData.get_stGetRankingAcntInfoReq().bNumberType = RankingSystem.ConvertRankingServerType(rankType);
			}
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, true);
		}
		...
		{
			CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(2604u);
			cSPkg.stPkgData.get_stGetRankingAcntInfoReq().bNumberType = 7;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}

		public void OnGetRankingList(SCPKG_GET_RANKING_LIST_RSP rsp)
		{
			Singleton<RankingSystem>.get_instance().ImpResRankingList(rsp);
		}
		...
		public void ImpResRankingList(SCPKG_GET_RANKING_LIST_RSP rsp)
		{
			...
			RankingSystem.RankingType rankingType = RankingSystem.ConvertRankingLocalType(rsp.stRankingListDetail.get_stOfSucc().bNumberType);
			if (rsp.stRankingListDetail.get_stOfSucc().iStart == 1)
			{
				this._rankingInfo[(int)rankingType].LastRetrieveTime = (uint)CRoleInfo.GetCurrentUTCTime();
				if (rsp.stRankingListDetail.get_stOfSucc().bNumberType == 7)
				{
					this._rankingInfo[(int)rankingType].ListInfo = this.GetLadderRankingInfoList(rsp.stRankingListDetail.get_stOfSucc());
				}
				else if (rsp.stRankingListDetail.get_stOfSucc().bNumberType == 65)
				{
					CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.get_instance().GetMasterRoleInfo();
					uint dwSubType = rsp.dwSubType;
					RankingSystem.LocalRankingInfo localRankingInfo = default(RankingSystem.LocalRankingInfo);
					localRankingInfo.ListInfo = rsp.stRankingListDetail.get_stOfSucc();
					...
					while ((long)num < (long)((ulong)localRankingInfo.ListInfo.dwItemNum))
					{
						COMDT_RANKING_LIST_ITEM_EXTRA_PLAYER stAcntInfo = localRankingInfo.ListInfo.astItemDetail[num].stExtraInfo.stDetailInfo.get_stMasterHero().stAcntInfo;
				...
				{
					this._rankingInfo[(int)rankingType].ListInfo = rsp.stRankingListDetail.get_stOfSucc();
				}
				this._rankingListReady = true;
			}
			else if (rsp.stRankingListDetail.get_stOfSucc().iStart == 101)
			{
				this._rankingInfo[(int)rankingType].LastRetrieveTime = (uint)CRoleInfo.GetCurrentUTCTime();
				this._rankingInfo[(int)rankingType].BackupListInfo = rsp.stRankingListDetail.get_stOfSucc();
				this._rankingBackupListReady = true;
			}
		...
		{
			if (rsp.stRankingListDetail.get_stOfSucc().bNumberType != 64)
			{
				return;
			}
			this._dailyRankMatchInfo = rsp.stRankingListDetail.get_stOfSucc();
		}
		...
		private CSDT_RANKING_LIST_SUCC GetLadderRankingInfoList(CSDT_RANKING_LIST_SUCC realTimeInfo)
		{
			...
			while ((long)num2 < (long)((ulong)this._dailyRankMatchInfo.dwItemNum))
			{
				...
				{
					if (this._dailyRankMatchInfo.astItemDetail[num2].stExtraInfo.stDetailInfo.get_stDailyRankMatch().ullUid == realTimeInfo.astItemDetail[num3].stExtraInfo.stDetailInfo.get_stDailyRankMatch().ullUid)
					{
						if (CLadderSystem.IsMaxRankGrade(realTimeInfo.astItemDetail[num3].stExtraInfo.stDetailInfo.get_stDailyRankMatch().stRankInfo.bGradeOfRank))
						{
		...
		private void OnCoinSent(SCPKG_CMD_DONATE_FRIEND_POINT data)
		{
			...
			int friendTypeFromSrvFriendType = CFriendContoller.GetFriendTypeFromSrvFriendType(data.bFriendType);
		...
		{
			Singleton<RankingSystem>.get_instance().ImpResRankingDetail(rsp);
		}

		public void ImpResRankingDetail(SCPKG_GET_RANKING_ACNT_INFO_RSP rsp)
		{
			RankingSystem.RankingType rankingType = RankingSystem.ConvertRankingLocalType(rsp.stAcntRankingDetail.get_stOfSucc().bNumberType);
			this._rankingInfo[(int)rankingType].LastRetrieveTime = (uint)CRoleInfo.GetCurrentUTCTime();
			this._rankingInfo[(int)rankingType].SelfInfo = rsp.stAcntRankingDetail.get_stOfSucc();
		...
		private COMDT_RANKING_LIST_ITEM_EXTRA_PLAYER GetRankItemDetailInfo(RankingSystem.RankingType rankType, int listIndex, uint subType = 0u)
		{
			...
			{
			case RankingSystem.RankingType.PvpRank:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stPvpExp();
			case RankingSystem.RankingType.Power:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stPower();
			case RankingSystem.RankingType.HeroNum:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stHeroNum();
			case RankingSystem.RankingType.SkinNum:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stSkinNum();
			case RankingSystem.RankingType.Ladder:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stLadderPoint();
			case RankingSystem.RankingType.Achievement:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stAchievement();
			case RankingSystem.RankingType.WinCount:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stWinGameNum();
			case RankingSystem.RankingType.ConWinCount:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stContinousWin();
			case RankingSystem.RankingType.ConsumeQuan:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stUseCoupons();
			case RankingSystem.RankingType.MentorPoint:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stMasterPoint();
			case RankingSystem.RankingType.GameVip:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stVipScore();
			case RankingSystem.RankingType.God:
				return cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.get_stMasterHero().stAcntInfo;
			default:
		...
		public uint GetRankClass(ulong playerUid)
		{
			if (this._dailyRankMatchInfo != null)
			{
				...
				{
					if (this._dailyRankMatchInfo.astItemDetail[num].stExtraInfo.stDetailInfo.get_stDailyRankMatch().ullUid == playerUid)
					{
		...
		public uint GetLadderRankNo(ulong playerUid)
		{
			...
			if (listInfo != null)
			{
				...
				{
					if (listInfo.astItemDetail[num].stExtraInfo.stDetailInfo.get_stDailyRankMatch().ullUid == playerUid)
					{
