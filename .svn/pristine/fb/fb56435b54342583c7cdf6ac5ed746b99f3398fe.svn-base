...
namespace Assets.Scripts.GameLogic
{
	public class BurnExpeditionController : Singleton<BurnExpeditionController>
	{
		...
		private void On_Burn_Challenge(CUIEvent uievent)
		{
			...
			ResLevelCfgInfo resLevelCfgInfo = BurnExpeditionUT.Get_LevelConfigInfo(Singleton<BurnExpeditionController>.get_instance().model.curSelect_LevelIndex);
			...
			Singleton<CHeroSelectBaseSystem>.get_instance().SetPVEDataWithBurnExpedition(dwBattleListID, reportInfo, StringHelper.UTF8BytesToString(ref resLevelCfgInfo.szName));
			Singleton<CHeroSelectBaseSystem>.get_instance().OpenForm(enSelectGameType.enBurning, mapHeroMaxCount, 0u, 0, 0);
		}
		...
		private void On_Burn_Reset(CUIEvent uievent)
		{
			...
			if (num > 0)
			{
				Singleton<CUIManager>.get_instance().OpenMessageBoxWithCancel(UT.GetText("Burn_Buy_Count_Tip"), enUIEventID.Burn_Reset_Ok, enUIEventID.Burn_Reset_Cancel, false);
			}
			else
			{
