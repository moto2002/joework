...
public class InBattleShortcutMenu
{
	...
	public void OpenForm()
	{
		...
		UT.SetTabList(Singleton<InBattleMsgMgr>.get_instance().title_list, 0, this.m_rightTablist);
	...
	{
		Singleton<CUIManager>.get_instance().OpenMessageBoxWithCancel(Singleton<CTextManager>.get_instance().GetText("InBattleShortcutMenu_UseDefault"), enUIEventID.InBatShortcut_UseDefault_Ok, enUIEventID.None, false);
	}

	private void On_InBatShortcut_UseDefault_Ok(CUIEvent uievent)
	{
		Singleton<CUIManager>.get_instance().OpenTips("InBattleShortcutMenu_UseDefault_OK", true, 1.5f, null, new object[0]);
		Singleton<InBattleMsgMgr>.get_instance().UseDefaultShortcut();
		this.RefreshLeft();
	}
	...
	{
		this._refresh_left_list(this.m_leftContentList, Singleton<InBattleMsgMgr>.get_instance().menuEntList);
	}
