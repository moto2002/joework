...
namespace Assets.Scripts.GameSystem
{
	public class FriendShower : MonoBehaviour
	{
		...
		public void ShowVerify(string text)
		{
			if (this.VerifyText != null)
			{
				this.VerifyText.text = Singleton<CFriendContoller>.get_instance().model.friend_static_text + text;
			}
		}
		...
		private void Showuid(ulong ullUid, uint dwLogicWorldId)
		{
			...
			if (Singleton<CFriendContoller>.get_instance().model.bShowUID)
			{
				if (transform != null)
				{
		...
		public void ShowInviteButton(bool isShow, bool isEnable)
		{
			...
			if (this.reCallText != null)
			{
				if (isEnable)
				{
					this.reCallText.text = Singleton<CTextManager>.get_instance().GetText("Friend_ReCall_Tips_1");
				}
				else
				{
					this.reCallText.text = Singleton<CTextManager>.get_instance().GetText("Friend_ReCall_Tips_2");
				}
			}
		...
		public void ShowRecommendGuild(bool isShow, bool isEnabled)
		{
			...
			if (isEnabled)
			{
				...
				{
					this.inviteGuildBtnText.text = Singleton<CFriendContoller>.get_instance().model.Guild_Recommend_txt;
				}
			}
			else
			{
				...
				if (this.inviteGuildBtnText != null)
				{
					this.inviteGuildBtnText.text = Singleton<CFriendContoller>.get_instance().model.Guild_Has_Recommended_txt;
				}
			}
		}
