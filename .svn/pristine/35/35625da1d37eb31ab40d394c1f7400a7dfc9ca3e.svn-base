	...
	public class FieldObj : MonoBehaviour
	{
		...
		public struct FOLevelGrid
		{
			...
			public void ReviseWorldPosToCenter(VInt3 inWorldPos, out VInt3 outWorldPos)
			{
				...
				int inCellY = 0;
				int inCellX;
				if (this.WorldPosToGrid(inWorldPos, out inCellX, out inCellY))
				{
		...
		public bool IsAreaPermanentLit(int x, int y, COM_PLAYERCAMP inCamp)
		{
			...
			if (this.QueryAttr(inCell, out sViewBlockAttr))
			{
				...
				if (sViewBlockAttr.LightType == 1 && inCamp == 1)
				{
					return true;
				}
				if (sViewBlockAttr.LightType == 2 && inCamp == 2)
				{
					return true;
				}
			}
		...
		public bool FindNearestNotBrickFromWorldLocNonFow(ref VInt3 newPos, ActorRoot ar)
		{
			...
			if (!this.LevelGrid.WorldPosToGrid(vInt, out vInt2.x, out vInt2.y))
			{
				return false;
			}
			bool flag = false;
			...
			if (this.FindNearestGrid(vInt2, vInt, FieldObj.EViewBlockType.Brick, 5, ar, out zero))
			{
				vInt2 = zero;
		...
		public bool FindNearestGrid(VInt2 inCenterCell, VInt3 inCenterPosWorld, FieldObj.EViewBlockType inBlockType, int inThicknessMax, ActorRoot ar, out VInt2 result)
		{
			result = VInt2.zero;
			FieldObj.SViewBlockAttr sViewBlockAttr = default(FieldObj.SViewBlockAttr);
			bool flag = false;
			...
			while (!flag && num <= inThicknessMax)
			{
				...
				if (list2.get_Count() > 0)
				{
					...
					{
						if (this.QueryAttr(list2.get_Item(i), out sViewBlockAttr) && sViewBlockAttr.BlockType == (byte)inBlockType)
						{
							list2.RemoveAt(i);
						}
					}
				...
				if (list2.get_Count() > 0)
				{
					...
					for (int j = 0; j < list2.get_Count(); j++)
					{
						...
						long sqrMagnitudeLong = (inCenterPosWorld - zero).get_sqrMagnitudeLong();
