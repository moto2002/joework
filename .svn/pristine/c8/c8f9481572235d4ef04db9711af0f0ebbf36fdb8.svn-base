...
namespace Assets.Scripts.GameLogic
{
	public class HorizonMarkerByFow : HorizonMarkerBase
	{
		internal class CampMarker
		{
			...
			public void AddHideMark(HorizonConfig.HideMark hm, int count)
			{
				if (count >= 0)
				{
					...
					expr_13_cp_0[(int)hm] = expr_13_cp_0[(int)hm] + (byte)count;
				}
				else
				{
					...
					else
					{
						...
						expr_4B_cp_0[(int)hm] = expr_4B_cp_0[(int)hm] - b;
					}
				}
			}
			...
			{
				this._hideMarks[(int)hm] = ((!bSet) ? 0 : 1);
			}
		...
		public int ExposeRadiusCache
		{
			...
			private set
			{
				Singleton<GameFowManager>.get_instance().m_pFieldObj.UnrealToGridX(value, out this.m_exposeRadiusCache);
			}
		}
		...
		private void ClearExposeTimer()
		{
			...
			for (int i = 0; i < num; i++)
			{
				if (this.m_exposeTimerSeq != null)
				{
					...
					{
						Singleton<CTimerManager>.get_instance().RemoveTimer(this.m_exposeTimerSeq[i]);
					}
					this.m_exposeTimerSeq[i] = -1;
				}
				if (this.m_showmarkTimerSeq != null)
				{
					...
					{
						Singleton<CTimerManager>.get_instance().RemoveTimer(this.m_showmarkTimerSeq[i]);
					}
					this.m_showmarkTimerSeq[i] = -1;
				}
			}
		...
		private void OnExposeOverCampOne(int inTimeSeq)
		{
			...
			{
				this.SetExposeMark(false, 1, this.actor.ActorControl.DoesIgnoreAlreadyLit());
		...
		private void OnExposeOverCampTwo(int inTimeSeq)
		{
			...
			{
				this.SetExposeMark(false, 2, this.actor.ActorControl.DoesIgnoreAlreadyLit());
				Singleton<CTimerManager>.get_instance().RemoveTimer(inTimeSeq);
		...
		private void OnShowMarkOverCampOne(int inTimeSeq)
		{
			...
			{
				this.AddShowMark(1, HorizonConfig.ShowMark.Jungle, -1);
				Singleton<CTimerManager>.get_instance().RemoveTimer(inTimeSeq);
		...
		private void OnShowMarkOverCampTwo(int inTimeSeq)
		{
			...
			{
				this.AddShowMark(2, HorizonConfig.ShowMark.Jungle, -1);
				Singleton<CTimerManager>.get_instance().RemoveTimer(inTimeSeq);
		...
		public override void ExposeAsAttacker(COM_PLAYERCAMP attackeeCamp, int inResetTime)
		{
			...
			CTimerManager instance = Singleton<CTimerManager>.get_instance();
		...
		public override void UpdateLogic(int delta)
		{
			...
			if (count > 0)
			{
				for (int i = count - 1; i >= 0; i--)
				{
					if (this.SubParObjList_.get_Item(i) == null)
					{
						this.SubParObjList_.RemoveAt(i);
					}
				}
		...
		{
			return targetCamp != null && this.m_bExposed[HorizonMarkerByFow.TranslateCampToIndex(targetCamp)];
		}

		public override int[] GetExposedCamps()
		{
			Array.Clear(this.m_exposeCampArr, 0, this.m_exposeCampArr.Length);
			if (this.actor.TheActorMeta.ActorCamp != null)
			{
		...
		{
			return targetCamp != null && this._campMarkers[HorizonMarkerByFow.TranslateCampToIndex(targetCamp)].HasHideMark(hm);
		}
		...
		{
			if (targetCamp == this.actor.TheActorMeta.ActorCamp)
			{
				return true;
			}
			if (targetCamp == null)
			{
				return !this.HasHideMark(1, HorizonConfig.HideMark.Skill) && !this.HasHideMark(2, HorizonConfig.HideMark.Skill);
			}
		...
		{
			if (targetCamp == this.actor.TheActorMeta.ActorCamp || targetCamp == null)
			{
				return false;
			}
			if (targetCamp == 3)
		...
		private void RefreshVisible()
		{
			...
			if (hostPlayer != null)
			{
				...
				this.actor.Visible = this.IsVisibleFor(hostPlayer.PlayerCamp);
