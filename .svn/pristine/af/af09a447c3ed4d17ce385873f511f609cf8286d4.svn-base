...
namespace Assets.Scripts.GameLogic
{
	public class AttackModeTargetSearcher : Singleton<AttackModeTargetSearcher>
	{
		...
		{
			if (!_actorPtr.get_handle().HorizonMarker.IsVisibleFor(this.curActorPtr.get_handle().TheActorMeta.ActorCamp) || !this.curActorPtr.get_handle().CanAttack(_actorPtr) || this.curActorPtr == _actorPtr || ((ulong)this.searchTypeMask & (ulong)(1L << (int)(this.curActorPtr.get_handle().TheActorMeta.ActorType & (ActorTypeDef)31))) > 0uL)
			{
				return;
			}
			if (TypeSearchCondition.Fit(_actorPtr, ActorTypeDef.Actor_Type_Hero))
			{
				...
				{
					this.heroFilter.Searcher(_actorPtr, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
				}
			}
			else if (TypeSearchCondition.Fit(_actorPtr, ActorTypeDef.Actor_Type_Organ))
			{
				if (DistanceSearchCondition.Fit(this.curActorPtr, _actorPtr, this.searchRadius))
				{
					this.organFiler.Searcher(_actorPtr, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
				}
			}
			else if (TypeSearchCondition.Fit(_actorPtr, ActorTypeDef.Actor_Type_Monster, false))
			{
				if (DistanceSearchCondition.Fit(this.curActorPtr, _actorPtr, this.searchRadius))
				{
					...
					{
						this.monsterNotInBattleFiler.Searcher(_actorPtr, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
					}
					else
					{
						this.monsterFiler.Searcher(_actorPtr, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
					}
				}
			}
			...
			{
				this.bossFiler.Searcher(_actorPtr, 5, new PropertyDelegate(TargetProperty.GetPropertyHpRate));
			}
		}
		...
		{
			if (!_actorPtr.get_handle().HorizonMarker.IsVisibleFor(this.curActorPtr.get_handle().TheActorMeta.ActorCamp) || !this.curActorPtr.get_handle().CanAttack(_actorPtr) || this.curActorPtr == _actorPtr)
			{
				return;
			}
			if (TypeSearchCondition.Fit(_actorPtr, ActorTypeDef.Actor_Type_Hero))
