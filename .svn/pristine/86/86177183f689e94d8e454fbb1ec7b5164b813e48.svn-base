...
namespace Assets.Scripts.GameSystem
{
	public class CVersionUpdateSystem : MonoSingleton<CVersionUpdateSystem>
	{
		...
		private static string[][] s_IIPSServerUrls = new string[][]
		{
			...
			new string[]
			{
				"exp.mtcls.qq.com:10011",
				"61.151.234.47:10011",
				"182.254.42.103:10011",
				"140.207.62.111:10011",
				"140.207.123.164:10011",
				"117.144.242.28:10011",
				"117.144.243.174:10011",
				"103.7.30.91:10011",
				"101.227.130.79:10011"
			},
			new string[]
			{
				"testa4.mtcls.qq.com:10001",
				"101.227.153.83:10001"
			},
			new string[]
			{
				string.Empty
		...
		{
			CVersionUpdateSystem.<CheckFirstExtractResource>c__Iterator31 <CheckFirstExtractResource>c__Iterator = new CVersionUpdateSystem.<CheckFirstExtractResource>c__Iterator31();
		...
		{
			CVersionUpdateSystem.<VersionUpdateComplete>c__Iterator32 <VersionUpdateComplete>c__Iterator = new CVersionUpdateSystem.<VersionUpdateComplete>c__Iterator32();
		...
		private void WriteCachedResourceInfo()
		{
			...
			{
				byte[] array = CFileManager.ReadFile(CVersionUpdateSystem.s_resourcePackerInfoSetFullPath);
				int num = 0;
				CResourcePackerInfoSet.ReadVersionAndBuildNumber(array, ref num, ref this.m_cachedResourceVersion, ref this.m_cachedResourceBuildNumber);
				this.m_cachedResourceType = (int)CVersionUpdateSystem.s_appType;
				array = new byte[1024];
				num = 0;
				int num2 = num;
				num += 4;
				CMemoryManager.WriteString(this.m_cachedResourceVersion, array, ref num);
				CMemoryManager.WriteString(this.m_cachedResourceBuildNumber, array, ref num);
				CMemoryManager.WriteByte((byte)this.m_cachedResourceType, array, ref num);
				CMemoryManager.WriteInt(num, array, ref num2);
				CFileManager.WriteFile(CVersionUpdateSystem.s_cachedResourceVersionFilePath, array, 0, num);
				this.UpdateUIVersionTextContent(CVersionUpdateSystem.s_appVersion, this.m_cachedResourceVersion);
			}
		}
		...
		{
			return CVersionUpdateSystem.s_platform == enPlatform.Android && ((this.m_appEnableYYB == 1 && Singleton<ApolloHelper>.GetInstance().GetChannelID() == 2002) || this.m_appEnableYYB == 2);
		}

		public void OnProgress(IIPSMobileVersionCallBack.VERSIONSTAGE curVersionStage, ulong totalSize, ulong nowSize)
		{
			this.UpdateUIProgress(enVersionUpdateFormWidget.Slider_SingleProgress, enVersionUpdateFormWidget.Text_SinglePercent, (totalSize != 0uL) ? (nowSize / totalSize) : 0f);
			...
			case 15:
				IL_55:
				if (curVersionStage == 94)
				{
					goto IL_FC;
				}
				if (curVersionStage != 95)
				{
					return;
				}
				goto IL_1E9;
		...
		public byte OnActionMsg(string msg)
		{
			...
			if (!string.IsNullOrEmpty(titleContentInMsg))
			{
				...
				string paramValueInContent2 = this.GetParamValueInContent(titleContentInMsg, "minversion");
				string paramValueInContent3 = this.GetParamValueInContent(titleContentInMsg, "maxversion");
				string paramValueInContent4 = this.GetParamValueInContent(titleContentInMsg, "EnableYYB");
				string paramValueInContent5 = this.GetParamValueInContent(titleContentInMsg, "AnnouncementUrl");
				string paramValueInContent6 = this.GetParamValueInContent(titleContentInMsg, "AnnouncementExtension");
				string paramValueInContent7 = this.GetParamValueInContent(titleContentInMsg, "NoIFS");
				...
				{
					this.m_appRecommendUpdateVersionMin = CVersion.GetVersionNumber(paramValueInContent2);
				}
				else
				{
					this.m_appRecommendUpdateVersionMin = 0u;
				...
				{
					this.m_appRecommendUpdateVersionMax = CVersion.GetVersionNumber(paramValueInContent3);
				}
				else
				{
					this.m_appRecommendUpdateVersionMax = 0u;
				...
				if (!string.IsNullOrEmpty(paramValueInContent4))
				{
					try
					{
						this.m_appEnableYYB = int.Parse(paramValueInContent4);
					}
					catch (Exception)
					{
						this.m_appEnableYYB = 0;
				...
				this.InitializeAnnouncement(paramValueInContent5, paramValueInContent6);
				if (!string.IsNullOrEmpty(paramValueInContent7))
				{
					try
					{
						this.m_appIsNoIFS = (int.Parse(paramValueInContent7) > 0);
					}
					catch (Exception)
					{
						this.m_appIsNoIFS = false;
			...
			string titleContentInMsg2 = this.GetTitleContentInMsg(msg, "@Resource");
			if (!string.IsNullOrEmpty(titleContentInMsg2))
			{
				string paramValueInContent8 = this.GetParamValueInContent(titleContentInMsg2, "needconfirm");
				string paramValueInContent9 = this.GetParamValueInContent(titleContentInMsg2, "size");
				string paramValueInContent10 = this.GetParamValueInContent(titleContentInMsg2, "AnnouncementUrl");
				string paramValueInContent11 = this.GetParamValueInContent(titleContentInMsg2, "AnnouncementExtension");
				string paramValueInContent12 = this.GetParamValueInContent(titleContentInMsg2, "CheckCompatibility");
				this.m_resourceDownloadNeedConfirm = false;
				if (!string.IsNullOrEmpty(paramValueInContent8))
				{
					this.m_resourceDownloadNeedConfirm = string.Equals(paramValueInContent8, "1");
				}
				this.m_resourceCheckCompatibility = true;
				if (!string.IsNullOrEmpty(paramValueInContent12))
				{
					this.m_resourceCheckCompatibility = string.Equals(paramValueInContent12, "1");
				}
				if (!this.m_useCurseResourceDownloadSize && !string.IsNullOrEmpty(paramValueInContent9))
				{
					try
					{
						this.m_resourceDownloadSize = uint.Parse(paramValueInContent9);
					}
					catch (Exception)
					{
						this.m_resourceDownloadSize = 0u;
				...
				this.InitializeAnnouncement(paramValueInContent10, paramValueInContent11);
			}
			return 1;
		}
		...
		private void OnAnnouncementListElementEnable(CUIEvent uiEvent)
		{
			...
			if (cUIListElementScript != null && this.m_announcementInfos != null && this.m_announcementInfos.get_Count() > uiEvent.m_srcWidgetIndexInBelongedList)
			{
				CAnnouncementInfo cAnnouncementInfo = this.m_announcementInfos.get_Item(uiEvent.m_srcWidgetIndexInBelongedList);
		...
		private void OnSwitchAnnouncementListElementToFront(CUIEvent uiEvent)
		{
			...
			if (widget != null)
			{
				...
				if (component != null)
				{
					...
					if (num >= 0 && num < this.m_announcementInfos.get_Count())
					{
		...
		private void OnAnnouncementListSelectChanged(CUIEvent uiEvent)
		{
			...
			else if (selectedIndex == this.m_announcementInfos.get_Count() - 1)
			{
				widget.CustomSetActive(true);
				widget2.CustomSetActive(false);
			}
		...
		{
			if (this.m_announcementInfos.get_Count() > 0)
			{
				return;
			}
		...
		private void CloseConfirmPanel()
		{
			...
			GameObject widget2 = this.m_versionUpdateFormScript.GetWidget(17);
			if (widget2 != null)
			{
				widget2.CustomSetActive(true);
			}
		...
		{
			if (this.m_versionUpdateFormScript == null || this.m_announcementInfos == null || this.m_announcementInfos.get_Count() <= 0 || this.m_isAnnouncementPanelOpened)
			{
				return;
			}
			...
			if (widget2 != null)
			{
				...
				if (component != null)
				{
					...
					for (int i = 0; i < this.m_announcementInfos.get_Count(); i++)
					{
						this.m_announcementInfos.get_Item(i).m_pointerSequence = component.GetElement();
					}
				}
			}
			...
			if (widget3 != null)
			{
				...
				if (component2 != null)
				{
					component2.SetElementAmount(this.m_announcementInfos.get_Count());
		...
		{
			if (index < 0 || index >= this.m_announcementInfos.get_Count())
			{
				return;
			}
			...
			if (index >= 0)
			{
				GameObject element = component.GetElement(this.m_announcementInfos.get_Item(index).m_pointerSequence);
				if (element != null)
				{
		...
		{
			return CVersionUpdateSystem.s_androidUtilityJavaClass.CallStatic<int>("GetNetworkType", new object[0]);
		}
