...
public class CMiniPlayerInfoSystem : Singleton<CMiniPlayerInfoSystem>
{
	...
	private void OnAddFriend(CUIEvent uiEvent)
	{
		...
		{
			Singleton<CFriendContoller>.get_instance().Open_Friend_Verify(this.m_CurSelectedUuid, (uint)this.m_CurSelectedLogicWorld, false, 0, -1, true);
		}
	}
	...
	private void OnInvite3v3(CUIEvent uiEvent)
	{
		if (this.m_CurSelectedUuid > 0uL && this.m_CurSelectedLogicWorld > 0)
		{
			...
			uint tagUInt = 0u;
			uint.TryParse(Singleton<CTextManager>.get_instance().GetText("MapID_PVP_3V3"), ref tagUInt);
	...
	private void OnInvite5v5(CUIEvent uiEvent)
	{
		if (this.m_CurSelectedUuid > 0uL && this.m_CurSelectedLogicWorld > 0)
		{
			...
			uint tagUInt = 0u;
			uint.TryParse(Singleton<CTextManager>.get_instance().GetText("MapID_PVP_5V5"), ref tagUInt);
			cUIEvent2.m_eventParams.tagUInt = tagUInt;
			Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(cUIEvent2);
		}
		Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(enUIEventID.Mini_Player_Info_CloseForm);
	...
	private void OnInviteEntertainment(CUIEvent uiEvent)
	{
		if (this.m_CurSelectedUuid > 0uL && this.m_CurSelectedLogicWorld > 0)
		{
			...
			uint tagUInt = 0u;
			uint.TryParse(Singleton<CTextManager>.get_instance().GetText("MapID_PVP_MELEE"), ref tagUInt);
			cUIEvent2.m_eventParams.tagUInt = tagUInt;
			Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(cUIEvent2);
		}
		Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(enUIEventID.Mini_Player_Info_CloseForm);
	...
	{
		string name = Singleton<CFriendContoller>.get_instance().model.GetName(this.m_CurSelectedUuid, (uint)this.m_CurSelectedLogicWorld);
		string strContent = string.Format(Singleton<CTextManager>.get_instance().GetText("Black_BlockTip", new string[]
		{
			name
		}), new object[0]);
