...
namespace Assets.Scripts.GameLogic
{
	public class ActorRoot : PooledClassObject, IUpdateLogic
	{
			...
			set
			{
				...
				if (this._bVisible != value)
				{
					...
					if (this._bVisible)
					{
						...
						MMGame_Math.SetLayer(this.gameObject, "Actor", "Particles", true);
					}
					else
					{
						MMGame_Math.SetLayer(this.gameObject, "Hide", true);
					}
		...
		public void Spawned()
		{
			...
			{
				Singleton<GamePlayerCenter>.get_instance().ConnectActorRootAndPlayer(ref this.SelfPtr);
			}
		}
		...
		public void InitActor()
		{
			...
			Singleton<GameEventSys>.get_instance().SendEvent<PoolObjHandle<ActorRoot>>(GameEventDef.Event_ActorInit, ref selfPtr);
		}
		...
		public void PrepareFight()
		{
			...
			PoolObjHandle<ActorRoot> selfPtr = this.SelfPtr;
			Singleton<GameEventSys>.get_instance().SendEvent<PoolObjHandle<ActorRoot>>(GameEventDef.Event_ActorPreFight, ref selfPtr);
		}
		...
		public void StartFight()
		{
			...
			PoolObjHandle<ActorRoot> selfPtr = this.SelfPtr;
			Singleton<GameEventSys>.get_instance().SendEvent<PoolObjHandle<ActorRoot>>(GameEventDef.Event_ActorStartFight, ref selfPtr);
		}
		...
		public void ReactiveActor(VInt3 bornPos, VInt3 bornDir)
		{
			...
			Singleton<GameEventSys>.get_instance().SendEvent<PoolObjHandle<ActorRoot>>(GameEventDef.Event_ActorInit, ref selfPtr);
			Singleton<GameEventSys>.get_instance().SendEvent<PoolObjHandle<ActorRoot>>(GameEventDef.Event_ActorPreFight, ref selfPtr);
			Singleton<GameObjMgr>.get_instance().AddActor(selfPtr);
			this.StartFight();
		...
		{
			return (this.TheActorMeta.ActorCamp != 1) ? 1 : 2;
		}
		...
		{
			return Singleton<GamePlayerCenter>.get_instance().GetHostPlayer().IsAtMyTeam(ref this.TheActorMeta);
		}
