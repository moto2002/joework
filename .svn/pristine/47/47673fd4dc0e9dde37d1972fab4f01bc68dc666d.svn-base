...
namespace Assets.Scripts.GameSystem
{
	public class CChatModel
	{
		...
		public CChatEntity GetLastUnread_Selected()
		{
			...
			if (list != null && this.index >= 0 && this.index < list.get_Count())
			{
				return list.get_Item(this.index++);
			}
			return null;
		}
		...
		{
			int result = -1;
			int count = this.playerInfos.get_Count();
			for (int i = 0; i < count; i++)
			{
				if (this.playerInfos.get_Item(i).ullUid == info.ullUid)
				{
					result = i;
					break;
				}
		...
		{
			for (int i = 0; i < this.playerInfos.get_Count(); i++)
			{
				COMDT_CHAT_PLAYER_INFO cOMDT_CHAT_PLAYER_INFO = this.playerInfos.get_Item(i);
