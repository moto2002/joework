...
namespace Assets.Scripts.GameLogic
{
	public class SkillComponent : LogicComponent
	{
		...
		{
			int passiveSkillID = this.actorPtr.get_handle().TheStaticData.TheBaseAttribute.PassiveSkillID1;
			int passiveSkillID2 = this.actorPtr.get_handle().TheStaticData.TheBaseAttribute.PassiveSkillID2;
			if (passiveSkillID != 0)
			{
				this.CreateTalent(passiveSkillID);
			}
			if (passiveSkillID2 != 0)
			{
				this.CreateTalent(passiveSkillID2);
			}
		}
		...
		public void InitRandomSkill(int inPassSkillRule)
		{
			...
			{
				DebugHelper.Assert(!Singleton<FrameSynchr>.get_instance().bActive || Singleton<FrameSynchr>.get_instance().isRunning);
				if (Singleton<FrameSynchr>.get_instance().isRunning)
				{
		...
		public void ResetSkillLevel()
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (skillSlot != null && (skillSlot.SlotType == SkillSlotType.SLOT_SKILL_1 || skillSlot.SlotType == SkillSlotType.SLOT_SKILL_2 || skillSlot.SlotType == SkillSlotType.SLOT_SKILL_3))
				{
					...
					{
						Singleton<CBattleSystem>.get_instance().FightForm.ClearSkillLvlStates(i);
					}
				}
			}
			if (captain == this.actorPtr && Singleton<CBattleSystem>.GetInstance().FightForm != null)
			{
				Singleton<CBattleSystem>.get_instance().FightForm.ResetSkillButtonManager(this.actorPtr, false, SkillSlotType.SLOT_SKILL_COUNT);
			}
			this.m_iSkillPoint = 0;
		}
		...
		public void SelectSkillTarget(SkillSlotType slot, Vector2 axis, bool isSkillCursorInCancelArea)
		{
			...
			if (this.TryGetSkillSlot(slot, out skillSlot))
			{
				skillSlot.skillIndicator.SelectSkillTarget(axis, isSkillCursorInCancelArea);
			}
		}
		...
		public void HostCancelUseSkillSlot(SkillSlotType slot, enSkillJoystickMode mode)
		{
			...
			{
				skillSlot.CancelUseSkill();
				if (mode == enSkillJoystickMode.SelectTarget || mode == enSkillJoystickMode.SelectNextSkillTarget)
				{
		...
		{
			if (this.actorPtr.get_handle().TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && ActorHelper.IsHostCtrlActor(ref this.actorPtr))
			{
				for (int i = 0; i < 10; i++)
				{
					SkillSlot skillSlot = this.SkillSlotArray[i];
		...
		public override void UpdateLogic(int nDelta)
		{
			...
			for (int i = 0; i < this.SpawnedBullets.get_Count(); i++)
			{
				BulletSkill bulletSkill = this.SpawnedBullets.get_Item(i);
				if (bulletSkill != null)
				{
		...
		private void RecycleOverLimitBullte(int _bulletTypeId, int _bulletUpperLimit)
		{
			if (_bulletUpperLimit > 0 && _bulletTypeId > 0)
			{
				...
				for (int i = 0; i < this.SpawnedBullets.get_Count(); i++)
				{
					if (this.SpawnedBullets.get_Item(i).BulletTypeId == _bulletTypeId)
					{
						num++;
						if (num == 1)
						{
					...
					{
						this.SpawnedBullets.get_Item(num2).Stop();
						this.SpawnedBullets.get_Item(num2).Release();
						this.SpawnedBullets.RemoveAt(num2);
						break;
					}
				}
			}
		...
		public bool RemoveBuff(PoolObjHandle<ActorRoot> inTargetActor, int inSkillCombineId)
		{
			if (inTargetActor)
			{
				inTargetActor.get_handle().BuffHolderComp.RemoveBuff(inSkillCombineId);
				return true;
			}
			return false;
		}
