...
public class MaterialHurtEffect : ActorComponent
{
	...
	private void Awake()
	{
		this.hurtCurve = (Singleton<CResourceManager>.GetInstance().GetResource("Shaders/Curve/RGBCurve_Hurt.asset", typeof(RGBCurve), 0, false, false).m_content as RGBCurve);
		this.freezeTex = (Singleton<CResourceManager>.GetInstance().GetResource("Prefab_Characters/Z_Other/Textures/Hero_BingDong.tga", typeof(Texture2D), 0, false, false).m_content as Texture2D);
		this.stoneTex = (Singleton<CResourceManager>.GetInstance().GetResource("Prefab_Characters/Z_Other/Textures/Hero_ShiHua.tga", typeof(Texture2D), 0, false, false).m_content as Texture2D);
	}
	...
	private void InitMats()
	{
		...
		if (this.mats != null)
		{
			for (int j = 0; j < this.mats.get_Count(); j++)
			{
				Material material = this.mats.get_Item(j);
	...
	public SMaterialEffect_Base FindEffect(int type)
	{
		...
		{
			if (this.playingEffects.get_Item(i).type == type)
			{
				return this.playingEffects.get_Item(i);
			}
		}
		return null;
	}
