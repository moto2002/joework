...
namespace Assets.Scripts.GameSystem
{
	public class CFriendRecruit
	{
		...
		public class RecruitData
		{
			...
			{
				this._userInfo = null;
				for (int i = 0; i < this.RewardList.get_Count(); i++)
				{
					CFriendRecruit.RecruitReward recruitReward = this.RewardList.get_Item(i);
		...
		public void SetRecruiterRewardBits(uint m_dwRecruiterRewardBits)
		{
			...
			for (int i = 0; i < this.m_beiZhaoMuZhe.RewardList.get_Count(); i++)
			{
				...
				else if (this.m_beiZhaoMuZhe.userInfo != null && this.m_beiZhaoMuZhe.userInfo.dwPvpLvl >= recruitReward.cfg.dwLevel)
				{
		...
		{
			return this.HasGetBeiZhaoMuZheReward(v);
		}
		...
		public void ParseFriend(COMDT_FRIEND_INFO stFriendInfo, COMDT_RECRUITMENT_DATA stRecruitmentInfo)
		{
			...
			{
				COMDT_RECRUITMENT_ACTIVE stRecruitmentActive = stRecruitmentInfo.stRecruitmentInfo.get_stRecruitmentActive();
		...
		public static CFriendRecruit.RecruitReward GetReward(ushort rewardID, ListView<CFriendRecruit.RecruitReward> list)
		{
			...
			{
				CFriendRecruit.RecruitReward recruitReward = list.get_Item(i);
