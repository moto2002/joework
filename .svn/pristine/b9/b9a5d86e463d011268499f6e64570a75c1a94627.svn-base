...
namespace Assets.Scripts.GameSystem
{
	...
	public class IDIPSys : MonoSingleton<IDIPSys>
	{
		...
		public bool RedPotState
		{
			get
			{
				return this.m_IDIPDataList.get_Count() > 0;
			}
		}
		...
		{
			IDIPSys.<GetChannelID>c__Iterator29 <GetChannelID>c__Iterator = new IDIPSys.<GetChannelID>c__Iterator29();
		...
		{
			IDIPSys.<DownloadImage>c__Iterator2A <DownloadImage>c__Iterator2A = new IDIPSys.<DownloadImage>c__Iterator2A();
			<DownloadImage>c__Iterator2A.preUrl = preUrl;
			<DownloadImage>c__Iterator2A.checkDays = checkDays;
			<DownloadImage>c__Iterator2A.callBack = callBack;
			<DownloadImage>c__Iterator2A.<$>preUrl = preUrl;
			<DownloadImage>c__Iterator2A.<$>checkDays = checkDays;
			<DownloadImage>c__Iterator2A.<$>callBack = callBack;
			<DownloadImage>c__Iterator2A.<>f__this = this;
			return <DownloadImage>c__Iterator2A;
		}
		...
		{
			IDIPSys.<DownloadImageByTag>c__Iterator2B <DownloadImageByTag>c__Iterator2B = new IDIPSys.<DownloadImageByTag>c__Iterator2B();
			<DownloadImageByTag>c__Iterator2B.preUrl = preUrl;
			<DownloadImageByTag>c__Iterator2B.tagPath = tagPath;
			<DownloadImageByTag>c__Iterator2B.checkDays = checkDays;
			<DownloadImageByTag>c__Iterator2B.callBack = callBack;
			<DownloadImageByTag>c__Iterator2B.ImageIDx = ImageIDx;
			<DownloadImageByTag>c__Iterator2B.<$>preUrl = preUrl;
			<DownloadImageByTag>c__Iterator2B.<$>tagPath = tagPath;
			<DownloadImageByTag>c__Iterator2B.<$>checkDays = checkDays;
			<DownloadImageByTag>c__Iterator2B.<$>callBack = callBack;
			<DownloadImageByTag>c__Iterator2B.<$>ImageIDx = ImageIDx;
			<DownloadImageByTag>c__Iterator2B.<>f__this = this;
			return <DownloadImageByTag>c__Iterator2B;
		}

		private void ProcessGetNoticeList(CSPkg msg)
		{
			this.m_MyOpenID = Singleton<ApolloHelper>.GetInstance().GetAccountInfo(false).get_OpenId();
			this.m_DataVersion = msg.stPkgData.get_stNoticeListRsp().dwDataVersion;
			ListView<IDIPSys.IDIPData> listView = new ListView<IDIPSys.IDIPData>();
			for (int i = 0; i < (int)msg.stPkgData.get_stNoticeListRsp().wNoticeCnt; i++)
			{
				CSDT_NOTICE_INFO cSDT_NOTICE_INFO = msg.stPkgData.get_stNoticeListRsp().astNoticeList[i];
				IDIPSys.IDIPData iDIPData = null;
				for (int j = 0; j < this.m_IDIPDataList.get_Count(); j++)
				{
					IDIPSys.IDIPData iDIPData2 = this.m_IDIPDataList.get_Item(j);
			...
			if (this.m_IDIPDataList.get_Count() > 0 && Singleton<CLobbySystem>.GetInstance().IsInLobbyForm())
			{
				this.ShowForm();
			}
		}
		...
		{
			for (int i = 0; i < this.m_IDIPDataList.get_Count(); i++)
			{
				IDIPSys.IDIPData iDIPData = this.m_IDIPDataList.get_Item(i);
				if (iDIPData != null && iDIPData.dwNoticeID == msg.stPkgData.get_stNoticeInfoRsp().dwNoticeID)
				{
					iDIPData.content = Utility.UTF8Convert(msg.stPkgData.get_stNoticeInfoRsp().szContent, (int)msg.stPkgData.get_stNoticeInfoRsp().wContentLen);
					this.CheckIsBtnUrl(iDIPData);
					iDIPData.ullNoticeTime = msg.stPkgData.get_stNoticeInfoRsp().ullNoticeTime;
					iDIPData.bLoad = true;
		...
		{
			if (msg.stPkgData.get_stNoticeNewRsp().bHaveNew > 0)
			{
		...
		private void RequestNoticeList()
		{
			...
			cSPkg.stPkgData.get_stNoticeListReq().dwDataVersion = this.m_DataVersion;
			Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		}
		...
		{
			if (idx < this.m_IDIPDataList.get_Count())
			{
				IDIPSys.IDIPData iDIPData = this.m_IDIPDataList.get_Item(idx);
				CSPkg cSPkg = NetworkModule.CreateDefaultCSPKG(1424u);
				cSPkg.stPkgData.get_stNoticeInfoReq().dwLogicWorldID = iDIPData.dwLogicWorldID;
				cSPkg.stPkgData.get_stNoticeInfoReq().dwNoticeID = iDIPData.dwNoticeID;
				cSPkg.stPkgData.get_stNoticeInfoReq().ullNoticeTime = iDIPData.ullNoticeTime;
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
			}
		}
		...
		{
			for (int i = 0; i < (int)msg.stPkgData.get_stAcntSelfMsgInfo().bMsgCnt; i++)
			{
				CSDT_SELFMSG_INFO cSDT_SELFMSG_INFO = msg.stPkgData.get_stAcntSelfMsgInfo().astMsgList[i];
		...
		public void SetBanTimeInfo(COM_ACNT_BANTIME_TYPE kType, uint kBanTime)
		{
			if (kType < 21)
			{
				this.m_BanTimeInfo[kType] = kBanTime;
			}
		}
		...
		{
			for (int i = 0; i < (int)msg.stPkgData.get_stBanTimeChg().bBanTypeNum; i++)
			{
				ushort kType = msg.stPkgData.get_stBanTimeChg().BanType[i];
				MonoSingleton<IDIPSys>.GetInstance().SetBanTimeInfo(kType, msg.stPkgData.get_stBanTimeChg().BanTime[i]);
			}
		}
		...
		public DateTime GetBanTime(COM_ACNT_BANTIME_TYPE kType)
		{
			if (kType < 21)
			{
				return Utility.ToUtcTime2Local((long)((ulong)this.m_BanTimeInfo[kType]));
			}
		...
		{
			DateTime banTime = this.GetBanTime(2);
		...
		{
			DateTime banTime = this.GetBanTime(1);
			DateTime dateTime = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
			if (banTime > dateTime)
			{
		...
		{
			DateTime banTime = this.GetBanTime(16);
			DateTime dateTime = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
			if (banTime > dateTime)
			{
		...
		public void RequestQQBox()
		{
			...
			if (accountInfo != null)
			{
				...
				Utility.StringToByteArray(str, ref cSPkg.stPkgData.get_stGainChestReq().szPf);
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
			}
		}
		...
		{
			if (msg.stPkgData.get_stGainChestRsp().iResult == 0)
			{
				int iActId = msg.stPkgData.get_stGainChestRsp().iActId;
				string text = Utility.UTF8Convert(msg.stPkgData.get_stGainChestRsp().szGainChestId);
				string title = Utility.UTF8Convert(msg.stPkgData.get_stGainChestRsp().szChestTitle, (int)msg.stPkgData.get_stGainChestRsp().wTitleLen);
				string desc = Utility.UTF8Convert(msg.stPkgData.get_stGainChestRsp().szChestContent, (int)msg.stPkgData.get_stGainChestRsp().wContentLen);
				Debug.Log("QQBox  receive srv msg boxid " + text);
				Singleton<ApolloHelper>.GetInstance().ShareQQBox(iActId.ToString(), text, title, desc);
			}
			else
			{
				Debug.Log("QQBox getboxinfo error " + msg.stPkgData.get_stGainChestRsp().iResult);
			}
		}
		...
		{
			ApolloAccountInfo accountInfo = Singleton<ApolloHelper>.GetInstance().GetAccountInfo(false);
			MonoSingleton<CTongCaiSys>.get_instance().OpenTongCaiH5(accountInfo);
		}
		...
		private string MakeXinYueHttp()
		{
			...
			ApolloAccountInfo accountInfo = Singleton<ApolloHelper>.GetInstance().GetAccountInfo(false);
			if (accountInfo != null && (accountInfo.get_Platform() == 2 || accountInfo.get_Platform() == 1))
			{
				int num = 52;
				...
				for (int i = 0; i < accountInfo.get_TokenList().get_Count(); i++)
				{
					if (accountInfo.get_TokenList().get_Item(i).get_Type() == 1)
					{
						text4 = accountInfo.get_TokenList().get_Item(i).get_Value();
						break;
					}
				}
