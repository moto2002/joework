...
namespace Assets.Scripts.GameLogic
{
	...
	public class PassiveHurtCondition : PassiveCondition
	{
		...
		private bool CheckAttackType(ref HurtEventResultInfo info)
		{
			...
			{
				if (info.atker && info.atker.get_handle().TheStaticData.TheHeroOnlyInfo.AttackDistanceType == 1)
				{
					result = this.CheckSkillSlot(ref info);
				}
			}
			else if (this.localParams[0] == 2 && info.atker && info.atker.get_handle().TheStaticData.TheHeroOnlyInfo.AttackDistanceType == 2)
			{
				result = this.CheckSkillSlot(ref info);
			}
			return result;
		...
		private bool CheckDamageRateForHp(ref HurtEventResultInfo info)
		{
			...
			else if (info.src && info.src.get_handle().ValueComponent != null)
			{
				int num2 = -info.hpChanged;
				if (num2 > num * (info.src.get_handle().ValueComponent.actorHp + num2) / 100)
				{
					result = true;
				}
			}
		...
		{
			return !this.sourceActor.get_handle().ActorControl.IsDeadState && this.bHurt;
		}
	}
}
